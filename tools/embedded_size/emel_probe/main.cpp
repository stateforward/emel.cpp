...
constexpr std::string_view k_assistant_generation_prefix =
    "<|im_start|>assistant\n";
...
bool qwen_messages_supported(
    const std::span<const emel::text::formatter::chat_message>
        messages) noexcept {
  if (messages.empty()) {
    return false;
  }
...
bool append_bytes(const std::string_view bytes, char *output,
                  const size_t output_capacity, size_t &offset) noexcept {
...
bool format_supported_qwen_contract(
  ...
  if ((request.output == nullptr && request.output_capacity > 0u) ||
      ...
      !request.add_generation_prompt || request.enable_thinking) {
    if (error_out != nullptr) {
  ...
  for (const auto &message : request.messages) {
    if (!append_bytes(k_im_start, request.output, request.output_capacity,
                      output_length) ||
        !append_bytes(message.role, request.output, request.output_capacity,
                      output_length) ||
        !append_bytes(k_message_separator, request.output,
                      request.output_capacity, output_length) ||
        !append_bytes(message.content, request.output, request.output_capacity,
                      output_length) ||
        !append_bytes(k_im_end, request.output, request.output_capacity,
                      output_length)) {
      if (error_out != nullptr) {
        *error_out = emel::text::formatter::error_code(
            emel::text::formatter::error::invalid_request);
  ...
  if (!append_bytes(k_assistant_generation_prefix, request.output,
                    request.output_capacity, output_length)) {
    if (error_out != nullptr) {
      *error_out = emel::text::formatter::error_code(
          emel::text::formatter::error::invalid_request);
...
bool template_matches_supported_qwen_contract(
...
size_t formatted_capacity_upper_bound(
  ...
  size_t capacity = 0u;
  for (const auto &message : messages) {
    capacity += k_im_start.size() + message.role.size() +
                k_message_separator.size() + message.content.size() +
                k_im_end.size();
  }
...
emel::error::type populate_model_metadata(const emel_fixture &fixture,
  ...
  const auto *architecture_entry =
      find_kv_entry(fixture, "general.architecture");
  if (architecture_entry == nullptr) {
#ifndef NDEBUG
    std::fprintf(stderr,
                 "populate_model_metadata: missing general.architecture\n");
#endif
...
#ifndef NDEBUG
    std::fprintf(stderr,
                 "populate_model_metadata: missing required qwen3 metadata\n");
#endif
    return emel::error::cast(emel::model::loader::error::model_invalid);
  }
  ...
  if (qwen3_key_length <= 0 || qwen3_value_length <= 0) {
    ...
    std::fprintf(
        ...
        qwen3_key_length, qwen3_value_length);
#endif
    return emel::error::cast(emel::model::loader::error::model_invalid);
  }
...
#ifndef NDEBUG
      std::fprintf(stderr,
                   "populate_model_metadata: invalid tokenizer.tokens array\n");
#endif
      return emel::error::cast(emel::model::loader::error::model_invalid);
    }
...
#ifndef NDEBUG
    std::fprintf(stderr,
                 "run_emel_validate_structure: tensors=%u layers=%d weights=%p "
                 "size=%llu\n",
                 req.model_data.n_tensors, req.model_data.n_layers,
...
run_emel_validate_architecture(void *,
  ...
  const emel::error::type err =
      emel::model::validate_execution_contract(req.model_data);
#ifndef NDEBUG
...
#ifndef NDEBUG
    std::fprintf(stderr, "prepare_emel_fixture: prebind gguf failed\n");
#endif
    return false;
  }
...
bool initialize_emel_session(emel_session &session) {
  ...
  if (!format_single_user_prompt(session.formatter, k_prompt,
                                 formatted_prompt)) {
    return false;
  }
...
int main(int argc, char **argv) {
  ...
  volatile size_t sink =
      result.output_length + static_cast<size_t>(result.tokens_generated);
  return sink == 0u ? 1 : 0;
}
