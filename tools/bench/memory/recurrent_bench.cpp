...
using memory_sm = emel::memory::recurrent::sm;
...
struct reference_state {
  ...
  reference_state()
    ...
    memory_bench::must_reference_succeed(memory != nullptr, "create_memory(recurrent)");
...
void append_emel_memory_recurrent_cases(std::vector<result> & results, const config & cfg) {
  ...
  results.push_back(measure_case("memory/recurrent_full", cfg, fn));
}
