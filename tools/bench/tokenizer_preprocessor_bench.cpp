...
std::string make_rwkv_long_text() {
  std::string out;
  out.reserve(2048);
  for (int i = 0; i < 32; ++i) {
    out += " hello ";
...
std::string make_plamo2_long_text() {
  std::string out;
  out.reserve(2048);
  for (int i = 0; i < 32; ++i) {
    out += " hello ";
...
reference_fragments build_reference_spm_fragments(const emel::model::data::vocab & vocab,
                                                  const std::string & text,
  ...
  if (!emel::tokenizer::preprocessor::detail::build_special_tokens(cache, vocab)) {
    std::fprintf(stderr, "error: spm reference special token build failed\n");
    std::abort();
  }
...
reference_fragments build_reference_rwkv_fragments(const emel::model::data::vocab & vocab,
                                                   const std::string & text,
                                                   const bool parse_special) {
  reference_fragments out;
  out.text = text;
  ...
  emel::tokenizer::preprocessor::special_token_cache cache = {};
  if (!emel::tokenizer::preprocessor::detail::build_special_tokens(cache, vocab)) {
    std::fprintf(stderr, "error: rwkv reference special token build failed\n");
    std::abort();
  }
  ...
  if (!emel::tokenizer::preprocessor::detail::partition_with_specials(
          out.text, cache, parse_special, fragments.data(), fragments.size(), &count)) {
    std::fprintf(stderr, "error: rwkv reference partition failed\n");
    std::abort();
  }
...
reference_fragments build_reference_plamo2_fragments(const emel::model::data::vocab & vocab,
                                                     const std::string & text,
                                                     const bool parse_special) {
  reference_fragments out;
  out.text = text;
  ...
  emel::tokenizer::preprocessor::special_token_cache cache = {};
  if (!emel::tokenizer::preprocessor::detail::build_special_tokens(cache, vocab)) {
    std::fprintf(stderr, "error: plamo2 reference special token build failed\n");
    std::abort();
  }
  ...
  if (!emel::tokenizer::preprocessor::detail::partition_with_specials(
          out.text, cache, parse_special, fragments.data(), fragments.size(), &count)) {
    std::fprintf(stderr, "error: plamo2 reference partition failed\n");
    std::abort();
  }
...
void ensure_preprocessor_spm_parity(const emel::model::data::vocab & vocab,
                                    const std::string & text,
                                    const bool parse_special) {
  emel::tokenizer::preprocessor::spm::sm machine{};
  std::array<fragment, k_fragment_capacity> fragments = {};
  size_t count = 0;
  int32_t err = EMEL_OK;
  if (!collect_emel_fragments(machine, vocab, text, parse_special, fragments, count, err) ||
      err != EMEL_OK) {
    std::fprintf(stderr, "error: spm preprocessor failed for parity check: %d\n", err);
    std::abort();
  }

  const reference_fragments reference =
      build_reference_spm_fragments(vocab, text, parse_special);
  if (count != reference.fragments.size()) {
    std::fprintf(stderr,
                 "error: spm preprocessor parity mismatch count %zu vs %zu\n",
                 count,
                 reference.fragments.size());
    std::abort();
  }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (lhs.kind != rhs.kind || lhs.text != rhs.text || lhs.token != rhs.token) {
      std::fprintf(stderr,
                   "error: spm preprocessor parity mismatch at %zu\n",
                   i);
      std::abort();
    }
  }
...
void ensure_preprocessor_rwkv_parity(const emel::model::data::vocab & vocab,
                                     const std::string & text,
                                     const bool parse_special) {
  emel::tokenizer::preprocessor::rwkv::sm machine{};
  std::array<fragment, k_fragment_capacity> fragments = {};
  size_t count = 0;
  int32_t err = EMEL_OK;
  if (!collect_emel_fragments(machine, vocab, text, parse_special, fragments, count, err) ||
      err != EMEL_OK) {
    std::fprintf(stderr, "error: rwkv preprocessor failed for parity check: %d\n", err);
    std::abort();
  }

  const reference_fragments reference =
      build_reference_rwkv_fragments(vocab, text, parse_special);
  if (count != reference.fragments.size()) {
    std::fprintf(stderr,
                 "error: rwkv preprocessor parity mismatch count %zu vs %zu\n",
                 count,
                 reference.fragments.size());
    std::abort();
  }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (lhs.kind != rhs.kind || lhs.text != rhs.text || lhs.token != rhs.token) {
      std::fprintf(stderr,
                   "error: rwkv preprocessor parity mismatch at %zu\n",
                   i);
      std::abort();
    }
  }
...
void ensure_preprocessor_plamo2_parity(const emel::model::data::vocab & vocab,
                                       const std::string & text,
                                       const bool parse_special) {
  emel::tokenizer::preprocessor::plamo2::sm machine{};
  std::array<fragment, k_fragment_capacity> fragments = {};
  size_t count = 0;
  int32_t err = EMEL_OK;
  if (!collect_emel_fragments(machine, vocab, text, parse_special, fragments, count, err) ||
      err != EMEL_OK) {
    std::fprintf(stderr, "error: plamo2 preprocessor failed for parity check: %d\n", err);
    std::abort();
  }

  const reference_fragments reference =
      build_reference_plamo2_fragments(vocab, text, parse_special);
  if (count != reference.fragments.size()) {
    std::fprintf(stderr,
                 "error: plamo2 preprocessor parity mismatch count %zu vs %zu\n",
                 count,
                 reference.fragments.size());
    std::abort();
  }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (lhs.kind != rhs.kind || lhs.text != rhs.text || lhs.token != rhs.token) {
      std::fprintf(stderr,
                   "error: plamo2 preprocessor parity mismatch at %zu\n",
                   i);
      std::abort();
    }
  }
...
void append_emel_tokenizer_preprocessor_cases(std::vector<result> & results,
  ...
  const rwkv_case rwkv_cases[] = {
    {"tokenizer/preprocessor_rwkv_short", "hello A  BBB world", true},
    {"tokenizer/preprocessor_rwkv_long", make_rwkv_long_text(), true},
  };

  for (const auto & entry : rwkv_cases) {
    const auto vocab = make_rwkv_vocab();
    ensure_preprocessor_rwkv_parity(*vocab, entry.text, entry.parse_special);
  ...
  const plamo2_case plamo2_cases[] = {
    {"tokenizer/preprocessor_plamo2_short", "hello A  BBB world", true},
    {"tokenizer/preprocessor_plamo2_long", make_plamo2_long_text(), true},
  };

  for (const auto & entry : plamo2_cases) {
    const auto vocab = make_plamo2_vocab();
    ensure_preprocessor_plamo2_parity(*vocab, entry.text, entry.parse_special);
  ...
  for (const auto & entry : spm_cases) {
    const auto vocab = make_spm_vocab();
    ensure_preprocessor_spm_parity(*vocab, entry.text, entry.parse_special);
...
void append_reference_tokenizer_preprocessor_cases(std::vector<result> & results,
  ...
  for (const auto & entry : rwkv_cases) {
    ...
    auto fn = [&]() {
      const auto fragments =
          build_reference_rwkv_fragments(*vocab, entry.text, entry.parse_special);
      static volatile size_t sink = 0;
      sink += fragments.fragments.size();
      for (const auto & frag : fragments.fragments) {
        sink += frag.text.size();
  ...
  for (const auto & entry : plamo2_cases) {
    ...
    auto fn = [&]() {
      const auto fragments =
          build_reference_plamo2_fragments(*vocab, entry.text, entry.parse_special);
      static volatile size_t sink = 0;
      sink += fragments.fragments.size();
      for (const auto & frag : fragments.fragments) {
        sink += frag.text.size();
  ...
  for (const auto & entry : spm_cases) {
    ...
    auto fn = [&]() {
      const auto fragments =
          build_reference_spm_fragments(*vocab, entry.text, entry.parse_special);
      static volatile size_t sink = 0;
      sink += fragments.fragments.size();
      for (const auto & frag : fragments.fragments) {
        sink += frag.text.size();
