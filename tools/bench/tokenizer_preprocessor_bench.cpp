...
bool collect_emel_fragments(emel::tokenizer::preprocessor::bpe::sm & machine,
...
void ensure_preprocessor_parity(const emel::model::data::vocab & vocab,
  ...
  size_t count = 0;
  int32_t err = EMEL_OK;
  if (!collect_emel_fragments(machine, vocab, text, fragments, count, err) ||
      err != EMEL_OK) {
...
void append_emel_tokenizer_preprocessor_cases(std::vector<result> & results,
  ...
  for (const auto & entry : cases) {
    ...
    auto fn = [&]() {
      (void)collect_emel_fragments(machine, *vocab, entry.text, fragments, count, err);
    };
