...
constexpr generation_case_spec k_bonsai_short_generation_case = {
    .name = emel::bench::k_generation_bonsai_case_name,
    .prompt = "hello",
    .max_tokens = 1,
};

constexpr generation_case_spec k_bonsai_generation_10_case = {
    .name = emel::bench::k_generation_bonsai_10_case_name,
    .prompt = "hello",
    .max_tokens = 10,
};

constexpr generation_case_spec k_bonsai_generation_100_case = {
    .name = emel::bench::k_generation_bonsai_100_case_name,
    .prompt = "hello",
    .max_tokens = 100,
};

constexpr generation_case_spec k_bonsai_generation_1000_case = {
    .name = emel::bench::k_generation_bonsai_1000_case_name,
    .prompt = "hello",
    .max_tokens = 1000,
};
...
constexpr generation_fixture_spec k_bonsai_generation_fixture = {
    .fixture =
        &emel::tools::generation_fixture_registry::k_bonsai_generation_fixture,
    .cases =
        {
            k_bonsai_short_generation_case,
            k_bonsai_generation_10_case,
            k_bonsai_generation_100_case,
            k_bonsai_generation_1000_case,
        },
};

constexpr std::array<generation_fixture_spec, 3> k_generation_compare_fixtures = {
    k_qwen3_generation_fixture,
    k_bonsai_generation_fixture,
    k_lfm2_generation_fixture,
};

constexpr std::array<generation_fixture_spec, 2> k_generation_bench_fixtures = {
    k_qwen3_generation_fixture,
    k_lfm2_generation_fixture,
};
...
std::string_view read_env_string(const char *name) {
  const char *value = std::getenv(name);
  if (value == nullptr || value[0] == '\0') {
    return {};
  }
...
bool fixture_matches_selected_generation_model(
    const emel::tools::generation_fixture_registry::maintained_fixture &fixture) {
  const std::string_view selected_model = read_env_string("EMEL_BENCH_GENERATION_MODEL");
  if (selected_model.empty()) {
    return true;
  }
  if (selected_model == fixture.fixture_rel || selected_model == fixture.name) {
    return true;
  }
  const std::string model_path = resolve_generation_model_path(fixture.fixture_rel);
  return selected_model == model_path;
}

bool generation_case_set_is_smoke() {
  return read_env_string("EMEL_BENCH_GENERATION_CASE_SET") == std::string_view{"smoke"};
}

std::vector<generation_fixture_spec>
filter_generation_fixture_specs(const std::span<const generation_fixture_spec> fixtures,
                                const bool match_reference_lane) {
  std::vector<generation_fixture_spec> selected;
  selected.reserve(fixtures.size());
  for (const generation_fixture_spec &fixture : fixtures) {
    if (match_reference_lane &&
        !fixture_matches_current_reference_lane(*fixture.fixture)) {
      continue;
    }
    if (!fixture_matches_selected_generation_model(*fixture.fixture)) {
      continue;
    }
  ...
  if (selected.empty()) {
    const std::string_view selected_model = read_env_string("EMEL_BENCH_GENERATION_MODEL");
    if (!selected_model.empty()) {
      fail_bench_setup("select_generation_fixture", std::string(selected_model).c_str());
    }
    if (match_reference_lane) {
      fail_bench_setup("select_generation_fixture",
                       "no maintained generation fixtures for compiled reference lane");
    }
  }
  return selected;
}

std::span<const generation_case_spec>
selected_generation_cases(const generation_fixture_spec &fixture) {
  if (generation_case_set_is_smoke()) {
    return std::span<const generation_case_spec>(fixture.cases.data(), 1u);
  }
  return std::span<const generation_case_spec>(fixture.cases.data(), fixture.cases.size());
}

bool generation_case_collects_runtime_evidence(
    const generation_fixture_spec &fixture,
    const generation_case_spec &generation_case) {
  if (fixture.fixture->current_publication &&
      generation_case.name == emel::bench::k_generation_case_name) {
    return true;
  }
  return !read_env_string("EMEL_BENCH_GENERATION_MODEL").empty() &&
         generation_case.max_tokens == 1;
}
...
std::string g_generation_evidence_case_name = {};
...
void reset_generation_flash_evidence() {
  ...
  g_generation_evidence_case_name.clear();
  g_generation_fixture_rel = {};
}
...
run_emel_load_weights(void *owner, const emel::model::loader::event::load &req,
  ...
  if (!fixture.weight_loader.process_event(bind_ev) ||
      !fixture.weight.bind_done || fixture.weight.bind_error) {
  ...
  if (!fixture.weight_loader.process_event(plan_ev) ||
      !fixture.weight.plan_done || fixture.weight.plan_error) {
    return map_weight_loader_error(fixture.weight.err);
  }
  ...
  emel::model::weight_loader::event::apply_effect_results apply_ev{
      std::span<const emel::model::weight_loader::effect_result>{
          fixture.effect_results.data(), effect_count},
  };
  apply_ev.on_done = {&fixture, on_weight_apply_done};
  apply_ev.on_error = {&fixture, on_weight_apply_error};
  if (!fixture.weight_loader.process_event(apply_ev) ||
      !fixture.weight.apply_done || fixture.weight.apply_error) {
    return map_weight_loader_error(fixture.weight.err);
  }
...
std::uint64_t prefill_probe_breakdown_total(
    const prefill_probe_breakdown &breakdown) noexcept {
...
bool measure_subprobe_bool(std::uint64_t &total_bucket_ns,
                           std::uint64_t &sub_bucket_ns, fn_type &&fn) {
  const auto start = steady_clock::now();
  const bool ok = fn();
...
bool bind_emel_prefill_probe_inputs(
...
const char *prefill_contract_name(
  ...
  case emel::generator::prefill_compute_contract::
...
bool resolve_emel_prefill_plan_step(const emel::model::data &model_data,
  ...
  if (prepare_err != emel::error::cast(emel::model::loader::error::none)) {
    return false;
  }
  ...
  const emel::generator::event::generate_run generate_run{request,
                                                          generate_context};

  const bool uses_chunk8 =
      emel::generator::guard::planning_uses_chunk8_prefill{}(generate_run,
                                                             generator_context);
  const bool uses_chunk4 =
      emel::generator::guard::planning_uses_chunk4_prefill{}(generate_run,
                                                             generator_context);
  if (uses_chunk8) {
  ...
  step_size_out =
      uses_chunk4 ? emel::generator::detail::k_prefill_q8_chunk_rows : 1;
  return true;
}
...
bool inspect_emel_prefill_plan(const emel::model::data &model_data,
  ...
  if (!resolve_emel_prefill_plan_step(
          model_data, static_cast<int32_t>(prompt_tokens.size()),
          resolved_step_size)) {
    return false;
  }
  ...
  const auto on_done =
      emel::callback<void(const emel::batch::planner::events::plan_done &)>::
          from<bench_plan_capture, capture_bench_plan_done>(&capture);
  const auto on_error =
      emel::callback<void(const emel::batch::planner::events::plan_error &)>::
          from<bench_plan_capture, capture_bench_plan_error>(&capture);
...
bool inspect_emel_prefill_contract(
    const emel::model::data &model_data, const int32_t prompt_token_count,
    emel::generator::prefill_compute_contract &contract_out) {
  emel::generator::action::context generator_context{};
  generator_context.model = &model_data;
  generator_context.state.selection_mode =
      emel::generator::selection_mode::preselected_argmax;
  if (emel::generator::detail::prepare(generator_context.compute.backend,
                                       model_data) !=
  ...
  if (emel::generator::prefill::guard::flash_runtime_supported{}(
    ...
    if (emel::generator::prefill::guard::
            uses_materialized_logits_with_chunk8_q8_k{}(prefill_run,
                                                        prefill_context)) {
      contract_out = emel::generator::prefill_compute_contract::
          flash_materialized_chunk8_q8_k;
      return true;
    }
    if (emel::generator::prefill::guard::
            uses_materialized_logits_with_chunk4_packed_q8_0{}(
                prefill_run, prefill_context)) {
      contract_out = emel::generator::prefill_compute_contract::
          flash_materialized_chunk4_packed_q8_0;
      return true;
    ...
    if (emel::generator::prefill::guard::
            uses_materialized_logits_with_chunk4_q8_k{}(prefill_run,
                                                        prefill_context)) {
      contract_out = emel::generator::prefill_compute_contract::
          flash_materialized_chunk4_q8_k;
      return true;
    ...
    if (emel::generator::prefill::guard::
            uses_preselected_argmax_with_chunk4_packed_q8_0{}(
                prefill_run, prefill_context)) {
      contract_out = emel::generator::prefill_compute_contract::
          flash_preselected_chunk4_packed_q8_0;
      return true;
    ...
    if (emel::generator::prefill::guard::
            uses_preselected_argmax_with_chunk4_q8_k{}(prefill_run,
                                                       prefill_context)) {
      contract_out = emel::generator::prefill_compute_contract::
  ...
  if (emel::generator::prefill::guard::
          uses_materialized_logits_with_chunk4_packed_q8_0{}(prefill_run,
                                                             prefill_context)) {
    contract_out = emel::generator::prefill_compute_contract::
        nonflash_materialized_chunk4_packed_q8_0;
    return true;
  ...
  if (emel::generator::prefill::guard::
          uses_preselected_argmax_with_chunk4_packed_q8_0{}(prefill_run,
                                                            prefill_context)) {
    contract_out = emel::generator::prefill_compute_contract::
        nonflash_preselected_chunk4_packed_q8_0;
    return true;
...
bool run_emel_runtime_layer_probe_scalar(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position, prefill_probe_breakdown &breakdown) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  if (!measure_probe_bool(breakdown.misc_ns, [&]() {
  ...
  if (block.uses_attention) {
    const bool qkv_shared_packed_q8_0 =
        emel::generator::detail::packed_q8_0_input_path_supported(
            backend, block.attention_q) &&
        emel::generator::detail::packed_q8_0_input_path_supported(
            backend, block.attention_k) &&
        emel::generator::detail::packed_q8_0_input_path_supported(
            backend, block.attention_v);
    const bool qkv_shared_q8 =
        emel::generator::detail::q8_input_path_supported(backend,
                                                         block.attention_q) &&
        emel::generator::detail::q8_input_path_supported(backend,
                                                         block.attention_k) &&
        emel::generator::detail::q8_input_path_supported(backend,
    ...
    if (qkv_shared_packed_q8_0) {
      if (!measure_probe_bool(breakdown.linear_ns, [&]() {
            return emel::generator::detail::prepare_packed_q8_0_input(
                       backend, backend.norm) &&
                   emel::generator::detail::
                       matmul_vector_prepared_packed_q8_0_input(
                           backend, block.attention_q, block.attention_q.cols,
                           backend.q) &&
                   emel::generator::detail::
                       matmul_vector_prepared_packed_q8_0_input(
                           backend, block.attention_k, block.attention_k.cols,
                           backend.k) &&
                   emel::generator::detail::
                       matmul_vector_prepared_packed_q8_0_input(
                           backend, block.attention_v, block.attention_v.cols,
                           backend.v);
          })) {
        return false;
      }
      ...
      if (!measure_probe_bool(breakdown.linear_ns, [&]() {
            return emel::generator::detail::quantize_vector_q8_k(backend.norm,
                                                                 q8_input) &&
                   emel::generator::detail::matmul_vector_q8_input(
                       backend, block.attention_q, q8_input,
                       block.attention_q.cols, backend.q) &&
                   emel::generator::detail::matmul_vector_q8_input(
                       backend, block.attention_k, q8_input,
                       block.attention_k.cols, backend.k) &&
                   emel::generator::detail::matmul_vector_q8_input(
                       backend, block.attention_v, q8_input,
                       block.attention_v.cols, backend.v);
          })) {
        return false;
      }
    } else if (!measure_probe_bool(breakdown.linear_ns, [&]() {
    ...
    if (emel::generator::detail::requires_attention_qk_norm(backend, block) &&
        !measure_probe_bool(breakdown.misc_ns, [&]() {
          return emel::generator::detail::apply_attention_qk_norm(backend,
                                                                  block);
        })) {
      return false;
    }

    measure_probe_void(breakdown.misc_ns, [&]() {
    ...
    if (!measure_probe_bool(
        ...
        !measure_probe_bool(breakdown.linear_ns, [&]() {
          return emel::generator::detail::matmul_vector(
  ...
  } else if (!measure_probe_bool(breakdown.misc_ns, [&]() {
  ...
  const bool gate_up_shared_packed_q8_0 =
      emel::generator::detail::packed_q8_0_input_path_supported(
          backend, block.feed_forward_gate) &&
      emel::generator::detail::packed_q8_0_input_path_supported(
          backend, block.feed_forward_up);
  const bool gate_up_shared_q8 =
      emel::generator::detail::q8_input_path_supported(
          backend, block.feed_forward_gate) &&
      emel::generator::detail::q8_input_path_supported(backend,
  ...
  if (gate_up_shared_packed_q8_0) {
    ...
    if (!measure_probe_bool(breakdown.linear_ns, [&]() {
          return emel::generator::detail::quantize_vector_q8_k(backend.norm,
                                                               q8_input) &&
                 emel::generator::detail::matmul_vector_q8_input(
                     backend, block.feed_forward_gate, q8_input,
                     block.feed_forward_gate.cols, backend.gate) &&
                 emel::generator::detail::matmul_vector_q8_input(
                     backend, block.feed_forward_up, q8_input,
                     block.feed_forward_up.cols, backend.up);
        })) {
      return false;
    }
               ...
               return emel::generator::detail::matmul_vector(
                          ...
                          backend.gate) &&
                      emel::generator::detail::matmul_vector(
                          backend, block.feed_forward_up, backend.norm,
                          backend.up);
             })) {
    return false;
  }
...
template <emel::generator::attention_mode mode,
          emel::generator::detail::chunk4_rhs_route route>
bool run_emel_runtime_layer_probe_chunk4(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const size_t token_base, prefill_probe_breakdown &breakdown) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  ...
  if (!measure_subprobe_bool(
          breakdown.misc_ns, breakdown.misc_attention_norm_ns, [&]() {
            return emel::generator::detail::rms_norm_chunk4(
                backend.hidden_chunk4, backend.n_embd, block.attention_norm,
                backend.rms_epsilon, backend.norm_chunk4);
          })) {
    return false;
  }
  if (block.uses_attention) {
    if (!measure_probe_bool(breakdown.linear_ns, [&]() {
          return emel::generator::detail::prepare_chunk4_rhs<route>(
                 ...
                 emel::generator::detail::matmul_chunk4_prepared<route>(
                     backend, block.attention_q, backend.n_embd,
                     backend.q_chunk4) &&
                 emel::generator::detail::matmul_chunk4_prepared<route>(
                     backend, block.attention_k, backend.n_embd,
                     backend.k_chunk4) &&
                 emel::generator::detail::matmul_chunk4_prepared<route>(
                     backend, block.attention_v, backend.n_embd,
                     backend.v_chunk4);
        })) {
      return false;
    }

    const bool qk_norm_runtime =
        emel::generator::detail::requires_attention_qk_norm(backend, block);
    for (int32_t row = 0;
         row < emel::generator::detail::k_prefill_q8_chunk_rows; ++row) {
      ...
      if (qk_norm_runtime &&
          !measure_subprobe_bool(
              breakdown.misc_ns, breakdown.misc_qk_norm_ns, [&]() {
                return emel::generator::detail::apply_headwise_rms_norm(
                           q_row, block.attention_q_norm, backend.n_head,
                           backend.head_dim, backend.rms_epsilon) &&
                       emel::generator::detail::apply_headwise_rms_norm(
                           k_row, block.attention_k_norm, backend.n_head_kv,
                           backend.head_dim_kv, backend.rms_epsilon);
              })) {
        return false;
      }
      measure_subprobe_void(breakdown.misc_ns, breakdown.misc_rope_ns, [&]() {
        emel::generator::detail::apply_rope(
            q_row, backend.n_head, backend.head_dim, backend.n_rot, position,
            backend.rope_freq_base, backend.rope_type, backend.rope_freq_scale,
            backend.rope_context_original, backend.rope_ext_factor,
            backend.rope_attn_factor, backend.rope_beta_fast,
            backend.rope_beta_slow);
        emel::generator::detail::apply_rope(
            k_row, backend.n_head_kv, backend.head_dim_kv, backend.n_rot,
            position, backend.rope_freq_base, backend.rope_type,
            backend.rope_freq_scale, backend.rope_context_original,
            backend.rope_ext_factor, backend.rope_attn_factor,
            backend.rope_beta_fast, backend.rope_beta_slow);
      ...
      if (!measure_subprobe_bool(
              breakdown.misc_ns, breakdown.misc_kv_store_ns,
              [&]() {
                return emel::generator::detail::store_attention_kv_cache(
                    backend, layer_index, position, k_row, v_row);
              }) ||
          !measure_probe_bool(breakdown.attention_ns, [&]() {
      ...
      measure_subprobe_void(
          breakdown.misc_ns, breakdown.misc_ctx_copy_ns, [&]() {
    ...
    if (!measure_probe_bool(breakdown.linear_ns, [&]() {
          return emel::generator::detail::prepare_chunk4_rhs<route>(
                 ...
                 emel::generator::detail::matmul_chunk4_prepared<route>(
                     backend, block.attention_output, q_dim,
                     backend.projected_chunk4) &&
                 emel::generator::detail::add_chunk4_rows_in_place(
                     backend.hidden_chunk4, backend.projected_chunk4,
                     backend.n_embd);
        })) {
      return false;
    }
    ...
    if (!measure_subprobe_bool(
            breakdown.linear_ns, breakdown.shortconv_in_proj_prepare_ns,
            [&]() {
              return emel::generator::detail::prepare_chunk4_rhs<route>(
                  backend, backend.norm_chunk4, backend.n_embd);
            }) ||
        !measure_subprobe_bool(
            breakdown.linear_ns, breakdown.shortconv_in_proj_ns, [&]() {
              return emel::generator::detail::matmul_chunk4_prepared<route>(
                  backend, block.shortconv_in_proj, backend.n_embd,
    ...
    const size_t layer_offset =
        emel::generator::detail::shortconv_state_layer_offset(backend,
                                                              layer_index);
    ...
    for (int32_t row = 0;
      ...
      measure_subprobe_void(
          breakdown.misc_ns, breakdown.shortconv_conv_ns, [&]() {
            for (int32_t idx = 0; idx < backend.n_embd; ++idx) {
              ...
              float conv_sum =
                  bx *
                  kernel[static_cast<size_t>(backend.shortconv_state_size)];
      ...
      measure_subprobe_void(
          breakdown.misc_ns, breakdown.shortconv_state_shift_ns, [&]() {
    ...
    if (!measure_subprobe_bool(
            breakdown.linear_ns, breakdown.shortconv_out_proj_prepare_ns,
            [&]() {
              return emel::generator::detail::prepare_chunk4_rhs<route>(
                  backend, backend.shortconv_conv_out_chunk4, backend.n_embd);
            }) ||
        !measure_subprobe_bool(
            breakdown.linear_ns, breakdown.shortconv_out_proj_ns, [&]() {
              return emel::generator::detail::matmul_chunk4_prepared<route>(
                         backend, block.shortconv_out_proj, backend.n_embd,
                         backend.projected_chunk4) &&
                     emel::generator::detail::add_chunk4_rows_in_place(
                         backend.hidden_chunk4, backend.projected_chunk4,
                         backend.n_embd);
  ...
  if (!measure_subprobe_bool(
          breakdown.misc_ns, breakdown.misc_ffn_norm_ns, [&]() {
            return emel::generator::detail::rms_norm_chunk4(
                backend.hidden_chunk4, backend.n_embd, block.feed_forward_norm,
                backend.rms_epsilon, backend.norm_chunk4);
          })) {
    return false;
  }
  ...
  if (!measure_probe_bool(breakdown.linear_ns, [&]() {
        return emel::generator::detail::prepare_chunk4_rhs<route>(
                   backend, backend.norm_chunk4, backend.n_embd) &&
               emel::generator::detail::matmul_chunk4_prepared<route>(
                   backend, block.feed_forward_gate, backend.n_embd,
                   backend.gate_chunk4) &&
               emel::generator::detail::matmul_chunk4_prepared<route>(
                   backend, block.feed_forward_up, backend.n_embd,
                   backend.up_chunk4);
      })) {
    return false;
  }

  if (!measure_subprobe_bool(breakdown.misc_ns, breakdown.misc_silu_ns, [&]() {
        return emel::generator::detail::apply_silu_mul_chunk4(
            backend.gate_chunk4, backend.up_chunk4, ffn_dim,
  ...
  if (!measure_probe_bool(breakdown.linear_ns, [&]() {
        return emel::generator::detail::prepare_chunk4_rhs<route>(
               ...
               emel::generator::detail::matmul_chunk4_prepared<route>(
                   backend, block.feed_forward_down, ffn_dim,
                   backend.projected_chunk4) &&
               emel::generator::detail::add_chunk4_rows_in_place(
                   backend.hidden_chunk4, backend.projected_chunk4,
                   backend.n_embd);
...
template <emel::generator::attention_mode mode>
bool run_emel_runtime_layer_probe_chunk8(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const size_t token_base, prefill_probe_breakdown &breakdown) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  const int32_t q_dim = backend.n_head * backend.head_dim;
  ...
  if (!measure_subprobe_bool(
          breakdown.misc_ns, breakdown.misc_attention_norm_ns, [&]() {
            return emel::generator::detail::rms_norm_chunk8(
                backend.hidden_chunk8, backend.n_embd, block.attention_norm,
                backend.rms_epsilon, backend.norm_chunk8);
          })) {
    return false;
  }
  if (block.uses_attention) {
    ...
    const bool qk_norm_runtime =
        emel::generator::detail::requires_attention_qk_norm(backend, block);
    for (int32_t row = 0;
         row < emel::generator::detail::k_prefill_q8_chunk8_rows; ++row) {
      const int32_t position =
          backend.bound_positions[token_base + static_cast<size_t>(row)];
    ...
    if (!measure_probe_bool(breakdown.linear_ns, [&]() {
          return emel::generator::detail::prepare_q8_chunk8_input(
                 ...
                 emel::generator::detail::add_chunk8_rows_in_place(
                     backend.hidden_chunk8, backend.projected_chunk8,
                     backend.n_embd);
        })) {
      return false;
    }
    ...
    if (backend.shortconv_kernel_size <= 0 ||
        ...
        backend.shortconv_bcx_chunk8.size() !=
            static_cast<size_t>(
                emel::generator::detail::k_prefill_q8_chunk8_rows) *
                static_cast<size_t>(3 * backend.n_embd) ||
        backend.shortconv_bx.size() != static_cast<size_t>(backend.n_embd) ||
  ...
  if (!measure_subprobe_bool(
          breakdown.misc_ns, breakdown.misc_ffn_norm_ns, [&]() {
            return emel::generator::detail::rms_norm_chunk8(
                backend.hidden_chunk8, backend.n_embd, block.feed_forward_norm,
                backend.rms_epsilon, backend.norm_chunk8);
          })) {
    return false;
  }
  ...
  if (!measure_subprobe_bool(breakdown.misc_ns, breakdown.misc_silu_ns, [&]() {
        return emel::generator::detail::apply_silu_mul_chunk8(
            backend.gate_chunk8, backend.up_chunk8, ffn_dim,
...
template <emel::generator::attention_mode mode>
bool run_emel_prefill_probe_scalar(
    emel::generator::detail::native_backend &backend, int32_t &selected_index,
    float &selected_score, prefill_probe_breakdown &breakdown) {
  backend.kv_cache_tokens = 0;
  measure_probe_void(breakdown.misc_ns, [&]() {
    emel::generator::detail::reset_shortconv_cache(backend);
  });
  for (size_t token_index = 0;
       token_index < static_cast<size_t>(backend.bound_token_count);
       ++token_index) {
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_emel_runtime_layer_probe_scalar<mode>(backend, layer, position,
                                                     breakdown)) {
        return false;
      }
    }
  ...
  return measure_probe_bool(breakdown.linear_ns, [&]() {
...
template <emel::generator::attention_mode mode,
          emel::generator::detail::chunk4_rhs_route route>
bool run_emel_prefill_probe_chunk4(
    emel::generator::detail::native_backend &backend, int32_t &selected_index,
    float &selected_score, prefill_probe_breakdown &breakdown) {
  backend.kv_cache_tokens = 0;
  measure_probe_void(breakdown.misc_ns, [&]() {
  ...
  for (size_t token_base = 0; token_base < chunk_limit;
       token_base += chunk_rows) {
    for (int32_t row = 0;
         row < emel::generator::detail::k_prefill_q8_chunk_rows; ++row) {
      ...
      if (!measure_probe_bool(breakdown.misc_ns, [&]() {
            return emel::generator::detail::copy_tensor_row(
                *backend.token_embedding.tensor, token_id,
                emel::generator::detail::chunk4_row_span<float>(
                    std::span<float>(backend.hidden_chunk4), row,
                    backend.n_embd));
          })) {
        return false;
      }
    ...
    std::copy(emel::generator::detail::chunk4_row_span<const float>(
  ...
  for (size_t token_index = chunk_limit; token_index < token_count;
       ++token_index) {
    const int32_t token_id = backend.bound_tokens[token_index];
    const int32_t position = backend.bound_positions[token_index];
    if (token_id < 0 || token_id >= backend.token_embedding.rows ||
...
template <emel::generator::attention_mode mode>
bool run_emel_prefill_probe_chunk8(
    emel::generator::detail::native_backend &backend, int32_t &selected_index,
    float &selected_score, prefill_probe_breakdown &breakdown) {
  backend.kv_cache_tokens = 0;
  measure_probe_void(breakdown.misc_ns, [&]() {
  ...
  for (size_t token_base = 0; token_base < chunk_limit;
    ...
    for (int32_t row = 0;
      ...
      if (!measure_probe_bool(breakdown.misc_ns, [&]() {
            return emel::generator::detail::copy_tensor_row(
                *backend.token_embedding.tensor, token_id,
                emel::generator::detail::chunk8_row_span<float>(
                    std::span<float>(backend.hidden_chunk8), row,
                    backend.n_embd));
          })) {
        return false;
      }
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_emel_runtime_layer_probe_chunk8<mode>(backend, layer, token_base,
                                                     breakdown)) {
        return false;
      }
    }

    std::copy(emel::generator::detail::chunk8_row_span<const float>(
...
bool measure_emel_prefill_probe(
    emel::generator::detail::native_backend &backend,
    const std::vector<int32_t> &prompt_tokens,
    const emel::generator::prefill_compute_contract contract,
    prefill_probe_breakdown &breakdown_out, int32_t &selected_index_out) {
  ...
  switch (contract) {
  case emel::generator::prefill_compute_contract::flash_preselected_chunk8_q8_k:
    return run_emel_prefill_probe_chunk8<
        emel::generator::attention_mode::flash>(backend, selected_index_out,
                                                selected_score, breakdown_out);
  case emel::generator::prefill_compute_contract::
      flash_preselected_chunk4_packed_q8_0:
  ...
  case emel::generator::prefill_compute_contract::flash_preselected_chunk4_q8_k:
    return run_emel_prefill_probe_chunk4<
        ...
        backend, selected_index_out, selected_score, breakdown_out);
  case emel::generator::prefill_compute_contract::flash_preselected_scalar:
    return run_emel_prefill_probe_scalar<
        emel::generator::attention_mode::flash>(backend, selected_index_out,
                                                selected_score, breakdown_out);
  case emel::generator::prefill_compute_contract::
      nonflash_preselected_chunk8_q8_k:
...
bool run_emel_runtime_layer(emel::generator::detail::native_backend &backend,
  ...
  return emel::generator::detail::flash_attention_supported(backend, position)
             ? emel::generator::detail::run_layer_flash(backend, layer_index,
                                                        position)
             : emel::generator::detail::run_layer_nonflash(backend, layer_index,
                                                           position);
}
...
bool measure_emel_stage_probe(emel_session &session,
  ...
  if (!inspect_emel_prefill_plan(session.model_data, prompt_tokens,
                                 probe_out.emel_prefill_step_size)) {
    return false;
  }
  emel::generator::prefill_compute_contract contract =
      emel::generator::prefill_compute_contract::none;
  if (!inspect_emel_prefill_contract(session.model_data,
                                     probe_out.emel_prompt_tokens, contract)) {
    return false;
  }
  ...
  generation_result result_out = {};
  int32_t selected_token = std::numeric_limits<int32_t>::min();
  const auto prefill_start = steady_clock::now();
  prefill_probe_breakdown prefill_breakdown = {};
  if (!measure_emel_prefill_probe(backend, prompt_tokens, contract,
                                  prefill_breakdown, selected_token)) {
    return false;
  }
  if (spec.max_tokens == 1) {
    ...
    probe_out.emel_prefill_misc_attention_norm_ns =
        prefill_breakdown.misc_attention_norm_ns;
    ...
    probe_out.emel_prefill_misc_kv_store_ns =
        prefill_breakdown.misc_kv_store_ns;
    probe_out.emel_prefill_misc_ctx_copy_ns =
        prefill_breakdown.misc_ctx_copy_ns;
    probe_out.emel_prefill_misc_shortconv_ns =
        prefill_breakdown.misc_shortconv_ns;
    probe_out.emel_prefill_shortconv_in_proj_ns =
        prefill_breakdown.shortconv_in_proj_ns;
    probe_out.emel_prefill_shortconv_in_proj_prepare_ns =
        prefill_breakdown.shortconv_in_proj_prepare_ns;
    probe_out.emel_prefill_shortconv_conv_ns =
        prefill_breakdown.shortconv_conv_ns;
    probe_out.emel_prefill_shortconv_state_shift_ns =
        prefill_breakdown.shortconv_state_shift_ns;
    probe_out.emel_prefill_shortconv_out_proj_ns =
        prefill_breakdown.shortconv_out_proj_ns;
    probe_out.emel_prefill_shortconv_out_proj_prepare_ns =
        prefill_breakdown.shortconv_out_proj_prepare_ns;
    probe_out.emel_prefill_misc_ffn_norm_ns =
        prefill_breakdown.misc_ffn_norm_ns;
...
maintained_emel_generation_fixtures() {
  static const std::vector<prepared_emel_generation_fixture> fixtures = [] {
    const auto selected_fixtures = filter_generation_fixture_specs(
        std::span<const generation_fixture_spec>(k_generation_bench_fixtures.data(),
                                                 k_generation_bench_fixtures.size()),
        false);
    std::vector<prepared_emel_generation_fixture> prepared_fixtures(
        selected_fixtures.size());
    for (size_t fixture_index = 0u;
         fixture_index < selected_fixtures.size(); ++fixture_index) {
      prepare_emel_generation_fixture(selected_fixtures[fixture_index],
                                      prepared_fixtures[fixture_index]);
    }
      ...
      [] {
        ...
        for (size_t fixture_index = 0u;
          ...
          prepare_reference_generation_fixture(
              selected_fixtures[fixture_index],
              prepared_fixtures[fixture_index]);
        }
        return prepared_fixtures;
      }();
...
maintained_compare_generation_fixtures() {
  static const std::vector<prepared_generation_fixture> fixtures = [] {
    const auto selected_fixtures = filter_generation_fixture_specs(
        std::span<const generation_fixture_spec>(k_generation_compare_fixtures.data(),
                                                 k_generation_compare_fixtures.size()),
        true);
    std::vector<prepared_generation_fixture> prepared_fixtures(
        selected_fixtures.size());
    for (size_t fixture_index = 0u;
         fixture_index < selected_fixtures.size(); ++fixture_index) {
      prepare_compare_generation_fixture(selected_fixtures[fixture_index],
                                         prepared_fixtures[fixture_index]);
    }
    return prepared_fixtures;
  }();
...
std::string_view generation_evidence_case_name() noexcept {
  return g_generation_evidence_case_name;
}
  ...
  if (compare_lane) {
    ...
    for (size_t fixture_index = 0u; fixture_index < fixtures.size();
      ...
      for (const generation_case_spec &generation_case :
           selected_generation_cases(*spec)) {
        ...
        auto fn = [&]() {
          ...
          flash_dispatch_calls =
              flash_dispatch_calls_after - flash_dispatch_calls_before;
          optimized_flash_dispatch_calls =
              optimized_flash_dispatch_calls_after -
              optimized_flash_dispatch_calls_before;
          shared_flash_dispatch_calls = shared_flash_dispatch_calls_after -
                                        shared_flash_dispatch_calls_before;
          native_q8_0_dispatch_calls = native_q8_0_dispatch_calls_after -
                                       native_q8_0_dispatch_calls_before;
          packed_q8_0_dispatch_calls = packed_q8_0_dispatch_calls_after -
                                       packed_q8_0_dispatch_calls_before;
          optimized_q2_dispatch_calls = optimized_q2_dispatch_calls_after -
                                        optimized_q2_dispatch_calls_before;
          shared_q2_dispatch_calls =
              shared_q2_dispatch_calls_after - shared_q2_dispatch_calls_before;
          optimized_q3_dispatch_calls = optimized_q3_dispatch_calls_after -
                                        optimized_q3_dispatch_calls_before;
          shared_q3_dispatch_calls =
              shared_q3_dispatch_calls_after - shared_q3_dispatch_calls_before;
          optimized_q4_dispatch_calls = optimized_q4_dispatch_calls_after -
                                        optimized_q4_dispatch_calls_before;
          shared_q4_dispatch_calls =
              shared_q4_dispatch_calls_after - shared_q4_dispatch_calls_before;
          optimized_q6_dispatch_calls = optimized_q6_dispatch_calls_after -
                                        optimized_q6_dispatch_calls_before;
        ...
        if (generation_case_collects_runtime_evidence(*spec, generation_case)) {
          g_generation_evidence_case_name.assign(generation_case.name);
  ...
  for (const prepared_emel_generation_fixture &prepared_fixture : fixtures) {
    ...
    for (const generation_case_spec &generation_case :
      ...
      auto fn = [&]() {
        ...
        flash_dispatch_calls =
            flash_dispatch_calls_after - flash_dispatch_calls_before;
        optimized_flash_dispatch_calls = optimized_flash_dispatch_calls_after -
                                         optimized_flash_dispatch_calls_before;
        shared_flash_dispatch_calls = shared_flash_dispatch_calls_after -
                                      shared_flash_dispatch_calls_before;
        native_q8_0_dispatch_calls = native_q8_0_dispatch_calls_after -
