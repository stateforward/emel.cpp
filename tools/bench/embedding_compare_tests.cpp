...
TEST_CASE("embedding compare records malformed jsonl lines in the summary") {
  const std::filesystem::path tmp_dir =
    std::filesystem::temp_directory_path() / "emel-embedding-compare-tests" / "malformed-jsonl";
  std::filesystem::create_directories(tmp_dir);
  const std::filesystem::path emel_jsonl = tmp_dir / "emel.jsonl";
  const std::filesystem::path reference_jsonl = tmp_dir / "reference.jsonl";
  const std::filesystem::path output_dir = tmp_dir / "out";
  ...
  write_text_file(emel_jsonl, "{\"schema\":\"embedding_compare/v1\",\"record_type\":\"result\"\n");
  write_text_file(reference_jsonl, "");
  ...
  CHECK(summary.find("\"comparison_status\": \"error\"") != std::string::npos);
  CHECK(summary.find("\"reason\": \"emel_lane_error\"") != std::string::npos);
  CHECK(summary.find("\"error_kind\": \"malformed_jsonl\"") != std::string::npos);
  CHECK(summary.find(emel_jsonl.string()) != std::string::npos);
  CHECK(capture.stdout_text.find("backend status=error reason=emel_lane_error") !=
        std::string::npos);
}
