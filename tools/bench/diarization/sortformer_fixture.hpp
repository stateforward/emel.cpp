...
inline void on_bind_done(void *owner,
                         const emel::gguf::loader::events::bind_done &) noexcept {
  auto &fixture = *static_cast<model_fixture *>(owner);
...
inline bool try_parse_sortformer_layer_index(const std::string_view name,
                                             ...
                                             int32_t &layer_index_out) noexcept {
...
inline emel::error::type run_emel_parse_model(
...
inline bool prepare(model_fixture &fixture) {
  fixture.ready = false;
  fixture.contract = {};
  if (emel::io::source::load_file_bytes(
          resolve_repo_path(k_model_rel_path).string(), fixture.file_bytes) !=
...
inline bool prepare(pcm_fixture &fixture) {
  ...
  std::vector<uint8_t> file_bytes = {};
  if (emel::io::source::load_file_bytes(
          resolve_repo_path(k_audio_rel_path).string(), file_bytes) !=
          emel::error::cast(emel::io::read::error::none) ||
      file_bytes.size() < 12u) {
    return false;
  }
...
inline bool prepare(expected_output_baseline &baseline) {
  ...
  std::vector<uint8_t> file_bytes = {};
  if (emel::io::source::load_file_bytes(
          resolve_repo_path(k_baseline_rel_path).string(), file_bytes) !=
          emel::error::cast(emel::io::read::error::none) ||
      file_bytes.empty()) {
    return false;
  }
