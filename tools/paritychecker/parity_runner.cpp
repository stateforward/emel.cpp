...
extern "C" {
void ggml_vec_dot_f16(int n, float *s, size_t bs, ggml_fp16_t *x, size_t bx,
                      ggml_fp16_t *y, size_t by, int nrc);
reference_ggml_float ggml_vec_soft_max_f32(int n, float *y, const float *x,
                                           float max);
void ggml_vec_dot_q2_K_q8_K(int n, float *s, size_t bs, const void *vx,
                            size_t bx, const void *vy, size_t by, int nrc);
void ggml_vec_dot_q3_K_q8_K(int n, float *s, size_t bs, const void *vx,
                            size_t bx, const void *vy, size_t by, int nrc);
void ggml_vec_dot_q6_K_q8_K(int n, float *s, size_t bs, const void *vx,
                            size_t bx, const void *vy, size_t by, int nrc);
void ggml_vec_dot_q2_K_q8_K_generic(int n, float *s, size_t bs, const void *vx,
                                    size_t bx, const void *vy, size_t by,
                                    int nrc);
...
constexpr std::string_view k_generation_baseline_source =
    "maintained_generation_baseline";
constexpr std::string_view k_generation_baseline_format =
    "emel_generation_baseline_v1";
...
constexpr std::string_view k_reference_impl_repository =
...
bool fixture_matches_current_reference_lane(
    const maintained_generation_fixture &fixture) {
  if (fixture.reference_repository != k_reference_impl_repository) {
    return false;
  }
  if (!fixture.reference_ref.empty() &&
      fixture.reference_ref != k_reference_impl_ref) {
    return false;
  }
  return true;
}

bool fixture_uses_append_only_baseline(
    const maintained_generation_fixture &fixture) {
  return fixture.generation_parity_contract ==
         emel::tools::generation_fixture_registry::
             k_generation_parity_contract_append_only_baseline;
}

bool fixture_uses_live_reference_generation(
    const maintained_generation_fixture &fixture) {
  return fixture.generation_parity_contract ==
         emel::tools::generation_fixture_registry::
...
std::filesystem::path
normalize_fixture_path(const std::filesystem::path &path) {
  std::error_code ec;
...
find_generation_fixture(const std::string &model_path) {
  ...
  for (const auto &fixture : emel::tools::generation_fixture_registry::
           k_supported_generation_parity_fixtures) {
...
bool run_emel_gbnf_parse(std::string_view grammar_text,
                         emel::gbnf::grammar &grammar_out, int32_t &err_out) {
  parser_done_capture done_capture{};
  parser_error_capture error_capture{};

  emel::callback<bool(const emel::gbnf::rule_parser::events::parsing_done &)>
      done_cb{&done_capture, on_gbnf_done};
  emel::callback<bool(const emel::gbnf::rule_parser::events::parsing_error &)>
...
bool run_llama_gbnf_parse(std::string_view grammar_text,
                          llama_grammar_rules &rules_out) {
...
quantized_contract_summary build_quantized_contract_summary(
  ...
  for (const auto &stage : audit.stages) {
    ...
    summary.approved_dense_f32_by_contract += static_cast<uint32_t>(
        stage.contract == emel::model::llama::detail::quantized_contract_kind::
...
bool run_emel_jinja_parse(std::string_view template_text,
                          ...
                          int32_t &parse_err_out, size_t &parse_error_pos_out) {
  ...
  const emel::text::jinja::event::parse::done_callback done_cb{
      &capture, on_jinja_parse_done};
  const emel::text::jinja::event::parse::error_callback error_cb{
      &capture, on_jinja_parse_error};
  const emel::text::jinja::event::parse parse_ev{
      template_text, program_out,   done_cb,
      error_cb,      parse_err_out, parse_error_pos_out,
  };
  const bool accepted = parser.process_event(parse_ev);
...
bool run_emel_jinja_render(const emel::text::jinja::program &program,
  ...
  size_t output_len = 0;
  int32_t render_err =
      static_cast<int32_t>(emel::text::jinja::formatter::error::none);
  size_t render_error_pos = 0;
  const emel::text::jinja::event::render::done_callback done_cb{
      &capture, on_jinja_render_done};
  const emel::text::jinja::event::render::error_callback error_cb{
      &capture, on_jinja_render_error};
  const emel::text::jinja::event::render render_ev{
      program, template_text, output_buffer[0],  output_buffer.size(),
      done_cb, error_cb,      nullptr,           &output_len,
      nullptr, &render_err,   &render_error_pos,
  };
...
bool compare_grammars(const emel::gbnf::grammar &emel_grammar,
  ...
  if (emel_grammar.rule_count != llama_rules.size()) {
    std::fprintf(stderr, "rule count mismatch: emel=%u llama=%zu\n",
                 emel_grammar.rule_count, llama_rules.size());
    return false;
  }
  ...
  for (uint32_t rule_id = 0; rule_id < emel_grammar.rule_count; ++rule_id) {
    ...
    if (emel_rule.length != llama_len) {
      std::fprintf(stderr,
                   ...
                   rule_id, emel_rule.length, llama_len);
      return false;
    }
    for (uint32_t i = 0; i < emel_rule.length; ++i) {
      ...
      if (emel_type != llama_type || emel_elem.value != llama_elem.value) {
        std::fprintf(stderr,
                     ...
                     rule_id, i, emel_type, emel_elem.value, llama_type,
                     llama_elem.value);
        return false;
      }
    }
...
void dump_emel_grammar(const emel::gbnf::grammar &grammar) {
  std::fprintf(stdout, "emel grammar: rules=%u elements=%u\n",
               grammar.rule_count, grammar.element_count);
  for (uint32_t rule_id = 0; rule_id < grammar.rule_count; ++rule_id) {
    ...
    for (uint32_t i = 0; i < rule.length; ++i) {
      ...
      std::fprintf(stdout, " (%u,%u)", static_cast<uint32_t>(elem.type),
                   elem.value);
    }
    std::fprintf(stdout, "\n");
  }
...
void dump_llama_grammar(const llama_grammar_rules &rules) {
  ...
  for (size_t rule_id = 0; rule_id < rules.size(); ++rule_id) {
    ...
    for (const auto &elem : rule) {
      std::fprintf(stdout, " (%u,%u)", static_cast<unsigned int>(elem.type),
...
struct weight_capture {
  ...
  emel::error::type err =
      emel::error::cast(emel::model::weight_loader::error::none);
};
...
bool find_named_line_value(std::string_view text, std::string_view key,
...
bool parse_csv_values(std::string_view text, std::span<value_type> out,
                      int32_t &count_out, parse_fn parse_value) {
  count_out = 0;
  if (text.empty()) {
    return true;
  }
  ...
  while (token_begin <= text.size()) {
    ...
    const size_t safe_end =
        token_end == std::string_view::npos ? text.size() : token_end;
...
bool write_generation_baseline_file(
  ...
  const std::string prompt_hex = encode_hex(
      std::as_bytes(std::span<const char>(opts.text.data(), opts.text.size())));
  const std::string output_hex = encode_hex(std::as_bytes(
      std::span<const char>(result.output.data(), result.output_length)));
...
bool load_generation_baseline_file(const std::filesystem::path &path,
  ...
  if (!find_named_line_value(text, "format", value) ||
      value != k_generation_baseline_format) {
    return false;
  }
  if (!find_named_line_value(text, "contract", value) ||
      value != k_generation_baseline_contract) {
    return false;
  }
  ...
  record_out.prompt.assign(reinterpret_cast<const char *>(prompt_bytes.data()),
                           prompt_bytes.size());
  ...
  if (!find_named_line_value(text, "token_ids", value) ||
      !parse_csv_values<int32_t>(
          value, std::span<int32_t>(record_out.result.trace.token_ids),
          token_count,
  ...
  if (!find_named_line_value(text, "top_score_gaps", value) ||
      !parse_csv_values<float>(
          value, std::span<float>(record_out.result.trace.top_score_gaps),
          gap_count,
  ...
  if (!decode_hex(value, output_bytes) ||
      output_bytes.size() != record_out.result.output_length) {
    return false;
  }
  if (!output_bytes.empty()) {
    std::memcpy(record_out.result.output.data(), output_bytes.data(),
                output_bytes.size());
  }
...
bool run_compute_logits_preselected_argmax_with_attribution(
    emel::generator::detail::native_backend &backend,
    generation_attribution &attribution, int32_t &selected_index,
...
void add_bucket_sample(generation_attribution_bucket &bucket,
                       const uint64_t ns) {
  bucket.ns += ns;
  bucket.calls += 1u;
}
...
uint64_t bucketed_generation_ns(const generation_attribution &attribution) {
  return attribution.embedding_lookup.ns + attribution.rms_norm.ns +
         attribution.qkv_matmul.ns + attribution.rope.ns +
         attribution.cache_store.ns + attribution.attention.ns +
         attribution.attention_output_proj.ns + attribution.residual_add.ns +
         attribution.ffn_gate_up_matmul.ns + attribution.swiglu.ns +
         attribution.ffn_down_matmul.ns + attribution.logits_q8_prepare.ns +
         attribution.logits_matmul.ns + attribution.logits_argmax.ns +
         attribution.output_append.ns;
}

void print_generation_attribution_bucket(
    const generation_attribution_bucket &bucket, const uint64_t total_ns) {
  const double pct = total_ns == 0u ? 0.0
                                    : (100.0 * static_cast<double>(bucket.ns) /
                                       static_cast<double>(total_ns));
  std::fprintf(stdout,
               "generation_attribution.bucket: name=%s ns=%" PRIu64
               " ms=%.3f pct_total=%.2f calls=%" PRIu64 "\n",
               bucket.name, bucket.ns, static_cast<double>(bucket.ns) / 1.0e6,
               pct, bucket.calls);
}
...
void dump_generation_attribution(const generation_attribution &attribution) {
  ...
  const uint64_t uncategorized_ns = attribution.total_ns > bucketed_ns
                                        ? attribution.total_ns - bucketed_ns
                                        : 0u;
  const double prefill_pct =
      attribution.total_ns == 0u
          ? 0.0
  ...
  std::fprintf(
      stdout,
      "generation_attribution: prompt_tokens=%d generated_tokens=%d "
      "total_ns=%" PRIu64 " total_ms=%.3f prefill_ns=%" PRIu64
      " prefill_pct=%.2f decode_ns=%" PRIu64
      " decode_pct=%.2f bucketed_ns=%" PRIu64 " uncategorized_ns=%" PRIu64 "\n",
      attribution.prompt_tokens, attribution.generated_tokens,
      attribution.total_ns, static_cast<double>(attribution.total_ns) / 1.0e6,
      attribution.prefill_ns, prefill_pct, attribution.decode_ns, decode_pct,
      bucketed_ns, uncategorized_ns);

  print_generation_attribution_bucket(attribution.embedding_lookup,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.rms_norm,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.qkv_matmul,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.rope, attribution.total_ns);
  print_generation_attribution_bucket(attribution.cache_store,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.attention,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.attention_output_proj,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.residual_add,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.ffn_gate_up_matmul,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.swiglu, attribution.total_ns);
  print_generation_attribution_bucket(attribution.ffn_down_matmul,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.logits_q8_prepare,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.logits_matmul,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.logits_argmax,
                                      attribution.total_ns);
  print_generation_attribution_bucket(attribution.output_append,
                                      attribution.total_ns);
}
...
struct generation_load_state {
  ...
  generation_load_state()
      : samplers{emel::logits::sampler::fn::from<generation_load_state,
                                                 sampler_select_argmax>(this)} {
  }
};

emel::model::detail::kv_binding
kv_binding_from_state(const generation_load_state &state) {
  return emel::model::detail::kv_binding{
      .arena = std::span<const uint8_t>{state.kv_arena.data(),
                                        state.kv_arena.size()},
      .entries =
          std::span<const emel::gguf::loader::kv_entry>{
              state.kv_entries.data(), state.kv_entries.size()},
  };
}

quantized_contract_summary
runtime_quantized_contract_summary(const generation_load_state &state) {
  return quantized_contract_summary{
      .native_quantized =
          state.generator->generation_native_quantized_stage_count(),
      .approved_dense_f32_by_contract =
          state.generator->generation_approved_dense_f32_stage_count(),
      .disallowed_fallback =
          state.generator->generation_disallowed_fallback_stage_count(),
      .explicit_no_claim =
          state.generator->generation_explicit_no_claim_stage_count(),
  };
}
...
bool quantized_contract_matches(const quantized_contract_summary &lhs,
  ...
  return lhs.native_quantized == rhs.native_quantized &&
         lhs.approved_dense_f32_by_contract ==
...
emel::error::type sampler_select_argmax(generation_load_state &state,
  ...
  if (state.emel_trace != nullptr) {
    append_trace_token(*state.emel_trace, summary.selected_token,
                       summary.best_score, summary.second_best_score);
  }
...
bool read_file_bytes(const std::string &path, std::vector<uint8_t> &out) {
  ...
  const bool seek_start_ok =
      file_size >= 0 && std::fseek(file, 0, SEEK_SET) == 0;
...
emel::error::type parse_gguf_kv_storage(
    generation_load_state &state, const std::span<const uint8_t> file_image,
    emel::model::data &model_data, uint32_t &tensor_count_out) {
  reset_gguf_capture(state);
  const emel::gguf::loader::event::probe_done_fn probe_done_cb{&state,
                                                               on_probe_done};
  const emel::gguf::loader::event::probe_error_fn probe_error_cb{
      &state, on_probe_error};
  emel::gguf::loader::requirements requirements = {};
  ...
  if (!state.gguf_loader.process_event(probe_ev) || !state.gguf.probe_done ||
  ...
  reset_gguf_capture(state);
  const emel::gguf::loader::event::bind_done_fn bind_done_cb{&state,
                                                             on_bind_done};
  const emel::gguf::loader::event::bind_error_fn bind_error_cb{&state,
                                                               on_bind_error};
  const emel::gguf::loader::event::bind_storage bind_ev{
      ...
      std::span<emel::model::data::tensor_record>{model_data.tensors.data(),
                                                  requirements.tensor_count},
      bind_done_cb,
      bind_error_cb,
  };

  if (!state.gguf_loader.process_event(bind_ev) || !state.gguf.bind_done ||
      state.gguf.bind_error) {
    return map_gguf_error(state.gguf.err);
  }

  reset_gguf_capture(state);
  const emel::gguf::loader::event::parse_done_fn parse_done_cb{&state,
                                                               on_parse_done};
  ...
  if (!state.gguf_loader.process_event(parse_ev) || !state.gguf.parse_done ||
      state.gguf.parse_error) {
    return map_gguf_error(state.gguf.err);
  }

  tensor_count_out = requirements.tensor_count;
  return emel::error::cast(emel::model::loader::error::none);
}

void reset_gguf_capture(generation_load_state &state) { state.gguf = {}; }

void reset_weight_capture(generation_load_state &state) { state.weight = {}; }

void reset_load_capture(generation_load_state &state) { state.load = {}; }
...
void on_weight_bind_done(
    void *owner, const emel::model::weight_loader::events::bind_done &) {
  auto &state = *static_cast<generation_load_state *>(owner);
  state.weight.bind_done = true;
  state.weight.bind_error = false;
}

void on_weight_bind_error(
    void *owner, const emel::model::weight_loader::events::bind_error &ev) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
void on_weight_plan_done(
    void *owner, const emel::model::weight_loader::events::plan_done &ev) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
void on_weight_plan_error(
    void *owner, const emel::model::weight_loader::events::plan_error &ev) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
void on_weight_apply_done(
    void *owner, const emel::model::weight_loader::events::apply_done &) {
  auto &state = *static_cast<generation_load_state *>(owner);
  state.weight.apply_done = true;
  state.weight.apply_error = false;
}

void on_weight_apply_error(
    void *owner, const emel::model::weight_loader::events::apply_error &ev) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
void on_initialize_done(void *owner,
                        const emel::generator::events::initialize_done &) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
void on_initialize_error(void *owner,
                         const emel::generator::events::initialize_error &ev) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
void on_generation_done(void *owner,
                        const emel::generator::events::generation_done &ev) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
void on_generation_error(void *owner,
                         const emel::generator::events::generation_error &ev) {
  auto &state = *static_cast<generation_load_state *>(owner);
...
bool load_generation_reference_backend(const std::string &model_path,
  ...
  const auto fail = [](const char *stage) {
    std::fprintf(stderr, "generation reference_backend_fail_stage=%s\n", stage);
    return false;
  };
  ...
  llama_model_ptr model(
      ...
      llama_model_free);
  if (model == nullptr) {
    return fail("reference_model_load");
  }

  const llama_vocab *vocab_ptr = llama_model_get_vocab(model.get());
  if (vocab_ptr == nullptr) {
    return fail("reference_vocab");
  }

  std::string_view vocab_fail_stage = {};
  if (!emel::model::detail::load_vocab_from_gguf(kv_binding_from_state(state),
                                                 state.model_data->vocab_data,
                                                 &vocab_fail_stage)) {
    if (!vocab_fail_stage.empty()) {
      std::fprintf(stderr, "generation vocab fail_stage=%.*s\n",
                   static_cast<int>(vocab_fail_stage.size()),
                   vocab_fail_stage.data());
    }
    return fail("emel_vocab_load");
  }
  ...
  state.reference.formatter = emel::tools::generation_formatter_contract::
  ...
  if (!emel::tools::generation_formatter_contract::reference_binding_supported(
    ...
    return fail("reference_formatter_contract");
  }
...
bool load_emel_vocab_from_gguf_file(const std::string &model_path,
  ...
  generation_load_state state{};
  if (!read_file_bytes(model_path, state.file_bytes) ||
  ...
  const emel::error::type parse_err =
      parse_gguf_kv_storage(state,
                            std::span<const uint8_t>{state.file_bytes.data(),
                                                     state.file_bytes.size()},
                            *state.model_data, tensor_count);
  if (parse_err != emel::error::cast(emel::model::loader::error::none)) {
    return false;
  }

  return emel::model::detail::load_vocab_from_gguf(kv_binding_from_state(state),
                                                   vocab_out);
}

emel::error::type
run_emel_initialize_generator(generation_load_state &state,
                              const emel::paritychecker::parity_options &opts) {
  ...
  request.preprocessor_variant =
      generation_preprocessor_variant(*state.model_data);
...
emel::error::type
run_emel_generate(generation_load_state &state,
                  const emel::paritychecker::parity_options &opts,
                  std::span<char> output, size_t &output_length_out,
...
argmax_summary select_argmax_from_logits(const float *logits,
...
bool tokenize_reference_prompt(const reference_backend &backend,
  ...
  int32_t token_capacity = static_cast<int32_t>(formatted_prompt.size()) + 8;
  token_capacity = std::max(token_capacity, 8);
  tokens_out.resize(static_cast<size_t>(token_capacity));
  int32_t token_count =
      llama_tokenize(backend.vocab, formatted_prompt.c_str(),
                     static_cast<int32_t>(formatted_prompt.size()),
                     tokens_out.data(), token_capacity, false, false);
  if (token_count < 0) {
...
bool tokenize_conditioned_prompt(
    generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    std::vector<int32_t> &tokens_out) {
  const int32_t token_capacity =
      std::max<int32_t>(1024, static_cast<int32_t>(opts.text.size()) * 8 + 64);
  ...
  const bool accepted = state.conditioner.process_event(prepare_ev);
...
float *read_direct_reference_logits(reference_backend &backend,
                                    llama_context *ctx) {
...
bool append_reference_piece(const reference_backend &backend,
  ...
  if (llama_vocab_is_control(backend.vocab, token) ||
      llama_vocab_is_eog(backend.vocab, token)) {
    return true;
  }

  std::array<char, 256> piece_storage = {};
  int32_t piece_len = llama_token_to_piece(
      backend.vocab, token, piece_storage.data(),
      static_cast<int32_t>(piece_storage.size()), 0, false);
  if (piece_len == 0) {
    return true;
  }
  if (piece_len < 0) {
    const int32_t required_len = -piece_len;
    std::vector<char> dynamic_piece(static_cast<size_t>(required_len));
    piece_len = llama_token_to_piece(backend.vocab, token, dynamic_piece.data(),
                                     required_len, 0, false);
    if (piece_len < 0 ||
        result_out.output_length + static_cast<size_t>(piece_len) >
            result_out.output.size()) {
      return false;
    }
    std::memcpy(result_out.output.data() + result_out.output_length,
                dynamic_piece.data(), static_cast<size_t>(piece_len));
    result_out.output_length += static_cast<size_t>(piece_len);
    return true;
  }
  if (result_out.output_length + static_cast<size_t>(piece_len) >
      result_out.output.size()) {
    return false;
  }
  if (piece_len > 0) {
    std::memcpy(result_out.output.data() + result_out.output_length,
                piece_storage.data(), static_cast<size_t>(piece_len));
  }
  result_out.output_length += static_cast<size_t>(piece_len);
  return true;
}
...
bool append_rendered_token(emel::text::renderer::sm &renderer,
  ...
  render_ev.output_capacity =
      result_out.output.size() - result_out.output_length;
...
bool initialize_emel_renderer(const emel::model::data &model_data,
  ...
  emel::text::renderer::event::initialize initialize_renderer_ev{
      model_data.vocab_data};
...
bool emel_token_is_stop(const emel::model::data::vocab &vocab,
                        const int32_t token_id) {
...
struct llama_batch_owner {
  ...
  llama_batch_owner(int32_t n_tokens, int32_t n_seq_max) {
  ...
  llama_batch_owner(const llama_batch_owner &) = delete;
  llama_batch_owner &operator=(const llama_batch_owner &) = delete;
};

bool prepare_reference_batch(llama_batch &batch,
                             std::span<const llama_token> tokens,
                             const int32_t position_base) {
  if (tokens.empty() || batch.token == nullptr || batch.pos == nullptr ||
  ...
  for (int32_t idx = 0; idx < batch.n_tokens; ++idx) {
    batch.token[idx] = tokens[static_cast<size_t>(idx)];
    batch.pos[idx] = position_base + idx;
    batch.n_seq_id[idx] = 1;
    if (batch.seq_id[idx] == nullptr) {
      return false;
    }
    batch.seq_id[idx][0] = 0;
    batch.logits[idx] = idx + 1 == batch.n_tokens ? 1 : 0;
  }
...
llama_context_ptr make_reference_context(reference_backend &backend) {
  ...
  return llama_context_ptr{
      backend.model != nullptr
          ? llama_init_from_model(backend.model.get(), context_params)
          : nullptr,
      llama_free,
  };
}
...
emel::error::type
run_reference_generate(generation_load_state &state,
                       const emel::paritychecker::parity_options &opts,
                       generation_result &result_out) {
  reference_backend &backend = state.reference;
  if (backend.model == nullptr || backend.vocab == nullptr ||
      backend.vocab_size <= 0) {
    return emel::error::cast(emel::generator::error::invalid_request);
  }
  ...
  llama_batch_owner prompt_batch_owner(
      static_cast<int32_t>(prompt_tokens.size()), 1);
  if (!prepare_reference_batch(prompt_batch_owner.batch, prompt_tokens, 0) ||
      run_direct_reference_decode(backend, ctx.get(),
                                  prompt_batch_owner.batch) != 0) {
    return emel::error::cast(emel::generator::error::backend);
  }
  ...
  for (int32_t step = 0; step < opts.max_tokens; ++step) {
    ...
    const argmax_summary summary =
        select_argmax_from_logits(logits, backend.vocab_size);
    const llama_token selected =
        static_cast<llama_token>(summary.selected_token);
    append_trace_token(result_out.trace, summary.selected_token,
                       summary.best_score, summary.second_best_score);
    ...
    const std::array<llama_token, 1> next_token = {selected};
    llama_batch_owner decode_batch_owner(1, 1);
    if (!prepare_reference_batch(
            decode_batch_owner.batch,
            std::span<const llama_token>(next_token.data(), next_token.size()),
            static_cast<int32_t>(prompt_tokens.size()) + step) ||
        run_direct_reference_decode(backend, ctx.get(),
                                    decode_batch_owner.batch) != 0) {
      return emel::error::cast(emel::generator::error::backend);
    }
  }
...
int32_t first_token_mismatch_index(const generation_result &lhs,
                                   const generation_result &rhs) {
...
size_t first_mismatch_offset(const generation_result &lhs,
                             const generation_result &rhs) {
...
bool generation_results_match(const generation_result &emel_result,
  ...
  return std::string_view{emel_result.output.data(),
                          emel_result.output_length} ==
         std::string_view{reference_result.output.data(),
                          reference_result.output_length};
}

using native_layer_runner = bool (*)(emel::generator::detail::native_backend &,
                                     int32_t, int32_t);

bool run_ggml_nonflash_attn_case(std::span<const float> q_data,
                                 std::span<const float> k_data,
                                 std::span<const float> v_data,
                                 int64_t head_dim, int64_t kv_tokens,
                                 int64_t active_kv_tokens, int64_t head_count,
                                 int64_t kv_head_count, float scale,
                                 std::vector<float> &out);
bool run_emel_nonflash_f16_ggml_softmax_case(
    std::span<const float> q_data, std::span<const float> k_data,
    std::span<const float> v_data, int64_t head_dim, int64_t kv_tokens,
    int64_t active_kv_tokens, int64_t head_count, int64_t kv_head_count,
...
bool run_emel_prod_style_attn_case(std::span<const float> q_data,
                                   ...
                                   int64_t kv_head_count, float scale,
                                   std::vector<float> &out);
bool run_emel_prod_style_float_value_attn_case(
    std::span<const float> q_data, std::span<const float> k_data,
    std::span<const float> v_data, int64_t head_dim, int64_t kv_tokens,
    int64_t active_kv_tokens, int64_t head_count, int64_t kv_head_count,
    float scale, std::vector<float> &out);
...
bool run_ggml_flash_attn_ext_case(std::span<const float> q_data,
                                  ...
                                  std::span<const float> v_data,
                                  int64_t head_dim, int64_t kv_tokens,
                                  int64_t head_count, int64_t kv_head_count,
                                  float scale, std::vector<float> &out);
bool run_ggml_flash_attn_ext_case(std::span<const float> q_data,
...
bool run_ggml_flash_attn_ext_masked_case(
    std::span<const float> q_data, std::span<const float> k_data,
    std::span<const float> v_data, int64_t head_dim, int64_t kv_tokens,
    int64_t active_kv_tokens, int64_t head_count, int64_t kv_head_count,
    float scale, std::vector<float> &out);
bool run_ggml_flash_attn_ext_masked_case(
    std::span<const float> q_data, std::span<const uint16_t> k_data,
    std::span<const uint16_t> v_data, int64_t head_dim, int64_t kv_tokens,
    int64_t active_kv_tokens, int64_t head_count, int64_t kv_head_count,
    float scale, std::vector<float> &out);
...
inline float read_debug_value(std::span<const uint16_t> values,
                              const size_t idx) {
  return fp16_storage_to_fp32(values[idx]);
}

emel::error::type
run_custom_native_generate(generation_load_state &state,
                           const emel::paritychecker::parity_options &opts,
  ...
  if (!tokenize_conditioned_prompt(state, opts, prompt_tokens) ||
      prompt_tokens.empty()) {
    return emel::error::cast(emel::generator::error::invalid_request);
  }
  ...
  for (int32_t step = 0; step < opts.max_tokens; ++step) {
    ...
    if (emel_token_is_stop(state.model_data->vocab_data,
                           summary.selected_token)) {
      break;
    }

    const int32_t position = static_cast<int32_t>(prompt_tokens.size()) +
                             result_out.tokens_generated - 1;
    if (!emel::generator::detail::copy_tensor_row(
            *backend.token_embedding.tensor, summary.selected_token,
            backend.hidden)) {
      return emel::error::cast(emel::generator::error::backend);
    }
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
...
bool run_layer_with_flash_attribution(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position, generation_attribution &attribution) {
  ...
  if (!time_bucket_bool(attribution.qkv_matmul,
                        [&] {
                          return emel::generator::detail::matmul_vector(
                              backend, block.attention_q, backend.norm,
                              backend.q);
                        }) ||
      !time_bucket_bool(attribution.qkv_matmul,
                        [&] {
                          return emel::generator::detail::matmul_vector(
                              backend, block.attention_k, backend.norm,
                              backend.k);
                        }) ||
      !time_bucket_bool(attribution.qkv_matmul, [&] {
        return emel::generator::detail::matmul_vector(
  ...
  if (emel::generator::detail::is_qwen3_runtime(backend) &&
      !time_bucket_bool(attribution.rms_norm, [&] {
        return emel::generator::detail::apply_qwen3_attention_qk_norm(backend,
                                                                      block);
      })) {
    return false;
  }
  ...
  time_bucket_void(attribution.rope, [&] {
    emel::generator::detail::apply_rope(
        backend.q, backend.n_head, backend.head_dim, backend.n_rot, position,
        backend.rope_freq_base, backend.rope_type, backend.rope_freq_scale,
        backend.rope_context_original, backend.rope_ext_factor,
        backend.rope_attn_factor, backend.rope_beta_fast,
        backend.rope_beta_slow);
    emel::generator::detail::apply_rope(
        backend.k, backend.n_head_kv, backend.head_dim_kv, backend.n_rot,
        position, backend.rope_freq_base, backend.rope_type,
        backend.rope_freq_scale, backend.rope_context_original,
        backend.rope_ext_factor, backend.rope_attn_factor,
        backend.rope_beta_fast, backend.rope_beta_slow);
  });
  ...
  time_bucket_void(attribution.cache_store, [&] {
    ...
    for (int32_t kv_head = 0; kv_head < backend.n_head_kv; ++kv_head) {
      const size_t src_offset = static_cast<size_t>(kv_head) *
      ...
      emel::generator::detail::store_fp16_rounded_cache(
          std::span<const float>(backend.k.data() +
                                     static_cast<std::ptrdiff_t>(src_offset),
                                 static_cast<size_t>(backend.head_dim_kv)),
          backend.flash_key_cache.data() + flash_cache_offset);
      emel::generator::detail::store_fp16_rounded_cache(
          std::span<const float>(backend.v.data() +
                                     static_cast<std::ptrdiff_t>(src_offset),
                                 static_cast<size_t>(backend.head_dim_kv)),
  ...
  if (!time_bucket_bool(attribution.attention, [&] {
        return emel::generator::detail::dispatch_flash_attention(
  ...
  if (!time_bucket_bool(attribution.attention_output_proj, [&] {
        return emel::generator::detail::matmul_vector(
            backend, block.attention_output, backend.attn_ctx,
            backend.projected);
      })) {
    return false;
  }
  ...
  time_bucket_void(attribution.residual_add, [&] {
    for (int32_t idx = 0; idx < backend.n_embd; ++idx) {
      backend.hidden[static_cast<size_t>(idx)] +=
          backend.projected[static_cast<size_t>(idx)];
    }
  });

  if (!time_bucket_bool(attribution.rms_norm, [&] {
        return emel::generator::detail::rms_norm(
            backend.hidden, block.feed_forward_norm, backend.rms_epsilon,
            backend.norm);
      })) {
    return false;
  }
  if (!time_bucket_bool(attribution.ffn_gate_up_matmul,
                        [&] {
                          return emel::generator::detail::matmul_vector(
                              backend, block.feed_forward_gate, backend.norm,
                              backend.gate);
                        }) ||
  ...
  if (!time_bucket_bool(attribution.ffn_down_matmul, [&] {
        return emel::generator::detail::matmul_vector(
            backend, block.feed_forward_down, backend.ffn_hidden,
            backend.projected);
      })) {
    return false;
  }
...
emel::error::type
run_attributed_native_generate(generation_load_state &state,
                               const emel::paritychecker::parity_options &opts,
                               generation_result &result_out,
  ...
  for (size_t token_index = 0; token_index < prompt_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_flash_attribution(backend, layer, position,
                                            attribution_out)) {
        return emel::error::cast(emel::generator::error::backend);
      }
    }
  ...
  attribution_out.prefill_ns =
      elapsed_ns(prefill_begin, attribution_clock::now());

  for (int32_t step = 0; step < opts.max_tokens; ++step) {
    const auto decode_begin = attribution_clock::now();
    const llama_token selected = static_cast<llama_token>(selected_token);
    append_trace_token(result_out.trace, selected_token, selected_score,
                       selected_score);
    result_out.tokens_generated += 1;
    if (!time_bucket_bool(attribution_out.output_append, [&] {
          return append_rendered_token(renderer, selected_token, result_out);
        })) {
      return emel::error::cast(emel::generator::error::backend);
    }
    if (emel_token_is_stop(state.model_data->vocab_data, selected_token)) {
      attribution_out.decode_ns +=
          elapsed_ns(decode_begin, attribution_clock::now());
      break;
    }
...
bool run_prefill_from_token_prefix(
    emel::generator::detail::native_backend &backend,
...
bool run_compute_logits_with_attribution(
  ...
  if (!time_bucket_bool(attribution.rms_norm, [&] {
        return emel::generator::detail::rms_norm(
            backend.hidden, backend.output_norm, backend.rms_epsilon,
            backend.norm);
      })) {
    return false;
  }
  ...
  const bool packed_q8_logits_path =
      emel::generator::detail::q8_input_path_supported(backend, backend.output);
...
bool run_compute_logits_preselected_argmax_with_attribution(
  ...
  const emel::generator::detail::tensor_matrix &output_matrix =
      backend.output_argmax.tensor != nullptr ? backend.output_argmax
                                              : backend.output;
  const bool packed_q8_logits_path =
      emel::generator::detail::q8_input_argmax_path_supported(backend,
                                                              output_matrix);
    ...
    return time_bucket_bool(
           ...
           time_bucket_bool(attribution.logits_matmul, [&] {
             return emel::generator::detail::matmul_vector_q8_input_argmax(
                 backend, output_matrix, q8_input, backend.n_embd,
...
bool matmul_vector_dequantized(
  ...
  for (int32_t row_index = 0; row_index < matrix.rows; ++row_index) {
    if (!emel::generator::detail::copy_tensor_row(*matrix.tensor, row_index,
                                                  row)) {
      return false;
    }
    double sum = 0.0;
...
void dump_generation_q23_stage_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result);
void dump_generation_prefix_timeline_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result);
void dump_generation_q23_timeline_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result);
void dump_generation_reference_q_timeline_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result);
void dump_generation_reference_q_stage_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result);

bool quantize_input_blocks_reference(
    std::span<const float> input,
    std::array<reference_block_q8_k, kernel_quant::MAX_Q8_K_BLOCKS> &blocks,
    uint64_t &block_count_out);

bool quantize_input_blocks(
    std::span<const float> input,
    std::array<kernel_quant::block_q8_k, kernel_quant::MAX_Q8_K_BLOCKS> &blocks,
    uint64_t &block_count_out);

void update_score_dot_probe(
    const emel::generator::detail::native_backend &backend,
    const int32_t token_index, const int32_t layer_index,
    const int32_t position_limit, score_dot_probe_result &result) {
  ...
  for (int32_t head = 0; head < backend.n_head; ++head) {
    ...
    const size_t kv_offset =
        static_cast<size_t>(kv_head) * static_cast<size_t>(kv_head_dim);
    for (int32_t dim = 0; dim < head_dim; ++dim) {
      q_f16[static_cast<size_t>(dim)] = ggml_fp32_to_fp16(
          backend.q_attn[q_offset + static_cast<size_t>(dim)]);
    }
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      for (int32_t dim = 0; dim < head_dim; ++dim) {
        k_f16[static_cast<size_t>(dim)] =
            ggml_fp32_to_fp16(fp16_storage_to_fp32(
                backend.key_cache[cache_offset + static_cast<size_t>(dim)]));
      }
      float reference_score = 0.0f;
      ggml_vec_dot_f16(head_dim, &reference_score, 0u, k_f16.data(), 0u,
                       q_f16.data(), 0u, 1);
      const float emel_score =
          emel::kernel::detail::dot_product_ggml_f16_scores(
...
bool capture_reference_value_cache_rows(llama_context *ctx, int32_t layer_index,
                                        std::vector<float> &values_out);
bool capture_reference_key_cache_rows(llama_context *ctx, int32_t layer_index,
                                      std::vector<float> &values_out);

float ggml_row_dot_reference_q8(
    const emel::generator::detail::tensor_matrix &matrix, uint32_t row,
    const reference_block_q8_k *q8_blocks, uint64_t block_count);

bool matmul_vector_reference_q8(
    const emel::generator::detail::tensor_matrix &matrix,
    std::span<const float> input, std::span<float> output) {
  if (matrix.tensor == nullptr || matrix.rows <= 0 || matrix.cols <= 0 ||
      static_cast<size_t>(matrix.cols) != input.size() ||
      static_cast<size_t>(matrix.rows) != output.size()) {
    return false;
  }
...
bool matmul_vector_scalar_quantized(
    const emel::generator::detail::tensor_matrix &matrix,
    std::span<const float> input, std::span<float> output) {
  if (matrix.tensor == nullptr || matrix.rows <= 0 || matrix.cols <= 0 ||
      static_cast<size_t>(matrix.cols) != input.size() ||
  ...
  const auto dtype =
      static_cast<emel::kernel::event::dtype>(matrix.tensor->type);
  ...
  std::array<kernel_quant::block_q8_k, kernel_quant::MAX_Q8_K_BLOCKS>
      q8_blocks = {};
  uint64_t block_count = 0;
...
bool compute_logits_with_matmul_mode(
    emel::generator::detail::native_backend &backend,
    const exact_matmul_mode mode) {
  return emel::generator::detail::rms_norm(backend.hidden, backend.output_norm,
                                           backend.rms_epsilon, backend.norm) &&
         matmul_vector_mode(backend, backend.output, backend.norm,
                            backend.bound_logits, mode.output, mode.only_dtype,
                            mode.dtype_mask, mode.use_reference_q8,
                            mode.use_scalar_quantized);
}

bool run_layer_with_matmul_mode_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position, const exact_matmul_mode mode) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  if (!emel::generator::detail::rms_norm(backend.hidden, block.attention_norm,
                                         backend.rms_epsilon, backend.norm) ||
      !matmul_vector_mode(backend, block.attention_q, backend.norm, backend.q,
                          mode.attention, mode.only_dtype, mode.dtype_mask,
                          mode.use_reference_q8, mode.use_scalar_quantized) ||
      !matmul_vector_mode(backend, block.attention_k, backend.norm, backend.k,
                          mode.attention, mode.only_dtype, mode.dtype_mask,
                          mode.use_reference_q8, mode.use_scalar_quantized) ||
      !matmul_vector_mode(backend, block.attention_v, backend.norm, backend.v,
                          mode.attention, mode.only_dtype, mode.dtype_mask,
                          mode.use_reference_q8, mode.use_scalar_quantized)) {
    return false;
  }
  ...
  emel::generator::detail::apply_rope(
      ...
      backend.rope_freq_base, backend.rope_type, backend.rope_freq_scale,
      backend.rope_context_original, backend.rope_ext_factor,
      backend.rope_attn_factor, backend.rope_beta_fast, backend.rope_beta_slow);
  emel::generator::detail::apply_rope(
      backend.k, backend.n_head_kv, backend.head_dim_kv, backend.n_rot,
      position, backend.rope_freq_base, backend.rope_type,
      backend.rope_freq_scale, backend.rope_context_original,
      backend.rope_ext_factor, backend.rope_attn_factor, backend.rope_beta_fast,
      backend.rope_beta_slow);
  emel::generator::detail::store_fp16_rounded_cache(
      std::span<const float>(backend.q.data(),
                             static_cast<size_t>(backend.n_embd)),
  ...
  if (!emel::generator::detail::compute_attention(
      ...
      !matmul_vector_mode(backend, block.attention_output, backend.attn_ctx,
                          backend.projected, mode.attention, mode.only_dtype,
                          mode.dtype_mask, mode.use_reference_q8,
                          mode.use_scalar_quantized)) {
    return false;
  }
  ...
  if (!emel::generator::detail::rms_norm(backend.hidden,
                                         block.feed_forward_norm,
                                         backend.rms_epsilon, backend.norm) ||
      !matmul_vector_mode(backend, block.feed_forward_gate, backend.norm,
                          backend.gate, mode.ffn, mode.only_dtype,
                          mode.dtype_mask, mode.use_reference_q8,
                          mode.use_scalar_quantized) ||
      !matmul_vector_mode(backend, block.feed_forward_up, backend.norm,
                          backend.up, mode.ffn, mode.only_dtype,
                          mode.dtype_mask, mode.use_reference_q8,
                          mode.use_scalar_quantized)) {
    return false;
  }
  ...
  if (!matmul_vector_mode(backend, block.feed_forward_down, backend.ffn_hidden,
                          backend.projected, mode.ffn, mode.only_dtype,
                          mode.dtype_mask, mode.use_reference_q8,
                          mode.use_scalar_quantized)) {
    return false;
  }
...
bool run_prefill_with_scalar_attention_matmul_mode(
    emel::generator::detail::native_backend &backend,
    std::span<const int32_t> prefix_tokens, const exact_matmul_mode mode) {
  if (prefix_tokens.empty()) {
    return false;
  }

  backend.kv_cache_tokens = 0;
  for (size_t token_index = 0; token_index < prefix_tokens.size();
       ++token_index) {
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_matmul_mode_scalar_attention(backend, layer, position,
                                                       mode)) {
        return false;
      }
    }
...
bool run_layer_with_exact_all_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{.attention = true, .ffn = true, .output = true};
  return run_layer_with_matmul_mode_scalar_attention(backend, layer_index,
                                                     position, mode);
}
...
bool run_layer_with_exact_ffn_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = false, .ffn = true, .output = false};
  return run_layer_with_matmul_mode_scalar_attention(backend, layer_index,
                                                     position, mode);
}
...
bool run_layer_with_exact_output_scalar_attention(
    ...
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = false, .ffn = false, .output = true};
  return run_layer_with_matmul_mode_scalar_attention(backend, layer_index,
                                                     position, mode);
}
...
bool run_layer_with_exact_q2_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_exact_q3_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_exact_q6_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_exact_q1_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
      .ffn = true,
      .output = true,
      .only_dtype = static_cast<uint8_t>(emel::kernel::event::dtype::q1_0_g128),
  };
  return run_layer_with_matmul_mode_scalar_attention(backend, layer_index,
                                                     position, mode);
}

bool run_layer_with_scalar_q2_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_scalar_q3_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_scalar_q6_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_reference_q2_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_reference_q6_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_reference_q3_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  const exact_matmul_mode mode{
      .attention = true,
...
bool run_layer_with_scalar_attention(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  if (!emel::generator::detail::rms_norm(backend.hidden, block.attention_norm,
                                         backend.rms_epsilon, backend.norm) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_q,
                                              backend.norm, backend.q) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_k,
                                              backend.norm, backend.k) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_v,
                                              backend.norm, backend.v)) {
    return false;
  }
  ...
  if (!emel::generator::detail::compute_attention(
          backend, layer_index, position + 1, backend.q_attn) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
    return false;
  }
  ...
  if (!emel::generator::detail::rms_norm(backend.hidden,
                                         block.feed_forward_norm,
                                         backend.rms_epsilon, backend.norm) ||
      !emel::generator::detail::matmul_vector(backend, block.feed_forward_gate,
                                              backend.norm, backend.gate) ||
      !emel::generator::detail::matmul_vector(backend, block.feed_forward_up,
                                              backend.norm, backend.up)) {
    return false;
  }
  ...
  if (!emel::generator::detail::matmul_vector(backend, block.feed_forward_down,
                                              backend.ffn_hidden,
                                              backend.projected)) {
    return false;
  }
...
bool dispatch_flash_attention_with_q(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position, std::span<const float> q_vector) {
  auto request = emel::generator::detail::make_flash_attn_request(
      backend, layer_index, position);
  request.src0 = emel::generator::detail::make_src_view_3d(
      q_vector.data(), static_cast<uint64_t>(backend.head_dim), 1u,
      static_cast<uint64_t>(backend.n_head));
  backend.kernel.set_kind(backend.kernel_kind);
  return backend.kernel.process_event(request);
}

bool run_layer_with_flash_request_q(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  if (!emel::generator::detail::rms_norm(backend.hidden, block.attention_norm,
...
bool run_layer_with_flash_request_q_attn(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  if (!emel::generator::detail::rms_norm(backend.hidden, block.attention_norm,
  ...
  if (!dispatch_flash_attention_with_q(
          backend, layer_index, position,
          std::span<const float>(backend.q_attn.data(),
                                 static_cast<size_t>(backend.n_embd))) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
...
bool run_layer_with_scalar_attention_score_dot_probe(
    emel::generator::detail::native_backend &backend, const int32_t token_index,
  ...
  update_score_dot_probe(backend, token_index, layer_index, position + 1,
                         probe);
...
bool run_prefill_with_scalar_attention(
    emel::generator::detail::native_backend &backend,
    std::span<const int32_t> prefix_tokens) {
  if (prefix_tokens.empty()) {
    return false;
...
bool run_prefill_with_flash_request_q(
    emel::generator::detail::native_backend &backend,
    std::span<const int32_t> prefix_tokens) {
  if (prefix_tokens.empty()) {
    return false;
...
bool run_prefill_with_flash_request_q_attn(
    emel::generator::detail::native_backend &backend,
    std::span<const int32_t> prefix_tokens) {
  if (prefix_tokens.empty()) {
    return false;
...
bool run_prefill_with_scalar_attention_score_dot_probe(
    emel::generator::detail::native_backend &backend,
    std::span<const int32_t> prefix_tokens, score_dot_probe_result &probe) {
  if (prefix_tokens.empty()) {
    return false;
  }
...
bool reference_stage_targets_layer(
...
bool reference_stage_is_kqv_out(
    const reference_stage_injection injection_stage) {
  switch (injection_stage) {
  case reference_stage_injection::layer0_kqv_out:
  case reference_stage_injection::layer1_kqv_out:
...
bool reference_stage_is_attn_out(
    const reference_stage_injection injection_stage) {
  switch (injection_stage) {
  case reference_stage_injection::layer0_attn_out:
  case reference_stage_injection::layer1_attn_out:
...
bool reference_stage_is_attn_norm(
    const reference_stage_injection injection_stage) {
  switch (injection_stage) {
  case reference_stage_injection::layer1_attn_norm:
    return true;
...
bool reference_stage_is_ffn_inp(
    const reference_stage_injection injection_stage) {
  switch (injection_stage) {
  case reference_stage_injection::layer1_ffn_inp:
    return true;
...
bool reference_stage_is_ffn_norm(
    const reference_stage_injection injection_stage) {
  switch (injection_stage) {
  case reference_stage_injection::layer0_ffn_norm:
  case reference_stage_injection::layer1_ffn_norm:
...
bool reference_stage_is_ffn_out(
    const reference_stage_injection injection_stage) {
  switch (injection_stage) {
  case reference_stage_injection::layer0_ffn_out:
  case reference_stage_injection::layer1_ffn_out:
...
reference_stage_rows(const reference_stage_capture_set &captures,
                     const reference_stage_injection injection_stage) {
  switch (injection_stage) {
  case reference_stage_injection::layer0_kqv_out:
...
std::span<const float>
reference_l_out_rows(const reference_stage_capture_set &captures,
                     const int32_t layer_index) {
  switch (layer_index) {
  case 0:
    return captures.layer0_l_out;
...
bool run_layer_with_scalar_attention_reference_stage(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position, const reference_stage_injection injection_stage,
    ...
    const exact_matmul_mode mode = {}) {
  auto capture_row = [](std::span<const float> values, const size_t row_width,
                        const int32_t row_index) {
    if (row_width == 0 || row_index < 0 || values.empty() ||
        values.size() % row_width != 0) {
  ...
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  const bool inject_stage_here =
      reference_stage_targets_layer(injection_stage, layer_index);
  if (!emel::generator::detail::rms_norm(backend.hidden, block.attention_norm,
                                         backend.rms_epsilon, backend.norm)) {
    return false;
  }
  ...
  if (inject_stage_here && reference_stage_is_attn_norm(injection_stage)) {
    const std::span<const float> reference_row =
        capture_row(reference_stage_rows(captures, injection_stage),
                    backend.norm.size(), position);
  ...
  if (!matmul_vector_mode(backend, block.attention_q, backend.norm, backend.q,
                          mode.attention, mode.only_dtype, mode.dtype_mask,
                          mode.use_reference_q8, mode.use_scalar_quantized) ||
      !matmul_vector_mode(backend, block.attention_k, backend.norm, backend.k,
                          mode.attention, mode.only_dtype, mode.dtype_mask,
  ...
  if (inject_stage_here && reference_stage_is_kqv_out(injection_stage)) {
    const std::span<const float> reference_row =
        capture_row(reference_stage_rows(captures, injection_stage),
                    backend.attn_ctx.size(), position);
    if (reference_row.size() != backend.attn_ctx.size()) {
      return false;
    }
    std::copy(reference_row.begin(), reference_row.end(),
  ...
  if (inject_stage_here && reference_stage_is_kqv_out(injection_stage) &&
    ...
    const exact_matmul_mode exact_attention_only{
        .attention = true, .ffn = false, .output = false};
    const exact_matmul_mode reference_q8_attention_only{.attention = true,
                                                        .ffn = false,
                                                        .output = false,
                                                        .use_reference_q8 =
                                                            true};
    ...
    if (!matmul_vector_mode(backend, block.attention_output, backend.attn_ctx,
                            backend.projected, projection_mode.attention,
  ...
  } else if (!matmul_vector_mode(backend, block.attention_output,
                                 backend.attn_ctx, backend.projected,
                                 mode.attention, mode.only_dtype,
                                 mode.dtype_mask, mode.use_reference_q8,
                                 mode.use_scalar_quantized)) {
    return false;
  }
  ...
  if (inject_stage_here && reference_stage_is_attn_out(injection_stage)) {
    const std::span<const float> reference_row =
        capture_row(reference_stage_rows(captures, injection_stage),
                    backend.projected.size(), position);
    if (reference_row.size() != backend.projected.size()) {
      return false;
    }
    std::copy(reference_row.begin(), reference_row.end(),
              backend.projected.begin());
  }
  ...
  if (inject_stage_here && reference_stage_is_ffn_norm(injection_stage)) {
    ...
    std::copy(reference_row.begin(), reference_row.end(), backend.norm.begin());
    if (!matmul_vector_mode(backend, block.feed_forward_gate, backend.norm,
                            backend.gate, mode.ffn, mode.only_dtype,
                            mode.dtype_mask, mode.use_reference_q8,
                            mode.use_scalar_quantized) ||
        !matmul_vector_mode(backend, block.feed_forward_up, backend.norm,
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_reference_stage(
              backend, layer, position, injection_stage, captures,
              projection_override, mode)) {
        return false;
      }
    }
    backend.kv_cache_tokens = position + 1;
...
reference_stage_replay_diff replay_reference_stage_l_out_diff(
    emel::generator::detail::native_backend &backend,
    std::span<const int32_t> prefix_tokens, const int32_t target_layer,
    const reference_stage_injection injection_stage,
    const reference_stage_capture_set &captures,
    const attention_projection_override projection_override =
        attention_projection_override::native,
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      ...
      if (layer == target_layer) {
        const std::span<const float> reference_row =
            capture_row(reference_l_out_rows(captures, target_layer),
                        backend.hidden.size(), position);
        if (reference_row.size() != backend.hidden.size()) {
          result.first_target_l_out_mismatch = -2;
          return result;
        ...
        for (size_t idx = 0; idx < backend.hidden.size(); ++idx) {
          diff = std::max(diff,
                          std::fabs(backend.hidden[idx] - reference_row[idx]));
        }
...
bool inject_reference_cache_row(
    emel::generator::detail::native_backend &backend,
    llama_context *reference_ctx, const int32_t layer_index,
    const int32_t position, const bool inject_key_cache,
    const bool inject_value_cache) {
  const int32_t kv_dim = backend.n_head_kv * backend.head_dim_kv;
  const size_t row_offset = emel::generator::detail::layer_cache_offset(
      backend, layer_index, position, kv_dim);
  const size_t row_width = static_cast<size_t>(kv_dim);

  if (inject_key_cache) {
    std::vector<float> reference_key_cache_rows;
    if (!capture_reference_key_cache_rows(reference_ctx, layer_index,
                                          reference_key_cache_rows) ||
        reference_key_cache_rows.size() !=
            static_cast<size_t>(position + 1) * row_width) {
      return false;
    }
    ...
    std::copy(reference_row.begin(), reference_row.end(),
              backend.key_cache.begin() + row_offset);
  }

  if (inject_value_cache) {
    std::vector<float> reference_value_cache_rows;
    if (!capture_reference_value_cache_rows(reference_ctx, layer_index,
                                            reference_value_cache_rows) ||
        reference_value_cache_rows.size() !=
            static_cast<size_t>(position + 1) * row_width) {
      return false;
    }
    const auto reference_row =
...
bool run_layer_with_scalar_attention_reference_cache(
  ...
  if (!inject_reference_cache_row(backend, reference_ctx, layer_index, position,
                                  inject_key_cache, inject_value_cache) ||
...
bool run_prefill_with_scalar_attention_reference_cache(
    ...
    std::span<const int32_t> prefix_tokens, const bool inject_key_cache,
    const bool inject_value_cache) {
  if (prefix_tokens.empty()) {
    return false;
  }
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_reference_cache(
              backend, reference_ctx.get(), layer, position, inject_key_cache,
              inject_value_cache)) {
        return false;
      }
    }
...
bool compute_attention_with_ggml_f16_value_contraction(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position_limit, std::span<const float> q_vector) {
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      for (int32_t dim = 0; dim < head_dim; ++dim) {
        score +=
            q_vector[q_offset + static_cast<size_t>(dim)] *
            fp16_storage_to_fp32(
                backend.key_cache[cache_offset + static_cast<size_t>(dim)]);
      }
      score *= inv_scale;
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      const float prob = std::exp(
          backend.attn_scores[static_cast<size_t>(position)] - max_score);
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      weight_f16[static_cast<size_t>(position)] =
          kernel_quant::fp32_to_fp16(weight);
    }

    for (int32_t dim = 0; dim < head_dim; ++dim) {
      for (int32_t position = 0; position < position_limit; ++position) {
        const size_t cache_offset =
            emel::generator::detail::layer_cache_offset(backend, layer_index,
                                                        position, kv_dim) +
            kv_offset;
        value_f16[static_cast<size_t>(position)] =
            kernel_quant::fp32_to_fp16(fp16_storage_to_fp32(
                backend.value_cache[cache_offset + static_cast<size_t>(dim)]));
      }

      float dot = 0.0f;
      ggml_vec_dot_f16(position_limit, &dot, 0u, value_f16.data(), 0u,
                       weight_f16.data(), 0u, 1);
...
bool compute_attention_with_ggml_nonflash_f16(
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      float score = 0.0f;
      ggml_vec_dot_f16(head_dim, &score, 0u, k_f16.data(), 0u, q_f16.data(), 0u,
                       1);
      score *= inv_scale;
      backend.attn_scores[static_cast<size_t>(position)] = score;
      max_score = std::max(max_score, score);
...
bool compute_attention_with_ggml_softmax(
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    const reference_ggml_float score_sum =
        ggml_vec_soft_max_f32(position_limit, backend.attn_probs.data(),
                              backend.attn_scores.data(), max_score);
    const float inv_score_sum =
        score_sum == 0.0 ? 0.0f : static_cast<float>(1.0 / score_sum);
    for (int32_t position = 0; position < position_limit; ++position) {
      backend.attn_probs[static_cast<size_t>(position)] *= inv_score_sum;
...
bool compute_attention_with_rounded_weight_scalar(
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      const float weight =
          kernel_quant::fp16_to_fp32(kernel_quant::fp32_to_fp16(
              backend.attn_probs[static_cast<size_t>(position)] / score_sum));
      const size_t cache_offset = emel::generator::detail::layer_cache_offset(
                                      backend, layer_index, position, kv_dim) +
                                  kv_offset;
...
bool compute_attention_with_ggml_flash_ext_helper(
    ...
    const int32_t position_limit, std::span<const float> q_vector) {
  const int32_t kv_dim = backend.n_head_kv * backend.head_dim_kv;
  const size_t layer_offset = emel::generator::detail::layer_cache_offset(
      backend, layer_index, 0, kv_dim);
  std::vector<float> attn_ctx;
  if (!run_ggml_flash_attn_ext_case(
          q_vector,
          std::span<const uint16_t>(
              backend.key_cache.data() + layer_offset,
              static_cast<size_t>(position_limit * kv_dim)),
          std::span<const uint16_t>(
              backend.value_cache.data() + layer_offset,
              static_cast<size_t>(position_limit * kv_dim)),
          ...
          1.0f / std::sqrt(static_cast<float>(backend.head_dim)), attn_ctx)) {
    return false;
  }
...
inline void scale_fp16_buffer(std::span<ggml_fp16_t> values,
                              const float scale) {
...
inline void mad_fp16_buffer(std::span<ggml_fp16_t> accum,
  ...
  for (size_t idx = 0; idx < accum.size(); ++idx) {
    const float sum = ggml_fp16_to_fp32(accum[idx]) +
                      ggml_fp16_to_fp32(values[idx]) * rounded_scale;
...
bool run_layer_with_scalar_attention_ggml_online_f16(
  ...
  if (!compute_attention_with_ggml_online_f16(backend, layer_index,
                                              position + 1, backend.q_attn) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
    return false;
...
bool run_layer_with_scalar_attention_online_f32(
  ...
  if (!compute_attention_with_online_f32(backend, layer_index, position + 1,
                                         backend.q_attn) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
...
bool run_layer_with_scalar_attention_ggml_softmax(
  ...
  if (!compute_attention_with_ggml_softmax(backend, layer_index, position + 1,
                                           backend.q_attn) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
...
bool run_layer_with_scalar_attention_ggml_flash_ext(
  ...
  if (!compute_attention_with_ggml_flash_ext_helper(
          backend, layer_index, position + 1,
          std::span<const float>(backend.q.data(),
                                 static_cast<size_t>(backend.n_embd))) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
...
bool run_prefill_with_scalar_attention_ggml_f16_value_contraction(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_ggml_f16_value_contraction(
...
bool run_prefill_with_scalar_attention_no_weight_rounding(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_no_weight_rounding(backend, layer,
                                                              position)) {
        return false;
      }
    }
...
bool run_prefill_with_scalar_attention_rounded_weight_scalar(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_rounded_weight_scalar(backend, layer,
                                                                 position)) {
        return false;
      }
    }
...
bool run_prefill_with_scalar_attention_ggml_online_f16(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_ggml_online_f16(backend, layer,
                                                           position)) {
        return false;
      }
    }
...
bool run_prefill_with_scalar_attention_online_f32(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_online_f32(backend, layer,
                                                      position)) {
        return false;
      }
    }
...
bool run_prefill_with_scalar_attention_ggml_flash_ext(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_ggml_flash_ext(backend, layer,
                                                          position)) {
        return false;
      }
    }
...
bool run_prefill_with_scalar_attention_double_softmax_sum(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_double_softmax_sum(backend, layer,
                                                              position)) {
        return false;
      }
    }
...
bool run_prefill_with_scalar_attention_ggml_softmax(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_ggml_softmax(backend, layer,
                                                        position)) {
        return false;
      }
    }
...
bool run_layer_with_scalar_attention_ggml_nonflash_f16(
  ...
  if (!compute_attention_with_ggml_nonflash_f16(backend, layer_index,
                                                position + 1, backend.q_attn) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
...
bool run_prefill_with_scalar_attention_ggml_nonflash_f16(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_ggml_nonflash_f16(backend, layer,
                                                             position)) {
        return false;
      }
    }
...
bool run_layer_with_scalar_attention_ggml_f16_scores(
  ...
  if (!compute_attention_with_ggml_f16_scores(backend, layer_index,
                                              position + 1, backend.q_attn) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
...
bool run_prefill_with_scalar_attention_ggml_f16_scores(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_ggml_f16_scores(backend, layer,
                                                           position)) {
        return false;
      }
    }
...
bool run_layer_with_scalar_attention_emel_prod_style(
  ...
  if (!compute_attention_with_emel_prod_style(backend, layer_index,
                                              position + 1, backend.q_attn) ||
      !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
...
bool run_prefill_with_scalar_attention_emel_prod_style(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_emel_prod_style(backend, layer,
                                                           position)) {
        return false;
      }
    }
...
bool run_prefill_with_scalar_attention_emel_prod_style_float_value(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_emel_prod_style_float_value(
              backend, layer, position)) {
        return false;
      }
    }
...
bool run_layer_with_scalar_attention_full_q(
    emel::generator::detail::native_backend &backend, const int32_t layer_index,
    const int32_t position) {
  auto &block = backend.blocks[static_cast<size_t>(layer_index)];
  if (!emel::generator::detail::rms_norm(backend.hidden, block.attention_norm,
...
bool run_prefill_with_scalar_attention_full_q(
    emel::generator::detail::native_backend &backend,
    std::span<const int32_t> prefix_tokens) {
  if (prefix_tokens.empty()) {
    return false;
...
bool run_prefill_with_scalar_attention_full_q_rounded_weight(
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention_full_q_rounded_weight(backend, layer,
                                                                 position)) {
        return false;
      }
    }
...
void dump_candidate_logits_compare(
    const char *label, const emel::generator::detail::native_backend &backend,
    const int32_t token_a, const int32_t token_b);
...
struct reference_graph_capture {
  std::array<reference_tensor_capture, 33> entries = {{
      {"attn_norm-0", {}},   {"Qcur-0", {}},       {"Kcur-0", {}},
      {"Vcur-0", {}},        {"kq-0", {}},         {"kq_soft_max-0", {}},
      {"kqv-0", {}},         {"kqv_out-0", {}},    {"attn_out-0", {}},
      {"ffn_inp-0", {}},     {"ffn_norm-0", {}},   {"ffn_gate-0", {}},
      {"ffn_up-0", {}},      {"ffn_swiglu-0", {}}, {"ffn_out-0", {}},
      {"l_out-0", {}},       {"attn_norm-1", {}},  {"Qcur-1", {}},
      {"Kcur-1", {}},        {"Vcur-1", {}},       {"kq-1", {}},
      {"kq_soft_max-1", {}}, {"kqv-1", {}},        {"kqv_out-1", {}},
      {"attn_out-1", {}},    {"ffn_inp-1", {}},    {"ffn_norm-1", {}},
      {"ffn_gate-1", {}},    {"ffn_up-1", {}},     {"ffn_swiglu-1", {}},
      {"ffn_out-1", {}},     {"l_out-1", {}},      {"result_norm", {}},
  }};
};

std::span<const float>
find_reference_tensor(const reference_graph_capture &capture, const char *name);
std::span<const float>
reference_last_token_row(std::span<const float> reference_values,
                         size_t row_width);
bool capture_reference_graph_for_tokens(
    const generation_load_state &state,
    std::span<const llama_token> prompt_tokens,
    reference_graph_capture &graph_capture);
bool capture_reference_graph_for_generation_prefix(
    const generation_load_state &state,
    std::span<const llama_token> prompt_tokens,
    std::span<const int32_t> generated_tokens,
    reference_graph_capture &graph_capture);
void dump_state_compare(const char *label, std::span<const float> emel_values,
...
void dump_prompt0_reference_attn_out_ffn_debug(
  ...
  if (!capture_reference_graph_for_generation_prefix(
          state, std::span<const llama_token>(prompt_tokens.data(), 1u),
          std::span<const int32_t>{}, graph_capture)) {
    std::fprintf(stdout, "generation_debug.reference_attn_out.prompt0.layer0: "
  ...
  if (!emel::generator::detail::copy_tensor_row(*backend.token_embedding.tensor,
                                                token_id, backend.hidden)) {
    std::fprintf(stdout, "generation_debug.reference_attn_out.prompt0.layer0: "
                         "token embedding replay failed\n");
    return;
  }

  auto dump_ffn_matmul_modes =
      [&](const char *label_base,
        ...
        if (matmul_vector_reference_q8(matrix, input, reference_q8_values)) {
          dump_state_compare(
              (std::string(label_base) + ".reference_q8").c_str(),
              reference_q8_values, reference_values);
        }

        std::vector<float> scalar_quant_values(reference_values.size(), 0.0f);
        if (matmul_vector_scalar_quantized(matrix, input,
                                           scalar_quant_values)) {
          dump_state_compare(
              (std::string(label_base) + ".scalar_quantized").c_str(),
              scalar_quant_values, reference_values);
        }
      };

  const auto reference_row = [&](const char *key, const size_t width) {
    return reference_last_token_row(find_reference_tensor(graph_capture, key),
                                    width);
  };
  ...
  const auto build_single_token_attn_ctx = [&](const auto value_cache_row) {
    ...
    for (int32_t head = 0; head < backend.n_head; ++head) {
      ...
      const size_t cache_row_offset = static_cast<size_t>(kv_head) *
                                      static_cast<size_t>(backend.head_dim_kv);
  ...
  if (reference_ctx != nullptr &&
    ...
    std::vector<float> reference_value_cache_rows;
    if (capture_reference_value_cache_rows(reference_ctx.get(), 0,
                                           reference_value_cache_rows) &&
      ...
      expected_reference_attn_ctx =
          build_single_token_attn_ctx(reference_value_cache_row);
      dump_state_compare("generation_debug.reference_attn_out.prompt0.layer0."
                         "value_cache.runtime",
                         runtime_value_cache_row, reference_value_cache_row);
      ...
      auto dump_value_cache_mode = [&](const char *label,
        ...
        emel::generator::detail::store_fp16_rounded_cache(
            values, rounded_values.data());
        dump_state_compare(label, rounded_values, reference_value_cache_row);
      };

      dump_value_cache_mode(
          "generation_debug.reference_attn_out.prompt0.layer0.value_cache."
          "exact",
          [&](const emel::generator::detail::tensor_matrix &matrix,
              std::span<const float> input, std::span<float> output) {
            return matmul_vector_dequantized(matrix, input, output);
  ...
  dump_state_compare("generation_debug.reference_attn_out.prompt0.layer0.attn_"
                     "ctx.runtime_value_cache",
                     backend.attn_ctx, expected_runtime_attn_ctx);
  if (!expected_reference_attn_ctx.empty()) {
    dump_state_compare("generation_debug.reference_attn_out.prompt0.layer0."
                       "attn_ctx.reference_value_cache",
                       backend.attn_ctx, expected_reference_attn_ctx);
  }
  dump_state_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.attn_out.runtime",
      backend.projected, reference_attn_out);
  dump_ffn_matmul_modes(
      "generation_debug.reference_attn_out.prompt0.layer0.attn_out",
      block.attention_output, expected_runtime_attn_ctx, reference_attn_out);

  std::copy(reference_attn_out.begin(), reference_attn_out.end(),
            backend.projected.begin());
  for (int32_t idx = 0; idx < backend.n_embd; ++idx) {
    backend.hidden[static_cast<size_t>(idx)] +=
        backend.projected[static_cast<size_t>(idx)];
  ...
  dump_state_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_inp",
      backend.hidden, reference_ffn_inp);
  ...
  dump_state_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_norm",
      backend.norm, reference_ffn_norm);
  dump_q8_quantize_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_norm_q8",
      backend.norm);
  dump_state_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_gate",
      backend.gate, reference_ffn_gate);
  dump_state_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_up", backend.up,
      reference_ffn_up);
  dump_ffn_matmul_modes(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_gate",
      block.feed_forward_gate, backend.norm, reference_ffn_gate);
  dump_ffn_matmul_modes(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_up",
      block.feed_forward_up, backend.norm, reference_ffn_up);
  ...
  dump_state_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_swiglu",
      backend.ffn_hidden, reference_ffn_swiglu);
  dump_q8_quantize_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_hidden_q8",
      backend.ffn_hidden);
  ...
  dump_state_compare(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_out",
      backend.projected, reference_ffn_out);
  dump_ffn_matmul_modes(
      "generation_debug.reference_attn_out.prompt0.layer0.ffn_out",
      block.feed_forward_down, backend.ffn_hidden, reference_ffn_out);
  ...
  dump_state_compare("generation_debug.reference_attn_out.prompt0.layer0.l_out",
                     backend.hidden, reference_l_out);
}
...
void dump_matrix_compare_reference_q8(
...
float ggml_row_dot(const emel::generator::detail::tensor_matrix &matrix,
                   uint32_t row, const kernel_quant::block_q8_k *q8_blocks,
                   uint64_t block_count);

void dump_generation_selected_step_stage_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &reference_result, const int32_t generated_index) {
  if (state.model_data == nullptr || generated_index < 0 ||
      generated_index >=
          static_cast<int32_t>(reference_result.trace.token_ids.size())) {
    return;
  }
  ...
  if (!capture_reference_graph_for_generation_prefix(
    ...
    std::fprintf(stdout,
                 "generation_debug.gen_step: reference graph capture failed\n");
    return;
  }
  ...
  if (reference_ctx == nullptr ||
      !run_reference_prefix_decode(
          reference_ctx.get(), prompt_tokens,
          std::span<const int32_t>(generated_prefix))) {
    std::fprintf(stdout,
                 "generation_debug.gen_step: reference decode failed\n");
    return;
  }
  ...
  const bool have_layer0_key_cache = capture_reference_key_cache_rows(
      reference_ctx.get(), 0, reference_layer0_key_cache);
  const bool have_layer0_value_cache = capture_reference_value_cache_rows(
      reference_ctx.get(), 0, reference_layer0_value_cache);
  const bool have_layer1_key_cache = capture_reference_key_cache_rows(
      reference_ctx.get(), 1, reference_layer1_key_cache);
  const bool have_layer1_value_cache = capture_reference_value_cache_rows(
      reference_ctx.get(), 1, reference_layer1_value_cache);
  ...
  prefix_tokens.insert(prefix_tokens.end(), generated_prefix.begin(),
  ...
  if (prefix_tokens.size() > 1u) {
    const std::span<const int32_t> prior_tokens{prefix_tokens.data(),
                                                prefix_tokens.size() - 1u};
    if (!run_prefill_from_token_prefix(backend, prior_tokens)) {
      std::fprintf(stdout,
                   "generation_debug.gen_step: prior prefix replay failed\n");
      return;
    }
  } else {
    backend.kv_cache_tokens = 0;
  ...
  if (!emel::generator::detail::copy_tensor_row(*backend.token_embedding.tensor,
                                                token_id, backend.hidden)) {
    std::fprintf(stdout,
                 "generation_debug.gen_step: token embedding replay failed\n");
    return;
  }
  ...
  std::fprintf(
      ...
      generated_index, prefix_tokens.size());

  const auto dump_cache_rows_compare = [](const std::string &label_prefix,
                                          const char *suffix,
    ...
    std::fprintf(stdout,
                 ...
                 label_prefix.c_str(), suffix, max_abs, max_pos, max_dim,
                 max_emel, max_reference);
  };

  const auto dump_kernel_row_compare =
      [&](const std::string &label_prefix, const char *suffix,
          const emel::generator::detail::tensor_matrix &matrix,
          std::span<const float> input, const uint32_t row,
          std::span<const float> reference_values) {
        std::array<kernel_quant::block_q8_k, kernel_quant::MAX_Q8_K_BLOCKS>
            q8_blocks = {};
        uint64_t block_count = 0;
        ...
        const auto *row_ptr =
            ...
            static_cast<size_t>(row) *
                emel::generator::detail::row_storage_bytes(*matrix.tensor,
                                                           matrix.cols);
        const float ggml_value =
            ggml_row_dot(matrix, row, q8_blocks.data(), block_count);
        ...
        case emel::kernel::event::dtype::q2_k: {
          const auto *lhs =
              reinterpret_cast<const kernel_quant::block_q2_k *>(row_ptr);
          scalar_value = emel::kernel::detail::dot_q2_k_q8_k_row_scalar(
        ...
        case emel::kernel::event::dtype::q3_k: {
          const auto *lhs =
              reinterpret_cast<const kernel_quant::block_q3_k *>(row_ptr);
          scalar_value = emel::kernel::detail::dot_q3_k_q8_k_row_scalar(
              lhs, q8_blocks.data(), block_count);
#if defined(__aarch64__) || defined(__ARM_NEON)
        ...
        case emel::kernel::event::dtype::q6_k: {
          const auto *lhs =
              reinterpret_cast<const kernel_quant::block_q6_k *>(row_ptr);
          scalar_value = emel::kernel::detail::dot_q6_k_q8_k_row_scalar(
              lhs, q8_blocks.data(), block_count);
#if defined(__aarch64__) || defined(__ARM_NEON)
        ...
        const float reference_value =
            row < reference_values.size()
                ? reference_values[static_cast<size_t>(row)]
        ...
        std::fprintf(
            ...
            label_prefix.c_str(), suffix, row,
            emel_out[static_cast<size_t>(row)],
            kernel_quant::fp32_to_bits(emel_out[static_cast<size_t>(row)]),
            ggml_value, kernel_quant::fp32_to_bits(ggml_value), scalar_value,
            kernel_quant::fp32_to_bits(scalar_value), neon_value,
            kernel_quant::fp32_to_bits(neon_value), reference_value,
  ...
  for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
    const std::string label_prefix = "generation_debug.gen_step.gen" +
    ...
    if (const std::span<const float> reference_attn_norm =
            reference_stage_row(("attn_norm-" + std::to_string(layer)).c_str(),
                                backend.norm.size());
        !reference_attn_norm.empty()) {
      dump_state_compare((label_prefix + ".attn_norm").c_str(), backend.norm,
                         reference_attn_norm);
    }
    dump_q8_quantize_compare((label_prefix + ".attn_norm_q8").c_str(),
                             backend.norm);
    dump_matrix_compare((label_prefix + ".attn_q_matmul").c_str(), backend,
                        block.attention_q, backend.norm);
    dump_matrix_compare_reference_q8(
        (label_prefix + ".attn_q_matmul_refq8").c_str(), backend,
        block.attention_q, backend.norm);
    dump_matrix_compare((label_prefix + ".attn_k_matmul").c_str(), backend,
                        block.attention_k, backend.norm);
    dump_matrix_compare_reference_q8(
        (label_prefix + ".attn_k_matmul_refq8").c_str(), backend,
        block.attention_k, backend.norm);
    dump_matrix_compare((label_prefix + ".attn_v_matmul").c_str(), backend,
                        block.attention_v, backend.norm);
    dump_matrix_compare_reference_q8(
        (label_prefix + ".attn_v_matmul_refq8").c_str(), backend,
        block.attention_v, backend.norm);

    if (!emel::generator::detail::matmul_vector(backend, block.attention_q,
                                                backend.norm, backend.q) ||
        !emel::generator::detail::matmul_vector(backend, block.attention_k,
                                                backend.norm, backend.k) ||
        !emel::generator::detail::matmul_vector(backend, block.attention_v,
    ...
    if (const std::span<const float> reference_q = reference_stage_row(
            ("Qcur-" + std::to_string(layer)).c_str(), backend.q.size());
        !reference_q.empty()) {
      dump_state_compare((label_prefix + ".q").c_str(), backend.q, reference_q);
    }
    if (const std::span<const float> reference_k = reference_stage_row(
            ("Kcur-" + std::to_string(layer)).c_str(), backend.k.size());
        !reference_k.empty()) {
      dump_state_compare((label_prefix + ".k").c_str(), backend.k, reference_k);

      std::vector<float> rounded_reference_k(reference_k.begin(),
                                             reference_k.end());
      emel::generator::detail::store_fp16_rounded_cache(
          reference_k, rounded_reference_k.data());
      if (layer == 0 && have_layer0_key_cache &&
          reference_layer0_key_cache.size() >=
              layer_cache_row_offset + static_cast<size_t>(backend.head_dim_kv *
                                                           backend.n_head_kv)) {
        dump_state_compare(
            (label_prefix + ".k.reference_round_to_cache").c_str(),
            rounded_reference_k,
            std::span<const float>(
                reference_layer0_key_cache.data() + layer_cache_row_offset,
                static_cast<size_t>(backend.head_dim_kv * backend.n_head_kv)));
      }
      if (layer == 1 && have_layer1_key_cache &&
        ...
        dump_state_compare(
            ...
            rounded_reference_k,
            std::span<const float>(
                reference_layer1_key_cache.data() + layer_cache_row_offset,
                static_cast<size_t>(backend.head_dim_kv * backend.n_head_kv)));
      }
    }
    if (const std::span<const float> reference_v = reference_stage_row(
            ("Vcur-" + std::to_string(layer)).c_str(), backend.v.size());
        !reference_v.empty()) {
      dump_state_compare((label_prefix + ".v").c_str(), backend.v, reference_v);

      std::vector<float> rounded_reference_v(reference_v.begin(),
                                             reference_v.end());
      emel::generator::detail::store_fp16_rounded_cache(
          reference_v, rounded_reference_v.data());
      if (layer == 0 && have_layer0_value_cache &&
          reference_layer0_value_cache.size() >=
              layer_cache_row_offset + static_cast<size_t>(backend.head_dim_kv *
                                                           backend.n_head_kv)) {
        dump_state_compare(
            (label_prefix + ".v.reference_round_to_cache").c_str(),
            rounded_reference_v,
            std::span<const float>(
                reference_layer0_value_cache.data() + layer_cache_row_offset,
                static_cast<size_t>(backend.head_dim_kv * backend.n_head_kv)));
      }
      if (layer == 1 && have_layer1_value_cache &&
        ...
        dump_state_compare(
            ...
            rounded_reference_v,
            std::span<const float>(
                reference_layer1_value_cache.data() + layer_cache_row_offset,
                static_cast<size_t>(backend.head_dim_kv * backend.n_head_kv)));
      }

      if (generated_index == 2 && layer == 0) {
        dump_kernel_row_compare(label_prefix, "attn_v_row319",
                                block.attention_v, backend.norm, 319u,
                                reference_v);
      }
    }
    if (generated_index == 2 && layer == 1) {
      if (const std::span<const float> reference_k = reference_stage_row(
              ("Kcur-" + std::to_string(layer)).c_str(), backend.k.size());
          !reference_k.empty()) {
        dump_kernel_row_compare(label_prefix, "attn_k_row475",
                                block.attention_k, backend.norm, 475u,
                                reference_k);
      }
    }
    ...
    if (have_reference_key_cache && reference_key_cache != nullptr &&
      ...
      dump_cache_rows_compare(
          label_prefix, "key_cache_prior",
          std::span<const uint16_t>(
              backend.key_cache.data() + layer_cache_base_offset,
              static_cast<size_t>(position) * static_cast<size_t>(kv_dim)),
    ...
    if (have_reference_value_cache && reference_value_cache != nullptr &&
        ...
        reference_value_cache->size() >=
            static_cast<size_t>(position) * static_cast<size_t>(kv_dim)) {
      dump_cache_rows_compare(
          label_prefix, "value_cache_prior",
          std::span<const uint16_t>(
              backend.value_cache.data() + layer_cache_base_offset,
              static_cast<size_t>(position) * static_cast<size_t>(kv_dim)),
    ...
    if (layer == 0 && have_layer0_key_cache &&
        reference_layer0_key_cache.size() >=
            layer_cache_row_offset + static_cast<size_t>(kv_dim)) {
      dump_state_compare(
          (label_prefix + ".key_cache").c_str(),
    ...
    if (layer == 0 && have_layer0_value_cache &&
        reference_layer0_value_cache.size() >=
            layer_cache_row_offset + static_cast<size_t>(kv_dim)) {
      dump_state_compare(
          (label_prefix + ".value_cache").c_str(),
    ...
    const std::span<const float> reference_kqv_out = reference_stage_row(
        ("kqv_out-" + std::to_string(layer)).c_str(), backend.attn_ctx.size());
    if (!reference_kqv_out.empty()) {
      const size_t layer_cache_offset =
          emel::generator::detail::layer_cache_offset(backend, layer, 0,
                                                      kv_dim);
      ...
      if (run_ggml_nonflash_attn_case(
              q_attn_rows, key_rows, value_rows,
              static_cast<int64_t>(backend.head_dim),
              static_cast<int64_t>(backend.n_ctx),
        ...
        dump_state_compare(
            (label_prefix + ".kqv_out.ggml_nonflash_exact").c_str(),
            ggml_nonflash_exact_ctx, reference_kqv_out);
      } else {
        std::fprintf(stdout, "%s.kqv_out.ggml_nonflash_exact: replay failed\n",
                     label_prefix.c_str());
      }
      ...
      if (run_emel_nonflash_f16_ggml_softmax_case(
        ...
        dump_state_compare(
            (label_prefix + ".kqv_out.emel_nonflash_f16").c_str(),
            emel_nonflash_f16_ctx, reference_kqv_out);
        if (!ggml_nonflash_exact_ctx.empty()) {
          dump_state_compare(
              (label_prefix + ".kqv_out.emel_vs_exact_nonflash").c_str(),
              emel_nonflash_f16_ctx, ggml_nonflash_exact_ctx);
        }
      } else {
        std::fprintf(stdout, "%s.kqv_out.emel_nonflash_f16: replay failed\n",
                     label_prefix.c_str());
      }
    }
    ...
    if (!reference_kqv_out.empty()) {
      dump_state_compare((label_prefix + ".kqv_out").c_str(), backend.attn_ctx,
                         reference_kqv_out);
    }
    if (const std::span<const float> reference_attn_out =
            reference_stage_row(("attn_out-" + std::to_string(layer)).c_str(),
                                backend.projected.size());
        !reference_attn_out.empty()) {
      dump_state_compare((label_prefix + ".attn_out").c_str(),
                         backend.projected, reference_attn_out);
    }
    ...
    if (const std::span<const float> reference_ffn_inp =
            reference_stage_row(("ffn_inp-" + std::to_string(layer)).c_str(),
                                backend.hidden.size());
        !reference_ffn_inp.empty()) {
      dump_state_compare((label_prefix + ".ffn_inp").c_str(), backend.hidden,
                         reference_ffn_inp);
    }
    ...
    if (const std::span<const float> reference_ffn_norm = reference_stage_row(
            ("ffn_norm-" + std::to_string(layer)).c_str(), backend.norm.size());
        !reference_ffn_norm.empty()) {
      dump_state_compare((label_prefix + ".ffn_norm").c_str(), backend.norm,
                         reference_ffn_norm);
    }
    dump_q8_quantize_compare((label_prefix + ".ffn_norm_q8").c_str(),
                             backend.norm);
    dump_matrix_compare((label_prefix + ".ffn_gate_matmul").c_str(), backend,
                        block.feed_forward_gate, backend.norm);
    dump_matrix_compare_reference_q8(
        (label_prefix + ".ffn_gate_matmul_refq8").c_str(), backend,
        block.feed_forward_gate, backend.norm);
    dump_matrix_compare((label_prefix + ".ffn_up_matmul").c_str(), backend,
                        block.feed_forward_up, backend.norm);
    dump_matrix_compare_reference_q8(
        (label_prefix + ".ffn_up_matmul_refq8").c_str(), backend,
        block.feed_forward_up, backend.norm);
    if (const std::span<const float> reference_ffn_gate = reference_stage_row(
            ("ffn_gate-" + std::to_string(layer)).c_str(), backend.gate.size());
        !reference_ffn_gate.empty()) {
      dump_state_compare((label_prefix + ".ffn_gate").c_str(), backend.gate,
                         reference_ffn_gate);
    }
    if (const std::span<const float> reference_ffn_up = reference_stage_row(
            ("ffn_up-" + std::to_string(layer)).c_str(), backend.up.size());
        !reference_ffn_up.empty()) {
      dump_state_compare((label_prefix + ".ffn_up").c_str(), backend.up,
                         reference_ffn_up);
    }
    ...
    if (const std::span<const float> reference_ffn_swiglu =
      ...
      dump_state_compare((label_prefix + ".ffn_swiglu").c_str(),
                         backend.ffn_hidden, reference_ffn_swiglu);
    }
    if (!emel::generator::detail::matmul_vector(
            backend, block.feed_forward_down, backend.ffn_hidden,
    ...
    dump_q8_quantize_compare((label_prefix + ".ffn_hidden_q8").c_str(),
                             backend.ffn_hidden);
    dump_matrix_compare((label_prefix + ".ffn_down_matmul").c_str(), backend,
                        block.feed_forward_down, backend.ffn_hidden);
    dump_matrix_compare_reference_q8(
        (label_prefix + ".ffn_down_matmul_refq8").c_str(), backend,
        block.feed_forward_down, backend.ffn_hidden);
    if (const std::span<const float> reference_ffn_out =
            reference_stage_row(("ffn_out-" + std::to_string(layer)).c_str(),
                                backend.projected.size());
        !reference_ffn_out.empty()) {
      dump_state_compare((label_prefix + ".ffn_out").c_str(), backend.projected,
                         reference_ffn_out);

      std::vector<float> exact_ffn_out(backend.projected.size(), 0.0f);
      if (matmul_vector_dequantized(block.feed_forward_down, backend.ffn_hidden,
                                    exact_ffn_out)) {
        dump_state_compare((label_prefix + ".ffn_out.exact").c_str(),
                           exact_ffn_out, reference_ffn_out);
      }

      std::vector<float> reference_q8_ffn_out(backend.projected.size(), 0.0f);
      if (matmul_vector_reference_q8(block.feed_forward_down,
                                     backend.ffn_hidden,
                                     reference_q8_ffn_out)) {
        dump_state_compare((label_prefix + ".ffn_out.reference_q8").c_str(),
                           reference_q8_ffn_out, reference_ffn_out);
      }

      std::vector<float> scalar_quant_ffn_out(backend.projected.size(), 0.0f);
      if (matmul_vector_scalar_quantized(block.feed_forward_down,
                                         backend.ffn_hidden,
                                         scalar_quant_ffn_out)) {
        dump_state_compare((label_prefix + ".ffn_out.scalar_quantized").c_str(),
                           scalar_quant_ffn_out, reference_ffn_out);
      }
    }
    ...
    if (const std::span<const float> reference_l_out = reference_stage_row(
            ("l_out-" + std::to_string(layer)).c_str(), backend.hidden.size());
        !reference_l_out.empty()) {
      dump_state_compare((label_prefix + ".l_out").c_str(), backend.hidden,
                         reference_l_out);
    }
  }
}

void dump_scalar_attention_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result) {
  const int32_t token_mismatch_index =
      first_token_mismatch_index(emel_result, reference_result);
  ...
  if (token_mismatch_index >= 299) {
    ...
    emel::generator::detail::native_backend
        emel_prod_style_float_value_backend = {};
    ...
    emel::generator::detail::native_backend
        ggml_f16_scores_ggml_softmax_backend = {};
    emel::generator::detail::native_backend ggml_nonflash_f16_backend = {};
    emel::generator::detail::native_backend
        ggml_nonflash_f16_ggml_softmax_backend = {};
    ...
    const exact_matmul_mode exact_all{
        .attention = true, .ffn = true, .output = true};
    const exact_matmul_mode exact_attention_only{
        .attention = true, .ffn = false, .output = false};
    const exact_matmul_mode exact_ffn_only{
        .attention = false, .ffn = true, .output = false};
    const exact_matmul_mode exact_output_only{
        .attention = false, .ffn = false, .output = true};
    ...
    if (emel::generator::detail::prepare(dispatch_backend, *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(runtime_flash_q_attn_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(emel_prod_style_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(emel_prod_style_float_value_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(score_dot_probe_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(full_q_backend, *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(f32_attention_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(rounded_weight_attention_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(exact_backend, *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(attention_exact_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(ffn_exact_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(output_exact_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(ggml_f16_value_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(ggml_online_f16_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(ggml_f16_scores_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(ggml_f16_scores_ggml_softmax_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(ggml_nonflash_f16_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(ggml_nonflash_f16_ggml_softmax_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        ...
        emel::generator::detail::prepare(q6_exact_backend, *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(q2_scalar_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(q3_scalar_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(q6_scalar_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(q2_reference_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        emel::generator::detail::prepare(q3_reference_backend,
                                         *state.model_data) !=
            emel::error::cast(emel::model::loader::error::none) ||
        !run_prefill_from_token_prefix(dispatch_backend, prefix_tokens) ||
        !run_prefill_with_flash_request_q_attn(runtime_flash_q_attn_backend,
                                               prefix_tokens) ||
        !run_prefill_with_scalar_attention_emel_prod_style(
            emel_prod_style_backend, prefix_tokens) ||
        ...
        !run_prefill_with_scalar_attention_full_q(full_q_backend,
                                                  prefix_tokens) ||
        ...
        !run_prefill_with_scalar_attention_matmul_mode(
            exact_backend, prefix_tokens, exact_all) ||
        !run_prefill_with_scalar_attention_matmul_mode(
      ...
      std::fprintf(stdout,
                   "generation_debug.long: unable to replay mismatch prefix\n");
      return;
    }

    const argmax_summary dispatch_summary = select_argmax_from_logits(
        dispatch_backend.bound_logits.data(), dispatch_backend.n_vocab);
    const argmax_summary runtime_flash_q_attn_summary =
        select_argmax_from_logits(
            runtime_flash_q_attn_backend.bound_logits.data(),
            runtime_flash_q_attn_backend.n_vocab);
    const argmax_summary emel_prod_style_summary =
        select_argmax_from_logits(emel_prod_style_backend.bound_logits.data(),
                                  emel_prod_style_backend.n_vocab);
    const argmax_summary emel_prod_style_float_value_summary =
        select_argmax_from_logits(
            emel_prod_style_float_value_backend.bound_logits.data(),
            emel_prod_style_float_value_backend.n_vocab);
    const argmax_summary full_q_summary = select_argmax_from_logits(
        full_q_backend.bound_logits.data(), full_q_backend.n_vocab);
    const argmax_summary f32_attention_summary =
        select_argmax_from_logits(f32_attention_backend.bound_logits.data(),
                                  f32_attention_backend.n_vocab);
    const argmax_summary rounded_weight_attention_summary =
        select_argmax_from_logits(
            rounded_weight_attention_backend.bound_logits.data(),
            rounded_weight_attention_backend.n_vocab);
    const argmax_summary exact_summary = select_argmax_from_logits(
        exact_backend.bound_logits.data(), exact_backend.n_vocab);
    const argmax_summary attention_exact_summary =
        select_argmax_from_logits(attention_exact_backend.bound_logits.data(),
                                  attention_exact_backend.n_vocab);
    const argmax_summary ffn_exact_summary = select_argmax_from_logits(
        ffn_exact_backend.bound_logits.data(), ffn_exact_backend.n_vocab);
    ...
    const argmax_summary ggml_f16_value_summary =
        select_argmax_from_logits(ggml_f16_value_backend.bound_logits.data(),
                                  ggml_f16_value_backend.n_vocab);
    const argmax_summary ggml_online_f16_summary =
        select_argmax_from_logits(ggml_online_f16_backend.bound_logits.data(),
                                  ggml_online_f16_backend.n_vocab);
    const argmax_summary ggml_f16_scores_summary =
        select_argmax_from_logits(ggml_f16_scores_backend.bound_logits.data(),
                                  ggml_f16_scores_backend.n_vocab);
    const argmax_summary ggml_f16_scores_ggml_softmax_summary =
        select_argmax_from_logits(
            ggml_f16_scores_ggml_softmax_backend.bound_logits.data(),
            ggml_f16_scores_ggml_softmax_backend.n_vocab);
    const argmax_summary ggml_nonflash_f16_summary =
        select_argmax_from_logits(ggml_nonflash_f16_backend.bound_logits.data(),
                                  ggml_nonflash_f16_backend.n_vocab);
    const argmax_summary ggml_nonflash_f16_ggml_softmax_summary =
        select_argmax_from_logits(
            ggml_nonflash_f16_ggml_softmax_backend.bound_logits.data(),
            ggml_nonflash_f16_ggml_softmax_backend.n_vocab);
    const argmax_summary q2_exact_summary = select_argmax_from_logits(
        q2_exact_backend.bound_logits.data(), q2_exact_backend.n_vocab);
    const argmax_summary q3_exact_summary = select_argmax_from_logits(
        q3_exact_backend.bound_logits.data(), q3_exact_backend.n_vocab);
    const argmax_summary q6_exact_summary = select_argmax_from_logits(
        q6_exact_backend.bound_logits.data(), q6_exact_backend.n_vocab);
    const argmax_summary q2_scalar_summary = select_argmax_from_logits(
        q2_scalar_backend.bound_logits.data(), q2_scalar_backend.n_vocab);
    const argmax_summary q3_scalar_summary = select_argmax_from_logits(
        q3_scalar_backend.bound_logits.data(), q3_scalar_backend.n_vocab);
    const argmax_summary q6_scalar_summary = select_argmax_from_logits(
        q6_scalar_backend.bound_logits.data(), q6_scalar_backend.n_vocab);
    ...
    const int32_t reference_token =
        reference_result.trace
            .token_ids[static_cast<size_t>(token_mismatch_index)];
    std::fprintf(
        stdout,
        ...
        prefix_tokens.size(), dispatch_summary.selected_token,
        ...
        full_q_summary.selected_token, f32_attention_summary.selected_token,
        rounded_weight_attention_summary.selected_token,
        exact_summary.selected_token, attention_exact_summary.selected_token,
        ffn_exact_summary.selected_token, output_exact_summary.selected_token,
        ...
        q2_exact_summary.selected_token, q3_exact_summary.selected_token,
        q6_exact_summary.selected_token, q2_scalar_summary.selected_token,
        q3_scalar_summary.selected_token, q6_scalar_summary.selected_token,
        q2_reference_summary.selected_token,
        q3_reference_summary.selected_token, reference_token);
    std::fprintf(
        stdout,
        "generation_debug.long.score_dot: first_abs=%g token=%d layer=%d "
        "head=%d "
        "position=%d emel=%g reference=%g max_abs=%g max_token=%d max_layer=%d "
        "max_head=%d max_position=%d max_emel=%g max_reference=%g\n",
        score_dot_probe.first_abs, score_dot_probe.first_token,
        score_dot_probe.first_layer, score_dot_probe.first_head,
        score_dot_probe.first_position, score_dot_probe.first_emel,
        score_dot_probe.first_reference, score_dot_probe.max_abs,
        score_dot_probe.max_token, score_dot_probe.max_layer,
        score_dot_probe.max_head, score_dot_probe.max_position,
        score_dot_probe.max_emel, score_dot_probe.max_reference);
    dump_generation_prefix_timeline_debug(state, opts, emel_result,
                                          reference_result);
    ...
    dump_generation_selected_step_stage_debug(state, opts, reference_result,
                                              32);
    if (token_mismatch_index > 0) {
      dump_generation_selected_step_stage_debug(state, opts, reference_result,
                                                token_mismatch_index - 1);
    }
    dump_generation_selected_step_stage_debug(state, opts, reference_result,
  ...
  const size_t layer0_row_width =
      static_cast<size_t>(state.model_data->params.n_embd);
  ...
  auto build_reference_stage_rows = [&](const char *key,
    ...
    for (size_t token_index = 0; token_index < prefix_tokens.size();
      ...
      std::span<const llama_token> prompt_prefix =
          is_prompt_token ? std::span<const llama_token>(prompt_tokens.data(),
                                                         token_index + 1u)
                          : std::span<const llama_token>(prompt_tokens.data(),
                                                         prompt_tokens.size());
      generated_prefix.clear();
      if (!is_prompt_token) {
        const size_t generated_count = token_index - prompt_tokens.size() + 1u;
        generated_prefix.insert(
            generated_prefix.end(), prefix_generated_tokens.begin(),
      ...
      const std::span<const float> row = reference_last_token_row(
          find_reference_tensor(token_capture, key), layer0_row_width);
  ...
  if (!build_reference_stage_rows("kqv_out-0", reference_layer0_kqv_out_rows) ||
      !build_reference_stage_rows("attn_out-0",
                                  reference_layer0_attn_out_rows) ||
      !build_reference_stage_rows("ffn_norm-0",
                                  reference_layer0_ffn_norm_rows) ||
      ...
      !build_reference_stage_rows("attn_out-1",
                                  reference_layer1_attn_out_rows) ||
      !build_reference_stage_rows("attn_norm-1",
                                  reference_layer1_attn_norm_rows) ||
      !build_reference_stage_rows("ffn_inp-1", reference_layer1_ffn_inp_rows) ||
      !build_reference_stage_rows("ffn_norm-1",
                                  reference_layer1_ffn_norm_rows) ||
    ...
    std::fprintf(stdout,
                 "generation_debug.flash: unable to capture reference graph\n");
    return;
  }
  ...
  emel::generator::detail::native_backend ggml_f16_scores_attention_backend =
      {};
  emel::generator::detail::native_backend
      ggml_f16_scores_ggml_softmax_attention_backend = {};
  emel::generator::detail::native_backend
      ggml_nonflash_f16_ggml_softmax_attention_backend = {};
  ...
  emel::generator::detail::native_backend
      full_q_rounded_weight_attention_backend = {};
  ...
  emel::generator::detail::native_backend reference_layer0_attn_out_backend =
      {};
  emel::generator::detail::native_backend
      reference_layer0_attn_out_shared_backend = {};
  emel::generator::detail::native_backend
  ...
  if (emel::generator::detail::prepare(dispatch_backend, *state.model_data) !=
      ...
      emel::generator::detail::prepare(rounded_weight_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(ggml_f16_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(ggml_online_f16_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(ggml_nonflash_f16_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(ggml_f16_scores_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      ...
      emel::generator::detail::prepare(
          ...
          *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(double_softmax_sum_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(ggml_softmax_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(full_q_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(full_q_f32_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(full_q_rounded_weight_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(full_q_ggml_f16_attention_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(exact_backend, *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      ...
      emel::generator::detail::prepare(q6_exact_backend, *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q2_scalar_quant_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q3_scalar_quant_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q23_scalar_quant_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q236_scalar_quant_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q6_scalar_quant_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q2_reference_backend,
                                       *state.model_data) !=
      ...
      emel::generator::detail::prepare(q3_reference_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q236_reference_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(q6_reference_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_key_cache_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_value_cache_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_kv_cache_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer0_kqv_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(
          reference_layer0_kqv_out_exact_attention_backend,
      ...
      emel::generator::detail::prepare(
          ...
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer0_attn_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(
          reference_layer0_attn_out_q2_scalar_quant_backend,
      ...
      emel::generator::detail::prepare(
          ...
          *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer0_ffn_norm_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer0_ffn_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer0_l_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer1_kqv_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer1_attn_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer1_attn_norm_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer1_ffn_inp_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer1_ffn_norm_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer1_ffn_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_layer1_l_out_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      !run_prefill_from_token_prefix(dispatch_backend, prefix_tokens) ||
      ...
      !run_prefill_with_scalar_attention_full_q(full_q_attention_backend,
                                                prefix_tokens) ||
      ...
      !run_prefill_with_scalar_attention_reference_cache(
          reference_key_cache_backend,
          const_cast<reference_backend &>(state.reference), prefix_tokens, true,
          false) ||
      !run_prefill_with_scalar_attention_reference_cache(
          reference_value_cache_backend,
          const_cast<reference_backend &>(state.reference), prefix_tokens,
          false, true) ||
      !run_prefill_with_scalar_attention_reference_cache(
      ...
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_kqv_out_backend, prefix_tokens,
          reference_stage_injection::layer0_kqv_out,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_kqv_out_exact_attention_backend, prefix_tokens,
          reference_stage_injection::layer0_kqv_out, reference_stage_captures,
          attention_projection_override::exact) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_kqv_out_reference_q8_backend, prefix_tokens,
          reference_stage_injection::layer0_kqv_out, reference_stage_captures,
          attention_projection_override::reference_q8) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_attn_out_backend, prefix_tokens,
          reference_stage_injection::layer0_attn_out,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_attn_out_q2_scalar_quant_backend, prefix_tokens,
          reference_stage_injection::layer0_attn_out, reference_stage_captures,
          attention_projection_override::native, scalar_quant_q2_only) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_attn_out_q3_scalar_quant_backend, prefix_tokens,
          reference_stage_injection::layer0_attn_out, reference_stage_captures,
          attention_projection_override::native, scalar_quant_q3_only) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_attn_out_q6_scalar_quant_backend, prefix_tokens,
          reference_stage_injection::layer0_attn_out, reference_stage_captures,
          attention_projection_override::native, scalar_quant_q6_only) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_attn_out_q236_scalar_quant_backend, prefix_tokens,
          reference_stage_injection::layer0_attn_out, reference_stage_captures,
          attention_projection_override::native, scalar_quant_q236) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_attn_out_q236_reference_backend, prefix_tokens,
          reference_stage_injection::layer0_attn_out, reference_stage_captures,
          attention_projection_override::native, reference_q236) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_ffn_norm_backend, prefix_tokens,
          reference_stage_injection::layer0_ffn_norm,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_ffn_out_backend, prefix_tokens,
          reference_stage_injection::layer0_ffn_out,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_l_out_backend, prefix_tokens,
          reference_stage_injection::layer0_l_out, reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer1_kqv_out_backend, prefix_tokens,
          reference_stage_injection::layer1_kqv_out,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer1_attn_out_backend, prefix_tokens,
          reference_stage_injection::layer1_attn_out,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer1_attn_norm_backend, prefix_tokens,
          reference_stage_injection::layer1_attn_norm,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer1_ffn_inp_backend, prefix_tokens,
          reference_stage_injection::layer1_ffn_inp,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer1_ffn_norm_backend, prefix_tokens,
          reference_stage_injection::layer1_ffn_norm,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer1_ffn_out_backend, prefix_tokens,
          reference_stage_injection::layer1_ffn_out,
          reference_stage_captures) ||
      !run_prefill_with_scalar_attention_reference_stage(
          reference_layer1_l_out_backend, prefix_tokens,
          reference_stage_injection::layer1_l_out, reference_stage_captures)) {
    std::fprintf(stdout,
                 "generation_debug.flash: unable to replay mismatch prefix\n");
    return;
  }
  ...
  if (!run_prefill_from_token_prefix(shared_backend, prefix_tokens)) {
    std::fprintf(
        stdout,
        "generation_debug.flash: unable to replay shared backend prefix\n");
    return;
  }
  if (emel::generator::detail::prepare(reference_layer0_attn_out_shared_backend,
                                       *state.model_data) !=
      emel::error::cast(emel::model::loader::error::none)) {
    std::fprintf(
        stdout,
        "generation_debug.flash: unable to prepare shared attn_out replay\n");
    return;
  }
  reference_layer0_attn_out_shared_backend.kernel_kind =
      emel::kernel::kernel_kind::x86_64;
  reference_layer0_attn_out_shared_backend.kernel.set_kind(
      reference_layer0_attn_out_shared_backend.kernel_kind);
  if (!run_prefill_with_scalar_attention_reference_stage(
          reference_layer0_attn_out_shared_backend, prefix_tokens,
          reference_stage_injection::layer0_attn_out,
          reference_stage_captures)) {
    std::fprintf(
        stdout,
        "generation_debug.flash: unable to replay shared attn_out prefix\n");
    return;
  }

  const int32_t emel_token =
      emel_result.trace.token_ids[static_cast<size_t>(token_mismatch_index)];
  const int32_t reference_token =
      reference_result.trace
          .token_ids[static_cast<size_t>(token_mismatch_index)];
  if (state.reference.model != nullptr &&
      state.reference.model->layers.size() >= 2) {
    for (int32_t layer = 0; layer < 2; ++layer) {
      const auto &ref_layer =
          state.reference.model->layers[static_cast<size_t>(layer)];
      std::fprintf(stdout,
                   "generation_debug.reference.layer%d.transforms: bo=%d "
                   "ffn_gate_b=%d ffn_up_b=%d ffn_down_b=%d\n",
                   layer, ref_layer.bo != nullptr ? 1 : 0,
  ...
  const argmax_summary dispatch_summary = select_argmax_from_logits(
      dispatch_backend.bound_logits.data(), dispatch_backend.n_vocab);
  const argmax_summary scalar_summary = select_argmax_from_logits(
      scalar_backend.bound_logits.data(), scalar_backend.n_vocab);
  const argmax_summary shared_summary = select_argmax_from_logits(
      shared_backend.bound_logits.data(), shared_backend.n_vocab);
  ...
  const argmax_summary rounded_weight_attention_summary =
      select_argmax_from_logits(
          rounded_weight_attention_backend.bound_logits.data(),
          rounded_weight_attention_backend.n_vocab);
  const argmax_summary ggml_f16_attention_summary =
      select_argmax_from_logits(ggml_f16_attention_backend.bound_logits.data(),
                                ggml_f16_attention_backend.n_vocab);
  const argmax_summary ggml_online_f16_attention_summary =
      select_argmax_from_logits(
          ggml_online_f16_attention_backend.bound_logits.data(),
          ggml_online_f16_attention_backend.n_vocab);
  const argmax_summary ggml_nonflash_f16_attention_summary =
      select_argmax_from_logits(
          ggml_nonflash_f16_attention_backend.bound_logits.data(),
          ggml_nonflash_f16_attention_backend.n_vocab);
  const argmax_summary ggml_f16_scores_attention_summary =
      select_argmax_from_logits(
          ggml_f16_scores_attention_backend.bound_logits.data(),
          ggml_f16_scores_attention_backend.n_vocab);
  const argmax_summary ggml_f16_scores_ggml_softmax_attention_summary =
      select_argmax_from_logits(
  ...
  const argmax_summary double_softmax_sum_attention_summary =
      select_argmax_from_logits(
          double_softmax_sum_attention_backend.bound_logits.data(),
          double_softmax_sum_attention_backend.n_vocab);
  const argmax_summary ggml_softmax_attention_summary =
      select_argmax_from_logits(
          ggml_softmax_attention_backend.bound_logits.data(),
          ggml_softmax_attention_backend.n_vocab);
  const argmax_summary full_q_attention_summary =
      select_argmax_from_logits(full_q_attention_backend.bound_logits.data(),
                                full_q_attention_backend.n_vocab);
  ...
  const argmax_summary full_q_rounded_weight_attention_summary =
      select_argmax_from_logits(
          full_q_rounded_weight_attention_backend.bound_logits.data(),
          full_q_rounded_weight_attention_backend.n_vocab);
  const argmax_summary full_q_ggml_f16_attention_summary =
      select_argmax_from_logits(
  ...
  const argmax_summary q6_exact_summary = select_argmax_from_logits(
      q6_exact_backend.bound_logits.data(), q6_exact_backend.n_vocab);
  const argmax_summary q2_scalar_quant_summary =
      select_argmax_from_logits(q2_scalar_quant_backend.bound_logits.data(),
                                q2_scalar_quant_backend.n_vocab);
  const argmax_summary q3_scalar_quant_summary =
      select_argmax_from_logits(q3_scalar_quant_backend.bound_logits.data(),
                                q3_scalar_quant_backend.n_vocab);
  const argmax_summary q23_scalar_quant_summary =
      select_argmax_from_logits(q23_scalar_quant_backend.bound_logits.data(),
                                q23_scalar_quant_backend.n_vocab);
  const argmax_summary q236_scalar_quant_summary =
      select_argmax_from_logits(q236_scalar_quant_backend.bound_logits.data(),
                                q236_scalar_quant_backend.n_vocab);
  const argmax_summary q6_scalar_quant_summary =
      select_argmax_from_logits(q6_scalar_quant_backend.bound_logits.data(),
                                q6_scalar_quant_backend.n_vocab);
  const argmax_summary q2_reference_summary = select_argmax_from_logits(
      q2_reference_backend.bound_logits.data(), q2_reference_backend.n_vocab);
  const argmax_summary q3_reference_summary = select_argmax_from_logits(
      q3_reference_backend.bound_logits.data(), q3_reference_backend.n_vocab);
  const argmax_summary q236_reference_summary =
      select_argmax_from_logits(q236_reference_backend.bound_logits.data(),
                                q236_reference_backend.n_vocab);
  ...
  const argmax_summary reference_key_cache_summary =
      select_argmax_from_logits(reference_key_cache_backend.bound_logits.data(),
                                reference_key_cache_backend.n_vocab);
  const argmax_summary reference_value_cache_summary =
      select_argmax_from_logits(
          reference_value_cache_backend.bound_logits.data(),
          reference_value_cache_backend.n_vocab);
  const argmax_summary reference_kv_cache_summary =
      select_argmax_from_logits(reference_kv_cache_backend.bound_logits.data(),
                                reference_kv_cache_backend.n_vocab);
  const argmax_summary reference_layer0_kqv_out_summary =
      select_argmax_from_logits(
  ...
  const argmax_summary reference_layer0_kqv_out_exact_attention_summary =
      select_argmax_from_logits(
          reference_layer0_kqv_out_exact_attention_backend.bound_logits.data(),
  ...
  const argmax_summary reference_layer0_kqv_out_reference_q8_summary =
      select_argmax_from_logits(
          reference_layer0_kqv_out_reference_q8_backend.bound_logits.data(),
          reference_layer0_kqv_out_reference_q8_backend.n_vocab);
  const argmax_summary reference_layer0_attn_out_summary =
      select_argmax_from_logits(
          reference_layer0_attn_out_backend.bound_logits.data(),
          reference_layer0_attn_out_backend.n_vocab);
  const argmax_summary reference_layer0_attn_out_shared_summary =
      select_argmax_from_logits(
  ...
  const argmax_summary reference_layer0_attn_out_q2_scalar_quant_summary =
      select_argmax_from_logits(
          reference_layer0_attn_out_q2_scalar_quant_backend.bound_logits.data(),
  ...
  const argmax_summary reference_layer0_attn_out_q3_scalar_quant_summary =
      select_argmax_from_logits(
          reference_layer0_attn_out_q3_scalar_quant_backend.bound_logits.data(),
  ...
  const argmax_summary reference_layer0_attn_out_q6_scalar_quant_summary =
      select_argmax_from_logits(
          reference_layer0_attn_out_q6_scalar_quant_backend.bound_logits.data(),
  ...
  const argmax_summary reference_layer0_attn_out_q236_scalar_quant_summary =
      select_argmax_from_logits(
          reference_layer0_attn_out_q236_scalar_quant_backend.bound_logits
              .data(),
  ...
  const auto dump_flash_kernel_compare =
      [&](const char *label, emel::generator::detail::native_backend &backend) {
        ...
        const int32_t last_position =
            static_cast<int32_t>(prefix_tokens.size()) - 1;
        auto neon_request = emel::generator::detail::make_flash_attn_request(
            backend, last_layer, last_position);
        ...
        neon_request.dst = emel::generator::detail::make_dst_view_3d(
            neon_dst.data(), neon_request.dst.ne[0], neon_request.dst.ne[1],
            neon_request.dst.ne[2]);
        auto shared_request = neon_request;
        shared_request.dst = emel::generator::detail::make_dst_view_3d(
            shared_dst.data(), shared_request.dst.ne[0],
            shared_request.dst.ne[1], shared_request.dst.ne[2]);
        ...
        if (!emel::kernel::aarch64::detail::run_flash_attn_ext_neon(
          ...
          std::fprintf(stdout,
                       "generation_debug.flash.kernel.%s: unable to compare\n",
                       label);
          return;
        }
        float max_abs = 0.0f;
        ...
        std::fprintf(stdout,
                     "generation_debug.flash.kernel.%s: max_abs=%g idx=%zu "
                     "neon=%g shared=%g\n",
                     label, max_abs, max_idx, neon_dst[max_idx],
                     shared_dst[max_idx]);
      };
  ...
  std::fprintf(
      stdout,
      "generation_debug.flash: prefix_tokens=%zu dispatch_argmax=%d "
      "scalar_argmax=%d "
      "shared_argmax=%d f32_attn_argmax=%d rounded_weight_attn_argmax=%d "
      "ggml_f16_attn_argmax=%d ggml_online_f16_attn_argmax=%d "
      "ggml_nonflash_f16_attn_argmax=%d ggml_f16_scores_attn_argmax=%d "
      "ggml_f16_scores_ggml_softmax_argmax=%d "
      "ggml_nonflash_f16_ggml_softmax_argmax=%d double_softmax_argmax=%d "
      "ggml_softmax_argmax=%d full_q_attn_argmax=%d full_q_f32_attn_argmax=%d "
      "full_q_rounded_weight_attn_argmax=%d full_q_ggml_f16_attn_argmax=%d "
      "exact_argmax=%d attention_exact_argmax=%d "
      ...
      prefix_tokens.size(), dispatch_summary.selected_token,
      scalar_summary.selected_token, shared_summary.selected_token,
      ...
      ffn_exact_summary.selected_token, output_exact_summary.selected_token,
      q2_exact_summary.selected_token, q3_exact_summary.selected_token,
      q6_exact_summary.selected_token, q2_scalar_quant_summary.selected_token,
      ...
      q2_reference_summary.selected_token, q3_reference_summary.selected_token,
      ...
      reference_layer1_l_out_summary.selected_token, reference_token);
  const reference_stage_replay_diff attn_out_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer0_attn_out_backend, prefix_tokens, 0,
          reference_stage_injection::layer0_attn_out, reference_stage_captures);
  const reference_stage_replay_diff attn_out_q236_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer0_attn_out_q236_reference_backend, prefix_tokens, 0,
          reference_stage_injection::layer0_attn_out, reference_stage_captures,
          attention_projection_override::native, reference_q236);
  const reference_stage_replay_diff ffn_norm_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer0_ffn_norm_backend, prefix_tokens, 0,
          reference_stage_injection::layer0_ffn_norm, reference_stage_captures);
  const reference_stage_replay_diff ffn_out_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer0_ffn_out_backend, prefix_tokens, 0,
          reference_stage_injection::layer0_ffn_out, reference_stage_captures);
  const reference_stage_replay_diff layer1_attn_out_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer1_attn_out_backend, prefix_tokens, 1,
          reference_stage_injection::layer1_attn_out, reference_stage_captures);
  const reference_stage_replay_diff layer1_attn_norm_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer1_attn_norm_backend, prefix_tokens, 1,
          reference_stage_injection::layer1_attn_norm,
          reference_stage_captures);
  const reference_stage_replay_diff layer1_ffn_inp_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer1_ffn_inp_backend, prefix_tokens, 1,
          reference_stage_injection::layer1_ffn_inp, reference_stage_captures);
  const reference_stage_replay_diff layer1_ffn_norm_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer1_ffn_norm_backend, prefix_tokens, 1,
          reference_stage_injection::layer1_ffn_norm, reference_stage_captures);
  const reference_stage_replay_diff layer1_ffn_out_replay_diff =
      replay_reference_stage_l_out_diff(
          reference_layer1_ffn_out_backend, prefix_tokens, 1,
          reference_stage_injection::layer1_ffn_out, reference_stage_captures);
  std::fprintf(
      stdout,
      ...
      "attn_out_q236_max_abs=%g ffn_norm_first_l_out_mismatch=%d "
      "ffn_norm_max_abs=%g "
  ...
  std::fprintf(
      ...
      attention_exact_backend
          .bound_logits[static_cast<size_t>(reference_token)],
  ...
  const auto dump_alt_generation = [&](const char *label,
                                       const native_layer_runner run_layer_fn) {
    generation_result alt_result = {};
    const emel::error::type alt_err =
        run_custom_native_generate(const_cast<generation_load_state &>(state),
                                   opts, run_layer_fn, alt_result);
    if (alt_err == emel::error::cast(emel::generator::error::none)) {
      std::fprintf(
          ...
          label, generation_results_match(alt_result, reference_result) ? 1 : 0,
          first_token_mismatch_index(alt_result, reference_result),
          first_mismatch_offset(alt_result, reference_result),
          alt_result.tokens_generated, alt_result.output_length);
      return;
    }

    std::fprintf(stdout, "generation_debug.alt.%s: error=%d\n", label,
                 static_cast<int>(alt_err));
  };
  dump_alt_generation("runtime_flash_q_full", run_layer_with_flash_request_q);
  dump_alt_generation("runtime_flash_q_attn_full",
                      run_layer_with_flash_request_q_attn);
  dump_alt_generation("emel_prod_style_full",
                      run_layer_with_scalar_attention_emel_prod_style);
  dump_alt_generation(
      "ggml_nonflash_exact_masked_full",
      run_layer_with_scalar_attention_ggml_nonflash_exact_masked);
  dump_alt_generation(
      "ggml_nonflash_exact_masked_full_q",
      run_layer_with_scalar_attention_ggml_nonflash_exact_masked_full_q);
  dump_alt_generation(
      "ggml_nonflash_exact_scores_prod_value_full",
  ...
  dump_alt_generation("exact_ffn_full",
                      run_layer_with_exact_ffn_scalar_attention);
  dump_alt_generation("exact_output_full",
                      run_layer_with_exact_output_scalar_attention);
  ...
  dump_candidate_logits_compare("generation_debug.flash.dispatch_output",
                                dispatch_backend, emel_token, reference_token);
  dump_candidate_logits_compare("generation_debug.flash.output_exact",
                                output_exact_backend, emel_token,
                                reference_token);
  ...
  if (reference_ctx == nullptr) {
    std::fprintf(
        stdout,
        "generation_debug.reference: unable to create reference context\n");
    return;
  }
  ...
  if (!run_reference_prefix_decode(reference_ctx.get(), prompt_tokens,
                                   generated_prefix)) {
    std::fprintf(
        stdout,
        "generation_debug.reference: unable to decode mismatch prefix\n");
    return;
  }
  ...
  if (reference_logits == nullptr) {
    std::fprintf(
        stdout,
        "generation_debug.reference: missing logits at mismatch prefix\n");
    return;
  }
  ...
  for (int32_t idx = 0; idx < dispatch_backend.n_vocab; ++idx) {
    const float diff =
        std::fabs(dispatch_backend.bound_logits[static_cast<size_t>(idx)] -
                  reference_logits[idx]);
    if (diff > max_abs) {
      max_abs = diff;
      max_idx = idx;
    }
  ...
  std::fprintf(
      ...
      "reference=%g\n",
      reference_summary.selected_token, max_abs, max_idx,
      dispatch_backend.bound_logits[static_cast<size_t>(max_idx)],
      reference_logits[max_idx]);
  std::fprintf(
      stdout,
      "generation_debug.reference.scores: dispatch_emel=%g "
      "dispatch_reference=%g "
      "reference_emel=%g reference_reference=%g\n",
      dispatch_backend.bound_logits[static_cast<size_t>(emel_token)],
      dispatch_backend.bound_logits[static_cast<size_t>(reference_token)],
      reference_logits[emel_token], reference_logits[reference_token]);
}

std::string_view token_piece_view(const reference_backend &backend,
                                  const int32_t token_id) {
...
void dump_generation_result(const char *label,
                            const generation_result &result) {
  std::fprintf(stdout, "%s: generated_tokens=%d output_bytes=%zu text=%.*s\n",
               label, result.tokens_generated, result.output_length,
               static_cast<int>(result.output_length), result.output.data());
}
...
void dump_generation_trace_window(const char *label,
  ...
  for (int32_t idx = start; idx < stop; ++idx) {
    ...
    std::fprintf(stdout, "%s.trace[%d]: token=%d gap=%g piece=%.*s\n", label,
                 idx, token_id,
                 result.trace.top_score_gaps[static_cast<size_t>(idx)],
...
void dump_prompt_token_window(const char *label,
                              const reference_backend &backend,
                              const std::span<const int32_t> tokens) {
  const int32_t stop =
      std::min<int32_t>(static_cast<int32_t>(tokens.size()), 12);
  for (int32_t idx = 0; idx < stop; ++idx) {
    const int32_t token_id = tokens[static_cast<size_t>(idx)];
    const std::string_view piece = token_piece_view(backend, token_id);
    std::fprintf(stdout, "%s.prompt[%d]: token=%d piece=%.*s\n", label, idx,
                 token_id, static_cast<int>(piece.size()),
                 piece.data() != nullptr ? piece.data() : "");
  }
}

void dump_prompt_token_window_reference(
    const char *label, const reference_backend &backend,
    const std::span<const llama_token> tokens) {
  const int32_t stop =
      std::min<int32_t>(static_cast<int32_t>(tokens.size()), 12);
  for (int32_t idx = 0; idx < stop; ++idx) {
    const int32_t token_id = tokens[static_cast<size_t>(idx)];
...
bool quantize_input_blocks(
  ...
  for (uint64_t block = 0; block < block_count_out; ++block) {
    kernel_quant::quantize_row_q8_k_strided(
        input.data() + block * kernel_quant::QK_K, 1, &blocks[block],
        kernel_quant::QK_K);
  }
  return true;
}
...
bool quantize_input_blocks_reference(
    ...
    std::array<reference_block_q8_k, kernel_quant::MAX_Q8_K_BLOCKS> &blocks,
    uint64_t &block_count_out) {
  static_assert(sizeof(reference_block_q8_k) ==
                sizeof(kernel_quant::block_q8_k));
  if ((input.size() % kernel_quant::QK_K) != 0u) {
    return false;
  }
  block_count_out = input.size() / kernel_quant::QK_K;
  ...
  for (uint64_t block = 0; block < block_count_out; ++block) {
    quantize_row_q8_K_ref(input.data() + block * kernel_quant::QK_K,
                          &blocks[block], kernel_quant::QK_K);
  }
  return true;
}
...
float ggml_row_dot(const emel::generator::detail::tensor_matrix &matrix,
  ...
  const auto *row_ptr =
      static_cast<const uint8_t *>(matrix.tensor->data) +
      static_cast<size_t>(row) * emel::generator::detail::row_storage_bytes(
                                     *matrix.tensor, matrix.cols);
  float out = 0.0f;
  switch (static_cast<emel::kernel::event::dtype>(matrix.tensor->type)) {
  case emel::kernel::event::dtype::q2_k:
    ggml_vec_dot_q2_K_q8_K(static_cast<int>(block_count * kernel_quant::QK_K),
                           &out, 0, row_ptr, 0, q8_blocks, 0, 1);
    return out;
  case emel::kernel::event::dtype::q3_k:
    ggml_vec_dot_q3_K_q8_K(static_cast<int>(block_count * kernel_quant::QK_K),
                           &out, 0, row_ptr, 0, q8_blocks, 0, 1);
    return out;
  case emel::kernel::event::dtype::q6_k:
    ggml_vec_dot_q6_K_q8_K(static_cast<int>(block_count * kernel_quant::QK_K),
                           &out, 0, row_ptr, 0, q8_blocks, 0, 1);
    return out;
  default:
...
float ggml_row_dot_reference_q8(
    const emel::generator::detail::tensor_matrix &matrix, const uint32_t row,
    const reference_block_q8_k *q8_blocks, const uint64_t block_count) {
  const auto *row_ptr =
      static_cast<const uint8_t *>(matrix.tensor->data) +
      static_cast<size_t>(row) * emel::generator::detail::row_storage_bytes(
                                     *matrix.tensor, matrix.cols);
  ...
  case emel::kernel::event::dtype::q2_k:
    ggml_vec_dot_q2_K_q8_K_generic(
        static_cast<int>(block_count * kernel_quant::QK_K), &out, 0, row_ptr, 0,
        q8_blocks, 0, 1);
    return out;
  case emel::kernel::event::dtype::q3_k:
...
float dequantized_row_dot(const emel::generator::detail::tensor_matrix &matrix,
                          const uint32_t row, std::span<const float> input) {
  if (matrix.tensor == nullptr || row >= static_cast<uint32_t>(matrix.rows) ||
      static_cast<size_t>(matrix.cols) != input.size()) {
    return std::numeric_limits<float>::quiet_NaN();
  }
  ...
  if (!emel::generator::detail::copy_tensor_row(
          *matrix.tensor, static_cast<int32_t>(row), weights)) {
    return std::numeric_limits<float>::quiet_NaN();
  }
...
void dump_candidate_logits_compare(
    const char *label, const emel::generator::detail::native_backend &backend,
    const int32_t token_a, const int32_t token_b) {
  if (backend.output.tensor == nullptr || backend.output.rows <= 0 ||
      backend.output.cols <= 0 ||
      static_cast<size_t>(backend.output.cols) != backend.norm.size() ||
      token_a < 0 || token_b < 0 || token_a >= backend.output.rows ||
  ...
  if (!quantize_input_blocks(backend.norm, emel_q8, emel_q8_blocks) ||
      !quantize_input_blocks_reference(backend.norm, reference_q8,
                                       reference_q8_blocks) ||
  ...
  const auto dump_token = [&](const char *token_label, const int32_t token_id) {
    const float emel_logit =
        backend.bound_logits[static_cast<size_t>(token_id)];
    const float ggml_emel_q8 =
        ggml_row_dot(backend.output, static_cast<uint32_t>(token_id),
                     emel_q8.data(), emel_q8_blocks);
    const float ggml_reference_q8 = ggml_row_dot_reference_q8(
        backend.output, static_cast<uint32_t>(token_id), reference_q8.data(),
        reference_q8_blocks);
    ...
    std::fprintf(
        ...
        label, token_label, token_id,
        static_cast<unsigned>(backend.output.tensor->type), emel_logit,
        ggml_emel_q8, ggml_reference_q8, exact, emel_logit - ggml_emel_q8,
        ggml_emel_q8 - exact);
  };
...
void dump_q8_quantize_compare(const char *label, std::span<const float> input) {
  ...
  for (uint64_t block = 0; block < emel_block_count; ++block) {
    const float d_diff =
        std::fabs(emel_blocks[block].d - reference_blocks[block].d);
    if (d_diff > max_d_abs) {
      max_d_abs = d_diff;
      max_d_block = block;
    ...
    for (int32_t idx = 0; idx < kernel_quant::QK_K; ++idx) {
      const int diff = std::abs(
          static_cast<int>(emel_blocks[block].qs[static_cast<size_t>(idx)]) -
          static_cast<int>(
              reference_blocks[block].qs[static_cast<size_t>(idx)]));
      if (diff > max_q_diff) {
        max_q_diff = diff;
        max_q_block = block;
        max_q_idx = idx;
    ...
    for (int32_t idx = 0; idx < kernel_quant::QK_K / 16; ++idx) {
      const int diff = std::abs(
          static_cast<int>(emel_blocks[block].bsums[static_cast<size_t>(idx)]) -
          static_cast<int>(
              reference_blocks[block].bsums[static_cast<size_t>(idx)]));
  ...
  std::fprintf(stdout,
               "%s: blocks=%" PRIu64 " max_d_abs=%g d_block=%" PRIu64
               " emel_d=%g reference_d=%g "
               "max_q_diff=%d q_block=%" PRIu64
               " q_idx=%d emel_q=%d reference_q=%d "
               "max_bsum_diff=%d bsum_block=%" PRIu64
               " bsum_idx=%d emel_bsum=%d reference_bsum=%d\n",
               label, emel_block_count, max_d_abs, max_d_block, emel_d_at_max,
               reference_d_at_max, max_q_diff, max_q_block, max_q_idx,
               emel_q_at_max, reference_q_at_max, max_bsum_diff, max_bsum_block,
               max_bsum_idx, emel_bsum_at_max, reference_bsum_at_max);
}
...
void dump_matrix_compare(const char *label,
                         ...
                         std::span<const float> input) {
  std::vector<float> emel_out(static_cast<size_t>(matrix.rows));
  if (!emel::generator::detail::matmul_vector(backend, matrix, input,
                                              emel_out)) {
  ...
  std::fprintf(
      ...
      static_cast<unsigned>(matrix.tensor->type), matrix.rows, matrix.cols,
      max_abs, max_row, emel_at_max, ggml_at_max);
}
...
void dump_row_compare(const char *label,
  ...
  const size_t row_bytes =
      emel::generator::detail::row_storage_bytes(tensor, cols);
  const auto *row_ptr = static_cast<const uint8_t *>(tensor.data) +
  ...
  for (int32_t idx = 0; idx < cols; ++idx) {
    const float diff = std::fabs(emel[static_cast<size_t>(idx)] -
                                 ggml[static_cast<size_t>(idx)]);
    if (diff > max_abs) {
      max_abs = diff;
      max_idx = idx;
    }
  ...
  std::fprintf(
      stdout, "%s: dtype=%u cols=%d max_abs=%g idx=%d emel=%g ggml=%g\n", label,
      static_cast<unsigned>(tensor.type), cols, max_abs, max_idx,
      emel[static_cast<size_t>(max_idx)], ggml[static_cast<size_t>(max_idx)]);
}
...
void dump_vector_compare(const char *label,
  ...
  for (int32_t idx = 0; idx < static_cast<int32_t>(emel_values.size()); ++idx) {
    const float diff = std::fabs(emel_values[static_cast<size_t>(idx)] -
                                 ggml[static_cast<size_t>(idx)]);
    if (diff > max_abs) {
      max_abs = diff;
      max_idx = idx;
  ...
  std::fprintf(stdout,
               ...
               label, static_cast<unsigned>(tensor.type), emel_values.size(),
               max_abs, max_idx, emel_values[static_cast<size_t>(max_idx)],
               ggml[static_cast<size_t>(max_idx)]);
}

bool capture_reference_eval_tensor(ggml_tensor *tensor, const bool ask,
                                   void *user_data) {
      ...
      for (int64_t i2 = 0; i2 < ne2; ++i2) {
        for (int64_t i1 = 0; i1 < ne1; ++i1) {
          for (int64_t i0 = 0; i0 < ne0; ++i0) {
            const size_t offset =
                static_cast<size_t>(i0) * static_cast<size_t>(tensor->nb[0]) +
...
std::span<const float>
find_reference_tensor(const reference_graph_capture &capture,
                      const char *name) {
...
const reference_tensor_capture *
find_reference_capture(const reference_graph_capture &capture,
                       const char *name) {
  for (const auto &entry : capture.entries) {
    if (std::strcmp(entry.name, name) == 0) {
      return &entry;
...
bool capture_reference_graph_for_tokens(
    const generation_load_state &state, std::span<const llama_token> tokens,
    reference_graph_capture &graph_capture) {
  llama_context_params context_params = llama_context_default_params();
  context_params.flash_attn_type = LLAMA_FLASH_ATTN_TYPE_DISABLED;
  context_params.n_ctx = 0;
...
bool capture_reference_graph_for_generation_prefix(
  ...
  return run_reference_prefix_decode(ctx.get(), prompt_tokens,
                                     generated_tokens);
}
...
bool capture_reference_graph(const generation_load_state &state,
  ...
  return capture_reference_graph_for_tokens(state, prompt_tokens,
                                            graph_capture);
}

float read_ggml_tensor_value_f32(const ggml_tensor &tensor, const int64_t i0,
                                 const int64_t i1 = 0, const int64_t i2 = 0,
                                 const int64_t i3 = 0) {
...
void dump_state_compare(const char *label, std::span<const float> emel_values,
  ...
  if (reference_values.empty() ||
      emel_values.size() != reference_values.size()) {
    std::fprintf(stdout, "%s: unavailable emel=%zu reference=%zu\n", label,
                 emel_values.size(), reference_values.size());
    return;
  }
  ...
  for (int32_t idx = 0; idx < static_cast<int32_t>(emel_values.size()); ++idx) {
    const float diff = std::fabs(emel_values[static_cast<size_t>(idx)] -
                                 reference_values[static_cast<size_t>(idx)]);
    if (diff > max_abs) {
      max_abs = diff;
      max_idx = idx;
    }
  ...
  std::fprintf(stdout, "%s: max_abs=%g idx=%d emel=%g reference=%g\n", label,
               max_abs, max_idx, emel_values[static_cast<size_t>(max_idx)],
...
void dump_state_compare(const char *label,
  ...
  float max_reference = 0.0f;
  for (int32_t idx = 0; idx < static_cast<int32_t>(emel_values.size()); ++idx) {
    const float emel_value =
        fp16_storage_to_fp32(emel_values[static_cast<size_t>(idx)]);
  ...
  std::fprintf(stdout, "%s: max_abs=%g idx=%d emel=%g reference=%g\n", label,
               max_abs, max_idx, max_emel, max_reference);
}
...
void dump_state_compare(const char *label,
  ...
  for (int32_t idx = 0; idx < static_cast<int32_t>(emel_values.size()); ++idx) {
    const float emel_value =
        fp16_storage_to_fp32(emel_values[static_cast<size_t>(idx)]);
    const float reference_value =
        fp16_storage_to_fp32(reference_values[static_cast<size_t>(idx)]);
    const float diff = std::fabs(emel_value - reference_value);
    if (diff > max_abs) {
      max_abs = diff;
      max_idx = idx;
...
std::span<const float>
reference_softmax_query_head_slice(const reference_tensor_capture &capture,
                                   const int32_t head,
  ...
  if (head < 0 || query_index < 0 || ne3 < 1 || head >= ne2 ||
      query_index >= ne1) {
    return {};
  }

  const size_t offset =
      ((static_cast<size_t>(head) * static_cast<size_t>(ne1)) +
       static_cast<size_t>(query_index)) *
      static_cast<size_t>(ne0);
  if (offset + static_cast<size_t>(ne0) > capture.values.size()) {
    return {};
  }
  return std::span<const float>(capture.values)
      .subspan(offset, static_cast<size_t>(ne0));
}

std::span<const float>
reference_token_head_slice(const reference_tensor_capture &capture,
  ...
  const int64_t ne2 = std::max<int64_t>(capture.shape[2], 1);
  const int64_t ne3 = std::max<int64_t>(capture.shape[3], 1);
  if (token_index < 0 || head_index < 0 || token_index >= ne3 ||
      head_index >= ne2) {
    return {};
  }
...
std::span<const float>
reference_token_tensor_slice(const reference_tensor_capture &capture,
                             const int32_t token_index);

std::vector<float> reference_attention_context_slice(
    const reference_tensor_capture &capture, const int32_t query_index,
    const int32_t head_count, const int32_t head_dim) {
  const int64_t ne0 = std::max<int64_t>(capture.shape[0], 1);
  const int64_t ne1 = std::max<int64_t>(capture.shape[1], 1);
  const int64_t ne2 = std::max<int64_t>(capture.shape[2], 1);
  if (query_index < 0 || head_count < 0 || head_dim < 0 || ne0 != head_dim ||
      ne1 <= query_index || ne2 != head_count) {
    return {};
  }
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    const size_t out_offset =
        static_cast<size_t>(head) * static_cast<size_t>(head_dim);
    if (capture_offset + static_cast<size_t>(head_dim) >
        capture.values.size()) {
      return {};
    }
    std::copy_n(capture.values.data() +
                    static_cast<std::ptrdiff_t>(capture_offset),
                static_cast<size_t>(head_dim),
...
bool compute_attention_with_softmax_debug(
    emel::generator::detail::native_backend &backend,
    const reference_graph_capture &graph_capture, const int32_t layer_index,
    const int32_t position_limit, const std::string &layer_prefix,
    std::span<const float> reference_value_cache_rows = {}) {
  const int32_t head_count = backend.n_head;
  const int32_t kv_head_count = backend.n_head_kv;
  const int32_t head_dim = backend.head_dim;
  ...
  const std::span<const float> reference_ctx = reference_last_token_row(
      find_reference_tensor(graph_capture, ctx_key.c_str()),
      backend.attn_ctx.size());
  ...
  std::vector<float> ctx_reference_all_ggml_f16_dot(backend.attn_ctx.size(),
                                                    0.0f);
  const std::vector<float> raw_reference_ctx =
      raw_ctx_capture != nullptr
          ? reference_attention_context_slice(*raw_ctx_capture, 0, head_count,
                                              head_dim)
          : std::vector<float>{};
  ...
  if (have_reference_value_cache || value_capture != nullptr) {
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      for (int32_t idx = 0; idx < kv_dim; ++idx) {
        const float cached_value = fp16_storage_to_fp32(
            backend.value_cache[cache_offset + static_cast<size_t>(idx)]);
        const float diff =
            std::fabs(cached_value - reference_row[static_cast<size_t>(idx)]);
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    const size_t kv_offset =
        static_cast<size_t>(kv_head) * static_cast<size_t>(kv_head_dim);
    std::vector<ggml_fp16_t> emel_weight_f16(
        static_cast<size_t>(position_limit));
    std::vector<ggml_fp16_t> reference_weight_f16(
        static_cast<size_t>(position_limit));
    std::vector<ggml_fp16_t> q_f16(static_cast<size_t>(head_dim));
    std::vector<ggml_fp16_t> k_f16(static_cast<size_t>(head_dim));
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      for (int32_t dim = 0; dim < head_dim; ++dim) {
        raw_score +=
            ...
            fp16_storage_to_fp32(
                backend.key_cache[cache_offset + static_cast<size_t>(dim)]);
        full_q_raw_score +=
            backend.q[q_offset + static_cast<size_t>(dim)] *
            fp16_storage_to_fp32(
                backend.key_cache[cache_offset + static_cast<size_t>(dim)]);
        full_q_score +=
            backend.q[q_offset + static_cast<size_t>(dim)] *
      ...
      if (static_cast<size_t>(position) < reference_kq.size()) {
        ...
        const float full_q_raw_diff = std::fabs(
            full_q_raw_score - reference_kq[static_cast<size_t>(position)]);
        if (full_q_raw_diff > full_q_kq_max_abs) {
          ...
          full_q_kq_reference_at_max =
              reference_kq[static_cast<size_t>(position)];
        }
        float ggml_f16_raw_score = 0.0f;
        ggml_vec_dot_f16(head_dim, &ggml_f16_raw_score, 0u, k_f16.data(), 0u,
                         q_f16.data(), 0u, 1);
        const float ggml_f16_raw_diff = std::fabs(
            ggml_f16_raw_score - reference_kq[static_cast<size_t>(position)]);
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      const float prob = std::exp(
          backend.attn_scores[static_cast<size_t>(position)] - max_score);
      const float full_q_prob = std::exp(
          full_q_scores[static_cast<size_t>(position)] - full_q_max_score);
      backend.attn_probs[static_cast<size_t>(position)] = prob;
      score_sum += prob;
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      const float reference_weight =
          static_cast<size_t>(position) < reference_probs.size()
              ...
              : weight;
      emel_weight_f16[static_cast<size_t>(position)] =
          kernel_quant::fp32_to_fp16(weight);
      ...
      if (static_cast<size_t>(position) < reference_probs.size()) {
        const float diff =
            std::fabs(weight - reference_probs[static_cast<size_t>(position)]);
        if (diff > max_abs) {
          max_abs = diff;
          max_head = head;
          max_pos = position;
        ...
        const float full_q_weight =
            full_q_score_sum == 0.0f
                ? 0.0f
                : std::exp(full_q_scores[static_cast<size_t>(position)] -
                           full_q_max_score) /
                      full_q_score_sum;
        const float full_q_diff = std::fabs(
            full_q_weight - reference_probs[static_cast<size_t>(position)]);
      ...
      const std::span<const float> reference_value =
          have_reference_value_cache ? reference_value_cache_rows.subspan(
                                           static_cast<size_t>(position) *
                                                   static_cast<size_t>(kv_dim) +
                                               kv_offset,
                                           static_cast<size_t>(head_dim))
      ...
      for (int32_t dim = 0; dim < head_dim; ++dim) {
        ...
        if (static_cast<size_t>(dim) < reference_value.size()) {
          const float diff = std::fabs(
              cached_value - reference_value[static_cast<size_t>(dim)]);
    ...
    std::vector<ggml_fp16_t> emel_value_f16(
        static_cast<size_t>(position_limit));
    std::vector<ggml_fp16_t> reference_value_f16(
        static_cast<size_t>(position_limit));
    for (int32_t dim = 0; dim < head_dim; ++dim) {
      ...
      for (int32_t position = 0; position < position_limit; ++position) {
        const size_t cache_offset =
            emel::generator::detail::layer_cache_offset(backend, layer_index,
                                                        position, kv_dim) +
            kv_offset;
        emel_value_f16[static_cast<size_t>(position)] =
            kernel_quant::fp32_to_fp16(fp16_storage_to_fp32(
                backend.value_cache[cache_offset + static_cast<size_t>(dim)]));
        const std::span<const float> reference_value =
            have_reference_value_cache
        ...
        if (static_cast<size_t>(dim) < reference_value.size()) {
          reference_value_f16[static_cast<size_t>(position)] =
              kernel_quant::fp32_to_fp16(
                  reference_value[static_cast<size_t>(dim)]);
        } else {
          have_reference_value_dim = false;
        }
      }
      float dot = 0.0f;
      ggml_vec_dot_f16(position_limit, &dot, 0u, emel_value_f16.data(), 0u,
                       emel_weight_f16.data(), 0u, 1);
      ctx_emel_ggml_f16_dot[q_offset + static_cast<size_t>(dim)] = dot;
      if (have_reference_value_dim) {
        float reference_dot = 0.0f;
        ggml_vec_dot_f16(position_limit, &reference_dot, 0u,
                         reference_value_f16.data(), 0u,
                         reference_weight_f16.data(), 0u, 1);
        ctx_reference_all_ggml_f16_dot[q_offset + static_cast<size_t>(dim)] =
            reference_dot;
      }
    }
  }
  ...
  if (!have_reference_value_cache && value_capture == nullptr) {
    ...
    std::fprintf(stdout,
                 "%s.value_cache: max_abs=%g head=%d pos=%d dim=%d emel=%g "
                 "reference=%g\n",
                 layer_prefix.c_str(), value_cache_max_abs, value_cache_head,
                 value_cache_pos, value_cache_dim, value_cache_emel,
                 value_cache_reference);
    std::fprintf(
        stdout,
        "%s.value_cache_row: max_abs=%g pos=%d idx=%d emel=%g reference=%g\n",
        layer_prefix.c_str(), value_cache_row_max_abs, value_cache_row_pos,
        value_cache_row_idx, value_cache_row_emel, value_cache_row_reference);
  }
  ...
  if (softmax_capture == nullptr) {
    ...
    if (kq_capture != nullptr) {
      std::fprintf(stdout,
                   ...
                   layer_prefix.c_str(), kq_max_abs, kq_max_head, kq_max_pos,
                   kq_emel_at_max, kq_reference_at_max);
      std::fprintf(
          stdout,
          "%s.kq_full_q: max_abs=%g head=%d pos=%d emel=%g reference=%g\n",
          layer_prefix.c_str(), full_q_kq_max_abs, full_q_kq_max_head,
          full_q_kq_max_pos, full_q_kq_emel_at_max, full_q_kq_reference_at_max);
      std::fprintf(
          stdout,
          "%s.kq_ggml_f16: max_abs=%g head=%d pos=%d emel=%g reference=%g\n",
          layer_prefix.c_str(), ggml_f16_kq_max_abs, ggml_f16_kq_max_head,
          ggml_f16_kq_max_pos, ggml_f16_kq_emel_at_max,
          ggml_f16_kq_reference_at_max);
    } else {
    ...
    std::fprintf(
        ...
        layer_prefix.c_str(), max_abs, max_head, max_pos, emel_at_max,
        reference_at_max);
    std::fprintf(stdout,
                 "%s.kq_soft_max_full_q: max_abs=%g head=%d pos=%d emel=%g "
                 "reference=%g\n",
                 layer_prefix.c_str(), full_q_max_abs, full_q_max_head,
                 full_q_max_pos, full_q_emel_at_max, full_q_reference_at_max);
    std::fprintf(stdout,
                 "%s.kq_soft_max_shape: ne=[%" PRId64 ",%" PRId64 ",%" PRId64
                 ",%" PRId64 "]\n",
                 layer_prefix.c_str(), softmax_capture->shape[0],
                 softmax_capture->shape[1], softmax_capture->shape[2],
    ...
    if (softmax_capture->shape[0] > position_limit) {
      ...
      for (int32_t head = 0; head < head_count; ++head) {
        const std::span<const float> reference_probs =
            reference_softmax_query_head_slice(*softmax_capture, head, 0);
        for (int32_t pos = position_limit;
             pos < static_cast<int32_t>(reference_probs.size()); ++pos) {
          const float value =
              std::fabs(reference_probs[static_cast<size_t>(pos)]);
          if (value > tail_max) {
            tail_max = value;
            tail_head = head;
            tail_pos = pos;
      ...
      std::fprintf(stdout, "%s.kq_soft_max_tail: max_abs=%g head=%d pos=%d\n",
                   layer_prefix.c_str(), tail_max, tail_head, tail_pos);
    }
  }

  if (ctx_capture != nullptr) {
    std::fprintf(stdout,
                 "%s.kqv_out_shape: ne=[%" PRId64 ",%" PRId64 ",%" PRId64
                 ",%" PRId64 "]\n",
                 layer_prefix.c_str(), ctx_capture->shape[0],
                 ctx_capture->shape[1], ctx_capture->shape[2],
                 ctx_capture->shape[3]);
  }
  if (raw_ctx_capture != nullptr) {
    std::fprintf(stdout,
                 "%s.kqv_raw_shape: ne=[%" PRId64 ",%" PRId64 ",%" PRId64
                 ",%" PRId64 "]\n",
                 layer_prefix.c_str(), raw_ctx_capture->shape[0],
                 raw_ctx_capture->shape[1], raw_ctx_capture->shape[2],
  ...
  if (attn_out_capture != nullptr) {
    std::fprintf(stdout,
                 "%s.attn_out_shape: ne=[%" PRId64 ",%" PRId64 ",%" PRId64
                 ",%" PRId64 "]\n",
                 layer_prefix.c_str(), attn_out_capture->shape[0],
                 attn_out_capture->shape[1], attn_out_capture->shape[2],
  ...
  if (ffn_inp_capture != nullptr) {
    std::fprintf(stdout,
                 "%s.ffn_inp_shape: ne=[%" PRId64 ",%" PRId64 ",%" PRId64
                 ",%" PRId64 "]\n",
                 layer_prefix.c_str(), ffn_inp_capture->shape[0],
                 ffn_inp_capture->shape[1], ffn_inp_capture->shape[2],
  ...
  if (ffn_out_capture != nullptr) {
    std::fprintf(stdout,
                 "%s.ffn_out_shape: ne=[%" PRId64 ",%" PRId64 ",%" PRId64
                 ",%" PRId64 "]\n",
                 layer_prefix.c_str(), ffn_out_capture->shape[0],
                 ffn_out_capture->shape[1], ffn_out_capture->shape[2],
                 ffn_out_capture->shape[3]);
  }

  if (!reference_ctx.empty()) {
    auto dump_ctx_variant = [&](const char *suffix,
      ...
      std::fprintf(stdout, "%s.%s: max_abs=%g idx=%d emel=%g reference=%g\n",
                   layer_prefix.c_str(), suffix, ctx_max_abs, ctx_max_idx,
    ...
    dump_ctx_variant("kqv_out_from_reference_all_ggml_f16_dot",
                     ctx_reference_all_ggml_f16_dot);
    ...
    if (!raw_reference_ctx.empty()) {
      ...
      if (!reference_ctx.empty() &&
          raw_reference_ctx.size() == reference_ctx.size()) {
        float raw_to_out_max_abs = 0.0f;
        int32_t raw_to_out_idx = 0;
        for (int32_t idx = 0; idx < static_cast<int32_t>(reference_ctx.size());
             ++idx) {
          const float diff =
              std::fabs(raw_reference_ctx[static_cast<size_t>(idx)] -
                        reference_ctx[static_cast<size_t>(idx)]);
        ...
        std::fprintf(stdout,
                     "%s.kqv_raw_to_kqv_out_reference: max_abs=%g idx=%d "
                     "raw=%g out=%g\n",
                     layer_prefix.c_str(), raw_to_out_max_abs, raw_to_out_idx,
  ...
  if (position_limit == 1 && backend.n_head == backend.n_head_kv &&
    ...
    for (size_t idx = 0; idx < backend.attn_ctx.size(); ++idx) {
      internal_v_max_abs =
          std::max(internal_v_max_abs,
                   std::fabs(backend.attn_ctx[idx] - backend.v[idx]));
    }
    std::fprintf(stdout, "%s.kqv_out_from_internal_v: max_abs=%g\n",
                 layer_prefix.c_str(), internal_v_max_abs);
  }
...
void dump_logits_compare(const generation_load_state &state,
  ...
  if (reference_embedding != nullptr) {
    ...
    for (int32_t idx = 0; idx < state.model_data->params.n_embd; ++idx) {
      const float diff = std::fabs(emel_embedding[static_cast<size_t>(idx)] -
                                   reference_embedding[idx]);
    ...
    std::fprintf(
        ...
        embedding_max_abs, embedding_idx,
  ...
  std::fprintf(stdout,
               ...
               max_abs, max_idx, emel_logits[static_cast<size_t>(max_idx)],
               reference_logits[max_idx], emel_best, reference_best);
}

void dump_generation_tensor_compare(
    generation_load_state &state,
    const emel::paritychecker::parity_options &opts) {
  emel::generator::detail::native_backend backend = {};
  if (state.model_data == nullptr ||
  ...
  std::fprintf(stdout, "tensor_compare: prompt_tokens=%zu first=%d\n",
               prompt_tokens.size(), static_cast<int>(prompt_tokens.front()));
  std::fprintf(
      stdout,
      "tensor_compare.params: n_embd=%d n_head=%d n_head_kv=%d n_layer=%d "
      "n_rot=%d rms_eps=%g rope_freq_base=%g use_parallel_residual=%d "
      "embedding_scale=%g residual_scale=%g "
      "attn_clamp=%g attn_softcap=%g final_softcap=%g\n",
      state.model_data->params.n_embd, state.model_data->params.n_head,
      state.model_data->params.n_head_kv, state.model_data->params.n_layer,
  ...
  for (uint32_t idx = 0; idx < state.model_data->n_tensors; ++idx) {
    const auto name = emel::model::tensor_name_view(
        *state.model_data, state.model_data->tensors[idx]);
    if (name.find("bias") != std::string_view::npos) {
      if (bias_tensor_count < 8) {
        std::fprintf(stdout, "tensor_compare.bias[%d]: %.*s\n",
                     bias_tensor_count, static_cast<int>(name.size()),
                     name.data());
      }
      bias_tensor_count += 1;
    }
    if (name.find("q_norm") != std::string_view::npos ||
      ...
      if (extra_norm_tensor_count < 8) {
        std::fprintf(stdout, "tensor_compare.extra[%d]: %.*s\n",
                     extra_norm_tensor_count, static_cast<int>(name.size()),
                     name.data());
      }
  ...
  emel::text::conditioner::event::prepare prepare_ev{conditioned_count,
                                                     conditioned_error};
  prepare_ev.messages =
      emel::tools::generation_formatter_contract::single_user_messages(
  ...
  std::fprintf(
      stdout, "tensor_compare: conditioner accepted=%d count=%d error=%d",
      conditioned_accepted ? 1 : 0, conditioned_count, conditioned_error);
  for (int32_t idx = 0; idx < conditioned_count; ++idx) {
    std::fprintf(stdout, " %d", conditioned_tokens[static_cast<size_t>(idx)]);
  }
  std::fprintf(stdout, "\n");

  dump_row_compare("tensor_compare.token_embedding",
                   *backend.token_embedding.tensor, prompt_tokens.front());
  emel::model::llama::detail::block_view block_view = {};
  if (emel::model::llama::detail::lookup_block_view(backend.execution, 0,
                                                    block_view) ==
      emel::error::cast(emel::model::loader::error::none)) {
    dump_vector_compare("tensor_compare.layer0.attn_norm",
                        *block_view.attention_norm.tensor,
                        backend.blocks[0].attention_norm);
    dump_vector_compare("tensor_compare.layer0.ffn_norm",
                        *block_view.feed_forward_norm.tensor,
                        backend.blocks[0].feed_forward_norm);
  }
  if (emel::model::llama::detail::lookup_block_view(backend.execution, 1,
                                                    block_view) ==
      emel::error::cast(emel::model::loader::error::none)) {
    dump_vector_compare("tensor_compare.layer1.attn_norm",
                        *block_view.attention_norm.tensor,
                        backend.blocks[1].attention_norm);
    dump_vector_compare("tensor_compare.layer1.ffn_norm",
                        *block_view.feed_forward_norm.tensor,
                        backend.blocks[1].feed_forward_norm);
  }
  dump_vector_compare("tensor_compare.output_norm",
                      *backend.execution.output_norm.tensor,
                      backend.output_norm);

  reference_graph_capture graph_capture = {};
  const bool have_reference_graph =
      capture_reference_graph(state, opts, graph_capture);
  ...
  if (!emel::generator::detail::copy_tensor_row(*backend.token_embedding.tensor,
                                                prompt_tokens.front(),
                                                backend.hidden) ||
      !emel::generator::detail::rms_norm(backend.hidden,
                                         backend.blocks[0].attention_norm,
                                         backend.rms_epsilon, backend.norm)) {
  ...
  auto compare_layer = [&](const int32_t layer, const int32_t position) {
    ...
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.attn_norm_state", backend.norm,
                         find_reference_tensor(graph_capture, "attn_norm-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.attn_norm_state", backend.norm,
                         find_reference_tensor(graph_capture, "attn_norm-1"));
    }

    dump_matrix_compare((prefix + ".attn_q").c_str(), backend,
                        block.attention_q, backend.norm);
    dump_matrix_compare((prefix + ".attn_k").c_str(), backend,
                        block.attention_k, backend.norm);
    dump_matrix_compare((prefix + ".attn_v").c_str(), backend,
                        block.attention_v, backend.norm);
    ...
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.v_state", backend.v,
                         find_reference_tensor(graph_capture, "Vcur-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.v_state", backend.v,
    ...
    if (!compute_attention_with_softmax_debug(backend, graph_capture, layer,
                                              position + 1, prefix)) {
      std::fprintf(stdout, "%s: attention failed\n", prefix.c_str());
      return false;
    }
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.kqv_out_state",
                         backend.attn_ctx,
                         find_reference_tensor(graph_capture, "kqv_out-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.kqv_out_state",
                         backend.attn_ctx,
                         find_reference_tensor(graph_capture, "kqv_out-1"));
    }

    dump_matrix_compare((prefix + ".attn_out").c_str(), backend,
                        block.attention_output, backend.attn_ctx);
    if (!emel::generator::detail::matmul_vector(backend, block.attention_output,
                                                backend.attn_ctx,
                                                backend.projected)) {
    ...
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.attn_out_state",
                         backend.projected,
                         find_reference_tensor(graph_capture, "attn_out-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.attn_out_state",
                         backend.projected,
    ...
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.ffn_norm_state", backend.norm,
                         find_reference_tensor(graph_capture, "ffn_norm-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.ffn_norm_state", backend.norm,
                         find_reference_tensor(graph_capture, "ffn_norm-1"));
    }

    dump_matrix_compare((prefix + ".ffn_gate").c_str(), backend,
                        block.feed_forward_gate, backend.norm);
    dump_matrix_compare((prefix + ".ffn_up").c_str(), backend,
                        block.feed_forward_up, backend.norm);
    if (!emel::generator::detail::matmul_vector(
            backend, block.feed_forward_gate, backend.norm, backend.gate) ||
        !emel::generator::detail::matmul_vector(backend, block.feed_forward_up,
    ...
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.ffn_gate_state", backend.gate,
                         find_reference_tensor(graph_capture, "ffn_gate-0"));
      dump_state_compare("tensor_compare.layer0.ffn_up_state", backend.up,
                         find_reference_tensor(graph_capture, "ffn_up-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.ffn_gate_state", backend.gate,
                         find_reference_tensor(graph_capture, "ffn_gate-1"));
      dump_state_compare("tensor_compare.layer1.ffn_up_state", backend.up,
    ...
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.ffn_swiglu_state",
                         backend.ffn_hidden,
                         find_reference_tensor(graph_capture, "ffn_swiglu-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.ffn_swiglu_state",
                         backend.ffn_hidden,
                         find_reference_tensor(graph_capture, "ffn_swiglu-1"));
    }

    dump_matrix_compare((prefix + ".ffn_down").c_str(), backend,
                        block.feed_forward_down, backend.ffn_hidden);
    if (!emel::generator::detail::matmul_vector(
            backend, block.feed_forward_down, backend.ffn_hidden,
            backend.projected)) {
    ...
    if (layer == 0) {
      dump_state_compare("tensor_compare.layer0.l_out_state", backend.hidden,
                         find_reference_tensor(graph_capture, "l_out-0"));
    } else if (layer == 1) {
      dump_state_compare("tensor_compare.layer1.l_out_state", backend.hidden,
  ...
  if (!emel::generator::detail::rms_norm(backend.hidden, backend.output_norm,
                                         backend.rms_epsilon, backend.norm)) {
    std::fprintf(stdout, "tensor_compare: final rms_norm failed\n");
    return;
  }
  dump_state_compare("tensor_compare.result_norm_state", backend.norm,
                     find_reference_tensor(graph_capture, "result_norm"));
  dump_matrix_compare("tensor_compare.output", backend, backend.output,
                      backend.norm);
  if (!emel::generator::detail::matmul_vector(
          backend, backend.output, backend.norm, backend.bound_logits)) {
...
void dump_generation_prefix_state_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result) {
  ...
  int32_t runtime_exact_first_sig_step = -1;
  {
    ...
    if (emel::generator::detail::prepare(runtime_backend, *state.model_data) ==
            emel::error::cast(emel::model::loader::error::none) &&
        emel::generator::detail::prepare(exact_nonflash_backend,
                                         *state.model_data) ==
            emel::error::cast(emel::model::loader::error::none)) {
      ...
      for (size_t token_index = 0; token_index < prefix_tokens.size();
        ...
        const int32_t position = static_cast<int32_t>(token_index);
        if (!emel::generator::detail::copy_tensor_row(
                *runtime_backend.token_embedding.tensor, token_id,
                runtime_backend.hidden) ||
            !emel::generator::detail::copy_tensor_row(
                *exact_nonflash_backend.token_embedding.tensor, token_id,
                exact_nonflash_backend.hidden)) {
          break;
        }

        for (int32_t layer = 0; layer < runtime_backend.n_layer; ++layer) {
          if (!run_layer_with_flash_request_q_attn(runtime_backend, layer,
                                                   position) ||
              !run_layer_with_scalar_attention_ggml_nonflash_exact_masked(
          ...
          const float hidden_max_abs = runtime_exact_max_abs_diff(
              runtime_backend.hidden, exact_nonflash_backend.hidden);
          const float attn_max_abs = runtime_exact_max_abs_diff(
              runtime_backend.attn_ctx, exact_nonflash_backend.attn_ctx);
          const int32_t generated_index = static_cast<int32_t>(token_index) -
                                          static_cast<int32_t>(prompt_count);
      ...
      std::fprintf(
          ...
          "first_hidden_gt_1e-4=%d layer=%d hidden_max_abs=%g "
          "attn_max_abs=%g\n",
          first_hidden_step, first_hidden_layer, first_hidden_max_abs,
          first_hidden_attn_max_abs, first_sig_step, first_sig_layer,
          first_sig_hidden_max_abs, first_sig_attn_max_abs);
  ...
  if (target_generated_index > 0) {
    ...
    if (emel::generator::detail::prepare(scan_actor_backend,
                                         *state.model_data) ==
            emel::error::cast(emel::model::loader::error::none) &&
        emel::generator::detail::prepare(scan_q2_reference_backend,
                                         *state.model_data) ==
            emel::error::cast(emel::model::loader::error::none)) {
      const size_t prompt_count = prompt_tokens.size();
      for (size_t token_index = 0; token_index < prefix_tokens.size();
        ...
        if (!emel::generator::detail::copy_tensor_row(
                *scan_actor_backend.token_embedding.tensor, scan_token_id,
                scan_actor_backend.hidden) ||
            !emel::generator::detail::copy_tensor_row(
                *scan_q2_reference_backend.token_embedding.tensor,
                scan_token_id, scan_q2_reference_backend.hidden)) {
          break;
        }

        const bool is_prompt_token = token_index < prompt_count;
        const int32_t generated_index =
            static_cast<int32_t>(token_index - prompt_count);
        bool found_target = false;
        for (int32_t layer = 0; layer < scan_actor_backend.n_layer; ++layer) {
          if (!run_layer_with_scalar_attention(scan_actor_backend, layer,
                                               scan_position) ||
              !run_layer_with_matmul_mode_scalar_attention(
                  scan_q2_reference_backend, layer, scan_position,
                  reference_q2_only)) {
            found_target = false;
            break;
          }
          ...
          if (!is_prompt_token && layer == 1 &&
              local_max_abs_diff(scan_actor_backend.hidden,
  ...
  if (!capture_reference_graph_for_generation_prefix(
          state, prompt_tokens, generated_prefix, graph_capture)) {
    std::fprintf(stdout,
                 "generation_debug.state: reference graph capture failed\n");
    return;
  }

  const std::span<const float> reference_layer1_l_out =
      find_reference_tensor(graph_capture, "l_out-1");
  const auto dump_attention_mode = [&](const char *label,
                                       const auto &run_prefill) {
    if (reference_layer1_l_out.empty()) {
      std::fprintf(stdout,
                   "generation_debug.mode_summary.%s: reference unavailable\n",
                   label);
      return;
    }
    ...
    std::fprintf(
        stdout, "generation_debug.mode_summary.%s.layer1_l_out: max_abs=%g\n",
        label, local_max_abs_diff(mode_backend.hidden, reference_layer1_l_out));
  };
  ...
  dump_attention_mode("runtime_flash_q_attn",
                      run_prefill_with_flash_request_q_attn);
  dump_attention_mode("emel_prod_style",
                      run_prefill_with_scalar_attention_emel_prod_style);
  dump_attention_mode("no_weight_rounding",
                      run_prefill_with_scalar_attention_no_weight_rounding);
  dump_attention_mode("rounded_weight_scalar",
                      run_prefill_with_scalar_attention_rounded_weight_scalar);
  dump_attention_mode(
      "ggml_f16_value_contraction",
      run_prefill_with_scalar_attention_ggml_f16_value_contraction);
  dump_attention_mode("online_f32",
                      run_prefill_with_scalar_attention_online_f32);
  dump_attention_mode("double_softmax_sum",
                      run_prefill_with_scalar_attention_double_softmax_sum);
  dump_attention_mode("ggml_softmax",
                      run_prefill_with_scalar_attention_ggml_softmax);
  dump_attention_mode("ggml_online_f16",
                      run_prefill_with_scalar_attention_ggml_online_f16);
  dump_attention_mode("ggml_nonflash_f16",
                      run_prefill_with_scalar_attention_ggml_nonflash_f16);
  dump_attention_mode("ggml_f16_scores",
                      run_prefill_with_scalar_attention_ggml_f16_scores);
  dump_attention_mode("full_q", run_prefill_with_scalar_attention_full_q);
  dump_attention_mode(
      "full_q_no_weight_rounding",
      run_prefill_with_scalar_attention_full_q_no_weight_rounding);
  dump_attention_mode("full_q_rounded_weight",
                      run_prefill_with_scalar_attention_full_q_rounded_weight);
  dump_attention_mode(
      "full_q_ggml_f16_value_contraction",
      run_prefill_with_scalar_attention_full_q_ggml_f16_value_contraction);
  ...
  const auto dump_full_generation_mode =
      [&](const char *label, const native_layer_runner run_layer_fn) {
        ...
        const emel::error::type alt_err = run_custom_native_generate(
            const_cast<generation_load_state &>(state), opts, run_layer_fn,
            alt_result);
        if (alt_err == emel::error::cast(emel::generator::error::none)) {
          std::fprintf(
              stdout,
        ...
        std::fprintf(stdout, "generation_debug.mode_full.%s: error=%d\n", label,
                     static_cast<int>(alt_err));
      };

  dump_full_generation_mode(
      "rounded_weight_scalar",
      run_layer_with_scalar_attention_rounded_weight_scalar);
  dump_full_generation_mode("runtime_flash_q_attn",
                            run_layer_with_flash_request_q_attn);
  dump_full_generation_mode(
      "ggml_flash_ext_masked",
      run_layer_with_scalar_attention_ggml_flash_ext_masked);
  dump_full_generation_mode("emel_prod_style",
                            run_layer_with_scalar_attention_emel_prod_style);
  dump_full_generation_mode(
      "emel_prod_style_float_value",
      run_layer_with_scalar_attention_emel_prod_style_float_value);
  dump_full_generation_mode(
      "emel_prod_style_float_value_reference_q2",
      run_layer_with_scalar_attention_emel_prod_style_float_value_reference_q2);
  dump_full_generation_mode("ggml_f16_scores",
                            run_layer_with_scalar_attention_ggml_f16_scores);
  dump_full_generation_mode(
      "ggml_f16_scores_ggml_softmax",
      run_layer_with_scalar_attention_ggml_f16_scores_ggml_softmax);
  dump_full_generation_mode(
      "ggml_f16_value_contraction",
      run_layer_with_scalar_attention_ggml_f16_value_contraction);
  dump_full_generation_mode("online_f32",
                            run_layer_with_scalar_attention_online_f32);
  dump_full_generation_mode("ggml_online_f16",
                            run_layer_with_scalar_attention_ggml_online_f16);
  dump_full_generation_mode("ggml_nonflash_f16",
                            run_layer_with_scalar_attention_ggml_nonflash_f16);
  dump_full_generation_mode(
      "ggml_nonflash_f16_ggml_softmax",
      run_layer_with_scalar_attention_ggml_nonflash_f16_ggml_softmax);
  dump_full_generation_mode(
      "ggml_nonflash_exact_masked",
      run_layer_with_scalar_attention_ggml_nonflash_exact_masked);
  dump_full_generation_mode(
      "ggml_nonflash_exact_masked_full_q",
      run_layer_with_scalar_attention_ggml_nonflash_exact_masked_full_q);
  dump_full_generation_mode(
      "ggml_nonflash_exact_scores_prod_value",
      run_layer_with_scalar_attention_ggml_nonflash_exact_scores_prod_value);
  dump_full_generation_mode("exact_attention",
                            run_layer_with_exact_attention_scalar_attention);
  dump_full_generation_mode("exact_q2",
                            run_layer_with_exact_q2_scalar_attention);
  dump_full_generation_mode("exact_q3",
                            run_layer_with_exact_q3_scalar_attention);
  dump_full_generation_mode("exact_q6",
                            run_layer_with_exact_q6_scalar_attention);
  dump_full_generation_mode("scalar_q2",
                            run_layer_with_scalar_q2_scalar_attention);
  dump_full_generation_mode("scalar_q3",
                            run_layer_with_scalar_q3_scalar_attention);
  dump_full_generation_mode("scalar_q6",
                            run_layer_with_scalar_q6_scalar_attention);
  dump_full_generation_mode("scalar_q236",
                            run_layer_with_scalar_q236_scalar_attention);
  dump_full_generation_mode("reference_q2",
                            run_layer_with_reference_q2_scalar_attention);
  dump_full_generation_mode("reference_q3",
                            run_layer_with_reference_q3_scalar_attention);
  dump_full_generation_mode("reference_q6",
                            run_layer_with_reference_q6_scalar_attention);
  dump_full_generation_mode("reference_q236",
                            run_layer_with_reference_q236_scalar_attention);

  llama_context_ptr reference_ctx =
      make_reference_context(const_cast<reference_backend &>(state.reference));
  if (reference_ctx == nullptr ||
      !run_reference_prefix_decode(reference_ctx.get(), prompt_tokens,
                                   generated_prefix)) {
    std::fprintf(stdout,
                 "generation_debug.state: reference decode replay failed\n");
    return;
  }
  ...
  if (prefix_tokens.size() > 1u) {
    ...
    if (!run_prefill_from_token_prefix(backend, prior_tokens)) {
      std::fprintf(stdout,
                   "generation_debug.state: prior prefix replay failed\n");
      return;
    }
  } else {
    backend.kv_cache_tokens = 0;
  ...
  if (!emel::generator::detail::copy_tensor_row(*backend.token_embedding.tensor,
                                                token_id, backend.hidden)) {
    std::fprintf(stdout,
                 "generation_debug.state: token embedding replay failed\n");
    return;
  }
  ...
  auto compare_layer = [&](const int32_t layer) -> bool {
    ...
    const std::string layer_prefix =
        "generation_debug.state.layer" + std::to_string(layer);
    ...
    dump_state_compare(
        (layer_prefix + ".attn_norm").c_str(), backend.norm,
        find_reference_tensor(graph_capture,
                              ("attn_norm-" + layer_suffix).c_str()));
    dump_q8_quantize_compare((layer_prefix + ".attn_norm_q8").c_str(),
                             backend.norm);
    dump_matrix_compare((layer_prefix + ".attn_q").c_str(), backend,
                        block.attention_q, backend.norm);
    dump_matrix_compare_reference_q8((layer_prefix + ".attn_q_refq8").c_str(),
                                     backend, block.attention_q, backend.norm);
    dump_matrix_compare((layer_prefix + ".attn_k").c_str(), backend,
                        block.attention_k, backend.norm);
    dump_matrix_compare_reference_q8((layer_prefix + ".attn_k_refq8").c_str(),
                                     backend, block.attention_k, backend.norm);
    dump_matrix_compare((layer_prefix + ".attn_v").c_str(), backend,
                        block.attention_v, backend.norm);
    dump_matrix_compare_reference_q8((layer_prefix + ".attn_v_refq8").c_str(),
                                     backend, block.attention_v, backend.norm);
    ...
    dump_state_compare(
        (layer_prefix + ".v").c_str(), backend.v,
    ...
    emel::generator::detail::store_fp16_rounded_cache(
        ...
        backend.q_attn.data());
    dump_state_compare(
        (layer_prefix + ".q").c_str(), backend.q,
        find_reference_tensor(graph_capture, ("Qcur-" + layer_suffix).c_str()));
    {
      const std::span<const float> reference_q = reference_last_token_row(
          find_reference_tensor(graph_capture,
                                ("Qcur-" + layer_suffix).c_str()),
          static_cast<size_t>(backend.n_head * backend.head_dim));
      if (reference_q.size() == backend.q_attn.size()) {
        std::vector<float> reference_q_attn(reference_q.begin(),
                                            reference_q.end());
        emel::generator::detail::store_fp16_rounded_cache(
            reference_q, reference_q_attn.data());
        dump_state_compare((layer_prefix + ".q_attn").c_str(), backend.q_attn,
                           reference_q_attn);
      } else {
    ...
    dump_state_compare(
        (layer_prefix + ".k").c_str(), backend.k,
    ...
    {
      auto flash_request = emel::generator::detail::make_flash_attn_request(
          backend, layer, position);
      std::vector<float> shared_flash_ctx(backend.attn_ctx.size(), 0.0f);
      flash_request.dst = emel::generator::detail::make_dst_view_3d(
          shared_flash_ctx.data(), flash_request.dst.ne[0],
          flash_request.dst.ne[1], flash_request.dst.ne[2]);
      ...
      if (emel::kernel::detail::run_flash_attn_ext_with_workspace(
        ...
        dump_state_compare(
            (layer_prefix + ".kqv_out_shared_flash").c_str(), shared_flash_ctx,
            find_reference_tensor(graph_capture,
                                  ("kqv_out-" + layer_suffix).c_str()));
      } else {
      ...
      const std::span<const float> reference_value_prefix =
          reference_value_cache.size() >=
                  static_cast<size_t>((position + 1) * kv_dim)
              ? std::span<const float>(
                    reference_value_cache.data(),
                    static_cast<size_t>((position + 1) * kv_dim))
              : std::span<const float>{};
      const reference_tensor_capture *softmax_capture_current =
          find_reference_capture(graph_capture,
                                 ("kq_soft_max-" + layer_suffix).c_str());
      const int32_t total_kv_tokens =
          softmax_capture_current != nullptr &&
                  softmax_capture_current->shape[0] > 0
              ? static_cast<int32_t>(softmax_capture_current->shape[0])
              : backend.n_ctx;
      std::vector<float> reference_key_padded(
          static_cast<size_t>(total_kv_tokens) * static_cast<size_t>(kv_dim),
          0.0f);
      std::vector<float> reference_value_padded(
          static_cast<size_t>(total_kv_tokens) * static_cast<size_t>(kv_dim),
          0.0f);
      if (!reference_key_prefix.empty()) {
        std::copy(reference_key_prefix.begin(), reference_key_prefix.end(),
                  reference_key_padded.begin());
      }
      if (!reference_value_prefix.empty()) {
        std::copy(reference_value_prefix.begin(), reference_value_prefix.end(),
      ...
      if (!reference_key_prefix.empty()) {
        ...
        for (int32_t cache_pos = 0; cache_pos <= position; ++cache_pos) {
          const size_t cache_row_offset =
              emel::generator::detail::layer_cache_offset(backend, layer,
                                                          cache_pos, kv_dim);
          ...
          const std::span<const float> reference_cache_row =
              reference_key_prefix.subspan(static_cast<size_t>(cache_pos) *
                                               static_cast<size_t>(kv_dim),
                                           static_cast<size_t>(kv_dim));
          for (int32_t head = 0; head < backend.n_head_kv; ++head) {
            for (int32_t dim = 0; dim < backend.head_dim_kv; ++dim) {
              ...
              const float diff = std::fabs(
                  read_debug_value(emel_cache_row, static_cast<size_t>(idx)) -
        ...
        std::fprintf(stdout,
                     "%s.key_cache: max_abs=%g head=%d pos=%d dim=%d emel=%g "
                     "reference=%g\n",
                     layer_prefix.c_str(), key_cache_max_abs, key_cache_head,
                     key_cache_pos, key_cache_dim, key_cache_emel,
                     key_cache_reference);
      }

      const auto dump_ggml_flash_case = [&](const std::string &label,
                                            std::span<const float> q_rows,
                                            const auto k_rows,
                                            const auto v_rows) {
        std::vector<float> ggml_flash_ctx;
        if (run_ggml_flash_attn_ext_case(
                q_rows, k_rows, v_rows, static_cast<int64_t>(backend.head_dim),
                static_cast<int64_t>(position + 1),
                static_cast<int64_t>(backend.n_head),
                static_cast<int64_t>(backend.n_head_kv),
      ...
      const auto dump_ggml_flash_masked_case =
          [&](const std::string &label, std::span<const float> q_rows,
              const auto k_rows, const auto v_rows) {
            std::vector<float> ggml_flash_ctx;
            if (run_ggml_flash_attn_ext_masked_case(
      ...
      const auto dump_ggml_nonflash_case = [&](const std::string &label,
                                               std::span<const float> q_rows,
                                               const auto k_rows,
                                               const auto v_rows) {
      ...
      const auto dump_emel_nonflash_case = [&](const std::string &label,
                                               std::span<const float> q_rows,
                                               const auto k_rows,
                                               const auto v_rows) {
      ...
      const auto dump_emel_prod_style_case = [&](const std::string &label,
                                                 std::span<const float> q_rows,
                                                 const auto k_rows,
                                                 const auto v_rows) {
      ...
      dump_ggml_flash_masked_case(
          layer_prefix + ".kqv_out_ggml_flash_masked_reference_qkv",
          reference_q, reference_key_padded, reference_value_padded);
      dump_ggml_flash_masked_case(
          layer_prefix + ".kqv_out_ggml_flash_masked_reference_q_emel_kv",
          reference_q,
          std::span<const uint16_t>(
              backend.key_cache.data() + layer_cache_offset,
          ...
          std::span<const uint16_t>(
              ...
              static_cast<size_t>(total_kv_tokens * kv_dim)));
      dump_ggml_flash_masked_case(
          layer_prefix + ".kqv_out_ggml_flash_masked_emel_q_reference_kv",
          std::span<const float>(
              backend.q.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          reference_key_padded, reference_value_padded);
      dump_ggml_flash_case(layer_prefix + ".kqv_out_ggml_flash_reference_qkv",
                           reference_q, reference_key_prefix,
                           reference_value_prefix);
      dump_ggml_flash_case(
          layer_prefix + ".kqv_out_ggml_flash_reference_q_emel_kv", reference_q,
          std::span<const uint16_t>(
              backend.key_cache.data() + layer_cache_offset,
              static_cast<size_t>((position + 1) * kv_dim)),
          std::span<const uint16_t>(
              ...
              static_cast<size_t>((position + 1) * kv_dim)));
      dump_ggml_flash_case(
          layer_prefix + ".kqv_out_ggml_flash_emel_q_reference_kv",
          std::span<const float>(
              backend.q.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          reference_key_prefix, reference_value_prefix);
      dump_ggml_nonflash_case(
          layer_prefix + ".kqv_out_ggml_nonflash_reference_qkv", reference_q,
          reference_key_padded, reference_value_padded);
      dump_emel_nonflash_case(
          layer_prefix + ".kqv_out_emel_nonflash_reference_qkv", reference_q,
          reference_key_padded, reference_value_padded);
      dump_emel_prod_style_case(
          layer_prefix + ".kqv_out_emel_prod_reference_qkv", reference_q,
          reference_key_padded, reference_value_padded);
      dump_ggml_nonflash_case(
          layer_prefix + ".kqv_out_ggml_nonflash_reference_q_emel_kv",
          reference_q,
          std::span<const uint16_t>(
              backend.key_cache.data() + layer_cache_offset,
              static_cast<size_t>(total_kv_tokens * kv_dim)),
          std::span<const uint16_t>(
              ...
              static_cast<size_t>(total_kv_tokens * kv_dim)));
      dump_emel_nonflash_case(
          layer_prefix + ".kqv_out_emel_nonflash_reference_q_emel_kv",
          reference_q,
          std::span<const uint16_t>(
              backend.key_cache.data() + layer_cache_offset,
              static_cast<size_t>(total_kv_tokens * kv_dim)),
          std::span<const uint16_t>(
              ...
              static_cast<size_t>(total_kv_tokens * kv_dim)));
      dump_ggml_nonflash_case(
          layer_prefix + ".kqv_out_ggml_nonflash_emel_q_reference_kv",
          std::span<const float>(
              backend.q.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          reference_key_padded, reference_value_padded);
      dump_emel_nonflash_case(
          layer_prefix + ".kqv_out_emel_nonflash_emel_q_reference_kv",
          std::span<const float>(
              backend.q.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          reference_key_padded, reference_value_padded);
      dump_emel_nonflash_case(
          layer_prefix + ".kqv_out_emel_nonflash_emel_q",
          std::span<const float>(
              backend.q.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          std::span<const uint16_t>(
          ...
          std::span<const uint16_t>(
              ...
              static_cast<size_t>(total_kv_tokens * kv_dim)));
      dump_emel_prod_style_case(
          layer_prefix + ".kqv_out_emel_prod_emel_q",
          std::span<const float>(
              backend.q.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          std::span<const uint16_t>(
          ...
          std::span<const uint16_t>(
              ...
              static_cast<size_t>(total_kv_tokens * kv_dim)));
      dump_ggml_nonflash_case(
          layer_prefix + ".kqv_out_ggml_nonflash_emel_q_attn",
          std::span<const float>(
              backend.q_attn.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          std::span<const uint16_t>(
          ...
          std::span<const uint16_t>(
              ...
              static_cast<size_t>(total_kv_tokens * kv_dim)));
      dump_emel_prod_style_case(
          layer_prefix + ".kqv_out_emel_prod_emel_q_attn",
          std::span<const float>(
              backend.q_attn.data(),
              static_cast<size_t>(backend.n_head * backend.head_dim)),
          std::span<const uint16_t>(
      ...
      if (run_ggml_flash_attn_ext_case(
              ...
              ggml_flash_ctx)) {
        dump_state_compare(
            (layer_prefix + ".kqv_out_ggml_flash_emel_q").c_str(),
            ggml_flash_ctx,
            find_reference_tensor(graph_capture,
                                  ("kqv_out-" + layer_suffix).c_str()));
      } else {
        std::fprintf(stdout, "%s.kqv_out_ggml_flash_emel_q: compute failed\n",
                     layer_prefix.c_str());
      }
      ...
      if (run_ggml_flash_attn_ext_masked_case(
        ...
        dump_state_compare(
            (layer_prefix + ".kqv_out_ggml_flash_masked_emel_q").c_str(),
            ggml_flash_masked_ctx,
            find_reference_tensor(graph_capture,
                                  ("kqv_out-" + layer_suffix).c_str()));
      } else {
      ...
      if (run_ggml_nonflash_attn_case(
              ...
              ggml_nonflash_ctx)) {
        dump_state_compare(
            (layer_prefix + ".kqv_out_ggml_nonflash_emel_q").c_str(),
            ggml_nonflash_ctx,
            find_reference_tensor(graph_capture,
                                  ("kqv_out-" + layer_suffix).c_str()));
      } else {
      ...
      neon_flash_request.dst = emel::generator::detail::make_dst_view_3d(
          neon_flash_ctx.data(), neon_flash_request.dst.ne[0],
          neon_flash_request.dst.ne[1], neon_flash_request.dst.ne[2]);
      emel::kernel::detail::flash_attn_workspace neon_flash_workspace = {};
      if (emel::kernel::aarch64::detail::run_flash_attn_ext_neon(
              neon_flash_request, true, neon_flash_workspace)) {
        dump_state_compare(
            (layer_prefix + ".kqv_out_neon_flash").c_str(), neon_flash_ctx,
            find_reference_tensor(graph_capture,
                                  ("kqv_out-" + layer_suffix).c_str()));
      } else {
    ...
    dump_state_compare((layer_prefix + ".kqv_out").c_str(), backend.attn_ctx,
                       find_reference_tensor(
                           graph_capture, ("kqv_out-" + layer_suffix).c_str()));
    dump_q8_quantize_compare((layer_prefix + ".attn_ctx_q8").c_str(),
                             backend.attn_ctx);
    dump_matrix_compare((layer_prefix + ".attn_out_matmul").c_str(), backend,
                        block.attention_output, backend.attn_ctx);
    dump_matrix_compare_reference_q8(
        (layer_prefix + ".attn_out_matmul_refq8").c_str(), backend,
        block.attention_output, backend.attn_ctx);
    ...
    dump_state_compare(
        (layer_prefix + ".attn_out").c_str(), backend.projected,
        find_reference_tensor(graph_capture,
                              ("attn_out-" + layer_suffix).c_str()));

    for (int32_t idx = 0; idx < backend.n_embd; ++idx) {
      backend.hidden[static_cast<size_t>(idx)] +=
          backend.projected[static_cast<size_t>(idx)];
    }
    dump_state_compare((layer_prefix + ".ffn_inp").c_str(), backend.hidden,
                       find_reference_tensor(
                           graph_capture, ("ffn_inp-" + layer_suffix).c_str()));
    ...
    dump_state_compare(
        (layer_prefix + ".ffn_norm").c_str(), backend.norm,
        find_reference_tensor(graph_capture,
                              ("ffn_norm-" + layer_suffix).c_str()));
    if (layer == 0) {
      const std::span<const float> reference_ffn_inp = find_reference_tensor(
          graph_capture, ("ffn_inp-" + layer_suffix).c_str());
      const std::span<const float> reference_ffn_norm = find_reference_tensor(
          graph_capture, ("ffn_norm-" + layer_suffix).c_str());
      if (!reference_ffn_inp.empty() && !reference_ffn_norm.empty()) {
        std::vector<float> emel_reference_ffn_norm(reference_ffn_norm.size(),
                                                   0.0f);
        if (emel::generator::detail::rms_norm(
                reference_ffn_inp, block.feed_forward_norm, backend.rms_epsilon,
                emel_reference_ffn_norm)) {
          dump_state_compare(
              (layer_prefix + ".ffn_norm_from_reference_inp").c_str(),
              emel_reference_ffn_norm, reference_ffn_norm);

          const std::span<const float> reference_ffn_gate =
              find_reference_tensor(graph_capture,
                                    ("ffn_gate-" + layer_suffix).c_str());
          const std::span<const float> reference_ffn_up = find_reference_tensor(
              graph_capture, ("ffn_up-" + layer_suffix).c_str());
          if (!reference_ffn_gate.empty() && !reference_ffn_up.empty()) {
            std::vector<float> emel_reference_ffn_gate(
                reference_ffn_gate.size(), 0.0f);
            std::vector<float> emel_reference_ffn_up(reference_ffn_up.size(),
                                                     0.0f);
            if (emel::generator::detail::matmul_vector(
                    backend, block.feed_forward_gate, emel_reference_ffn_norm,
                    emel_reference_ffn_gate) &&
                emel::generator::detail::matmul_vector(
                    backend, block.feed_forward_up, emel_reference_ffn_norm,
                    emel_reference_ffn_up)) {
              dump_state_compare(
                  (layer_prefix + ".ffn_gate_from_reference_norm").c_str(),
                  emel_reference_ffn_gate, reference_ffn_gate);
              dump_state_compare(
                  (layer_prefix + ".ffn_up_from_reference_norm").c_str(),
                  emel_reference_ffn_up, reference_ffn_up);
            }
          }
        }
      }
    }
    dump_q8_quantize_compare((layer_prefix + ".ffn_norm_q8").c_str(),
                             backend.norm);
    dump_matrix_compare((layer_prefix + ".ffn_gate_matmul").c_str(), backend,
                        block.feed_forward_gate, backend.norm);
    dump_matrix_compare_reference_q8(
        (layer_prefix + ".ffn_gate_matmul_refq8").c_str(), backend,
        block.feed_forward_gate, backend.norm);
    dump_matrix_compare((layer_prefix + ".ffn_up_matmul").c_str(), backend,
                        block.feed_forward_up, backend.norm);
    dump_matrix_compare_reference_q8(
        (layer_prefix + ".ffn_up_matmul_refq8").c_str(), backend,
        block.feed_forward_up, backend.norm);
    ...
    dump_state_compare(
        (layer_prefix + ".ffn_gate").c_str(), backend.gate,
        find_reference_tensor(graph_capture,
                              ("ffn_gate-" + layer_suffix).c_str()));
    dump_state_compare((layer_prefix + ".ffn_up").c_str(), backend.up,
                       find_reference_tensor(
                           graph_capture, ("ffn_up-" + layer_suffix).c_str()));
    ...
    dump_state_compare(
        (layer_prefix + ".ffn_swiglu").c_str(), backend.ffn_hidden,
        find_reference_tensor(graph_capture,
                              ("ffn_swiglu-" + layer_suffix).c_str()));
    dump_q8_quantize_compare((layer_prefix + ".ffn_hidden_q8").c_str(),
                             backend.ffn_hidden);
    dump_matrix_compare((layer_prefix + ".ffn_down_matmul").c_str(), backend,
                        block.feed_forward_down, backend.ffn_hidden);
    dump_matrix_compare_reference_q8(
        (layer_prefix + ".ffn_down_matmul_refq8").c_str(), backend,
        block.feed_forward_down, backend.ffn_hidden);
    ...
    dump_state_compare((layer_prefix + ".ffn_out").c_str(), backend.projected,
                       find_reference_tensor(
                           graph_capture, ("ffn_out-" + layer_suffix).c_str()));

    for (int32_t idx = 0; idx < backend.n_embd; ++idx) {
      backend.hidden[static_cast<size_t>(idx)] +=
          backend.projected[static_cast<size_t>(idx)];
    }
    dump_state_compare((layer_prefix + ".l_out").c_str(), backend.hidden,
                       find_reference_tensor(
                           graph_capture, ("l_out-" + layer_suffix).c_str()));
    return true;
  };
  ...
  if (!emel::generator::detail::rms_norm(backend.hidden, backend.output_norm,
                                         backend.rms_epsilon, backend.norm)) {
    std::fprintf(stdout,
                 "generation_debug.state.result_norm: rms_norm failed\n");
    return;
  }
  dump_state_compare("generation_debug.state.result_norm", backend.norm,
                     find_reference_tensor(graph_capture, "result_norm"));
}

double l2_norm_diff(std::span<const float> emel_values,
                    std::span<const float> reference_values) {
...
float max_abs_diff(std::span<const float> emel_values,
                   std::span<const float> reference_values) {
  if (emel_values.size() != reference_values.size()) {
    return -1.0f;
  }
  ...
  for (size_t idx = 0; idx < emel_values.size(); ++idx) {
    max_abs =
        std::max(max_abs, std::fabs(emel_values[idx] - reference_values[idx]));
  }
  return max_abs;
}
...
float max_abs_diff(std::span<const uint16_t> emel_values,
  ...
  for (size_t idx = 0; idx < emel_values.size(); ++idx) {
    max_abs =
        std::max(max_abs, std::fabs(fp16_storage_to_fp32(emel_values[idx]) -
                                    reference_values[idx]));
  }
  return max_abs;
}
...
std::span<const float>
reference_token_row(std::span<const float> reference_values,
                    const size_t row_width, const size_t row_index) {
  if (row_width == 0 || reference_values.empty() ||
      reference_values.size() % row_width != 0) {
    return {};
  }
...
std::vector<float>
reference_token_tensor_values(const reference_tensor_capture &capture,
                              const int32_t token_index) {
  const int64_t ne0 = std::max<int64_t>(capture.shape[0], 1);
  const int64_t ne1 = std::max<int64_t>(capture.shape[1], 1);
  const int64_t ne2 = std::max<int64_t>(capture.shape[2], 1);
  ...
  if (ne3 > 1 && token_index < ne3) {
    ...
    return std::vector<float>(
        capture.values.begin() + static_cast<std::ptrdiff_t>(offset),
  ...
  for (int64_t i3 = 0; i3 < ne3; ++i3) {
    for (int64_t i2 = 0; i2 < ne2; ++i2) {
      ...
      std::copy_n(capture.values.begin() +
                      static_cast<std::ptrdiff_t>(capture_offset),
                  static_cast<size_t>(ne0),
...
void dump_generation_prefix_timeline_debug(
  ...
  std::vector<int32_t> prefix_tokens;
  prefix_tokens.reserve(prompt_tokens.size() +
                        static_cast<size_t>(prefix_generated_tokens));
  for (const llama_token token : prompt_tokens) {
    prefix_tokens.push_back(static_cast<int32_t>(token));
  }
  ...
  reference_graph_capture graph_capture = {};
  if (!capture_reference_graph_for_tokens(state, prefix_tokens_llama,
                                          graph_capture)) {
    std::fprintf(stdout,
                 "generation_debug.timeline: reference graph capture failed\n");
    return;
  }
  ...
  if (emel::generator::detail::prepare(backend, *state.model_data) !=
      ...
      emel::generator::detail::prepare(exact_q3_backend, *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none) ||
      emel::generator::detail::prepare(reference_q3_backend,
                                       *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none)) {
  ...
  if (reference_ctx == nullptr) {
    std::fprintf(stdout,
                 "generation_debug.timeline: reference context failed\n");
    return;
  }

  const reference_tensor_capture *layer0_v_capture =
      find_reference_capture(graph_capture, "Vcur-0");
  const reference_tensor_capture *layer0_q_capture =
      find_reference_capture(graph_capture, "Qcur-0");
  const reference_tensor_capture *layer0_k_capture =
      find_reference_capture(graph_capture, "Kcur-0");
  const std::span<const float> layer0_kqv_out_capture =
      find_reference_tensor(graph_capture, "kqv_out-0");
  const std::span<const float> layer0_attn_out_capture =
      find_reference_tensor(graph_capture, "attn_out-0");
  const reference_tensor_capture *layer1_v_capture =
      find_reference_capture(graph_capture, "Vcur-1");
  const size_t prompt_count = prompt_tokens.size();
  ...
  std::array<int32_t, cache_thresholds.size()>
      layer1_key_cache_first_generated = {};
  std::array<int32_t, cache_thresholds.size()>
      layer1_value_cache_first_generated = {};
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    if (llama_decode(reference_ctx.get(), decode_batch) != 0) {
      std::fprintf(stdout,
                   "generation_debug.timeline: reference decode failed\n");
      return;
    }
    if (!emel::generator::detail::copy_tensor_row(
            *backend.token_embedding.tensor, token_id, backend.hidden)) {
      std::fprintf(
          stdout, "generation_debug.timeline: token embedding replay failed\n");
      return;
    }
    ...
    for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention(backend, layer, position)) {
        std::fprintf(stdout,
                     "generation_debug.timeline: layer replay failed\n");
        return;
      }
      ...
      const std::string token_prefix =
          is_prompt_token
              ? "generation_debug.timeline.prompt" + std::to_string(token_index)
      ...
      if (layer == 0) {
        std::vector<float> reference_layer0_key_cache;
        std::vector<float> reference_layer0_value_cache;
        if (capture_reference_key_cache_rows(reference_ctx.get(), 0,
                                             reference_layer0_key_cache) &&
            reference_layer0_key_cache.size() ==
                static_cast<size_t>((position + 1) * kv_dim)) {
          float max_cache_abs = 0.0f;
          for (int32_t cache_pos = 0; cache_pos <= position; ++cache_pos) {
            const size_t cache_offset_compare =
                emel::generator::detail::layer_cache_offset(backend, layer,
                                                            cache_pos, kv_dim);
            const std::span<const uint16_t> emel_cache_row{
            ...
            const std::span<const float> reference_cache_row =
                std::span<const float>(reference_layer0_key_cache)
                    .subspan(static_cast<size_t>(cache_pos) *
                                 static_cast<size_t>(kv_dim),
                             static_cast<size_t>(kv_dim));
            max_cache_abs =
                std::max(max_cache_abs,
                         max_abs_diff(emel_cache_row, reference_cache_row));
          }
          std::fprintf(stdout, "%s.layer0_key_cache: max_abs=%g\n",
                       token_prefix.c_str(), max_cache_abs);
        }
        if (capture_reference_value_cache_rows(reference_ctx.get(), 0,
                                               reference_layer0_value_cache) &&
            reference_layer0_value_cache.size() ==
                static_cast<size_t>((position + 1) * kv_dim)) {
          float max_cache_abs = 0.0f;
          for (int32_t cache_pos = 0; cache_pos <= position; ++cache_pos) {
          ...
          std::fprintf(stdout, "%s.layer0_value_cache: max_abs=%g\n",
                       token_prefix.c_str(), max_cache_abs);
          if (token_index == 0u) {
            const std::span<const float> reference_cache_row =
                std::span<const float>(reference_layer0_value_cache)
                    .subspan(static_cast<size_t>(position) *
                                 static_cast<size_t>(kv_dim),
                             static_cast<size_t>(kv_dim));
            std::vector<float> ggml_rounded_cache_row(
                static_cast<size_t>(kv_dim));
            for (int32_t idx = 0; idx < kv_dim; ++idx) {
            ...
            dump_state_compare(
                (token_prefix + ".layer0_value_cache_ggml_round").c_str(),
                ggml_rounded_cache_row, reference_cache_row);
            dump_state_compare(
                (token_prefix + ".layer0_value_cache_vs_ggml_round").c_str(),
                std::span<const uint16_t>(backend.value_cache.data() +
                                              cache_offset,
                                          static_cast<size_t>(kv_dim)),
                ggml_rounded_cache_row);
            if (!emel::generator::detail::copy_tensor_row(
                    *exact_q3_backend.token_embedding.tensor, token_id,
              ...
              const size_t exact_cache_offset =
                  emel::generator::detail::layer_cache_offset(
            ...
            if (!emel::generator::detail::copy_tensor_row(
                    *reference_q3_backend.token_embedding.tensor, token_id,
                    reference_q3_backend.hidden) ||
                !run_layer_with_matmul_mode_scalar_attention(
                    reference_q3_backend, layer, position, reference_q3_only)) {
              std::fprintf(
                  stdout, "%s.layer0_value_cache_reference_q3: replay failed\n",
                  token_prefix.c_str());
            } else {
              const size_t reference_q3_cache_offset =
                  emel::generator::detail::layer_cache_offset(
                      reference_q3_backend, layer, position, kv_dim);
              const std::span<const uint16_t> reference_q3_cache_row{
                  reference_q3_backend.value_cache.data() +
                      reference_q3_cache_offset,
                  static_cast<size_t>(kv_dim),
              };
              std::fprintf(
                  stdout, "%s.layer0_value_cache_reference_q3: max_abs=%g\n",
                  token_prefix.c_str(),
        ...
        if (token_index == 0u) {
          ...
          std::fprintf(stdout, "%s.layer0_value_cache0_drift: max_abs=%g\n",
                       token_prefix.c_str(),
        ...
        if (layer0_v_capture != nullptr) {
          ...
          std::fprintf(
              stdout, "%s.layer0_v: max_abs=%g\n", token_prefix.c_str(),
              reference_v.empty() ? -1.0f
        ...
        if (layer0_q_capture != nullptr) {
          ...
          std::fprintf(
              stdout, "%s.layer0_q: max_abs=%g\n", token_prefix.c_str(),
              reference_q.empty() ? -1.0f
        ...
        if (layer0_k_capture != nullptr) {
          ...
          std::fprintf(
              stdout, "%s.layer0_k: max_abs=%g\n", token_prefix.c_str(),
              reference_k.empty() ? -1.0f
        ...
        std::fprintf(stdout, "%s.layer0_kqv_out: max_abs=%g\n",
                     token_prefix.c_str(),
        ...
        std::fprintf(stdout, "%s.layer0_attn_out: max_abs=%g\n",
                     token_prefix.c_str(),
                     max_abs_diff(backend.projected, reference_attn_out));
        const std::span<const float> reference_l_out =
            reference_token_row(find_reference_tensor(graph_capture, "l_out-0"),
                                backend.hidden.size(), token_index);
        std::fprintf(stdout, "%s.layer0_l_out: max_abs=%g\n",
                     token_prefix.c_str(),
        ...
        std::vector<float> reference_layer1_key_cache;
        std::vector<float> reference_layer1_value_cache;
        if (capture_reference_key_cache_rows(reference_ctx.get(), 1,
                                             reference_layer1_key_cache) &&
            reference_layer1_key_cache.size() ==
                static_cast<size_t>((position + 1) * kv_dim)) {
          float max_cache_abs = 0.0f;
          for (int32_t cache_pos = 0; cache_pos <= position; ++cache_pos) {
          ...
          std::fprintf(stdout, "%s.layer1_key_cache: max_abs=%g\n",
                       token_prefix.c_str(), max_cache_abs);
          if (!is_prompt_token) {
            ...
            for (size_t threshold_index = 0;
                 threshold_index < cache_thresholds.size(); ++threshold_index) {
        ...
        if (capture_reference_value_cache_rows(reference_ctx.get(), 1,
                                               reference_layer1_value_cache) &&
            reference_layer1_value_cache.size() ==
                static_cast<size_t>((position + 1) * kv_dim)) {
          float max_cache_abs = 0.0f;
          for (int32_t cache_pos = 0; cache_pos <= position; ++cache_pos) {
          ...
          std::fprintf(stdout, "%s.layer1_value_cache: max_abs=%g\n",
                       token_prefix.c_str(), max_cache_abs);
          if (!is_prompt_token) {
        ...
        if (token_index == 0u) {
          ...
          std::fprintf(stdout, "%s.layer1_value_cache0_drift: max_abs=%g\n",
                       token_prefix.c_str(),
        ...
        if (layer1_v_capture != nullptr) {
          ...
          std::fprintf(
              stdout, "%s.layer1_v: max_abs=%g\n", token_prefix.c_str(),
              reference_v.empty() ? -1.0f
                                  : max_abs_diff(backend.v, reference_v));
        }
        const std::span<const float> reference_l_out =
            reference_token_row(find_reference_tensor(graph_capture, "l_out-1"),
                                backend.hidden.size(), token_index);
        std::fprintf(stdout, "%s.layer1_l_out: max_abs=%g\n",
                     token_prefix.c_str(),
                     max_abs_diff(backend.hidden, reference_l_out));
      }
    }
  ...
  std::fprintf(
      ...
      layer1_key_cache_max_abs, layer1_key_cache_max_generated);
  std::fprintf(stdout,
               "generation_debug.timeline.summary.layer1_value_cache: "
               "max_abs=%g gen=%d\n",
               layer1_value_cache_max_abs, layer1_value_cache_max_generated);
  for (size_t threshold_index = 0; threshold_index < cache_thresholds.size();
       ++threshold_index) {
    std::fprintf(stdout,
                 "generation_debug.timeline.summary.layer1_threshold=%g "
                 "key_first_gen=%d "
                 "value_first_gen=%d\n",
...
void dump_generation_q23_timeline_debug(
  ...
  if (emel::generator::detail::prepare(actor_backend, *state.model_data) !=
      ...
      emel::generator::detail::prepare(q23_backend, *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none)) {
    std::fprintf(stdout,
                 "generation_debug.q23_timeline: backend prepare failed\n");
    return;
  }
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    const int32_t position = static_cast<int32_t>(token_index);
    if (!emel::generator::detail::copy_tensor_row(
            *actor_backend.token_embedding.tensor, token_id,
            actor_backend.hidden) ||
        !emel::generator::detail::copy_tensor_row(
            *q23_backend.token_embedding.tensor, token_id,
            q23_backend.hidden)) {
      std::fprintf(
          stdout,
          "generation_debug.q23_timeline: token embedding replay failed\n");
      return;
    }
    ...
    const int32_t generated_index =
        static_cast<int32_t>(token_index - prompt_count);
    const bool emit_token_debug =
        is_prompt_token || generated_index >= timeline_start_generated;
    const std::string token_prefix =
        is_prompt_token ? "generation_debug.q23_timeline.prompt" +
    ...
    for (int32_t layer = 0; layer < actor_backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention(actor_backend, layer, position) ||
        ...
        std::fprintf(stdout,
                     "generation_debug.q23_timeline: layer replay failed\n");
        return;
      }
      ...
      std::fprintf(stdout, "%s.layer%d_l_out: max_abs=%g\n",
                   token_prefix.c_str(), layer,
...
void dump_generation_reference_q_timeline_debug(
  ...
  if (emel::generator::detail::prepare(actor_backend, *state.model_data) !=
      ...
      emel::generator::detail::prepare(q3_reference_backend,
          ...
          emel::error::cast(emel::model::loader::error::none)) {
    std::fprintf(stdout,
                 "generation_debug.qref_timeline: backend prepare failed\n");
    return;
  }
  ...
  std::array<std::array<int32_t, qref_thresholds.size()>, 2>
      q2_first_generated{};
  std::array<std::array<int32_t, qref_thresholds.size()>, 2>
      q3_first_generated{};
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    if (!emel::generator::detail::copy_tensor_row(
            *actor_backend.token_embedding.tensor, token_id,
            actor_backend.hidden) ||
        !emel::generator::detail::copy_tensor_row(
            *q2_reference_backend.token_embedding.tensor, token_id,
            q2_reference_backend.hidden) ||
        !emel::generator::detail::copy_tensor_row(
            *q3_reference_backend.token_embedding.tensor, token_id,
            q3_reference_backend.hidden)) {
      std::fprintf(
          stdout,
          "generation_debug.qref_timeline: token embedding replay failed\n");
      return;
    }
    ...
    const bool emit_token_debug =
        is_prompt_token || generated_index >= timeline_start_generated;
    const std::string token_prefix =
        is_prompt_token ? "generation_debug.qref_timeline.prompt" +
                              std::to_string(token_index)
    ...
    for (int32_t layer = 0; layer < actor_backend.n_layer; ++layer) {
      if (!run_layer_with_scalar_attention(actor_backend, layer, position) ||
        ...
        std::fprintf(stdout,
                     "generation_debug.qref_timeline: layer replay failed\n");
        return;
      }

      const float q2_diff =
          max_abs_diff(actor_backend.hidden, q2_reference_backend.hidden);
      const float q3_diff =
          max_abs_diff(actor_backend.hidden, q3_reference_backend.hidden);
      if (!is_prompt_token && layer < static_cast<int32_t>(q2_max_abs.size())) {
        ...
        for (size_t threshold_index = 0;
             threshold_index < qref_thresholds.size(); ++threshold_index) {
          if (q2_first_generated[static_cast<size_t>(layer)][threshold_index] <
                  0 &&
              q2_diff > qref_thresholds[threshold_index]) {
            q2_first_generated[static_cast<size_t>(layer)][threshold_index] =
                generated_index;
          }
          if (q3_first_generated[static_cast<size_t>(layer)][threshold_index] <
                  0 &&
              q3_diff > qref_thresholds[threshold_index]) {
            q3_first_generated[static_cast<size_t>(layer)][threshold_index] =
                generated_index;
          }
        }
      }
      ...
      std::fprintf(stdout, "%s.layer%d_q2_reference_l_out: max_abs=%g\n",
                   token_prefix.c_str(), layer, q2_diff);
      std::fprintf(stdout, "%s.layer%d_q3_reference_l_out: max_abs=%g\n",
                   token_prefix.c_str(), layer, q3_diff);
    }
  ...
  for (size_t layer = 0; layer < q2_max_abs.size(); ++layer) {
    std::fprintf(stdout,
                 "generation_debug.qref_summary.layer%zu.q2_max=%g gen=%d "
                 "q3_max=%g gen=%d\n",
                 layer, q2_max_abs[layer], q2_max_generated[layer],
                 q3_max_abs[layer], q3_max_generated[layer]);
    for (size_t threshold_index = 0; threshold_index < qref_thresholds.size();
         ++threshold_index) {
      std::fprintf(
          stdout,
          "generation_debug.qref_summary.layer%zu.threshold=%g q2_first_gen=%d "
          "q3_first_gen=%d\n",
          layer, qref_thresholds[threshold_index],
...
void dump_generation_q23_stage_debug(
  ...
  if (reference_ctx == nullptr) {
    std::fprintf(stdout,
                 "generation_debug.q23_stage: reference context failed\n");
    return;
  }
  ...
  if (emel::generator::detail::prepare(actor_backend, *state.model_data) !=
      ...
      emel::generator::detail::prepare(q3_reference_backend,
          ...
          emel::error::cast(emel::model::loader::error::none)) {
    std::fprintf(stdout,
                 "generation_debug.q23_stage: backend prepare failed\n");
    return;
  }
  ...
  if (llama_decode(reference_ctx.get(), reference_batch) != 0) {
    std::fprintf(stdout,
                 "generation_debug.q23_stage: reference decode failed\n");
    return;
  }
  ...
  if (!emel::generator::detail::copy_tensor_row(
      ...
      !emel::generator::detail::copy_tensor_row(
          *q3_reference_backend.token_embedding.tensor, token_id,
          q3_reference_backend.hidden)) {
    std::fprintf(stdout,
                 "generation_debug.q23_stage: token embedding replay failed\n");
    return;
  }

  auto dump_stage = [&](const char *label, const auto actor_values,
                        const auto q23_values) {
    std::fprintf(stdout, "generation_debug.q23_stage.%s: max_abs=%g\n", label,
  ...
  if (!emel::generator::detail::rms_norm(
          actor_backend.hidden, actor_block.attention_norm,
          actor_backend.rms_epsilon, actor_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q23_backend.hidden, q23_block.attention_norm, q23_backend.rms_epsilon,
          q23_backend.norm)) {
  ...
  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_q, actor_backend.norm,
          actor_backend.q) ||
      !matmul_vector_mode(
          q23_backend, q23_block.attention_q, q23_backend.norm, q23_backend.q,
          scalar_quant_q23.attention, scalar_quant_q23.only_dtype,
          scalar_quant_q23.dtype_mask, scalar_quant_q23.use_reference_q8,
          scalar_quant_q23.use_scalar_quantized) ||
      !emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_k, actor_backend.norm,
          actor_backend.k) ||
      !matmul_vector_mode(
          q23_backend, q23_block.attention_k, q23_backend.norm, q23_backend.k,
          scalar_quant_q23.attention, scalar_quant_q23.only_dtype,
          scalar_quant_q23.dtype_mask, scalar_quant_q23.use_reference_q8,
          scalar_quant_q23.use_scalar_quantized) ||
      !emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_v, actor_backend.norm,
          actor_backend.v) ||
      !matmul_vector_mode(
          q23_backend, q23_block.attention_v, q23_backend.norm, q23_backend.v,
          scalar_quant_q23.attention, scalar_quant_q23.only_dtype,
          scalar_quant_q23.dtype_mask, scalar_quant_q23.use_reference_q8,
          scalar_quant_q23.use_scalar_quantized)) {
  ...
  dump_q8_quantize_compare(
      "generation_debug.q23_stage.prompt0.layer0.attn_norm_q8",
      actor_backend.norm);
  ...
  if (matmul_vector_reference_q8(actor_block.attention_q, actor_backend.norm,
                                 reference_q) &&
      matmul_vector_reference_q8(actor_block.attention_k, actor_backend.norm,
                                 reference_k) &&
      matmul_vector_reference_q8(actor_block.attention_v, actor_backend.norm,
                                 reference_v)) {
    dump_stage("prompt0.layer0.q_pre_rope_actor_vs_reference_q8",
               actor_backend.q, reference_q);
    dump_stage("prompt0.layer0.q_pre_rope_q23_vs_reference_q8", q23_backend.q,
               reference_q);
    dump_stage("prompt0.layer0.k_pre_rope_actor_vs_reference_q8",
               actor_backend.k, reference_k);
    dump_stage("prompt0.layer0.k_pre_rope_q23_vs_reference_q8", q23_backend.k,
               reference_k);
    dump_stage("prompt0.layer0.v_pre_cache_actor_vs_reference_q8",
               actor_backend.v, reference_v);
    dump_stage("prompt0.layer0.v_pre_cache_q23_vs_reference_q8", q23_backend.v,
               reference_v);
  }

  emel::generator::detail::apply_rope(
      actor_backend.q, actor_backend.n_head, actor_backend.head_dim,
      actor_backend.n_rot, position, actor_backend.rope_freq_base,
      actor_backend.rope_type, actor_backend.rope_freq_scale,
      actor_backend.rope_context_original, actor_backend.rope_ext_factor,
      actor_backend.rope_attn_factor, actor_backend.rope_beta_fast,
      actor_backend.rope_beta_slow);
  emel::generator::detail::apply_rope(
      actor_backend.k, actor_backend.n_head_kv, actor_backend.head_dim_kv,
      actor_backend.n_rot, position, actor_backend.rope_freq_base,
      actor_backend.rope_type, actor_backend.rope_freq_scale,
      actor_backend.rope_context_original, actor_backend.rope_ext_factor,
      actor_backend.rope_attn_factor, actor_backend.rope_beta_fast,
      actor_backend.rope_beta_slow);
  emel::generator::detail::apply_rope(
      q23_backend.q, q23_backend.n_head, q23_backend.head_dim,
      q23_backend.n_rot, position, q23_backend.rope_freq_base,
      q23_backend.rope_type, q23_backend.rope_freq_scale,
      q23_backend.rope_context_original, q23_backend.rope_ext_factor,
      q23_backend.rope_attn_factor, q23_backend.rope_beta_fast,
      q23_backend.rope_beta_slow);
  emel::generator::detail::apply_rope(
      q23_backend.k, q23_backend.n_head_kv, q23_backend.head_dim_kv,
      q23_backend.n_rot, position, q23_backend.rope_freq_base,
      q23_backend.rope_type, q23_backend.rope_freq_scale,
      q23_backend.rope_context_original, q23_backend.rope_ext_factor,
      q23_backend.rope_attn_factor, q23_backend.rope_beta_fast,
  ...
  emel::generator::detail::store_fp16_rounded_cache(
      std::span<const float>(actor_backend.q.data(),
                             static_cast<size_t>(actor_backend.n_embd)),
      actor_backend.q_attn.data());
  emel::generator::detail::store_fp16_rounded_cache(
      std::span<const float>(q23_backend.q.data(),
                             static_cast<size_t>(q23_backend.n_embd)),
  ...
  const size_t actor_cache_offset = emel::generator::detail::layer_cache_offset(
      actor_backend, layer_index, position, kv_dim);
  const size_t q23_cache_offset = emel::generator::detail::layer_cache_offset(
      q23_backend, layer_index, position, kv_dim);
  emel::generator::detail::store_fp16_rounded_cache(
      std::span<const float>(actor_backend.k.data(),
                             static_cast<size_t>(kv_dim)),
      actor_backend.key_cache.data() + actor_cache_offset);
  emel::generator::detail::store_fp16_rounded_cache(
      std::span<const float>(actor_backend.v.data(),
                             static_cast<size_t>(kv_dim)),
  ...
  dump_stage(
      "prompt0.layer0.key_cache",
      std::span<const uint16_t>(actor_backend.key_cache.data() +
                                    actor_cache_offset,
                                static_cast<size_t>(kv_dim)),
      std::span<const uint16_t>(q23_backend.key_cache.data() + q23_cache_offset,
                                static_cast<size_t>(kv_dim)));
  dump_stage("prompt0.layer0.value_cache",
             std::span<const uint16_t>(actor_backend.value_cache.data() +
                                           actor_cache_offset,
                                       static_cast<size_t>(kv_dim)),
             std::span<const uint16_t>(q23_backend.value_cache.data() +
                                           q23_cache_offset,
                                       static_cast<size_t>(kv_dim)));
  std::vector<float> reference_value_cache_rows;
  if (capture_reference_value_cache_rows(reference_ctx.get(), layer_index,
                                         reference_value_cache_rows) &&
      reference_value_cache_rows.size() >= static_cast<size_t>(kv_dim)) {
    const std::span<const float> reference_value_cache_row{
    ...
    dump_state_compare("generation_debug.q23_stage.prompt0.layer0.value_cache_"
                       "q23_vs_reference",
                       std::span<const uint16_t>(
                           q23_backend.value_cache.data() + q23_cache_offset,
                           static_cast<size_t>(kv_dim)),
    ...
    if (run_layer_with_matmul_mode_scalar_attention(
      ...
      const size_t q3_reference_cache_offset =
          emel::generator::detail::layer_cache_offset(
              q3_reference_backend, layer_index, position, kv_dim);
      dump_state_compare(
          "generation_debug.q23_stage.prompt0.layer0.value_cache_q3_reference_"
          "vs_reference",
          std::span<const uint16_t>(q3_reference_backend.value_cache.data() +
                                        q3_reference_cache_offset,
                                    static_cast<size_t>(kv_dim)),
          reference_value_cache_row);
      dump_state_compare(
          "generation_debug.q23_stage.prompt0.layer0.value_cache_q3_reference_"
      ...
      std::fprintf(stdout, "generation_debug.q23_stage.prompt0.layer0.value_"
                           "cache_q3_reference_vs_reference: replay failed\n");
    }
  } else {
    std::fprintf(stdout, "generation_debug.q23_stage.prompt0.layer0.value_"
                         "cache_vs_reference: unavailable\n");
  }
  ...
  dump_stage("prompt0.layer0.kqv_out", actor_backend.attn_ctx,
             q23_backend.attn_ctx);

  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_output, actor_backend.attn_ctx,
          actor_backend.projected) ||
      !matmul_vector_mode(
          q23_backend, q23_block.attention_output, q23_backend.attn_ctx,
          q23_backend.projected, scalar_quant_q23.attention,
          scalar_quant_q23.only_dtype, scalar_quant_q23.dtype_mask,
          scalar_quant_q23.use_reference_q8,
          scalar_quant_q23.use_scalar_quantized)) {
    std::fprintf(
        stdout, "generation_debug.q23_stage: attention output matmul failed\n");
    return;
  }
  dump_stage("prompt0.layer0.attn_out", actor_backend.projected,
             q23_backend.projected);

  for (int32_t idx = 0; idx < actor_backend.n_embd; ++idx) {
    actor_backend.hidden[static_cast<size_t>(idx)] +=
        actor_backend.projected[static_cast<size_t>(idx)];
    q23_backend.hidden[static_cast<size_t>(idx)] +=
        q23_backend.projected[static_cast<size_t>(idx)];
  }
  dump_stage("prompt0.layer0.attn_residual", actor_backend.hidden,
             q23_backend.hidden);

  if (!emel::generator::detail::rms_norm(
          actor_backend.hidden, actor_block.feed_forward_norm,
          actor_backend.rms_epsilon, actor_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q23_backend.hidden, q23_block.feed_forward_norm,
          q23_backend.rms_epsilon, q23_backend.norm)) {
  ...
  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.feed_forward_gate, actor_backend.norm,
          actor_backend.gate) ||
      !matmul_vector_mode(
          q23_backend, q23_block.feed_forward_gate, q23_backend.norm,
          q23_backend.gate, scalar_quant_q23.ffn, scalar_quant_q23.only_dtype,
          scalar_quant_q23.dtype_mask, scalar_quant_q23.use_reference_q8,
          scalar_quant_q23.use_scalar_quantized) ||
      !emel::generator::detail::matmul_vector(
          actor_backend, actor_block.feed_forward_up, actor_backend.norm,
          actor_backend.up) ||
      !matmul_vector_mode(
          q23_backend, q23_block.feed_forward_up, q23_backend.norm,
          q23_backend.up, scalar_quant_q23.ffn, scalar_quant_q23.only_dtype,
          scalar_quant_q23.dtype_mask, scalar_quant_q23.use_reference_q8,
          scalar_quant_q23.use_scalar_quantized)) {
    std::fprintf(stdout,
                 "generation_debug.q23_stage: ffn gate/up matmul failed\n");
    return;
  }
  ...
  for (size_t idx = 0; idx < actor_backend.gate.size(); ++idx) {
    actor_backend.ffn_hidden[idx] =
        emel::generator::detail::silu(actor_backend.gate[idx]) *
        actor_backend.up[idx];
    q23_backend.ffn_hidden[idx] =
        emel::generator::detail::silu(q23_backend.gate[idx]) *
        q23_backend.up[idx];
  }
  dump_stage("prompt0.layer0.ffn_swiglu", actor_backend.ffn_hidden,
             q23_backend.ffn_hidden);

  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.feed_forward_down,
          actor_backend.ffn_hidden, actor_backend.projected) ||
      !matmul_vector_mode(q23_backend, q23_block.feed_forward_down,
                          q23_backend.ffn_hidden, q23_backend.projected,
                          scalar_quant_q23.ffn, scalar_quant_q23.only_dtype,
                          scalar_quant_q23.dtype_mask,
                          scalar_quant_q23.use_reference_q8,
                          scalar_quant_q23.use_scalar_quantized)) {
    std::fprintf(stdout,
                 "generation_debug.q23_stage: ffn down matmul failed\n");
    return;
  }
  dump_stage("prompt0.layer0.ffn_out", actor_backend.projected,
             q23_backend.projected);
...
void dump_generation_reference_q_stage_debug(
  ...
  if (emel::generator::detail::prepare(actor_backend, *state.model_data) !=
      ...
      emel::generator::detail::prepare(q3_scalar_backend, *state.model_data) !=
          emel::error::cast(emel::model::loader::error::none)) {
    std::fprintf(stdout,
                 "generation_debug.qref_stage: backend prepare failed\n");
    return;
  }
  ...
  if (target_generated_index > 0) {
    ...
    if (emel::generator::detail::prepare(scan_actor_backend,
        ...
        emel::generator::detail::prepare(scan_q2_reference_backend,
                                         *state.model_data) ==
            emel::error::cast(emel::model::loader::error::none) &&
        emel::generator::detail::prepare(scan_q3_reference_backend,
                                         *state.model_data) ==
            emel::error::cast(emel::model::loader::error::none)) {
      const size_t prompt_count = prompt_tokens.size();
      for (size_t token_index = 0; token_index < prefix_tokens.size();
        ...
        if (!emel::generator::detail::copy_tensor_row(
            ...
            !emel::generator::detail::copy_tensor_row(
                *scan_q2_reference_backend.token_embedding.tensor,
                scan_token_id, scan_q2_reference_backend.hidden) ||
            !emel::generator::detail::copy_tensor_row(
                *scan_q3_reference_backend.token_embedding.tensor,
                scan_token_id, scan_q3_reference_backend.hidden)) {
          break;
        }
        ...
        for (int32_t layer = 0; layer < scan_actor_backend.n_layer; ++layer) {
          if (!run_layer_with_scalar_attention(scan_actor_backend, layer,
              ...
              !run_layer_with_matmul_mode_scalar_attention(
                  ...
                  reference_q2_only) ||
              !run_layer_with_matmul_mode_scalar_attention(
                  scan_q3_reference_backend, layer, scan_position,
                  reference_q3_only)) {
            found_target = false;
            break;
          }
          ...
          if (!is_prompt_token && layer == 1 &&
              max_abs_diff(scan_actor_backend.hidden,
                           scan_q2_reference_backend.hidden) >
                  target_q2_threshold) {
            target_generated_index = generated_index;
            found_target = true;
  ...
  if (target_prefix_token_count <= 0 ||
      target_prefix_token_count > static_cast<int32_t>(prefix_tokens.size())) {
    std::fprintf(stdout,
                 "generation_debug.qref_stage: invalid target prefix\n");
    return;
  }
  ...
  if (target_prefix_token_count > 1) {
    const std::span<const int32_t> prior_tokens{
        prefix_tokens.data(),
        static_cast<size_t>(target_prefix_token_count - 1)};
    if (!run_prefill_with_scalar_attention(actor_backend, prior_tokens) ||
      ...
      std::fprintf(stdout,
                   "generation_debug.qref_stage: prior prefix replay failed\n");
      return;
    }
  } else {
  ...
  if (!emel::generator::detail::copy_tensor_row(
          *actor_backend.token_embedding.tensor, token_id,
          actor_backend.hidden) ||
      !emel::generator::detail::copy_tensor_row(
          *q2_exact_backend.token_embedding.tensor, token_id,
      ...
      !emel::generator::detail::copy_tensor_row(
          *q3_reference_backend.token_embedding.tensor, token_id,
          q3_reference_backend.hidden) ||
      !emel::generator::detail::copy_tensor_row(
          *q3_scalar_backend.token_embedding.tensor, token_id,
          q3_scalar_backend.hidden)) {
    std::fprintf(
        stdout, "generation_debug.qref_stage: token embedding replay failed\n");
    return;
  }

  auto dump_stage = [&](const char *label, const auto actor_values,
                        const auto q2_exact_values, const auto q2_values,
                        const auto q3_values, const auto q3_scalar_values) {
    std::fprintf(stdout,
    ...
    std::fprintf(stdout,
                 ...
                 label, max_abs_diff(actor_values, q2_values));
    std::fprintf(stdout,
                 "generation_debug.qref_stage.%s.q3_reference: max_abs=%g\n",
                 label, max_abs_diff(actor_values, q3_values));
    std::fprintf(stdout,
                 "generation_debug.qref_stage.%s.q3_scalar: max_abs=%g\n",
                 label, max_abs_diff(actor_values, q3_scalar_values));
  };

  if (!run_layer_with_scalar_attention(actor_backend, 0, position) ||
      !run_layer_with_matmul_mode_scalar_attention(q2_exact_backend, 0,
                                                   position, exact_q2_only) ||
      !run_layer_with_matmul_mode_scalar_attention(
  ...
  const std::string stage_prefix = "gen" + std::to_string(generated_index) +
                                   ".layer" + std::to_string(layer_index);
  dump_stage((stage_prefix + ".input_hidden").c_str(), actor_backend.hidden,
             q2_exact_backend.hidden, q2_reference_backend.hidden,
             q3_reference_backend.hidden, q3_scalar_backend.hidden);

  auto &actor_block = actor_backend.blocks[static_cast<size_t>(layer_index)];
  auto &q2_exact_block =
      q2_exact_backend.blocks[static_cast<size_t>(layer_index)];
  auto &q2_block =
      q2_reference_backend.blocks[static_cast<size_t>(layer_index)];
  auto &q3_block =
      q3_reference_backend.blocks[static_cast<size_t>(layer_index)];
  auto &q3_scalar_block =
      q3_scalar_backend.blocks[static_cast<size_t>(layer_index)];
  if (!emel::generator::detail::rms_norm(
          actor_backend.hidden, actor_block.attention_norm,
          actor_backend.rms_epsilon, actor_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q2_exact_backend.hidden, q2_exact_block.attention_norm,
          q2_exact_backend.rms_epsilon, q2_exact_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q2_reference_backend.hidden, q2_block.attention_norm,
          q2_reference_backend.rms_epsilon, q2_reference_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q3_reference_backend.hidden, q3_block.attention_norm,
          q3_reference_backend.rms_epsilon, q3_reference_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q3_scalar_backend.hidden, q3_scalar_block.attention_norm,
          q3_scalar_backend.rms_epsilon, q3_scalar_backend.norm)) {
    std::fprintf(stdout, "generation_debug.qref_stage: attn rms_norm failed\n");
    return;
  }
  dump_stage((stage_prefix + ".attn_norm").c_str(), actor_backend.norm,
             q2_exact_backend.norm, q2_reference_backend.norm,
             q3_reference_backend.norm, q3_scalar_backend.norm);
  dump_q8_quantize_compare((stage_prefix + ".attn_norm_q8").c_str(),
                           actor_backend.norm);
  dump_matrix_compare((stage_prefix + ".attn_q_matmul").c_str(), actor_backend,
                      actor_block.attention_q, actor_backend.norm);
  dump_matrix_compare_reference_q8(
      (stage_prefix + ".attn_q_matmul_refq8").c_str(), actor_backend,
      actor_block.attention_q, actor_backend.norm);
  dump_matrix_compare((stage_prefix + ".attn_k_matmul").c_str(), actor_backend,
                      actor_block.attention_k, actor_backend.norm);
  dump_matrix_compare_reference_q8(
      (stage_prefix + ".attn_k_matmul_refq8").c_str(), actor_backend,
      actor_block.attention_k, actor_backend.norm);
  dump_matrix_compare((stage_prefix + ".attn_v_matmul").c_str(), actor_backend,
                      actor_block.attention_v, actor_backend.norm);
  dump_matrix_compare_reference_q8(
      (stage_prefix + ".attn_v_matmul_refq8").c_str(), actor_backend,
      actor_block.attention_v, actor_backend.norm);

  const auto run_mode_matmul =
      [&](emel::generator::detail::native_backend &backend,
          const emel::generator::detail::tensor_matrix &matrix,
          std::span<const float> input, std::span<float> output,
          const exact_matmul_mode mode, const bool attention_mode) {
        return matmul_vector_mode(
            backend, matrix, input, output,
            attention_mode ? mode.attention : mode.ffn, mode.only_dtype,
            mode.dtype_mask, mode.use_reference_q8, mode.use_scalar_quantized);
      };

  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_q, actor_backend.norm,
          actor_backend.q) ||
      !run_mode_matmul(q2_exact_backend, q2_exact_block.attention_q,
                       q2_exact_backend.norm, q2_exact_backend.q, exact_q2_only,
                       true) ||
      !run_mode_matmul(q2_reference_backend, q2_block.attention_q,
                       q2_reference_backend.norm, q2_reference_backend.q,
                       reference_q2_only, true) ||
      !run_mode_matmul(q3_reference_backend, q3_block.attention_q,
                       q3_reference_backend.norm, q3_reference_backend.q,
                       reference_q3_only, true) ||
      !run_mode_matmul(q3_scalar_backend, q3_scalar_block.attention_q,
                       q3_scalar_backend.norm, q3_scalar_backend.q,
                       scalar_quant_q3_only, true) ||
      !emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_k, actor_backend.norm,
          actor_backend.k) ||
      !run_mode_matmul(q2_exact_backend, q2_exact_block.attention_k,
                       q2_exact_backend.norm, q2_exact_backend.k, exact_q2_only,
                       true) ||
      !run_mode_matmul(q2_reference_backend, q2_block.attention_k,
                       q2_reference_backend.norm, q2_reference_backend.k,
                       reference_q2_only, true) ||
      !run_mode_matmul(q3_reference_backend, q3_block.attention_k,
                       q3_reference_backend.norm, q3_reference_backend.k,
                       reference_q3_only, true) ||
      !run_mode_matmul(q3_scalar_backend, q3_scalar_block.attention_k,
                       q3_scalar_backend.norm, q3_scalar_backend.k,
                       scalar_quant_q3_only, true) ||
      !emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_v, actor_backend.norm,
          actor_backend.v) ||
      !run_mode_matmul(q2_exact_backend, q2_exact_block.attention_v,
                       q2_exact_backend.norm, q2_exact_backend.v, exact_q2_only,
                       true) ||
      !run_mode_matmul(q2_reference_backend, q2_block.attention_v,
                       q2_reference_backend.norm, q2_reference_backend.v,
                       reference_q2_only, true) ||
      !run_mode_matmul(q3_reference_backend, q3_block.attention_v,
                       q3_reference_backend.norm, q3_reference_backend.v,
                       reference_q3_only, true) ||
      !run_mode_matmul(q3_scalar_backend, q3_scalar_block.attention_v,
                       q3_scalar_backend.norm, q3_scalar_backend.v,
                       scalar_quant_q3_only, true)) {
    std::fprintf(stdout, "generation_debug.qref_stage: qkv matmul failed\n");
    return;
  }
  dump_stage((stage_prefix + ".q_pre_rope").c_str(), actor_backend.q,
             q2_exact_backend.q, q2_reference_backend.q, q3_reference_backend.q,
             q3_scalar_backend.q);
  dump_stage((stage_prefix + ".k_pre_rope").c_str(), actor_backend.k,
             q2_exact_backend.k, q2_reference_backend.k, q3_reference_backend.k,
             q3_scalar_backend.k);
  dump_stage((stage_prefix + ".v_pre_cache").c_str(), actor_backend.v,
             q2_exact_backend.v, q2_reference_backend.v, q3_reference_backend.v,
             q3_scalar_backend.v);

  auto apply_rope_and_cache =
      [&](emel::generator::detail::native_backend &backend) {
        emel::generator::detail::apply_rope(
            backend.q, backend.n_head, backend.head_dim, backend.n_rot,
            position, backend.rope_freq_base, backend.rope_type,
            backend.rope_freq_scale, backend.rope_context_original,
            backend.rope_ext_factor, backend.rope_attn_factor,
            backend.rope_beta_fast, backend.rope_beta_slow);
  ...
  dump_stage((stage_prefix + ".q").c_str(), actor_backend.q, q2_exact_backend.q,
             q2_reference_backend.q, q3_reference_backend.q,
             q3_scalar_backend.q);
  dump_stage((stage_prefix + ".k").c_str(), actor_backend.k, q2_exact_backend.k,
             q2_reference_backend.k, q3_reference_backend.k,
             q3_scalar_backend.k);
  dump_stage((stage_prefix + ".q_attn").c_str(), actor_backend.q_attn,
             q2_exact_backend.q_attn, q2_reference_backend.q_attn,
             q3_reference_backend.q_attn, q3_scalar_backend.q_attn);
  dump_stage((stage_prefix + ".key_cache").c_str(), actor_backend.key_cache,
             q2_exact_backend.key_cache, q2_reference_backend.key_cache,
             q3_reference_backend.key_cache, q3_scalar_backend.key_cache);
  dump_stage((stage_prefix + ".value_cache").c_str(), actor_backend.value_cache,
             q2_exact_backend.value_cache, q2_reference_backend.value_cache,
             q3_reference_backend.value_cache, q3_scalar_backend.value_cache);

  if (!emel::generator::detail::compute_attention(
          actor_backend, layer_index, position + 1, actor_backend.q_attn) ||
      !emel::generator::detail::compute_attention(q2_exact_backend, layer_index,
                                                  position + 1,
                                                  q2_exact_backend.q_attn) ||
      !emel::generator::detail::compute_attention(
          q2_reference_backend, layer_index, position + 1,
          q2_reference_backend.q_attn) ||
      !emel::generator::detail::compute_attention(
          q3_reference_backend, layer_index, position + 1,
          q3_reference_backend.q_attn) ||
      !emel::generator::detail::compute_attention(q3_scalar_backend,
                                                  layer_index, position + 1,
                                                  q3_scalar_backend.q_attn)) {
    std::fprintf(stdout, "generation_debug.qref_stage: attention failed\n");
    return;
  }
  dump_stage((stage_prefix + ".kqv_out").c_str(), actor_backend.attn_ctx,
             q2_exact_backend.attn_ctx, q2_reference_backend.attn_ctx,
             q3_reference_backend.attn_ctx, q3_scalar_backend.attn_ctx);
  dump_q8_quantize_compare((stage_prefix + ".attn_ctx_q8").c_str(),
                           actor_backend.attn_ctx);
  dump_matrix_compare((stage_prefix + ".attn_out_matmul").c_str(),
                      actor_backend, actor_block.attention_output,
                      actor_backend.attn_ctx);
  dump_matrix_compare_reference_q8(
      (stage_prefix + ".attn_out_matmul_refq8").c_str(), actor_backend,
      actor_block.attention_output, actor_backend.attn_ctx);

  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.attention_output, actor_backend.attn_ctx,
          actor_backend.projected) ||
      !run_mode_matmul(q2_exact_backend, q2_exact_block.attention_output,
                       q2_exact_backend.attn_ctx, q2_exact_backend.projected,
                       exact_q2_only, true) ||
      !run_mode_matmul(q2_reference_backend, q2_block.attention_output,
                       q2_reference_backend.attn_ctx,
                       q2_reference_backend.projected, reference_q2_only,
                       true) ||
      !run_mode_matmul(q3_reference_backend, q3_block.attention_output,
                       q3_reference_backend.attn_ctx,
                       q3_reference_backend.projected, reference_q3_only,
                       true) ||
      !run_mode_matmul(q3_scalar_backend, q3_scalar_block.attention_output,
                       q3_scalar_backend.attn_ctx, q3_scalar_backend.projected,
                       scalar_quant_q3_only, true)) {
    std::fprintf(
        stdout,
        "generation_debug.qref_stage: attention output matmul failed\n");
    return;
  }
  dump_stage((stage_prefix + ".attn_out").c_str(), actor_backend.projected,
             q2_exact_backend.projected, q2_reference_backend.projected,
             q3_reference_backend.projected, q3_scalar_backend.projected);
  ...
  dump_stage((stage_prefix + ".attn_residual").c_str(), actor_backend.hidden,
             q2_exact_backend.hidden, q2_reference_backend.hidden,
             q3_reference_backend.hidden, q3_scalar_backend.hidden);
  ...
  if (!emel::generator::detail::rms_norm(
          ...
          actor_backend.rms_epsilon, actor_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q2_exact_backend.hidden, q2_exact_block.feed_forward_norm,
          q2_exact_backend.rms_epsilon, q2_exact_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q2_reference_backend.hidden, q2_block.feed_forward_norm,
          q2_reference_backend.rms_epsilon, q2_reference_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q3_reference_backend.hidden, q3_block.feed_forward_norm,
          q3_reference_backend.rms_epsilon, q3_reference_backend.norm) ||
      !emel::generator::detail::rms_norm(
          q3_scalar_backend.hidden, q3_scalar_block.feed_forward_norm,
          q3_scalar_backend.rms_epsilon, q3_scalar_backend.norm)) {
    std::fprintf(stdout, "generation_debug.qref_stage: ffn rms_norm failed\n");
    return;
  }
  dump_stage((stage_prefix + ".ffn_norm").c_str(), actor_backend.norm,
             q2_exact_backend.norm, q2_reference_backend.norm,
             q3_reference_backend.norm, q3_scalar_backend.norm);
  dump_q8_quantize_compare((stage_prefix + ".ffn_norm_q8").c_str(),
                           actor_backend.norm);
  dump_matrix_compare((stage_prefix + ".ffn_gate_matmul").c_str(),
                      actor_backend, actor_block.feed_forward_gate,
                      actor_backend.norm);
  dump_matrix_compare_reference_q8(
      (stage_prefix + ".ffn_gate_matmul_refq8").c_str(), actor_backend,
      actor_block.feed_forward_gate, actor_backend.norm);
  dump_matrix_compare((stage_prefix + ".ffn_up_matmul").c_str(), actor_backend,
                      actor_block.feed_forward_up, actor_backend.norm);
  dump_matrix_compare_reference_q8(
      (stage_prefix + ".ffn_up_matmul_refq8").c_str(), actor_backend,
      actor_block.feed_forward_up, actor_backend.norm);

  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.feed_forward_gate, actor_backend.norm,
          actor_backend.gate) ||
      !run_mode_matmul(q2_exact_backend, q2_exact_block.feed_forward_gate,
                       q2_exact_backend.norm, q2_exact_backend.gate,
                       exact_q2_only, false) ||
      !run_mode_matmul(q2_reference_backend, q2_block.feed_forward_gate,
                       q2_reference_backend.norm, q2_reference_backend.gate,
                       reference_q2_only, false) ||
      !run_mode_matmul(q3_reference_backend, q3_block.feed_forward_gate,
                       q3_reference_backend.norm, q3_reference_backend.gate,
                       reference_q3_only, false) ||
      !run_mode_matmul(q3_scalar_backend, q3_scalar_block.feed_forward_gate,
                       q3_scalar_backend.norm, q3_scalar_backend.gate,
                       scalar_quant_q3_only, false) ||
      !emel::generator::detail::matmul_vector(
          actor_backend, actor_block.feed_forward_up, actor_backend.norm,
          actor_backend.up) ||
      !run_mode_matmul(q2_exact_backend, q2_exact_block.feed_forward_up,
                       q2_exact_backend.norm, q2_exact_backend.up,
                       exact_q2_only, false) ||
      !run_mode_matmul(q2_reference_backend, q2_block.feed_forward_up,
                       q2_reference_backend.norm, q2_reference_backend.up,
                       reference_q2_only, false) ||
      !run_mode_matmul(q3_reference_backend, q3_block.feed_forward_up,
                       q3_reference_backend.norm, q3_reference_backend.up,
                       reference_q3_only, false) ||
      !run_mode_matmul(q3_scalar_backend, q3_scalar_block.feed_forward_up,
                       q3_scalar_backend.norm, q3_scalar_backend.up,
                       scalar_quant_q3_only, false)) {
    std::fprintf(stdout,
                 "generation_debug.qref_stage: ffn gate/up matmul failed\n");
    return;
  }
  dump_stage((stage_prefix + ".ffn_gate").c_str(), actor_backend.gate,
             q2_exact_backend.gate, q2_reference_backend.gate,
             q3_reference_backend.gate, q3_scalar_backend.gate);
  dump_stage((stage_prefix + ".ffn_up").c_str(), actor_backend.up,
             q2_exact_backend.up, q2_reference_backend.up,
             q3_reference_backend.up, q3_scalar_backend.up);
  ...
  for (size_t idx = 0; idx < actor_backend.gate.size(); ++idx) {
    ...
    q2_exact_backend.ffn_hidden[idx] =
        emel::generator::detail::silu(q2_exact_backend.gate[idx]) *
        q2_exact_backend.up[idx];
  ...
  dump_stage((stage_prefix + ".ffn_swiglu").c_str(), actor_backend.ffn_hidden,
             q2_exact_backend.ffn_hidden, q2_reference_backend.ffn_hidden,
             q3_reference_backend.ffn_hidden, q3_scalar_backend.ffn_hidden);
  dump_q8_quantize_compare((stage_prefix + ".ffn_hidden_q8").c_str(),
                           actor_backend.ffn_hidden);
  dump_matrix_compare((stage_prefix + ".ffn_down_matmul").c_str(),
                      actor_backend, actor_block.feed_forward_down,
                      actor_backend.ffn_hidden);
  dump_matrix_compare_reference_q8(
      (stage_prefix + ".ffn_down_matmul_refq8").c_str(), actor_backend,
      actor_block.feed_forward_down, actor_backend.ffn_hidden);

  if (!emel::generator::detail::matmul_vector(
          actor_backend, actor_block.feed_forward_down,
          actor_backend.ffn_hidden, actor_backend.projected) ||
      !run_mode_matmul(q2_exact_backend, q2_exact_block.feed_forward_down,
                       q2_exact_backend.ffn_hidden, q2_exact_backend.projected,
                       exact_q2_only, false) ||
      !run_mode_matmul(q2_reference_backend, q2_block.feed_forward_down,
                       q2_reference_backend.ffn_hidden,
                       q2_reference_backend.projected, reference_q2_only,
                       false) ||
      !run_mode_matmul(q3_reference_backend, q3_block.feed_forward_down,
                       q3_reference_backend.ffn_hidden,
                       q3_reference_backend.projected, reference_q3_only,
                       false) ||
      !run_mode_matmul(q3_scalar_backend, q3_scalar_block.feed_forward_down,
                       q3_scalar_backend.ffn_hidden,
                       q3_scalar_backend.projected, scalar_quant_q3_only,
                       false)) {
    std::fprintf(stdout,
                 "generation_debug.qref_stage: ffn down matmul failed\n");
    return;
  }
  dump_stage((stage_prefix + ".ffn_out").c_str(), actor_backend.projected,
             q2_exact_backend.projected, q2_reference_backend.projected,
             q3_reference_backend.projected, q3_scalar_backend.projected);
  ...
  dump_stage((stage_prefix + ".l_out").c_str(), actor_backend.hidden,
             q2_exact_backend.hidden, q2_reference_backend.hidden,
             q3_reference_backend.hidden, q3_scalar_backend.hidden);
}

void dump_generation_gen0_attention_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result) {
  ...
  if (!capture_reference_graph_for_generation_prefix(
          state, prompt_tokens, generated_prefix, graph_capture)) {
    std::fprintf(stdout,
                 "generation_debug.gen0: reference graph capture failed\n");
    return;
  }
  ...
  if (reference_ctx == nullptr ||
      !run_reference_prefix_decode(reference_ctx.get(), prompt_tokens,
                                   generated_prefix)) {
    std::fprintf(stdout,
                 "generation_debug.gen0: reference decode replay failed\n");
    return;
  }
  ...
  if (!emel::generator::detail::copy_tensor_row(*backend.token_embedding.tensor,
                                                token_id, backend.hidden)) {
    std::fprintf(stdout,
                 "generation_debug.gen0: token embedding replay failed\n");
    return;
  }
  ...
  auto dump_gen0_state = [&](const char *suffix, const auto emel_values,
    ...
    std::fprintf(stdout, "generation_debug.gen0.layer0.%s: max_abs=%g\n",
                 suffix, max_abs_diff(emel_values, reference_values));
  };
  ...
  {
    auto flash_request =
        emel::generator::detail::make_flash_attn_request(backend, 0, position);
    std::vector<float> shared_flash_ctx(backend.attn_ctx.size(), 0.0f);
    flash_request.dst = emel::generator::detail::make_dst_view_3d(
        shared_flash_ctx.data(), flash_request.dst.ne[0],
        flash_request.dst.ne[1], flash_request.dst.ne[2]);
    ...
    if (emel::kernel::detail::run_flash_attn_ext_with_workspace(
      ...
      std::fprintf(stdout, "generation_debug.gen0.layer0.kqv_out_shared_flash: "
                           "compute failed\n");
    }
    ...
    if (emel::kernel::aarch64::detail::run_flash_attn_ext_neon(
      ...
      std::fprintf(
          stdout,
          "generation_debug.gen0.layer0.kqv_out_neon_flash: compute failed\n");
    }
#endif
  }

  (void)compute_attention_with_softmax_debug(
      backend, graph_capture, 0, position + 1, "generation_debug.gen0.layer0",
      reference_value_cache_rows);

  dump_matrix_compare("generation_debug.gen0.layer0.attn_out_matmul", backend,
                      block.attention_output, backend.attn_ctx);
  if (!emel::generator::detail::matmul_vector(backend, block.attention_output,
                                              backend.attn_ctx,
                                              backend.projected)) {
    std::fprintf(stdout,
                 "generation_debug.gen0: attention output matmul failed\n");
    return;
  }
  ...
  dump_matrix_compare("generation_debug.gen0.layer0.ffn_gate_matmul", backend,
                      block.feed_forward_gate, backend.norm);
  dump_matrix_compare("generation_debug.gen0.layer0.ffn_up_matmul", backend,
                      block.feed_forward_up, backend.norm);
  if (!emel::generator::detail::matmul_vector(backend, block.feed_forward_gate,
                                              backend.norm, backend.gate) ||
      !emel::generator::detail::matmul_vector(backend, block.feed_forward_up,
                                              backend.norm, backend.up)) {
  ...
  dump_matrix_compare("generation_debug.gen0.layer0.ffn_down_matmul", backend,
                      block.feed_forward_down, backend.ffn_hidden);
  if (!emel::generator::detail::matmul_vector(backend, block.feed_forward_down,
                                              backend.ffn_hidden,
                                              backend.projected)) {
...
void dump_generation_target_attention_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result) {
  ...
  const auto dump_target_generated_index = [&](const int32_t
                                                   target_generated_index) {
    if (target_generated_index < 0 ||
        target_generated_index >= reference_result.trace.token_count) {
      return;
    }
    ...
    if (!capture_reference_graph_for_generation_prefix(
            state, prompt_tokens, generated_prefix, graph_capture)) {
      std::fprintf(stdout,
                   "generation_debug.target: reference graph capture failed\n");
      return;
    }

    llama_context_ptr reference_ctx = make_reference_context(
        const_cast<reference_backend &>(state.reference));
    if (reference_ctx == nullptr ||
        !run_reference_prefix_decode(reference_ctx.get(), prompt_tokens,
                                     generated_prefix)) {
      std::fprintf(stdout,
                   "generation_debug.target: reference decode replay failed\n");
      return;
    }
    ...
    if (!run_prefill_from_token_prefix(backend, prior_tokens)) {
      std::fprintf(stdout,
                   "generation_debug.target: prior prefix replay failed\n");
      return;
    }
    ...
    if (!emel::generator::detail::copy_tensor_row(
            *backend.token_embedding.tensor, token_id, backend.hidden)) {
      std::fprintf(stdout,
                   "generation_debug.target: token embedding replay failed\n");
      return;
    }
    ...
    const std::string layer_prefix = "generation_debug.target.gen" +
                                     std::to_string(target_generated_index) +
                                     ".layer0";
    const auto dump_target_state =
        [&](const char *suffix, const auto emel_values,
            std::span<const float> reference_values) {
          std::fprintf(stdout, "%s.%s: max_abs=%g\n", layer_prefix.c_str(),
                       suffix, max_abs_diff(emel_values, reference_values));
        };
    ...
    {
      const std::span<const float> reference_q = reference_last_token_row(
          find_reference_tensor(graph_capture, "Qcur-0"),
          backend.q_attn.size());
      if (reference_q.size() == backend.q_attn.size()) {
        std::vector<float> reference_q_attn(reference_q.begin(),
                                            reference_q.end());
    ...
    (void)compute_attention_with_softmax_debug(backend, graph_capture, 0,
                                               position + 1, layer_prefix,
                                               reference_value_cache_rows);
  };
...
void dump_generation_residual_l2_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result) {
  const int32_t token_mismatch_index =
      first_token_mismatch_index(emel_result, reference_result);
  const int32_t prefix_generated_tokens =
      std::min<int32_t>(12, token_mismatch_index);
  if (state.model_data == nullptr || prefix_generated_tokens <= 0) {
    return;
  }
  ...
  if (!capture_reference_graph_for_generation_prefix(
    ...
    std::fprintf(
        stdout,
        "generation_debug.residual_l2: reference graph capture failed\n");
    return;
  }
  ...
  if (emel::generator::detail::prepare(backend, *state.model_data) !=
      emel::error::cast(emel::model::loader::error::none)) {
    std::fprintf(stdout,
                 "generation_debug.residual_l2: backend prepare failed\n");
    return;
  }
  ...
  if (prefix_tokens.size() > 1u) {
    ...
    if (!run_prefill_from_token_prefix(backend, prior_tokens)) {
      std::fprintf(
          stdout, "generation_debug.residual_l2: prior prefix replay failed\n");
      return;
    }
  } else {
    backend.kv_cache_tokens = 0;
  ...
  if (!emel::generator::detail::copy_tensor_row(*backend.token_embedding.tensor,
    ...
    std::fprintf(
        stdout,
        "generation_debug.residual_l2: token embedding replay failed\n");
    return;
  }

  std::fprintf(stdout,
               "generation_debug.residual_l2: generated_prefix=%d "
               "total_prefix_tokens=%zu\n",
               prefix_generated_tokens, prefix_tokens.size());
  ...
  for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
    ...
    if (!emel::generator::detail::rms_norm(backend.hidden, block.attention_norm,
        ...
        !emel::generator::detail::matmul_vector(backend, block.attention_v,
                                                backend.norm, backend.v)) {
      std::fprintf(stdout,
                   "generation_debug.residual_l2.layer%d: qkv prep failed\n",
                   layer);
      return;
    }
    ...
    if (!emel::generator::detail::compute_attention(
        ...
        !emel::generator::detail::matmul_vector(backend, block.attention_output,
                                                ...
                                                backend.projected)) {
      std::fprintf(stdout,
                   "generation_debug.residual_l2.layer%d: attention failed\n",
                   layer);
      return;
    }
    ...
    if (!emel::generator::detail::rms_norm(backend.hidden,
        ...
        !emel::generator::detail::matmul_vector(backend, block.feed_forward_up,
                                                backend.norm, backend.up)) {
      std::fprintf(stdout,
                   "generation_debug.residual_l2.layer%d: ffn gate/up failed\n",
                   layer);
      return;
    }
    ...
    if (!emel::generator::detail::matmul_vector(
            ...
            backend.projected)) {
      std::fprintf(stdout,
                   "generation_debug.residual_l2.layer%d: ffn down failed\n",
                   layer);
      return;
    }
    ...
    const std::span<const float> reference_hidden = reference_last_token_row(
        find_reference_tensor(graph_capture, key.c_str()),
        backend.hidden.size());
    const double l2 = l2_norm_diff(backend.hidden, reference_hidden);
    std::fprintf(stdout, "generation_debug.residual_l2.layer%d: l2=%g\n", layer,
                 l2);
  }
}
...
void dump_generation_live_backend_prefix_debug(
    const generation_load_state &state,
    const emel::paritychecker::parity_options &opts,
    const generation_result &emel_result,
    const generation_result &reference_result) {
  ...
  if (!capture_reference_graph_for_tokens(state, prefix_tokens_llama,
                                          graph_capture)) {
    std::fprintf(stdout,
                 "generation_debug.live: reference graph capture failed\n");
    return;
  }
  ...
  const int32_t timeline_start_generated =
      std::max<int32_t>(0, token_mismatch_index - timeline_window);
  const size_t prompt_debug_index = prompt_count > 0u ? prompt_count - 1u : 0u;
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size();
    ...
    const int32_t position = static_cast<int32_t>(token_index);
    if (!emel::generator::detail::copy_tensor_row(
            *dispatch_backend.token_embedding.tensor, token_id,
            dispatch_backend.hidden) ||
        !emel::generator::detail::copy_tensor_row(
            *shared_backend.token_embedding.tensor, token_id,
            shared_backend.hidden)) {
      std::fprintf(stdout,
                   "generation_debug.live: token embedding replay failed\n");
      return;
    }
    ...
    for (int32_t layer = 0; layer < dispatch_backend.n_layer; ++layer) {
      ...
      if (!emel::generator::detail::run_layer_flash(dispatch_backend, layer,
                                                    position) ||
          !emel::generator::detail::run_layer_flash(shared_backend, layer,
                                                    position)) {
      ...
      const int32_t generated_index =
          static_cast<int32_t>(token_index - prompt_count);
      if (is_prompt_token && token_mismatch_index == 0 &&
          token_index != prompt_debug_index) {
        continue;
      }
      if (!is_prompt_token && generated_index < timeline_start_generated) {
        continue;
      ...
      const std::string token_prefix =
          is_prompt_token
              ? "generation_debug.live.prompt" + std::to_string(token_index)
              : "generation_debug.live.gen" + std::to_string(generated_index);
      const std::string layer_prefix =
          token_prefix + ".layer" + std::to_string(layer);
      const std::string layer_suffix = std::to_string(layer);
      auto &block = dispatch_backend.blocks[static_cast<size_t>(layer)];
      const int32_t kv_dim =
          dispatch_backend.n_head_kv * dispatch_backend.head_dim_kv;
      const size_t cache_offset = emel::generator::detail::layer_cache_offset(
      ...
      std::fprintf(stdout, "%s.q: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_backend.q, shared_backend.q));
      std::fprintf(stdout, "%s.k: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_backend.k, shared_backend.k));
      std::fprintf(stdout, "%s.v: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_backend.v, shared_backend.v));
      std::fprintf(stdout, "%s.key_cache: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_key_cache, shared_key_cache));
      std::fprintf(stdout, "%s.value_cache: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_value_cache, shared_value_cache));
      std::fprintf(
          stdout, "%s.kqv_out: max_abs=%g\n", layer_prefix.c_str(),
          max_abs_diff(dispatch_backend.attn_ctx, shared_backend.attn_ctx));
      std::fprintf(stdout, "%s.ffn_norm: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_backend.norm, shared_backend.norm));
      std::fprintf(stdout, "%s.ffn_gate: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_backend.gate, shared_backend.gate));
      std::fprintf(stdout, "%s.ffn_up: max_abs=%g\n", layer_prefix.c_str(),
                   max_abs_diff(dispatch_backend.up, shared_backend.up));
      std::fprintf(
          stdout, "%s.ffn_out: max_abs=%g\n", layer_prefix.c_str(),
          max_abs_diff(dispatch_backend.projected, shared_backend.projected));
      std::fprintf(
          stdout, "%s.l_out: max_abs=%g\n", layer_prefix.c_str(),
          max_abs_diff(dispatch_backend.hidden, shared_backend.hidden));
      {
        std::vector<float> dispatch_attn_norm_reference(
            static_cast<size_t>(dispatch_backend.norm.size()));
        if (emel::generator::detail::rms_norm(
                dispatch_hidden_in, block.attention_norm,
                dispatch_backend.rms_epsilon, dispatch_attn_norm_reference)) {
          const std::span<const float> reference_attn_norm =
              reference_token_row(
                  find_reference_tensor(graph_capture,
                                        ("attn_norm-" + layer_suffix).c_str()),
          ...
          if (!reference_attn_norm.empty()) {
            std::fprintf(stdout,
                         ...
                         layer_prefix.c_str(),
                         max_abs_diff(dispatch_attn_norm_reference,
                                      reference_attn_norm));
          }
          dump_matrix_compare(
              (layer_prefix + ".dispatch_attn_q_matmul_ggml").c_str(),
              dispatch_backend, block.attention_q,
              dispatch_attn_norm_reference);
          dump_matrix_compare(
              (layer_prefix + ".dispatch_attn_k_matmul_ggml").c_str(),
              dispatch_backend, block.attention_k,
              dispatch_attn_norm_reference);
          dump_matrix_compare(
              (layer_prefix + ".dispatch_attn_v_matmul_ggml").c_str(),
              dispatch_backend, block.attention_v,
              dispatch_attn_norm_reference);
        }
      }
      if (const reference_tensor_capture *v_capture = find_reference_capture(
              graph_capture, ("Vcur-" + layer_suffix).c_str())) {
        const std::vector<float> reference_v = reference_token_tensor_values(
            *v_capture, static_cast<int32_t>(token_index));
        std::fprintf(stdout, "%s.dispatch_reference_v: max_abs=%g\n",
                     layer_prefix.c_str(),
                     reference_v.empty()
                         ? -1.0f
                         : max_abs_diff(dispatch_backend.v, reference_v));
      }
      if (const reference_tensor_capture *q_capture = find_reference_capture(
              graph_capture, ("Qcur-" + layer_suffix).c_str())) {
        const std::vector<float> reference_q = reference_token_tensor_values(
            *q_capture, static_cast<int32_t>(token_index));
        std::fprintf(stdout, "%s.dispatch_reference_q: max_abs=%g\n",
                     layer_prefix.c_str(),
                     reference_q.empty()
                         ? -1.0f
                         : max_abs_diff(dispatch_backend.q, reference_q));
      }
      if (const reference_tensor_capture *k_capture = find_reference_capture(
              graph_capture, ("Kcur-" + layer_suffix).c_str())) {
        const std::vector<float> reference_k = reference_token_tensor_values(
            *k_capture, static_cast<int32_t>(token_index));
        std::fprintf(stdout, "%s.dispatch_reference_k: max_abs=%g\n",
                     layer_prefix.c_str(),
                     reference_k.empty()
                         ? -1.0f
                         : max_abs_diff(dispatch_backend.k, reference_k));
      }
      {
        const std::span<const float> reference_kqv_out = reference_token_row(
            find_reference_tensor(graph_capture,
        ...
        if (!reference_kqv_out.empty()) {
          std::fprintf(
              stdout, "%s.dispatch_reference_kqv_out: max_abs=%g\n",
              layer_prefix.c_str(),
      ...
      }
      {
        const std::span<const float> reference_l_out = reference_token_row(
            find_reference_tensor(graph_capture,
                                  ("l_out-" + layer_suffix).c_str()),
            dispatch_backend.hidden.size(), static_cast<int32_t>(token_index));
        if (!reference_l_out.empty()) {
          std::fprintf(stdout, "%s.dispatch_reference_l_out: max_abs=%g\n",
                       layer_prefix.c_str(),
                       max_abs_diff(dispatch_backend.hidden, reference_l_out));
        }
      }

      std::vector<float> shared_attn_norm(
          static_cast<size_t>(shared_backend.norm.size()));
      std::vector<float> dispatch_attn_q_same(
          static_cast<size_t>(dispatch_backend.q.size()));
      std::vector<float> shared_attn_q_same(
          static_cast<size_t>(shared_backend.q.size()));
      std::vector<float> dispatch_attn_k_same(
          static_cast<size_t>(dispatch_backend.k.size()));
      std::vector<float> shared_attn_k_same(
          static_cast<size_t>(shared_backend.k.size()));
      std::vector<float> dispatch_attn_v_same(
          static_cast<size_t>(dispatch_backend.v.size()));
      std::vector<float> shared_attn_v_same(
          static_cast<size_t>(shared_backend.v.size()));
      std::vector<float> reference_attn_q_same(
          static_cast<size_t>(shared_backend.q.size()));
      std::vector<float> reference_attn_k_same(
          static_cast<size_t>(shared_backend.k.size()));
      std::vector<float> reference_attn_v_same(
          static_cast<size_t>(shared_backend.v.size()));
      if (emel::generator::detail::rms_norm(
              shared_hidden_in, block.attention_norm,
              dispatch_backend.rms_epsilon, shared_attn_norm) &&
          emel::generator::detail::matmul_vector(
              dispatch_backend, block.attention_q, shared_attn_norm,
              dispatch_attn_q_same) &&
          emel::generator::detail::matmul_vector(
              shared_backend, block.attention_q, shared_attn_norm,
              shared_attn_q_same) &&
          emel::generator::detail::matmul_vector(
              dispatch_backend, block.attention_k, shared_attn_norm,
              dispatch_attn_k_same) &&
          emel::generator::detail::matmul_vector(
              shared_backend, block.attention_k, shared_attn_norm,
              shared_attn_k_same) &&
          emel::generator::detail::matmul_vector(
              dispatch_backend, block.attention_v, shared_attn_norm,
              dispatch_attn_v_same) &&
          emel::generator::detail::matmul_vector(
              shared_backend, block.attention_v, shared_attn_norm,
              shared_attn_v_same)) {
        std::fprintf(stdout, "%s.same_input_q: max_abs=%g\n",
                     layer_prefix.c_str(),
                     max_abs_diff(dispatch_attn_q_same, shared_attn_q_same));
        std::fprintf(stdout, "%s.same_input_k: max_abs=%g\n",
                     layer_prefix.c_str(),
                     max_abs_diff(dispatch_attn_k_same, shared_attn_k_same));
        std::fprintf(stdout, "%s.same_input_v: max_abs=%g\n",
                     layer_prefix.c_str(),
                     max_abs_diff(dispatch_attn_v_same, shared_attn_v_same));
        if (matmul_vector_reference_q8(block.attention_q, shared_attn_norm,
                                       reference_attn_q_same)) {
          std::fprintf(
              stdout, "%s.same_input_q_dispatch_reference_q8: max_abs=%g\n",
              layer_prefix.c_str(),
        ...
        if (matmul_vector_reference_q8(block.attention_k, shared_attn_norm,
                                       reference_attn_k_same)) {
          std::fprintf(
              stdout, "%s.same_input_k_dispatch_reference_q8: max_abs=%g\n",
              layer_prefix.c_str(),
        ...
        if (matmul_vector_reference_q8(block.attention_v, shared_attn_norm,
                                       reference_attn_v_same)) {
          std::fprintf(
              stdout, "%s.same_input_v_dispatch_reference_q8: max_abs=%g\n",
              layer_prefix.c_str(),
        ...
        }
        {
          const auto compare_flash_request =
              [&](const char *label,
                  emel::generator::detail::native_backend &backend) {
                auto neon_request =
                    emel::generator::detail::make_flash_attn_request(
                        backend, layer, position);
                std::vector<float> neon_dst(backend.attn_ctx.size(), 0.0f);
                std::vector<float> shared_dst(backend.attn_ctx.size(), 0.0f);
                neon_request.dst = emel::generator::detail::make_dst_view_3d(
                    neon_dst.data(), neon_request.dst.ne[0],
                    neon_request.dst.ne[1], neon_request.dst.ne[2]);
                auto shared_request = neon_request;
                shared_request.dst = emel::generator::detail::make_dst_view_3d(
                    shared_dst.data(), shared_request.dst.ne[0],
                    shared_request.dst.ne[1], shared_request.dst.ne[2]);
                ...
                if (emel::kernel::aarch64::detail::run_flash_attn_ext_neon(
                    ...
                    emel::kernel::detail::run_flash_attn_ext_with_workspace(
                        shared_request, shared_workspace)) {
                  std::fprintf(stdout, "%s.%s: max_abs=%g\n",
                               layer_prefix.c_str(), label,
                               max_abs_diff(neon_dst, shared_dst));
                }
              };
          compare_flash_request("same_request_flash_dispatch_state",
                                dispatch_backend);
          compare_flash_request("same_request_flash_shared_state",
                                shared_backend);
        }
        {
          auto prepare_attention_scratch =
              [&](emel::generator::detail::native_backend &scratch) {
                return emel::generator::detail::prepare(scratch,
                                                        *state.model_data) ==
                           emel::error::cast(
                               emel::model::loader::error::none) &&
                       ((scratch.kernel_kind = shared_backend.kernel_kind),
                        true) &&
                       ((scratch.kernel.set_kind(scratch.kernel_kind)), true) &&
                       ((scratch.key_cache = shared_backend.key_cache), true) &&
                       ((scratch.value_cache = shared_backend.value_cache),
                        true) &&
          ...
          if (prepare_attention_scratch(prod_backend) &&
              compute_attention_with_emel_prod_style(
                  prod_backend, layer, position + 1,
                  std::span<const float>(
                      prod_backend.q_attn.data(),
                      static_cast<size_t>(prod_backend.n_embd)))) {
            std::fprintf(
                stdout, "%s.same_input_flash_vs_emel_prod_style: max_abs=%g\n",
                layer_prefix.c_str(),
                max_abs_diff(dispatch_backend.attn_ctx, prod_backend.attn_ctx));
            if (!reference_kqv_out.empty()) {
              std::fprintf(
                  stdout,
                  "%s.same_input_emel_prod_style_reference_kqv_out: "
                  "max_abs=%g\n",
                  layer_prefix.c_str(),
      ...
      std::vector<float> dispatch_ffn_gate_same(
          static_cast<size_t>(dispatch_backend.gate.size()));
      std::vector<float> shared_ffn_gate_same(
          static_cast<size_t>(shared_backend.gate.size()));
      std::vector<float> dispatch_ffn_up_same(
          static_cast<size_t>(dispatch_backend.up.size()));
      std::vector<float> shared_ffn_up_same(
          static_cast<size_t>(shared_backend.up.size()));
      std::vector<float> dispatch_ffn_out_same(
          static_cast<size_t>(dispatch_backend.projected.size()));
      std::vector<float> shared_ffn_out_same(
          static_cast<size_t>(shared_backend.projected.size()));
      std::vector<float> shared_ffn_hidden(
          static_cast<size_t>(shared_backend.ffn_hidden.size()));
      if (emel::generator::detail::matmul_vector(
              dispatch_backend, block.feed_forward_gate, shared_backend.norm,
              dispatch_ffn_gate_same) &&
          emel::generator::detail::matmul_vector(
              shared_backend, block.feed_forward_gate, shared_backend.norm,
              shared_ffn_gate_same) &&
          emel::generator::detail::matmul_vector(
              dispatch_backend, block.feed_forward_up, shared_backend.norm,
              dispatch_ffn_up_same) &&
          emel::generator::detail::matmul_vector(
              shared_backend, block.feed_forward_up, shared_backend.norm,
              shared_ffn_up_same)) {
        for (size_t idx = 0; idx < shared_ffn_hidden.size(); ++idx) {
          shared_ffn_hidden[idx] =
              emel::generator::detail::silu(shared_ffn_gate_same[idx]) *
              shared_ffn_up_same[idx];
        }
        std::fprintf(
            stdout, "%s.same_input_ffn_gate: max_abs=%g\n",
            layer_prefix.c_str(),
            max_abs_diff(dispatch_ffn_gate_same, shared_ffn_gate_same));
        std::fprintf(stdout, "%s.same_input_ffn_up: max_abs=%g\n",
                     layer_prefix.c_str(),
                     max_abs_diff(dispatch_ffn_up_same, shared_ffn_up_same));
        if (emel::generator::detail::matmul_vector(
                dispatch_backend, block.feed_forward_down, shared_ffn_hidden,
                dispatch_ffn_out_same) &&
            emel::generator::detail::matmul_vector(
                shared_backend, block.feed_forward_down, shared_ffn_hidden,
                shared_ffn_out_same)) {
          std::fprintf(
              stdout, "%s.same_input_ffn_out: max_abs=%g\n",
              layer_prefix.c_str(),
...
void dump_reference_decode_seam(const generation_load_state &state) {
  ...
  std::fprintf(stdout, "reference_impl: source=%.*s repo=%.*s ref=%.*s\n",
               static_cast<int>(k_reference_impl_source.size()),
               k_reference_impl_source.data(),
               static_cast<int>(k_reference_impl_repository.size()),
               k_reference_impl_repository.data(),
  ...
  std::fprintf(stdout, "kernel_dispatch: kind=%s calls=%" PRIu64 "\n",
               kernel_kind_name(kernel_kind),
               state.generator->generation_kernel_dispatch_calls());
  std::fprintf(stdout,
               "flash_dispatch: calls=%" PRIu64 " optimized=%" PRIu64
               " shared=%" PRIu64 "\n",
  ...
  std::fprintf(stdout,
               ...
               "approved_dense_f32_by_contract=%u disallowed_fallback=%u "
               "explicit_no_claim=%u\n",
  ...
  if (emel::model::llama::detail::build_execution_view(*state.model_data,
                                                       execution) ==
      emel::error::cast(emel::model::loader::error::none)) {
    const auto audit =
        emel::model::llama::detail::build_quantized_path_audit(execution);
    ...
    for (const auto &stage : audit.stages) {
      ...
      const auto tensor_name =
          emel::model::llama::detail::tensor_type_name(stage.tensor_type);
      const auto contract_name =
          emel::model::llama::detail::quantized_contract_kind_name(
              stage.contract);
      const uint32_t supported =
          stage.contract == emel::model::llama::detail::
      ...
      std::fprintf(
          ...
          static_cast<int>(contract_name.size()), contract_name.data(),
          supported, stage.consistent_across_layers ? 1u : 0u);
    }
  }
}

void dump_generation_failure_surface(
    generation_load_state &state, const generation_result *emel_result,
    const generation_result *reference_result,
    const emel::paritychecker::parity_options &opts) {
  enum class failure_surface_kind {
    summary,
    lite,
    full,
  };
  ...
  const auto resolve_failure_surface = [&]() {
    const char *value = std::getenv("EMEL_PARITYCHECKER_FAILURE_SURFACE");
    if (value != nullptr) {
      if (std::strcmp(value, "summary") == 0) {
        return failure_surface_kind::summary;
      }
      if (std::strcmp(value, "lite") == 0) {
        return failure_surface_kind::lite;
      }
      if (std::strcmp(value, "full") == 0) {
        return failure_surface_kind::full;
      }
    }
    if (env_enabled("EMEL_PARITYCHECKER_LITE_DEBUG")) {
      return failure_surface_kind::lite;
    }
    return failure_surface_kind::full;
  };
  const failure_surface_kind failure_surface = resolve_failure_surface();
  const auto dump_generation_lite_modes = [&](const generation_result
                                                  &reference_generation) {
    struct mode_case {
      const char *label = nullptr;
      native_layer_runner run_layer = nullptr;
    };

    const char *lite_case_set =
        std::getenv("EMEL_PARITYCHECKER_LITE_DEBUG_CASES");
    const bool smoke_only =
        lite_case_set != nullptr && std::strcmp(lite_case_set, "smoke") == 0;
    const std::array<mode_case, 2> smoke_mode_cases{{
        {"runtime_flash_q", run_layer_with_flash_request_q},
        {"runtime_flash_q_attn", run_layer_with_flash_request_q_attn},
    }};
    const std::array<mode_case, 4> mode_cases{{
        {"runtime_flash_q", run_layer_with_flash_request_q},
        {"runtime_flash_q_attn", run_layer_with_flash_request_q_attn},
        {"emel_prod_style", run_layer_with_scalar_attention_emel_prod_style},
        {"exact_q1", run_layer_with_exact_q1_scalar_attention},
    }};
    const std::span<const mode_case> selected_mode_cases =
        smoke_only ? std::span<const mode_case>(smoke_mode_cases)
                   : std::span<const mode_case>(mode_cases);

    std::fprintf(stderr, "generation_debug.lite: begin\n");
    std::fflush(stderr);
    for (const auto &mode : selected_mode_cases) {
      generation_result alt_result = {};
      const emel::error::type alt_err =
          run_custom_native_generate(state, opts, mode.run_layer, alt_result);
      if (alt_err != emel::error::cast(emel::generator::error::none)) {
        std::fprintf(stderr, "generation_debug.lite.%s: error=%d\n", mode.label,
                     static_cast<int>(alt_err));
        std::fflush(stderr);
        continue;
      }
      ...
      std::fprintf(
          stderr,
          "generation_debug.lite.%s: match=%d token_mismatch=%d "
          "byte_mismatch=%zu "
          "tokens=%d output_bytes=%zu alt_token0=%d ref_token0=%d\n",
          mode.label,
          generation_results_match(alt_result, reference_generation) ? 1 : 0,
          first_token_mismatch_index(alt_result, reference_generation),
          first_mismatch_offset(alt_result, reference_generation),
          alt_result.tokens_generated, alt_result.output_length,
          alt_result.trace.token_count > 0 ? alt_result.trace.token_ids[0] : -1,
          reference_generation.trace.token_count > 0
              ? reference_generation.trace.token_ids[0]
              : -1);
      std::fflush(stderr);
    }
    std::fprintf(stderr, "generation_debug.lite: end\n");
    std::fflush(stderr);
  };

  if (failure_surface == failure_surface_kind::summary) {
    return;
  }
  ...
  if (emel_result != nullptr && reference_result != nullptr) {
    std::string emel_formatted_prompt = {};
    std::string reference_formatted_prompt = {};
    const bool have_emel_formatted_prompt =
        emel::tools::generation_formatter_contract::format_single_user_prompt(
            state.formatter_binding, opts.text, emel_formatted_prompt);
    const bool have_reference_formatted_prompt = emel::tools::
        generation_formatter_contract::format_reference_single_user_prompt(
            state.reference.formatter, opts.text, reference_formatted_prompt);
    if (have_emel_formatted_prompt && have_reference_formatted_prompt) {
      std::fprintf(
          stdout,
          "prompt_contract: emel_bytes=%zu reference_bytes=%zu match=%u\n",
          emel_formatted_prompt.size(), reference_formatted_prompt.size(),
          emel_formatted_prompt == reference_formatted_prompt ? 1u : 0u);
      std::fprintf(stdout, "prompt_text.emel=%.*s\n",
                   static_cast<int>(emel_formatted_prompt.size()),
                   emel_formatted_prompt.data());
      std::fprintf(stdout, "prompt_text.reference=%.*s\n",
                   static_cast<int>(reference_formatted_prompt.size()),
                   reference_formatted_prompt.data());
    }

    std::vector<int32_t> emel_prompt_tokens = {};
    std::vector<llama_token> reference_prompt_tokens = {};
    const bool have_emel_prompt_tokens =
        tokenize_conditioned_prompt(state, opts, emel_prompt_tokens);
    const bool have_reference_prompt_tokens = tokenize_reference_prompt(
        state.reference, opts, reference_prompt_tokens);
    if (have_emel_prompt_tokens && have_reference_prompt_tokens) {
      std::fprintf(stdout, "prompt_tokens: emel=%zu reference=%zu\n",
                   emel_prompt_tokens.size(), reference_prompt_tokens.size());
      dump_prompt_token_window("emel", state.reference, emel_prompt_tokens);
      dump_prompt_token_window_reference("reference", state.reference,
    ...
    const bool have_emel_token =
        token_mismatch_index >= 0 &&
        token_mismatch_index < emel_result->trace.token_count;
    const bool have_reference_token =
        token_mismatch_index >= 0 &&
        token_mismatch_index < reference_result->trace.token_count;
    std::fprintf(stdout,
                 "generation_trace: emel_tokens=%d reference_tokens=%d "
                 "first_token_mismatch=%d\n",
                 emel_result->trace.token_count,
                 reference_result->trace.token_count, token_mismatch_index);
    if (have_emel_token && have_reference_token) {
      std::fprintf(
          stdout,
          "generation_trace.mismatch: emel_token=%d emel_gap=%g "
          "reference_token=%d "
          "reference_gap=%g\n",
          emel_result->trace
              .token_ids[static_cast<size_t>(token_mismatch_index)],
          emel_result->trace
              .top_score_gaps[static_cast<size_t>(token_mismatch_index)],
          reference_result->trace
              .token_ids[static_cast<size_t>(token_mismatch_index)],
          reference_result->trace
              .top_score_gaps[static_cast<size_t>(token_mismatch_index)]);
      dump_generation_trace_window("emel", state.reference, *emel_result,
                                   token_mismatch_index);
      dump_generation_trace_window("reference", state.reference,
                                   *reference_result, token_mismatch_index);
    }
    if (failure_surface == failure_surface_kind::lite) {
      dump_generation_lite_modes(*reference_result);
      return;
    }
    dump_generation_live_backend_prefix_debug(state, opts, *emel_result,
                                              *reference_result);
    dump_generation_gen0_attention_debug(state, opts, *emel_result,
                                         *reference_result);
    dump_generation_target_attention_debug(state, opts, *emel_result,
                                           *reference_result);
    dump_generation_prefix_state_debug(state, opts, *emel_result,
                                       *reference_result);
    dump_generation_reference_q_stage_debug(state, opts, *emel_result,
                                            *reference_result);
...
kv_value_view(const generation_load_state &state,
              const emel::gguf::loader::kv_entry &entry) {
...
const emel::gguf::loader::kv_entry *
find_kv_entry(const generation_load_state &state, const std::string_view key) {
...
bool decode_integer_array_first_nonzero(
    const generation_load_state &state,
    const emel::gguf::loader::kv_entry &entry, int32_t &value_out) {
  const std::span<const uint8_t> bytes = kv_value_view(state, entry);
  namespace constants = emel::gguf::loader::detail::constants;
...
emel::error::type populate_model_metadata(const generation_load_state &state,
                                          emel::model::data &model_data) {
  const emel::model::detail::kv_binding metadata_binding{
      .arena = std::span<const uint8_t>{state.kv_arena},
      .entries =
          std::span<const emel::gguf::loader::kv_entry>{state.kv_entries},
  };
  return emel::model::detail::populate_model_metadata_from_gguf(
      metadata_binding, model_data);
}
...
void resolve_generation_formatter_binding(generation_load_state &state) {
  ...
  if (entry != nullptr &&
      !decode_string_value(state, *entry, primary_template)) {
    state.formatter_binding =
        emel::tools::generation_formatter_contract::formatter_binding{
            .formatter_ctx = nullptr,
            .format_prompt = emel::text::formatter::format_raw,
            .support = emel::tools::generation_formatter_contract::
                support_kind::unsupported_template,
  ...
  state.formatter_binding = emel::tools::generation_formatter_contract::
      resolve_primary_template_binding(primary_template, named_template_count);
}

void print_generation_formatter_contract(
    FILE *stream, const generation_load_state &state,
    const maintained_generation_fixture &fixture) {
  ...
  const std::string_view architecture =
      state.model_data != nullptr
          ? emel::model::architecture_name_view(*state.model_data)
          : std::string_view{};
  if (!architecture.empty()) {
    std::fprintf(stream, "architecture_contract=%.*s\n",
...
run_emel_parse_model(void *owner, const emel::model::loader::event::load &req) {
  ...
  uint32_t tensor_count = 0u;
  const emel::error::type parse_err =
      parse_gguf_kv_storage(state, file_image, req.model_data, tensor_count);
  if (parse_err != emel::error::cast(emel::model::loader::error::none)) {
    return parse_err;
  }

  req.model_data.n_tensors = tensor_count;
...
emel::error::type
run_emel_load_weights(void *owner, const emel::model::loader::event::load &req,
  ...
  if (!state.weight_loader.process_event(bind_ev) || !state.weight.bind_done ||
  ...
  emel::model::weight_loader::event::plan_load plan_ev{
      std::span<emel::model::weight_loader::effect_request>{
          state.effect_requests},
  };
  plan_ev.on_done = {&state, on_weight_plan_done};
  plan_ev.on_error = {&state, on_weight_plan_error};
  if (!state.weight_loader.process_event(plan_ev) || !state.weight.plan_done ||
  ...
  emel::model::weight_loader::event::apply_effect_results apply_ev{
      std::span<const emel::model::weight_loader::effect_result>{
          state.effect_results.data(), effect_count},
  };
  ...
  if (!state.weight_loader.process_event(apply_ev) ||
      !state.weight.apply_done || state.weight.apply_error) {
    return map_weight_loader_error(state.weight.err);
  }
...
std::vector<float> make_signed_data(const int64_t count, const float scale,
                                    const float bias) {
...
std::vector<float> make_positive_data(const int64_t count, const float scale,
                                      const float bias) {
...
template <class tensor_type> void fill_default_nb(tensor_type &tensor) {
...
make_src_view(const float *data, const uint64_t ne0, const uint64_t ne1 = 1,
              const uint64_t ne2 = 1, const uint64_t ne3 = 1) {
...
emel::kernel::event::tensor_view_mut
make_dst_view(float *data, const uint64_t ne0, const uint64_t ne1 = 1,
              const uint64_t ne2 = 1, const uint64_t ne3 = 1) {
...
bool almost_equal_f32(const float actual, const float expected) {
  ...
  const double tol =
      k_f32_atol + k_f32_rtol * std::fabs(static_cast<double>(expected));
  return diff <= tol;
}

bool compare_f32_vectors(const char *backend, const char *case_name,
                         const std::vector<float> &actual,
                         const std::vector<float> &expected) {
  if (actual.size() != expected.size()) {
    std::fprintf(stderr, "[%s] %s size mismatch: emel=%zu ggml=%zu\n", backend,
                 case_name, actual.size(), expected.size());
    return false;
  }

  for (size_t i = 0; i < actual.size(); ++i) {
    if (!almost_equal_f32(actual[i], expected[i])) {
      std::fprintf(stderr, "[%s] %s mismatch at %zu: emel=%0.8f ggml=%0.8f\n",
                   backend, case_name, i, actual[i], expected[i]);
      return false;
    }
  }
  return true;
...
void set_tensor_f32(ggml_tensor *tensor, const std::vector<float> &values) {
  std::memcpy(ggml_get_data_f32(tensor), values.data(),
...
bool run_ggml_unary(const std::vector<float> &src, std::vector<float> &out,
                    build_fn build) {
  ggml_case_context c{};
  ggml_tensor *a = ggml_new_tensor_1d(c.ctx, GGML_TYPE_F32,
...
bool run_ggml_binary(const std::vector<float> &lhs,
                     const std::vector<float> &rhs, std::vector<float> &out,
                     build_fn build) {
  ggml_case_context c{};
  ggml_tensor *a = ggml_new_tensor_1d(c.ctx, GGML_TYPE_F32,
                                      static_cast<int64_t>(lhs.size()));
  ggml_tensor *b = ggml_new_tensor_1d(c.ctx, GGML_TYPE_F32,
...
bool run_ggml_softmax(const std::vector<float> &src, std::vector<float> &out) {
  ggml_case_context c{};
  ggml_tensor *a =
      ggml_new_tensor_2d(c.ctx, GGML_TYPE_F32, k_softmax_width, k_softmax_rows);
  set_tensor_f32(a, src);
...
bool run_ggml_mul_mat(const std::vector<float> &matrix_a,
  ...
  ggml_case_context c{};
  ggml_tensor *a =
      ggml_new_tensor_2d(c.ctx, GGML_TYPE_F32, k_mm_k, k_mm_n); // [n, k]
  ggml_tensor *b =
      ggml_new_tensor_2d(c.ctx, GGML_TYPE_F32, k_mm_k, k_mm_m); // [m, k]
...
bool compute_attention_with_ggml_nonflash_exact_scores_prod_value(
  ...
  const size_t kv_size =
      static_cast<size_t>(head_dim * kv_tokens * kv_head_count);
  if (q_vector.size() != q_size || position_limit <= 0 ||
      position_limit > kv_tokens) {
    return false;
  }

  ggml_case_context c{};
  ggml_tensor *q =
      ggml_new_tensor_3d(c.ctx, GGML_TYPE_F32, head_dim, 1, head_count);
  ggml_tensor *k = ggml_new_tensor_3d(c.ctx, GGML_TYPE_F16, head_dim, kv_tokens,
                                      kv_head_count);
    ...
    for (int32_t token = 0; token < kv_tokens; ++token) {
      for (int32_t dim = 0; dim < head_dim; ++dim) {
        const size_t src_index =
            layer_offset +
            static_cast<size_t>(((token * kv_head_count) + head) * head_dim +
                                dim);
        const size_t dst_index =
            static_cast<size_t>(((head * kv_tokens) + token) * head_dim + dim);
        k_f16[dst_index] = ggml_fp32_to_fp16(
            fp16_storage_to_fp32(backend.key_cache[src_index]));
      }
    }
  }
  std::memcpy(ggml_get_data(k), k_f16.data(),
              k_f16.size() * sizeof(ggml_fp16_t));
  ...
  kq = ggml_soft_max_ext(c.ctx, kq, mask,
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    for (int32_t token = 0; token < position_limit; ++token) {
      ...
      const size_t cache_offset = emel::generator::detail::layer_cache_offset(
                                      backend, layer_index, token, kv_dim) +
                                  kv_offset;
      for (int32_t dim = 0; dim < head_dim; ++dim) {
        backend.attn_ctx[q_offset + static_cast<size_t>(dim)] +=
            weight *
...
bool run_ggml_nonflash_attn_case(
    std::span<const float> q_data, std::span<const float> k_data,
    std::span<const float> v_data, const int64_t head_dim,
    const int64_t kv_tokens, const int64_t active_kv_tokens,
    const int64_t head_count, const int64_t kv_head_count, const float scale,
    std::vector<float> &out) {
  const size_t q_size = static_cast<size_t>(head_dim * head_count);
  const size_t kv_size =
      static_cast<size_t>(head_dim * kv_tokens * kv_head_count);
  if (q_data.size() != q_size || k_data.size() != kv_size ||
      v_data.size() != kv_size || head_dim <= 0 || kv_tokens <= 0 ||
      active_kv_tokens <= 0 || active_kv_tokens > kv_tokens ||
  ...
  ggml_tensor *k = ggml_new_tensor_3d(c.ctx, GGML_TYPE_F16, head_dim, kv_tokens,
                                      kv_head_count);
  ggml_tensor *v = ggml_new_tensor_3d(c.ctx, GGML_TYPE_F16, kv_tokens, head_dim,
                                      kv_head_count);
  ggml_tensor *mask = ggml_new_tensor_2d(c.ctx, GGML_TYPE_F32, kv_tokens, 1);
  ...
  for (int64_t head = 0; head < kv_head_count; ++head) {
    for (int64_t token = 0; token < kv_tokens; ++token) {
      for (int64_t dim = 0; dim < head_dim; ++dim) {
        ...
        const size_t k_dst_index =
            static_cast<size_t>(((head * kv_tokens) + token) * head_dim + dim);
        const size_t v_dst_index =
            static_cast<size_t>(((head * head_dim) + dim) * kv_tokens + token);
  ...
  std::memcpy(ggml_get_data(k), k_f16.data(),
              k_f16.size() * sizeof(ggml_fp16_t));
  std::memcpy(ggml_get_data(v), v_f16.data(),
              v_f16.size() * sizeof(ggml_fp16_t));
  auto *mask_data = ggml_get_data_f32(mask);
  for (int64_t token = 0; token < kv_tokens; ++token) {
...
bool run_ggml_nonflash_attn_case(
    std::span<const float> q_data, std::span<const uint16_t> k_data,
    std::span<const uint16_t> v_data, const int64_t head_dim,
    const int64_t kv_tokens, const int64_t active_kv_tokens,
    const int64_t head_count, const int64_t kv_head_count, const float scale,
    std::vector<float> &out) {
  const std::vector<float> k_decoded = decode_fp16_storage(k_data);
  const std::vector<float> v_decoded = decode_fp16_storage(v_data);
  return run_ggml_nonflash_attn_case(q_data, k_decoded, v_decoded, head_dim,
                                     kv_tokens, active_kv_tokens, head_count,
                                     kv_head_count, scale, out);
}
...
bool run_emel_nonflash_f16_ggml_softmax_case(
  ...
  for (int64_t head = 0; head < head_count; ++head) {
    ...
    for (int64_t dim = 0; dim < head_dim; ++dim) {
      q_f16[static_cast<size_t>(dim)] =
          ggml_fp32_to_fp16(q_data[q_offset + static_cast<size_t>(dim)]);
    }

    float max_score = -std::numeric_limits<float>::infinity();
    for (int64_t token = 0; token < active_kv_tokens; ++token) {
      const size_t kv_offset =
          static_cast<size_t>(((token * kv_head_count) + kv_head) * head_dim);
      for (int64_t dim = 0; dim < head_dim; ++dim) {
        k_f16[static_cast<size_t>(dim)] =
            ggml_fp32_to_fp16(k_data[kv_offset + static_cast<size_t>(dim)]);
      }

      float score = 0.0f;
      ggml_vec_dot_f16(static_cast<int>(head_dim), &score, 0u, k_f16.data(), 0u,
                       q_f16.data(), 0u, 1);
      score *= scale;
    ...
    const reference_ggml_float score_sum = ggml_vec_soft_max_f32(
        static_cast<int>(kv_tokens), probs.data(), scores.data(), max_score);
    const float inv_score_sum =
        score_sum == 0.0 ? 0.0f : static_cast<float>(1.0 / score_sum);
    for (int64_t token = 0; token < kv_tokens; ++token) {
    ...
    for (int64_t dim = 0; dim < head_dim; ++dim) {
      for (int64_t token = 0; token < kv_tokens; ++token) {
        const size_t kv_offset =
            static_cast<size_t>(((token * kv_head_count) + kv_head) * head_dim);
        value_f16[static_cast<size_t>(token)] =
            ggml_fp32_to_fp16(v_data[kv_offset + static_cast<size_t>(dim)]);
      }

      float dot = 0.0f;
      ggml_vec_dot_f16(static_cast<int>(kv_tokens), &dot, 0u, value_f16.data(),
                       0u, weight_f16.data(), 0u, 1);
...
bool run_emel_nonflash_f16_ggml_softmax_case(
  ...
  const std::vector<float> k_decoded = decode_fp16_storage(k_data);
  const std::vector<float> v_decoded = decode_fp16_storage(v_data);
  return run_emel_nonflash_f16_ggml_softmax_case(
      q_data, k_decoded, v_decoded, head_dim, kv_tokens, active_kv_tokens,
      head_count, kv_head_count, scale, out);
}

bool run_emel_prod_style_attn_case(
    std::span<const float> q_data, std::span<const float> k_data,
    std::span<const float> v_data, const int64_t head_dim,
    const int64_t kv_tokens, const int64_t active_kv_tokens,
    const int64_t head_count, const int64_t kv_head_count, const float scale,
  ...
  for (int64_t head = 0; head < head_count; ++head) {
    ...
    for (int64_t token = 0; token < active_kv_tokens; ++token) {
      const float weight =
          rounded_probs[static_cast<size_t>(token)] * inv_score_sum;
    ...
    for (int64_t dim = 0; dim < head_dim; ++dim) {
      for (int64_t token = 0; token < kv_tokens; ++token) {
        if (token < active_kv_tokens) {
          const size_t kv_offset = static_cast<size_t>(
              ((token * kv_head_count) + kv_head) * head_dim);
          value_column[static_cast<size_t>(token)] =
      ...
      out[q_offset + static_cast<size_t>(dim)] =
          ::emel::kernel::detail::dot_product_ggml_f16_scores(
              value_column.data(), rounded_probs.data(),
...
bool run_emel_prod_style_attn_case(
  ...
  const std::vector<float> k_decoded = decode_fp16_storage(k_data);
  const std::vector<float> v_decoded = decode_fp16_storage(v_data);
  return run_emel_prod_style_attn_case(q_data, k_decoded, v_decoded, head_dim,
                                       kv_tokens, active_kv_tokens, head_count,
                                       kv_head_count, scale, out);
}
...
bool run_emel_prod_style_float_value_attn_case(
  ...
  const std::vector<float> k_decoded = decode_fp16_storage(k_data);
  const std::vector<float> v_decoded = decode_fp16_storage(v_data);
  return run_emel_prod_style_float_value_attn_case(
      q_data, k_decoded, v_decoded, head_dim, kv_tokens, active_kv_tokens,
      head_count, kv_head_count, scale, out);
}

bool run_ggml_flash_attn_ext_case(
    std::span<const float> q_data, std::span<const float> k_data,
    std::span<const float> v_data, const int64_t head_dim,
    const int64_t kv_tokens, const int64_t head_count,
    const int64_t kv_head_count, const float scale, std::vector<float> &out) {
  const size_t q_size = static_cast<size_t>(head_dim * head_count);
  const size_t kv_size =
      static_cast<size_t>(head_dim * kv_tokens * kv_head_count);
  if (q_data.size() != q_size || k_data.size() != kv_size ||
  ...
  ggml_case_context c{};
  ggml_tensor *q_backing =
      ggml_new_tensor_1d(c.ctx, GGML_TYPE_F32, static_cast<int64_t>(q_size));
  ggml_tensor *q =
      q_backing == nullptr
          ? nullptr
          : ggml_view_3d(c.ctx, q_backing, head_dim, 1, head_count,
                         sizeof(float) * static_cast<size_t>(head_dim),
                         sizeof(float) * static_cast<size_t>(head_dim), 0);
  ...
  ggml_tensor *k =
      k_backing == nullptr
          ? nullptr
          : ggml_view_3d(c.ctx, k_backing, head_dim, kv_tokens, kv_head_count,
                         sizeof(ggml_fp16_t) *
  ...
  ggml_tensor *v =
      v_backing == nullptr
          ? nullptr
          : ggml_view_3d(c.ctx, v_backing, head_dim, kv_tokens, kv_head_count,
                         sizeof(ggml_fp16_t) *
                             static_cast<size_t>(head_dim * kv_head_count),
                         sizeof(ggml_fp16_t) * static_cast<size_t>(head_dim),
                         0);
  ...
  std::memcpy(ggml_get_data(k_backing), k_f16.data(),
              k_f16.size() * sizeof(ggml_fp16_t));
  std::memcpy(ggml_get_data(v_backing), v_f16.data(),
              v_f16.size() * sizeof(ggml_fp16_t));
...
bool run_ggml_flash_attn_ext_case(
  ...
  const std::vector<float> k_decoded = decode_fp16_storage(k_data);
  const std::vector<float> v_decoded = decode_fp16_storage(v_data);
  return run_ggml_flash_attn_ext_case(q_data, k_decoded, v_decoded, head_dim,
                                      kv_tokens, head_count, kv_head_count,
                                      scale, out);
}
...
bool run_ggml_flash_attn_ext_masked_case(
  ...
  const std::vector<float> k_decoded = decode_fp16_storage(k_data);
  const std::vector<float> v_decoded = decode_fp16_storage(v_data);
  return run_ggml_flash_attn_ext_masked_case(
      q_data, k_decoded, v_decoded, head_dim, kv_tokens, active_kv_tokens,
      head_count, kv_head_count, scale, out);
}
...
bool run_backend_kernel_parity(const char *backend, exec_fn exec) {
...

  {
    ...
    } else if (!compare_f32_vectors(backend, "op_soft_max", emel_out,
                                    ggml_out)) {
      ok = false;
    }
  }

  {
    ...
    for (int64_t p = 0; p < k_mm_k; ++p) {
      for (int64_t j = 0; j < k_mm_n; ++j) {
        src1[static_cast<size_t>(p * k_mm_n + j)] =
            matrix_a[static_cast<size_t>(j * k_mm_k + p)];
      }
    }
    std::vector<float> emel_out(static_cast<size_t>(k_mm_n * k_mm_m));
    emel::kernel::event::op_mul_mat ev{
        .src0 = make_src_view(matrix_b.data(), static_cast<uint64_t>(k_mm_k),
                              static_cast<uint64_t>(k_mm_m)),
        .src1 = make_src_view(src1.data(), static_cast<uint64_t>(k_mm_n),
    ...
    } else if (!compare_f32_vectors(backend, "op_mul_mat", emel_out,
                                    ggml_out)) {
      ok = false;
    }
  }

  {
    ...
    } else if (!compare_f32_vectors(backend, "op_unary_neg", emel_out,
                                    ggml_out)) {
      ok = false;
    }
  }

  {
    ...
    } else if (!compare_f32_vectors(backend, "op_unary_relu", emel_out,
                                    ggml_out)) {
      ok = false;
    }
  }
...
int run_kernel_parity(const emel::paritychecker::parity_options &) {
  ...
  auto x86_exec = [&](const auto &ev) { return x86_machine.process_event(ev); };
...
int run_tokenizer_parity(const emel::paritychecker::parity_options &opts) {
  ...
  if (!load_emel_vocab_from_gguf_file(opts.model_path, *emel_vocab)) {
    std::fprintf(stderr, "failed to load emel vocab from gguf metadata\n");
    return 1;
  }
  ...
  case tokenizer_model::SPM:
    return emel::paritychecker::run_tokenizer_spm_parity(opts, *llama_vocab_ptr,
                                                         *emel_vocab);
  case tokenizer_model::BPE:
    return emel::paritychecker::run_tokenizer_bpe_parity(opts, *llama_vocab_ptr,
                                                         *emel_vocab);
  case tokenizer_model::WPM:
    return emel::paritychecker::run_tokenizer_wpm_parity(opts, *llama_vocab_ptr,
                                                         *emel_vocab);
  case tokenizer_model::UGM:
    return emel::paritychecker::run_tokenizer_ugm_parity(opts, *llama_vocab_ptr,
                                                         *emel_vocab);
  case tokenizer_model::RWKV:
...
int run_gbnf_parser_parity(const emel::paritychecker::parity_options &opts) {
  ...
  if (emel_ok != llama_ok) {
    std::fprintf(stderr,
                 ...
                 emel_ok ? "ok" : "error", llama_ok ? "ok" : "error", emel_err);
    if (opts.dump) {
      if (emel_ok) {
  ...
  std::fprintf(stdout, "parity ok (%u rules, %u elements)\n",
               emel_grammar.rule_count, emel_grammar.element_count);
  return 0;
}

int run_jinja_parity(const emel::paritychecker::parity_options &opts) {
  emel::text::jinja::program emel_program{};
  int32_t emel_parse_err =
      static_cast<int32_t>(emel::text::jinja::parser::error::none);
  ...
  const bool reference_parse_ok =
      run_reference_jinja_parse(opts.text, reference_program);

  if (emel_parse_ok != reference_parse_ok) {
    std::fprintf(stderr,
                 "jinja parse outcome mismatch: emel=%s reference=%s "
                 "(emel_err=%d at %zu)\n",
                 emel_parse_ok ? "ok" : "error",
                 reference_parse_ok ? "ok" : "error", emel_parse_err,
  ...
  const std::string_view reference_cmp =
      strip_trailing_newline(reference_rendered);
  if (emel_cmp != reference_cmp) {
    std::fprintf(stderr,
                 "jinja render mismatch: emel_len=%zu reference_len=%zu\n",
                 emel_rendered.size(), reference_rendered.size());
    if (opts.dump) {
...
int run_generation_harness_contract(
    const emel::paritychecker::parity_options &opts) {
  llama_backend_guard backend_guard{};
  llama_log_silencer log_silencer{};
  ...
  const maintained_generation_fixture *fixture =
      find_generation_fixture(opts.model_path);
  if (fixture == nullptr) {
    ...
    std::fprintf(stderr,
                 ...
                 supported_fixtures.c_str(), opts.model_path.c_str());
    return 1;
  }

  generation_load_state state{};
  if (!read_file_bytes(opts.model_path, state.file_bytes) ||
      state.file_bytes.empty()) {
    std::fprintf(stderr,
  ...
  if (!state.model_loader.process_event(request) || !state.load.done ||
      state.load.error) {
    resolve_generation_formatter_binding(state);
    print_generation_formatter_contract(stderr, state, *fixture);
    const emel::error::type err =
        state.load.error
            ? state.load.err
            : emel::error::cast(emel::model::loader::error::internal_error);
    std::fprintf(stderr, "generation load failed (fixture=%s err=%s)\n",
                 fixture->name.data(), model_loader_error_name(err));
    return 1;
  }
  ...
  if (!emel::tools::generation_formatter_contract::binding_supported(
    ...
    std::fprintf(stderr, "generation load failed (fixture=%s err=%s)\n",
                 fixture->name.data(),
                 model_loader_error_name(emel::error::cast(
                     emel::model::loader::error::model_invalid)));
    return 1;
  }

  if (!load_generation_reference_backend(opts.model_path, state)) {
    print_generation_formatter_contract(stderr, state, *fixture);
    std::fprintf(stderr, "generation vocab load failed (fixture=%s)\n",
                 fixture->name.data());
    return 1;
  }
  ...
  if (initialize_err != emel::error::cast(emel::generator::error::none)) {
    ...
    for (uint32_t idx = 0; idx < state.model_data->n_tensors; ++idx) {
      const std::string_view name = emel::model::tensor_name_view(
          *state.model_data, state.model_data->tensors[idx]);
      output_weight_count += static_cast<int32_t>(name == "output.weight");
      q_norm_count += static_cast<int32_t>(name.find("attn_q_norm.weight") !=
                                           std::string_view::npos);
      k_norm_count += static_cast<int32_t>(name.find("attn_k_norm.weight") !=
                                           std::string_view::npos);
    }
    std::fprintf(
        stderr,
        "generation initialize debug architecture=%.*s n_layers=%d "
        "n_tensors=%u "
        "output_weight=%d q_norm=%d k_norm=%d\n",
        static_cast<int>(
            emel::model::architecture_name_view(*state.model_data).size()),
    ...
    if (emel::model::llama::detail::build_execution_view(*state.model_data,
      ...
      if (emel::model::llama::detail::lookup_block_view(debug_execution, 0,
                                                        debug_block) ==
          emel::error::cast(emel::model::loader::error::none)) {
        const auto print_tensor_shape =
            [&](const char *label,
              ...
              std::fprintf(
                  stderr,
                  "generation initialize debug %s type=%d dims=%" PRIu64
                  "x%" PRIu64 "\n",
                  label, tensor->type, dim0, dim1);
            };
      ...
      std::fprintf(stderr,
                   "generation initialize debug build_execution_view failed\n");
    }
    std::fprintf(stderr, "generation initialize failed (fixture=%s err=%s)\n",
                 fixture->name.data(), generator_error_name(initialize_err));
    return 1;
  }
  ...
  const emel::error::type generation_err =
      run_emel_generate(state, opts, std::span<char>{emel_result.output},
                        emel_result.output_length, emel_result.trace);
  if (generation_err != emel::error::cast(emel::generator::error::none)) {
    print_generation_formatter_contract(stderr, state, *fixture);
    std::fprintf(stderr,
                 "generation error (fixture=%s err=%s generated_tokens=%d "
                 "output_bytes=%zu)\n",
                 fixture->name.data(), generator_error_name(generation_err),
  ...
  if (runtime_contract.disallowed_fallback != 0u ||
      runtime_contract.explicit_no_claim != 0u) {
    std::fprintf(
        stderr,
        "generation quantized contract failed (fixture=%s native_quantized=%u "
        "approved_dense_f32_by_contract=%u disallowed_fallback=%u "
        "explicit_no_claim=%u)\n",
        fixture->name.data(), runtime_contract.native_quantized,
        runtime_contract.approved_dense_f32_by_contract,
        runtime_contract.disallowed_fallback,
        runtime_contract.explicit_no_claim);
    dump_generation_failure_surface(state, &emel_result, nullptr, opts);
  ...
  if (emel::model::llama::detail::build_execution_view(*state.model_data,
    ...
    if (!quantized_contract_matches(runtime_contract, audit_contract)) {
      std::fprintf(
          ...
          "runtime_native_quantized=%u "
          "runtime_approved_dense_f32_by_contract=%u "
          ...
          runtime_contract.approved_dense_f32_by_contract,
          runtime_contract.disallowed_fallback,
          runtime_contract.explicit_no_claim, audit_contract.native_quantized,
          audit_contract.approved_dense_f32_by_contract,
          audit_contract.disallowed_fallback, audit_contract.explicit_no_claim);
      dump_generation_failure_surface(state, &emel_result, nullptr, opts);
      return 1;
    }
  }
  ...
  const std::filesystem::path baseline_path =
      opts.write_generation_baseline_path.empty()
  ...
  if (!opts.write_generation_baseline_path.empty()) {
    generation_result baseline_result{};
    std::string_view baseline_source = {};
    if (fixture_uses_live_reference_generation(*fixture)) {
      const emel::error::type reference_generation_err =
          run_reference_generate(state, opts, baseline_result);
      if (reference_generation_err !=
          emel::error::cast(emel::generator::error::none)) {
        std::fprintf(stderr, "reference generation error (fixture=%s err=%s)\n",
                     fixture->name.data(),
                     generator_error_name(reference_generation_err));
        dump_generation_failure_surface(state, &emel_result, nullptr, opts);
        return 1;
      }
      baseline_source = k_reference_impl_source;
    } else {
      baseline_result = emel_result;
      baseline_source = k_generation_baseline_source;
    }

    if (!write_generation_baseline_file(baseline_path, *fixture, opts,
                                        baseline_result)) {
      std::fprintf(
          stderr, "generation baseline write failed (fixture=%s baseline=%s)\n",
          fixture->name.data(), baseline_path.string().c_str());
      dump_generation_failure_surface(state, &emel_result, nullptr, opts);
      return 1;
    }
    ...
    std::fprintf(
        ...
        "prompt_bytes=%zu max_tokens=%d generated_tokens=%d "
        "output_bytes=%zu)\n",
        fixture->name.data(), baseline_path.string().c_str(),
        static_cast<int>(k_generation_baseline_contract.size()),
        k_generation_baseline_contract.data(), opts.text.size(),
        opts.max_tokens, baseline_result.tokens_generated,
        baseline_result.output_length);
    print_generation_formatter_contract(stdout, state, *fixture);
    std::fprintf(
        stdout,
        "reference_impl: source=%.*s repo=%.*s ref=%.*s contract=%.*s "
        "baseline=%s\n",
        static_cast<int>(baseline_source.size()), baseline_source.data(),
        static_cast<int>(k_reference_impl_repository.size()),
        k_reference_impl_repository.data(),
        static_cast<int>(k_reference_impl_ref.size()),
        k_reference_impl_ref.data(),
        static_cast<int>(k_generation_baseline_contract.size()),
        k_generation_baseline_contract.data(), baseline_path.string().c_str());
    dump_reference_decode_seam(state);
    return 0;
  }

  generation_result reference_result{};
  if (fixture_uses_append_only_baseline(*fixture)) {
    ...
    if (baseline_record.fixture_name != fixture->name ||
      ...
      std::fprintf(
          ...
          "baseline_fixture=%s baseline_prompt_bytes=%zu "
          "baseline_max_tokens=%d)\n",
          fixture->name.data(), baseline_path.string().c_str(),
          baseline_record.fixture_name.c_str(), baseline_record.prompt.size(),
          baseline_record.max_tokens);
      dump_generation_failure_surface(state, &emel_result, nullptr, opts);
      return 1;
    }
    reference_result = baseline_record.result;
  } else if (fixture_uses_live_reference_generation(*fixture)) {
    const emel::error::type reference_generation_err =
        run_reference_generate(state, opts, reference_result);
    if (reference_generation_err !=
        emel::error::cast(emel::generator::error::none)) {
      std::fprintf(stderr, "reference generation error (fixture=%s err=%s)\n",
                   fixture->name.data(),
    ...
    std::fprintf(
        stderr,
        "generation parity contract unsupported (fixture=%s contract=%.*s)\n",
        fixture->name.data(),
        static_cast<int>(fixture->generation_parity_contract.size()),
        fixture->generation_parity_contract.data());
    dump_generation_failure_surface(state, &emel_result, nullptr, opts);
    return 1;
  }
  ...
  if (!generation_results_match(emel_result, reference_result)) {
    const size_t mismatch_offset =
        first_mismatch_offset(emel_result, reference_result);
    std::fprintf(stderr,
                 "generation parity mismatch (fixture=%s emel_tokens=%d "
                 "reference_tokens=%d "
                 "emel_bytes=%zu reference_bytes=%zu first_mismatch=%zu)\n",
                 fixture->name.data(), emel_result.tokens_generated,
                 reference_result.tokens_generated, emel_result.output_length,
                 reference_result.output_length, mismatch_offset);
    dump_generation_failure_surface(state, &emel_result, &reference_result,
                                    opts);
    return 1;
  }

  std::fprintf(stdout,
               "generation parity ok (fixture=%s prompt_bytes=%zu "
               "max_tokens=%d generated_tokens=%d "
               "output_bytes=%zu flash_dispatch_calls=%" PRIu64
               " optimized_flash_dispatch_calls=%" PRIu64
               " shared_flash_dispatch_calls=%" PRIu64 " text=%.*s)\n",
               fixture->name.data(), opts.text.size(), opts.max_tokens,
               emel_result.tokens_generated, emel_result.output_length,
               flash_dispatch_calls, optimized_flash_dispatch_calls,
               shared_flash_dispatch_calls,
  ...
  if (fixture_uses_append_only_baseline(*fixture)) {
    ...
    std::fprintf(stdout,
                 "reference_impl: source=%.*s repo=%.*s ref=%.*s "
                 "contract=live_reference_generation\n",
                 static_cast<int>(k_reference_impl_source.size()),
                 k_reference_impl_source.data(),
                 static_cast<int>(k_reference_impl_repository.size()),
                 k_reference_impl_repository.data(),
  ...
  if (opts.attribution) {
    ...
    if (attribution_err != emel::error::cast(emel::generator::error::none)) {
      std::fprintf(stderr,
                   ...
                   fixture->name.data(), generator_error_name(attribution_err));
      return 1;
    }
    if (!generation_results_match(attributed_result, reference_result)) {
      const size_t mismatch_offset =
          first_mismatch_offset(attributed_result, reference_result);
      const int32_t token_mismatch_index =
          first_token_mismatch_index(attributed_result, reference_result);
      const bool attributed_token_available =
          token_mismatch_index >= 0 &&
          token_mismatch_index < attributed_result.trace.token_count;
      const bool reference_token_available =
          token_mismatch_index >= 0 &&
          token_mismatch_index < reference_result.trace.token_count;
      const int32_t attributed_token =
          attributed_token_available
              ? attributed_result.trace
                    .token_ids[static_cast<size_t>(token_mismatch_index)]
              : -1;
      const int32_t reference_token =
          reference_token_available
              ? reference_result.trace
                    .token_ids[static_cast<size_t>(token_mismatch_index)]
              : -1;
      std::fprintf(
          ...
          "reference_tokens=%d first_mismatch=%zu first_token_mismatch=%d "
          "attributed_token=%d reference_token=%d "
          "attributed_output_bytes=%zu reference_output_bytes=%zu)\n",
          fixture->name.data(), attributed_result.tokens_generated,
          reference_result.tokens_generated, mismatch_offset,
          token_mismatch_index, attributed_token, reference_token,
          attributed_result.output_length, reference_result.output_length);
      return 1;
    }
