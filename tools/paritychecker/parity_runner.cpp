    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      ...
      for (int32_t dim = 0; dim < head_dim; ++dim) {
        acc[static_cast<size_t>(dim)] +=
            backend.value_cache[cache_offset + static_cast<size_t>(dim)] * weight;
      }
...
void dump_scalar_attention_debug(const generation_load_state & state,
  ...
  if (token_mismatch_index >= 299) {
    ...
    dump_generation_q23_stage_debug(state, opts, emel_result, reference_result);
        ...
        if (capture_reference_value_cache_rows(reference_ctx.get(), 0, reference_layer0_value_cache) &&
          ...
          if (token_index == 0u) {
            ...
            if (!emel::generator::detail::copy_tensor_row(
              ...
              const std::span<const float> exact_cache_row{
            ...
            if (!emel::generator::detail::copy_tensor_row(
              ...
              const std::span<const float> reference_q3_cache_row{
...
void dump_generation_q23_stage_debug(const generation_load_state & state,
  ...
  auto dump_stage = [&](const char * label,
                        ...
                        std::span<const float> q23_values) {
    std::fprintf(stdout,
  ...
  dump_stage("prompt0.layer0.key_cache",
             std::span<const float>(actor_backend.key_cache.data() + actor_cache_offset,
                                    static_cast<size_t>(kv_dim)),
             std::span<const float>(q23_backend.key_cache.data() + q23_cache_offset,
                                    static_cast<size_t>(kv_dim)));
  dump_stage("prompt0.layer0.value_cache",
             std::span<const float>(actor_backend.value_cache.data() + actor_cache_offset,
                                    static_cast<size_t>(kv_dim)),
             std::span<const float>(q23_backend.value_cache.data() + q23_cache_offset,
                                    static_cast<size_t>(kv_dim)));
  std::vector<float> reference_value_cache_rows;
  if (capture_reference_value_cache_rows(reference_ctx.get(), layer_index, reference_value_cache_rows) &&
    ...
    if (run_layer_with_matmul_mode_scalar_attention(
      ...
      dump_state_compare(
          ...
          std::span<const float>(q3_reference_backend.value_cache.data() + q3_reference_cache_offset,
                                 static_cast<size_t>(kv_dim)),
          reference_value_cache_row);
      dump_state_compare(
...
void dump_generation_gen0_attention_debug(const generation_load_state & state,
  ...
  dump_gen0_state("key_cache", std::span<const float>(backend.key_cache.data() + cache_offset,
                                                       static_cast<size_t>(kv_dim)),
                  "Kcur-0");
  dump_gen0_state("value_cache_state", std::span<const float>(backend.value_cache.data() + cache_offset,
                                                              static_cast<size_t>(kv_dim)),
                  "Vcur-0");
...
void dump_generation_target_attention_debug(const generation_load_state & state,
                                            ...
                                            const generation_result & reference_result) {
  const int32_t token_mismatch_index = first_token_mismatch_index(emel_result, reference_result);
  const int32_t target_generated_index = token_mismatch_index - 1;
  if (state.model_data == nullptr || token_mismatch_index <= 0 || target_generated_index < 0 ||
  ...
  auto dump_target_state = [&](const char * suffix,
                               std::span<const float> emel_values,
                               std::span<const float> reference_values) {
    std::fprintf(stdout,
                 "%s.%s: max_abs=%g\n",
...
void dump_generation_live_backend_prefix_debug(const generation_load_state & state,
  ...
  for (size_t token_index = 0; token_index < prefix_tokens.size(); ++token_index) {
    ...
    for (int32_t layer = 0; layer < dispatch_backend.n_layer; ++layer) {
      ...
      const std::span<const float> dispatch_key_cache{
          dispatch_backend.key_cache.data() + cache_offset,
          static_cast<size_t>(kv_dim),
      };
      const std::span<const float> shared_key_cache{
          shared_backend.key_cache.data() + cache_offset,
          static_cast<size_t>(kv_dim),
      };
      const std::span<const float> dispatch_value_cache{
          dispatch_backend.value_cache.data() + cache_offset,
          static_cast<size_t>(kv_dim),
      };
      const std::span<const float> shared_value_cache{
      ...
      if (const reference_tensor_capture * v_capture =
        ...
        const std::span<const float> reference_v = reference_token_row(
            std::span<const float>(v_capture->values),
            dispatch_backend.v.size(),
      ...
      if (const reference_tensor_capture * q_capture =
        ...
        const std::span<const float> reference_q = reference_token_row(
            std::span<const float>(q_capture->values),
            dispatch_backend.q.size(),
            static_cast<size_t>(token_index));
        std::fprintf(stdout,
      ...
      if (const reference_tensor_capture * k_capture =
        ...
        const std::span<const float> reference_k = reference_token_row(
            std::span<const float>(k_capture->values),
            dispatch_backend.k.size(),
            static_cast<size_t>(token_index));
        std::fprintf(stdout,
