...
std::filesystem::path maintained_generation_baseline_path(
  ...
  return parity_snapshot_dir() /
         ("generation_" + std::string(fixture.slug) +
...
bool run_paritychecker_process(const std::string &model,
                               const parity_case &test_case) {
  std::string command;
#if defined(_WIN32)
...
bool run_jinja_paritychecker_process(
...
std::filesystem::path make_temp_fixture_path(const char *stem,
...
process_capture run_generation_paritychecker_capture_with_args(
  ...
  const std::filesystem::path stdout_path =
      make_temp_capture_path("paritychecker-stdout");
  const std::filesystem::path stderr_path =
      make_temp_capture_path("paritychecker-stderr");
  std::string command;
#if defined(_WIN32)
  command = quote_arg_windows(paritychecker_binary_path().string());
...
#if !defined(_WIN32)
process_capture
run_paritychecker_capture_with_stdin(const std::vector<std::string> &args,
                                     const std::string &stdin_text) {
  const std::filesystem::path stdout_path =
      make_temp_capture_path("paritychecker-stdin-stdout");
  const std::filesystem::path stderr_path =
      make_temp_capture_path("paritychecker-stdin-stderr");
...
process_capture
run_generation_paritychecker_capture(const std::filesystem::path &model_path,
...
int parse_named_metric(const std::string &text, const std::string &key) {
  ...
  while (value_end < text.size() && text[value_end] >= '0' &&
         text[value_end] <= '9') {
    ++value_end;
  }
...
int parse_flash_dispatch_metric(const std::string &text,
                                const std::string &key) {
  const size_t line_pos = text.find("flash_dispatch:");
  if (line_pos == std::string::npos) {
    return -1;
...
void check_generation_quantized_stage_audit(const process_capture &capture) {
  CHECK(capture.stdout_text.find("quantized_runtime_contract:") !=
        std::string::npos);
  CHECK(capture.stdout_text.find("quantized_stage_inventory:") !=
        std::string::npos);
  CHECK(capture.stdout_text.find(
            "quantized_stage_audit: stage=token_embedding") !=
        std::string::npos);
  CHECK(capture.stdout_text.find("quantized_stage_audit: stage=attention_q") !=
        std::string::npos);
  CHECK(capture.stdout_text.find(
            "quantized_stage_audit: stage=attention_q_norm") !=
        std::string::npos);
  CHECK(capture.stdout_text.find(
            "quantized_stage_audit: stage=attention_k_norm") !=
        std::string::npos);
  CHECK(capture.stdout_text.find("approved_dense_f32_by_contract") !=
        std::string::npos);
  CHECK(parse_named_metric_on_line(
            capture.stdout_text,
            "quantized_runtime_contract:", "native_quantized") == 8);
  CHECK(parse_named_metric_on_line(capture.stdout_text,
                                   "quantized_runtime_contract:",
                                   "approved_dense_f32_by_contract") == 6);
  CHECK(parse_named_metric_on_line(
            capture.stdout_text,
            "quantized_runtime_contract:", "disallowed_fallback") == 0);
  CHECK(parse_named_metric_on_line(
            capture.stdout_text,
            "quantized_runtime_contract:", "explicit_no_claim") == 0);
  CHECK(parse_named_metric_on_line(
            capture.stdout_text,
            "quantized_stage_inventory:", "native_quantized") == 8);
  CHECK(parse_named_metric_on_line(capture.stdout_text,
                                   "quantized_stage_inventory:",
                                   "approved_dense_f32_by_contract") == 6);
  CHECK(parse_named_metric_on_line(
            capture.stdout_text,
            "quantized_stage_inventory:", "disallowed_fallback") == 0);
  CHECK(parse_named_metric_on_line(
            capture.stdout_text,
            "quantized_stage_inventory:", "explicit_no_claim") == 0);
}
...
TEST_CASE(
    "paritychecker help describes the maintained generation fixture contract") {
  const process_capture capture =
      run_generation_paritychecker_capture_with_args({"--help"});

  CHECK(capture.exit_code == 2);
  CHECK(capture.stdout_text.empty());
  CHECK(capture.stderr_text.find(
            "--generation mode requires --model one maintained fixture") !=
        std::string::npos);
  for (const auto &fixture : emel::tools::generation_fixture_registry::
           k_maintained_generation_fixtures) {
    CHECK(capture.stderr_text.find(std::string(fixture.fixture_rel)) !=
          std::string::npos);
  }
...
TEST_CASE("parity runner owns cli parsing and validation") {
  const process_capture no_args =
      run_generation_paritychecker_capture_with_args({});
  CHECK(no_args.exit_code == 2);
  CHECK(no_args.stdout_text.empty());
  CHECK(no_args.stderr_text.find("usage:") != std::string::npos);

  const process_capture invalid_combo =
      run_generation_paritychecker_capture_with_args({
          "--gbnf",
          "--text",
          "root ::= \"a\"",
  ...
  const std::string main_source = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "parity_main.cpp");
  ...
  const std::string runner_source = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "parity_runner.cpp");
  REQUIRE_FALSE(runner_source.empty());
  CHECK(runner_source.find("int run_parity_cli(int argc, char ** argv)") !=
        std::string::npos);
  CHECK(runner_source.find("bool parse_args(int argc, char ** argv") !=
        std::string::npos);
  CHECK(runner_source.find("void print_usage(const char * exe)") !=
        std::string::npos);
}
...
TEST_CASE("parity assets resolve maintained generation fixtures centrally") {
  ...
  for (const auto &fixture : emel::tools::generation_fixture_registry::
    ...
    const auto *resolved = emel::paritychecker::assets::find_generation_fixture(
        expected_path.string());
    REQUIRE(resolved != nullptr);
    CHECK(resolved->slug == fixture.slug);

    const auto impostor_path = make_temp_fixture_path(
        "paritychecker-fixture-impostor", std::string(fixture.name));
    CHECK(emel::paritychecker::assets::find_generation_fixture(
              impostor_path.string()) == nullptr);
    CHECK(fixture_list.find(std::string(fixture.fixture_rel)) !=
          std::string::npos);
  }
}

TEST_CASE("parity assets use public io source file loading") {
  const auto reference_ref =
      repo_root_dir() / "tools" / "paritychecker" / "reference_ref.txt";
  REQUIRE(emel::paritychecker::assets::file_exists(reference_ref.string()));

  std::vector<uint8_t> bytes;
  REQUIRE(emel::io::source::load_file_bytes(reference_ref.string(), bytes) ==
          emel::error::cast(emel::io::read::error::none));
  CHECK_FALSE(bytes.empty());

  const std::string source = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "parity_engines.cpp");
  REQUIRE_FALSE(source.empty());
  CHECK(source.find("#include \"parity_assets.hpp\"") != std::string::npos);
  CHECK(source.find("#include \"emel/io/source/any.hpp\"") !=
        std::string::npos);
  ...
  CHECK(source.find(
            "const maintained_generation_fixture * find_generation_fixture(") ==
        std::string::npos);
...
TEST_CASE("parity runner dispatches through explicit engine adapters") {
  const auto assert_engine = [](const emel::paritychecker::parity_mode mode,
  ...
  CHECK(runner_source.find("run_generation_harness_contract") ==
        std::string::npos);
...
TEST_CASE("paritychecker build registration keeps runner and engine sources "
          "modular") {
  const std::string cmake_source = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "CMakeLists.txt");
  REQUIRE_FALSE(cmake_source.empty());

  CHECK(cmake_source.find("set(PARITYCHECKER_RUNNER_SOURCES") !=
        std::string::npos);
  ...
  CHECK(cmake_source.find("set(PARITYCHECKER_REFERENCE_SUPPORT_SOURCES") !=
        std::string::npos);
  CHECK(cmake_source.find("set(PARITYCHECKER_COMMON_SOURCES") !=
        std::string::npos);
  ...
  const std::string engine_source = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "parity_engine.cpp");
...
TEST_CASE(
    "parity dependency manifest covers registered engines conservatively") {
  ...
  const auto assert_runner = [](const emel::paritychecker::parity_mode mode,
    ...
    for (const auto &record : runner_records) {
      ...
      has_config =
          has_config || record.kind == manifest::dependency_kind::config;
      has_fixture =
          has_fixture || record.kind == manifest::dependency_kind::fixture;
      has_model = has_model || record.kind == manifest::dependency_kind::model;
      has_script =
          has_script || record.kind == manifest::dependency_kind::script;
      has_snapshot =
          has_snapshot || record.kind == manifest::dependency_kind::snapshot;
    }
  ...
  assert_runner(emel::paritychecker::parity_mode::gbnf_parser, true, false,
                false);
  ...
  CHECK(
      manifest::records_for(static_cast<emel::paritychecker::parity_mode>(255))
          .empty());
}

TEST_CASE(
    "parity dependency manifest renders and writes a deterministic format") {
  namespace manifest = emel::paritychecker::dependency_manifest;
  ...
  CHECK(rendered.find("record runner=tokenizer mode=tokenizer kind=fixture "
                      "path=tests/text/tokenizer/parity_texts "
                      "reason=tokenizer_text_cases\n") != std::string::npos);
  ...
  const std::string cmake_source = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "CMakeLists.txt");
  REQUIRE_FALSE(cmake_source.empty());
  CHECK(cmake_source.find("set(PARITYCHECKER_MANIFEST_SOURCES") !=
        std::string::npos);
  CHECK(cmake_source.find("${PARITYCHECKER_MANIFEST_SOURCES}") !=
        std::string::npos);

  const std::string docs = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "dependency_manifest.md");
...
TEST_CASE(
    "paritychecker cli reports dependency manifest freshness conservatively") {
  namespace manifest = emel::paritychecker::dependency_manifest;
...
TEST_CASE(
    "quality gate escalates parity on dependency manifest freshness gaps") {
  namespace manifest = emel::paritychecker::dependency_manifest;

  const std::string baseline = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "dependency_manifest.txt");
  ...
  CHECK(gate_source.find("PARITY_DEPENDENCY_MANIFEST_BASELINE") !=
        std::string::npos);
  ...
  CHECK(gate_source.find("EMEL_PARITY_DEPENDENCY_MANIFEST_UNCERTAIN") !=
        std::string::npos);
...
TEST_CASE("paritychecker sources do not bridge into actor internals") {
  ...
  REQUIRE(!file_exists(paritychecker_dir / ("generation_internal_"
                                            "diagnostics.hpp")));

  const std::vector<std::string> forbidden_patterns = {
      "#include \"emel/text/generator/"
      "detail.hpp\"",
      "#include \"emel/text/generator/"
      "actions.hpp\"",
      "#include \"emel/text/generator/"
      "guards.hpp\"",
      "#include \"emel/text/generator/prefill/"
      "guards.hpp\"",
      "emel::text::generator::"
      "detail::",
      ...
      "emel::text::generator::"
      "guard::",
      "emel::text::generator::prefill::"
      "guard::",
      "#include \"emel/text/detokenizer/"
      "actions.hpp\"",
      "#include \"emel/text/jinja/parser/"
      "detail.hpp\"",
      "emel::text::detokenizer::"
      "action::detail::",
      "emel::text::jinja::parser::"
      "detail::",
      "->generation_",
      "generation_internal_"
      "diagnostics.hpp",
      "emel::gguf::loader::"
      "detail::",
      "emel::model::"
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(paritychecker_dir)) {
    const auto path = entry.path();
    if (!entry.is_regular_file() ||
        path.filename() == "paritychecker_tests.cpp") {
      continue;
    }
    ...
    while (line_begin <= source.size()) {
      ...
      if (line.find("#include \"emel/") != std::string::npos) {
        ...
        const bool approved_kernel_surface =
            line.find("#include \"emel/kernel/detail.hpp\"") !=
                std::string::npos ||
            line.find("#include \"emel/kernel/aarch64/detail.hpp\"") !=
                std::string::npos ||
            line.find("#include \"emel/kernel/x86_64/detail.hpp\"") !=
                std::string::npos;
        const bool include_allowed =
            !actor_internal_include || approved_kernel_surface;
        CHECK_MESSAGE(include_allowed,
                      path.filename().string()
                          << " contains actor-internal include " << line);
      }
...
TEST_CASE(
    "parity shared runner sources stay free of lane-owned runtime objects") {
  const auto paritychecker_dir = repo_root_dir() / "tools" / "paritychecker";
  ...
  for (const auto &path : shared_runner_sources) {
    ...
    for (const auto &pattern : forbidden) {
      CHECK_MESSAGE(source.find(pattern) == std::string::npos,
                    path.filename().string()
                        << " contains shared-lane pattern " << pattern);
    }
  }
}

TEST_CASE(
    "parity engine source keeps EMEL and reference lane objects separate") {
  const std::string source = read_text_file(
      repo_root_dir() / "tools" / "paritychecker" / "parity_engines.cpp");
  REQUIRE_FALSE(source.empty());
  ...
  CHECK(source.find(
        ...
        std::string::npos);
  CHECK(source.find(
            "load_emel_vocab_from_gguf_file(opts.model_path, *emel_vocab)") !=
        std::string::npos);
  CHECK(source.find("struct reference_backend") != std::string::npos);
  CHECK(source.find("llama_model_ptr model") != std::string::npos);
  CHECK(source.find("std::unique_ptr<emel::model::data> model_data") !=
        std::string::npos);
  CHECK(source.find("reference_backend reference") != std::string::npos);
  CHECK(source.find(
            "load_generation_reference_backend(opts.model_path, state)") !=
        std::string::npos);
  CHECK(source.find("state.model_loader.process_event(request)") !=
        std::string::npos);
  CHECK(source.find("run_emel_generate(state,") != std::string::npos);

  const size_t harness_pos =
      source.find("int run_generation_harness_contract(");
  ...
  const size_t parity_compare_pos = source.find(
      "generation_results_match(emel_result, reference_result)", harness_pos);
  const size_t baseline_load_pos = source.find(
      "load_generation_baseline_file(baseline_path, baseline_record)",
      harness_pos);
  ...
  for (const auto &pattern : forbidden) {
    CHECK_MESSAGE(source.find(pattern) == std::string::npos,
                  "parity_engines.cpp contains lane-sharing pattern "
                      << pattern);
  }
}

TEST_CASE(
    "paritychecker generation reports a deterministic missing-model failure") {
  ...
  CHECK(capture.stderr_text.find(
            "generation load failed: missing model file") != std::string::npos);
}

TEST_CASE("generation formatter contract classifier models supported and "
          "unsupported templates explicitly") {
  std::string supported_template = {};
  for (const std::string_view marker :
       emel::tools::generation_formatter_contract::
           k_supported_primary_template_markers) {
    supported_template.append(marker);
    supported_template.push_back('\n');
  }

  const auto supported = emel::tools::generation_formatter_contract::
      resolve_primary_template_binding(supported_template, 0u);
  CHECK(
      emel::tools::generation_formatter_contract::binding_supported(supported));
  ...
  CHECK(formatted_prompt == "<|startoftext|><|im_start|>user\nhello<|im_end|>"
                            "\n<|im_start|>assistant\n");

  std::string supported_qwen_template = {};
  for (const std::string_view marker :
       emel::tools::generation_formatter_contract::
           k_supported_qwen_primary_template_markers) {
    supported_qwen_template.append(marker);
    supported_qwen_template.push_back('\n');
  }

  const auto supported_qwen = emel::tools::generation_formatter_contract::
      resolve_primary_template_binding(supported_qwen_template, 0u);
  ...
  for (const std::string_view marker :
       emel::tools::generation_formatter_contract::
           k_supported_gemma4_primary_template_markers) {
    supported_gemma4_template.append(marker);
    supported_gemma4_template.push_back('\n');
  }

  const auto supported_gemma4 = emel::tools::generation_formatter_contract::
      resolve_primary_template_binding(supported_gemma4_template, 0u);
  CHECK(emel::tools::generation_formatter_contract::binding_supported(
      supported_gemma4));
  CHECK(
      supported_gemma4.contract ==
  ...
  const auto unsupported = emel::tools::generation_formatter_contract::
      resolve_primary_template_binding("{{ unsupported }}", 0u);
  CHECK_FALSE(emel::tools::generation_formatter_contract::binding_supported(
      unsupported));
  CHECK(unsupported.contract == emel::tools::generation_formatter_contract::
                                    k_unsupported_template_contract);

  const auto named_variant = emel::tools::generation_formatter_contract::
      resolve_primary_template_binding(supported_template, 1u);
  CHECK_FALSE(emel::tools::generation_formatter_contract::binding_supported(
      named_variant));
  CHECK(named_variant.contract == emel::tools::generation_formatter_contract::
                                      k_unsupported_template_contract);
}

TEST_CASE("paritychecker generation keeps append-only maintained baselines for "
          "supported fixtures") {
  for (const auto &fixture : emel::tools::generation_fixture_registry::
           k_maintained_generation_fixtures) {
...
TEST_CASE("paritychecker matches current maintained generation publication "
          "against live reference") {
  for (const auto &fixture : emel::tools::generation_fixture_registry::
           k_maintained_generation_fixtures) {
    ...
    const std::filesystem::path model_path =
        maintained_generation_fixture_path(fixture);
    INFO("fixture: " << fixture.name);
    ...
    CHECK(capture.exit_code == 0);
    CHECK(capture.stderr_text.empty());
    CHECK(capture.stdout_text.find("generation parity ok") !=
          std::string::npos);
    CHECK(capture.stdout_text.find(std::string("fixture=") +
    ...
    CHECK(parse_named_metric_on_line(
              capture.stdout_text,
              "reference_decode_seams:", "reference_decode_calls") > 0);
  }
}
...
TEST_CASE("paritychecker maintained generation fixtures reject same-basename "
          "files outside tests/models") {
  for (const auto &fixture : emel::tools::generation_fixture_registry::
           k_maintained_generation_fixtures) {
    const std::filesystem::path impostor_model_path = make_temp_fixture_path(
        "paritychecker-fixture", std::string(fixture.name));
    {
    ...
    const process_capture capture =
        run_generation_paritychecker_capture(impostor_model_path, "hello");
    ...
    CHECK(capture.stdout_text.find("generation parity ok") ==
          std::string::npos);
    CHECK(capture.stderr_text.find(
              "generation requires maintained fixture path") !=
          std::string::npos);
