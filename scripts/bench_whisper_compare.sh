...
usage() {
  cat <<'USAGE'
usage: scripts/bench_whisper_compare.sh [--output-dir DIR] [--skip-reference-build]
...
for tool in cmake ninja python3 shasum; do
...
expected_tokenizer_sha256="${EMEL_WHISPER_TOKENIZER_MODEL_SHA256:-dfc530298b6fbed1a97c6472c575b026453706e2a204c7f7038f2c9d208b0759}"

actual_tokenizer_sha256="$(shasum -a 256 "$tokenizer_model" | awk '{print $1}')"
if [[ "$actual_tokenizer_sha256" != "$expected_tokenizer_sha256" ]]; then
  echo "error: tokenizer sha256 mismatch: $actual_tokenizer_sha256 != $expected_tokenizer_sha256" >&2
  exit 1
fi

if [[ -z "$emel_model" ]]; then
  emel_model="$model_path"
  rm -rf "$OUTPUT_DIR/normalized"
fi

python3 "$ROOT_DIR/tools/bench/whisper_compare.py" \
