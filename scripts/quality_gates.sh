...
NEEDLE_EVAL_BINARY="${EMEL_NEEDLE_EVAL_BINARY:-$ROOT_DIR/build/zig/needle_eval/emel_needle_eval}"
...
emel_quality_gate_os() {
  printf '%s\n' "${EMEL_MEMORY_TEST_OS:-$(uname -s)}"
}

emel_linux_current_limits() {
  if [[ -n "${EMEL_MEMORY_TEST_CURRENT_MAX:-}" ||
        -n "${EMEL_MEMORY_TEST_CURRENT_SWAP:-}" ]]; then
    printf '%s %s\n' "${EMEL_MEMORY_TEST_CURRENT_MAX:-max}" \
      "${EMEL_MEMORY_TEST_CURRENT_SWAP:-max}"
    return
  fi
  local cgroup_dir current_max current_swap
  cgroup_dir="$(emel_cgroup_v2_dir)" || return 1
  [[ -r "$cgroup_dir/memory.max" && -r "$cgroup_dir/memory.swap.max" ]] || return 1
  IFS= read -r current_max <"$cgroup_dir/memory.max"
  IFS= read -r current_swap <"$cgroup_dir/memory.swap.max"
  printf '%s %s\n' "$current_max" "$current_swap"
}

emel_verify_linux_envelope() {
  local current_max current_swap
  read -r current_max current_swap < <(emel_linux_current_limits) || {
    emel_envelope_error "entered Linux memory envelope but cannot read its cgroup v2 limits"
    return 1
  }
  emel_is_uint "$current_max" && ((current_max <= EMEL_MEMORY_CAP_BYTES)) || {
    emel_envelope_error "memory envelope verification failed: memory.max=$current_max, required <=$EMEL_MEMORY_CAP_BYTES"
    return 1
  }
  [[ "$current_swap" == "0" ]] || {
    emel_envelope_error "memory envelope verification failed: memory.swap.max=$current_swap, required 0"
    return 1
  }
}

emel_systemd_scope_command() {
  printf 'systemd-run --user --scope --quiet --property=MemoryMax=%s --property=MemorySwapMax=0 env EMEL_MEMORY_ENVELOPE_ACTIVE=1 EMEL_MEMORY_ENVELOPE_KIND=systemd EMEL_MEMORY_BASE_TOTAL_BYTES=%s %q' \
    "$EMEL_MEMORY_CAP_BYTES" "$EMEL_MEMORY_BASE_TOTAL_BYTES" "$0"
  local arg
  for arg in "$@"; do
    printf ' %q' "$arg"
  done
...
emel_systemd_user_available() {
  command -v systemd-run >/dev/null 2>&1 &&
    command -v systemctl >/dev/null 2>&1 &&
    systemctl --user show-environment >/dev/null 2>&1 &&
    systemd-run --user --scope --quiet \
      --property="MemoryMax=$EMEL_MEMORY_CAP_BYTES" \
      --property=MemorySwapMax=0 true >/dev/null 2>&1
}

emel_run_systemd_scope() {
  exec systemd-run --user --scope --quiet \
    --property="MemoryMax=$EMEL_MEMORY_CAP_BYTES" \
    --property=MemorySwapMax=0 \
    env EMEL_MEMORY_ENVELOPE_ACTIVE=1 \
    EMEL_MEMORY_ENVELOPE_KIND=systemd \
    EMEL_MEMORY_BASE_TOTAL_BYTES="$EMEL_MEMORY_BASE_TOTAL_BYTES" \
    "$0" "$@"
}

emel_cgroup_wrapper() {
  local parent_dir scope_dir ready_file child status
  parent_dir="$(emel_cgroup_v2_dir)" || return 125
  [[ -w "$parent_dir/cgroup.procs" ]] || return 125
  scope_dir="$parent_dir/emel-quality-gates-$$"
  mkdir "$scope_dir" 2>/dev/null || return 125
  ready_file="${TMPDIR:-/tmp}/emel-quality-gates-cgroup-$$.ready"
  rm -f "$ready_file"
  if ! printf '%s\n' "$EMEL_MEMORY_CAP_BYTES" >"$scope_dir/memory.max" ||
     ! printf '0\n' >"$scope_dir/memory.swap.max"; then
    rmdir "$scope_dir" 2>/dev/null || true
    return 125
  fi

  env EMEL_MEMORY_ENVELOPE_ACTIVE=1 \
    EMEL_MEMORY_ENVELOPE_KIND=delegated-cgroup \
    EMEL_MEMORY_BASE_TOTAL_BYTES="$EMEL_MEMORY_BASE_TOTAL_BYTES" \
    EMEL_MEMORY_CGROUP_READY_FILE="$ready_file" \
    "$0" "$@" &
  child=$!
  if ! printf '%s\n' "$child" >"$scope_dir/cgroup.procs"; then
  ...
  awk -v root="$root_pid" '
    ...
    END {
      ...
      for (pass = 1; pass <= NR; ++pass)
        for (i = 1; i <= NR; ++i)
          if (member[ppid[i]]) member[pid[i]] = 1
      total = 0
      for (i = 1; i <= NR; ++i)
        if (member[pid[i]]) total += rss[i]
      printf "%.0f\n", total
    }
  '
}

emel_process_tree_pids() {
  local root_pid="$1"
  awk -v root="$root_pid" '
    { pid[NR] = $1; ppid[NR] = $2 }
    END {
      ...
      for (pass = 1; pass <= NR; ++pass)
        for (i = 1; i <= NR; ++i)
          if (member[ppid[i]]) member[pid[i]] = 1
      for (i = NR; i >= 1; --i)
        if (member[pid[i]]) printf "%s ", pid[i]
      printf "\n"
    }
  '
}

emel_darwin_watchdog_command() {
  printf 'Darwin watchdog: ps -axo pid=,ppid=,rss=; root=%s; kill_at_bytes=%s; interval_seconds=%s\n' \
    '${child_pid}' "$((EMEL_MEMORY_CAP_BYTES * 4 / 5))" "${EMEL_MEMORY_WATCH_INTERVAL_SECONDS:-0.01}"
}

emel_kill_process_tree() {
  local root_pid="$1" pids
  pids="$(ps -axo pid=,ppid=,rss= | emel_process_tree_pids "$root_pid")"
  [[ -n "${pids// /}" ]] || pids="$root_pid"
...
emel_darwin_watchdog_wrapper() {
  local ready_file child status rss_kib kill_at_kib interval
  for tool in ps awk kill sleep; do
    command -v "$tool" >/dev/null 2>&1 || return 125
  done
  interval="${EMEL_MEMORY_WATCH_INTERVAL_SECONDS:-0.01}"
  [[ "$interval" =~ ^0\.[0-9]+$ ]] || {
    emel_envelope_error "EMEL_MEMORY_WATCH_INTERVAL_SECONDS must be a fractional second"
    return 125
  }
  ready_file="${TMPDIR:-/tmp}/emel-quality-gates-watchdog-$$.ready"
  rm -f "$ready_file"
  env EMEL_MEMORY_ENVELOPE_ACTIVE=1 \
    EMEL_MEMORY_ENVELOPE_KIND=darwin-rss-watchdog \
    EMEL_MEMORY_BASE_TOTAL_BYTES="$EMEL_MEMORY_BASE_TOTAL_BYTES" \
    EMEL_MEMORY_WATCHDOG_READY_FILE="$ready_file" \
    "$0" "$@" &
  child=$!
  kill_at_kib=$((EMEL_MEMORY_CAP_BYTES * 4 / 5 / 1024))
  : >"$ready_file"
  status=0
  while kill -0 "$child" 2>/dev/null; do
    rss_kib="$(ps -axo pid=,ppid=,rss= | emel_process_tree_rss_kib "$child")" || {
      emel_envelope_error "Darwin memory watchdog could not account for the gate process tree"
      emel_kill_process_tree "$child"
      wait "$child" 2>/dev/null || true
      rm -f "$ready_file"
      return 125
    }
    if ! emel_is_uint "$rss_kib"; then
      emel_envelope_error "Darwin memory watchdog received invalid RSS accounting: $rss_kib"
      emel_kill_process_tree "$child"
      wait "$child" 2>/dev/null || true
      rm -f "$ready_file"
      return 125
    fi
    if ((rss_kib >= kill_at_kib)); then
      emel_envelope_error "quality gate process tree reached watchdog limit $((kill_at_kib * 1024)) bytes (80% of $EMEL_MEMORY_CAP_BYTES-byte hard ceiling); terminating with burst headroom before the ceiling"
      emel_kill_process_tree "$child"
      wait "$child" 2>/dev/null || true
      rm -f "$ready_file"
      return 137
...
emel_enter_memory_envelope() {
  local os status
  if [[ "${EMEL_MEMORY_TEST_FORCE_ENVELOPE_FAILURE:-0}" == "1" ]]; then
    if [[ "${EMEL_ALLOW_UNCAPPED_MEMORY:-0}" == "1" ]]; then
      printf 'warning: proceeding without a memory envelope because EMEL_ALLOW_UNCAPPED_MEMORY=1\n' >&2
      return
    fi
    emel_envelope_error "could not install the $EMEL_MEMORY_CAP_BYTES-byte process-tree memory envelope; enable user systemd/cgroup delegation or set EMEL_ALLOW_UNCAPPED_MEMORY=1 to accept OOM risk explicitly"
    return 1
  fi
  if [[ "${EMEL_MEMORY_ENVELOPE_ACTIVE:-0}" == "1" ]]; then
    if [[ -n "${EMEL_MEMORY_CGROUP_READY_FILE:-}" ]]; then
      while [[ ! -e "$EMEL_MEMORY_CGROUP_READY_FILE" ]]; do sleep 0.01; done
    fi
    if [[ -n "${EMEL_MEMORY_WATCHDOG_READY_FILE:-}" ]]; then
      while [[ ! -e "$EMEL_MEMORY_WATCHDOG_READY_FILE" ]]; do sleep 0.01; done
    fi
    case "${EMEL_MEMORY_ENVELOPE_KIND:-}" in
      systemd|delegated-cgroup) emel_verify_linux_envelope ;;
      darwin-rss-watchdog) ;;
      *) emel_envelope_error "invalid memory envelope recursion marker"; return 1 ;;
    esac
    return
  fi

  os="$(emel_quality_gate_os)"
  if [[ "${EMEL_MEMORY_ENVELOPE_DRY_RUN:-0}" == "1" ]]; then
    case "$os" in
      Linux) emel_systemd_scope_command "$@" ;;
      Darwin) emel_darwin_watchdog_command ;;
      *) emel_envelope_error "no memory envelope implementation for $os"; return 1 ;;
    esac
    return 2
  fi
  ...
  case "$os" in
    Linux)
      if emel_systemd_user_available; then
        emel_run_systemd_scope "$@"
      fi
      status=0
      emel_cgroup_wrapper "$@" || status=$?
      if ((status != 125)); then
        exit "$status"
      fi
      ;;
    Darwin)
      status=0
      emel_darwin_watchdog_wrapper "$@" || status=$?
      if ((status != 125)); then
        exit "$status"
      fi
      ;;
    *) emel_envelope_error "no memory envelope implementation for $os" ;;
  esac
...
if [[ "${1:-}" == "--memory-cap-check" ]]; then
  ...
  printf 'envelope_active=%s\n' "${EMEL_MEMORY_ENVELOPE_ACTIVE:-0}"
  printf 'envelope_kind=%s\n' "${EMEL_MEMORY_ENVELOPE_KIND:-none}"
  if [[ "${EMEL_MEMORY_ENVELOPE_ACTIVE:-0}" != "1" ]]; then
    EMEL_MEMORY_ENVELOPE_DRY_RUN=1 emel_enter_memory_envelope "$@" || [[ $? -eq 2 ]]
  fi
  exit 0
fi

if [[ "${1:-}" == "--memory-cap-accounting-check" ]]; then
  [[ "${2:-}" =~ ^[0-9]+$ ]] || {
    emel_envelope_error "--memory-cap-accounting-check requires a numeric root pid"
    exit 1
  }
  emel_process_tree_rss_kib "$2"
  exit 0
fi

if [[ "${1:-}" == "--memory-cap-enforce-check" ]]; then
  shift
  emel_enter_memory_envelope --memory-cap-enforce-check "$@"
  printf 'uncapped_override=1\n'
  exit 0
fi
...
if [[ "${1:-}" == "--memory-cap-run" ]]; then
  shift
  (($# > 0)) || {
    emel_envelope_error "--memory-cap-run requires a command"
    exit 1
  }
  emel_enter_memory_envelope --memory-cap-run "$@"
  exec "$@"
fi
