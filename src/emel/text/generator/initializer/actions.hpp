...
struct begin_initialize {
  void operator()(const event::run & ev, context & ctx) const noexcept {
    ...
    generator.buffers.seq_primary_ids[0] = emel::text::generator::action::k_sequence_id;

    emel::text::generator::action::capture_renderer_session(ev.request, generator);
    emel::text::generator::action::reset_generation_tensor_epochs(generator);
...
struct request_renderer_initialize {
  void operator()(const event::run & ev, context & ctx) const noexcept {
    ev.ctx.phase_code = 0;
    ev.ctx.phase_accepted =
        emel::text::generator::action::dispatch_renderer_initialize(ctx.generator, ev.ctx.phase_code);
  }
};
...
struct request_graph_reserve {
  void operator()(const event::run & ev, context & ctx) const noexcept {
    ...
    const auto * lifecycle = emel::text::generator::detail::reserve_lifecycle(
        generator.compute.backend,
    ...
    const auto on_done =
        emel::callback<bool(const emel::graph::events::reserve_done &)>::from<
            emel::text::generator::action::capture_graph_reserve_done>();
    const auto on_error =
        emel::callback<bool(const emel::graph::events::reserve_error &)>::from<
            emel::text::generator::event::initialize_ctx,
            emel::text::generator::action::capture_graph_reserve_error>(&ev.ctx);
...
struct configure_preselected_argmax {
  void operator()(const event::run & ev, context & ctx) const noexcept {
    ...
    ev.ctx.phase_accepted = ev.ctx.buffers_ready;
    ev.ctx.phase_code =
        static_cast<int32_t>(emel::error::cast(emel::text::generator::error::none));
  }
};
