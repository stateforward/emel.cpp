...
struct block_weights {
  emel::model::generation_residual_route residual_route =
      emel::model::generation_residual_route::attention;
  emel::model::generation_attention_qk_norm_route qk_norm_route =
      emel::model::generation_attention_qk_norm_route::none;
  emel::model::generation_attention_value_route value_route =
      emel::model::generation_attention_value_route::dedicated_value;
  emel::model::generation_attention_v_norm_route v_norm_route =
      emel::model::generation_attention_v_norm_route::none;
  emel::model::generation_attention_window_route window_route =
      emel::model::generation_attention_window_route::full_context;
...
using attention_value_route = emel::model::generation_attention_value_route;
using attention_v_norm_route = emel::model::generation_attention_v_norm_route;
using attention_window_route = emel::model::generation_attention_window_route;
...
struct stream_binding {
  ...
  std::vector<
      std::array<const emel::model::data::tensor_record *, k_stream_role_count>>
      pristine = {};
  std::vector<
      std::array<const emel::model::data::tensor_record *, k_stream_role_count>>
      raw = {};
...
struct native_backend {
  ...
  emel::model::generation::topology topology = {};
  ...
  emel::model::generation::quantized_path_audit quantized_audit = {};
...
inline size_t row_storage_bytes(const tensor_record &tensor,
  ...
  return emel::kernel::detail::row_storage_bytes_for_dtype(
      static_cast<uint8_t>(tensor.type), static_cast<uint64_t>(cols));
}
...
inline int32_t effective_attention_kv_dim(const native_backend &backend,
                                          const block_weights &block) noexcept;
inline int32_t
effective_attention_head_dim(const native_backend &backend,
                             const block_weights &block) noexcept;
inline int32_t
effective_attention_head_dim_kv(const native_backend &backend,
                                const block_weights &block) noexcept;
inline int32_t
effective_attention_rope_dim(const native_backend &backend,
                             const block_weights &block) noexcept;
inline float
effective_attention_rope_freq_base(const native_backend &backend,
                                   const block_weights &block) noexcept;
...
inline bool requires_attention_qk_norm(const native_backend &backend,
                                       const block_weights &block) noexcept {
  (void)backend;
  return block.residual_route == residual_route::attention &&
         block.qk_norm_route ==
             emel::model::generation_attention_qk_norm_route::headwise_rms;
}
...
effective_attention_rope_freq_base(const native_backend &backend,
  ...
  return backend.rope_freq_base > 0.0f ? backend.rope_freq_base
                                       : block.attention_rope_freq_base;
}

inline int32_t effective_max_q_dim(const native_backend &backend) noexcept {
  return backend.max_q_dim > 0 ? backend.max_q_dim
                               : backend.n_head * backend.head_dim;
}

inline int32_t effective_max_kv_dim(const native_backend &backend) noexcept {
  return backend.max_kv_dim > 0 ? backend.max_kv_dim
...
first_attention_block(const native_backend &backend) noexcept {
  for (const auto &block : backend.blocks) {
    if (block.residual_route == residual_route::attention) {
      return &block;
    }
  }
...
inline kv_addressing_view
kv_addressing_from_snapshot(const emel::memory::view::snapshot &snapshot,
...
inline kv_addressing_view kv_addressing_from_request(
  ...
  return kv_addressing_from_snapshot(*request.memory_view,
                                     request.seq_primary_ids[0]);
}
...
inline size_t shortconv_state_layer_offset(const native_backend &backend,
                                           const int32_t layer_index) noexcept {
  return shortconv_state_layer_offset(backend, identity_kv_addressing(),
                                      layer_index);
}
...
inline const tensor_record *select_output_projection_tensor(
    const emel::model::generation::execution_view &execution) noexcept {
...
inline bool apply_rms_norm_in_place(std::span<float> vector,
                                    const float epsilon) noexcept {
  if (vector.empty()) {
    return false;
  }
  ...
  const float mean =
      static_cast<float>(square_sum / static_cast<double>(vector.size()));
...
inline bool apply_attention_qk_norm(native_backend &backend,
  ...
  auto q = std::span<float>(
      backend.q.data(),
      static_cast<size_t>(effective_attention_q_dim(backend, block)));
  auto k = std::span<float>(
      backend.k.data(),
      static_cast<size_t>(effective_attention_kv_dim(backend, block)));
  return apply_headwise_rms_norm(q, block.attention_q_norm, backend.n_head,
                                 effective_attention_head_dim(backend, block),
                                 backend.rms_epsilon) &&
         apply_headwise_rms_norm(
             k, block.attention_k_norm, backend.n_head_kv,
             effective_attention_head_dim_kv(backend, block),
             backend.rms_epsilon);
}

inline bool
apply_attention_qk_norm_contract(native_backend &backend,
                                 const block_weights &block) noexcept {
...
inline bool requires_attention_v_norm(const native_backend &backend,
  ...
  return block.residual_route == residual_route::attention &&
         block.v_norm_route ==
             emel::model::generation_attention_v_norm_route::rms;
}
...
inline bool prepare_block_native_matrices(native_backend &backend) noexcept {
  for (auto &block : backend.blocks) {
    const bool residual_ok =
        block.residual_route == residual_route::attention
...
inline const emel::graph::processor::event::lifecycle_manifest *
prefill_lifecycle(native_backend &backend, int32_t *input_tokens,
                  const int32_t input_token_capacity, int32_t *positions,
                  const int32_t position_capacity, float *logits,
                  const int32_t logits_capacity) noexcept {
  bind_runtime_lifecycle(backend, input_tokens, input_token_capacity, positions,
...
inline const emel::graph::processor::event::lifecycle_manifest *
decode_lifecycle(native_backend &backend, int32_t *input_tokens,
                 const int32_t input_token_capacity, int32_t *positions,
                 const int32_t position_capacity, float *logits,
                 const int32_t logits_capacity) noexcept {
  bind_runtime_lifecycle(backend, input_tokens, input_token_capacity, positions,
...
template <matmul_lane_mode lanes = matmul_lane_mode::serial>
inline bool
compute_mul_mat(native_backend &backend,
                const emel::kernel::event::op_mul_mat &ev) noexcept {
...
inline bool matmul_vector_packed_q8_0(native_backend &backend,
  ...
  return prepare_packed_q8_0_input(backend, input) &&
         matmul_vector_prepared_packed_q8_0_input<lanes>(backend, matrix,
                                                         matrix.cols, output);
}
...
inline bool matmul_vector_q8_k(native_backend &backend,
  ...
  return quantize_vector_q8_k(input, q8_input) &&
         matmul_vector_q8_input<lanes>(backend, matrix, q8_input, matrix.cols,
                                       output);
}
...
inline bool matmul_vector_routed(native_backend &backend,
  ...
  if constexpr (route == scalar_matmul_route::packed_q8_0) {
    ...
    return matmul_vector_native_quantized<lanes>(backend, matrix, input,
                                                 output);
  } else {
    return matmul_vector<lanes>(backend, matrix, input, output);
  }
...
inline bool matmul_chunk4_prepared(native_backend &backend,
                                   ...
                                   std::span<float> output) noexcept {
  if constexpr (route == chunk4_rhs_route::packed_q8_0) {
    return matmul_chunk4_prepared_packed_q8_0_input<lanes>(backend, matrix,
                                                           input_cols, output);
  } else {
...
inline void apply_rope(std::span<float> vector, const int32_t head_count,
                       ...
                       const float rope_freq_base) noexcept {
  apply_rope_pairing(vector, normal_rope_pairing(), head_count, head_dim, n_rot,
                     position, rope_freq_base);
}
...
inline void apply_attention_rope(std::span<float> vector,
                                 ...
                                 const int32_t position,
                                 const float rope_freq_base) noexcept {
  apply_rope_pairing(vector, block.attention_rope_pairing, head_count, head_dim,
                     n_rot, position, rope_freq_base);
}
...
inline size_t physical_kv_position(const kv_addressing_view &kv,
  ...
  const int32_t block_id =
      kv.blocks[static_cast<size_t>(position / kv.block_tokens)];
  return static_cast<size_t>(block_id * kv.block_tokens +
                             (position % kv.block_tokens));
}
...
inline size_t layer_cache_offset(const native_backend &backend,
  ...
  if (backend.layer_cache_offsets.size() !=
      static_cast<size_t>(backend.n_layer)) {
...
inline size_t layer_cache_offset(const native_backend &backend,
                                 ...
                                 const int32_t layer,
                                 const int32_t position) noexcept {
  return layer_cache_offset(backend, identity_kv_addressing(), block, layer,
                            position);
}

inline size_t flash_layer_cache_layer_offset(const native_backend &backend,
                                             const int32_t layer) noexcept {
  if (backend.flash_layer_cache_offsets.size() !=
      static_cast<size_t>(backend.n_layer)) {
...
inline size_t flash_layer_cache_head_position_offset(
    const native_backend &backend, const kv_addressing_view &kv,
    const block_weights &block, const int32_t layer, const int32_t kv_head,
    const int32_t position) noexcept {
...
inline size_t flash_layer_cache_head_position_offset(
    const native_backend &backend, const block_weights &block,
...
store_bound_request(native_backend &backend,
                    ...
                    int32_t *err_out) noexcept {
  auto *io =
      static_cast<emel::text::generator::compute_io *>(request.compute_ctx);
  ...
  for (int32_t head = 0; head < head_count; ++head) {
    ...
    for (int32_t position = 0; position < position_limit; ++position) {
      const size_t cache_offset =
          layer_cache_offset(backend, kv, block, layer_index, position) +
          kv_offset;
      const float score =
...
make_flash_attn_request(const native_backend &backend,
                        const block_weights &block, const int32_t layer_index,
                        const int32_t position) noexcept {
  ...
  const uint64_t head_dim =
      static_cast<uint64_t>(effective_attention_head_dim(backend, block));
  ...
  const size_t layer_offset =
      flash_layer_cache_layer_offset(backend, layer_index);
  ...
  const float *q_data =
      backend.q.size() >= q_dim
          ? backend.q.data()
          : (backend.q_attn.size() >= q_dim ? backend.q_attn.data() : nullptr);
  auto attn_ctx =
      std::span<float>(const_cast<float *>(backend.attn_ctx.data()), q_dim);

  request.src0 = make_src_view_3d(const_cast<float *>(q_data),
  ...
  request.src1 = make_src_view_strided_3d(
      ...
      sizeof(uint16_t) * static_cast<uint64_t>(backend.kv_positions_capacity) *
          kv_head_dim);
  request.src2 = make_src_view_strided_3d(
      ...
      sizeof(uint16_t) * static_cast<uint64_t>(backend.kv_positions_capacity) *
          kv_head_dim);
  request.dst = make_dst_view_3d(attn_ctx.data(), head_dim, 1u, head_count);
...
inline bool dispatch_flash_attention(native_backend &backend,
                                     ...
                                     const int32_t position) noexcept {
  const auto request =
      make_flash_attn_request(backend, block, layer_index, position);
  backend.kernel.set_kind(backend.kernel_kind);
...
inline bool store_attention_kv_cache(native_backend &backend,
  ...
  for (int32_t kv_head = 0; kv_head < backend.n_head_kv; ++kv_head) {
    ...
    store_fp16_rounded_cache(
        k_vector.subspan(src_offset,
                         static_cast<size_t>(
                             effective_attention_head_dim_kv(backend, block))),
        backend.flash_key_cache.data() + flash_cache_offset);
    store_fp16_rounded_cache(
        v_vector.subspan(src_offset,
                         static_cast<size_t>(
                             effective_attention_head_dim_kv(backend, block))),
...
inline bool copy_attention_kv_block(native_backend &backend,
  ...
  if (block_tokens <= 0 || block_tokens != backend.kv_block_tokens ||
      src_start < 0 || dst_start < 0 ||
      src_end > backend.kv_positions_capacity ||
      dst_end > backend.kv_positions_capacity ||
      backend.layer_cache_offsets.size() !=
          static_cast<size_t>(backend.n_layer) ||
      backend.flash_layer_cache_offsets.size() !=
          static_cast<size_t>(backend.n_layer) ||
      backend.blocks.size() != static_cast<size_t>(backend.n_layer)) {
    if (err_out != nullptr) {
      *err_out = k_error_invalid;
  ...
  for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
    ...
    std::copy_n(backend.key_cache.data() + src_offset, row_width,
                backend.key_cache.data() + dst_offset);
    std::copy_n(backend.value_cache.data() + src_offset, row_width,
    ...
    for (int32_t kv_head = 0; kv_head < backend.n_head_kv; ++kv_head) {
      ...
      if (flash_src_offset + flash_row_width > backend.flash_key_cache.size() ||
          ...
          flash_src_offset + flash_row_width >
              backend.flash_value_cache.size() ||
          flash_dst_offset + flash_row_width >
              backend.flash_value_cache.size()) {
        if (err_out != nullptr) {
          *err_out = k_error_invalid;
        }
      ...
      std::copy_n(backend.flash_key_cache.data() + flash_src_offset,
                  flash_row_width,
                  backend.flash_key_cache.data() + flash_dst_offset);
      std::copy_n(backend.flash_value_cache.data() + flash_src_offset,
                  flash_row_width,
...
inline bool run_attention_for_q_vector(
    native_backend &backend, const kv_addressing_view &kv,
    const block_weights &block, const int32_t layer_index,
    const int32_t position, std::span<const float> q_vector) noexcept {
  if constexpr (mode == emel::text::generator::attention_mode::flash) {
    ...
    return compute_attention(backend, kv, block, layer_index, position + 1,
                             q_attn);
  }
}

template <emel::text::generator::attention_mode mode>
inline bool run_attention(native_backend &backend, const kv_addressing_view &kv,
                          const block_weights &block, const int32_t layer_index,
                          const int32_t position) noexcept {
  return run_attention_for_q_vector<mode>(
      backend, kv, block, layer_index, position,
...
inline bool run_shortconv_block(native_backend &backend,
                                ...
                                const block_weights &block,
                                const int32_t layer_index) noexcept {
  // Branch-only coverage exclusion: this data-plane wrapper is instantiated
  // for every scalar route, so gcovr counts the same validation and span
  // instrumentation branches once per route. Success and rejection remain
  // line-covered by generator detail tests. GCOVR_EXCL_BR_START
  ...
  if (!matmul_vector_routed<route, lanes>(backend, block.shortconv_in_proj,
                                          backend.norm,
  ...
  const size_t layer_offset =
      shortconv_state_layer_offset(backend, kv, layer_index);
  ...
  if (!matmul_vector_routed<route, lanes>(backend, block.shortconv_out_proj,
                                          backend.shortconv_conv_out,
                                          backend.projected)) {
    return false;
  }
...
inline bool run_shortconv_block(native_backend &backend,
                                const block_weights &block,
                                const int32_t layer_index) noexcept {
  return run_shortconv_block<route, lanes>(backend, identity_kv_addressing(),
                                           block, layer_index);
}
...
inline bool bind_streamed_block_views(
    native_backend &backend, const int32_t layer_index,
    const uint8_t *slot_base,
    const emel::model::tensor::window::detail::layer_descriptor
        &layout) noexcept {
  ...
  const auto rebind = [&](tensor_matrix &matrix,
                          const size_t role) noexcept -> bool {
    if (raw[role] == nullptr) {
      return true;
    }
    ...
    stream.records[role].data =
        slot_base + layout.weights[extent_index].slot_offset;
...
inline void reset_stream_block_views(native_backend &backend) noexcept {
  ...
  for (size_t layer = 0; layer < layer_count; ++layer) {
    ...
    const auto restore = [&](tensor_matrix &matrix,
...
struct stream_acquire_capture {
  ...
  static void
  on_done(void *object,
          const emel::model::tensor::window::events::acquire_layer_window_done
              &ev) noexcept {
  ...
  static void
  on_error(void *,
           const emel::model::tensor::window::events::acquire_layer_window_error
               &) noexcept {}
};
...
// Branch-only coverage exclusion: these route-specialized template bodies are
// instantiated across the scalar/chunk route matrix, and gcovr attributes large
// amounts of std::span/SML short-circuit branch noise to the template call
// sites. Line coverage still applies; route behavior is covered by focused
// generator detail tests.
// GCOVR_EXCL_BR_START
...
inline bool compute_layer_attention_residual(native_backend &backend,
  ...
  auto attn_ctx = std::span<const float>(backend.attn_ctx.data(),
  ...
  if constexpr (route == scalar_matmul_route::packed_q8_0) {
    ...
    if (!quantize_vector_q8_k(backend.norm, q8_input) ||
        !matmul_vector_q8_input<lanes>(backend, block.attention_q, q8_input,
                                       block.attention_q.cols, q) ||
        !matmul_vector_q8_input<lanes>(backend, block.attention_k, q8_input,
                                       block.attention_k.cols, k) ||
        !matmul_vector_q8_input<lanes>(backend, block.attention_v, q8_input,
                                       block.attention_v.cols, v)) {
      return false;
    }
  } else if constexpr (route == scalar_matmul_route::native_quantized ||
                       route ==
                           scalar_matmul_route::native_quantized_q8_k_logits) {
    if (!matmul_vector_native_quantized<lanes>(backend, block.attention_q,
                                               backend.norm, q) ||
        !matmul_vector_native_quantized<lanes>(backend, block.attention_k,
                                               backend.norm, k) ||
        !matmul_vector_native_quantized<lanes>(backend, block.attention_v,
                                               backend.norm, v)) {
      return false;
    }
  } else {
  ...
  if constexpr (qk_route == attention_qk_norm_route::headwise_rms) {
    if (!apply_attention_qk_norm(backend, block)) {
      return false;
    }
  }
  if constexpr (v_route == attention_v_norm_route::rms) {
    if (!apply_rms_norm_in_place(v, backend.rms_epsilon)) {
      return false;
    }
  }

  apply_attention_rope(q, block, backend.n_head,
                       effective_attention_head_dim(backend, block),
                       effective_attention_rope_dim(backend, block), position,
                       effective_attention_rope_freq_base(backend, block));
  apply_attention_rope(k, block, backend.n_head_kv,
                       effective_attention_head_dim_kv(backend, block),
                       effective_attention_rope_dim(backend, block), position,
                       effective_attention_rope_freq_base(backend, block));
  if (!store_attention_kv_cache(backend, kv, block, layer_index, position, k,
                                v) ||
      !run_attention<mode>(backend, kv, block, layer_index, position) ||
      !matmul_vector_routed<route, lanes>(backend, block.attention_output,
                                          attn_ctx, backend.projected)) {
    return false;
  }
...
inline bool compute_layer_feed_forward(native_backend &backend,
  ...
  auto gate =
      std::span<float>(backend.gate.data(), static_cast<size_t>(ffn_dim));
  auto up = std::span<float>(backend.up.data(), static_cast<size_t>(ffn_dim));
  auto ffn_hidden =
      std::span<float>(backend.ffn_hidden.data(), static_cast<size_t>(ffn_dim));
  if constexpr (route == scalar_matmul_route::packed_q8_0) {
    if (!prepare_packed_q8_0_input(backend, backend.norm) ||
        !matmul_vector_prepared_packed_q8_0_input<lanes>(
    ...
    auto q8_input = std::span<emel::kernel::detail::quant::block_q8_k>(
        backend.q8_input_storage.data(), block_count);
    if (!quantize_vector_q8_k(backend.norm, q8_input) ||
        !matmul_vector_q8_input<lanes>(backend, block.feed_forward_gate,
                                       q8_input, block.feed_forward_gate.cols,
                                       gate) ||
        !matmul_vector_q8_input<lanes>(backend, block.feed_forward_up, q8_input,
                                       block.feed_forward_up.cols, up)) {
      return false;
    }
  } else if constexpr (route == scalar_matmul_route::native_quantized ||
                       route ==
                           scalar_matmul_route::native_quantized_q8_k_logits) {
    if (!matmul_vector_native_quantized<lanes>(backend, block.feed_forward_gate,
                                               backend.norm, gate) ||
        !matmul_vector_native_quantized<lanes>(backend, block.feed_forward_up,
                                               backend.norm, up)) {
      return false;
    }
  } else {
    if (!matmul_vector<lanes>(backend, block.feed_forward_gate, backend.norm,
                              gate) ||
        !matmul_vector<lanes>(backend, block.feed_forward_up, backend.norm,
                              up)) {
      return false;
    }
  }
  ...
  if (!matmul_vector_routed<route, lanes>(backend, block.feed_forward_down,
                                          ffn_hidden, backend.projected)) {
    return false;
  }
...
inline bool compute_logits(native_backend &backend) noexcept {
  ...
  if constexpr (route == scalar_matmul_route::packed_q8_0) {
    ...
    return quantize_vector_q8_k(backend.norm, q8_input) &&
           matmul_vector_q8_input<lanes>(backend, backend.output, q8_input,
    ...
    return matmul_vector<lanes>(backend, backend.output, backend.norm,
...
inline bool run_shortconv_block_chunk4(native_backend &backend,
  ...
  if (!prepare_chunk4_rhs<route>(backend, backend.norm_chunk4,
                                 backend.n_embd) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.shortconv_in_proj,
                                            backend.n_embd,
  ...
  if (!prepare_chunk4_rhs<route>(backend, backend.shortconv_conv_out_chunk4,
                                 backend.n_embd) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.shortconv_out_proj,
                                            backend.n_embd,
...
template <matmul_lane_mode lanes = matmul_lane_mode::serial>
inline bool run_shortconv_block_chunk8_q8_k(
    native_backend &backend, const kv_addressing_view &kv,
    const block_weights &block, const int32_t layer_index) noexcept {
  if (backend.shortconv_kernel_size <= 0 || backend.shortconv_state_size <= 0 ||
      block.shortconv_in_proj.tensor == nullptr ||
      block.shortconv_out_proj.tensor == nullptr ||
      static_cast<size_t>(block.shortconv_in_proj.rows) !=
  ...
  if (!prepare_q8_chunk8_input(backend, backend.norm_chunk8, backend.n_embd) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.shortconv_in_proj,
                                     backend.n_embd,
  ...
  if (!prepare_q8_chunk8_input(backend, backend.shortconv_conv_out_chunk8,
                               backend.n_embd) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.shortconv_out_proj,
                                     backend.n_embd,
...
inline bool compute_layer_chunk4_attention_residual(
  ...
  const int32_t q_dim = effective_attention_q_dim(backend, block);
  const int32_t kv_dim = effective_attention_kv_dim(backend, block);
  auto q_chunk = std::span<float>(backend.q_chunk4.data(),
                                  static_cast<size_t>(k_prefill_q8_chunk_rows) *
                                      static_cast<size_t>(q_dim));
  auto k_chunk = std::span<float>(backend.k_chunk4.data(),
                                  static_cast<size_t>(k_prefill_q8_chunk_rows) *
                                      static_cast<size_t>(kv_dim));
  auto v_chunk = std::span<float>(backend.v_chunk4.data(),
                                  static_cast<size_t>(k_prefill_q8_chunk_rows) *
                                      static_cast<size_t>(kv_dim));
  auto attn_ctx_chunk =
  ...
  if (!prepare_chunk4_rhs<route>(backend, backend.norm_chunk4,
                                 backend.n_embd) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.attention_q,
                                            backend.n_embd, q_chunk) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.attention_k,
                                            backend.n_embd, k_chunk) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.attention_v,
                                            backend.n_embd, v_chunk)) {
    return false;
  }
  ...
  for (int32_t row = 0; row < k_prefill_q8_chunk_rows; ++row) {
    ...
    if constexpr (qk_route == attention_qk_norm_route::headwise_rms) {
      if (!apply_headwise_rms_norm(q_row, block.attention_q_norm,
                                   backend.n_head,
                                   effective_attention_head_dim(backend, block),
                                   backend.rms_epsilon) ||
          !apply_headwise_rms_norm(
              k_row, block.attention_k_norm, backend.n_head_kv,
              effective_attention_head_dim_kv(backend, block),
              backend.rms_epsilon)) {
        return false;
      }
    ...
    if constexpr (v_route == attention_v_norm_route::rms) {
      if (!apply_rms_norm_in_place(v_row, backend.rms_epsilon)) {
        return false;
      }
    }

    apply_attention_rope(q_row, block, backend.n_head,
                         effective_attention_head_dim(backend, block),
                         effective_attention_rope_dim(backend, block), position,
                         effective_attention_rope_freq_base(backend, block));
    apply_attention_rope(k_row, block, backend.n_head_kv,
                         effective_attention_head_dim_kv(backend, block),
                         effective_attention_rope_dim(backend, block), position,
                         effective_attention_rope_freq_base(backend, block));

    if (!store_attention_kv_cache(backend, kv, block, layer_index, position,
                                  k_row, v_row) ||
        !run_attention_for_q_vector<mode>(backend, kv, block, layer_index,
                                          position, q_row)) {
      return false;
    }

    std::copy(backend.attn_ctx.begin(), backend.attn_ctx.begin() + q_dim,
              chunk4_row_span<float>(attn_ctx_chunk, row, q_dim).begin());
    backend.kv_cache_tokens = position + 1;
  }

  if (!prepare_chunk4_rhs<route>(backend, attn_ctx_chunk, q_dim) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.attention_output,
                                            q_dim, backend.projected_chunk4) ||
      !add_chunk4_rows_in_place(backend.hidden_chunk4, backend.projected_chunk4,
                                backend.n_embd)) {
    return false;
  }
...
compute_layer_chunk4_feed_forward(native_backend &backend,
  ...
  auto gate_chunk = std::span<float>(
      backend.gate_chunk4.data(), static_cast<size_t>(k_prefill_q8_chunk_rows) *
                                      static_cast<size_t>(ffn_dim));
  auto up_chunk = std::span<float>(
      backend.up_chunk4.data(), static_cast<size_t>(k_prefill_q8_chunk_rows) *
                                    static_cast<size_t>(ffn_dim));
  ...
  if (!prepare_chunk4_rhs<route>(backend, backend.norm_chunk4,
                                 backend.n_embd) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.feed_forward_gate,
                                            backend.n_embd, gate_chunk) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.feed_forward_up,
                                            backend.n_embd, up_chunk)) {
    return false;
  }

  if (!apply_silu_mul_chunk4(gate_chunk, up_chunk, ffn_dim, ffn_hidden_chunk) ||
      !prepare_chunk4_rhs<route>(backend, ffn_hidden_chunk, ffn_dim) ||
      !matmul_chunk4_prepared<route, lanes>(backend, block.feed_forward_down,
                                            ffn_dim,
                                            backend.projected_chunk4) ||
      !add_chunk4_rows_in_place(backend.hidden_chunk4, backend.projected_chunk4,
                                backend.n_embd)) {
...
inline bool compute_layer_chunk8_q8_k_attention_residual(
  ...
  const int32_t q_dim = effective_attention_q_dim(backend, block);
  const int32_t kv_dim = effective_attention_kv_dim(backend, block);
  auto q_chunk = std::span<float>(
      backend.q_chunk8.data(), static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                                   static_cast<size_t>(q_dim));
  auto k_chunk = std::span<float>(
      backend.k_chunk8.data(), static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                                   static_cast<size_t>(kv_dim));
  auto v_chunk = std::span<float>(
      backend.v_chunk8.data(), static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                                   static_cast<size_t>(kv_dim));
  auto attn_ctx_chunk =
  ...
  if (!prepare_q8_chunk8_input(backend, backend.norm_chunk8, backend.n_embd) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.attention_q, backend.n_embd,
                                     q_chunk) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.attention_k, backend.n_embd,
                                     k_chunk) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.attention_v, backend.n_embd,
                                     v_chunk)) {
    return false;
  }
  ...
  if (!prepare_q8_chunk8_input(backend, attn_ctx_chunk, q_dim) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.attention_output, q_dim,
                                     backend.projected_chunk8) ||
      !add_chunk8_rows_in_place(backend.hidden_chunk8, backend.projected_chunk8,
                                backend.n_embd)) {
    return false;
  ...
  auto gate_chunk =
      std::span<float>(backend.gate_chunk8.data(),
                       static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                           static_cast<size_t>(ffn_dim));
  auto up_chunk = std::span<float>(
      backend.up_chunk8.data(), static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                                    static_cast<size_t>(ffn_dim));
  auto ffn_hidden_chunk =
      std::span<float>(backend.ffn_hidden_chunk8.data(),
                       static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                           static_cast<size_t>(ffn_dim));
  if (!prepare_q8_chunk8_input(backend, backend.norm_chunk8, backend.n_embd) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.feed_forward_gate,
                                     backend.n_embd, gate_chunk) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.feed_forward_up,
                                     backend.n_embd, up_chunk)) {
    return false;
  }

  if (!apply_silu_mul_chunk8(gate_chunk, up_chunk, ffn_dim, ffn_hidden_chunk) ||
      !prepare_q8_chunk8_input(backend, ffn_hidden_chunk, ffn_dim) ||
      !matmul_chunk8_q8_input<lanes>(backend, block.feed_forward_down, ffn_dim,
                                     backend.projected_chunk8) ||
      !add_chunk8_rows_in_place(backend.hidden_chunk8, backend.projected_chunk8,
                                backend.n_embd)) {
    return false;
...
template <emel::text::generator::attention_mode mode, scalar_matmul_route route>
inline bool run_prefill(native_backend &backend,
                        const kv_addressing_view &kv) noexcept {
...
template <emel::text::generator::attention_mode mode, chunk4_rhs_route route,
          matmul_lane_mode lanes = matmul_lane_mode::serial>
inline bool run_prefill_chunk4(native_backend &backend,
                               const kv_addressing_view &kv) noexcept {
  backend.kv_cache_tokens = 0;
  reset_shortconv_cache(backend);
  ...
  if (chunk_limit == 0u ||
      !run_prefill_chunk4_tokens<mode, route, lanes>(backend, kv,
                                                     chunk_limit) ||
      !run_prefill_scalar_tokens<mode, static_cast<scalar_matmul_route>(route),
                                 lanes>(backend, kv, chunk_limit,
                                        token_count)) {
    return false;
  }

  return compute_logits<static_cast<scalar_matmul_route>(route), lanes>(
      backend);
}
...
inline bool run_prefill_chunk4(native_backend &backend) noexcept {
  return run_prefill_chunk4<mode, route, lanes>(backend,
                                                identity_kv_addressing());
}
...
inline bool run_prefill_chunk8_q8_k(native_backend &backend) noexcept {
  return run_prefill_chunk8_q8_k<mode, lanes>(backend,
                                              identity_kv_addressing());
}

inline bool run_prefill_flash(native_backend &backend) noexcept {
  return run_prefill<emel::text::generator::attention_mode::flash,
                     scalar_matmul_route::kernel>(backend,
                                                  identity_kv_addressing());
}

inline bool run_prefill_nonflash(native_backend &backend) noexcept {
  return run_prefill<emel::text::generator::attention_mode::nonflash,
                     scalar_matmul_route::kernel>(backend,
                                                  identity_kv_addressing());
}
...
template <emel::text::generator::attention_mode mode, chunk4_rhs_route route,
          matmul_lane_mode lanes = matmul_lane_mode::serial>
inline bool run_prefill_chunk4_preselected_argmax(
    native_backend &backend, const kv_addressing_view &kv,
...
template <emel::text::generator::attention_mode mode,
          matmul_lane_mode lanes = matmul_lane_mode::serial>
inline bool run_prefill_chunk8_preselected_argmax_q8_k(
    native_backend &backend, const kv_addressing_view &kv,
    int32_t &selected_index, float &selected_score) noexcept {
  backend.kv_cache_tokens = 0;
  reset_shortconv_cache(backend);
...
inline bool run_decode(native_backend &backend,
  ...
  // Branch-only coverage exclusion: decode route wrappers are instantiated for
  // each explicit SML-selected route. Tests cover the success/rejection lines;
  // transition guards own behavior choice. GCOVR_EXCL_BR_START
...
inline bool run_decode_preselected_argmax(
    native_backend &backend,
    const emel::graph::processor::event::execute &request,
    const kv_addressing_view &kv, int32_t &selected_index,
    float &selected_score, int32_t *err_out = nullptr) noexcept {
  // Branch-only coverage exclusion: mirrors run_decode's route-instantiated
  // validation/loop wrapper and keeps line coverage active for the tested
  // success and rejection paths. GCOVR_EXCL_BR_START
  if (backend.bound_token_count != 1 || backend.bound_position_count != 1 ||
      request.kv_tokens < 0 || backend.kv_cache_tokens != request.kv_tokens) {
    return false;
  }
...
inline bool run_decode_preselected_argmax(
  ...
  return run_decode_preselected_argmax<mode, route, argmax_route, lanes, wmode>(
      backend, request, identity_kv_addressing(), selected_index,
      selected_score, err_out);
}
...
inline emel::error::type prepare(
  ...
  backend.generation_execution = generation_contract.generation_execution;
  backend.topology = generation_contract.topology;
  backend.prefill_plan = generation_contract.prefill_plan;
  backend.decode_plan = generation_contract.decode_plan;
  backend.quantized_audit = generation_contract.quantized_audit;
  backend.generation_execution.execution = &backend.execution;
  ...
  backend.kv_positions_capacity = emel::memory::view::positions_capacity_for(
      kv_block_tokens, backend.n_ctx);
  ...
  for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
    ...
    if (emel::model::generation::lookup_block_view(backend.execution, layer,
                                                   block) !=
        emel::error::cast(emel::model::loader::error::none)) {
      return emel::error::cast(emel::model::loader::error::model_invalid);
    }
    ...
    weights.residual_route = layer_execution.residual_route;
    weights.qk_norm_route = layer_execution.qk_norm_route;
    weights.value_route = layer_execution.value_route;
    weights.v_norm_route = layer_execution.v_norm_route;
    weights.window_route = layer_execution.window_route;
    weights.expected_attention_key_length =
        layer_execution.attention_key_length;
    weights.expected_attention_value_length =
        layer_execution.attention_value_length;
    ...
    if (weights.residual_route == residual_route::attention) {
    ...
    if (!bind_tensor_rows(*block.shortconv_in_proj.tensor,
  ...
  backend.flash_layer_cache_offsets.resize(
      static_cast<size_t>(backend.n_layer));
  ...
  for (int32_t layer = 0; layer < backend.n_layer; ++layer) {
    ...
    backend.max_ffn_dim =
        std::max(backend.max_ffn_dim, block.feed_forward_gate.rows);
    ...
    if (block.residual_route == residual_route::attention) {
      ...
      if (block.attention_head_dim <= 0 || block.attention_head_dim_kv <= 0 ||
          ...
          (block.expected_attention_value_length > 0 &&
           block.attention_head_dim_kv !=
               block.expected_attention_value_length) ||
          (qk_norm_required &&
      ...
      backend.flash_layer_cache_offsets[static_cast<size_t>(layer)] =
          flash_cache_offset;
  ...
  backend.head_dim =
      backend.max_q_dim > 0 ? backend.max_q_dim / backend.n_head : 0;
  backend.head_dim_kv =
      backend.max_kv_dim > 0 ? backend.max_kv_dim / backend.n_head_kv : 0;
  ...
  backend.q_chunk4.resize(static_cast<size_t>(k_prefill_q8_chunk_rows) *
                          static_cast<size_t>(backend.max_q_dim));
  ...
  backend.k_chunk4.resize(static_cast<size_t>(k_prefill_q8_chunk_rows) *
                          static_cast<size_t>(backend.max_kv_dim));
  ...
  backend.gate_chunk4.resize(static_cast<size_t>(k_prefill_q8_chunk_rows) *
                             static_cast<size_t>(backend.max_ffn_dim));
  backend.gate_chunk8.resize(static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                             static_cast<size_t>(backend.max_ffn_dim));
  backend.up.resize(static_cast<size_t>(backend.max_ffn_dim));
  backend.up_chunk4.resize(static_cast<size_t>(k_prefill_q8_chunk_rows) *
                           static_cast<size_t>(backend.max_ffn_dim));
  backend.up_chunk8.resize(static_cast<size_t>(k_prefill_q8_chunk8_rows) *
                           static_cast<size_t>(backend.max_ffn_dim));
...
inline uint32_t quantized_contract_stage_count(
    ...
    const emel::model::generation::quantized_contract_kind kind) noexcept {
  uint32_t count = 0u;
...
inline emel::text::generator::compute_io &
bind_compute_io(const emel::graph::processor::event::execute &request) noexcept;
...
inline bool
valid_kv_physical_position(const emel::memory::view::snapshot &snapshot,
                           const native_backend &backend, const int32_t seq_id,
                           const int32_t position) noexcept {
  ...
  const int64_t physical_position =
      static_cast<int64_t>(block_id) *
          static_cast<int64_t>(snapshot.block_tokens) +
      static_cast<int64_t>(position % snapshot.block_tokens);
  return block_id >= 0 && physical_position >= 0 &&
         physical_position <
             static_cast<int64_t>(backend.kv_positions_capacity);
}

inline bool
valid_identity_kv_prefix(const emel::memory::view::snapshot &snapshot,
                         const int32_t seq_id,
                         const int32_t max_position) noexcept {
  const int32_t max_logical_block = max_position / snapshot.block_tokens;
  for (int32_t logical_block = 0; logical_block <= max_logical_block;
       ++logical_block) {
...
inline bool valid_single_sequence_kv_request(
    const emel::graph::processor::event::execute &request,
    int32_t *err_out) noexcept {
  if (request.compute_ctx == nullptr || request.memory_view == nullptr ||
      request.seq_primary_ids == nullptr ||
      request.seq_primary_ids_count != 1 || request.positions == nullptr ||
  ...
  const int32_t recurrent_needed =
      static_cast<int32_t>(backend.shortconv_state_size > 0);
  ...
  const auto &io = bind_compute_io(request);
  if (io.selected_attention_mode ==
...
inline bool
validate_guarded_compute(const emel::graph::processor::event::execute &request,
                         int32_t *err_out) noexcept {
...
inline bool
bind_guarded_inputs(const emel::graph::processor::event::execute &request,
                    int32_t *err_out) noexcept {
  (void)err_out;
...
template <emel::text::generator::attention_mode mode, scalar_matmul_route route,
          ...
          window_mode wmode = window_mode::resident>
inline bool
run_kernel_scalar_mode(const emel::graph::processor::event::execute &request,
                       int32_t *err_out) noexcept {
  auto &backend = bind_native_backend(request);
...
inline bool run_kernel_flash_prefill_scalar_packed_q8_0(
    const emel::graph::processor::event::execute &request,
    int32_t *err_out) noexcept {
  return run_kernel_scalar_mode<emel::text::generator::attention_mode::flash,
...
inline bool run_kernel_flash_prefill_scalar_q8_k(
    ...
    int32_t *err_out) noexcept {
  return run_kernel_scalar_mode<emel::text::generator::attention_mode::flash,
                                scalar_matmul_route::q8_k, step_kind::prefill>(
      request, err_out);
}
...
inline bool run_kernel_flash_prefill_scalar_native_quantized_q8_k_logits(
  ...
  return run_kernel_scalar_mode<
      emel::text::generator::attention_mode::flash,
      scalar_matmul_route::native_quantized_q8_k_logits, step_kind::prefill>(
      request, err_out);
}
...
inline bool run_kernel_nonflash_prefill_scalar_packed_q8_0(
    const emel::graph::processor::event::execute &request,
    int32_t *err_out) noexcept {
  return run_kernel_scalar_mode<emel::text::generator::attention_mode::nonflash,
                                scalar_matmul_route::packed_q8_0,
                                step_kind::prefill>(request, err_out);
}
...
inline bool run_kernel_flash_decode_q8_k(
    ...
    int32_t *err_out) noexcept {
  return run_kernel_scalar_mode<emel::text::generator::attention_mode::flash,
                                scalar_matmul_route::q8_k, step_kind::decode>(
      request, err_out);
}
...
inline bool run_kernel_flash_decode_packed_q8_0_streamed(
  ...
  return run_kernel_scalar_mode<emel::text::generator::attention_mode::flash,
                                scalar_matmul_route::packed_q8_0,
                                step_kind::decode, matmul_lane_mode::serial,
...
inline bool run_kernel_flash_decode_native_quantized_q8_k_logits(
  ...
  return run_kernel_scalar_mode<
      emel::text::generator::attention_mode::flash,
      scalar_matmul_route::native_quantized_q8_k_logits, step_kind::decode>(
      request, err_out);
}
...
inline bool run_kernel_flash_decode_native_quantized_q8_k_logits_streamed(
  ...
  return run_kernel_scalar_mode<
      emel::text::generator::attention_mode::flash,
      scalar_matmul_route::native_quantized_q8_k_logits, step_kind::decode,
      matmul_lane_mode::serial, window_mode::streamed>(request, err_out);
}
...
inline bool run_kernel_flash_decode_kernel(
    ...
    int32_t *err_out) noexcept {
  return run_kernel_scalar_mode<emel::text::generator::attention_mode::flash,
                                scalar_matmul_route::kernel, step_kind::decode>(
      request, err_out);
}
...
inline bool run_kernel_flash_decode_parallel_packed_q8_0(
  ...
  return run_kernel_scalar_mode<emel::text::generator::attention_mode::flash,
                                scalar_matmul_route::packed_q8_0,
                                step_kind::decode, matmul_lane_mode::parallel>(
      request, err_out);
}
...
inline bool run_kernel_flash_prefill_chunk4_q8_k(
    ...
    int32_t *err_out) noexcept {
  return run_kernel_prefill_chunk4_mode<
      emel::text::generator::attention_mode::flash, chunk4_rhs_route::q8_k>(
      request, err_out);
}
...
inline bool run_kernel_nonflash_prefill_chunk4_q8_k(
    ...
    int32_t *err_out) noexcept {
  return run_kernel_prefill_chunk4_mode<
      emel::text::generator::attention_mode::nonflash, chunk4_rhs_route::q8_k>(
      request, err_out);
}
...
inline bool run_kernel_flash_prefill_parallel_chunk8_q8_k(
    ...
    int32_t *err_out) noexcept {
  return run_kernel_prefill_chunk8_q8_k_mode<
      emel::text::generator::attention_mode::flash, matmul_lane_mode::parallel>(
      request, err_out);
}
...
inline bool run_kernel_flash_prefill_parallel_chunk4_packed_q8_0(
  ...
  return run_kernel_prefill_chunk4_mode<
      emel::text::generator::attention_mode::flash,
      chunk4_rhs_route::packed_q8_0, matmul_lane_mode::parallel>(request,
                                                                 err_out);
}
...
inline bool run_kernel_flash_prefill_scalar_preselected_argmax_q8_k(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::flash, scalar_matmul_route::q8_k,
      scalar_argmax_route::q8_k, step_kind::prefill>(request, err_out);
}
...
inline bool run_kernel_flash_prefill_scalar_preselected_argmax_kernel(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::flash, scalar_matmul_route::kernel,
      scalar_argmax_route::kernel, step_kind::prefill>(request, err_out);
}
...
inline bool run_kernel_nonflash_prefill_scalar_preselected_argmax_q8_k(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::nonflash,
      scalar_matmul_route::q8_k, scalar_argmax_route::q8_k, step_kind::prefill>(
      request, err_out);
}
...
inline bool run_kernel_nonflash_prefill_scalar_preselected_argmax_kernel(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::nonflash,
      scalar_matmul_route::kernel, scalar_argmax_route::kernel,
      step_kind::prefill>(request, err_out);
}
...
inline bool run_kernel_flash_decode_preselected_argmax_q8_k(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::flash, scalar_matmul_route::q8_k,
      scalar_argmax_route::q8_k, step_kind::decode>(request, err_out);
}
...
inline bool run_kernel_flash_decode_preselected_argmax_kernel(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::flash, scalar_matmul_route::kernel,
      scalar_argmax_route::kernel, step_kind::decode>(request, err_out);
}
...
inline bool run_kernel_flash_decode_parallel_preselected_argmax_q8_k(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::flash, scalar_matmul_route::q8_k,
      scalar_argmax_route::q8_k, step_kind::decode, matmul_lane_mode::parallel>(
      request, err_out);
}
...
run_kernel_flash_decode_parallel_preselected_argmax_native_quantized_q8_k(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::flash,
      scalar_matmul_route::native_quantized, scalar_argmax_route::q8_k,
      step_kind::decode, matmul_lane_mode::parallel>(request, err_out);
}
...
inline bool run_kernel_flash_decode_parallel_preselected_argmax_kernel(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::flash, scalar_matmul_route::kernel,
      scalar_argmax_route::kernel, step_kind::decode,
      matmul_lane_mode::parallel>(request, err_out);
}
...
inline bool run_kernel_nonflash_decode_preselected_argmax_q8_k(
  ...
  return run_kernel_scalar_preselected_argmax_mode<
      emel::text::generator::attention_mode::nonflash,
      scalar_matmul_route::q8_k, scalar_argmax_route::q8_k, step_kind::decode>(
      request, err_out);
}
...
inline bool
extract_guarded_outputs(const emel::graph::processor::event::execute &request,
