...
struct begin_generate {
  void operator()(const event::generate_run & ev, context &) const noexcept {
    ...
    ev.ctx.phase_accepted = false;
    ev.ctx.phase_code = 0;
    ev.ctx.prefill_contract = emel::text::generator::prefill_compute_contract::none;
...
struct request_initializer {
  void operator()(const event::initialize_run & ev, context & ctx) const noexcept {
    const emel::text::generator::initializer::event::run runtime{ev.request, ev.ctx};
...
struct request_prefill {
  void operator()(const event::generate_run & ev, context & ctx) const noexcept {
    const emel::text::generator::prefill::event::run runtime{ev.request, ev.ctx};
...
inline void request_phase_compute(const event::generate_run & ev, context & ctx) noexcept {
  ...
  emel::graph::event::compute compute_ev{
    ...
    .validate = emel::text::generator::detail::validate_guarded_compute,
    .prepare_graph = emel::text::generator::detail::prepare_graph,
    .alloc_graph = emel::text::generator::detail::alloc_graph,
    .bind_inputs = emel::text::generator::detail::bind_guarded_inputs,
    .run_kernel = run_kernel_fn,
    .extract_outputs = emel::text::generator::detail::extract_guarded_outputs,
    .dispatch_done = on_done,
    .dispatch_error = on_error,
  };
  if constexpr (kind == emel::text::generator::detail::step_kind::prefill) {
    ...
    compute_ev.lifecycle = emel::text::generator::detail::prefill_lifecycle(
        ctx.compute.backend,
    ...
    compute_ev.output_out = &ev.ctx.graph_output;
    compute_ev.lifecycle = emel::text::generator::detail::decode_lifecycle(
        ctx.compute.backend,
        ctx.buffers.prompt_tokens.data(),
        ctx.limits.prompt_capacity,
        ctx.buffers.positions.data(),
...
inline void request_phase_compute_preselected_argmax(const event::generate_run & ev,
  ...
  emel::graph::event::compute compute_ev{
    ...
    .seq_primary_ids = ctx.buffers.seq_primary_ids.data(),
    .seq_primary_ids_count = 1,
    .validate = emel::text::generator::detail::validate_guarded_preselected_argmax,
    .prepare_graph = emel::text::generator::detail::prepare_graph,
    .alloc_graph = emel::text::generator::detail::alloc_graph,
    .bind_inputs = emel::text::generator::detail::bind_guarded_inputs,
    .run_kernel = run_kernel_fn,
    .extract_outputs = emel::text::generator::detail::extract_guarded_preselected_argmax,
    .dispatch_done = on_done,
    .dispatch_error = on_error,
  };
  if constexpr (kind == emel::text::generator::detail::step_kind::prefill) {
...
inline void reset_graph_lifecycle_snapshot(
    const event::capture_graph_lifecycle & ev,
    const context & ctx) noexcept {
  ev.out.reservation = &ctx.state.graph_reservation;
  ev.out.first_tensor = {};
  ev.out.runtime_tensor = {};
  ev.out.first_tensor_err = emel::error::cast(emel::graph::tensor::error::none);
  ev.out.runtime_tensor_err = emel::error::cast(emel::graph::tensor::error::none);
}

struct capture_graph_lifecycle_without_runtime_tensor {
  void operator()(const event::capture_graph_lifecycle & ev, context & ctx) const noexcept {
    reset_graph_lifecycle_snapshot(ev, ctx);
    ev.out.first_tensor_captured =
        ctx.graph.try_capture_tensor(0, ev.out.first_tensor, ev.out.first_tensor_err);
    ev.out.runtime_tensor_captured = ctx.graph.try_capture_tensor(
        -1,
        ev.out.runtime_tensor,
        ev.out.runtime_tensor_err);
  }
};

struct capture_graph_lifecycle_with_runtime_tensor {
  void operator()(const event::capture_graph_lifecycle & ev, context & ctx) const noexcept {
    reset_graph_lifecycle_snapshot(ev, ctx);
    ev.out.reservation = &ctx.state.graph_reservation;
    ev.out.first_tensor_captured =
        ctx.graph.try_capture_tensor(0, ev.out.first_tensor, ev.out.first_tensor_err);
    ev.out.runtime_tensor_captured = ctx.graph.try_capture_tensor(
        ctx.state.graph_reservation.lifecycle->tensor_count - 1,
        ev.out.runtime_tensor,
        ev.out.runtime_tensor_err);
  }
};
...
inline constexpr request_planning_with_step_size<emel::text::generator::detail::k_prefill_q8_chunk_rows>
...
inline constexpr request_decode_compute_flash_packed_q8_0
    request_decode_compute_flash_packed_q8_0{};
inline constexpr request_decode_compute_flash_q8_k request_decode_compute_flash_q8_k{};
inline constexpr request_decode_compute_flash_native_quantized
    request_decode_compute_flash_native_quantized{};
...
inline constexpr request_decode_compute_nonflash_native_quantized
    request_decode_compute_nonflash_native_quantized{};
...
inline constexpr request_decode_compute_flash_preselected_argmax_native_quantized_kernel
...
inline constexpr request_decode_compute_nonflash_preselected_argmax_native_quantized_kernel
...
inline constexpr capture_graph_lifecycle_with_runtime_tensor
    capture_graph_lifecycle_with_runtime_tensor{};
