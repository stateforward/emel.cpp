  ...
  for (const std::string &word : words) {
    ...
    for (int32_t i = 0; i < n; ++i) {
      ...
      for (int32_t j = end; j > i; --j) {
        ...
        const int32_t token = emel::text::encoders::detail::lookup_token(ctx, piece);
    ...
    if (count == word_token_start) {
      ...
      if (unk == k_token_null) {
        unk = emel::text::encoders::detail::lookup_token(ctx, "<unk>");
      }
      if (unk == k_token_null) {
        continue;
      }
      if (!emel::text::encoders::detail::push_token(ev, unk, count)) {
        result.error = EMEL_ERR_INVALID_ARGUMENT;
        return result;
      }
    }
