...
inline bool ensure_plamo2_tables(emel::text::encoders::plamo2::action::context &ctx,
  ...
  for (uint32_t token_id = 0; token_id < vocab.n_tokens; ++token_id) {
    const std::string_view text = emel::text::encoders::detail::token_text(vocab, token_id);
    if (text.empty()) {
      continue;
    }
...
inline encode_result encode_plamo2(const event::encode &ev,
  ...
  while (pos < static_cast<int32_t>(data_len)) {
    ...
    if (path.token_id >= 0) {
      if (!emel::text::encoders::detail::push_token(ev, path.token_id, count)) {
        result.error = EMEL_ERR_INVALID_ARGUMENT;
        return result;
      }
    } else {
      ...
      for (int32_t i = 0; i < s; ++i) {
        ...
        if (byte_token <= 0 ||
            !emel::text::encoders::detail::push_token(ev, byte_token, count)) {
          result.error = EMEL_ERR_INVALID_ARGUMENT;
          return result;
        }
      }
