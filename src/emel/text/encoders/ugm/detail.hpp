...
inline bool init_xcda_tables(emel::text::encoders::ugm::action::context &ctx) {
...
inline bool ensure_ugm_tables(emel::text::encoders::ugm::action::context &ctx,
  ...
  for (uint32_t id = 0; id < vocab.n_tokens; ++id) {
    ...
    const std::string_view text = emel::text::encoders::detail::token_text(vocab, id);
    if (text.empty()) {
      continue;
    }
...
inline encode_result encode_ugm(const event::encode &ev,
  ...
  const int32_t unk_id = (vocab.unk_id != k_token_null)
                             ? vocab.unk_id
                             : emel::text::encoders::detail::lookup_token(ctx, "<unk>");
  ...
  while (input_offset < input_len) {
    const size_t n_utf8_code_units =
        std::min(static_cast<size_t>(emel::text::encoders::detail::utf8_len(
