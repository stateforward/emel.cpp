...
struct eval_next_stmt {
  void operator()(context & ctx) const noexcept {
    ...
    const emel::text::jinja::ast_node * node = (*ctx.statements)[ctx.statement_index].get();
    ...
    } else if (auto * stmt = dynamic_cast<const emel::text::jinja::for_statement *>(node)) {
      detail::render_for(ctx, stmt, ctx.globals, io, false, flow);
    } else if (auto * stmt = dynamic_cast<const emel::text::jinja::set_statement *>(node)) {
      detail::render_set(ctx, stmt, ctx.globals, io);
    } else if (auto * stmt = dynamic_cast<const emel::text::jinja::macro_statement *>(node)) {
    ...
    } else if (auto * stmt = dynamic_cast<const emel::text::jinja::filter_statement *>(node)) {
      detail::render_filter_statement(ctx, stmt, ctx.globals, io);
    } else if (auto * stmt = dynamic_cast<const emel::text::jinja::call_statement *>(node)) {
