...
struct call_args {
  std::array<emel::text::jinja::value, k_max_call_args> pos = {};
  size_t pos_count = 0;
...
using builtin_fn = bool (*)(action::context &, const call_args &, emel::text::jinja::value &) noexcept;
using filter_fn = bool (*)(action::context &, const emel::text::jinja::value &, const call_args &, emel::text::jinja::value &) noexcept;
using test_fn = bool (*)(action::context &, const emel::text::jinja::value &, const call_args &, emel::text::jinja::value &) noexcept;
...
inline emel::text::jinja::value make_undefined(std::string_view hint = {}) noexcept {
  emel::text::jinja::value v;
  v.type = emel::text::jinja::value_type::undefined;
  v.hint = hint;
  return v;
}

inline emel::text::jinja::value make_none() noexcept {
  emel::text::jinja::value v;
  v.type = emel::text::jinja::value_type::none;
  return v;
}

inline emel::text::jinja::value make_bool(const bool v) noexcept {
...
inline emel::text::jinja::value make_int(const int64_t v) noexcept {
  emel::text::jinja::value out;
...
inline emel::text::jinja::value make_float(const double v) noexcept {
  emel::text::jinja::value out;
  out.type = emel::text::jinja::value_type::floating;
  out.float_v = v;
  out.int_v = static_cast<int64_t>(v);
  return out;
}

inline emel::text::jinja::value make_string(std::string_view view, const bool is_input = false) noexcept {
  emel::text::jinja::value out;
...
inline emel::text::jinja::value make_function(const emel::text::jinja::function_ref ref) noexcept {
  emel::text::jinja::value out;
  out.type = emel::text::jinja::value_type::function;
  out.func_v = ref;
  return out;
}
...
inline bool end_capture(action::context & ctx, render_io & io, emel::text::jinja::value & out) noexcept {
  if (io.depth == 0) {
    set_error(ctx, EMEL_ERR_BACKEND, 0);
    return false;
  }
...
inline emel::text::jinja::value make_array(action::context & ctx,
                                    const emel::text::jinja::value * items,
  ...
  emel::text::jinja::value * dest = ctx.array_items.data() + ctx.array_items_used;
...
inline emel::text::jinja::value make_object(action::context & ctx,
                                     const emel::text::jinja::object_entry * entries,
                                     const size_t count,
                                     const size_t capacity,
                                     const bool has_builtins) noexcept {
  emel::text::jinja::value out;
  out.type = emel::text::jinja::value_type::object;
  if (capacity == 0) {
  ...
  emel::text::jinja::object_entry * dest = ctx.object_entries.data() + ctx.object_entries_used;
...
inline bool push_scope(action::context & ctx) noexcept {
  ...
  emel::text::jinja::object_entry * entries = ctx.object_entries.data() + ctx.object_entries_used;
...
inline bool value_is_truthy(const emel::text::jinja::value & v) noexcept {
  switch (v.type) {
    case emel::text::jinja::value_type::undefined:
    case emel::text::jinja::value_type::none:
      return false;
    ...
    case emel::text::jinja::value_type::array:
      return v.array_v.count > 0;
    case emel::text::jinja::value_type::object:
      return v.object_v.count > 0;
    case emel::text::jinja::value_type::function:
      return true;
  }
  return false;
}

inline bool value_is_number(const emel::text::jinja::value & v) noexcept {
  return v.type == emel::text::jinja::value_type::integer ||
         v.type == emel::text::jinja::value_type::floating;
}

inline bool value_equal(const emel::text::jinja::value & a, const emel::text::jinja::value & b) noexcept {
  if (a.type != b.type) {
    if (value_is_number(a) && value_is_number(b)) {
      const double da = a.type == emel::text::jinja::value_type::integer ?
        static_cast<double>(a.int_v) : a.float_v;
      const double db = b.type == emel::text::jinja::value_type::integer ?
...
inline const emel::text::jinja::object_entry * find_object_entry(
    const emel::text::jinja::object_value & object,
    const std::string_view key) noexcept {
  for (size_t i = 0; i < object.count; ++i) {
    const emel::text::jinja::object_entry & entry = object.entries[i];
...
inline emel::text::jinja::object_entry * find_object_entry_mut(
    emel::text::jinja::object_value & object,
    const std::string_view key) noexcept {
  for (size_t i = 0; i < object.count; ++i) {
    emel::text::jinja::object_entry & entry = object.entries[i];
    if (entry.key.type == emel::text::jinja::value_type::string &&
        entry.key.string_v.view == key) {
      return &entry;
    }
...
inline bool set_object_value(action::context & ctx,
                             ...
                             const emel::text::jinja::value & val) noexcept {
  emel::text::jinja::object_entry * existing = find_object_entry_mut(object, key);
  if (existing != nullptr) {
    existing->val = val;
    return true;
  }
...
inline emel::text::jinja::value lookup_identifier(
    action::context & ctx,
    const emel::text::jinja::object_value * globals,
    const std::string_view name,
    const builtin_entry * builtins,
    const size_t builtin_count) noexcept {
  for (size_t i = ctx.scope_count; i > 0; --i) {
    emel::text::jinja::object_value & scope_obj = ctx.scopes[i - 1].locals;
    const emel::text::jinja::object_entry * entry = find_object_entry(scope_obj, name);
    if (entry != nullptr) {
      return entry->val;
    }
  }
  if (globals != nullptr) {
    const emel::text::jinja::object_entry * entry = find_object_entry(*globals, name);
    if (entry != nullptr) {
      return entry->val;
    }
  }
  for (size_t i = 0; i < builtin_count; ++i) {
    if (builtins[i].name == name) {
      emel::text::jinja::function_ref ref;
      ref.kind = emel::text::jinja::function_kind::builtin;
      ref.data = &builtins[i];
...
inline std::string_view value_to_string(action::context & ctx, const emel::text::jinja::value & v) noexcept {
  if (v.type == emel::text::jinja::value_type::string) {
    return v.string_v.view;
  }
  if (v.type == emel::text::jinja::value_type::none || v.type == emel::text::jinja::value_type::undefined) {
    return {};
  }
  char buffer[64] = {};
  int written = 0;
...
inline bool write_value(action::context & ctx, render_io & io, const emel::text::jinja::value & v) noexcept {
  if (v.type == emel::text::jinja::value_type::none || v.type == emel::text::jinja::value_type::undefined) {
    return true;
  }
  if (v.type == emel::text::jinja::value_type::array) {
  ...
  if (v.type == emel::text::jinja::value_type::object) {
    return true;
  }
...
inline bool add_pos(call_args & args, const emel::text::jinja::value & val) noexcept {
...
inline bool add_kw(call_args & args, const std::string_view key, const emel::text::jinja::value & val) noexcept {
...
inline const emel::text::jinja::value * find_kw(const call_args & args, std::string_view key) noexcept {
...
inline const emel::text::jinja::value * get_pos(const call_args & args, const size_t index) noexcept {
...
inline bool value_is_string(const emel::text::jinja::value & v) noexcept {
  return v.type == emel::text::jinja::value_type::string;
}

inline bool value_is_array(const emel::text::jinja::value & v) noexcept {
  return v.type == emel::text::jinja::value_type::array;
}

inline bool value_is_object(const emel::text::jinja::value & v) noexcept {
  return v.type == emel::text::jinja::value_type::object;
}

inline bool builtin_default(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  const emel::text::jinja::value * val = get_pos(args, 0);
  const emel::text::jinja::value * def = get_pos(args, 1);
  const emel::text::jinja::value * use_when_false = get_pos(args, 2);
  if (val == nullptr || def == nullptr) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
    return false;
  }
  bool apply = (val->type == emel::text::jinja::value_type::undefined ||
                val->type == emel::text::jinja::value_type::none);
  if (!apply && use_when_false != nullptr && use_when_false->type == emel::text::jinja::value_type::boolean) {
...
inline bool builtin_length(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  const emel::text::jinja::value * val = get_pos(args, 0);
  if (val == nullptr) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
    return false;
...
inline bool builtin_range(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  ...
  int64_t step = 1;
  if (args.pos_count == 1) {
    const emel::text::jinja::value & v0 = args.pos[0];
    if (v0.type != emel::text::jinja::value_type::integer) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
      return false;
    }
    stop = v0.int_v;
    ...
    const emel::text::jinja::value & v0 = args.pos[0];
    const emel::text::jinja::value & v1 = args.pos[1];
    if (v0.type != emel::text::jinja::value_type::integer ||
        v1.type != emel::text::jinja::value_type::integer) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
      return false;
    }
    start = v0.int_v;
    ...
    const emel::text::jinja::value & v0 = args.pos[0];
    const emel::text::jinja::value & v1 = args.pos[1];
    const emel::text::jinja::value & v2 = args.pos[2];
    if (v0.type != emel::text::jinja::value_type::integer ||
        v1.type != emel::text::jinja::value_type::integer ||
        v2.type != emel::text::jinja::value_type::integer) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
      return false;
    }
    start = v0.int_v;
  ...
  std::array<emel::text::jinja::value, action::k_max_array_items> tmp = {};
  size_t count = 0;
  if (step > 0) {
...
inline bool builtin_namespace(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  if (args.kw_count == 0) {
    emel::text::jinja::object_entry empty = {};
    out = make_object(ctx, &empty, 0, 0, false);
    return ctx.phase_error == EMEL_OK;
  }
  std::array<emel::text::jinja::object_entry, k_max_call_args> tmp = {};
  for (size_t i = 0; i < args.kw_count; ++i) {
...
inline bool builtin_upper(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  const emel::text::jinja::value * val = get_pos(args, 0);
  if (val == nullptr) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
    return false;
...
inline bool builtin_lower(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  const emel::text::jinja::value * val = get_pos(args, 0);
  if (val == nullptr) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
    return false;
...
inline bool builtin_trim(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  const emel::text::jinja::value * val = get_pos(args, 0);
  if (val == nullptr) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
    return false;
...
inline bool builtin_replace(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  const emel::text::jinja::value * input_val = get_pos(args, 0);
  const emel::text::jinja::value * old_val = get_pos(args, 1);
  const emel::text::jinja::value * new_val = get_pos(args, 2);
...
inline bool builtin_join(action::context & ctx, const call_args & args, emel::text::jinja::value & out) noexcept {
  const emel::text::jinja::value * input = get_pos(args, 0);
  const emel::text::jinja::value * delim = get_pos(args, 1);
  if (input == nullptr) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
    return false;
  }
  if (input->type != emel::text::jinja::value_type::array) {
...
inline bool filter_default(action::context & ctx, const emel::text::jinja::value & input,
                           const call_args & args, emel::text::jinja::value & out) noexcept {
...
inline bool filter_length(action::context & ctx, const emel::text::jinja::value & input,
                          const call_args & args, emel::text::jinja::value & out) noexcept {
  call_args combined = args;
...
inline bool filter_upper(action::context & ctx, const emel::text::jinja::value & input,
                         const call_args & args, emel::text::jinja::value & out) noexcept {
  call_args combined = args;
  if (combined.pos_count >= k_max_call_args) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
...
inline bool filter_lower(action::context & ctx, const emel::text::jinja::value & input,
                         const call_args & args, emel::text::jinja::value & out) noexcept {
  call_args combined = args;
  if (combined.pos_count >= k_max_call_args) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
...
inline bool filter_trim(action::context & ctx, const emel::text::jinja::value & input,
                        const call_args & args, emel::text::jinja::value & out) noexcept {
  call_args combined = args;
  if (combined.pos_count >= k_max_call_args) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
...
inline bool filter_replace(action::context & ctx, const emel::text::jinja::value & input,
                           const call_args & args, emel::text::jinja::value & out) noexcept {
  call_args combined = args;
  if (combined.pos_count >= k_max_call_args) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
...
inline bool filter_join(action::context & ctx, const emel::text::jinja::value & input,
                        const call_args & args, emel::text::jinja::value & out) noexcept {
  call_args combined = args;
  if (combined.pos_count >= k_max_call_args) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, 0);
...
inline bool test_defined(action::context &, const emel::text::jinja::value & input,
                         const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type != emel::text::jinja::value_type::undefined);
  return true;
}

inline bool test_undefined(action::context &, const emel::text::jinja::value & input,
                           const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::undefined);
  return true;
}

inline bool test_none(action::context &, const emel::text::jinja::value & input,
                      const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::none);
  return true;
}

inline bool test_string(action::context &, const emel::text::jinja::value & input,
                        const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::string);
  return true;
}

inline bool test_boolean(action::context &, const emel::text::jinja::value & input,
                         const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::boolean);
  return true;
}

inline bool test_number(action::context &, const emel::text::jinja::value & input,
                        const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(value_is_number(input));
  return true;
}

inline bool test_iterable(action::context &, const emel::text::jinja::value & input,
                          const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::array ||
                  input.type == emel::text::jinja::value_type::string ||
                  input.type == emel::text::jinja::value_type::object);
  return true;
}

inline bool test_mapping(action::context &, const emel::text::jinja::value & input,
                         const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::object);
  return true;
}

inline bool test_even(action::context &, const emel::text::jinja::value & input,
                      const call_args &, emel::text::jinja::value & out) noexcept {
  if (input.type != emel::text::jinja::value_type::integer) {
    out = make_bool(false);
    return true;
  }
...
inline bool test_odd(action::context &, const emel::text::jinja::value & input,
                     const call_args &, emel::text::jinja::value & out) noexcept {
  if (input.type != emel::text::jinja::value_type::integer) {
    out = make_bool(false);
    return true;
...
inline bool test_true(action::context &, const emel::text::jinja::value & input,
                      const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::boolean && input.bool_v);
  return true;
}

inline bool test_false(action::context &, const emel::text::jinja::value & input,
                       const call_args &, emel::text::jinja::value & out) noexcept {
  out = make_bool(input.type == emel::text::jinja::value_type::boolean && !input.bool_v);
  return true;
}
...
inline bool collect_call_args(action::context & ctx,
                              const emel::text::jinja::ast_list & args,
                              call_args & out,
                              const emel::text::jinja::object_value * globals,
                              render_io & io);

inline emel::text::jinja::value eval_expr(action::context & ctx,
                                   const emel::text::jinja::ast_node * node,
                                   const emel::text::jinja::object_value * globals,
                                   render_io & io);

inline bool render_statements(action::context & ctx,
                              const emel::text::jinja::ast_list & statements,
                              const emel::text::jinja::object_value * globals,
                              render_io & io,
                              bool allow_control,
                              control_flow & flow);
...
inline bool collect_call_args(action::context & ctx,
  ...
  for (const auto & arg : args) {
    ...
    if (auto * spread = dynamic_cast<emel::text::jinja::spread_expression *>(arg.get())) {
      emel::text::jinja::value val = eval_expr(ctx, spread->operand.get(), globals, io);
      if (val.type != emel::text::jinja::value_type::array) {
    ...
    if (auto * kw = dynamic_cast<emel::text::jinja::keyword_argument_expression *>(arg.get())) {
      auto * key_id = dynamic_cast<emel::text::jinja::identifier *>(kw->key.get());
      if (key_id == nullptr) {
        set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, kw->pos);
        return false;
      }
      emel::text::jinja::value val = eval_expr(ctx, kw->value.get(), globals, io);
    ...
    emel::text::jinja::value val = eval_expr(ctx, arg.get(), globals, io);
    if (!add_pos(out, val)) {
...
inline emel::text::jinja::value invoke_function(action::context & ctx,
                                          const emel::text::jinja::function_ref & fn,
                                          const call_args & args,
                                          const emel::text::jinja::object_value * globals,
                                          render_io & io) noexcept {
  if (fn.kind == emel::text::jinja::function_kind::builtin) {
    ...
    emel::text::jinja::value out = make_undefined();
    if (!entry->fn(ctx, args, out)) {
      return make_undefined("builtin_failed");
    }
    return out;
  }
  if (fn.kind == emel::text::jinja::function_kind::macro) {
    ...
    const emel::text::jinja::macro_statement * stmt = slot->macro.stmt;
    if (!push_scope(ctx)) {
    ...
    for (size_t i = 0; i < stmt->args.size(); ++i) {
      ...
      auto * param_id = dynamic_cast<emel::text::jinja::identifier *>(param.get());
      auto * param_kw = dynamic_cast<emel::text::jinja::keyword_argument_expression *>(param.get());
      std::string_view name = {};
    emel::text::jinja::value param_value = make_undefined();
      if (param_id != nullptr) {
        name = param_id->name;
      } else if (param_kw != nullptr) {
        auto * key_id = dynamic_cast<emel::text::jinja::identifier *>(param_kw->key.get());
        if (key_id != nullptr) {
          name = key_id->name;
        }
      }
      ...
      const emel::text::jinja::value * kw_val = find_kw(args, name);
      if (kw_val != nullptr) {
        param_value = *kw_val;
    ...
    emel::text::jinja::value out = make_string(
  ...
  if (fn.kind == emel::text::jinja::function_kind::caller) {
    const auto * slot = static_cast<const action::callable_slot *>(fn.data);
    ...
    if (slot->caller.params != nullptr) {
      for (size_t i = 0; i < slot->caller.params->size(); ++i) {
        ...
        emel::text::jinja::value arg_val = i < args.pos_count ? args.pos[i] : make_undefined();
...
inline emel::text::jinja::value eval_member(action::context & ctx,
                                     const emel::text::jinja::member_expression * node,
                                     const emel::text::jinja::object_value * globals,
                                     render_io & io) {
  emel::text::jinja::value object = eval_expr(ctx, node->object.get(), globals, io);
  if (object.type == emel::text::jinja::value_type::undefined) {
    return make_undefined("member_object");
  }
  emel::text::jinja::value property = make_undefined();
  if (node->computed) {
    if (auto * slice = dynamic_cast<emel::text::jinja::slice_expression *>(node->property.get())) {
      int64_t start = 0;
      int64_t stop = 0;
      int64_t step = 1;
      if (slice->start) {
        emel::text::jinja::value start_val = eval_expr(ctx, slice->start.get(), globals, io);
        if (start_val.type == emel::text::jinja::value_type::integer) {
          start = start_val.int_v;
        }
      }
      if (slice->stop) {
        emel::text::jinja::value stop_val = eval_expr(ctx, slice->stop.get(), globals, io);
        if (stop_val.type == emel::text::jinja::value_type::integer) {
          stop = stop_val.int_v;
        }
      } else if (object.type == emel::text::jinja::value_type::array) {
        stop = static_cast<int64_t>(object.array_v.count);
      } else if (object.type == emel::text::jinja::value_type::string) {
        stop = static_cast<int64_t>(object.string_v.view.size());
      }
      if (slice->step) {
        emel::text::jinja::value step_val = eval_expr(ctx, slice->step.get(), globals, io);
        if (step_val.type == emel::text::jinja::value_type::integer) {
          step = step_val.int_v;
        }
      }
      if (step == 0) {
      ...
      if (object.type == emel::text::jinja::value_type::string) {
      ...
      if (object.type == emel::text::jinja::value_type::array) {
    ...
    auto * prop_id = dynamic_cast<emel::text::jinja::identifier *>(node->property.get());
    if (prop_id == nullptr) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
  ...
  if (object.type == emel::text::jinja::value_type::array) {
    if (property.type == emel::text::jinja::value_type::integer) {
  ...
  if (object.type == emel::text::jinja::value_type::object) {
    if (property.type == emel::text::jinja::value_type::string) {
      const emel::text::jinja::object_entry * entry = find_object_entry(object.object_v, property.string_v.view);
      if (entry != nullptr) {
        return entry->val;
      }
    }
...
inline emel::text::jinja::value eval_expr(action::context & ctx,
  ...
  if (auto * literal = dynamic_cast<const emel::text::jinja::string_literal *>(node)) {
    return make_string(literal->value, false);
  }
  if (auto * ident = dynamic_cast<const emel::text::jinja::identifier *>(node)) {
  ...
  if (auto * literal = dynamic_cast<const emel::text::jinja::integer_literal *>(node)) {
    return make_int(literal->value);
  }
  if (auto * literal = dynamic_cast<const emel::text::jinja::float_literal *>(node)) {
    return make_float(literal->value);
  }
  if (auto * tuple = dynamic_cast<const emel::text::jinja::tuple_literal *>(node)) {
    std::array<emel::text::jinja::value, action::k_max_array_items> tmp = {};
    size_t count = 0;
  ...
  if (auto * arr = dynamic_cast<const emel::text::jinja::array_literal *>(node)) {
    std::array<emel::text::jinja::value, action::k_max_array_items> tmp = {};
    size_t count = 0;
  ...
  if (auto * obj = dynamic_cast<const emel::text::jinja::object_literal *>(node)) {
    std::array<emel::text::jinja::object_entry, action::k_max_object_entries> tmp = {};
    size_t count = 0;
  ...
  if (auto * unary = dynamic_cast<const emel::text::jinja::unary_expression *>(node)) {
    emel::text::jinja::value operand = eval_expr(ctx, unary->operand.get(), globals, io);
    if (unary->op.value == "not") {
      return make_bool(!value_is_truthy(operand));
    }
    if (unary->op.value == "+") {
      if (operand.type == emel::text::jinja::value_type::integer) {
        return operand;
      }
      if (operand.type == emel::text::jinja::value_type::floating) {
        return operand;
      }
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
  ...
  if (auto * binary = dynamic_cast<const emel::text::jinja::binary_expression *>(node)) {
    const std::string_view op = binary->op.value;
    if (op == "or") {
      emel::text::jinja::value left = eval_expr(ctx, binary->left.get(), globals, io);
    ...
    emel::text::jinja::value left = eval_expr(ctx, binary->left.get(), globals, io);
    emel::text::jinja::value right = eval_expr(ctx, binary->right.get(), globals, io);
    if (op == "+") {
      if (value_is_number(left) && value_is_number(right)) {
        const double lv = left.type == emel::text::jinja::value_type::integer ?
          static_cast<double>(left.int_v) : left.float_v;
        const double rv = right.type == emel::text::jinja::value_type::integer ?
          static_cast<double>(right.int_v) : right.float_v;
        if (left.type == emel::text::jinja::value_type::integer && right.type == emel::text::jinja::value_type::integer) {
    ...
    if (op == "*") {
      ...
      if (left.type == emel::text::jinja::value_type::string && right.type == emel::text::jinja::value_type::integer) {
    ...
    if (op == "%") {
      if (left.type != emel::text::jinja::value_type::integer ||
          right.type != emel::text::jinja::value_type::integer) {
        set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
    ...
    if (op == "in" || op == "not in") {
      bool contains = false;
      if (right.type == emel::text::jinja::value_type::array) {
      ...
      } else if (right.type == emel::text::jinja::value_type::object) {
      ...
      } else if (right.type == emel::text::jinja::value_type::string &&
                 left.type == emel::text::jinja::value_type::string) {
        contains = right.string_v.view.find(left.string_v.view) != std::string_view::npos;
      } else if (right.type == emel::text::jinja::value_type::undefined) {
        contains = false;
      }
      if (op == "not in") {
        contains = !contains;
  ...
  if (auto * ternary = dynamic_cast<const emel::text::jinja::ternary_expression *>(node)) {
    emel::text::jinja::value test = eval_expr(ctx, ternary->test.get(), globals, io);
  ...
  if (auto * select = dynamic_cast<const emel::text::jinja::select_expression *>(node)) {
    emel::text::jinja::value test = eval_expr(ctx, select->test.get(), globals, io);
    if (value_is_truthy(test)) {
      return eval_expr(ctx, select->value.get(), globals, io);
    }
    return make_undefined("select");
  }
  if (auto * test_expr = dynamic_cast<const emel::text::jinja::test_expression *>(node)) {
    emel::text::jinja::value operand = eval_expr(ctx, test_expr->operand.get(), globals, io);
    std::string_view test_name = {};
    call_args args = {};
    if (auto * id = dynamic_cast<emel::text::jinja::identifier *>(test_expr->test.get())) {
      test_name = id->name;
    } else if (auto * call = dynamic_cast<emel::text::jinja::call_expression *>(test_expr->test.get())) {
      auto * id = dynamic_cast<emel::text::jinja::identifier *>(call->callee.get());
      if (id == nullptr) {
        set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
    ...
    emel::text::jinja::value result = make_bool(false);
  ...
  if (auto * filter_expr = dynamic_cast<const emel::text::jinja::filter_expression *>(node)) {
    emel::text::jinja::value operand = eval_expr(ctx, filter_expr->operand.get(), globals, io);
    std::string_view name = {};
    call_args args = {};
    if (auto * id = dynamic_cast<emel::text::jinja::identifier *>(filter_expr->filter.get())) {
      name = id->name;
    } else if (auto * call = dynamic_cast<emel::text::jinja::call_expression *>(filter_expr->filter.get())) {
      auto * id = dynamic_cast<emel::text::jinja::identifier *>(call->callee.get());
      if (id == nullptr) {
        set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
  ...
  if (auto * call = dynamic_cast<const emel::text::jinja::call_expression *>(node)) {
    emel::text::jinja::value callee = eval_expr(ctx, call->callee.get(), globals, io);
    call_args args = {};
    collect_call_args(ctx, call->args, args, globals, io);
    if (callee.type != emel::text::jinja::value_type::function) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
      return make_undefined("call_target");
    }
    return invoke_function(ctx, callee.func_v, args, globals, io);
  }
  if (auto * member = dynamic_cast<const emel::text::jinja::member_expression *>(node)) {
...
inline bool render_if(action::context & ctx,
                      const emel::text::jinja::if_statement * stmt,
                      const emel::text::jinja::object_value * globals,
                      render_io & io,
                      const bool allow_control,
                      control_flow & flow) {
  emel::text::jinja::value test = eval_expr(ctx, stmt->test.get(), globals, io);
...
inline bool bind_loop_var(action::context & ctx,
                          const emel::text::jinja::ast_node * loop_var,
                          const emel::text::jinja::value & item) {
  if (auto * id = dynamic_cast<const emel::text::jinja::identifier *>(loop_var)) {
    return set_object_value(ctx, ctx.scopes[ctx.scope_count - 1].locals, id->name, item);
  }
  if (auto * tuple = dynamic_cast<const emel::text::jinja::tuple_literal *>(loop_var)) {
    if (item.type != emel::text::jinja::value_type::array) {
    ...
    for (size_t i = 0; i < tuple->values.size(); ++i) {
      auto * id = dynamic_cast<emel::text::jinja::identifier *>(tuple->values[i].get());
      if (id == nullptr) {
        set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, loop_var->pos);
        return false;
      }
...
inline bool render_for(action::context & ctx,
                       const emel::text::jinja::for_statement * stmt,
                       const emel::text::jinja::object_value * globals,
                       render_io & io,
                       const bool allow_control,
                       control_flow & flow) {
  const emel::text::jinja::ast_node * iter_expr = stmt->iterable.get();
  const emel::text::jinja::ast_node * test_expr = nullptr;
  if (auto * select = dynamic_cast<const emel::text::jinja::select_expression *>(iter_expr)) {
    iter_expr = select->value.get();
    test_expr = select->test.get();
  }
  emel::text::jinja::value iterable = eval_expr(ctx, iter_expr, globals, io);
  if (iterable.type == emel::text::jinja::value_type::undefined) {
    iterable = make_array(ctx, nullptr, 0);
  }
  std::array<emel::text::jinja::value, action::k_max_array_items> items = {};
  size_t item_count = 0;
  if (iterable.type == emel::text::jinja::value_type::array) {
    for (size_t i = 0; i < iterable.array_v.count && item_count < items.size(); ++i) {
      items[item_count++] = iterable.array_v.items[i];
    }
  } else if (iterable.type == emel::text::jinja::value_type::object) {
    for (size_t i = 0; i < iterable.object_v.count && item_count < items.size(); ++i) {
      if (auto * tuple = dynamic_cast<const emel::text::jinja::tuple_literal *>(stmt->loop_var.get())) {
        if (tuple->values.size() >= 2) {
          emel::text::jinja::value tmp_items[2] = {
  ...
  std::array<emel::text::jinja::value, action::k_max_array_items> filtered = {};
  ...
  for (size_t i = 0; i < item_count && filtered_count < filtered.size(); ++i) {
    ...
    emel::text::jinja::value test_val = eval_expr(ctx, test_expr, globals, io);
...
inline bool render_set(action::context & ctx,
                       const emel::text::jinja::set_statement * stmt,
                       const emel::text::jinja::object_value * globals,
                       render_io & io) {
  emel::text::jinja::value rhs = make_undefined();
  if (!stmt->body.empty()) {
  ...
  if (auto * id = dynamic_cast<const emel::text::jinja::identifier *>(stmt->left.get())) {
    return set_object_value(ctx, ctx.scopes[ctx.scope_count - 1].locals, id->name, rhs);
  }
  if (auto * tuple = dynamic_cast<const emel::text::jinja::tuple_literal *>(stmt->left.get())) {
    if (rhs.type != emel::text::jinja::value_type::array) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, stmt->pos);
      return false;
    }
  ...
  if (auto * member = dynamic_cast<const emel::text::jinja::member_expression *>(stmt->left.get())) {
    if (member->computed) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, stmt->pos);
      return false;
    }
    auto * prop_id = dynamic_cast<emel::text::jinja::identifier *>(member->property.get());
    if (prop_id == nullptr) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, stmt->pos);
      return false;
    }
    emel::text::jinja::value target = eval_expr(ctx, member->object.get(), globals, io);
    if (target.type != emel::text::jinja::value_type::object) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, stmt->pos);
      return false;
    }
...
inline bool render_filter_statement(action::context & ctx,
                                    const emel::text::jinja::filter_statement * stmt,
                                    const emel::text::jinja::object_value * globals,
                                    render_io & io) {
  emel::text::jinja::value body_val = make_undefined();
  if (!begin_capture(ctx, io)) {
    return false;
  }
  control_flow flow = control_flow::none;
  ...
  std::string_view name = {};
  call_args args = {};
  if (auto * id = dynamic_cast<emel::text::jinja::identifier *>(stmt->filter_node.get())) {
    name = id->name;
  } else if (auto * call = dynamic_cast<emel::text::jinja::call_expression *>(stmt->filter_node.get())) {
    auto * id = dynamic_cast<emel::text::jinja::identifier *>(call->callee.get());
    if (id == nullptr) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, stmt->pos);
      return false;
  ...
  emel::text::jinja::value filtered = make_undefined();
...
inline bool render_call_statement(action::context & ctx,
                                  const emel::text::jinja::call_statement * stmt,
                                  const emel::text::jinja::object_value * globals,
                                  render_io & io) {
  auto * call_expr = dynamic_cast<emel::text::jinja::call_expression *>(stmt->call_expr.get());
  if (call_expr == nullptr) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, stmt->pos);
    return false;
  }
  emel::text::jinja::value callee = eval_expr(ctx, call_expr->callee.get(), globals, io);
  if (callee.type != emel::text::jinja::value_type::function) {
    set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, stmt->pos);
    return false;
  }
  ...
  slot.kind = emel::text::jinja::function_kind::caller;
  slot.caller.body = &stmt->body;
  slot.caller.params = &stmt->caller_args;
  emel::text::jinja::function_ref caller_ref;
  caller_ref.kind = emel::text::jinja::function_kind::caller;
  ...
  emel::text::jinja::value result = invoke_function(ctx, callee.func_v, args, globals, io);
  pop_scope(ctx);
...
inline bool render_statement(action::context & ctx,
  ...
  if (dynamic_cast<const emel::text::jinja::break_statement *>(node) != nullptr) {
    flow = control_flow::break_loop;
    return true;
  }
  if (dynamic_cast<const emel::text::jinja::continue_statement *>(node) != nullptr) {
    flow = control_flow::continue_loop;
    return true;
  }
  if (auto * stmt = dynamic_cast<const emel::text::jinja::if_statement *>(node)) {
    return render_if(ctx, stmt, globals, io, allow_control, flow);
  }
  if (auto * stmt = dynamic_cast<const emel::text::jinja::for_statement *>(node)) {
    return render_for(ctx, stmt, globals, io, allow_control, flow);
  }
  if (auto * stmt = dynamic_cast<const emel::text::jinja::set_statement *>(node)) {
    return render_set(ctx, stmt, globals, io);
  }
  if (auto * stmt = dynamic_cast<const emel::text::jinja::macro_statement *>(node)) {
    if (ctx.callable_count >= action::k_max_callables) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
      return false;
    }
    auto * name_id = dynamic_cast<emel::text::jinja::identifier *>(stmt->name.get());
    if (name_id == nullptr) {
      set_error(ctx, EMEL_ERR_INVALID_ARGUMENT, node->pos);
      return false;
    }
    action::callable_slot & slot = ctx.callables[ctx.callable_count++];
    slot.kind = emel::text::jinja::function_kind::macro;
    slot.macro.stmt = stmt;
    emel::text::jinja::function_ref ref;
    ref.kind = emel::text::jinja::function_kind::macro;
    ref.data = &slot;
    return set_object_value(ctx, ctx.scopes[ctx.scope_count - 1].locals, name_id->name, make_function(ref));
  }
  if (auto * stmt = dynamic_cast<const emel::text::jinja::filter_statement *>(node)) {
    return render_filter_statement(ctx, stmt, globals, io);
  }
  if (auto * stmt = dynamic_cast<const emel::text::jinja::call_statement *>(node)) {
    return render_call_statement(ctx, stmt, globals, io);
  }
  emel::text::jinja::value val = eval_expr(ctx, node, globals, io);
