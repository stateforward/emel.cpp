...
namespace emel::gbnf::nonterm_parser::action {
...
struct consume_definition_existing {
  void operator()(const rule_parser::event::parse_rules & ev,
    ...
    uint32_t rule_id = 0;
    (void)ctx.symbols.find(ev.flow.token.text, hash, rule_id);
    ctx.rule_defined[rule_id] = true;
    ev.flow.err = emel::error::cast(rule_parser::error::none);
    ev.flow.nonterm_rule_id = rule_id;
  }
};
...
struct consume_definition_new {
  void operator()(const rule_parser::event::parse_rules & ev,
    ...
    (void)ctx.symbols.insert(ev.flow.token.text, hash, rule_id);
    ctx.rule_defined[rule_id] = true;
    ev.flow.err = emel::error::cast(rule_parser::error::none);
    ev.flow.nonterm_rule_id = rule_id;
  }
