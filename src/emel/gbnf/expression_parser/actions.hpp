...
namespace emel::gbnf::expression_parser::action {
...
struct consume_identifier {
  void operator()(const rule_parser::event::parse_rules & ev,
    ...
    ev.flow.expression_kind = events::parse_kind::identifier;
  }
};
...
struct consume_non_identifier {
  void operator()(const rule_parser::event::parse_rules & ev,
                  const context &) const noexcept {
    ev.flow.err = emel::error::cast(rule_parser::error::none);
    ev.flow.expression_kind = events::parse_kind::non_identifier;
  }
};
...
struct dispatch_parse_failed {
  void operator()(const rule_parser::event::parse_rules & ev,
    ...
    ev.flow.expression_kind = events::parse_kind::unknown;
  }
};
