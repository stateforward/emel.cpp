...
inline bool has_insert_slot(const rule_parser::event::parse_rules & ev,
  ...
  for (uint32_t probes = 0; probes < slot_count; ++probes) {
    ...
    if (entry.hash == hash && entry.name == ev.ctx.token.text) {
      return true;
    }
    slot = (slot + 1u) & mask;
  }
...
struct mode_definition {
  bool operator()(const rule_parser::event::parse_rules & ev,
                  const action::context &) const noexcept {
    return ev.ctx.nonterm_mode == events::parse_mode::definition;
  }
};
...
struct mode_reference {
  bool operator()(const rule_parser::event::parse_rules & ev,
                  const action::context &) const noexcept {
    return ev.ctx.nonterm_mode == events::parse_mode::reference;
  }
};
...
struct definition_existing_valid {
  bool operator()(const rule_parser::event::parse_rules & ev,
    ...
    uint32_t id = 0;
    if (!ctx.symbols.find(ev.ctx.token.text, hash, id)) {
      return false;
    }
...
struct reference_existing_valid {
  bool operator()(const rule_parser::event::parse_rules & ev,
    ...
    const uint32_t hash = rule_parser::detail::symbol_table::hash_name(ev.ctx.token.text);
    uint32_t id = 0;
    return ctx.symbols.find(ev.ctx.token.text, hash, id);
  }
};
