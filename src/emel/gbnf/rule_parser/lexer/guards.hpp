...
inline bool has_scan_char(const event::scan_next & ev, const action::context & ctx) noexcept;
inline uint32_t scan_start(const event::scan_next & ev) noexcept;
inline char scan_char(const event::scan_next & ev) noexcept;

}  // namespace detail

struct valid_next {
  bool operator()(const event::scan_next & ev, const action::context &) const noexcept {
    return ev.request.on_done && ev.request.on_error;
  }
};

struct invalid_next {
  bool operator()(const event::scan_next & ev, const action::context & ctx) const noexcept {
...
struct valid_cursor_position {
  bool operator()(const event::scan_next & ev, const action::context & ctx) const noexcept {
    return valid_next{}(ev, ctx) &&
           ev.request.cursor.offset <= ev.request.cursor.input.size();
  }
};
...
struct has_remaining_input {
  bool operator()(const event::scan_next & ev, const action::context & ctx) const noexcept {
    return valid_cursor_position{}(ev, ctx) &&
           ev.request.cursor.offset < ev.request.cursor.input.size();
  }
};
...
inline bool has_scan_char(const event::scan_next & ev, const action::context & ctx) noexcept {
  return valid_cursor_position{}(ev, ctx) && ev.ctx.has_input;
}
...
struct starts_newline_crlf {
  bool operator()(const event::scan_next & ev, const action::context & ctx) const noexcept {
    ...
    return detail::has_prefix(ev.request.cursor.input, start, "\r\n");
  }
};
...
struct starts_rule_reference_negated_candidate {
  bool operator()(const event::scan_next & ev, const action::context & ctx) const noexcept {
    ...
    return has_bang && detail::has_prefix(ev.request.cursor.input, start + 1u, "<[");
  }
};
