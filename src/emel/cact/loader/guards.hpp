...
inline bool error_is(const emel::error::type runtime_err, const error expected) noexcept {
...
struct guard_bind_capacity_insufficient {
  bool operator()(const event::bind_runtime & ev, const action::context & ctx) const noexcept {
    return guard_bind_valid_request{}(ev, ctx) && !guard_bind_capacity_sufficient{}(ev, ctx);
  }
};
...
struct guard_parse_bound_capacity_sufficient {
  bool operator()(const event::parse_runtime & ev, const action::context & ctx) const noexcept {
    return guard_parse_has_bound_storage{}(ev, ctx) && ctx.tensors.size() >= ctx.probed.num_tensors;
  }
};

struct guard_parse_bound_capacity_insufficient {
  bool operator()(const event::parse_runtime & ev, const action::context & ctx) const noexcept {
    return guard_parse_has_bound_storage{}(ev, ctx) && !guard_parse_bound_capacity_sufficient{}(ev, ctx);
  }
};
