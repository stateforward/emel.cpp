...
inline bool prepare_graph_weights(context &ctx) noexcept {
  ...
  for (uint32_t i = 0u; i < bound.layer_count; ++i) {
    ...
    ok = ok && prepare_view(ctx, layer.q_proj, prepared.q_proj);
    ok = ok && prepare_view(ctx, layer.k_proj, prepared.k_proj);
    ok = ok && prepare_view(ctx, layer.v_proj, prepared.v_proj);
  ...
  for (uint32_t i = 0u; i < bound.engram_site_count; ++i) {
    ...
    ok = ok && prepare_view(ctx, site.tables, prepared.tables);
    ok = ok && prepare_view(ctx, site.key_proj, prepared.key_proj);
