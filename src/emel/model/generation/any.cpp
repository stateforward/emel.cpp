...
const data::tensor_record *
find_tensor_by_name(const data &model_data,
...
emel::error::type build_contract(const emel::model::data &model_data,
  ...
  if (resolved == nullptr || resolved->build_generation_contract == nullptr) {
    return emel::error::cast(emel::model::loader::error::model_invalid);
  }
  return resolved->build_generation_contract(model_data, contract_out);
}
...
bool bind_output_view(const emel::model::data &model_data,
                      ...
                      const bool allow_tied_output,
                      tensor_view &output_out) noexcept {
  if (bind_tensor_view(model_data, k_output_name, output_out)) {
    return true;
  }

  if (!allow_tied_output || token_embedding.tensor == nullptr) {
    return false;
  }
...
bool bind_block_tensor_view(const emel::model::data &model_data,
  ...
  return make_block_tensor_name(block_index, suffix, buffer, name) &&
         bind_tensor_view(model_data, name, view_out);
}
...
emel::error::type bind_attention_block(
  ...
  const bool common_ok =
      bind_block_tensor_view(model_data, block_index, "attn_norm.weight",
                             block_out.attention_norm) &&
      bind_block_tensor_view(model_data, block_index, "attn_q.weight",
                             block_out.attention_q) &&
      bind_block_tensor_view(model_data, block_index, "attn_k.weight",
                             block_out.attention_k) &&
      (!require_qk_norm ||
       bind_block_tensor_view(model_data, block_index, "attn_q_norm.weight",
                              block_out.attention_q_norm)) &&
      (!require_qk_norm ||
       bind_block_tensor_view(model_data, block_index, "attn_k_norm.weight",
                              block_out.attention_k_norm)) &&
      bind_block_tensor_view(model_data, block_index, "attn_output.weight",
                             block_out.attention_output) &&
      bind_block_tensor_view(model_data, block_index, "ffn_norm.weight",
                             block_out.feed_forward_norm) &&
      bind_block_tensor_view(model_data, block_index, "ffn_gate.weight",
                             block_out.feed_forward_gate) &&
      bind_block_tensor_view(model_data, block_index, "ffn_down.weight",
                             block_out.feed_forward_down) &&
      bind_block_tensor_view(model_data, block_index, "ffn_up.weight",
                             block_out.feed_forward_up);
  if (!common_ok) {
    block_out = {};
    return emel::error::cast(emel::model::loader::error::model_invalid);
  }

  if (use_shared_key_value) {
    block_out.attention_v = block_out.attention_k;
    return emel::error::cast(emel::model::loader::error::none);
  }

  if (!bind_block_tensor_view(model_data, block_index, "attn_v.weight",
                              block_out.attention_v)) {
    block_out = {};
    return emel::error::cast(emel::model::loader::error::model_invalid);
  }
...
emel::error::type bind_shortconv_block(
  ...
  const bool ok =
      ...
      bind_block_tensor_view(model_data, block_index, "ffn_up.weight",
                             block_out.feed_forward_up) &&
      bind_block_tensor_view(model_data, block_index, "shortconv.conv.weight",
                             block_out.shortconv_conv) &&
      bind_block_tensor_view(model_data, block_index,
                             "shortconv.in_proj.weight",
                             block_out.shortconv_in_proj) &&
      bind_block_tensor_view(model_data, block_index,
                             "shortconv.out_proj.weight",
                             block_out.shortconv_out_proj);
  if (!ok) {
    block_out = {};
    return emel::error::cast(emel::model::loader::error::model_invalid);
  }
...
emel::error::type complete_contract(contract &contract_out) noexcept {
  if (contract_out.execution.model == nullptr ||
      ...
      contract_out.topology.execution != &contract_out.execution) {
    return emel::error::cast(emel::model::loader::error::invalid_request);
  }
  ...
  contract_out.quantized_audit =
      build_quantized_path_audit(contract_out.execution);
  return emel::error::cast(emel::model::loader::error::none);
}
