...
using block_view = detail::block_view;
using execution_view = detail::execution_view;
using quantized_contract_kind = detail::quantized_contract_kind;
using quantized_path_audit = detail::quantized_path_audit;
using quantized_stage_audit = detail::quantized_stage_audit;
using quantized_stage_family = detail::quantized_stage_family;

inline emel::error::type build_execution_view(const emel::model::data & model_data,
                                              execution_view & view_out) noexcept {
  return detail::build_execution_view(model_data, view_out);
}
