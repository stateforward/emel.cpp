...
inline void reset_tensor_bind_events(
    event::tensor_phase_events &events) noexcept {
...
inline void
record_bind_done_event(
    void *object,
    const emel::model::tensor::events::bind_done &) noexcept {
...
inline void record_bind_error_event(
    void *object,
    const emel::model::tensor::events::bind_error &ev) noexcept {
  auto *events = static_cast<event::tensor_phase_events *>(object);
  events->bind_done.raised = false;
...
inline void reset_tensor_plan_events(
    event::tensor_phase_events &events) noexcept {
...
inline void
record_plan_done_event(
    void *object,
    const emel::model::tensor::events::plan_done &ev) noexcept {
  auto *events = static_cast<event::tensor_phase_events *>(object);
...
inline void record_plan_error_event(
    void *object,
    const emel::model::tensor::events::plan_error &ev) noexcept {
  auto *events = static_cast<event::tensor_phase_events *>(object);
  events->plan_done.raised = false;
  events->plan_done.effect_count = 0u;
...
inline void reset_tensor_apply_events(
    event::tensor_phase_events &events) noexcept {
...
inline void
record_apply_done_event(
    void *object,
    const emel::model::tensor::events::apply_done &) noexcept {
  auto *events = static_cast<event::tensor_phase_events *>(object);
...
inline void record_apply_error_event(
    void *object,
    const emel::model::tensor::events::apply_error &ev) noexcept {
  auto *events = static_cast<event::tensor_phase_events *>(object);
  events->apply_done.raised = false;
...
inline emel::error::type mask_if(const bool predicate) noexcept {
  return emel::error::type{0u} -
         static_cast<emel::error::type>(predicate);
}

inline emel::error::type map_tensor_error(
    const emel::error::type tensor_err) noexcept {
  ...
  const auto known_mask =
      mask_if(tensor_err == tensor_none) |
