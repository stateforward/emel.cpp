...
constexpr std::array<int32_t, 6> k_lfm2_attention_layers = {2,  5,  8,
                                                            10, 12, 14};
...
bool is_gemma4_shared_kv_layer(const data &model_data,
...
uint32_t block_tensor_count_for_architecture(
...
bool assign_tensor_view(const data &model_data, const std::string_view name,
...
bool assign_output_view(const data &model_data,
  ...
  if (!uses_tied_output_contract(architecture) ||
...
bool assign_output_norm_view(
    const data &model_data, const std::string_view architecture,
  ...
  return uses_lfm2_block_contract(architecture) &&
         assign_tensor_view(model_data, k_lfm2_output_norm_name,
                            output_norm_out);
}
...
llama::detail::quantized_contract_kind classify_stage_contract(
    ...
    const bool saw_applicable_tensor, const bool all_supported_quantized,
    const bool all_vector_approved, const bool any_f32) noexcept {
  ...
  if (is_vector_dequant_stage(family)) {
    return all_vector_approved
               ? llama::detail::quantized_contract_kind::
...
const data::tensor_record *
stage_tensor(const llama::detail::execution_view &execution,
             const llama::detail::block_view *block,
             const llama::detail::quantized_stage_family family) noexcept {
  switch (family) {
...
std::string_view
metadata_string_view(const data::metadata &metadata,
  ...
  if (length == 0u || begin + length > metadata.blob_bytes_used ||
      begin + length > metadata.blob.size()) {
    return {};
  }
...
bool is_omniembed_execution_architecture(
    const std::string_view architecture) noexcept {
  return emel::model::omniembed::detail::is_execution_architecture(
      architecture);
}
...
emel::error::type build_execution_view(const emel::model::data &model_data,
  ...
  if (!assign_tensor_view(model_data, k_token_embedding_name,
      ...
      !assign_output_view(model_data, architecture, view.token_embedding,
                          view.output)) {
    return emel::error::cast(emel::model::loader::error::model_invalid);
  }
