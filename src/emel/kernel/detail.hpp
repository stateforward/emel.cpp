...
inline bool can_run_get_rows(const request_type &request) noexcept {
  ...
  // The index scan below reads through src1.data, and the exec reads the
  // gathered table through src0.data and writes through dst.data, so the
  // guard must require storage for all three before deciding validity.
