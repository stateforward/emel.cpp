...
inline void execute_neon_unary_neg(const float *src, float *dst,
                                   const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
  uint64_t i = 0;
  for (; i + 4 <= count; i += 4) {
...
inline void execute_neon_unary_relu(const float *src, float *dst,
                                    const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
  uint64_t i = 0;
...
inline bool execute_neon_image_pointwise_f32(
  ...
  return false;
#else
  if (request.input == nullptr || request.packed_rhs == nullptr ||
      request.output == nullptr || request.pixel_count <= 0 ||
      request.input_channels <= 0 || request.output_channels <= 0 ||
      request.packed_rhs_cols != request.output_channels) {
    return false;
  }
  if constexpr (fuse_batch_norm) {
    if (request.batch_norm_scale == nullptr ||
        request.batch_norm_shift == nullptr) {
      return false;
    }
  }
  ...
  for (int32_t panel_col = 0; panel_col < n; panel_col += k_col_block) {
    ...
    const float *panel = request.packed_rhs + static_cast<size_t>(panel_col) *
    ...
    for (int32_t depth_base = 0; depth_base < k; depth_base += k_depth_block) {
      ...
      for (int32_t local_col = 0; local_col < vec_pair_cols;
           local_col += k_col_pair) {
        int32_t pixel_index = 0;
        for (; pixel_index + k_row_block <= request.pixel_count;
             pixel_index += k_row_block) {
          float *out0 =
              c +
              static_cast<size_t>(pixel_index + 0) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          float *out1 =
              c +
              static_cast<size_t>(pixel_index + 1) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          float *out2 =
              c +
              static_cast<size_t>(pixel_index + 2) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          float *out3 =
              c +
              static_cast<size_t>(pixel_index + 3) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          float *out4 =
              c +
              static_cast<size_t>(pixel_index + 4) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          float *out5 =
              c +
              static_cast<size_t>(pixel_index + 5) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          float *out6 =
              c +
              static_cast<size_t>(pixel_index + 6) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          float *out7 =
              c +
              static_cast<size_t>(pixel_index + 7) * static_cast<size_t>(n) +
              static_cast<size_t>(panel_col + local_col);
          ...
          const float *row0 =
              a +
              static_cast<size_t>(pixel_index + 0) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *row1 =
              a +
              static_cast<size_t>(pixel_index + 1) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *row2 =
              a +
              static_cast<size_t>(pixel_index + 2) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *row3 =
              a +
              static_cast<size_t>(pixel_index + 3) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *row4 =
              a +
              static_cast<size_t>(pixel_index + 4) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *row5 =
              a +
              static_cast<size_t>(pixel_index + 5) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *row6 =
              a +
              static_cast<size_t>(pixel_index + 6) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *row7 =
              a +
              static_cast<size_t>(pixel_index + 7) * static_cast<size_t>(k) +
              static_cast<size_t>(depth_base);
          const float *weights = panel +
                                 static_cast<size_t>(depth_base) *
          ...
          if constexpr (fuse_batch_norm) {
            if (last_depth_block) {
              const float *scale = request.batch_norm_scale +
                                   static_cast<size_t>(panel_col + local_col);
              const float *shift = request.batch_norm_shift +
                                   static_cast<size_t>(panel_col + local_col);
        ...
        for (; pixel_index < request.pixel_count; ++pixel_index) {
          ...
          if constexpr (fuse_batch_norm) {
            if (last_depth_block) {
              ...
              acc1 =
                  vmlaq_f32(vld1q_f32(shift + 4), acc1, vld1q_f32(scale + 4));
              if constexpr (apply_relu) {
      ...
      for (int32_t local_col = vec_pair_cols; local_col < vec_cols;
           local_col += k_col_vec) {
        int32_t pixel_index = 0;
        for (; pixel_index + k_row_block <= request.pixel_count;
             pixel_index += k_row_block) {
      ...
      for (int32_t local_col = vec_cols; local_col < panel_cols; ++local_col) {
        for (int32_t pixel_index = 0; pixel_index < request.pixel_count;
             ++pixel_index) {
          float acc = first_depth_block
                          ? 0.0f
                          : c[static_cast<size_t>(pixel_index) *
          ...
          if constexpr (fuse_batch_norm) {
            if (last_depth_block) {
              acc = acc * request.batch_norm_scale[static_cast<size_t>(
                              panel_col + local_col)] +
                    request.batch_norm_shift[static_cast<size_t>(panel_col +
                                                                 local_col)];
              if constexpr (apply_relu) {
...
inline bool execute_neon_image_depthwise_f32(
  ...
  return false;
#else
  if (request.input == nullptr || request.kernel_major == nullptr ||
      request.output == nullptr || request.input_spatial <= 0 ||
      request.output_spatial <= 0 || request.output_channels <= 0 ||
      request.kernel_h <= 0 || request.kernel_w <= 0 || request.stride <= 0) {
    return false;
  }
  ...
  for (int32_t oy = 0; oy < request.output_spatial; ++oy) {
    for (int32_t ox = 0; ox < request.output_spatial; ++ox) {
      float *output_pixel =
          request.output +
          static_cast<size_t>(oy * request.output_spatial + ox) *
        ...
        for (int32_t ky = 0; ky < request.kernel_h; ++ky) {
          ...
          for (int32_t kx = 0; kx < request.kernel_w; ++kx) {
            ...
            const float *kernel_weights =
                ...
                static_cast<size_t>(ky * request.kernel_w + kx) *
                    static_cast<size_t>(channels);
            acc0 = vmlaq_f32(
                acc0, vld1q_f32(input_pixel + static_cast<size_t>(channel)),
                vld1q_f32(kernel_weights + static_cast<size_t>(channel)));
            acc1 = vmlaq_f32(
                acc1, vld1q_f32(input_pixel + static_cast<size_t>(channel + 4)),
                vld1q_f32(kernel_weights + static_cast<size_t>(channel + 4)));
            acc2 = vmlaq_f32(
                acc2, vld1q_f32(input_pixel + static_cast<size_t>(channel + 8)),
          ...
          for (int32_t kx = 0; kx < request.kernel_w; ++kx) {
            ...
            const float *kernel_weights =
                ...
                static_cast<size_t>(ky * request.kernel_w + kx) *
                    static_cast<size_t>(channels);
            acc = vmlaq_f32(
                acc, vld1q_f32(input_pixel + static_cast<size_t>(channel)),
                vld1q_f32(kernel_weights + static_cast<size_t>(channel)));
          }
        }
...
template <class request_type>
inline bool can_use_neon(const request_type &request,
...
inline bool can_run_neon_mul_mat_q6_vector_request(
  ...
  return k != 0u && m != 0u && block_count != 0u &&
         block_count <= ::emel::kernel::detail::quant::MAX_Q8_K_BLOCKS &&
         request.src1.ne[0] == 1u && request.src1.ne[1] == k &&
         request.dst.ne[0] == 1u && request.dst.ne[1] == m &&
         ...
         ::emel::kernel::detail::dtype_code(request.dst.type) ==
             ::emel::kernel::detail::dtype_f32 &&
         request.src0.nb[0] == 1u && row_bytes != 0u &&
...
inline bool can_use_neon_mul_mat_q6_vector(const event::op_mul_mat &request,
                                           const bool neon_available) noexcept {
  return neon_available && can_run_neon_mul_mat_q6_vector_request(request);
}

inline bool can_run_neon_mul_mat_q6_vector_q8_rhs_request(
    const event::op_mul_mat &request) noexcept {
  const uint64_t k = request.src0.ne[0];
  const uint64_t m = request.src0.ne[1];
  const uint64_t block_count = k / ::emel::kernel::detail::quant::QK_K;
...
inline bool
can_use_neon_mul_mat_q6_vector_q8_rhs(const event::op_mul_mat &request,
                                      const bool neon_available) noexcept {
  return neon_available &&
         can_run_neon_mul_mat_q6_vector_q8_rhs_request(request);
}
...
inline bool neon_q4_0_vector_supported() noexcept {
#if defined(__aarch64__) && defined(__ARM_NEON) &&                             \
    defined(__ARM_FEATURE_DOTPROD)
  return true;
#else
...
inline bool neon_q4_1_vector_supported() noexcept {
#if defined(__aarch64__) && defined(__ARM_NEON) &&                             \
    defined(__ARM_FEATURE_DOTPROD)
  return true;
#else
...
inline bool can_run_neon_mul_mat_q4_0_vector_request(
    const event::op_mul_mat &request) noexcept {
  const uint64_t k = request.src0.ne[0];
  const uint64_t m = request.src0.ne[1];
  const uint64_t block_count = k / ::emel::kernel::detail::quant::QK4_0;
  const size_t row_bytes = ::emel::kernel::detail::quantized_row_storage_bytes(
      ::emel::kernel::detail::dtype_q4_0, k);
  return k != 0u && m != 0u && block_count != 0u &&
...
inline bool
can_use_neon_mul_mat_q4_0_vector(const event::op_mul_mat &request,
                                 const bool neon_available) noexcept {
  return neon_available && neon_q4_0_vector_supported() &&
         can_run_neon_mul_mat_q4_0_vector_request(request);
}

inline bool can_run_neon_mul_mat_q4_1_vector_request(
    const event::op_mul_mat &request) noexcept {
  const uint64_t k = request.src0.ne[0];
  const uint64_t m = request.src0.ne[1];
  const uint64_t block_count = k / ::emel::kernel::detail::quant::QK4_1;
  const size_t row_bytes = ::emel::kernel::detail::quantized_row_storage_bytes(
      ::emel::kernel::detail::dtype_q4_1, k);
  return k != 0u && m != 0u && block_count != 0u &&
         block_count <= ::emel::kernel::detail::quant::MAX_Q8_0_BLOCKS &&
         request.src1.ne[0] == 1u && request.src1.ne[1] == k &&
         request.dst.ne[0] == 1u && request.dst.ne[1] == m &&
...
inline bool
can_use_neon_mul_mat_q4_1_vector(const event::op_mul_mat &request,
                                 const bool neon_available) noexcept {
  return neon_available && neon_q4_1_vector_supported() &&
         can_run_neon_mul_mat_q4_1_vector_request(request);
}
...
inline bool
can_use_neon_mul_mat_q5_0_vector(const event::op_mul_mat &request,
                                 const bool neon_available) noexcept {
  return neon_available && neon_q5_0_vector_supported() &&
...
inline bool
can_use_neon_mul_mat_q8_0_vector(const event::op_mul_mat &request,
                                 const bool neon_available) noexcept {
  return neon_available && can_run_neon_mul_mat_q8_0_vector_request(request);
}

inline bool can_run_neon_mul_mat_q8_0_vector_q8_rhs_request(
    const event::op_mul_mat &request) noexcept {
  const uint64_t k = request.src0.ne[0];
  const uint64_t m = request.src0.ne[1];
  const uint64_t block_count = k / ::emel::kernel::detail::quant::QK8_0;
...
inline bool
can_use_neon_mul_mat_q8_0_vector_q8_rhs(const event::op_mul_mat &request,
                                        const bool neon_available) noexcept {
  return neon_available &&
         can_run_neon_mul_mat_q8_0_vector_q8_rhs_request(request);
}
...
inline bool
can_run_neon_mul_mat_q8_0_packed_request(const event::op_mul_mat &request,
  ...
  return k != 0u && m != 0u && block_count != 0u &&
         ...
         ::emel::kernel::detail::dtype_code(request.dst.type) ==
             ::emel::kernel::detail::dtype_f32 &&
         request.src0.nb[0] == 1u && group_bytes != 0u && rhs_row_bytes != 0u &&
...
inline bool
can_use_neon_mul_mat_q8_0_packed_bl4(const event::op_mul_mat &request,
                                     const bool neon_available) noexcept {
  return neon_available && neon_q8_0_packed_bl4_supported() &&
         can_run_neon_mul_mat_q8_0_packed_request(
             request, ::emel::kernel::detail::dtype_q8_0_x4_bl4);
}

inline bool
can_use_neon_mul_mat_q8_0_packed_bl8(const event::op_mul_mat &request,
                                     const bool neon_available) noexcept {
  return neon_available && neon_q8_0_packed_bl8_supported() &&
         can_run_neon_mul_mat_q8_0_packed_request(
...
inline bool can_use_neon_mul_mat_q8_0_packed_bl8_full_groups(
  ...
  return can_use_neon_mul_mat_q8_0_packed_bl8(request, neon_available) &&
         (request.src0.ne[1] % ::emel::kernel::detail::quant::Q8_0_X4_ROWS) ==
             0u;
}
...
inline bool can_use_neon_mul_mat_q8_0_packed_bl8_tail_safe(
    const event::op_mul_mat &request, const bool neon_available) noexcept {
  return can_use_neon_mul_mat_q8_0_packed_bl8(request, neon_available) &&
         !can_use_neon_mul_mat_q8_0_packed_bl8_full_groups(request,
                                                           neon_available);
}
...
inline bool neon_q4_vector_q8_rhs_supported() noexcept {
#if defined(__aarch64__) && defined(__ARM_NEON) &&                             \
    defined(__ARM_FEATURE_DOTPROD)
  return true;
#else
...
inline bool can_run_neon_mul_mat_q4_vector_packed_f32_rhs_request(
    const event::op_mul_mat &request, const uint8_t packed_dtype) noexcept {
  const uint64_t k = request.src0.ne[0];
  const uint64_t m = request.src0.ne[1];
  const uint64_t block_count = k / ::emel::kernel::detail::quant::QK_K;
...
inline bool can_run_neon_mul_mat_q4_vector_q8_rhs_request(
    const event::op_mul_mat &request) noexcept {
  const uint64_t k = request.src0.ne[0];
  const uint64_t m = request.src0.ne[1];
  const uint64_t block_count = k / ::emel::kernel::detail::quant::QK_K;
  const size_t row_bytes = ::emel::kernel::detail::quantized_row_storage_bytes(
      ::emel::kernel::detail::dtype_q4_k, k);
  const size_t rhs_row_bytes =
      ::emel::kernel::detail::quantized_row_storage_bytes(
          ::emel::kernel::detail::dtype_q8_k, k);
  return k != 0u && m != 0u && block_count != 0u &&
...
inline bool can_run_neon_mul_mat_q4_vector_f32_rhs_request(
    const event::op_mul_mat &request) noexcept {
  const uint64_t k = request.src0.ne[0];
  const uint64_t m = request.src0.ne[1];
  const uint64_t block_count = k / ::emel::kernel::detail::quant::QK_K;
...
inline bool
can_use_neon_mul_mat_q6_vector_packed(const event::op_mul_mat &request,
                                      const bool neon_available) noexcept {
...
inline bool can_use_neon_mul_mat_q4_vector_packed_f32_rhs_bl4(
    const event::op_mul_mat &request, const bool neon_available) noexcept {
  return neon_available && neon_q4_vector_packed_supported() &&
...
inline bool can_use_neon_mul_mat_q4_vector_packed_f32_rhs_bl8(
    const event::op_mul_mat &request, const bool neon_available) noexcept {
  return neon_available && neon_q4_vector_packed_supported() &&
         can_run_neon_mul_mat_q4_vector_packed_f32_rhs_request(
             request, ::emel::kernel::detail::dtype_q4_k_x8_bl8);
...
inline bool
can_use_neon_mul_mat_q4_vector_q8_rhs(const event::op_mul_mat &request,
                                      const bool neon_available) noexcept {
  return neon_available && neon_q4_vector_q8_rhs_supported() &&
         can_run_neon_mul_mat_q4_vector_q8_rhs_request(request);
}

inline bool
can_use_neon_mul_mat_q4_vector_f32_rhs(const event::op_mul_mat &request,
                                       const bool neon_available) noexcept {
  return neon_available && neon_q4_vector_q8_rhs_supported() &&
         can_run_neon_mul_mat_q4_vector_f32_rhs_request(request);
}
...
inline bool can_run_neon_mul_mat_argmax_q6_vector_prepared_q8_rhs_request(
  ...
  const size_t rhs_row_bytes =
      ::emel::kernel::detail::quantized_row_storage_bytes(
          ::emel::kernel::detail::dtype_q8_k, k);
  return request.index_out != nullptr && k != 0u && m != 0u &&
         request.src1.ne[0] == 1u && request.src1.ne[1] == k &&
         request.dst.ne[0] == 1u && request.dst.ne[1] == 1u &&
         request.src0.ne[2] == 1u && request.src0.ne[3] == 1u &&
         request.src1.ne[2] == 1u && request.src1.ne[3] == 1u &&
         request.dst.ne[2] == 1u && request.dst.ne[3] == 1u &&
         ::emel::kernel::detail::dtype_code(request.src0.type) ==
...
inline bool can_run_neon_mul_mat_argmax_q6_vector_q8_argmax_prepared_request(
  ...
  const uint64_t group_count =
      ::emel::kernel::detail::quant::packed_q6_k_x8_group_count(m);
  const size_t group_bytes = ::emel::kernel::detail::quant::
      argmax_prepared_q6_k_x8_q8_group_storage_bytes(k);
  const size_t rhs_row_bytes =
      ::emel::kernel::detail::quantized_row_storage_bytes(
          ::emel::kernel::detail::dtype_q8_k, k);
  return request.index_out != nullptr && k != 0u && m != 0u &&
...
inline bool can_run_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_request(
  ...
  return request.index_out != nullptr && k != 0u && m != 0u &&
         ...
         request.src0.nb[2] == group_bytes * group_count &&
         request.src0.nb[3] == request.src0.nb[2] &&
         ::emel::kernel::aarch64::detail::is_dense_contiguous(request.src1) &&
         ::emel::kernel::aarch64::detail::is_dense_contiguous(request.dst);
}
...
inline bool can_use_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_bl4(
    const event::op_mul_mat_argmax &request,
    const bool neon_available) noexcept {
  return neon_available && neon_q4_vector_packed_supported() &&
...
inline bool can_use_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_bl8(
    const event::op_mul_mat_argmax &request,
    const bool neon_available) noexcept {
  return neon_available && neon_q4_vector_packed_supported() &&
         can_run_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_request(
...
inline bool can_use_neon_flash_attn_ext_f16kv_one_chunk(
    const request_type &request, const bool neon_available) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
  (void)neon_available;
  return false;
...
inline bool can_run_neon_flash_attn_ext_f16kv_one_chunk_request(
    const request_type &request, const bool neon_available,
...
inline const float *tensor_row_ptr(const tensor_type &tensor,
  ...
  return reinterpret_cast<const float *>(base + row1 * tensor.nb[1] +
                                         row2 * tensor.nb[2]);
}
...
inline float *tensor_row_ptr_mut(const tensor_type &tensor, const uint64_t row1,
  ...
  return reinterpret_cast<float *>(base + row1 * tensor.nb[1] +
                                   row2 * tensor.nb[2]);
}

inline void scale_f32_neon(float *data, const float scale,
                           const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
...
inline void axpy_f32_neon(float *dst, const float *src, const float alpha,
                          const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
...
inline void convert_f32_to_f16_buffer_neon(const float *src, uint16_t *dst,
                                           const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
...
inline void scale_f16_buffer_neon(uint16_t *data, const float scale,
                                  const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
#if defined(__ARM_FEATURE_FP16_VECTOR_ARITHMETIC)
  ...
  for (; idx + 32u <= count; idx += 32u) {
    ...
    const float16x8_t data2 =
        vreinterpretq_f16_u16(vld1q_u16(data + idx + 16u));
    const float16x8_t data3 =
        vreinterpretq_f16_u16(vld1q_u16(data + idx + 24u));
    vst1q_u16(data + idx + 0u,
              vreinterpretq_u16_f16(vmulq_f16(data0, scale_v)));
    vst1q_u16(data + idx + 8u,
              vreinterpretq_u16_f16(vmulq_f16(data1, scale_v)));
    vst1q_u16(data + idx + 16u,
              vreinterpretq_u16_f16(vmulq_f16(data2, scale_v)));
    vst1q_u16(data + idx + 24u,
              vreinterpretq_u16_f16(vmulq_f16(data3, scale_v)));
  }
  for (; idx + 8u <= count; idx += 8u) {
...
inline void axpy_f16_buffer_neon(uint16_t *dst, const uint16_t *src,
                                 const float alpha,
                                 const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
#if defined(__ARM_FEATURE_FP16_VECTOR_ARITHMETIC)
...
inline void convert_f16_buffer_to_f32_neon(const uint16_t *src, float *dst,
                                           const uint64_t count) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
#if defined(__ARM_FEATURE_FP16_VECTOR_ARITHMETIC)
  uint64_t idx = 0u;
...
inline void run_flash_attn_ext_f16kv_one_chunk_neon_unchecked(
  ...
  for (uint64_t head = 0; head < head_count; ++head) {
    ...
    for (uint64_t token = 0; token < kv_tokens; ++token) {
      ...
      const float score = dot_product_f16_f16_scores_neon(
...
template <class request_type>
inline bool run_flash_attn_ext_neon(
    const request_type &request, const bool neon_available,
    ::emel::kernel::detail::flash_attn_workspace &workspace) noexcept {
...
inline float dot_q2_k_q8_k_block_neon(
  ...
  for (uint64_t j = 0; j < (::emel::kernel::detail::quant::QK_K / 128); ++j) {
    ...
    }
    {
      ...
      q8 += 32;
      q2bytes.val[0] =
          vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[0], 2), m3));
      q2bytes.val[1] =
          vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[1], 2), m3));
      isum += vaddvq_s32(vdotq_s32(zero, q2bytes.val[0], q8bytes.val[0])) *
    ...
    }
    {
      ...
      q8 += 32;
      q2bytes.val[0] =
          vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[0], 4), m3));
      q2bytes.val[1] =
          vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[1], 4), m3));
      isum += vaddvq_s32(vdotq_s32(zero, q2bytes.val[0], q8bytes.val[0])) *
    ...
    }
    {
      ...
      q8 += 32;
      q2bytes.val[0] =
          vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[0], 6), m3));
      q2bytes.val[1] =
          vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(q2bits.val[1], 6), m3));
      isum += vaddvq_s32(vdotq_s32(zero, q2bytes.val[0], q8bytes.val[0])) *
...
inline float
dot_q2_k_q8_k_row_neon(const ::emel::kernel::detail::quant::block_q2_k *lhs,
...
inline float dot_q3_k_q8_k_block_neon(
  ...
  for (uint64_t j = 0; j < (::emel::kernel::detail::quant::QK_K / 128); ++j) {
    ...
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[0], q8bytes_1.val[0])) *
            scales[scale_index + 0];
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[1], q8bytes_1.val[1])) *
            scales[scale_index + 1];
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[2], q8bytes_1.val[2])) *
            scales[scale_index + 2];
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[3], q8bytes_1.val[3])) *
            scales[scale_index + 3];
    scale_index += 4;
    ...
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[0], q8bytes_2.val[0])) *
            scales[scale_index + 0];
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[1], q8bytes_2.val[1])) *
            scales[scale_index + 1];
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[2], q8bytes_2.val[2])) *
            scales[scale_index + 2];
    isum += vaddvq_s32(vdotq_s32(zero, q3bytes.val[3], q8bytes_2.val[3])) *
            scales[scale_index + 3];
    scale_index += 4;
...
inline float
dot_q3_k_q8_k_row_neon(const ::emel::kernel::detail::quant::block_q3_k *lhs,
                       const ::emel::kernel::detail::quant::block_q8_k *rhs,
                       const uint64_t block_count) noexcept {
#if !defined(__ARM_FEATURE_DOTPROD)
...
inline void
decode_q4_k_scales_words(const ::emel::kernel::detail::quant::block_q4_k &lhs,
...
inline void accumulate_q4_k_q8_k_block_neon(
  ...
  int32_t low_sum = 0;
  int32_t high_sum = 0;
  for (uint64_t group = 0; group < (::emel::kernel::detail::quant::QK_K / 64u);
       ++group) {
    const uint8x16x2_t q4bits = load_u8x16x2(q4);
    q4 += 32u;

    int8x16x2_t q8bytes = vld1q_s8_x2(q8);
    q8 += 32u;
    const int8x16_t q4low0 = vreinterpretq_s8_u8(vandq_u8(q4bits.val[0], m4b));
    const int8x16_t q4low1 = vreinterpretq_s8_u8(vandq_u8(q4bits.val[1], m4b));
    const int32x4_t low_dot =
        vdotq_s32(vdotq_s32(zero, q4low0, q8bytes.val[0]), q4low1,
                  q8bytes.val[1]);
    low_sum += vaddvq_s32(low_dot) *
               static_cast<int32_t>(scales[group * 2u + 0u]);

    q8bytes = vld1q_s8_x2(q8);
    q8 += 32u;
    const int8x16_t q4high0 =
        vreinterpretq_s8_u8(vshrq_n_u8(q4bits.val[0], 4));
    const int8x16_t q4high1 =
        vreinterpretq_s8_u8(vshrq_n_u8(q4bits.val[1], 4));
    const int32x4_t high_dot =
        vdotq_s32(vdotq_s32(zero, q4high0, q8bytes.val[0]), q4high1,
                  q8bytes.val[1]);
    high_sum += vaddvq_s32(high_dot) *
...
inline float
dot_q4_k_q8_k_row_neon(const ::emel::kernel::detail::quant::block_q4_k *lhs,
                       const ::emel::kernel::detail::quant::block_q8_k *rhs,
                       const uint64_t block_count) noexcept {
#if !defined(__ARM_FEATURE_DOTPROD)
...
inline void
dot_q4_k_q8_k_2rows_neon(const ::emel::kernel::detail::quant::block_q4_k *lhs0,
                         const ::emel::kernel::detail::quant::block_q4_k *lhs1,
                         const ::emel::kernel::detail::quant::block_q8_k *rhs,
                         const uint64_t block_count, float *out) noexcept {
#if !defined(__ARM_FEATURE_DOTPROD)
...
inline void
dot_q4_k_q8_k_4rows_neon(const ::emel::kernel::detail::quant::block_q4_k *lhs0,
                         const ::emel::kernel::detail::quant::block_q4_k *lhs1,
                         const ::emel::kernel::detail::quant::block_q4_k *lhs2,
                         const ::emel::kernel::detail::quant::block_q4_k *lhs3,
                         const ::emel::kernel::detail::quant::block_q8_k *rhs,
                         const uint64_t block_count, float *out) noexcept {
#if !defined(__ARM_FEATURE_DOTPROD)
  out[0] = dot_q4_k_q8_k_row_neon(lhs0, rhs, block_count);
  out[1] = dot_q4_k_q8_k_row_neon(lhs1, rhs, block_count);
  out[2] = dot_q4_k_q8_k_row_neon(lhs2, rhs, block_count);
  out[3] = dot_q4_k_q8_k_row_neon(lhs3, rhs, block_count);
#else
  out[0] = dot_q4_k_q8_k_row_neon(lhs0, rhs, block_count);
  out[1] = dot_q4_k_q8_k_row_neon(lhs1, rhs, block_count);
  out[2] = dot_q4_k_q8_k_row_neon(lhs2, rhs, block_count);
...
inline void decode_q4_k_x8_6bit_scales_neon(const uint8_t *scales_in,
  ...
  constexpr uint32_t kmask2 = 0x0f0f0f0fu;
  constexpr uint32_t kmask3 = 0x03030303u;
  std::array<uint32_t, 3> sm = {};
  std::memcpy(sm.data(), scales_in, 12u);

  const uint32_t mins_0_3 = sm[1] & kmask1;
  const uint32_t mins_4_7 =
      ((sm[2] >> 4u) & kmask2) | (((sm[1] >> 6u) & kmask3) << 4u);
  const uint32x2_t mins_u32 = {mins_0_3, mins_4_7};
  mins_out = vreinterpretq_s16_u16(vmovl_u8(vreinterpret_u8_u32(mins_u32)));

  std::array<uint32_t, 2> scales_u32 = {};
  scales_u32[0] = sm[0] & kmask1;
  scales_u32[1] =
      (sm[2] & kmask2) | (((sm[0] >> 6u) & kmask3) << 4u);
...
#else
  constexpr uint64_t col_groups =
      ::emel::kernel::detail::quant::Q4_K_X8_ROWS / 4u;
  const uint8x16_t m4b = vdupq_n_u8(0x0fu);
    ...
    for (uint64_t sb = 0; sb < (::emel::kernel::detail::quant::QK_K / 64u);
      ...
      for (uint64_t half = 0; half < 2u; ++half) {
        const uint8_t *prepared =
            q4_block.scales.data() + sb * 24u + half * 12u;
        decode_q4_k_x8_6bit_scales_neon(prepared, q4sb_mins[half],
                                        q4sb_scales[half]);
      }
      ...
      for (uint64_t group = 0; group < col_groups; ++group) {
        std::array<uint8x16_t, ::emel::kernel::detail::quant::Q4_K_X8_ROWS>
            q4_cols = {};
        for (uint64_t row = 0; row < q4_cols.size(); ++row) {
          q4_cols[row] = vld1q_u8(q4_block.qs.data() +
                                  sb * ::emel::kernel::detail::quant::QK_K +
                                  row * 32u + 16u * group);
        }

        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[0], m4b)),
            q8_qs[0], 0);
        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[1], m4b)),
            q8_qs[0], 1);
        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[2], m4b)),
            q8_qs[0], 2);
        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[3], m4b)),
            q8_qs[0], 3);
        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[4], m4b)),
            q8_qs[1], 0);
        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[5], m4b)),
            q8_qs[1], 1);
        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[6], m4b)),
            q8_qs[1], 2);
        acc_lo[group] = vdotq_laneq_s32(
            acc_lo[group], vreinterpretq_s8_u8(vandq_u8(q4_cols[7], m4b)),
            q8_qs[1], 3);

        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[0], 4)),
            q8_qs[2], 0);
        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[1], 4)),
            q8_qs[2], 1);
        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[2], 4)),
            q8_qs[2], 2);
        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[3], 4)),
            q8_qs[2], 3);
        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[4], 4)),
            q8_qs[3], 0);
        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[5], 4)),
            q8_qs[3], 1);
        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[6], 4)),
            q8_qs[3], 2);
        acc_hi[group] = vdotq_laneq_s32(
            acc_hi[group], vreinterpretq_s8_u8(vshrq_n_u8(q4_cols[7], 4)),
            q8_qs[3], 3);
      }
      ...
      const float32x4_t sumf_0123 =
          vcvtq_f32_s32(vaddq_s32(vmulq_s32(vmovl_s16(sc_0123_lo), acc_lo[0]),
      ...
      const float32x4_t sumf_4567 =
          vcvtq_f32_s32(vaddq_s32(vmulq_s32(vmovl_s16(sc_4567_lo), acc_lo[1]),
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    ...
    for (uint64_t sb = 0; sb < (::emel::kernel::detail::quant::QK_K / 64u);
      ...
      for (uint64_t pair = 0; pair < col_pairs; ++pair) {
        ...
        acc_lo[pair] =
            vdotq_s32(acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_0, m4b)),
                      q8_qs[0]);
        acc_lo[pair] =
            vdotq_s32(acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_1, m4b)),
                      q8_qs[1]);
        acc_lo[pair] =
            vdotq_s32(acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_2, m4b)),
                      q8_qs[2]);
        acc_lo[pair] =
            vdotq_s32(acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_3, m4b)),
                      q8_qs[3]);

        acc_hi[pair] =
            vdotq_s32(acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_0, 4)),
                      q8_qs[4]);
        acc_hi[pair] =
            vdotq_s32(acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_1, 4)),
                      q8_qs[5]);
        acc_hi[pair] =
            vdotq_s32(acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_2, 4)),
                      q8_qs[6]);
        acc_hi[pair] =
            vdotq_s32(acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_3, 4)),
                      q8_qs[7]);
      }
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    ...
    for (uint64_t sb = 0; sb < (::emel::kernel::detail::quant::QK_K / 64u);
      ...
      const uint8_t *q4_base =
          q4_block.qs.data() + sb * ::emel::kernel::detail::quant::QK_K;
      for (uint64_t rhs_row = 0; rhs_row < rhs_rows; ++rhs_row) {
        const int8_t *q8_base =
            rhs_rows_ptrs[rhs_row][block].qs.data() + sb * 64u;
        std::array<int8x16_t, 8> q8_qs = {};
        for (uint64_t i = 0; i < q8_qs.size(); ++i) {
          q8_qs[i] = vreinterpretq_s8_s64(vld1q_dup_s64(
              reinterpret_cast<const int64_t *>(q8_base + i * 8u)));
        }
        ...
        for (uint64_t pair = 0; pair < col_pairs; ++pair) {
          ...
          acc_lo[pair] = vdotq_s32(
              acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_pair_0, m4b)),
              q8_qs[0]);
          acc_lo[pair] = vdotq_s32(
              acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_pair_1, m4b)),
              q8_qs[1]);
          acc_lo[pair] = vdotq_s32(
              acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_pair_2, m4b)),
              q8_qs[2]);
          acc_lo[pair] = vdotq_s32(
              acc_lo[pair], vreinterpretq_s8_u8(vandq_u8(q4_qs_pair_3, m4b)),
              q8_qs[3]);

          acc_hi[pair] = vdotq_s32(
              acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_pair_0, 4)),
              q8_qs[4]);
          acc_hi[pair] = vdotq_s32(
              acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_pair_1, 4)),
              q8_qs[5]);
          acc_hi[pair] = vdotq_s32(
              acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_pair_2, 4)),
              q8_qs[6]);
          acc_hi[pair] = vdotq_s32(
              acc_hi[pair], vreinterpretq_s8_u8(vshrq_n_u8(q4_qs_pair_3, 4)),
              q8_qs[7]);
        }
...
inline void dot_q4_k_x8_q8_k_group_bl8_x8_neon(
    ...
    const uint64_t block_count, float *out0, float *out1, float *out2,
    float *out3, float *out4, float *out5, float *out6, float *out7) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  dot_q4_k_x8_q8_k_group_bl8_neon(lhs, rhs0, block_count, out0);
  dot_q4_k_x8_q8_k_group_bl8_neon(lhs, rhs1, block_count, out1);
  ...
  const std::array<float *, rhs_rows> out_rows{out0, out1, out2, out3,
                                               out4, out5, out6, out7};
  for (uint64_t rhs_row = 0; rhs_row < rhs_rows; ++rhs_row) {
...
inline float dot_q6_k_q8_k_block_neon(
  ...
  for (uint64_t j = 0; j < (::emel::kernel::detail::quant::QK_K / 128); ++j) {
    ...
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[0], q8bytes_1.val[0]),
                      static_cast<int32_t>(scale[0]));
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[1], q8bytes_1.val[1]),
                      static_cast<int32_t>(scale[1]));
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[2], q8bytes_1.val[2]),
                      static_cast<int32_t>(scale[2]));
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[3], q8bytes_1.val[3]),
                      static_cast<int32_t>(scale[3]));
    scale += 4;
    ...
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[0], q8bytes_2.val[0]),
                      static_cast<int32_t>(scale[0]));
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[1], q8bytes_2.val[1]),
                      static_cast<int32_t>(scale[1]));
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[2], q8bytes_2.val[2]),
                      static_cast<int32_t>(scale[2]));
    acc = vmlaq_n_s32(acc, vdotq_s32(zero, q6bytes.val[3], q8bytes_2.val[3]),
                      static_cast<int32_t>(scale[3]));
    scale += 4;
  }
...
inline float
dot_q6_k_q8_k_row_neon(const ::emel::kernel::detail::quant::block_q6_k *lhs,
                       const ::emel::kernel::detail::quant::block_q8_k *rhs,
                       const uint64_t block_count) noexcept {
  float sum = 0.0f;
  for (uint64_t block = 0; block < block_count; ++block) {
...
inline int32_t dot_q8_0_q8_0_block_sum_neon(
  ...
  const int32x4_t acc =
      vaddq_s32(vdotq_s32(zero, lhs0, rhs0), vdotq_s32(zero, lhs1, rhs1));
#else
  const int8x16_t lhs0 = vld1q_s8(lhs.qs.data());
  const int8x16_t rhs0 = vld1q_s8(rhs.qs.data());
  const int8x16_t lhs1 = vld1q_s8(lhs.qs.data() + 16);
  ...
  const int32x4_t acc =
      vaddq_s32(vaddq_s32(vpaddlq_s16(prod0_lo), vpaddlq_s16(prod0_hi)),
...
inline void
decode_q5_0_block_neon(const ::emel::kernel::detail::quant::block_q5_0 &block,
                       int8x16_t &low_out, int8x16_t &high_out) noexcept {
  ...
  const uint8x16_t low_mask =
      vcombine_u8(load_q5_0_high_bit_mask_u8(block.qh[0]),
                  load_q5_0_high_bit_mask_u8(block.qh[1]));
  const uint8x16_t high_mask =
      vcombine_u8(load_q5_0_high_bit_mask_u8(block.qh[2]),
                  load_q5_0_high_bit_mask_u8(block.qh[3]));
  const int8x16_t bias = vdupq_n_s8(16);
  low_out =
      vsubq_s8(vreinterpretq_s8_u8(vorrq_u8(low_nibbles, low_mask)), bias);
  high_out =
...
#else
    const int16x8_t prod0_lo =
        vmull_s8(vget_low_s8(lhs_lo), vget_low_s8(rhs_lo));
    const int16x8_t prod0_hi =
        vmull_s8(vget_high_s8(lhs_lo), vget_high_s8(rhs_lo));
    const int16x8_t prod1_lo =
        vmull_s8(vget_low_s8(lhs_hi), vget_low_s8(rhs_hi));
    const int16x8_t prod1_hi =
        vmull_s8(vget_high_s8(lhs_hi), vget_high_s8(rhs_hi));
    const int32x4_t acc =
        vaddq_s32(vaddq_s32(vpaddlq_s16(prod0_lo), vpaddlq_s16(prod0_hi)),
                  vaddq_s32(vpaddlq_s16(prod1_lo), vpaddlq_s16(prod1_hi)));
#endif
...
inline void
dot_q5_0_q8_0_4rows_neon(const ::emel::kernel::detail::quant::block_q5_0 *row0,
...
#if !(defined(__aarch64__) && defined(__ARM_NEON))
  out[0] =
      ::emel::kernel::detail::dot_q5_0_q8_0_row_scalar(row0, rhs, block_count);
  out[1] =
      ::emel::kernel::detail::dot_q5_0_q8_0_row_scalar(row1, rhs, block_count);
  out[2] =
      ::emel::kernel::detail::dot_q5_0_q8_0_row_scalar(row2, rhs, block_count);
  out[3] =
      ::emel::kernel::detail::dot_q5_0_q8_0_row_scalar(row3, rhs, block_count);
#else
  float sum0 = 0.0f;
  float sum1 = 0.0f;
  float sum2 = 0.0f;
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
...
#else
    const auto accumulate = [&](const int8x16_t lhs_lo,
                                const int8x16_t lhs_hi) noexcept {
      const int16x8_t prod0_lo =
          vmull_s8(vget_low_s8(lhs_lo), vget_low_s8(rhs_lo));
      const int16x8_t prod0_hi =
          vmull_s8(vget_high_s8(lhs_lo), vget_high_s8(rhs_lo));
      ...
      const int16x8_t prod1_hi =
          vmull_s8(vget_high_s8(lhs_hi), vget_high_s8(rhs_hi));
      return vaddq_s32(vaddq_s32(vpaddlq_s16(prod0_lo), vpaddlq_s16(prod0_hi)),
                       vaddq_s32(vpaddlq_s16(prod1_lo), vpaddlq_s16(prod1_hi)));
    };
    ...
    sum0 += static_cast<float>(horizontal_sum_s32_neon(acc0)) *
            (::emel::kernel::detail::quant::fp16_to_fp32(row0[block].d) *
             rhs_scale);
    sum1 += static_cast<float>(horizontal_sum_s32_neon(acc1)) *
            (::emel::kernel::detail::quant::fp16_to_fp32(row1[block].d) *
             rhs_scale);
    sum2 += static_cast<float>(horizontal_sum_s32_neon(acc2)) *
            (::emel::kernel::detail::quant::fp16_to_fp32(row2[block].d) *
             rhs_scale);
    sum3 += static_cast<float>(horizontal_sum_s32_neon(acc3)) *
            (::emel::kernel::detail::quant::fp16_to_fp32(row3[block].d) *
             rhs_scale);
  }
...
inline float
dot_q4_0_q8_0_row_neon(const ::emel::kernel::detail::quant::block_q4_0 *lhs,
                       const ::emel::kernel::detail::quant::block_q8_0 *rhs,
                       const uint64_t block_count) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON))
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    const uint8x16_t qs = vld1q_u8(lhs[block].qs.data());
    const int8x16_t lhs_lo =
        vsubq_s8(vreinterpretq_s8_u8(vandq_u8(qs, low_mask)), bias);
    const int8x16_t lhs_hi =
        vsubq_s8(vreinterpretq_s8_u8(vshrq_n_u8(qs, 4u)), bias);
    const int8x16_t rhs_lo = vld1q_s8(rhs[block].qs.data());
    const int8x16_t rhs_hi = vld1q_s8(rhs[block].qs.data() + 16u);
#if defined(__ARM_FEATURE_DOTPROD)
    int32x4_t acc = vdupq_n_s32(0);
...
inline void
dot_q4_0_q8_0_4rows_neon(const ::emel::kernel::detail::quant::block_q4_0 *row0,
                         const ::emel::kernel::detail::quant::block_q4_0 *row1,
                         const ::emel::kernel::detail::quant::block_q4_0 *row2,
                         const ::emel::kernel::detail::quant::block_q4_0 *row3,
                         const ::emel::kernel::detail::quant::block_q8_0 *rhs,
                         const uint64_t block_count, float *out) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON))
  out[0] =
      ::emel::kernel::detail::dot_q4_0_q8_0_row_scalar(row0, rhs, block_count);
  out[1] =
      ::emel::kernel::detail::dot_q4_0_q8_0_row_scalar(row1, rhs, block_count);
  out[2] =
      ::emel::kernel::detail::dot_q4_0_q8_0_row_scalar(row2, rhs, block_count);
  out[3] =
      ::emel::kernel::detail::dot_q4_0_q8_0_row_scalar(row3, rhs, block_count);
#else
  float sum0 = 0.0f;
  float sum1 = 0.0f;
  float sum2 = 0.0f;
  ...
  const auto decode = [&](const ::emel::kernel::detail::quant::block_q4_0 &blk,
                          int8x16_t &lo, int8x16_t &hi) noexcept {
    const uint8x16_t qs = vld1q_u8(blk.qs.data());
    lo = vsubq_s8(vreinterpretq_s8_u8(vandq_u8(qs, low_mask)), bias);
    hi = vsubq_s8(vreinterpretq_s8_u8(vshrq_n_u8(qs, 4u)), bias);
  };

  for (uint64_t block = 0; block < block_count; ++block) {
    int8x16_t lhs0_lo = {}, lhs0_hi = {};
    int8x16_t lhs1_lo = {}, lhs1_hi = {};
    int8x16_t lhs2_lo = {}, lhs2_hi = {};
    int8x16_t lhs3_lo = {}, lhs3_hi = {};
    decode(row0[block], lhs0_lo, lhs0_hi);
    decode(row1[block], lhs1_lo, lhs1_hi);
    decode(row2[block], lhs2_lo, lhs2_hi);
    decode(row3[block], lhs3_lo, lhs3_hi);
    const int8x16_t rhs_lo = vld1q_s8(rhs[block].qs.data());
    const int8x16_t rhs_hi = vld1q_s8(rhs[block].qs.data() + 16u);
#if defined(__ARM_FEATURE_DOTPROD)
    int32x4_t acc0 = vdupq_n_s32(0);
...
inline int32_t horizontal_sum_s8_neon(const int8x16_t value) noexcept {
  ...
  return horizontal_sum_s32_neon(pair);
#else
  (void)value;
  return 0;
#endif
...
dot_q4_1_q8_0_row_neon(const ::emel::kernel::detail::quant::block_q4_1 *lhs,
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    const uint8x16_t qs = vld1q_u8(lhs[block].qs.data());
    const int8x16_t lhs_lo = vreinterpretq_s8_u8(vandq_u8(qs, low_mask));
    const int8x16_t lhs_hi = vreinterpretq_s8_u8(vshrq_n_u8(qs, 4u));
    const int8x16_t rhs_lo = vld1q_s8(rhs[block].qs.data());
    const int8x16_t rhs_hi = vld1q_s8(rhs[block].qs.data() + 16u);
#if defined(__ARM_FEATURE_DOTPROD)
    int32x4_t acc = vdupq_n_s32(0);
    ...
    const int32x4_t acc =
        vaddq_s32(vaddq_s32(vpaddlq_s16(prod0_lo), vpaddlq_s16(prod0_hi)),
                  vaddq_s32(vpaddlq_s16(prod1_lo), vpaddlq_s16(prod1_hi)));
#endif
    const int32_t int_dot = horizontal_sum_s32_neon(acc);
    const int32_t rhs_sum =
        horizontal_sum_s8_neon(rhs_lo) + horizontal_sum_s8_neon(rhs_hi);
    const float rhs_d =
        ::emel::kernel::detail::quant::fp16_to_fp32(rhs[block].d);
    sumf += rhs_d * (::emel::kernel::detail::quant::fp16_to_fp32(lhs[block].d) *
                         static_cast<float>(int_dot) +
...
inline void
dot_q4_1_q8_0_4rows_neon(const ::emel::kernel::detail::quant::block_q4_1 *row0,
                         const ::emel::kernel::detail::quant::block_q4_1 *row1,
                         const ::emel::kernel::detail::quant::block_q4_1 *row2,
                         const ::emel::kernel::detail::quant::block_q4_1 *row3,
                         const ::emel::kernel::detail::quant::block_q8_0 *rhs,
                         const uint64_t block_count, float *out) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON))
  out[0] =
      ::emel::kernel::detail::dot_q4_1_q8_0_row_scalar(row0, rhs, block_count);
  out[1] =
      ::emel::kernel::detail::dot_q4_1_q8_0_row_scalar(row1, rhs, block_count);
  out[2] =
      ::emel::kernel::detail::dot_q4_1_q8_0_row_scalar(row2, rhs, block_count);
  out[3] =
      ::emel::kernel::detail::dot_q4_1_q8_0_row_scalar(row3, rhs, block_count);
#else
  float sum0 = 0.0f;
  float sum1 = 0.0f;
  float sum2 = 0.0f;
  ...
  const auto decode = [&](const ::emel::kernel::detail::quant::block_q4_1 &blk,
                          int8x16_t &lo, int8x16_t &hi) noexcept {
    const uint8x16_t qs = vld1q_u8(blk.qs.data());
    lo = vreinterpretq_s8_u8(vandq_u8(qs, low_mask));
    hi = vreinterpretq_s8_u8(vshrq_n_u8(qs, 4u));
  };
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    ...
    const int32x4_t acc3 = accumulate(lhs3_lo, lhs3_hi);
#endif
    const int32_t int_dot0 = horizontal_sum_s32_neon(acc0);
    const int32_t int_dot1 = horizontal_sum_s32_neon(acc1);
    const int32_t int_dot2 = horizontal_sum_s32_neon(acc2);
    const int32_t int_dot3 = horizontal_sum_s32_neon(acc3);
    const int32_t rhs_sum =
        horizontal_sum_s8_neon(rhs_lo) + horizontal_sum_s8_neon(rhs_hi);
    const float rhs_d =
        ::emel::kernel::detail::quant::fp16_to_fp32(rhs[block].d);
    const float rhs_sum_f = static_cast<float>(rhs_sum);
    sum0 +=
        rhs_d * (::emel::kernel::detail::quant::fp16_to_fp32(row0[block].d) *
                     static_cast<float>(int_dot0) +
                 ::emel::kernel::detail::quant::fp16_to_fp32(row0[block].m) *
                     rhs_sum_f);
    sum1 +=
        rhs_d * (::emel::kernel::detail::quant::fp16_to_fp32(row1[block].d) *
                     static_cast<float>(int_dot1) +
                 ::emel::kernel::detail::quant::fp16_to_fp32(row1[block].m) *
                     rhs_sum_f);
    sum2 +=
        rhs_d * (::emel::kernel::detail::quant::fp16_to_fp32(row2[block].d) *
                     static_cast<float>(int_dot2) +
                 ::emel::kernel::detail::quant::fp16_to_fp32(row2[block].m) *
                     rhs_sum_f);
    sum3 +=
        rhs_d * (::emel::kernel::detail::quant::fp16_to_fp32(row3[block].d) *
                     static_cast<float>(int_dot3) +
                 ::emel::kernel::detail::quant::fp16_to_fp32(row3[block].m) *
                     rhs_sum_f);
  }
...
dot_q8_0_q8_0_4row_neon(const ::emel::kernel::detail::quant::block_q8_0 *row0,
                        ...
                        const ::emel::kernel::detail::quant::block_q8_0 *row2,
                        const ::emel::kernel::detail::quant::block_q8_0 *row3,
                        const ::emel::kernel::detail::quant::block_q8_0 *rhs,
                        const uint64_t block_count) noexcept {
#if defined(__ARM_FEATURE_DOTPROD)
  ...
  const int32x4_t zero = vdupq_n_s32(0);
  for (uint64_t block = 0; block < block_count; ++block) {
    const int8x16_t rhs0 = vld1q_s8(rhs[block].qs.data());
    const int8x16_t rhs1 = vld1q_s8(rhs[block].qs.data() + 16u);
    const int32x4_t acc0 =
        vaddq_s32(vdotq_s32(zero, vld1q_s8(row0[block].qs.data()), rhs0),
                  vdotq_s32(zero, vld1q_s8(row0[block].qs.data() + 16u), rhs1));
    const int32x4_t acc1 =
        vaddq_s32(vdotq_s32(zero, vld1q_s8(row1[block].qs.data()), rhs0),
                  vdotq_s32(zero, vld1q_s8(row1[block].qs.data() + 16u), rhs1));
    const int32x4_t acc2 =
        vaddq_s32(vdotq_s32(zero, vld1q_s8(row2[block].qs.data()), rhs0),
                  vdotq_s32(zero, vld1q_s8(row2[block].qs.data() + 16u), rhs1));
    const int32x4_t acc3 =
        vaddq_s32(vdotq_s32(zero, vld1q_s8(row3[block].qs.data()), rhs0),
                  vdotq_s32(zero, vld1q_s8(row3[block].qs.data() + 16u), rhs1));
    const float rhs_d =
        ::emel::kernel::detail::quant::fp16_to_fp32(rhs[block].d);
    const int32x4_t pair01 = vpaddq_s32(acc0, acc1);
    const int32x4_t pair23 = vpaddq_s32(acc2, acc3);
    const int32x4_t lane_sums = vpaddq_s32(pair01, pair23);
    float32x4_t scales = vdupq_n_f32(0.0f);
    scales = vsetq_lane_f32(
        ::emel::kernel::detail::quant::fp16_to_fp32(row0[block].d) * rhs_d,
        scales, 0);
    scales = vsetq_lane_f32(
        ::emel::kernel::detail::quant::fp16_to_fp32(row1[block].d) * rhs_d,
        scales, 1);
    scales = vsetq_lane_f32(
        ::emel::kernel::detail::quant::fp16_to_fp32(row2[block].d) * rhs_d,
        scales, 2);
    scales = vsetq_lane_f32(
        ::emel::kernel::detail::quant::fp16_to_fp32(row3[block].d) * rhs_d,
        scales, 3);
    sum = vmlaq_f32(sum, vcvtq_f32_s32(lane_sums), scales);
  }
  return sum;
#else
  const float lanes[4] = {
      dot_q8_0_q8_0_row_neon(row0, rhs, block_count),
      dot_q8_0_q8_0_row_neon(row1, rhs, block_count),
      dot_q8_0_q8_0_row_neon(row2, rhs, block_count),
      dot_q8_0_q8_0_row_neon(row3, rhs, block_count),
  };
  return vld1q_f32(lanes);
#endif
}

inline void store_q8_0_x4_results(float *dst, const uint64_t row_base,
...
inline int32_t
q6_k_sum_mins_neon(const ::emel::kernel::detail::quant::block_q6_k &lhs,
                   const int16x8_t q8sums0, const int16x8_t q8sums1) noexcept {
#if !defined(__ARM_FEATURE_DOTPROD)
  (void)lhs;
  (void)q8sums0;
  (void)q8sums1;
...
inline int32x4_t q6_k_lower_half_dot_accumulate_neon(
    const int32x4_t acc, const uint8x16x2_t &qhbits, const uint8x16x4_t &q6bits,
    const int8x16x4_t &q8bytes, const int8_t *scale) noexcept {
#if !defined(__ARM_FEATURE_DOTPROD)
  (void)qhbits;
  (void)q6bits;
  (void)q8bytes;
  ...
  const int8x16_t q6bytes3 =
      vreinterpretq_s8_u8(vorrq_u8(vandq_u8(q6bits.val[3], m4b), q6h3));
  int32x4_t next = acc;
  next = vmlaq_n_s32(next, vdotq_s32(zero, q6bytes0, q8bytes.val[0]),
                     static_cast<int32_t>(scale[0]));
  next = vmlaq_n_s32(next, vdotq_s32(zero, q6bytes1, q8bytes.val[1]),
                     static_cast<int32_t>(scale[1]));
  next = vmlaq_n_s32(next, vdotq_s32(zero, q6bytes2, q8bytes.val[2]),
                     static_cast<int32_t>(scale[2]));
  next = vmlaq_n_s32(next, vdotq_s32(zero, q6bytes3, q8bytes.val[3]),
                     static_cast<int32_t>(scale[3]));
  return next;
#endif
}
...
inline int32x4_t q6_k_upper_half_dot_accumulate_neon(
    const int32x4_t acc, const uint8x16x2_t &qhbits, const uint8x16x4_t &q6bits,
    const int8x16x4_t &q8bytes, const int8_t *scale) noexcept {
#if !defined(__ARM_FEATURE_DOTPROD)
  (void)qhbits;
...
dot_q6_k_q8_k_4rows_neon(const ::emel::kernel::detail::quant::block_q6_k *lhs0,
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    ...
    for (uint64_t j = 0; j < (::emel::kernel::detail::quant::QK_K / 128u);
         ++j) {
      const int8x16x4_t q8bytes_1 = load_s8x16x4(q8);
      q8 += 64;
      ...
      acc0 = q6_k_lower_half_dot_accumulate_neon(acc0, qhbits0, q6bits0,
                                                 q8bytes_1, scale0);
      acc1 = q6_k_lower_half_dot_accumulate_neon(acc1, qhbits1, q6bits1,
                                                 q8bytes_1, scale1);
      acc2 = q6_k_lower_half_dot_accumulate_neon(acc2, qhbits2, q6bits2,
                                                 q8bytes_1, scale2);
      acc3 = q6_k_lower_half_dot_accumulate_neon(acc3, qhbits3, q6bits3,
                                                 q8bytes_1, scale3);
      scale0 += 4;
      scale1 += 4;
      scale2 += 4;
      ...
      const int8x16x4_t q8bytes_2 = load_s8x16x4(q8);
      q8 += 64;
      acc0 = q6_k_upper_half_dot_accumulate_neon(acc0, qhbits0, q6bits0,
                                                 q8bytes_2, scale0);
      acc1 = q6_k_upper_half_dot_accumulate_neon(acc1, qhbits1, q6bits1,
                                                 q8bytes_2, scale1);
      acc2 = q6_k_upper_half_dot_accumulate_neon(acc2, qhbits2, q6bits2,
                                                 q8bytes_2, scale2);
      acc3 = q6_k_upper_half_dot_accumulate_neon(acc3, qhbits3, q6bits3,
                                                 q8bytes_2, scale3);
      scale0 += 4;
      scale1 += 4;
      scale2 += 4;
...
inline void dot_q6_k_x8_q8_k_group_neon(
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    ...
    const auto &q8_block = rhs[block];
    const float16x4_t q6_d_0_f16 =
        vreinterpret_f16_u16(vld1_u16(q6_block.d.data()));
    const float16x4_t q6_d_1_f16 =
        vreinterpret_f16_u16(vld1_u16(q6_block.d.data() + 4));
    ...
    for (int scale = 0; scale < 16; ++scale) {
      const int16x8_t scales = vmovl_s8(
          vld1_s8(q6_block.scales.data() + static_cast<size_t>(scale) * 8u));
    ...
    for (int half = 0; half < 2; ++half) {
      const uint8_t *ql_base =
          q6_block.ql.data() + static_cast<size_t>(half) * 512u;
      const uint8_t *qh_base =
          q6_block.qh.data() + static_cast<size_t>(half) * 256u;
      for (int sb = 0;
           sb < static_cast<int>(::emel::kernel::detail::quant::QK_K / 64u);
           ++sb) {
        const int8_t *q8_base_l = q8_block.qs.data() +
                                  static_cast<size_t>(half) * 128u +
        ...
        const int ql_off_base =
            sb * static_cast<int>(::emel::kernel::detail::quant::QK_K / 2u);
        ...
        for (int cp = 0; cp < col_pairs; ++cp) {
          ...
          const int8x16_t q6_l0 = vreinterpretq_s8_u8(vsliq_n_u8(
              vandq_u8(q6_qs_cp_0_l, m4b), vandq_u8(q6_qs_cp_0_h, mask_lo), 4));
          const int8x16_t q6_l1 = vreinterpretq_s8_u8(vsliq_n_u8(
              vandq_u8(q6_qs_cp_1_l, m4b), vandq_u8(q6_qs_cp_1_h, mask_lo), 4));
          const int8x16_t q6_h0 = vreinterpretq_s8_u8(
              vorrq_u8(vshrq_n_u8(q6_qs_cp_0_l, 4), q6_qs_cp_0_hh));
          const int8x16_t q6_h1 = vreinterpretq_s8_u8(
              vorrq_u8(vshrq_n_u8(q6_qs_cp_1_l, 4), q6_qs_cp_1_hh));
          ...
          const int32x2_t scale_vec_l = {
              static_cast<int32_t>(
                  q6_scales[static_cast<size_t>(scale_idx_l) * 8u +
          ...
          const int32x2_t scale_vec_h = {
              static_cast<int32_t>(
                  q6_scales[static_cast<size_t>(scale_idx_h) * 8u +
                            static_cast<size_t>(cp) * 2u + 0u]),
              static_cast<int32_t>(
                  q6_scales[static_cast<size_t>(scale_idx_h) * 8u +
                            static_cast<size_t>(cp) * 2u + 1u]),
    ...
    const float32x2_t w_01 =
        vmul_f32(vcvt_f32_s32(acc[0]), vget_low_f32(sb_scale_0));
    const float32x2_t w_23 =
        vmul_f32(vcvt_f32_s32(acc[1]), vget_high_f32(sb_scale_0));
    const float32x2_t w_45 =
        vmul_f32(vcvt_f32_s32(acc[2]), vget_low_f32(sb_scale_1));
    const float32x2_t w_67 =
        vmul_f32(vcvt_f32_s32(acc[3]), vget_high_f32(sb_scale_1));
...
inline void dot_q6_k_x8_q8_k_group_prepared_neon(
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    ...
    for (uint64_t scale = 0;
         scale < (::emel::kernel::detail::quant::QK_K / 16u); ++scale) {
      const int8x16_t q8_values =
          vld1q_s8(q8_block.qs.data() + static_cast<size_t>(scale) * 16u);
      for (uint64_t row = 0; row < ::emel::kernel::detail::quant::Q6_K_X8_ROWS;
           ++row) {
        ...
        const int8_t scale_value =
            q6_block.scales[static_cast<size_t>(scale) *
                                ::emel::kernel::detail::quant::Q6_K_X8_ROWS +
                            row];
        acc[row] =
            vmlaq_n_s32(acc[row], dot_acc, static_cast<int32_t>(scale_value));
      }
    }
    ...
    const float32x4_t block_scale_0 =
        vmulq_n_f32(vcvt_f32_f16(q6_d_0_f16), q8_d);
    const float32x4_t block_scale_1 =
        vmulq_n_f32(vcvt_f32_f16(q6_d_1_f16), q8_d);
    ...
    const float32x4_t scaled_0 =
        vmulq_f32(vcvtq_f32_s32(packed_acc_0), block_scale_0);
    const float32x4_t scaled_1 =
        vmulq_f32(vcvtq_f32_s32(packed_acc_1), block_scale_1);
...
inline void dot_q6_k_x8_q8_k_group_prepared_i8mm(
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    const auto &q6_block = lhs[block];
    const auto &q8_block = rhs[block];
    std::array<int32x4_t, ::emel::kernel::detail::quant::Q6_K_X8_ROWS / 2u>
        acc_pairs = {};
    ...
    for (uint64_t scale = 0;
      ...
      for (uint64_t pair = 0; pair < acc_pairs.size(); ++pair) {
        ...
        const int8x16_t lhs_high =
            vld1q_s8(q6_block.qs.data() + pair_base + 16u);
...
inline void dot_q6_k_x8_q8_k_group_prepared_i8mm_x4(
...
#else
  // Each mmla consumes a distinct RHS row pair {rowA seg | rowB seg}, so all
  // four mmla output lanes carry live results (the previous variant
  // duplicated one RHS row into both operand halves and discarded half the
  // mmla throughput). Integer dots are exact, and the per-block float fold
  // below is unchanged, so results stay bit-identical per RHS row.
  constexpr uint64_t rhs_rows = 4u;
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    const auto &q6_block = lhs[block];
    // acc_pairs[rp][pair] lanes: {c(2p)*rA, c(2p)*rB, c(2p+1)*rA, c(2p+1)*rB}.
    std::array<
        std::array<int32x4_t, ::emel::kernel::detail::quant::Q6_K_X8_ROWS / 2u>,
        rhs_pairs>
        acc_pairs = {};
    ...
    for (uint64_t scale = 0;
         scale < (::emel::kernel::detail::quant::QK_K / 16u); ++scale) {
      std::array<int8x16_t, rhs_pairs> q8_low_pair = {};
      std::array<int8x16_t, rhs_pairs> q8_high_pair = {};
      for (uint64_t rp = 0; rp < rhs_pairs; ++rp) {
        const int8x16_t q8_values_a =
            vld1q_s8(rhs_rows_ptrs[rp * 2u][block].qs.data() +
                     static_cast<size_t>(scale) * 16u);
        const int8x16_t q8_values_b =
            vld1q_s8(rhs_rows_ptrs[rp * 2u + 1u][block].qs.data() +
                     static_cast<size_t>(scale) * 16u);
        q8_low_pair[rp] =
            vcombine_s8(vget_low_s8(q8_values_a), vget_low_s8(q8_values_b));
        q8_high_pair[rp] =
            vcombine_s8(vget_high_s8(q8_values_a), vget_high_s8(q8_values_b));
      }

      for (uint64_t pair = 0;
           pair < (::emel::kernel::detail::quant::Q6_K_X8_ROWS / 2u); ++pair) {
        const uint64_t row0 = pair * 2u;
        const uint64_t row1 = row0 + 1u;
        const size_t pair_base =
            static_cast<size_t>(scale) *
        ...
        for (uint64_t rp = 0; rp < rhs_pairs; ++rp) {
          int32x4_t pair_dot = vmmlaq_s32(zero, lhs_low, q8_low_pair[rp]);
          pair_dot = vmmlaq_s32(pair_dot, lhs_high, q8_high_pair[rp]);
          acc_pairs[rp][pair] =
              vmlaq_s32(acc_pairs[rp][pair], pair_dot, scale_pair);
        }
      }
    }
    ...
    for (uint64_t rp = 0; rp < rhs_pairs; ++rp) {
      ...
      const float32x4_t block_scale_a_0 =
          vmulq_n_f32(q6_d_0, rhs_rows_ptrs[row_a][block].d);
      const float32x4_t block_scale_a_1 =
          vmulq_n_f32(q6_d_1, rhs_rows_ptrs[row_a][block].d);
      const float32x4_t block_scale_b_0 =
          vmulq_n_f32(q6_d_0, rhs_rows_ptrs[row_b][block].d);
      const float32x4_t block_scale_b_1 =
          vmulq_n_f32(q6_d_1, rhs_rows_ptrs[row_b][block].d);
      const int32x4_t packed_acc_a_0 =
          vuzp1q_s32(acc_pairs[rp][0], acc_pairs[rp][1]);
      const int32x4_t packed_acc_a_1 =
          vuzp1q_s32(acc_pairs[rp][2], acc_pairs[rp][3]);
      const int32x4_t packed_acc_b_0 =
          vuzp2q_s32(acc_pairs[rp][0], acc_pairs[rp][1]);
      const int32x4_t packed_acc_b_1 =
          vuzp2q_s32(acc_pairs[rp][2], acc_pairs[rp][3]);
      sums_0[row_a] = vfmaq_f32(sums_0[row_a], vcvtq_f32_s32(packed_acc_a_0),
                                block_scale_a_0);
      sums_1[row_a] = vfmaq_f32(sums_1[row_a], vcvtq_f32_s32(packed_acc_a_1),
                                block_scale_a_1);
      sums_0[row_b] = vfmaq_f32(sums_0[row_b], vcvtq_f32_s32(packed_acc_b_0),
                                block_scale_b_0);
      sums_1[row_b] = vfmaq_f32(sums_1[row_b], vcvtq_f32_s32(packed_acc_b_1),
                                block_scale_b_1);
    }
  }
...
#else
  // Distinct RHS row pairs per mmla (see the x4 variant): all four output
  // lanes are live, halving the mmla count per subblock while keeping the
  // integer dots and the per-block float fold bit-identical per RHS row.
...
inline void reduce_q6_k_x8_q8_argmax_prepared_i8mm_group(
    ...
    const ::emel::kernel::detail::quant::block_q8_k *rhs,
    const uint64_t block_count, const uint64_t row_base,
    const uint64_t rows_in_group, float &best_value, int32_t &best_index,
  ...
  for (uint64_t block = 0; block < block_count; ++block) {
    ...
    const float32x4_t block_scale_0 =
        vmulq_n_f32(vld1q_f32(q6_block.d.data()), q8_block.d);
    const float32x4_t block_scale_1 =
        vmulq_n_f32(vld1q_f32(q6_block.d.data() + 4u), q8_block.d);
    const int32x4_t packed_acc_0 = vuzp1q_s32(acc_pairs[0], acc_pairs[1]);
    const int32x4_t packed_acc_1 = vuzp1q_s32(acc_pairs[2], acc_pairs[3]);
    sums_0 = vaddq_f32(sums_0,
                       vmulq_f32(vcvtq_f32_s32(packed_acc_0), block_scale_0));
    sums_1 = vaddq_f32(sums_1,
                       vmulq_f32(vcvtq_f32_s32(packed_acc_1), block_scale_1));
  }
...
inline void execute_neon_mul_mat_q6_vector_prepared_q8_rhs_i8mm_unchecked(
  ...
  const auto *q8_blocks =
      static_cast<const ::emel::kernel::detail::quant::block_q8_k *>(
          request.src1.data);
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    const auto *group_ptr = reinterpret_cast<
        const ::emel::kernel::detail::quant::block_q6_kx8_q8_prepared *>(
        prepared + group * group_bytes);
    std::array<float, ::emel::kernel::detail::quant::Q6_K_X8_ROWS> group_out =
        {};
    dot_q6_k_x8_q8_k_group_prepared_i8mm(group_ptr, q8_blocks, block_count,
                                         group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q6_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
        static_cast<uint64_t>(::emel::kernel::detail::quant::Q6_K_X8_ROWS),
        m - row_base);
    for (uint64_t row = 0; row < rows_in_group; ++row) {
...
inline void
execute_neon_mul_mat_argmax_q6_vector_prepared_q8_rhs_i8mm_unchecked(
    const event::op_mul_mat_argmax &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_MATMUL_INT8))
  (void)request;
...
inline void
execute_neon_mul_mat_argmax_q6_vector_q8_argmax_prepared_i8mm_unchecked(
    const event::op_mul_mat_argmax &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_MATMUL_INT8))
  (void)request;
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    const auto *group_ptr = reinterpret_cast<
        const ::emel::kernel::detail::quant::block_q6_kx8_q8_argmax_prepared *>(
        prepared + group * group_bytes);
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q6_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
    ...
    reduce_q6_k_x8_q8_argmax_prepared_i8mm_group(
        group_ptr, q8_blocks, block_count, row_base, rows_in_group, best_value,
        best_index, have_best);
  }
...
inline void execute_neon_mul_mat_argmax_q6_vector_packed_q8_rhs_unchecked(
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q6_K_X8_ROWS> group_out =
        {};
    dot_q6_k_x8_q8_k_group_neon(group_ptr, q8_blocks, block_count,
                                group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q6_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
...
inline void execute_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_bl4_unchecked(
    const event::op_mul_mat_argmax &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  (void)request;
  ...
  ::emel::kernel::detail::quant::quantize_row_q8_k_strided(
      static_cast<const float *>(request.src1.data), 1u, q8_blocks.data(),
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q4_K_X8_ROWS> group_out =
        {};
    dot_q4_k_x8_q8_k_group_bl4_neon(group_ptr, q8_blocks.data(), block_count,
                                    group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q4_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
        static_cast<uint64_t>(::emel::kernel::detail::quant::Q4_K_X8_ROWS),
        m - row_base);
    for (uint64_t row = 0; row < rows_in_group; ++row) {
      const float value = group_out[row];
      if (!have_best || value > best_value) {
...
inline void execute_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_bl8_unchecked(
    const event::op_mul_mat_argmax &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  (void)request;
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q4_K_X8_ROWS> group_out =
        {};
    dot_q4_k_x8_q8_k_group_bl8_neon(group_ptr, q8_blocks.data(), block_count,
                                    group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q4_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
...
inline void execute_neon_mul_mat_q4_vector_packed_q8_rhs_bl4_unchecked(
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q4_K_X8_ROWS> group_out =
        {};
    dot_q4_k_x8_q8_k_group_bl4_neon(group_ptr, q8_blocks, block_count,
                                    group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q4_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
...
inline void execute_neon_mul_mat_q4_vector_packed_f32_rhs_bl4_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  (void)request;
...
inline void
store_batch_major_group_results(const float *group_out, float *dst,
                                const size_t dst_row_stride,
                                const uint64_t rhs_row, const uint64_t row_base,
...
inline void
execute_neon_mul_mat_q4_vector_packed_q8_rhs_bl4_matrix_x4_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  (void)request;
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    for (uint64_t rhs_row = 0; rhs_row < rhs_rows; ++rhs_row) {
      ...
      dot_q4_k_x8_q8_k_group_bl4_neon(group_ptr, q8_blocks, block_count,
                                      group_out.data());
      store_batch_major_group_results(group_out.data(), dst, dst_row_stride,
                                      rhs_row, row_base, rows_in_group);
    }
  }
#endif
}
...
inline void execute_neon_mul_mat_q4_vector_packed_q8_rhs_bl8_unchecked(
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q4_K_X8_ROWS> group_out =
        {};
    dot_q4_k_x8_q8_k_group_bl8_neon(group_ptr, q8_blocks, block_count,
                                    group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q4_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
...
inline void execute_neon_mul_mat_q4_vector_packed_f32_rhs_bl8_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  (void)request;
...
inline void
execute_neon_mul_mat_q4_vector_packed_q8_rhs_bl8_matrix_x4_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  (void)request;
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q4_K_X8_ROWS> group_out3 =
        {};
    dot_q4_k_x8_q8_k_group_bl8_x4_neon(group_ptr, rhs_row0, rhs_row1, rhs_row2,
                                       rhs_row3, block_count, group_out0.data(),
                                       group_out1.data(), group_out2.data(),
                                       group_out3.data());
    store_batch_major_group_results(group_out0.data(), dst, dst_row_stride, 0u,
                                    row_base, rows_in_group);
    store_batch_major_group_results(group_out1.data(), dst, dst_row_stride, 1u,
                                    row_base, rows_in_group);
    store_batch_major_group_results(group_out2.data(), dst, dst_row_stride, 2u,
                                    row_base, rows_in_group);
    store_batch_major_group_results(group_out3.data(), dst, dst_row_stride, 3u,
                                    row_base, rows_in_group);
  }
#endif
}
...
inline void
execute_neon_mul_mat_q4_vector_packed_q8_rhs_bl8_matrix_x8_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  (void)request;
  ...
  float *dst = static_cast<float *>(request.dst.data);
  const size_t dst_row_stride = request.dst.nb[0] / sizeof(float);
  const std::array<const ::emel::kernel::detail::quant::block_q8_k *, 8>
      rhs_rows{
          reinterpret_cast<const ::emel::kernel::detail::quant::block_q8_k *>(
              rhs_base + 0u * rhs_row_bytes),
          reinterpret_cast<const ::emel::kernel::detail::quant::block_q8_k *>(
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    const uint64_t rows_in_group = std::min(
        static_cast<uint64_t>(::emel::kernel::detail::quant::Q4_K_X8_ROWS),
        m - row_base);
    std::array<std::array<float, ::emel::kernel::detail::quant::Q4_K_X8_ROWS>,
               8>
        group_out = {};
    dot_q4_k_x8_q8_k_group_bl8_x8_neon(
        group_ptr, rhs_rows[0], rhs_rows[1], rhs_rows[2], rhs_rows[3],
        rhs_rows[4], rhs_rows[5], rhs_rows[6], rhs_rows[7], block_count,
        group_out[0].data(), group_out[1].data(), group_out[2].data(),
        group_out[3].data(), group_out[4].data(), group_out[5].data(),
        group_out[6].data(), group_out[7].data());
    for (uint64_t rhs_row = 0; rhs_row < rhs_rows.size(); ++rhs_row) {
      store_batch_major_group_results(group_out[rhs_row].data(), dst,
                                      dst_row_stride, rhs_row, row_base,
                                      rows_in_group);
    }
  }
#endif
...
inline void execute_neon_mul_mat_q6_vector_packed_unchecked(
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q6_K_X8_ROWS> group_out =
        {};
    dot_q6_k_x8_q8_k_group_neon(group_ptr, q8_blocks.data(), block_count,
                                group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q6_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
...
inline void execute_neon_mul_mat_q6_vector_prepared_q8_rhs_unchecked(
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    std::array<float, ::emel::kernel::detail::quant::Q6_K_X8_ROWS> group_out =
        {};
    dot_q6_k_x8_q8_k_group_prepared_neon(group_ptr, q8_blocks, block_count,
                                         group_out.data());
    const uint64_t row_base =
        group * ::emel::kernel::detail::quant::Q6_K_X8_ROWS;
    const uint64_t rows_in_group = std::min(
...
inline void
execute_neon_mul_mat_q6_vector_prepared_q8_rhs_i8mm_matrix_x4_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_MATMUL_INT8))
  (void)request;
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    dot_q6_k_x8_q8_k_group_prepared_i8mm_x4(
        group_ptr, rhs_row0, rhs_row1, rhs_row2, rhs_row3, block_count,
        group_out0.data(), group_out1.data(), group_out2.data(),
        group_out3.data());
    store_batch_major_group_results(group_out0.data(), dst, dst_row_stride, 0u,
                                    row_base, rows_in_group);
    store_batch_major_group_results(group_out1.data(), dst, dst_row_stride, 1u,
...
execute_neon_mul_mat_q6_vector_prepared_q8_rhs_i8mm_matrix_x8_unchecked(
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    const uint64_t rows_in_group = std::min(
        static_cast<uint64_t>(::emel::kernel::detail::quant::Q6_K_X8_ROWS),
        m - row_base);
    std::array<std::array<float, ::emel::kernel::detail::quant::Q6_K_X8_ROWS>,
               8>
        group_out = {};
...
inline void execute_neon_mul_mat_q6_vector_unchecked(
  ...
  ::emel::kernel::detail::quant::quantize_row_q8_k_strided(b, 1u,
                                                           q8_blocks.data(), k);
  ...
  for (; row + 4u <= m; row += 4u) {
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q6_k *>(
            a + (row + 3u) * row_bytes);
    float out[4] = {};
    dot_q6_k_q8_k_4rows_neon(row0, row1, row2, row3, q8_blocks.data(),
                             block_count, out);
    c[row + 0u] = out[0];
    c[row + 1u] = out[1];
    c[row + 2u] = out[2];
...
inline void execute_neon_mul_mat_q6_vector_q8_rhs_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
#else
  ...
  for (; row + 4u <= m; row += 4u) {
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q6_k *>(
            a + (row + 3u) * row_bytes);
    float out[4] = {};
    dot_q6_k_q8_k_4rows_neon(row0, row1, row2, row3, q8_blocks, block_count,
                             out);
    c[row + 0u] = out[0];
    c[row + 1u] = out[1];
    c[row + 2u] = out[2];
  ...
  for (; row < m; ++row) {
    const auto *row_ptr =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q6_k *>(
            a + row * row_bytes);
    c[row] = dot_q6_k_q8_k_row_neon(row_ptr, q8_blocks, block_count);
  }
#endif
}
...
inline void execute_neon_mul_mat_q8_0_packed_bl4_unchecked(
  ...
  const auto *packed_base =
      static_cast<const ::emel::kernel::detail::quant::block_q8_0x4 *>(
          request.src0.data);
  const auto *q8_blocks =
  ...
  for (uint64_t group = 0; group < group_count; ++group) {
    ...
    for (uint64_t block = 0; block < block_count; ++block) {
      ...
      acc =
          vfmaq_f32(acc, vcvtq_f32_s32(isum), vmulq_f32(lhs_scale, rhs_scale));
    }
...
inline void execute_neon_mul_mat_q8_0_packed_bl8_matrix_x4_unchecked(
  ...
  for (uint64_t col_base = 0; col_base < m; col_base += 4u) {
    const auto *packed_weights =
        packed_weights_base + ((col_base / 4u) * block_count);
    ...
    for (uint64_t block = 0; block < block_count; ++block) {
      ...
      for (int chunk = 0; chunk < 4; ++chunk) {
        ...
        const int8x16_t rhs23 =
            vld1q_s8(packed_rhs->qs.data() + chunk * 32 + 16);
        const int8x16_t lhs01 =
            vld1q_s8(packed_weights->qs.data() + chunk * 32);
        const int8x16_t lhs23 =
            vld1q_s8(packed_weights->qs.data() + chunk * 32 + 16);
      ...
      const float32x4_t rhs_scale = vcvt_f32_f16(
          vld1_f16(reinterpret_cast<const __fp16 *>(packed_rhs->d.data())));
      const float32x4_t lhs_scale = vcvt_f32_f16(
          vld1_f16(reinterpret_cast<const __fp16 *>(packed_weights->d.data())));

      acc_f32[0] = vfmaq_f32(acc_f32[0], vcvtq_f32_s32(row0),
                             vmulq_laneq_f32(lhs_scale, rhs_scale, 0));
      acc_f32[1] = vfmaq_f32(acc_f32[1], vcvtq_f32_s32(row1),
                             vmulq_laneq_f32(lhs_scale, rhs_scale, 1));
      acc_f32[2] = vfmaq_f32(acc_f32[2], vcvtq_f32_s32(row2),
                             vmulq_laneq_f32(lhs_scale, rhs_scale, 2));
      acc_f32[3] = vfmaq_f32(acc_f32[3], vcvtq_f32_s32(row3),
                             vmulq_laneq_f32(lhs_scale, rhs_scale, 3));
    ...
    for (int row = 0; row < 4; ++row) {
      vst1q_f32(dst + (static_cast<size_t>(row) * dst_row_stride) + col_base,
                acc_f32[row]);
    }
  }
#endif
...
inline void execute_neon_mul_mat_q8_0_vector_unchecked(
  ...
  for (; row + 4u <= m; row += 4u) {
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q8_0 *>(
            a + (row + 3u) * row_bytes);
    vst1q_f32(c + row, dot_q8_0_q8_0_4row_neon(row0, row1, row2, row3,
...
inline void execute_neon_mul_mat_q8_0_vector_q8_rhs_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
#else
  ...
  for (; row < m; ++row) {
    const auto *row_ptr =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q8_0 *>(
            a + row * row_bytes);
    c[row] = dot_q8_0_q8_0_row_neon(row_ptr, q8_blocks, block_count);
  }
#endif
}

inline void execute_neon_mul_mat_q4_vector_q8_rhs_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
#else
  ...
  for (; row + 4u <= m; row += 4u) {
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_k *>(
            a + (row + 3u) * row_bytes);
    float out[4] = {};
    dot_q4_k_q8_k_4rows_neon(row0, row1, row2, row3, q8_blocks, block_count,
                             out);
    c[row + 0u] = out[0];
    c[row + 1u] = out[1];
    c[row + 2u] = out[2];
  ...
  for (; row + 2u <= m; row += 2u) {
    ...
    const auto *row1 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_k *>(
            a + (row + 1u) * row_bytes);
    float out[2] = {};
    dot_q4_k_q8_k_2rows_neon(row0, row1, q8_blocks, block_count, out);
    c[row + 0u] = out[0];
    c[row + 1u] = out[1];
  }
  ...
  for (; row < m; ++row) {
    const auto *row_ptr =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_k *>(
            a + row * row_bytes);
    c[row] = dot_q4_k_q8_k_row_neon(row_ptr, q8_blocks, block_count);
  }
#endif
}

inline void execute_neon_mul_mat_q4_vector_f32_rhs_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
#else
  ...
  for (; row + 4u <= m; row += 4u) {
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_k *>(
            a + (row + 3u) * row_bytes);
    float out[4] = {};
    dot_q4_k_q8_k_4rows_neon(row0, row1, row2, row3, q8_blocks.data(),
                             block_count, out);
    c[row + 0u] = out[0];
    c[row + 1u] = out[1];
    c[row + 2u] = out[2];
  ...
  for (; row < m; ++row) {
    const auto *row_ptr =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_k *>(
            a + row * row_bytes);
    c[row] = dot_q4_k_q8_k_row_neon(row_ptr, q8_blocks.data(), block_count);
  }
#endif
}

// Exact port of the pinned ggml_vec_dot_f16 aarch64 NEON fp16-arithmetic
// path (GGML_F16_STEP=32): four float16x8_t accumulators advanced with
// vfmaq_f16, pairwise vaddq_f16 reduce, f32 convert + horizontal add, and an
// f32-product tail widened into the double running sum. Accumulation happens
// in fp16 lanes, which is what live ggml computes on hosts with
// __ARM_FEATURE_FP16_VECTOR_ARITHMETIC (the x86 port in
// emel/kernel/detail.hpp accumulates in f32/double instead).
inline float dot_f16_f16_row_neon(const int64_t count, const uint16_t *x,
  ...
  for (int64_t i = 0; i < np; i += 32) {
    for (int64_t j = 0; j < 4; ++j) {
      const float16x8_t ax =
          vld1q_f16(reinterpret_cast<const float16_t *>(x + i + j * 8));
      const float16x8_t ay =
          vld1q_f16(reinterpret_cast<const float16_t *>(y + i + j * 8));
  ...
  const float32x4_t lo = vcvt_f32_f16(vget_low_f16(sum[0]));
  const float32x4_t hi = vcvt_f32_f16(vget_high_f16(sum[0]));
  double sumf = static_cast<double>(vaddvq_f32(vaddq_f32(lo, hi)));
  for (int64_t i = np; i < count; ++i) {
    sumf +=
        static_cast<double>(::emel::kernel::detail::quant::fp16_to_fp32(x[i]) *
                            ::emel::kernel::detail::quant::fp16_to_fp32(y[i]));
  }
  return static_cast<float>(sumf);
#endif
}
...
inline float dot_f32_f32_row_neon(const int64_t count, const float *x,
  ...
  float32x4_t sum[4] = {vdupq_n_f32(0.0f), vdupq_n_f32(0.0f), vdupq_n_f32(0.0f),
                        vdupq_n_f32(0.0f)};
...
inline void dot_f32_f32_4rows_neon(const int64_t count, const float *row0,
                                   const float *row1, const float *row2,
                                   const float *row3, const float *y,
...
#else
  const int64_t np = count & ~static_cast<int64_t>(15);
  float32x4_t sum0[4] = {vdupq_n_f32(0.0f), vdupq_n_f32(0.0f),
                         vdupq_n_f32(0.0f), vdupq_n_f32(0.0f)};
  float32x4_t sum1[4] = {vdupq_n_f32(0.0f), vdupq_n_f32(0.0f),
                         vdupq_n_f32(0.0f), vdupq_n_f32(0.0f)};
  float32x4_t sum2[4] = {vdupq_n_f32(0.0f), vdupq_n_f32(0.0f),
                         vdupq_n_f32(0.0f), vdupq_n_f32(0.0f)};
  float32x4_t sum3[4] = {vdupq_n_f32(0.0f), vdupq_n_f32(0.0f),
                         vdupq_n_f32(0.0f), vdupq_n_f32(0.0f)};
  for (int64_t i = 0; i < np; i += 16) {
    for (int64_t j = 0; j < 4; ++j) {
      const int64_t offset = i + j * 4;
      const float32x4_t ay = vld1q_f32(y + offset);
      sum0[j] = vfmaq_f32(sum0[j], vld1q_f32(row0 + offset), ay);
      sum1[j] = vfmaq_f32(sum1[j], vld1q_f32(row1 + offset), ay);
      sum2[j] = vfmaq_f32(sum2[j], vld1q_f32(row2 + offset), ay);
      sum3[j] = vfmaq_f32(sum3[j], vld1q_f32(row3 + offset), ay);
    }
  }
  sum0[0] = vaddq_f32(sum0[0], sum0[2]);
  sum0[1] = vaddq_f32(sum0[1], sum0[3]);
  sum0[0] = vaddq_f32(sum0[0], sum0[1]);
  sum1[0] = vaddq_f32(sum1[0], sum1[2]);
  sum1[1] = vaddq_f32(sum1[1], sum1[3]);
  sum1[0] = vaddq_f32(sum1[0], sum1[1]);
  sum2[0] = vaddq_f32(sum2[0], sum2[2]);
  sum2[1] = vaddq_f32(sum2[1], sum2[3]);
  sum2[0] = vaddq_f32(sum2[0], sum2[1]);
  sum3[0] = vaddq_f32(sum3[0], sum3[2]);
  sum3[1] = vaddq_f32(sum3[1], sum3[3]);
  sum3[0] = vaddq_f32(sum3[0], sum3[1]);
  out0 = vaddvq_f32(sum0[0]);
  out1 = vaddvq_f32(sum1[0]);
  out2 = vaddvq_f32(sum2[0]);
  out3 = vaddvq_f32(sum3[0]);
  for (int64_t i = np; i < count; ++i) {
    out0 += row0[i] * y[i];
    out1 += row1[i] * y[i];
    out2 += row2[i] * y[i];
...
inline void execute_neon_mul_mat_f32_vector_unchecked(
  ...
  uint64_t row = 0u;
  for (; row + 4u <= m; row += 4u) {
    dot_f32_f32_4rows_neon(static_cast<int64_t>(k), a + (row + 0u) * k,
                           a + (row + 1u) * k, a + (row + 2u) * k,
                           a + (row + 3u) * k, y, dst[row + 0u], dst[row + 1u],
                           dst[row + 2u], dst[row + 3u]);
  }
  for (; row < m; ++row) {
...
inline void execute_neon_mul_mat_f16_vector_unchecked(
  ...
  for (uint64_t col = 0; col < n; ++col) {
    ...
    for (uint64_t row = 0; row < m; ++row) {
      dst[row + col * m] =
          dot_f16_f16_row_neon(static_cast<int64_t>(k), src0 + row * k, y);
    }
  }
#endif
}

// NEON acceleration of the emel-owned conv_transpose_1d f32 scatter kernel
// (kernel/detail.hpp run_conv_transpose_1d_as<false>). Every dst element
// accumulates the same contributions in the same order as the shared scalar
// route, and the tap loop uses fused multiply-add (vfmaq_f32) because the
// pinned aarch64 toolchains contract the scalar route's `dst += input *
// weight` into fma (-ffp-contract=on): the outputs are bit-identical to the
// scalar route as compiled, only faster. The route test asserts that
// bit-identity, so any toolchain contraction drift fails loudly.
inline void execute_neon_conv_transpose_1d_f32_unchecked(
    const event::op_conv_transpose_1d &request) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
#else
  int32_t s0 = 0;
  ...
  const uint64_t w_nb1 =
      ::emel::kernel::detail::tensor_stride_bytes(request.src0, 1);
  const uint64_t w_nb2 =
      ::emel::kernel::detail::tensor_stride_bytes(request.src0, 2);

  for (int64_t in_channel = 0; in_channel < in_channels; ++in_channel) {
    for (int64_t in = 0; in < length; ++in) {
      const float input = ::emel::kernel::detail::read_f32_at(
          request.src1, static_cast<uint64_t>(in),
          static_cast<uint64_t>(in_channel));
      const float32x4_t input_v = vdupq_n_f32(input);
      for (int64_t out_channel = 0; out_channel < out_channels; ++out_channel) {
        float *dst_run = dst + out_channel * out_length + in * s0;
        const float *weights = reinterpret_cast<const float *>(
            weight_base + static_cast<uint64_t>(out_channel) * w_nb1 +
            static_cast<uint64_t>(in_channel) * w_nb2);
        int64_t tap = 0;
        for (; tap + 4 <= kernel; tap += 4) {
          const float32x4_t acc = vld1q_f32(dst_run + tap);
          const float32x4_t w = vld1q_f32(weights + tap);
          vst1q_f32(dst_run + tap, vfmaq_f32(acc, input_v, w));
        }
        for (; tap < kernel; ++tap) {
          dst_run[tap] += input * weights[tap];
        }
      }
    }
  }
...
inline void execute_neon_mul_mat_q5_0_vector_unchecked(
  ...
  for (; row + 4u <= m; row += 4u) {
    const auto *row0 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q5_0 *>(
            a + (row + 0u) * row_bytes);
    const auto *row1 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q5_0 *>(
            a + (row + 1u) * row_bytes);
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q5_0 *>(
            a + (row + 3u) * row_bytes);
    float sum[4] = {};
    dot_q5_0_q8_0_4rows_neon(row0, row1, row2, row3, q8_blocks.data(),
                             block_count, sum);
    c[row + 0u] = sum[0];
    c[row + 1u] = sum[1];
    c[row + 2u] = sum[2];
...
inline bool
execute_neon_mul_mat_q5_0_vector(const event::op_mul_mat &request) noexcept {
  if (!can_run_neon_mul_mat_q5_0_vector_request(request) ||
...
inline void execute_neon_mul_mat_q4_0_vector_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
#else
  ...
  for (; row + 4u <= m; row += 4u) {
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_0 *>(
            a + (row + 3u) * row_bytes);
    float sum[4] = {};
    dot_q4_0_q8_0_4rows_neon(row0, row1, row2, row3, q8_blocks.data(),
                             block_count, sum);
    c[row + 0u] = sum[0];
    c[row + 1u] = sum[1];
    c[row + 2u] = sum[2];
  ...
  for (; row < m; ++row) {
    const auto *row_ptr =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_0 *>(
            a + row * row_bytes);
    c[row] = dot_q4_0_q8_0_row_neon(row_ptr, q8_blocks.data(), block_count);
  }
#endif
}

inline bool
execute_neon_mul_mat_q4_0_vector(const event::op_mul_mat &request) noexcept {
  if (!can_run_neon_mul_mat_q4_0_vector_request(request) ||
      !neon_q4_0_vector_supported()) {
    return false;
  }
  execute_neon_mul_mat_q4_0_vector_unchecked(request);
  return true;
}

inline void execute_neon_mul_mat_q4_1_vector_unchecked(
    const event::op_mul_mat &request) noexcept {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  (void)request;
#else
  ...
  for (; row + 4u <= m; row += 4u) {
    ...
    const auto *row3 =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_1 *>(
            a + (row + 3u) * row_bytes);
    float sum[4] = {};
    dot_q4_1_q8_0_4rows_neon(row0, row1, row2, row3, q8_blocks.data(),
                             block_count, sum);
    c[row + 0u] = sum[0];
    c[row + 1u] = sum[1];
    c[row + 2u] = sum[2];
  ...
  for (; row < m; ++row) {
    const auto *row_ptr =
        reinterpret_cast<const ::emel::kernel::detail::quant::block_q4_1 *>(
            a + row * row_bytes);
    c[row] = dot_q4_1_q8_0_row_neon(row_ptr, q8_blocks.data(), block_count);
  }
#endif
}

inline bool
execute_neon_mul_mat_q4_1_vector(const event::op_mul_mat &request) noexcept {
  if (!can_run_neon_mul_mat_q4_1_vector_request(request) ||
      !neon_q4_1_vector_supported()) {
    return false;
  }
  execute_neon_mul_mat_q4_1_vector_unchecked(request);
  return true;
}
      ...
      if (src0_type == ::emel::kernel::detail::dtype_q4_k) {
        ...
        for (; i + 4u <= m; i += 4u) {
          const auto *row0 = reinterpret_cast<
              ...
              a + (i + 0u) * row_bytes);
          const auto *row1 = reinterpret_cast<
              const ::emel::kernel::detail::quant::block_q4_k *>(
              a + (i + 1u) * row_bytes);
          const auto *row2 = reinterpret_cast<
              const ::emel::kernel::detail::quant::block_q4_k *>(
              a + (i + 2u) * row_bytes);
          const auto *row3 = reinterpret_cast<
              const ::emel::kernel::detail::quant::block_q4_k *>(
              a + (i + 3u) * row_bytes);
          float out[4] = {};
          dot_q4_k_q8_k_4rows_neon(row0, row1, row2, row3, q8_blocks.data(),
        ...
        for (; i + 2u <= m; i += 2u) {
          ...
          const auto *row1 = reinterpret_cast<
              ...
              a + (i + 1u) * row_bytes);
          float out[2] = {};
          dot_q4_k_q8_k_2rows_neon(row0, row1, q8_blocks.data(), block_count,
                                   out);
          c[(i + 0u) * n + j] = out[0];
          c[(i + 1u) * n + j] = out[1];
        }
        for (; i < m; ++i) {
          const auto *row_ptr = reinterpret_cast<
              const ::emel::kernel::detail::quant::block_q4_k *>(a +
                                                                 i * row_bytes);
          c[i * n + j] =
              dot_q4_k_q8_k_row_neon(row_ptr, q8_blocks.data(), block_count);
        }
        continue;
      }
      ...
      for (uint64_t i = 0; i < m; ++i) {
        ...
        if (src0_type == ::emel::kernel::detail::dtype_q2_k) {
          c[i * n + j] = dot_q2_k_q8_k_row_neon(
              reinterpret_cast<
                  const ::emel::kernel::detail::quant::block_q2_k *>(row_ptr),
              q8_blocks.data(), block_count);
        } else if (src0_type == ::emel::kernel::detail::dtype_q3_k) {
          c[i * n + j] = dot_q3_k_q8_k_row_neon(
              reinterpret_cast<
                  const ::emel::kernel::detail::quant::block_q3_k *>(row_ptr),
              q8_blocks.data(), block_count);
        } else {
          c[i * n + j] = dot_q6_k_q8_k_row_neon(
              reinterpret_cast<
                  const ::emel::kernel::detail::quant::block_q6_k *>(row_ptr),
              q8_blocks.data(), block_count);
        }
      }
    }
      ...
      for (; j + (col_vec * 4u) <= j_vec_end; j += col_vec * 4u) {
        ...
        for (; i < m; ++i) {
          ...
          for (uint64_t kk = 0; kk < depth; ++kk) {
            ...
            const float32x4_t bv2 =
                vld1q_f32(b_panel + b_base + (col_vec * 2u));
            const float32x4_t bv3 =
                vld1q_f32(b_panel + b_base + (col_vec * 3u));
...
inline bool
prepare_neon_mul_mat_f32_lhs_4row(const float *src, const uint64_t k,
                                  const uint64_t m, float *dst,
...
inline void
execute_neon_unary_neg_request(const event::op_unary &request) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
  const uint64_t count =
      ::emel::kernel::detail::tensor_element_count(request.dst);
  const float *src = static_cast<const float *>(request.src0.data);
...
inline void
execute_neon_unary_relu_request(const event::op_unary &request) noexcept {
#if defined(__aarch64__) || defined(__ARM_NEON)
  const uint64_t count =
      ::emel::kernel::detail::tensor_element_count(request.dst);
  const float *src = static_cast<const float *>(request.src0.data);
...
inline bool execute_request(const request_type &request,
  ...
  const bool simd_succeeded =
      can_use_neon(request, ctx.neon_available) && execute_simd(request);
...
template <class dispatch_event_type> struct exec_scalar_op {
  void operator()(const dispatch_event_type &ev, context &ctx) const noexcept {
    ...
    if constexpr (std::is_same_v<request_type,
      ...
      ::emel::kernel::detail::
          run_flash_attn_ext_active_kv_with_workspace_unchecked(
...
template <class dispatch_event_type> struct exec_simd_op {
  void operator()(const dispatch_event_type &ev, context &ctx) const noexcept {
    ...
    if constexpr (std::is_same_v<request_type,
      ...
      const uint8_t src0_type =
          ::emel::kernel::detail::dtype_code(ev.request.src0.type);
      ctx.optimized_q2_dispatch_count += static_cast<uint64_t>(
          src0_type == ::emel::kernel::detail::dtype_q2_k);
      ctx.optimized_q3_dispatch_count += static_cast<uint64_t>(
          src0_type == ::emel::kernel::detail::dtype_q3_k);
      ctx.optimized_q4_dispatch_count += static_cast<uint64_t>(
          src0_type == ::emel::kernel::detail::dtype_q4_k);
      ctx.optimized_q6_dispatch_count += static_cast<uint64_t>(
          src0_type == ::emel::kernel::detail::dtype_q6_k);
    }
...
struct exec_simd_q6_vector_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::execute_neon_mul_mat_q6_vector_unchecked(
        ev.request);
...
struct exec_simd_q4_vector_packed_q8_rhs_bl4_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
    ...
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q4_vector_packed_q8_rhs_bl4_unchecked(ev.request);
...
struct exec_simd_q4_vector_packed_f32_rhs_bl4_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q4_vector_packed_f32_rhs_bl4_unchecked(
            ev.request);
    ++ctx.optimized_q4_dispatch_count;
    ++ctx.optimized_q4_vector_dispatch_count;
    ++ctx.optimized_q4_vector_packed_dispatch_count;
...
struct exec_simd_q4_vector_packed_q8_rhs_bl8_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q4_vector_packed_q8_rhs_bl8_unchecked(ev.request);
    ++ctx.optimized_q4_dispatch_count;
    ++ctx.optimized_q4_vector_dispatch_count;
    ++ctx.optimized_q4_vector_packed_dispatch_count;
    ++ctx.optimized_q4_vector_packed_q8_rhs_dispatch_count;
...
struct exec_simd_q4_vector_packed_f32_rhs_bl8_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q4_vector_packed_f32_rhs_bl8_unchecked(
            ev.request);
    ++ctx.optimized_q4_dispatch_count;
    ++ctx.optimized_q4_vector_dispatch_count;
    ++ctx.optimized_q4_vector_packed_dispatch_count;
...
struct exec_simd_q4_vector_q8_rhs_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q4_vector_q8_rhs_unchecked(ev.request);
    ++ctx.optimized_q4_dispatch_count;
    ++ctx.optimized_q4_vector_dispatch_count;
    detail::mark_done(ev, ctx);
  }
...
struct exec_simd_q4_vector_f32_rhs_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q4_vector_f32_rhs_unchecked(ev.request);
    ++ctx.optimized_q4_dispatch_count;
    ++ctx.optimized_q4_vector_dispatch_count;
    detail::mark_done(ev, ctx);
  }
...
struct exec_simd_q8_0_vector_q8_rhs_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q8_0_vector_q8_rhs_unchecked(ev.request);
    ++ctx.optimized_q8_0_dispatch_count;
    ++ctx.optimized_q8_0_vector_dispatch_count;
    detail::mark_done(ev, ctx);
  }
...
struct exec_simd_f16_vector_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::execute_neon_mul_mat_f16_vector_unchecked(
        ev.request);
    ++ctx.optimized_f16_vector_dispatch_count;
    detail::mark_done(ev, ctx);
  }
};
...
struct exec_simd_f32_op_conv_transpose_1d {
  void operator()(
      ...
      context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_conv_transpose_1d_f32_unchecked(ev.request);
    ++ctx.optimized_conv_transpose_f32_dispatch_count;
    detail::mark_done(ev, ctx);
  }
};
...
struct exec_simd_q8_0_packed_bl4_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q8_0_packed_bl4_unchecked(ev.request);
    ++ctx.optimized_q8_0_dispatch_count;
...
struct exec_simd_q8_0_packed_bl8_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q8_0_packed_bl8_unchecked(ev.request);
    ++ctx.optimized_q8_0_dispatch_count;
    ++ctx.optimized_q8_0_packed_dispatch_count;
...
struct exec_simd_q6_vector_packed_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q6_vector_packed_unchecked(ev.request);
    ++ctx.optimized_q6_dispatch_count;
    ++ctx.optimized_q6_vector_dispatch_count;
...
struct exec_simd_q6_vector_q8_rhs_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q6_vector_q8_rhs_unchecked(ev.request);
    ++ctx.optimized_q6_dispatch_count;
    ++ctx.optimized_q6_vector_dispatch_count;
    detail::mark_done(ev, ctx);
  }
...
struct exec_simd_q6_vector_packed_q8_rhs_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q6_vector_packed_q8_rhs_unchecked(ev.request);
    ++ctx.optimized_q6_dispatch_count;
    ++ctx.optimized_q6_vector_dispatch_count;
    ++ctx.optimized_q6_vector_packed_dispatch_count;
...
struct exec_simd_q6_vector_packed_q8_rhs_matrix_x4_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q6_vector_packed_q8_rhs_matrix_x4_unchecked(
            ev.request);
    ++ctx.optimized_q6_dispatch_count;
    ++ctx.optimized_q6_vector_dispatch_count;
    ++ctx.optimized_q6_vector_packed_dispatch_count;
...
struct exec_simd_q6_vector_prepared_q8_rhs_op_mul_mat {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_mul_mat &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_q6_vector_prepared_q8_rhs_unchecked(ev.request);
    ++ctx.optimized_q6_dispatch_count;
    ++ctx.optimized_q6_vector_dispatch_count;
    ++ctx.optimized_q6_vector_packed_dispatch_count;
    ++ctx.optimized_q6_vector_packed_q8_rhs_dispatch_count;
...
struct exec_simd_q4_vector_packed_f32_rhs_bl4_op_mul_mat_argmax {
  void operator()(
      const ::emel::kernel::aarch64::event::dispatch_op_mul_mat_argmax &ev,
      context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_bl4_unchecked(
            ev.request);
    ++ctx.optimized_q4_dispatch_count;
    ++ctx.optimized_q4_vector_dispatch_count;
    ++ctx.optimized_q4_vector_packed_dispatch_count;
...
struct exec_simd_q4_vector_packed_f32_rhs_bl8_op_mul_mat_argmax {
  void operator()(
      const ::emel::kernel::aarch64::event::dispatch_op_mul_mat_argmax &ev,
      context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_argmax_q4_vector_packed_f32_rhs_bl8_unchecked(
            ev.request);
    ++ctx.optimized_q4_dispatch_count;
    ++ctx.optimized_q4_vector_dispatch_count;
    ++ctx.optimized_q4_vector_packed_dispatch_count;
...
struct exec_simd_q6_vector_packed_q8_rhs_op_mul_mat_argmax {
  void operator()(
      ...
      context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        execute_neon_mul_mat_argmax_q6_vector_packed_q8_rhs_unchecked(
            ev.request);
    ++ctx.optimized_q6_dispatch_count;
    ++ctx.optimized_q6_vector_dispatch_count;
    ++ctx.optimized_q6_vector_argmax_dispatch_count;
...
struct exec_simd_flash_attn_ext_f16kv_one_chunk {
  void operator()(
      ...
      context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::
        run_flash_attn_ext_f16kv_one_chunk_neon_unchecked(
            ev.request, ctx.flash_attn_workspace);
...
template <::emel::kernel::event::unary_subop subop> struct exec_simd_unary_op {
  void operator()(const ::emel::kernel::aarch64::event::dispatch_op_unary &ev,
                  context &ctx) const noexcept {
    ::emel::kernel::aarch64::detail::execute_simd_unary_subop_unchecked<subop>(
        ev.request);
    detail::mark_done(ev, ctx);
  }
};
...
#define EMEL_KERNEL_DECLARE_RUN_TYPE(op_name)                                  \
  using exec_##op_name##_t = detail::exec_scalar_op<                           \
      ::emel::kernel::aarch64::event::dispatch_##op_name>;
EMEL_KERNEL_OP_EVENT_LIST(EMEL_KERNEL_DECLARE_RUN_TYPE)
#undef EMEL_KERNEL_DECLARE_RUN_TYPE

using exec_simd_op_dup_t =
    detail::exec_simd_op<::emel::kernel::aarch64::event::dispatch_op_dup>;
using exec_simd_op_add_t =
    detail::exec_simd_op<::emel::kernel::aarch64::event::dispatch_op_add>;
using exec_simd_op_sub_t =
    detail::exec_simd_op<::emel::kernel::aarch64::event::dispatch_op_sub>;
using exec_simd_op_mul_t =
    detail::exec_simd_op<::emel::kernel::aarch64::event::dispatch_op_mul>;
using exec_simd_op_div_t =
    detail::exec_simd_op<::emel::kernel::aarch64::event::dispatch_op_div>;
using exec_simd_op_sqr_t =
    detail::exec_simd_op<::emel::kernel::aarch64::event::dispatch_op_sqr>;
using exec_simd_op_sqrt_t =
    detail::exec_simd_op<::emel::kernel::aarch64::event::dispatch_op_sqrt>;
...
using exec_simd_op_mul_mat_q4_vector_packed_f32_rhs_bl4_t =
    detail::exec_simd_q4_vector_packed_f32_rhs_bl4_op_mul_mat;
using exec_simd_op_mul_mat_q4_vector_packed_q8_rhs_bl8_t =
    detail::exec_simd_q4_vector_packed_q8_rhs_bl8_op_mul_mat;
using exec_simd_op_mul_mat_q4_vector_packed_f32_rhs_bl8_t =
    detail::exec_simd_q4_vector_packed_f32_rhs_bl8_op_mul_mat;
...
using exec_simd_op_mul_mat_q4_vector_q8_rhs_t =
    detail::exec_simd_q4_vector_q8_rhs_op_mul_mat;
using exec_simd_op_mul_mat_q4_vector_f32_rhs_t =
    detail::exec_simd_q4_vector_f32_rhs_op_mul_mat;
using exec_simd_op_mul_mat_q5_0_vector_t =
    detail::exec_simd_q5_0_vector_op_mul_mat;
using exec_simd_op_mul_mat_q4_0_vector_t =
    detail::exec_simd_q4_0_vector_op_mul_mat;
using exec_simd_op_mul_mat_q4_1_vector_t =
    detail::exec_simd_q4_1_vector_op_mul_mat;
using exec_simd_op_mul_mat_q8_0_packed_bl4_t =
    detail::exec_simd_q8_0_packed_bl4_op_mul_mat;
...
using exec_simd_op_mul_mat_q8_0_packed_bl8_t =
    detail::exec_simd_q8_0_packed_bl8_op_mul_mat;
using exec_simd_op_mul_mat_q8_0_vector_t =
    detail::exec_simd_q8_0_vector_op_mul_mat;
using exec_simd_op_mul_mat_q8_0_vector_q8_rhs_t =
    detail::exec_simd_q8_0_vector_q8_rhs_op_mul_mat;
...
using exec_simd_op_mul_mat_q6_vector_packed_t =
    detail::exec_simd_q6_vector_packed_op_mul_mat;
using exec_simd_op_mul_mat_q6_vector_q8_rhs_t =
    detail::exec_simd_q6_vector_q8_rhs_op_mul_mat;
...
using exec_simd_op_mul_mat_argmax_q4_vector_packed_f32_rhs_bl4_t =
    detail::exec_simd_q4_vector_packed_f32_rhs_bl4_op_mul_mat_argmax;
using exec_simd_op_mul_mat_argmax_q4_vector_packed_f32_rhs_bl8_t =
    detail::exec_simd_q4_vector_packed_f32_rhs_bl8_op_mul_mat_argmax;
...
using exec_scalar_op_add_broadcast_row_t =
    ::emel::kernel::detail::exec_scalar_binary_broadcast_row_op<
        ::emel::kernel::aarch64::event::dispatch_op_add, context,
        detail::mark_done_op, false>;
using exec_scalar_op_mul_broadcast_row_t =
    ::emel::kernel::detail::exec_scalar_binary_broadcast_row_op<
        ::emel::kernel::aarch64::event::dispatch_op_mul, context,
        detail::mark_done_op, true>;
...
inline constexpr exec_simd_op_mul_mat_q4_vector_packed_f32_rhs_bl4_t
    exec_simd_op_mul_mat_q4_vector_packed_f32_rhs_bl4{};
inline constexpr exec_simd_op_mul_mat_q4_vector_packed_q8_rhs_bl8_t
    exec_simd_op_mul_mat_q4_vector_packed_q8_rhs_bl8{};
inline constexpr exec_simd_op_mul_mat_q4_vector_packed_f32_rhs_bl8_t
    exec_simd_op_mul_mat_q4_vector_packed_f32_rhs_bl8{};
...
inline constexpr exec_simd_op_mul_mat_q4_vector_q8_rhs_t
    exec_simd_op_mul_mat_q4_vector_q8_rhs{};
inline constexpr exec_simd_op_mul_mat_q4_vector_f32_rhs_t
    exec_simd_op_mul_mat_q4_vector_f32_rhs{};
inline constexpr exec_simd_op_mul_mat_q5_0_vector_t
    exec_simd_op_mul_mat_q5_0_vector{};
inline constexpr exec_simd_op_mul_mat_q4_0_vector_t
    exec_simd_op_mul_mat_q4_0_vector{};
inline constexpr exec_simd_op_mul_mat_q4_1_vector_t
    exec_simd_op_mul_mat_q4_1_vector{};
inline constexpr exec_simd_op_mul_mat_q8_0_packed_bl4_t
    exec_simd_op_mul_mat_q8_0_packed_bl4{};
...
inline constexpr exec_simd_op_mul_mat_q8_0_packed_bl8_t
    exec_simd_op_mul_mat_q8_0_packed_bl8{};
inline constexpr exec_simd_op_mul_mat_q8_0_vector_t
    exec_simd_op_mul_mat_q8_0_vector{};
inline constexpr exec_simd_op_mul_mat_q8_0_vector_q8_rhs_t
    exec_simd_op_mul_mat_q8_0_vector_q8_rhs{};
...
inline constexpr exec_simd_op_mul_mat_q6_vector_packed_t
    exec_simd_op_mul_mat_q6_vector_packed{};
inline constexpr exec_simd_op_mul_mat_q6_vector_q8_rhs_t
    exec_simd_op_mul_mat_q6_vector_q8_rhs{};
...
inline constexpr exec_simd_op_mul_mat_argmax_q4_vector_packed_f32_rhs_bl4_t
    exec_simd_op_mul_mat_argmax_q4_vector_packed_f32_rhs_bl4{};
inline constexpr exec_simd_op_mul_mat_argmax_q4_vector_packed_f32_rhs_bl8_t
    exec_simd_op_mul_mat_argmax_q4_vector_packed_f32_rhs_bl8{};
...
inline constexpr exec_simd_op_mul_mat_q6_vector_t
    exec_simd_op_mul_mat_q6_vector{};
