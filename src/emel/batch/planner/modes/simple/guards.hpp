...
inline int32_t compute_required_step_count(const event::plan_runtime & ev) noexcept {
...
inline constexpr auto guard_exceeds_step_capacity =
    [](const event::plan_runtime & ev,
       const context &) noexcept {
      return compute_required_step_count(ev) > emel::batch::planner::action::MAX_PLAN_STEPS;
    };
...
inline constexpr auto guard_simple_plan_capacity_ok =
    [](const event::plan_runtime & ev,
       const context & ctx) noexcept {
      return guard_has_valid_step_size(ev, ctx) &&
...
inline constexpr auto guard_planning_failed =
    [](const event::plan_runtime & ev,
       const context &) noexcept {
      return !emel::batch::planner::guard::guard_has_complete_plan(ev);
    };
