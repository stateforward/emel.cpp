...
inline void create_plan_impl(const event::request & ev, context & ctx) noexcept {
  ...
  while (used_count < ev.n_tokens) {
    ...
    for (int32_t i = 0; i < ev.n_tokens; ++i) {
      ...
      const seq_mask_t mask = detail::normalized_seq_mask(ev, i);
      bool overlap = false;
      for (int32_t g = 0; g < group_count; ++g) {
        if (detail::mask_overlaps(groups[g].mask, mask)) {
          overlap = true;
          break;
        }
      }
      ...
      if (ev.equal_sequential && ev.seq_primary_ids != nullptr) {
        const int32_t primary = ev.seq_primary_ids[i];
      ...
      group_count += 1;
      if (group_count > ctx.effective_step_size) {
        break;
      }
    }

    if (group_count == 0) {
      detail::fail_plan(ctx, emel::batch::planner::error::planning_progress_stalled);
      return;
    }

    int32_t min_avail = ev.n_tokens + 1;
    for (int32_t g = 0; g < group_count; ++g) {
      int32_t avail = 0;
      for (int32_t i = 0; i < ev.n_tokens; ++i) {
        if (used[static_cast<size_t>(i)] != 0) {
        ...
        if (detail::mask_equal(detail::normalized_seq_mask(ev, i), groups[g].mask)) {
          avail += 1;
        }
      }
    ...
    const int32_t max_rows = ctx.effective_step_size / group_count;
    ...
    for (int32_t g = 0; g < group_count; ++g) {
      ...
      for (int32_t i = 0; i < ev.n_tokens && remaining > 0; ++i) {
        if (used[static_cast<size_t>(i)] != 0) {
          continue;
        }
        if (!detail::mask_equal(detail::normalized_seq_mask(ev, i), groups[g].mask)) {
        ...
        used_count += 1;
        if (!detail::append_token_index(ctx, i)) {
          detail::fail_plan(
    ...
    if (remaining != 0) {
      detail::fail_plan(ctx, emel::batch::planner::error::algorithm_failed);
      return;
    }
    }

    const int32_t added = n_seq_tokens * group_count;
    if (!detail::push_step_size(ctx, added)) {
...
inline void create_plan_primary_fast_path_impl(const event::request & ev, context & ctx) noexcept {
  if (ctx.effective_step_size <= 0) {
    detail::fail_plan(ctx, emel::batch::planner::error::invalid_step_size);
    return;
  }
  if (ev.seq_primary_ids == nullptr) {
    detail::fail_plan(ctx, emel::batch::planner::error::invalid_sequence_id);
    return;
  }
  ...
  for (int32_t i = 0; i < ev.n_tokens; ++i) {
    const int32_t seq_id = ev.seq_primary_ids[i];
  ...
  while (remaining > 0) {
    ...
    for (int32_t i = 0; i < ev.n_tokens; ++i) {
      ...
      if (ev.equal_sequential && group_count > 0 && seq_id != last_primary + 1) {
        continue;
      }
      group_used[slot] = 1;
    ...
    for (int32_t g = 0; g < group_count; ++g) {
      ...
      for (int32_t i = 0; i < n_seq_tokens; ++i) {
        ...
        if (!detail::append_token_index(ctx, idx)) {
          detail::fail_plan(
              ctx, emel::batch::planner::error::output_indices_full);
          return;
        }
...
inline constexpr auto create_plan =
    [](const event::request & ev, context & ctx) noexcept {
      create_plan_impl(ev, ctx);
    };

inline constexpr auto create_plan_primary_fast_path =
    [](const event::request & ev, context & ctx) noexcept {
      create_plan_primary_fast_path_impl(ev, ctx);
    };

inline constexpr auto create_plan_general = [](const event::request & ev, context & ctx) noexcept {
  create_plan(ev, ctx);
};
