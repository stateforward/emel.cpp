...
inline void create_plan_impl(const event::request_runtime & ev) noexcept {
  ...
  while (used_count < ev.request.n_tokens) {
    ...
    for (int32_t i = 0; i < ev.request.n_tokens; ++i) {
      ...
      const seq_mask_t mask = detail::normalized_seq_mask(ev.request, i);
      bool overlap = false;
    ...
    if (group_count == 0) {
      detail::fail_plan(ev, emel::batch::planner::error::planning_progress_stalled);
      return;
    }
    ...
    for (int32_t g = 0; g < group_count; ++g) {
      ...
      for (int32_t i = 0; i < ev.request.n_tokens; ++i) {
        ...
        if (detail::mask_equal(detail::normalized_seq_mask(ev.request, i), groups[g].mask)) {
          avail += 1;
        }
      }
    ...
    for (int32_t g = 0; g < group_count; ++g) {
      ...
      if (remaining != 0) {
        detail::fail_plan(ev, emel::batch::planner::error::algorithm_failed);
        return;
      }
    }

    const int32_t added = n_seq_tokens * group_count;
    if (!detail::push_step_size(ev.ctx, added)) {
      detail::fail_plan(ev, emel::batch::planner::error::output_steps_full);
      return;
    }
  }
...
inline void create_plan_primary_fast_path_impl(const event::request_runtime & ev) noexcept {
  ...
  if (ev.request.seq_primary_ids == nullptr) {
    detail::fail_plan(ev, emel::batch::planner::error::invalid_sequence_id);
    return;
  }
