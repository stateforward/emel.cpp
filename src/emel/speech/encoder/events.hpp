...
namespace emel::speech::encoder {

// Component execution contract: variant-neutral encoder dimensions bound from a
// loaded model by a variant binder (e.g.
// encoder::whisper::bind_execution_contract) at the variant boundary. The
...
namespace emel::speech::encoder::events {
...
namespace emel::speech::encoder::event {

struct encode {
  encode(const execution_contract &contract_ref, std::span<const float> pcm_ref,
         const int32_t sample_rate_ref, const int32_t channel_count_ref,
         std::span<float> workspace_ref, std::span<float> encoder_state_ref,
         int32_t &frame_count_out_ref, int32_t &width_out_ref,
