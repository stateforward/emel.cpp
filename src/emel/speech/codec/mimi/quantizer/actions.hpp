...
template <bool conv_f16> struct effect_run_quantize {
  void operator()(const event::encode_run &runtime_ev,
    ...
    runtime_ev.ctx.stage_ok = mimi::detail::compute_rvq_encode<conv_f16>(
...
template <bool conv_f16> struct effect_run_dequantize {
  void operator()(const event::decode_run &runtime_ev,
                  context &ctx) const noexcept {
    const auto &request = runtime_ev.request;
    mimi::detail::frame_buffer latent{request.latent_out.data(),
                                      request.runtime.dim, 1};
    runtime_ev.ctx.stage_ok = mimi::detail::compute_rvq_decode<conv_f16>(
