...
struct run {
  run(const emel::model::sortformer::execution_contract & contract_ref,
      std::span<const float> pcm_ref,
      const int32_t sample_rate_ref,
      const int32_t channel_count_ref,
      std::span<float> probabilities_ref,
      std::span<emel::diarization::sortformer::output::segment_record> segments_ref,
  ...
  std::span<emel::diarization::sortformer::output::segment_record> segments = {};
