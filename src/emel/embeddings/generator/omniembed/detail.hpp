...
#include "emel/embeddings/generator/omniembed/context.hpp"
...
namespace emel::embeddings::generator::omniembed::detail {
...
inline constexpr std::array<image_ui_block_config, 21> k_image_ui_block_configs = {{
    {1, 0, 2, 48, 80, true, true, false},
    {1, 1, 1, 80, 80, true, true, true},
    {2, 0, 2, 80, 160, true, true, false},
    {2, 1, 1, 160, 160, true, true, true},
    {2, 2, 1, 160, 160, true, true, true},
    {2, 3, 1, 160, 160, true, true, true},
    {2, 4, 1, 160, 160, true, true, true},
    {2, 5, 1, 160, 160, true, false, true},
    {2, 6, 1, 160, 160, false, false, true},
    {2, 7, 1, 160, 160, true, false, true},
    {3, 0, 2, 160, 256, true, true, false},
    {3, 1, 1, 256, 256, true, true, true},
    {3, 2, 1, 256, 256, true, true, true},
    {3, 3, 1, 256, 256, true, true, true},
    {3, 4, 1, 256, 256, false, false, true},
    {3, 5, 1, 256, 256, true, false, true},
    {3, 6, 1, 256, 256, true, true, true},
    {3, 7, 1, 256, 256, true, true, true},
    {3, 8, 1, 256, 256, false, false, true},
    {3, 9, 1, 256, 256, false, false, true},
    {3, 10, 1, 256, 256, true, false, true},
}};
...
inline bool bind_image_ui_block(const emel::model::data & model,
                                const image_ui_block_config & config,
  ...
  const auto find_block_tensor = [&](const std::string_view suffix) noexcept
    ...
    return make_image_block_name(config.stage_index, config.block_index, suffix, buffer, name) ?
  ...
  if (!config.has_dw_start && dw_start != nullptr) {
    return false;
  }
  ...
  if (!config.has_dw_mid && dw_mid != nullptr) {
    return false;
  }

  if (!bind_pointwise_f16(*pw_exp,
                          expanded_channels,
                          config.input_channels,
                          runtime_out.pw_exp) ||
...
inline bool build_text_runtime(
    const emel::model::data & model,
    const action::embedding_execution_contract & contract,
  ...
  runtime_out.route_kind = action::text_route_kind::encoder;
  runtime_out.ready = true;
  return true;
}
...
inline bool build_image_runtime(
  ...
  for (const auto & config : k_image_ui_block_configs) {
    if (config.input_channels != channels) {
      return false;
    }
    auto & block = runtime_out.blocks[static_cast<size_t>(runtime_out.block_count)];
    if (!bind_image_ui_block(model, config, block)) {
      return false;
    }
    ...
    if (config.has_dw_mid) {
      output_spatial = output_dim_same(expansion_spatial, block.dw_mid.kernel_h, config.stride);
    }
  ...
  runtime_out.route_kind = action::image_route_kind::encoder;
...
inline constexpr std::array<audio_block_config, 15> k_audio_block_configs = {{
...
inline bool build_audio_runtime(
  ...
  for (int32_t index = 0; index < static_cast<int32_t>(k_audio_block_configs.size()); ++index) {
    auto & block = runtime_out.blocks[static_cast<size_t>(index)];
    if (!bind_audio_block(model, index + 1, k_audio_block_configs[static_cast<size_t>(index)], block)) {
      return false;
    }
    ++runtime_out.block_count;

    const auto & config = k_audio_block_configs[static_cast<size_t>(index)];
  ...
  runtime_out.route_kind = action::audio_route_kind::encoder;
  runtime_out.ready = runtime_out.input_sample_count > 0 &&
      ...
      runtime_out.block_count == static_cast<int32_t>(k_audio_block_configs.size());
  return runtime_out.ready;
}

inline bool bind_embedding_execution_contract(
    const emel::model::data & model,
    action::embedding_execution_contract & contract_out) noexcept {
  contract_out = {};

  emel::model::omniembed::detail::execution_contract model_contract = {};
  if (emel::model::omniembed::detail::build_execution_contract(model, model_contract) !=
      emel::error::cast(emel::model::loader::error::none)) {
    return false;
  }

  contract_out.model = model_contract.model;
  contract_out.embedding_length = model_contract.embedding_length;
  contract_out.image_encoder_length = model_contract.image_encoder_length;
  contract_out.audio_encoder_length = model_contract.audio_encoder_length;
  contract_out.matryoshka_dimension_count = model_contract.matryoshka_dimension_count;
  contract_out.matryoshka_dimensions = model_contract.matryoshka_dimensions;
  return true;
}

inline emel::embeddings::generator::action::context &
base_context(action::context & ctx) noexcept {
  return ctx;
}

inline const emel::embeddings::generator::action::context &
base_context(const action::context & ctx) noexcept {
  return ctx;
}

inline void destroy_runtime_state(void * ptr) noexcept {
  delete static_cast<emel::embeddings::generator::omniembed::runtime_state *>(ptr);
}

inline emel::embeddings::generator::omniembed::runtime_state *
runtime_state_or_null(action::context & ctx) noexcept {
  return static_cast<emel::embeddings::generator::omniembed::runtime_state *>(ctx.route.data);
}

inline const emel::embeddings::generator::omniembed::runtime_state *
runtime_state_or_null(const action::context & ctx) noexcept {
  return static_cast<const emel::embeddings::generator::omniembed::runtime_state *>(ctx.route.data);
}

inline emel::embeddings::generator::omniembed::runtime_state &
runtime(action::context & ctx) noexcept {
  return *runtime_state_or_null(ctx);
}

inline const emel::embeddings::generator::omniembed::runtime_state &
runtime(const action::context & ctx) noexcept {
  return *runtime_state_or_null(ctx);
}

inline void reset_base_route_status(action::context & ctx) noexcept {
  auto & base = base_context(ctx);
...
inline void publish_base_route_status(action::context & ctx) noexcept {
  auto & base = base_context(ctx);
  base.text_status.ready = runtime(ctx).text.ready;
  base.text_status.route_kind = runtime(ctx).text.route_kind;
  base.text_status.max_positions = runtime(ctx).text.max_positions;
  base.text_status.shared_embedding_size = runtime(ctx).text.shared_embedding_size;
  base.image_status.ready = runtime(ctx).image.ready;
  base.image_status.route_kind = runtime(ctx).image.route_kind;
  base.audio_status.ready = runtime(ctx).audio.ready;
  base.audio_status.route_kind = runtime(ctx).audio.route_kind;
  base.scratch_status.token_ids = runtime(ctx).scratch.token_ids.get();
  base.scratch_status.full_embedding = runtime(ctx).scratch.full_embedding.get();
  base.scratch_status.ready = runtime(ctx).scratch.ready;
}

inline bool reserve_scratch(action::context & ctx, const emel::model::data & model) noexcept {
  ctx.route.reset();
  reset_base_route_status(ctx);
  ctx.execution_contract = {};
  auto state = std::unique_ptr<emel::embeddings::generator::omniembed::runtime_state>{
      new (std::nothrow) emel::embeddings::generator::omniembed::runtime_state};
  if (state == nullptr) {
    return false;
  }
  ctx.route.reset(state.release(), destroy_runtime_state);

  if (!bind_embedding_execution_contract(model, ctx.execution_contract) ||
      !build_text_runtime(model, ctx.execution_contract, runtime(ctx).text) ||
      !build_image_runtime(model, ctx.execution_contract, runtime(ctx).image) ||
      !build_audio_runtime(model, ctx.execution_contract, runtime(ctx).audio)) {
    runtime(ctx).text = {};
    runtime(ctx).image = {};
    runtime(ctx).audio = {};
    runtime(ctx).scratch = {};
  ...
  const size_t token_count = static_cast<size_t>(runtime(ctx).text.max_positions);
  const size_t hidden_size = static_cast<size_t>(runtime(ctx).text.hidden_size);
  const size_t intermediate_size = static_cast<size_t>(runtime(ctx).text.intermediate_size);
  const size_t output_size = static_cast<size_t>(runtime(ctx).text.output_size);
  const size_t projection_hidden = static_cast<size_t>(std::max(
      runtime(ctx).text.projection_hidden_size,
      std::max(runtime(ctx).image.ready ? runtime(ctx).image.projection.hidden_size : 0,
               runtime(ctx).audio.ready ? runtime(ctx).audio.projection.hidden_size : 0)));
  const size_t projection_residual = static_cast<size_t>(std::max(
      static_cast<int32_t>(projection_hidden),
      runtime(ctx).audio.ready ? runtime(ctx).audio.max_dense_input_size : 0));
  const size_t shared_embedding = static_cast<size_t>(runtime(ctx).text.shared_embedding_size);
  const size_t image_input_elements =
      runtime(ctx).image.ready ? static_cast<size_t>(3 * runtime(ctx).image.input_size * runtime(ctx).image.input_size) : 0u;
  const size_t image_feature_elements =
      runtime(ctx).image.ready ? static_cast<size_t>(runtime(ctx).image.feature_buffer_elements) : 0u;
  const size_t image_embedding_size =
      runtime(ctx).image.ready ? static_cast<size_t>(runtime(ctx).image.embedding_size) : 0u;
  const size_t audio_waveform_size =
      runtime(ctx).audio.ready ? static_cast<size_t>(runtime(ctx).audio.resampled_sample_count) : 0u;
  const size_t audio_preemphasized_size =
      runtime(ctx).audio.ready ? static_cast<size_t>(runtime(ctx).audio.preemphasized_sample_count) : 0u;
  const size_t audio_fft_frame_size =
      runtime(ctx).audio.ready ? static_cast<size_t>(runtime(ctx).audio.n_fft) : 0u;
  const size_t audio_power_size =
      runtime(ctx).audio.ready ? static_cast<size_t>(runtime(ctx).audio.n_fft / 2 + 1) : 0u;
  const size_t audio_input_elements =
      runtime(ctx).audio.ready ? static_cast<size_t>(runtime(ctx).audio.num_mel_bins * runtime(ctx).audio.time_frames) : 0u;
  const size_t audio_feature_elements =
      runtime(ctx).audio.ready ? static_cast<size_t>(runtime(ctx).audio.feature_buffer_elements) : 0u;
  const size_t audio_embedding_size =
      runtime(ctx).audio.ready ? static_cast<size_t>(runtime(ctx).audio.embedding_size) : 0u;
  const size_t max_q8_cols = static_cast<size_t>(std::max(
      runtime(ctx).text.hidden_size,
      std::max(runtime(ctx).text.intermediate_size,
               std::max(runtime(ctx).text.output_size,
                        std::max(std::max(runtime(ctx).text.projection_hidden_size,
                                          runtime(ctx).image.ready ? runtime(ctx).image.projection.hidden_size : 0),
                                 std::max(runtime(ctx).audio.ready ? runtime(ctx).audio.projection.hidden_size : 0,
                                          runtime(ctx).audio.ready ? runtime(ctx).audio.max_dense_input_size : 0))))));
  ...
  runtime(ctx).scratch = std::move(scratch);
  publish_base_route_status(ctx);
  return runtime(ctx).scratch.ready;
}
...
inline int32_t shared_embedding_size(const action::context & ctx) noexcept {
  return ctx.model != nullptr ? ctx.model->params.n_embd_out : runtime(ctx).text.shared_embedding_size;
}
...
inline bool is_supported_truncate_dimension(const action::context & ctx,
  ...
  if (dimension == runtime(ctx).text.shared_embedding_size) {
    return true;
  }
...
inline bool embed_tokens(action::context & ctx, const int32_t token_count) noexcept {
  auto word = std::span<float>{runtime(ctx).scratch.token_hidden.get(), static_cast<size_t>(runtime(ctx).text.hidden_size)};
  auto pos = std::span<float>{runtime(ctx).scratch.projection_residual.get(), static_cast<size_t>(runtime(ctx).text.hidden_size)};
  auto type = std::span<float>{runtime(ctx).scratch.pooled.get(), static_cast<size_t>(runtime(ctx).text.hidden_size)};
  for (int32_t token_index = 0; token_index < token_count; ++token_index) {
    const int32_t token_id = runtime(ctx).scratch.token_ids[static_cast<size_t>(token_index)];
    if (token_id < 0 || token_id >= runtime(ctx).text.word_embeddings.rows) {
      return false;
    }
    if (!copy_embedding_row(runtime(ctx).text.word_embeddings, token_id, word) ||
        !copy_embedding_row(runtime(ctx).text.position_embeddings, token_index, pos) ||
        !copy_embedding_row(runtime(ctx).text.token_type_embeddings, 0, type)) {
      return false;
    }
    auto out = token_span(runtime(ctx).scratch.sequence_a.get(), token_index, runtime(ctx).text.hidden_size);
    for (int32_t feature = 0; feature < runtime(ctx).text.hidden_size; ++feature) {
    ...
    if (!layer_norm(out,
                    runtime(ctx).text.embeddings_norm_weight,
                    runtime(ctx).text.embeddings_norm_bias,
                    runtime(ctx).text.encoder_layer_norm_epsilon,
                    out)) {
      return false;
    }
  }
...
inline bool run_attention_layer(action::context & ctx,
  ...
  auto q8_input = std::span<emel::kernel::detail::quant::block_q8_0>{
      runtime(ctx).scratch.q8_input.get(),
      runtime(ctx).scratch.q8_input_block_capacity,
  };
  for (int32_t token_index = 0; token_index < token_count; ++token_index) {
    const auto input = token_span(sequence_in, token_index, runtime(ctx).text.hidden_size);
    auto query = token_span(runtime(ctx).scratch.query.get(), token_index, runtime(ctx).text.hidden_size);
    auto key = token_span(runtime(ctx).scratch.key.get(), token_index, runtime(ctx).text.hidden_size);
    auto value = token_span(runtime(ctx).scratch.value.get(), token_index, runtime(ctx).text.hidden_size);
    if (!matmul(runtime(ctx).kernel, layer.attention_query, input, q8_input, query) ||
        !add_bias(query, layer.attention_query_bias) ||
        !matmul(runtime(ctx).kernel, layer.attention_key, input, q8_input, key) ||
        !add_bias(key, layer.attention_key_bias) ||
        !matmul(runtime(ctx).kernel, layer.attention_value, input, q8_input, value) ||
  ...
  for (int32_t token_index = 0; token_index < token_count; ++token_index) {
    auto query = token_span(runtime(ctx).scratch.query.get(), token_index, runtime(ctx).text.hidden_size);
    auto context = token_span(runtime(ctx).scratch.attention_context.get(), token_index, runtime(ctx).text.hidden_size);
    std::fill(context.begin(), context.end(), 0.0f);

    for (int32_t head_index = 0; head_index < runtime(ctx).text.attention_head_count; ++head_index) {
      const auto query_head =
          head_span(std::span<const float>{query.data(), query.size()},
                    head_index,
                    runtime(ctx).text.attention_head_dim);
      auto scores = std::span<float>{runtime(ctx).scratch.attention_scores.get(), static_cast<size_t>(token_count)};
      for (int32_t other_index = 0; other_index < token_count; ++other_index) {
        const auto key_head =
            head_span(token_span(runtime(ctx).scratch.key.get(), other_index, runtime(ctx).text.hidden_size),
                      head_index,
                      runtime(ctx).text.attention_head_dim);
      ...
      auto context_head = head_span(context, head_index, runtime(ctx).text.attention_head_dim);
      for (int32_t other_index = 0; other_index < token_count; ++other_index) {
        const auto value_head =
            head_span(token_span(runtime(ctx).scratch.value.get(), other_index, runtime(ctx).text.hidden_size),
                      head_index,
                      runtime(ctx).text.attention_head_dim);
        const float weight = scores[static_cast<size_t>(other_index)];
        for (int32_t feature = 0; feature < runtime(ctx).text.attention_head_dim; ++feature) {
    ...
    auto hidden = std::span<float>{runtime(ctx).scratch.token_hidden.get(), static_cast<size_t>(runtime(ctx).text.hidden_size)};
    const auto input = token_span(sequence_in, token_index, runtime(ctx).text.hidden_size);
    if (!matmul(runtime(ctx).kernel, layer.attention_output, context, q8_input, hidden) ||
    ...
    auto feed_forward = std::span<float>{runtime(ctx).scratch.feed_forward.get(), static_cast<size_t>(runtime(ctx).text.intermediate_size)};
    if (!matmul(runtime(ctx).kernel, layer.intermediate, hidden, q8_input, feed_forward) ||
        !add_bias(feed_forward, layer.intermediate_bias)) {
      return false;
    }
    apply_gelu(feed_forward);

    auto output = token_span(sequence_out, token_index, runtime(ctx).text.hidden_size);
    if (!matmul(runtime(ctx).kernel, layer.output, feed_forward, q8_input, output) ||
...
inline bool mean_pool(action::context & ctx, const int32_t token_count, const float * sequence) noexcept {
  auto pooled = std::span<float>{runtime(ctx).scratch.pooled.get(), static_cast<size_t>(runtime(ctx).text.hidden_size)};
  std::fill(pooled.begin(), pooled.end(), 0.0f);
  const float scale = 1.0f / static_cast<float>(token_count);
  for (int32_t token_index = 0; token_index < token_count; ++token_index) {
    const auto row = token_span(sequence, token_index, runtime(ctx).text.hidden_size);
    for (int32_t feature = 0; feature < runtime(ctx).text.hidden_size; ++feature) {
...
inline bool prepare_image_input(action::context & ctx,
  ...
  if (ctx.model == nullptr ||
      !runtime(ctx).image.ready ||
      !is_valid_image_payload(rgba, width, height) ||
      runtime(ctx).scratch.image_input == nullptr) {
    return false;
  }

  const int32_t output_size = runtime(ctx).image.input_size;
  ...
  float * output = runtime(ctx).scratch.image_input.get();
...
inline bool prepare_audio_input(action::context & ctx,
                                ...
                                const int32_t sample_rate) noexcept {
  if (ctx.model == nullptr ||
      !runtime(ctx).audio.ready ||
      !is_valid_audio_payload(pcm, sample_rate) ||
      runtime(ctx).scratch.audio_waveform == nullptr ||
      runtime(ctx).scratch.audio_preemphasized == nullptr ||
      runtime(ctx).scratch.audio_fft_frame == nullptr ||
      runtime(ctx).scratch.audio_power == nullptr ||
      runtime(ctx).scratch.audio_input == nullptr ||
      runtime(ctx).audio.fft_window == nullptr ||
      runtime(ctx).audio.mel_filters == nullptr ||
      runtime(ctx).audio.fft_twiddle_cos == nullptr ||
      runtime(ctx).audio.fft_twiddle_sin == nullptr ||
      runtime(ctx).audio.fft_bit_reverse == nullptr ||
      runtime(ctx).audio.mel_bin_start == nullptr ||
      runtime(ctx).audio.mel_bin_end == nullptr) {
    return false;
  }

  float * resampled = runtime(ctx).scratch.audio_waveform.get();
  for (int32_t index = 0; index < runtime(ctx).audio.resampled_sample_count; ++index) {
    const float source_position =
        static_cast<float>(index) *
        static_cast<float>(sample_rate) /
        static_cast<float>(runtime(ctx).audio.resampled_sample_rate);
    const int32_t left = std::min(
        static_cast<int32_t>(source_position),
        runtime(ctx).audio.input_sample_count - 1);
    const int32_t right = std::min(left + 1, runtime(ctx).audio.input_sample_count - 1);
  ...
  float * preemphasized = runtime(ctx).scratch.audio_preemphasized.get();
  for (int32_t index = 0; index < runtime(ctx).audio.preemphasized_sample_count; ++index) {
    preemphasized[index] = resampled[index + 1] -
        runtime(ctx).audio.preemphasis_coefficient * resampled[index];
  }

  const int32_t fft_bin_count = runtime(ctx).audio.n_fft / 2 + 1;
  for (int32_t frame = 0; frame < runtime(ctx).audio.time_frames; ++frame) {
    float * fft_frame = runtime(ctx).scratch.audio_fft_frame.get();
    for (int32_t sample_index = 0; sample_index < runtime(ctx).audio.n_fft; ++sample_index) {
      const int32_t centered_index = frame * runtime(ctx).audio.hop_size + sample_index - runtime(ctx).audio.n_fft / 2;
      const int32_t reflected_index =
          reflect_index(centered_index, runtime(ctx).audio.preemphasized_sample_count);
      fft_frame[sample_index] =
          preemphasized[reflected_index] * runtime(ctx).audio.fft_window[sample_index];
    }

    float * fft_state = runtime(ctx).scratch.audio_waveform.get();
    float * power = runtime(ctx).scratch.audio_power.get();
    if (!compute_fft_power_spectrum(runtime(ctx).audio, fft_frame, fft_state, power)) {
      return false;
    }

    for (int32_t mel_bin = 0; mel_bin < runtime(ctx).audio.num_mel_bins; ++mel_bin) {
      const float * weights =
          runtime(ctx).audio.mel_filters.get() + static_cast<size_t>(mel_bin * fft_bin_count);
      const int32_t fft_start = runtime(ctx).audio.mel_bin_start[static_cast<size_t>(mel_bin)];
      const int32_t fft_end = runtime(ctx).audio.mel_bin_end[static_cast<size_t>(mel_bin)];
      float energy = 0.0f;
      for (int32_t fft_bin = fft_start; fft_bin < fft_end; ++fft_bin) {
        energy += weights[fft_bin] * power[fft_bin];
      }
      const float value = std::log(energy + runtime(ctx).audio.log_offset);
      runtime(ctx).scratch.audio_input[static_cast<size_t>(mel_bin * runtime(ctx).audio.time_frames + frame)] =
          (value + runtime(ctx).audio.normalize_bias) / runtime(ctx).audio.normalize_scale;
    }
  }
...
inline bool run_audio_se(action::context & ctx,
  ...
  if (!se.ready ||
      values == nullptr ||
      runtime(ctx).scratch.audio_embedding == nullptr ||
      runtime(ctx).scratch.projection_hidden == nullptr ||
      runtime(ctx).scratch.projection_residual == nullptr) {
    return false;
  }

  auto pooled = std::span<float>{runtime(ctx).scratch.audio_embedding.get(), static_cast<size_t>(se.input_size)};
  auto hidden = std::span<float>{runtime(ctx).scratch.projection_hidden.get(), static_cast<size_t>(se.hidden_size)};
  auto scale = std::span<float>{runtime(ctx).scratch.projection_residual.get(), static_cast<size_t>(se.input_size)};
  auto q8_input = std::span<emel::kernel::detail::quant::block_q8_0>{
      runtime(ctx).scratch.q8_input.get(),
      runtime(ctx).scratch.q8_input_block_capacity,
  };
  ...
  if (!matmul(runtime(ctx).kernel, se.fc1, pooled, q8_input, hidden) ||
      !add_bias(hidden, se.fc1_bias)) {
    return false;
  }
  apply_activation_in_place<true, false>(hidden);
  if (!matmul(runtime(ctx).kernel, se.fc2, hidden, q8_input, scale) ||
...
template <bool has_expand, bool use_hardswish, bool has_se, bool has_skip>
inline bool run_audio_block_chosen(action::context & ctx,
  ...
  if (!block.ready ||
      ...
      runtime(ctx).scratch.audio_c == nullptr) {
    return false;
  }
  ...
  if constexpr (has_skip) {
    std::memcpy(runtime(ctx).scratch.audio_c.get(), current_buffer, input_elements * sizeof(float));
  }
  ...
  if constexpr (has_expand) {
    ...
    if constexpr (use_hardswish) {
      apply_batch_norm_hwc_rect<false, true>(
          alternate_buffer,
          height,
          width,
          block.expand_bn,
          runtime(ctx).audio.batch_norm_epsilon);
    } else {
  ...
  if constexpr (has_se) {
    if (!run_audio_se(ctx, block.se, depthwise_dst, output_height, output_width)) {
      return false;
    }
  }
  ...
  if constexpr (has_skip) {
    if (!add_in_place(
            ...
            std::span<const float>{runtime(ctx).scratch.audio_c.get(), input_elements})) {
      return false;
    }
  }

  current_buffer = project_dst;
  alternate_buffer = project_dst == runtime(ctx).scratch.audio_a.get() ?
      runtime(ctx).scratch.audio_b.get() :
      runtime(ctx).scratch.audio_a.get();
  height = output_height;
  width = output_width;
  return true;
}

template <size_t block_index>
inline bool run_audio_block_at(action::context & ctx,
                               float *& current_buffer,
                               float *& alternate_buffer,
                               int32_t & height,
                               int32_t & width) noexcept {
  static constexpr audio_block_config config =
      k_audio_block_configs[block_index];
  static constexpr bool has_expand =
      config.expanded_channels != config.input_channels;
  static constexpr bool has_skip =
      config.stride == 1 && config.input_channels == config.output_channels;
  return run_audio_block_chosen<has_expand,
                                config.use_hardswish,
                                config.use_se,
                                has_skip>(
      ctx,
      runtime(ctx).audio.blocks[block_index],
      current_buffer,
      alternate_buffer,
      height,
      width);
...
template <size_t block_index = 0u>
inline bool run_audio_block_sequence(action::context & ctx,
                                     float *& current_buffer,
                                     float *& alternate_buffer,
                                     int32_t & height,
                                     int32_t & width) noexcept {
  if (!run_audio_block_at<block_index>(
          ctx, current_buffer, alternate_buffer, height, width)) {
    return false;
  }
  if constexpr (block_index + 1u < k_audio_block_configs.size()) {
    return run_audio_block_sequence<block_index + 1u>(
        ctx, current_buffer, alternate_buffer, height, width);
  }
  return true;
}
...
inline emel::error::type run_audio_embedding(action::context & ctx) noexcept {
  if (!runtime(ctx).audio.ready ||
      !runtime(ctx).scratch.ready ||
      runtime(ctx).scratch.audio_input == nullptr ||
      runtime(ctx).scratch.audio_a == nullptr ||
      runtime(ctx).scratch.audio_b == nullptr ||
      runtime(ctx).scratch.audio_embedding == nullptr ||
      runtime(ctx).scratch.projection_residual == nullptr) {
    return to_error(error::backend);
  }
  float * conv_patch = runtime(ctx).scratch.projection_residual.get();
  const size_t conv_patch_capacity = shared_dense_scratch_capacity(ctx);

  int32_t height = 0;
  int32_t width = 0;
  if (!standard_conv_hwc_rect(runtime(ctx).kernel,
                              runtime(ctx).audio.stem.conv,
                              runtime(ctx).scratch.audio_input.get(),
                              runtime(ctx).audio.num_mel_bins,
                              runtime(ctx).audio.time_frames,
                              2,
                              2,
                              conv_patch,
                              conv_patch_capacity,
  ...
  apply_batch_norm_hwc_rect<false, true>(
      ...
      height,
      width,
      runtime(ctx).audio.stem.norm,
      runtime(ctx).audio.batch_norm_epsilon);
  float * current_buffer = runtime(ctx).scratch.audio_a.get();
  float * alternate_buffer = runtime(ctx).scratch.audio_b.get();

  if (runtime(ctx).audio.block_count != static_cast<int32_t>(k_audio_block_configs.size()) ||
      !run_audio_block_sequence(ctx, current_buffer, alternate_buffer, height, width)) {
    return to_error(error::backend);
  }
  ...
  apply_batch_norm_hwc_rect<false, true>(
      ...
      head_height,
      head_width,
      runtime(ctx).audio.head.norm,
      runtime(ctx).audio.batch_norm_epsilon);
  average_pool_hwc_rect(
      alternate_buffer,
      head_height,
      head_width,
      runtime(ctx).audio.embedding_size,
      runtime(ctx).scratch.audio_embedding.get());
  if (!run_projection_head(
          ctx,
          runtime(ctx).audio.projection,
          std::span<const float>{runtime(ctx).scratch.audio_embedding.get(),
                                 static_cast<size_t>(runtime(ctx).audio.embedding_size)})) {
    return to_error(error::backend);
  }
...
inline bool run_edge_residual(action::context & ctx, int32_t & spatial) noexcept {
  if (!runtime(ctx).image.stage0.ready ||
      runtime(ctx).scratch.projection_residual == nullptr) {
    return false;
  }
  float * conv_patch = runtime(ctx).scratch.projection_residual.get();
  ...
  if (!standard_conv_hwc_bn<true>(
          runtime(ctx).image.stage0.conv_exp,
          runtime(ctx).scratch.image_a.get(),
          spatial,
          runtime(ctx).image.stage0.stride,
          conv_patch,
          conv_patch_capacity,
          runtime(ctx).image.stage0.bn1,
          runtime(ctx).scratch.image_b.get(),
          stage_spatial)) {
    return false;
  }
  if (!pointwise_conv_hwc_direct_f32_bn<false>(runtime(ctx).image.stage0.conv_pwl,
                                               runtime(ctx).scratch.image_b.get(),
                                               stage_spatial * stage_spatial,
                                               runtime(ctx).image.stage0.bn2,
                                               runtime(ctx).scratch.image_a.get())) {
    return false;
  }
...
template <bool has_dw_start, bool has_dw_mid, bool has_skip>
inline bool run_universal_inverted_block_chosen(
    action::context & ctx,
  ...
  const size_t input_elements =
      ...
      static_cast<size_t>(block.input_channels);
  if constexpr (has_skip) {
    std::memcpy(runtime(ctx).scratch.image_c.get(), current_buffer, input_elements * sizeof(float));
  }

  const float * expansion_src = current_buffer;
  float * expansion_dst = alternate_buffer;
  int32_t expansion_spatial = spatial;
  if constexpr (has_dw_start) {
    const int32_t dw_start_stride = has_dw_mid ? 1 : block.stride;
    ...
    apply_batch_norm_hwc<false>(
        ...
        expansion_spatial,
        block.dw_start_bn,
        runtime(ctx).image.batch_norm_epsilon);
  ...
  if constexpr (has_skip) {
    if (!add_in_place(
            ...
            std::span<const float>{runtime(ctx).scratch.image_c.get(), input_elements})) {
      return false;
    }
  }

  current_buffer = projection_dst;
  alternate_buffer = projection_dst == runtime(ctx).scratch.image_a.get() ?
      runtime(ctx).scratch.image_b.get() :
      runtime(ctx).scratch.image_a.get();
  spatial = output_spatial;
  return true;
...
template <size_t block_index>
inline bool run_image_ui_block_at(action::context & ctx,
                                  float *& current_buffer,
                                  float *& alternate_buffer,
                                  int32_t & spatial) noexcept {
  static constexpr image_ui_block_config config =
      k_image_ui_block_configs[block_index];
  return run_universal_inverted_block_chosen<config.has_dw_start,
                                             config.has_dw_mid,
                                             config.has_skip>(
      ctx,
      runtime(ctx).image.blocks[block_index],
      current_buffer,
      alternate_buffer,
      spatial);
}
...
template <size_t block_index = 0u>
inline bool run_image_ui_block_sequence(action::context & ctx,
                                        float *& current_buffer,
                                        float *& alternate_buffer,
                                        int32_t & spatial) noexcept {
  if (!run_image_ui_block_at<block_index>(
          ctx, current_buffer, alternate_buffer, spatial)) {
    return false;
  }
  if constexpr (block_index + 1u < k_image_ui_block_configs.size()) {
    return run_image_ui_block_sequence<block_index + 1u>(
        ctx, current_buffer, alternate_buffer, spatial);
  }
  return true;
}
...
inline emel::error::type run_image_embedding(action::context & ctx) noexcept {
  if (!runtime(ctx).image.ready ||
      !runtime(ctx).scratch.ready ||
      runtime(ctx).scratch.image_input == nullptr ||
      runtime(ctx).scratch.image_a == nullptr ||
      runtime(ctx).scratch.image_b == nullptr ||
      runtime(ctx).scratch.image_c == nullptr ||
      runtime(ctx).scratch.image_embedding == nullptr ||
      runtime(ctx).scratch.projection_residual == nullptr) {
    return to_error(error::backend);
  }
  float * conv_patch = runtime(ctx).scratch.projection_residual.get();
  ...
  int32_t spatial = 0;
  if (!standard_conv_hwc_bn<true>(
          runtime(ctx).image.stem,
          runtime(ctx).scratch.image_input.get(),
          runtime(ctx).image.input_size,
          2,
          conv_patch,
          conv_patch_capacity,
          runtime(ctx).image.stem_bn,
          runtime(ctx).scratch.image_a.get(),
          spatial)) {
    return to_error(error::backend);
  }
  ...
  float * current_buffer = runtime(ctx).scratch.image_a.get();
  float * alternate_buffer = runtime(ctx).scratch.image_b.get();

  if (runtime(ctx).image.block_count != static_cast<int32_t>(k_image_ui_block_configs.size()) ||
      !run_image_ui_block_sequence(ctx, current_buffer, alternate_buffer, spatial)) {
    return to_error(error::backend);
  }

  if (!pointwise_conv_hwc_direct_f32_bn<true>(runtime(ctx).image.stage4.conv,
                                              current_buffer,
                                              spatial * spatial,
                                              runtime(ctx).image.stage4.norm,
                                              alternate_buffer)) {
    return to_error(error::backend);
  }
  ...
  average_pool_hwc(
      current_buffer,
      spatial,
      runtime(ctx).image.stage4.output_channels,
      runtime(ctx).scratch.projection_hidden.get());
  if (!pointwise_conv_hwc_direct_f32_bn<true>(runtime(ctx).image.head.conv,
                                              runtime(ctx).scratch.projection_hidden.get(),
                                              1,
                                              runtime(ctx).image.head.norm,
                                              runtime(ctx).scratch.image_embedding.get())) {
    return to_error(error::backend);
  }

  if (!run_projection_head(
          ctx,
          runtime(ctx).image.projection,
          std::span<const float>{
              runtime(ctx).scratch.image_embedding.get(),
              static_cast<size_t>(runtime(ctx).image.embedding_size),
          })) {
    return to_error(error::backend);
  }
  return to_error(error::none);
...
inline bool run_projection_head(action::context & ctx,
  ...
  auto projection_hidden = std::span<float>{
      runtime(ctx).scratch.projection_hidden.get(), static_cast<size_t>(projection.hidden_size)};
  auto projection_residual = std::span<float>{
      runtime(ctx).scratch.projection_residual.get(), static_cast<size_t>(projection.hidden_size)};
  auto full_embedding = std::span<float>{
      runtime(ctx).scratch.full_embedding.get(), static_cast<size_t>(projection.output_size)};

  if (!matmul(runtime(ctx).kernel, projection.expand, input_embedding, q8_input, projection_hidden) ||
  ...
  if (!matmul(runtime(ctx).kernel, projection.residual, projection_hidden, q8_input, projection_residual) ||
  ...
  return matmul(runtime(ctx).kernel, projection.project, projection_residual, q8_input, full_embedding) &&
...
inline bool run_text_projection(action::context & ctx) noexcept {
  ...
  auto pooled = std::span<const float>{runtime(ctx).scratch.pooled.get(), static_cast<size_t>(runtime(ctx).text.hidden_size)};
  auto text_embedding =
      std::span<float>{runtime(ctx).scratch.text_embedding.get(), static_cast<size_t>(runtime(ctx).text.output_size)};
  if (!matmul(runtime(ctx).kernel, runtime(ctx).text.dense, pooled, q8_input, text_embedding) ||
      !add_bias(text_embedding, runtime(ctx).text.dense_bias) ||
      !l2_normalize(text_embedding)) {
    return false;
  }
  return run_projection_head(ctx, runtime(ctx).text.projection, text_embedding);
}

inline emel::error::type run_text_embedding(action::context & ctx,
                                            const int32_t token_count) noexcept {
  if (!runtime(ctx).text.ready ||
      !runtime(ctx).scratch.ready ||
      token_count <= 0 ||
      token_count > runtime(ctx).text.max_positions) {
    return to_error(error::backend);
  }
  ...
  float * sequence_in = runtime(ctx).scratch.sequence_a.get();
  float * sequence_out = runtime(ctx).scratch.sequence_b.get();
  for (int32_t layer_index = 0; layer_index < runtime(ctx).text.layer_count; ++layer_index) {
    if (!run_attention_layer(
            ctx, runtime(ctx).text.layers[static_cast<size_t>(layer_index)], token_count, sequence_in, sequence_out)) {
      return to_error(error::backend);
    }
...
inline bool publish_embedding(const action::context & ctx,
  ...
  auto source = std::span<const float>{
      runtime(ctx).scratch.full_embedding.get(),
