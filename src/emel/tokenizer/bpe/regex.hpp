...
inline void assign_bpe_regex(emel::model::data::tokenizer_pre & pre_id,
                             std::vector<std::string> & regex_exprs,
                             const emel::model::data::vocab & vocab) {
  const auto pre = vocab.tokenizer_pre_id;
  if (pre_id == pre && !regex_exprs.empty()) {
    return;
  }
  pre_id = pre;
  regex_exprs.clear();
  auto set_regex = [&](std::initializer_list<const char *> list) {
    regex_exprs.reserve(list.size());
    for (const char * expr : list) {
      regex_exprs.emplace_back(expr);
    }
  };
