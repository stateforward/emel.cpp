...
/**
 * hybrid memory orchestration model.
 *
 * state purposes:
 * - `initialized` / `ready`: idle states before and after reserve.
 * - lifecycle states coordinate reserve/allocate/branch/free across kv +
 * recurrent actors.
 * - `done` / `errored`: terminal outcomes that return to idle states.
 *
 * guard semantics:
 * - guards are pure predicates over context and request validity.
 *
 * action side effects:
 * - actions dispatch child-machine events and apply rollback on partial
 * failures.
 */
struct model {
  auto operator()() const {
...
struct sm : public emel::sm<model> {
  ...
  bool has_sequence(int32_t seq_id) const noexcept {
    return action::has_sequence(context_, seq_id);
  }
  bool reserved() const noexcept { return context_.reserved; }
  const emel::memory::kv::sm &kv_memory() const noexcept {
    return context_.kv_memory;
  }
  const emel::memory::recurrent::sm &recurrent_memory() const noexcept {
    return context_.recurrent_memory;
  }
