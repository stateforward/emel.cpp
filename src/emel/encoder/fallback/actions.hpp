...
namespace emel::encoder::fallback::action {
...
struct begin_encode {
  void operator()(const event::encode & ev, context & ctx) const noexcept {
    ...
    emel::encoder::action::detail::sync_vocab(ctx, ev.vocab);
    ...
    const auto result = emel::encoder::fallback::detail::encode_fallback(ev, ctx, *ctx.vocab);
