...
namespace emel::graph::processor::kernel_step::action {

struct run_callback {
  void operator()(const processor::event::execute_step & ev, context &) const noexcept {
    int32_t callback_err = 0;
    const bool callback_ok = ev.request.run_kernel(ev.request, &callback_err);
...
struct mark_done {
  void operator()(const processor::event::execute_step & ev, context &) const noexcept {
    ev.ctx.kernel_outcome = events::phase_outcome::done;
...
struct mark_failed_existing_error {
  void operator()(const processor::event::execute_step & ev, context &) const noexcept {
    ev.ctx.kernel_outcome = events::phase_outcome::failed;
  }
};
...
struct on_unexpected {
  ...
  void operator()(const event_type & ev, const context &) const noexcept {
    if constexpr (requires { ev.ctx.kernel_outcome; ev.ctx.err; }) {
      ev.ctx.kernel_outcome = events::phase_outcome::failed;
