...
#include "emel/graph/processor/validate_step/context.hpp"
#include "emel/graph/processor/validate_step/events.hpp"

namespace emel::graph::processor::validate_step::action {

struct run_callback {
  void operator()(const processor::event::execute_step & ev, context &) const noexcept {
    int32_t callback_err = 0;
    const bool callback_ok = ev.request.validate(ev.request, &callback_err);
...
struct on_unexpected {
  ...
  void operator()(const event_type & ev, const context &) const noexcept {
    if constexpr (requires { ev.ctx.validate_outcome; ev.ctx.err; }) {
      ev.ctx.validate_outcome = events::phase_outcome::failed;
