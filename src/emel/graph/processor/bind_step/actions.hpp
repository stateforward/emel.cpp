...
namespace emel::graph::processor::bind_step::action {

struct run_callback {
  void operator()(const processor::event::execute_step & ev, context &) const noexcept {
    int32_t callback_err = 0;
    const bool callback_ok = ev.request.bind_inputs(ev.request, &callback_err);
...
struct mark_done {
  void operator()(const processor::event::execute_step & ev, context &) const noexcept {
    ev.ctx.bind_outcome = events::phase_outcome::done;
...
struct mark_failed_existing_error {
  void operator()(const processor::event::execute_step & ev, context &) const noexcept {
    ev.ctx.bind_outcome = events::phase_outcome::failed;
  }
};
...
struct on_unexpected {
  ...
  void operator()(const event_type & ev, const context &) const noexcept {
    if constexpr (requires { ev.ctx.bind_outcome; ev.ctx.err; }) {
      ev.ctx.bind_outcome = events::phase_outcome::failed;
