...
namespace emel::graph::allocator::ordering_pass::action {

struct mark_done {
  void operator()(const allocator::event::allocate_graph_plan & ev,
                  context &) const noexcept {
    ev.ctx.ordering_outcome = events::phase_outcome::done;
    ev.ctx.sorted_tensor_count = ev.ctx.required_intervals;
    ev.ctx.required_buffer_bytes =
        static_cast<uint64_t>(ev.ctx.required_intervals) * ev.request.bytes_per_tensor;
...
struct mark_failed_prereq {
  void operator()(const allocator::event::allocate_graph_plan & ev,
                  context &) const noexcept {
    ev.ctx.ordering_outcome = events::phase_outcome::failed;
...
struct on_unexpected {
  ...
  void operator()(const event_type & ev, const context &) const noexcept {
    if constexpr (requires { ev.ctx.ordering_outcome; ev.ctx.err; }) {
      ev.ctx.ordering_outcome = events::phase_outcome::failed;
      ev.ctx.err = emel::error::cast(allocator::error::internal_error);
    }
  }
...
inline constexpr mark_failed_overflow mark_failed_overflow{};
