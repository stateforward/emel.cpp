...
namespace emel::graph::assembler::assemble_alloc_pass::action {

struct allocator_reply {
  assembler::event::assemble_ctx * assemble_ctx = nullptr;
};
...
inline bool on_allocator_done(void * owner, const allocator::events::allocation_done & ev) noexcept {
  ...
  reply->assemble_ctx->alloc_plan = ev.plan;
  reply->assemble_ctx->alloc_outcome = events::phase_outcome::done;
  reply->assemble_ctx->err = emel::error::cast(assembler::error::none);
  return true;
}

inline bool on_allocator_error(void * owner, const allocator::events::allocation_error & ev) noexcept {
  auto * reply = static_cast<allocator_reply *>(owner);
  reply->assemble_ctx->alloc_outcome = events::phase_outcome::failed;
  reply->assemble_ctx->err = static_cast<emel::error::type>(ev.err);
  return true;
}
...
struct request_allocator_plan {
  void operator()(const assembler::event::assemble_graph & ev, context & ctx) const noexcept {
    ...
    const allocator::event::allocate_graph allocate_ev{
      .graph_topology = ev.request.step_plan,
