...
namespace emel::graph::assembler::reserve_build_pass::action {
...
struct mark_done {
  void operator()(const assembler::event::reserve_graph & ev, context &) const noexcept {
    ...
    ev.ctx.assembled_node_count = ev.request.max_node_count;
    ev.ctx.assembled_tensor_count = ev.request.max_tensor_count;
