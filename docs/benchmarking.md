...
Repository benchmark scripts intentionally fail closed on macOS until a native aggregate
descendant memory controller is available; `--system` does not bypass that safety requirement.
...
The PersonaPlex reference implementation remains macOS-specific, but repository benchmark and
setup scripts intentionally fail closed there until a native aggregate descendant memory
controller is available. Existing audio can still be supplied when the workflow runs under a
supported hard envelope.
...
## generation profiling availability

`scripts/profile_generation.sh` is intentionally unavailable on macOS under the repository's
hard memory-envelope contract. macOS does not provide a supported native aggregate descendant
memory controller, and a sampled process watchdog cannot guarantee the 50% ceiling. The script
therefore fails closed before invoking `xctrace` or `bench_runner`.

Re-enabling actionable macOS profiling requires a future kernel-enforced aggregate controller
that preserves native macOS build and profiling semantics. A Linux container is not a substitute
because it produces and profiles Linux artifacts.
