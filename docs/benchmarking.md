...
Changed-file quality gates pass the deterministic 24 kHz tone generated by
`scripts/setup_moshi_cpp_reference.sh` explicitly through `--audio`, so the maintained lane is
portable to non-macOS hosts. Set `EMEL_QUALITY_GATES_PERSONAPLEX_AUDIO` to inject another gate
fixture.
