...
- [x] `graph/allocator/ordering_pass/sm.hpp` +
  ...
  `phase_unclassified_failure` fallback guard and modeled prefailed propagation
  explicitly via `phase_prefailed` / `mark_failed_prefailed`.
- [x] `graph/allocator/placement_pass/sm.hpp` +
  `graph/allocator/placement_pass/guards.hpp` +
  `graph/allocator/placement_pass/actions.hpp`
  reworked placement pass completion routing to explicit prefailed vs
...
- [x] `text/encoders/rwkv/sm.hpp` + `text/encoders/rwkv/guards.hpp`
  ...
  use the explicit guards.
- [x] `text/encoders/plamo2/sm.hpp` + `text/encoders/plamo2/guards.hpp`
  replaced `table_sync_unknown_error` / `decode_result_unknown_error` /
  `encode_result_unknown_error` fallback composition (`!guard_a && !...`) with
  explicit direct unclassified-error-code guards and updated `sm.hpp`
  phase-result routing to use the explicit guards.
- [x] `text/encoders/ugm/sm.hpp` + `text/encoders/ugm/guards.hpp`
  replaced `table_sync_unknown_error` / `normalize_result_unknown_error` /
  `input_prepare_result_unknown_error` / `dp_forward_result_unknown_error`
  fallback composition (`!guard_a && !...`) with explicit direct
  unclassified-error-code guards and updated `sm.hpp` phase-result routing to
  use the explicit guards.
- [x] `text/encoders/spm/sm.hpp` + `text/encoders/spm/guards.hpp`
  replaced `table_sync_unknown_error` / `prepare_result_unknown_error` /
  `merge_result_unknown_error` / `encode_result_unknown_error` fallback
