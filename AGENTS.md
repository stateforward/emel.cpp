...
`co_sm` coroutine actors are opt-in only when a milestone explicitly approves
them. ALWAYS treat coroutine continuations as internal actor/scheduler progress,
not mailbox messages or deferred events. ALWAYS require `co_sm` schedulers to be
bounded, FIFO, single-consumer per actor, and run-to-completion for the selected
cooperative tick. ALWAYS model suspend/resume points as explicit SML states and
transitions. NEVER retain stack-backed event payload, source spans, target
windows, mutable references, callbacks, or other dispatch-local data across
suspension. NEVER store same-RTC callbacks for later coroutine resumption.
NEVER allow coroutine bodies, awaitables, actions, detail helpers, or scheduler
callbacks to choose runtime behavior; route choices remain in guards and
transition tables. NEVER allocate coroutine frames, scheduler queues, or
continuation storage during dispatch/progress. NEVER expose coroutine task,
scheduler, awaitable, or frame types through public C ABI or generic public
...
For explicitly approved coroutine actors, expose the canonical component type as
`emel::co_sm<model, ...>` (or the project-owned equivalent) instead of replacing
ordinary synchronous `emel::sm` aliases.
...
ALWAYS use the default changed-file scoped quality gate for normal phase
iteration so unrelated dirty-tree benchmarks, docs, parity, or fuzz lanes do
not make the phase gate stale or all-day by default.
ALWAYS pass `EMEL_QUALITY_GATES_CHANGED_FILES` when the worktree contains
unrelated user or agent changes, and scope it to the files actually changed by
the current implementation.
ALWAYS use `EMEL_QUALITY_GATES_BENCH_SUITE` when a phase has a known benchmark
domain, for example `diarization_sortformer`, instead of running unrelated
benchmark suites during local iteration.
ALWAYS use `EMEL_QUALITY_GATES_SCOPE=full scripts/quality_gates.sh` before
milestone closeout, release readiness, or cross-cutting changes that can affect
multiple runtime domains.
NEVER weaken coverage, parity, benchmark, fuzz, or docs checks to save time.
...
claims, not artifact-backed claims.
ALWAYS verify those claims by tracing the actual repo codepath from the pinned
fixture/model contract through the maintained loader/builder into the runtime,
parity, benchmark, and publication entrypoints that make the claim.
NEVER mark a requirement, audit, milestone, benchmark claim, parity proof, or
closeout ledger as satisfied from ROADMAP.md, STATE.md, REQUIREMENTS.md,
SUMMARY.md, VERIFICATION.md, or VALIDATION.md alone when the claim depends on
which runtime path, model contract, fixture, or tool entrypoint actually runs.
If code inspection shows a synthetic, fabricated, tool-only, or test-only
contract/fixture/path is still feeding the claimed maintained runtime,
