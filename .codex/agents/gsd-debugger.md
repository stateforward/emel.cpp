...
- For `emel.cpp` fixes, reproduce the bug with a failing `doctest` before patching and use SML introspection/testing helpers where relevant
- If the needed fix would change state-machine structure, apply that change directly when it is
  required for correctness, performance, or to make hidden runtime control flow explicit. Stop only
  if the change would intentionally broaden scope or externally visible behavior beyond the declared
  contract.
...
<required_reading>
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/common-bug-patterns.md
</required_reading>

**Project instructions:** Read `./AGENTS.md`, `./docs/rules/sml.rules.md`, and `./docs/rules/cpp.rules.md` if they exist in the working directory before trusting the current code as a pattern source. If you make an `emel.cpp` code change, run `scripts/quality_gates.sh` before declaring the debug session fixed unless a concrete blocker prevents it. In dirty worktrees, pass `EMEL_QUALITY_GATES_CHANGED_FILES` for the files changed by the debug fix and a relevant `EMEL_QUALITY_GATES_BENCH_SUITE` when applicable; do not weaken relevant lanes. Record blockers explicitly.

**Hidden-control-flow rule for emel.cpp:** helper-local selection on dtype, backend support, variant, block flags, activation mode, fallback, or authoritative output/buffer lane is a real control-flow decision. Treat it as a root-cause candidate and rule violation even if `actions.hpp` contains no runtime `if` statements.
...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = /Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex
                  → checks /Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = /Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done
                  → writes to /Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/hooks/
...
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/thinking-models-debug.md
...
- Read @/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/common-bug-patterns.md
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
