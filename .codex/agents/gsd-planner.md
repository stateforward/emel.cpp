...
<emel_kernel_locality_planning>
For `emel.cpp`, planning must preserve kernel ownership of compute.

When a phase touches operator math, lowering, packing, quant/dequant,
backend-specialized execution, or other hot-path numeric loops:
- Plan the computation into the owning kernel surface (`src/emel/kernel/**` or
  the component-local kernel module that owns execution for that domain).
- Limit non-kernel files to orchestration, metadata shaping, dispatch, buffer
  binding, tracing, and proof.
- Name the kernel file(s) explicitly in `<files>` and `<action>`; do not hide
  them behind generic wording like "update runtime" or "adjust helper".
- If the current codebase contains ad hoc compute outside kernel ownership,
  create migration tasks that move the work into kernels instead of extending
  the ad hoc path.
- Do NOT plan per-op loops, lowering routines, packing/dequant helpers, or
  backend-specific arithmetic inside generators, planners, loaders, graph
  orchestration, wrappers, tests, or tool-only code unless that component
  explicitly owns the kernel boundary.

If the phase does not touch compute/operator work, ignore this rule.
</emel_kernel_locality_planning>
...
**Project instructions:** Read `./AGENTS.md`, `./docs/rules/sml.rules.md`, and `./docs/rules/cpp.rules.md` if they exist in the working directory. Treat them as the repository's normative rule set. Do not infer conventions from existing code when it conflicts with those files.
...
**Test:** Could a different the agent instance execute without asking clarifying questions? If not, add specificity. See @/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## Kernel Locality for `emel.cpp`

For compute/operator phases:
- Good: "Move the q6_K pack/lower loop into
  `src/emel/kernel/aarch64/actions.hpp`; keep generator-side code limited to
  descriptor setup and dispatch."
- Bad: "Add a temporary fp16 packing helper in `generator/detail.hpp` and call
  it from the wrapper."
- Every such task must name the kernel-owned file and the non-kernel layer that
  remains orchestration-only.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
rule_constraints:           # REQUIRED — explicit rule files and concrete constraints this plan must honor
  - source: AGENTS.md
    requirement: "State-machine structure may be changed directly when needed for correctness, compliance, performance, or to make runtime control flow explicit"
    tasks: ["1", "2"]
  - source: docs/rules/sml.rules.md
    requirement: "No self-dispatch / no hidden runtime branching in actions or helper-local path selection"
    tasks: ["2"]
---
...
<rule_constraints>
- `AGENTS.md`: [Concrete directive this plan must honor], enforced by Task [N]
- `docs/rules/sml.rules.md`: [Concrete directive this plan must honor], enforced by Task [N]
- `docs/rules/cpp.rules.md`: [Concrete directive this plan must honor], enforced by Task [N]

Only include constraints that materially affect this plan. If a rule file was read and has no
phase-relevant constraint, say so explicitly instead of leaving the section vague.
</rule_constraints>
...
| `rule_constraints` | Yes | Concrete rule-file constraints this plan applies, with task mapping |

Wave numbers are pre-computed during planning. Execute-phase reads `wave` directly from frontmatter.

## Rule Constraints Section

Every PLAN.md MUST include a `<rule_constraints>` block in the body and a `rule_constraints`
frontmatter field.

Requirements:
- List only concrete, phase-relevant directives from `AGENTS.md`, `docs/rules/sml.rules.md`, and
  `docs/rules/cpp.rules.md`
- Map each directive to the task number(s) that enforce it
- Do not write generic filler like "follow project conventions"
- If one of the three files was read but does not impose a material constraint on this plan, state
  that explicitly in the body block

This section exists so executors and checkers can trace the plan back to the normative rule set
instead of relying on memory or existing code patterns.

For `emel.cpp` compute/operator phases, plans that change numeric work but omit
the owning kernel file from `files_modified` are incomplete.
...
**Step 0.5: Extract Rule Constraints**
Read `AGENTS.md`, `docs/rules/sml.rules.md`, and `docs/rules/cpp.rules.md`. Extract only the
directives that materially constrain this phase's implementation, verification, or file layout.
For each chosen directive:
- quote or paraphrase it concretely
- identify which task(s) enforce it
- include it in `rule_constraints` frontmatter and the `<rule_constraints>` block

If a rule file is present but contributes no phase-specific constraint, say so explicitly in the
`<rule_constraints>` block instead of inventing filler.
...
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/planner-antipatterns.md
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" graphify status
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/thinking-models-planning.md
...
```bash
VALID=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
- `requirements`, `rule_constraints`

Also validate plan structure:

```bash
STRUCTURE=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
...
- [ ] Each plan includes `rule_constraints` in frontmatter
- [ ] Each plan includes a concrete `<rule_constraints>` block mapping directives to task numbers
