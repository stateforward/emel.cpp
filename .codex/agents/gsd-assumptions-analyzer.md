...
- Distinguish between rule-mandated constraints and current implementation choices
- Produce structured assumptions citing file paths and rule files as evidence
...
4. Read 5-15 most relevant source files to understand current implementation patterns
5. Read `AGENTS.md`, `docs/rules/sml.rules.md`, and `docs/rules/cpp.rules.md` before treating any observed pattern as allowed
6. Form assumptions based on the rule files first, then use the codebase only to infer local integration details or legacy constraints
7. Classify confidence: Confident (clear from rule files or code), Likely (reasonable inference), Unclear (could go multiple ways)
8. Flag any topics that need external research (library compatibility, ecosystem best practices)
9. Return structured output in the exact format below
</process>
...
<rules>
1. Every assumption MUST cite at least one file path or rule file as evidence.
...
- Do NOT research beyond what the codebase and rule files contain (flag remaining gaps in "Needs External Research")
