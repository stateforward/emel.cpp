...
**4. emel.cpp Rule Violations** — For this repository, treat violations of `AGENTS.md`, `docs/rules/sml.rules.md`, and `docs/rules/cpp.rules.md` as review findings. This includes RTC/no-queue violations, self-dispatch, hidden runtime branching in actions or helper-local path selection in `detail.hpp`/helper layers, dynamic allocation during dispatch, forbidden context usage, source-first SML rows in modified code, missing unexpected-event handling, missing failing tests before bug fixes, and other rule-file conflicts.

**Out of Scope:** Do NOT normalize a pattern just because it already exists in the codebase. Existing violations remain findings, not conventions.
...
- **C/C++**: Buffer overflow patterns, use-after-free indicators, null pointer dereferences, missing bounds checks, memory leaks, UB risks, forbidden casts, strict-aliasing violations, nondeterministic/RT-unsafe behavior
- **Boost.SML / actor model**: Queue usage, self-dispatch, runtime branching in actions, helper-local route selection disguised as `detail.hpp`/action-callee logic, dispatch-time allocation, unbounded anonymous/completion chains, forbidden context mirroring, missing explicit unexpected-event handling, and transition tables that contradict the repo rule files

**Hidden-control-flow litmus test for emel.cpp:** treat a helper as violating the rules when it branches on runtime dtype, backend support, modality/variant/family, tensor layout, block flags, activation mode, fallback availability, or authoritative buffer/output lane in a way that changes which algorithm/path/behavior runs. Do not accept syntactic cleanup that merely removes `if` from `actions.hpp` while preserving the same route selection in helpers.
...
**Critical** — Security vulnerabilities, data loss risks, crashes, authentication bypasses, RTC/determinism violations in dispatch-critical paths:
