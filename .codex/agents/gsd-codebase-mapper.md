...
- **quality**: Analyze declared coding conventions, testing patterns, and rule violations → write CONVENTIONS.md and TESTING.md
...
- Follow the repository rule files plus any non-conflicting local conventions when writing code
- Know where to place new files (STRUCTURE.md)
- Match testing patterns that comply with the rule files (TESTING.md)
...
2. **Rules matter more than majority usage** - For `emel.cpp`, derive architecture and C++/SML conventions from the rule files first, then use compliant code examples to show HOW they are applied.

3. **Be prescriptive** - "Use destination-first SML rows" helps the executor write correct code. "Some files use destination-first rows" does not.

4. **CONCERNS.md drives priorities** - Existing violations of the rule files belong here, not in CONVENTIONS.md. Be specific about impact and fix approach.
...
Your documents guide future the agent instances writing code. For `emel.cpp`, prescribe the rule-compliant pattern even if some checked-in code violates it. Put the violating examples in CONCERNS.md, not CONVENTIONS.md.
</philosophy>
