...
<emel_kernel_locality_research>
For `emel.cpp`, if the phase touches operator math, lowering, packing,
quant/dequant, backend-specialized numeric work, or hot-path compute:
- Include a `## Kernel Ownership` section in RESEARCH.md.
- Name the owning kernel surface/file(s) where the computation belongs (prefer
  `src/emel/kernel/**` or the component-local kernel module that already owns
  execution for that domain).
- Name the non-kernel layers that must remain orchestration-only (generators,
  planners, loaders, graph/tensor orchestration, wrappers, tools, tests, and
  helper-only detail files).
- Treat existing ad hoc compute outside kernel ownership as migration targets,
  not patterns to extend.
- Do NOT recommend helper-local or wrapper-local compute as an expedient
  implementation path.
</emel_kernel_locality_research>
...
**Rule enforcement:** If `./AGENTS.md`, `./docs/rules/sml.rules.md`, or `./docs/rules/cpp.rules.md` exist, extract all actionable directives (required tools, forbidden patterns, coding conventions, testing rules, RTC/SML constraints, performance rules). Include a `## Project Constraints (from rule files)` section in RESEARCH.md listing these directives so the planner can verify compliance. Treat these rule files with the same authority as locked decisions from CONTEXT.md — research should not recommend approaches that contradict them, and existing code is not a counterexample.
</project_context>
...
| `## Kernel Ownership` | For compute/operator phases, planner uses this to keep ops in kernels and non-kernel layers orchestration-only |
...
If the phase touches compute/operator work in `emel.cpp`, `## Kernel Ownership`
is mandatory and must explicitly identify which layers stay orchestration-only.
</downstream_consumer>
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/thinking-models-research.md
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
