...
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/few-shot-examples/plan-checker.md
...
**Process:**
1. Read `./AGENTS.md`, `./docs/rules/sml.rules.md`, and `./docs/rules/cpp.rules.md` in the working directory (already loaded in `<project_context>`)
...
6. For `emel.cpp` compute/operator phases, verify the plan places numeric work in
   owning kernel files and keeps non-kernel layers orchestration-only
...
- Plan treats hidden control flow as a syntactic cleanup problem ("remove `if` from actions") instead
  of a semantic orchestration problem (helper-local route selection still present in helpers/detail)
- Plan forbids necessary state-machine restructuring even though the repo rules allow structural
  changes to make runtime control flow explicit
- Plan puts operator math, lowering, packing, quant/dequant, or backend-specific
  loops in generators, planners, loaders, graph orchestration, wrappers, tests,
  or tools instead of the owning kernel file
- Plan changes compute/operator behavior but no kernel-owned file appears in
  `files_modified` or task `<files>`
- Plan describes a "temporary", "ad hoc", or helper-local compute path outside
  kernel ownership to bridge a missing kernel implementation
...
**Example — kernel locality violation:**
```yaml
issue:
  dimension: claude_md_compliance
  severity: blocker
  description: "Plan places image-tower lowering loops in generator detail helpers instead of kernel-owned files"
  plan: "59.1.1.1-01"
  task: 1
  claude_md_rule: "Keep operator arithmetic, lowering, packing, quant/dequant, and backend-specific numeric work in the owning kernel layer"
  plan_action: "Add temporary lowering helper in src/emel/generator/detail.hpp"
  fix_hint: "Move the lowering implementation into the owning src/emel/kernel/** file and keep generator work dispatch-only"
```

**Rule-traceability requirement:**
- Every PLAN.md must include a `rule_constraints` frontmatter field
- Every PLAN.md must include a `<rule_constraints>` body block
- Both must name concrete directives from `AGENTS.md`, `docs/rules/sml.rules.md`, and
  `docs/rules/cpp.rules.md` as applicable to the phase
- The body block must map each directive to the task number(s) enforcing it

If a plan says only "follow project conventions" or similar generic wording, treat that as missing
...
```bash
INIT=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
- Missing `rule_constraints` frontmatter or body block → `claude_md_compliance`
...
```bash
MUST_HAVES=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
## Step 3.5: Parse rule_constraints

For each plan:

```bash
RULE_CONSTRAINTS=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field rule_constraints 2>/dev/null)
rg -n "^<rule_constraints>|^</rule_constraints>|AGENTS\\.md|docs/rules/sml\\.rules\\.md|docs/rules/cpp\\.rules\\.md|Task [0-9]" "$PLAN_PATH"
```

Check:
- frontmatter field exists and is non-empty
- body block exists
- entries are concrete, not generic filler
- task mappings point to real tasks in the plan
- the constraints materially match the phase scope
...
```bash
PLAN_STRUCTURE=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
