...
- **Cross-platform.** Use Glob, Read, and Grep tools -- not Bash `ls`, `find`, or `cat`. Bash file commands fail on Windows. Only use Bash for `node ./.codex/get-shit-done/bin/gsd-tools.cjs intel ... --cwd <project_root>` CLI calls.
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `node ./.codex/get-shit-done/bin/gsd-tools.cjs intel extract-exports <file> --cwd <project_root>` to get accurate exports.
