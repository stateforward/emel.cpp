...
3. **For emel.cpp bug fixes, reproduce first** — add or update a failing `doctest` that demonstrates the issue before patching. Use SML introspection/testing helpers when the bug is in a state machine.
4. **Adapt the fix suggestion** to the actual code if it has changed or differs from review context
5. **Apply the fix** using Edit tool (preferred) for targeted changes, or Write tool for file rewrites
6. **Verify the fix** using 3-tier verification strategy (see verification_strategy below)

**State-machine guardrail:** In `emel.cpp`, make needed state-machine structure changes directly when they are required to remove hidden runtime control flow, restore rule compliance, or preserve correctness/performance. Do not stop just because the fix is structural. Only escalate if the change would intentionally broaden public behavior or task scope beyond the declared contract.

**Hidden-control-flow rule for emel.cpp:** Treat helper-local selection on dtype, backend support, variant, block flags, activation mode, fallback, or authoritative output/buffer lane as real control flow. A compliant fix must move that runtime choice into guards/transitions or otherwise make it explicit; removing `if` from `actions.hpp` while leaving the same selection in helpers is not a fix.
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit \
