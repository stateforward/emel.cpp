...
**Rule enforcement:** If `./AGENTS.md`, `./docs/rules/sml.rules.md`, or `./docs/rules/cpp.rules.md` exist, treat them as hard constraints during execution. Before committing each task, verify that code changes do not violate those rules. If a task action or existing code conflicts with them, follow the rule files instead of the codebase pattern and document the adjustment as a deviation. For `emel.cpp`, hidden runtime control flow is semantic: helper-local selection on dtype, backend support, variant, block flags, activation mode, fallback, or authoritative output lane is a rule violation even if `actions.hpp` is branch-free. Fix that by making the structure explicit in guards/transitions instead of doing superficial syntax cleanup. For `emel.cpp` bug-fix tasks, add or update a failing `doctest` before patching. After each implementation change, run `scripts/quality_gates.sh`; if the worktree contains unrelated changes, pass `EMEL_QUALITY_GATES_CHANGED_FILES` scoped to the files you changed, and pass `EMEL_QUALITY_GATES_BENCH_SUITE` for the relevant benchmark domain. Do not weaken relevant coverage/parity/benchmark/docs checks. Use `EMEL_QUALITY_GATES_SCOPE=full` for milestone closeout, release readiness, or cross-domain changes. If the gate cannot run, record the blocker and do not report full verification.
</project_context>
...
```bash
INIT=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/ios-scaffold.md
...
**Trigger:** Fix requires significant structural modification that broadens scope beyond the declared task or contract

**Examples:** New DB table (not column), major schema changes, new service layer, switching libraries/frameworks, changing auth approach, new infrastructure, breaking API changes

**Not Rule 4 for emel.cpp:** Restructuring a state machine to remove hidden runtime control flow, move runtime choice from helpers into guards/transitions, or make behavior explicit is normal compliance work and should be done directly rather than escalated.
...
@/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/executor-examples.md
</deviation_rules>
...
**See @/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/references/checkpoints.md**
...
```bash
node /Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/templates/summary.md
...
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" state record-metric \
  --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
  --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"

# Add decisions (extract from SUMMARY.md key-decisions)
for decision in "${DECISIONS[@]}"; do
  node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" state add-decision \
    --phase "${PHASE}" --summary "${decision}"
done

# Update session info
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" state record-session \
  --stopped-at "Completed ${PHASE}-${PLAN}-PLAN.md"
```

```bash
# Update ROADMAP.md progress for this phase (plan counts, status)
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"

# Mark completed requirements from PLAN.md frontmatter
# Extract the `requirements` array from the plan's frontmatter, then mark each complete
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
