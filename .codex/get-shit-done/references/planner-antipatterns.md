...
## Kernel Locality Anti-Patterns (`emel.cpp`)

### Bad — ad hoc compute in orchestration layers

```xml
<task type="auto">
  <files>src/emel/generator/detail.hpp</files>
  <action>Add a temporary image-lowering loop here and call it from the generator wrapper until the kernel path is ready.</action>
</task>
```

**Why bad:** This extends the ad hoc path. Numeric operator work belongs in the
owning kernel surface, not helper or wrapper layers.

### Good — kernel-owned compute with orchestration-only wiring

```xml
<task type="auto">
  <files>src/emel/kernel/aarch64/actions.hpp, src/emel/generator/actions.hpp</files>
  <action>Move the image-lowering loop into the owning AArch64 kernel action. Keep the generator action limited to preparing descriptors and dispatching the kernel event.</action>
</task>
```

**Why good:** The plan makes the kernel own compute and keeps higher layers on
dispatch/wiring duties only.
