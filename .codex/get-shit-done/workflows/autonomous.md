...
```bash
ROADMAP=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap analyze)
```
...
```bash
DETAIL=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
**3b.5. Refresh Stale Plans**

Autonomous mode should repair stale plans automatically instead of stopping on them.

Check whether any plan in the phase still predates the current planning contract:

```bash
STALE_PLAN_COUNT=0
for plan in "${PHASE_DIR}"/*-PLAN.md; do
  [ -f "$plan" ] || continue
  if ! grep -q "^rule_constraints:" "$plan" || ! grep -q "^<rule_constraints>" "$plan"; then
    STALE_PLAN_COUNT=$((STALE_PLAN_COUNT + 1))
  fi
done
echo "$STALE_PLAN_COUNT"
```

If `STALE_PLAN_COUNT` is greater than `0`, display:

```
Phase ${PHASE_NUM}: Refreshing ${STALE_PLAN_COUNT} stale plan(s) to add current rule traceability...
```
...
Re-check the plans after refresh. If any plan still lacks `rule_constraints` frontmatter or a
`<rule_constraints>` block, go to handle_blocker:
"Phase ${PHASE_NUM} plans remain stale after automatic refresh."
...
Refresh any stale gap plans before re-executing:

```bash
STALE_GAP_PLAN_COUNT=0
for plan in "${PHASE_DIR}"/*-PLAN.md; do
  [ -f "$plan" ] || continue
  if ! grep -q "^rule_constraints:" "$plan" || ! grep -q "^<rule_constraints>" "$plan"; then
    STALE_GAP_PLAN_COUNT=$((STALE_GAP_PLAN_COUNT + 1))
  fi
done
echo "$STALE_GAP_PLAN_COUNT"
```

If `STALE_GAP_PLAN_COUNT` is greater than `0`:

```
Skill(skill="gsd-plan-phase", args="${PHASE_NUM} --gaps")
```

If plans are still stale after the automatic refresh, go to handle_blocker:
"Gap closure plans for phase ${PHASE_NUM} remain stale after automatic refresh."
...
```bash
UI_REVIEW_CFG=$(node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```bash
node "/Users/gabrielwillen/VSCode/stateforward/emel/emel.cpp/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
- [ ] Stale plans are refreshed through `gsd-plan-phase` instead of blocking autonomous execution
