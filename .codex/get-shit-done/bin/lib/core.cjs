...
function loadConfig(cwd) {
  ...
  try {
    ...
    const KNOWN_TOP_LEVEL = new Set([
      ...
      'model_overrides', 'agent_skills', 'context_window', 'resolve_model_ids', 'claude_md_path', 'instructions_path',
    ...
    return {
      ...
      instructions_path: get('instructions_path') || get('claude_md_path') || null,
      claude_md_path: get('claude_md_path') || get('instructions_path') || null,
    };
  } catch {
...
function extractCurrentMilestone(content, cwd) {
  ...
  const nextMilestonePattern = new RegExp(
    `^#{1,${headingLevel}}\\s+(?!Phase\\s+\\d)(?:.*v\\d+\\.\\d+|✅|📋|🚧)`,
    'mi'
  );
