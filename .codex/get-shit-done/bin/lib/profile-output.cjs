 ...
 * Deprecated aliases remain supported:
...
const CLAUDE_MD_FALLBACKS = {
  ...
  conventions: 'Conventions not yet established. Follow explicit rule files and documented project constraints rather than inferring conventions from checked-in code.',
  architecture: 'Architecture not yet mapped. Follow explicit architectural rules and documented constraints rather than copying unchecked codebase patterns.',
...
function resolveInstructionsPathFromConfig(cwd, defaultPath = './AGENTS.md') {
  try {
    const config = loadConfig(cwd);
    return config.instructions_path || config.claude_md_path || defaultPath;
  } catch {
    return defaultPath;
  }
}
...
function cmdGenerateAgentsProfile(cwd, options, raw) {
  ...
  if (options.global) {
    ...
    const configInstructionsPath = resolveInstructionsPathFromConfig(cwd, './AGENTS.md');
    targetPath = path.isAbsolute(configInstructionsPath) ? configInstructionsPath : path.join(cwd, configInstructionsPath);
  }
...
function cmdGenerateAgentsMd(cwd, options, raw) {
  ...
  if (!outputPath) {
    const configInstructionsPath = resolveInstructionsPathFromConfig(cwd, './AGENTS.md');
    outputPath = path.isAbsolute(configInstructionsPath) ? configInstructionsPath : path.join(cwd, configInstructionsPath);
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  return cmdGenerateAgentsProfile(cwd, options, raw);
}

function cmdGenerateClaudeMd(cwd, options, raw) {
  return cmdGenerateAgentsMd(cwd, options, raw);
}
