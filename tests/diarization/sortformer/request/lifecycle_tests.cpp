...
namespace sortformer_request = emel::diarization::sortformer::request;
...
struct callback_probe {
  ...
  const sortformer_request::event::prepare * request = nullptr;
  int32_t frame_count = 0;
  int32_t feature_bin_count = 0;
  emel::error::type err = emel::error::cast(sortformer_request::error::none);

  void on_done(const sortformer_request::events::prepare_done & ev) noexcept {
    done_called = true;
    request = ev.request;
  ...
  void on_error(const sortformer_request::events::prepare_error & ev) noexcept {
    error_called = true;
    request = ev.request;
    err = ev.err;
  }
...
emel::model::sortformer::detail::execution_contract make_contract(
  ...
  contract.sample_rate = request_detail::k_sample_rate;
  contract.speaker_count = request_detail::k_speaker_count;
  contract.frame_shift_ms = request_detail::k_frame_shift_ms;
  contract.chunk_len = request_detail::k_chunk_len;
...
std::vector<float> make_pcm() {
  std::vector<float> pcm(static_cast<size_t>(request_detail::k_required_sample_count));
...
sortformer_request::event::prepare make_request(
  ...
  sortformer_request::event::prepare request{
    contract,
    pcm,
    sample_rate,
    channel_count,
...
TEST_CASE("sortformer request extracts deterministic Sortformer features") {
  auto model = std::make_unique<request_model_fixture>();
  const auto contract = make_contract(model->model);
  auto pcm = make_pcm();
  std::vector<float> features(
      static_cast<size_t>(request_detail::k_required_feature_count));
  int32_t frame_count = -1;
  int32_t feature_bin_count = -1;
  emel::error::type err = emel::error::cast(sortformer_request::error::feature_extractor);
  ...
  auto request = make_request(contract,
                              ...
                              err);
  request.on_done =
      emel::callback<void(const sortformer_request::events::prepare_done &)>::from<
          callback_probe,
          &callback_probe::on_done>(&probe);

  sortformer_request::sm machine{};
  REQUIRE(machine.process_event(request));
  CHECK(machine.is(stateforward::sml::state<sortformer_request::state_ready>));
  CHECK(err == emel::error::cast(sortformer_request::error::none));
  CHECK(frame_count == feature_extractor_detail::k_feature_frame_count);
  CHECK(feature_bin_count == request_detail::k_feature_bin_count);
  ...
  CHECK(probe.feature_bin_count == request_detail::k_feature_bin_count);
  ...
  emel::error::type second_err =
      emel::error::cast(sortformer_request::error::feature_extractor);
  ...
  CHECK(second_err == emel::error::cast(sortformer_request::error::none));
  CHECK(second_features == features);
}

TEST_CASE("sortformer request rejects invalid sample rate") {
  auto model = std::make_unique<request_model_fixture>();
  const auto contract = make_contract(model->model);
  auto pcm = make_pcm();
  std::vector<float> features(
  ...
  CHECK(err == emel::error::cast(sortformer_request::error::sample_rate));
  CHECK(frame_count == 0);
  CHECK(feature_bin_count == 0);
}

TEST_CASE("sortformer request rejects invalid channel count") {
  auto model = std::make_unique<request_model_fixture>();
  const auto contract = make_contract(model->model);
  auto pcm = make_pcm();
  std::vector<float> features(
  ...
  sortformer_request::sm machine{};
  CHECK_FALSE(machine.process_event(request));
  CHECK(err == emel::error::cast(sortformer_request::error::channel_count));
  CHECK(frame_count == 0);
  CHECK(feature_bin_count == 0);
}

TEST_CASE("sortformer request rejects wrong pcm shape") {
  auto model = std::make_unique<request_model_fixture>();
  const auto contract = make_contract(model->model);
  ...
  sortformer_request::sm machine{};
  CHECK_FALSE(machine.process_event(request));
  CHECK(err == emel::error::cast(sortformer_request::error::pcm_shape));
  CHECK(frame_count == 0);
  CHECK(feature_bin_count == 0);
}

TEST_CASE("sortformer request rejects insufficient feature output capacity") {
  auto model = std::make_unique<request_model_fixture>();
  const auto contract = make_contract(model->model);
  auto pcm = make_pcm();
  std::vector<float> features(
      static_cast<size_t>(request_detail::k_required_feature_count - 1));
  int32_t frame_count = -1;
  int32_t feature_bin_count = -1;
  emel::error::type err = emel::error::cast(sortformer_request::error::none);
  ...
  sortformer_request::sm machine{};
  CHECK_FALSE(machine.process_event(request));
  CHECK(err == emel::error::cast(sortformer_request::error::capacity));
  CHECK(frame_count == 0);
  CHECK(feature_bin_count == 0);
}

TEST_CASE("sortformer request rejects invalid Sortformer profile and emits error callback") {
  auto model = std::make_unique<request_model_fixture>();
  auto contract = make_contract(model->model);
  contract.chunk_len = request_detail::k_chunk_len - 1;
  auto pcm = make_pcm();
  std::vector<float> features(
      static_cast<size_t>(request_detail::k_required_feature_count));
  int32_t frame_count = -1;
  ...
  auto request = make_request(contract, pcm, 16000, 1, features, frame_count, feature_bin_count, err);
  request.on_error =
      emel::callback<void(const sortformer_request::events::prepare_error &)>::from<
          callback_probe,
          &callback_probe::on_error>(&probe);

  sortformer_request::sm machine{};
  CHECK_FALSE(machine.process_event(request));
  CHECK(err == emel::error::cast(sortformer_request::error::model_invalid));
  CHECK_FALSE(probe.done_called);
  CHECK(probe.error_called);
  CHECK(probe.request == &request);
  CHECK(probe.err == emel::error::cast(sortformer_request::error::model_invalid));
  CHECK(frame_count == 0);
  CHECK(feature_bin_count == 0);
}
