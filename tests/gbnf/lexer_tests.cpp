...
struct probe {
  ...
  emel::gbnf::rule_parser::lexer::cursor next_cursor = {};
  int32_t err = 0;
};
...
int32_t error_code(const emel::gbnf::rule_parser::lexer::error err) {
...
TEST_CASE("gbnf_lexer_next_emits_token_stream_then_eof") {
  ...
  probe out{};
  emel::callback<bool(const emel::gbnf::rule_parser::lexer::events::next_done &)> done_cb{&out, on_done};
  emel::callback<bool(const emel::gbnf::rule_parser::lexer::events::next_error &)> error_cb{&out, on_error};
  ...
  CHECK(out.has_token);
  CHECK(out.token.kind == emel::gbnf::rule_parser::lexer::event::token_kind::identifier);
  ...
  emel::gbnf::rule_parser::lexer::event::next next_def{cursor, done_cb, error_cb};
  ...
  CHECK_FALSE(out.error_called);
  CHECK(out.has_token);
  CHECK(out.token.kind == emel::gbnf::rule_parser::lexer::event::token_kind::definition_operator);
  CHECK(out.token.text == "::=");
  CHECK(out.next_cursor.offset == 8);
  CHECK(out.next_cursor.token_count == 2);

  cursor = out.next_cursor;
  emel::gbnf::rule_parser::lexer::event::next next_lit{cursor, done_cb, error_cb};
  out = {};
  CHECK(machine.process_event(next_lit));
  CHECK(out.done_called);
  CHECK_FALSE(out.error_called);
  CHECK(out.has_token);
  CHECK(out.token.kind == emel::gbnf::rule_parser::lexer::event::token_kind::string_literal);
  CHECK(out.token.text == "\"a\"");
  CHECK(out.next_cursor.offset == cursor.input.size());
  CHECK(out.next_cursor.token_count == 3);

  cursor = out.next_cursor;
  emel::gbnf::rule_parser::lexer::event::next next_eof{cursor, done_cb, error_cb};
  out = {};
...
TEST_CASE("gbnf_lexer_requires_callbacks") {
  ...
  probe out{};
  emel::callback<bool(const emel::gbnf::rule_parser::lexer::events::next_done &)> missing_done{};
  emel::callback<bool(const emel::gbnf::rule_parser::lexer::events::next_error &)> error_cb{&out, on_error};
