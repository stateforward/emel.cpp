...
TEST_CASE("CQ4 realistic batch4 matches independent scalar and prepared calls") {
  ...
  for (uint32_t target = 0u; target < pair_outputs.size(); ++target)
    pair_outputs[target].resize(outs[target]);
  std::array<float, in> pair_workspace{};
  const emel::kernel::cq::event::prepared_gemv_batch4_pair_request pair_request{
      .q = {&targets[0].prepared, pair_outputs[0]},
      .gate = {&targets[3].prepared, pair_outputs[3]},
      .k = {&targets[1].prepared, pair_outputs[1]},
      .v = {&targets[2].prepared, pair_outputs[2]},
      .codebook = prepared_codebook,
      .activation = activation,
      .workspace = pair_workspace,
      .output_scale = 0.625f};
  emel::kernel::cq::event::dispatch_result pair_result{};
  REQUIRE(sm.process_event(
      emel::kernel::cq::event::execute_prepared_avx2_batch4_pair_q4{
          pair_request, pair_result}));
  for (uint32_t target = 0u; target < pair_outputs.size(); ++target)
    for (uint32_t row = 0u; row < pair_outputs[target].size(); ++row)
      CHECK(pair_outputs[target][row] == targets[target].separate[row] * 0.625f);

  const emel::kernel::cq::event::prepared_gemv_batch4_pair_request mismatch{
      .q = {&targets[0].prepared, pair_outputs[0]},
      .gate = {&targets[1].prepared, pair_outputs[1]},
      .k = {&targets[1].prepared, pair_outputs[1]},
      .v = {&targets[2].prepared, pair_outputs[2]},
      .codebook = prepared_codebook,
      .activation = activation,
      .workspace = pair_workspace};
  emel::kernel::cq::event::dispatch_result mismatch_result{};
  CHECK_FALSE(sm.process_event(
      emel::kernel::cq::event::execute_prepared_avx2_batch4_pair_q4{
          mismatch, mismatch_result}));
  CHECK_FALSE(mismatch_result.accepted);

  const emel::kernel::cq::event::prepared_gemv_batch4_pair_request alias{
      .q = {&targets[0].prepared, pair_outputs[0]},
      .gate = {&targets[3].prepared, pair_outputs[0]},
      .k = {&targets[1].prepared, pair_outputs[1]},
      .v = {&targets[2].prepared, pair_outputs[2]},
      .codebook = prepared_codebook,
      .activation = activation,
      .workspace = pair_workspace};
  emel::kernel::cq::event::dispatch_result alias_result{};
  CHECK_FALSE(sm.process_event(
      emel::kernel::cq::event::execute_prepared_avx2_batch4_pair_q4{
          alias, alias_result}));
  CHECK_FALSE(alias_result.accepted);

  const emel::kernel::cq::event::prepared_gemv_batch4_pair_request
      workspace_alias{
          .q = {&targets[0].prepared, pair_workspace},
          .gate = {&targets[3].prepared, pair_outputs[3]},
          .k = {&targets[1].prepared, pair_outputs[1]},
          .v = {&targets[2].prepared, pair_outputs[2]},
          .codebook = prepared_codebook,
          .activation = activation,
          .workspace = pair_workspace};
  emel::kernel::cq::event::dispatch_result workspace_alias_result{};
  CHECK_FALSE(sm.process_event(
      emel::kernel::cq::event::execute_prepared_avx2_batch4_pair_q4{
          workspace_alias, workspace_alias_result}));
  CHECK_FALSE(workspace_alias_result.accepted);
...
TEST_CASE("CQ4 pair block32 preserves raw order and rejects tail geometry") {
#if defined(__AVX2__) && defined(__FMA__)
  constexpr uint32_t group = 128u;
  constexpr uint32_t in = 256u;
  constexpr uint32_t out = 45u;
  std::array<float, 28u> codebook{};
  for (uint32_t i = 0u; i < 16u; ++i)
    codebook[12u + i] =
        static_cast<float>(static_cast<int32_t>((i * 13u) & 15u) - 7) *
        0.15625f;
  emel::kernel::cq::event::prepared_codebook_q4 prepared_codebook{};
  emel::kernel::cq::action::prepare_codebook_q4(
      {codebook, prepared_codebook});
  ...
  for (uint32_t i = 0u; i < in; ++i)
    activation[i] =
        static_cast<float>(static_cast<int32_t>((i * 17u) % 67u) - 33) *
        0.03125f;
  ...
  struct pair_fixture {
    ...
    std::vector<float> norms_by_group32;
    emel::kernel::cq::event::prepared_q4_view prepared{};
    std::array<float, out> independent{};
    std::array<float, out> paired{};
  };
  std::array<pair_fixture, 2u> fixtures{};
  emel::kernel::cq::sm sm;
  for (uint32_t target = 0u; target < fixtures.size(); ++target) {
    auto &fixture = fixtures[target];
    std::vector<uint32_t> selectors(static_cast<size_t>(out) * in);
    for (uint32_t row = 0u; row < out; ++row)
      for (uint32_t i = 0u; i < in; ++i)
        selectors[static_cast<size_t>(row) * in + i] =
            (row * 5u + i * 11u + target * 7u) & 15u;
    std::vector<uint16_t> norm_bits(static_cast<size_t>(out) * in / group);
    for (size_t i = 0u; i < norm_bits.size(); ++i)
      norm_bits[i] = static_cast<uint16_t>(0x3400u +
                                           ((i * 3u + target) & 7u) * 0x100u);
    fixture.blob = blob<4u>(selectors, out, in, group, norm_bits);
    fixture.source = view(fixture.blob, out, in, group, 4u);
    fixture.indices.resize(static_cast<size_t>(out) * in);
    fixture.indices_by_input32.resize(static_cast<size_t>(out) * in);
    fixture.norms.resize(static_cast<size_t>(out) * in / group);
    fixture.norms_by_group32.resize(blocked_norm_count(fixture.source));
    const emel::kernel::cq::event::prepare_q4_request prepare_request{
        fixture.source, fixture.indices, fixture.indices_by_input32,
        fixture.norms, fixture.norms_by_group32, fixture.prepared};
    emel::kernel::cq::event::dispatch_result prepare_result{};
    REQUIRE(sm.process_event(
        emel::kernel::cq::event::prepare_q4{prepare_request, prepare_result}));
  }
  const emel::kernel::cq::event::prepared_gemv_batch4_pair_request request{
      .q = {&fixtures[0].prepared, fixtures[0].paired},
      .gate = {&fixtures[1].prepared, fixtures[1].paired},
      .k = {&fixtures[0].prepared, fixtures[0].paired},
      .v = {&fixtures[1].prepared, fixtures[1].paired},
      .codebook = prepared_codebook,
      .activation = activation,
