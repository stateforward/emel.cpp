...
// Mirrors the generator's parallel matmul lane slicing: a group-aligned row
// slice of src0/dst embedded in the full batch-major output, sharing src1.
inline emel::kernel::event::op_mul_mat
make_row_sliced_lane_event(const emel::kernel::event::op_mul_mat &ev,
                           const uint64_t group_rows, const uint64_t row_begin,
  ...
  sliced.dst.data =
      static_cast<uint8_t *>(ev.dst.data) + row_begin * ev.dst.nb[1];
...
TEST_CASE(
    "kernel_aarch64_mul_mat_simd_matches_scalar_when_matrix_has_eight_rows") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
...
TEST_CASE("kernel_aarch64_prepared_f32_lhs_4row_matches_scalar_with_depth_and_"
          "tails") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  ...
  std::vector<float> prepared(
      static_cast<size_t>(
          emel::kernel::aarch64::detail::prepared_f32_lhs_4row_value_count(k,
                                                                           m)),
      0.0f);
  ...
  for (uint64_t index = 0; index < src1.size(); ++index) {
    ...
    src1[static_cast<size_t>(index)] =
        static_cast<float>(centered) * 0.0078125f;
  }
  std::fill(dst_prepared.begin(), dst_prepared.end(),
            std::numeric_limits<float>::quiet_NaN());

  REQUIRE(emel::kernel::aarch64::detail::prepare_neon_mul_mat_f32_lhs_4row(
      src0.data(), k, m, prepared.data(),
  ...
  CHECK(
      emel::kernel::aarch64::detail::execute_neon_mul_mat_prepared_f32_lhs_4row(
          prepared_ev, prepared.data(),
          static_cast<uint64_t>(prepared.size())));
  CHECK(emel::kernel::detail::execute_scalar(scalar_ev));
  ...
  for (uint64_t index = 0; index < dst_prepared.size(); ++index) {
    ...
    CHECK(
        dst_prepared[static_cast<size_t>(index)] ==
        doctest::Approx(dst_scalar[static_cast<size_t>(index)]).epsilon(1e-5f));
  }
#endif
}

TEST_CASE("kernel_aarch64_prepared_f32_lhs_4row_accumulates_twelve_column_"
          "depth_blocks") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
...
TEST_CASE("kernel_aarch64_prepared_f32_lhs_4row_rejects_invalid_contract") {
  ...
  CHECK_FALSE(emel::kernel::aarch64::detail::prepare_neon_mul_mat_f32_lhs_4row(
      nullptr, k, m, prepared.data(), static_cast<uint64_t>(prepared.size())));
  CHECK_FALSE(emel::kernel::aarch64::detail::prepare_neon_mul_mat_f32_lhs_4row(
      src0.data(), k, m, prepared.data(),
  ...
  CHECK_FALSE(
      emel::kernel::aarch64::detail::execute_neon_mul_mat_prepared_f32_lhs_4row(
          ev, nullptr, static_cast<uint64_t>(prepared.size())));
  CHECK_FALSE(
      emel::kernel::aarch64::detail::execute_neon_mul_mat_prepared_f32_lhs_4row(
          ev, prepared.data(), static_cast<uint64_t>(prepared.size() - 1u)));

  ev.src1.ne[1] = k - 1u;
  CHECK_FALSE(
      emel::kernel::aarch64::detail::execute_neon_mul_mat_prepared_f32_lhs_4row(
          ev, prepared.data(), static_cast<uint64_t>(prepared.size())));
#endif
}
...
TEST_CASE("kernel_aarch64_image_pointwise_f32_matches_scalar_reference") {
  ...
  std::array<float, static_cast<size_t>(pixel_count) *
                        static_cast<size_t>(input_channels)>
      input = {};
  std::array<float, static_cast<size_t>(input_channels) *
  ...
  for (int32_t output_channel = 0; output_channel < output_channels;
       ++output_channel) {
    ...
    for (int32_t input_channel = 0; input_channel < input_channels;
         ++input_channel) {
      const size_t idx = static_cast<size_t>(input_channel) *
                             static_cast<size_t>(output_channels) +
  ...
  for (int32_t panel_col = 0; panel_col < output_channels;
       panel_col += panel_cols) {
    const int32_t cols_this_panel =
        std::min(panel_cols, output_channels - panel_col);
    float *panel_dst =
        packed_weights.data() +
        static_cast<size_t>(panel_col) * static_cast<size_t>(input_channels);
    for (int32_t input_channel = 0; input_channel < input_channels;
         ++input_channel) {
      for (int32_t local_col = 0; local_col < cols_this_panel; ++local_col) {
        panel_dst[static_cast<size_t>(input_channel) *
                      static_cast<size_t>(cols_this_panel) +
  ...
  CHECK(emel::kernel::aarch64::detail::execute_neon_image_pointwise_f32<true,
                                                                        true>({
      .input = input.data(),
  ...
  for (int32_t pixel_index = 0; pixel_index < pixel_count; ++pixel_index) {
    const float *input_pixel =
        input.data() +
        static_cast<size_t>(pixel_index) * static_cast<size_t>(input_channels);
    float *output_pixel =
        output_scalar.data() +
        static_cast<size_t>(pixel_index) * static_cast<size_t>(output_channels);
    for (int32_t output_channel = 0; output_channel < output_channels;
         ++output_channel) {
      float acc = 0.0f;
      for (int32_t input_channel = 0; input_channel < input_channels;
  ...
  for (size_t idx = 0; idx < output_kernel.size(); ++idx) {
    CHECK(output_kernel[idx] ==
          doctest::Approx(output_scalar[idx]).epsilon(1.0e-5f));
  }
#endif
}
...
TEST_CASE("kernel_aarch64_image_depthwise_f32_matches_scalar_reference") {
  ...
  constexpr int32_t output_spatial =
      (input_spatial + 2 * pad - kernel_h) / stride + 1;

  std::array<float, static_cast<size_t>(input_spatial) *
                        static_cast<size_t>(input_spatial) *
                        static_cast<size_t>(channels)>
      input = {};
  std::array<float, static_cast<size_t>(kernel_h) *
                        static_cast<size_t>(kernel_w) *
                        static_cast<size_t>(channels)>
      kernel_major = {};
  std::array<float, static_cast<size_t>(output_spatial) *
                        static_cast<size_t>(output_spatial) *
                        static_cast<size_t>(channels)>
      output_kernel = {};
  std::array<float, static_cast<size_t>(output_spatial) *
                        static_cast<size_t>(output_spatial) *
  ...
  for (int32_t oy = 0; oy < output_spatial; ++oy) {
    for (int32_t ox = 0; ox < output_spatial; ++ox) {
      float *output_pixel =
          output_scalar.data() + static_cast<size_t>(oy * output_spatial + ox) *
                                     static_cast<size_t>(channels);
      ...
      for (int32_t ky = 0; ky < kernel_h; ++ky) {
        ...
        for (int32_t kx = 0; kx < kernel_w; ++kx) {
          ...
          const float *input_pixel =
              input.data() + static_cast<size_t>(iy * input_spatial + ix) *
                                 static_cast<size_t>(channels);
          const float *kernel_weights =
              kernel_major.data() + static_cast<size_t>(ky * kernel_w + kx) *
                                        static_cast<size_t>(channels);
...
TEST_CASE("kernel_aarch64_q4_0_vector_route_is_explicit_and_numeric_match") {
  ...
  for (size_t index = 0; index < dense_rows.size(); ++index) {
    dense_rows[index] =
        static_cast<float>(static_cast<int32_t>((index * 5u) % 21u) - 10) *
        0.125f;
  }

  std::array<block_q4_0, row_count> q4_rows = {};
  for (uint64_t row = 0; row < row_count; ++row) {
    quantize_row_q4_0_ref(dense_rows.data() + row * QK4_0, q4_rows.data() + row,
  ...
  for (size_t index = 0; index < input.size(); ++index) {
    input[index] =
        static_cast<float>(static_cast<int32_t>((index * 7u) % 19u) - 9) *
        0.25f;
  }
  ...
  CHECK(emel::kernel::aarch64::detail::can_use_neon_mul_mat_q4_0_vector(ev,
                                                                        true));
...
TEST_CASE("kernel_aarch64_q4_1_vector_route_is_explicit_and_numeric_match") {
  ...
  for (uint64_t row = 0; row < row_count; ++row) {
    quantize_row_q4_1_ref(dense_rows.data() + row * QK4_1, q4_rows.data() + row,
  ...
  CHECK(emel::kernel::aarch64::detail::can_use_neon_mul_mat_q4_1_vector(ev,
                                                                        true));
...
TEST_CASE("kernel_aarch64_q5_0_vector_route_is_explicit_and_numeric_match") {
  ...
  for (uint64_t row = 0; row < row_count; ++row) {
    quantize_row_q5_0_ref(dense_rows.data() + row * QK5_0, q5_rows.data() + row,
  ...
  CHECK(emel::kernel::aarch64::detail::can_use_neon_mul_mat_q5_0_vector(ev,
                                                                        true));
...
TEST_CASE(
    "kernel_aarch64_q4_q5_vector_unchecked_paths_match_scalar_with_tail_rows") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  ...
  auto check_rows = [](const auto &optimized, const auto &scalar) {
    for (size_t row = 0; row < optimized.size(); ++row) {
      CHECK(optimized[row] == doctest::Approx(scalar[row]).epsilon(1.0e-5f));
    }
  };

  {
    ...
    std::array<block_q4_0, row_count> q4_rows = {};
    for (uint64_t row = 0; row < row_count; ++row) {
      quantize_row_q4_0_ref(dense_rows.data() +
                                static_cast<size_t>(row) * QK4_0,
                            q4_rows.data() + row, static_cast<int64_t>(QK4_0));
    }
    ...
    const emel::kernel::event::op_mul_mat optimized_ev{
        .src0 =
            make_quantized_src(q4_rows.data(), dtype::q4_0, QK4_0, row_count),
        .src1 = make_src(input.data(), dtype::f32, 1u, QK4_0),
...

  {
    ...
    std::array<block_q4_1, row_count> q4_rows = {};
    for (uint64_t row = 0; row < row_count; ++row) {
      quantize_row_q4_1_ref(dense_rows.data() +
                                static_cast<size_t>(row) * QK4_1,
                            q4_rows.data() + row, static_cast<int64_t>(QK4_1));
    }
    ...
    const emel::kernel::event::op_mul_mat optimized_ev{
        .src0 =
            make_quantized_src(q4_rows.data(), dtype::q4_1, QK4_1, row_count),
        .src1 = make_src(input.data(), dtype::f32, 1u, QK4_1),
        .dst = make_dst(optimized.data(), dtype::f32, 1u, row_count),
    };
...

  {
    ...
    std::array<block_q5_0, row_count> q5_rows = {};
    for (uint64_t row = 0; row < row_count; ++row) {
      quantize_row_q5_0_ref(dense_rows.data() +
                                static_cast<size_t>(row) * QK5_0,
                            q5_rows.data() + row, static_cast<int64_t>(QK5_0));
    }
    ...
    const emel::kernel::event::op_mul_mat optimized_ev{
        .src0 =
            make_quantized_src(q5_rows.data(), dtype::q5_0, QK5_0, row_count),
        .src1 = make_src(input.data(), dtype::f32, 1u, QK5_0),
        .dst = make_dst(optimized.data(), dtype::f32, 1u, row_count),
    };
...
TEST_CASE("kernel_aarch64_q2_row_neon_matches_scalar") {
  ...
  for (size_t block = 0; block < block_count; ++block) {
    ...
    for (size_t i = 0; i < q2.qs.size(); ++i) {
      q2.qs[i] =
          static_cast<uint8_t>((i * (23u + block)) ^ ((i + block) >> 2u));
    }
  }
  ...
  const float scalar = emel::kernel::detail::dot_q2_k_q8_k_row_scalar(
      q2_blocks.data(), q8_blocks.data(), q8_blocks.size());
  const float neon = emel::kernel::aarch64::detail::dot_q2_k_q8_k_row_neon(
      q2_blocks.data(), q8_blocks.data(), q8_blocks.size());
...
TEST_CASE("kernel_aarch64_q2_row_neon_matches_shared_accumulation_order") {
  ...
  const float shared = emel::kernel::detail::dot_q2_k_q8_k_row_scalar(
      q2_blocks.data(), q8_blocks.data(), block_count);
  const float optimized = emel::kernel::aarch64::detail::dot_q2_k_q8_k_row_neon(
      q2_blocks.data(), q8_blocks.data(), block_count);
...
TEST_CASE("kernel_aarch64_q3_row_neon_matches_scalar") {
  ...
  for (size_t block = 0; block < block_count; ++block) {
    ...
    for (size_t i = 0; i < q3.scales.size(); ++i) {
      q3.scales[i] =
          static_cast<uint8_t>((i * (17u + block)) ^ (0x5au + block));
    }
    for (size_t i = 0; i < q3.hmask.size(); ++i) {
    ...
    for (size_t i = 0; i < q3.qs.size(); ++i) {
      q3.qs[i] =
          static_cast<uint8_t>((i * (13u + block)) ^ (0x33u + block * 7u));
    }
  }
  ...
  const float scalar = emel::kernel::detail::dot_q3_k_q8_k_row_scalar(
      q3_blocks.data(), q8_blocks.data(), q8_blocks.size());
  const float neon = emel::kernel::aarch64::detail::dot_q3_k_q8_k_row_neon(
      q3_blocks.data(), q8_blocks.data(), q8_blocks.size());
...
TEST_CASE("kernel_aarch64_q6_row_neon_matches_scalar") {
  ...
  const float scalar = emel::kernel::detail::dot_q6_k_q8_k_row_scalar(
      &q6, q8_blocks.data(), q8_blocks.size());
  const float neon = emel::kernel::aarch64::detail::dot_q6_k_q8_k_row_neon(
      &q6, q8_blocks.data(), q8_blocks.size());
...
TEST_CASE("kernel_aarch64_q4_k_row_neon_matches_scalar") {
  ...
  const float neon = emel::kernel::aarch64::detail::dot_q4_k_q8_k_row_neon(
      &q4, q8_blocks.data(), q8_blocks.size());
...
TEST_CASE("kernel_aarch64_q4_k_2rows_neon_matches_scalar") {
  ...
  for (size_t row = 0; row < q4_rows.size(); ++row) {
    ...
    for (size_t i = 0; i < q4_rows[row].scales.size(); ++i) {
      q4_rows[row].scales[i] =
          static_cast<uint8_t>((i * (11u + row)) ^ (0x5au + row));
    }
    for (size_t i = 0; i < q4_rows[row].qs.size(); ++i) {
      q4_rows[row].qs[i] =
          static_cast<uint8_t>((i * (9u + row)) ^ (0x96u - row));
    }
  }
...
TEST_CASE("kernel_aarch64_q4_k_4rows_neon_matches_scalar") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  CHECK(true);
  return;
#else
  ...
  std::array<block_q4_k, 4> q4_rows = {};
  for (size_t row = 0; row < q4_rows.size(); ++row) {
    q4_rows[row].d = static_cast<uint16_t>(0x3c00u + row * 0x0040u);
    q4_rows[row].dmin = static_cast<uint16_t>(0x3800u + row * 0x0080u);
    for (size_t i = 0; i < q4_rows[row].scales.size(); ++i) {
      q4_rows[row].scales[i] =
          static_cast<uint8_t>((i * (13u + row)) ^ (0x53u + row));
    }
    for (size_t i = 0; i < q4_rows[row].qs.size(); ++i) {
      q4_rows[row].qs[i] =
          static_cast<uint8_t>((i * (7u + row)) ^ (0x9bu - row));
    }
  }

  std::array<float, QK_K> src1 = {};
  for (size_t i = 0; i < src1.size(); ++i) {
    const int32_t centered = static_cast<int32_t>((i * 5u) % 29u) - 14;
    src1[i] = static_cast<float>(centered) * 0.0625f;
  }
  ...
  float neon[4] = {};
  emel::kernel::aarch64::detail::dot_q4_k_q8_k_4rows_neon(
      &q4_rows[0], &q4_rows[1], &q4_rows[2], &q4_rows[3], q8_blocks.data(),
      q8_blocks.size(), neon);
...
TEST_CASE("kernel_aarch64_q4_k_multirow_preserves_one_row_accumulation_order") {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  CHECK(true);
#else
  ...
  constexpr size_t MAX_BLOCK_COUNT = 64u;
  std::array<block_q4_k, ROW_COUNT * MAX_BLOCK_COUNT> q4_rows = {};
  std::array<block_q8_k, MAX_BLOCK_COUNT> q8_blocks = {};
  for (size_t row = 0; row < ROW_COUNT; ++row) {
    for (size_t block = 0; block < MAX_BLOCK_COUNT; ++block) {
      auto &q4 = q4_rows[row * MAX_BLOCK_COUNT + block];
      q4.d = emel::kernel::detail::quant::fp32_to_fp16(
          0.0009765625f *
  ...
  constexpr std::array<uint64_t, 4> BLOCK_COUNTS = {1u, 2u, 7u, 64u};
  for (const uint64_t block_count : BLOCK_COUNTS) {
    std::array<float, ROW_COUNT> one_row = {};
    for (size_t row = 0; row < ROW_COUNT; ++row) {
      one_row[row] = emel::kernel::aarch64::detail::dot_q4_k_q8_k_row_neon(
          q4_rows.data() + row * MAX_BLOCK_COUNT, q8_blocks.data(),
          block_count);
    }

    std::array<float, 2> two_rows = {};
    emel::kernel::aarch64::detail::dot_q4_k_q8_k_2rows_neon(
        q4_rows.data(), q4_rows.data() + MAX_BLOCK_COUNT, q8_blocks.data(),
        block_count, two_rows.data());
    CHECK(std::bit_cast<uint32_t>(two_rows[0]) ==
          std::bit_cast<uint32_t>(one_row[0]));
    CHECK(std::bit_cast<uint32_t>(two_rows[1]) ==
          std::bit_cast<uint32_t>(one_row[1]));

    std::array<float, ROW_COUNT> four_rows = {};
    emel::kernel::aarch64::detail::dot_q4_k_q8_k_4rows_neon(
        q4_rows.data(), q4_rows.data() + MAX_BLOCK_COUNT,
        q4_rows.data() + MAX_BLOCK_COUNT * 2u,
        q4_rows.data() + MAX_BLOCK_COUNT * 3u, q8_blocks.data(), block_count,
        four_rows.data());
    for (size_t row = 0; row < ROW_COUNT; ++row) {
      CHECK(std::bit_cast<uint32_t>(four_rows[row]) ==
            std::bit_cast<uint32_t>(one_row[row]));
    }
  }
#endif
}
...
TEST_CASE(
    "kernel_aarch64_f32_vector_gemv_route_is_explicit_and_bit_identical") {
...
TEST_CASE("kernel_aarch64_q4_k_uses_neon_dispatch_when_dotprod_is_available") {
  ...
  CHECK(emel::kernel::aarch64::detail::
            can_use_neon_mul_mat_q4_vector_f32_rhs(shared_ev, true));
#else
  ...
  CHECK(machine.shared_q4_dispatch_count() == 0u);
#else
  CHECK(machine.optimized_q4_dispatch_count() == 0u);
  CHECK(machine.optimized_q4_vector_dispatch_count() == 0u);
  CHECK(machine.shared_q4_dispatch_count() == 1u);
#endif
}
...
TEST_CASE("kernel_aarch64_supported_quantized_dispatch_is_alloc_free") {
  ...
  CHECK(machine.shared_q6_dispatch_count() == 0u);
  CHECK(machine.optimized_q8_0_dispatch_count() ==
        1u + expected_packed_q8_dispatches);
  CHECK(machine.optimized_q8_0_vector_dispatch_count() == 1u);
  CHECK(machine.optimized_q8_0_packed_dispatch_count() ==
        expected_packed_q8_dispatches);
  CHECK(machine.shared_q8_0_dispatch_count() == 0u);
#else
  CHECK(machine.optimized_q2_dispatch_count() == 0u);
  CHECK(machine.shared_q2_dispatch_count() == 1u);
...
TEST_CASE(
    "kernel_aarch64_q6_packed_vector_route_is_explicit_and_numeric_match") {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  CHECK(true);
#else
  ...
  const emel::kernel::event::op_mul_mat prepared_ev{
      .src0 =
          make_prepared_q6_k_x8_q8_src(prepared_storage.data(), QK_K, k_rows),
      .src1 = make_q8_k_vector_src(q8_input.data(), QK_K),
  ...
  for (size_t row = 0; row < native_out.size(); ++row) {
    ...
    CHECK(prepared_out[row] ==
          doctest::Approx(native_out[row]).epsilon(1.0e-6f));
  }
  ...
  CHECK(packed_machine.optimized_q6_vector_packed_q8_rhs_dispatch_count() ==
        1u);
  ...
  CHECK(prepared_machine.optimized_q6_vector_packed_q8_rhs_dispatch_count() ==
        1u);
  CHECK(prepared_machine.optimized_q6_vector_prepared_q8_rhs_dispatch_count() ==
        1u);
#if defined(__ARM_FEATURE_MATMUL_INT8)
  CHECK(prepared_machine
            .optimized_q6_vector_prepared_q8_rhs_i8mm_dispatch_count() == 1u);
#else
  CHECK(prepared_machine
            .optimized_q6_vector_prepared_q8_rhs_i8mm_dispatch_count() == 0u);
#endif
...
TEST_CASE(
    "kernel_aarch64_q4_packed_vector_route_is_explicit_and_numeric_match") {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  CHECK(true);
#else
  ...
  std::array<uint8_t, sizeof(emel::kernel::detail::quant::block_q4_kx8)>
      packed_bl4_storage = {};
  std::array<uint8_t, sizeof(emel::kernel::detail::quant::block_q4_kx8)>
      packed_bl8_storage = {};
  ...
  std::array<float, Q4_K_X8_ROWS> packed_bl4_f32_out = {};
  std::array<float, Q4_K_X8_ROWS> packed_bl8_out = {};
  std::array<float, Q4_K_X8_ROWS> packed_bl8_f32_out = {};
  const emel::kernel::event::op_mul_mat native_ev{
      .src0 = make_quantized_src(native_rows.data(), dtype::q4_k, QK_K,
                                 Q4_K_X8_ROWS),
      .src1 = make_src(input.data(), dtype::f32, 1u, QK_K),
      .dst = make_dst(native_out.data(), dtype::f32, 1u, Q4_K_X8_ROWS),
  };
  const emel::kernel::event::op_mul_mat packed_bl4_ev{
      .src0 = make_packed_q4_k_x8_bl4_src(packed_bl4_storage.data(), QK_K,
                                          Q4_K_X8_ROWS),
      .src1 = make_q8_k_vector_src(q8_input.data(), QK_K),
      .dst = make_dst(packed_bl4_out.data(), dtype::f32, 1u, Q4_K_X8_ROWS),
  };
  const emel::kernel::event::op_mul_mat packed_bl8_ev{
      .src0 = make_packed_q4_k_x8_bl8_src(packed_bl8_storage.data(), QK_K,
                                          Q4_K_X8_ROWS),
      .src1 = make_q8_k_vector_src(q8_input.data(), QK_K),
      .dst = make_dst(packed_bl8_out.data(), dtype::f32, 1u, Q4_K_X8_ROWS),
  };
  const emel::kernel::event::op_mul_mat packed_bl4_f32_ev{
      .src0 = make_packed_q4_k_x8_bl4_src(packed_bl4_storage.data(), QK_K,
                                          Q4_K_X8_ROWS),
      .src1 = make_src(input.data(), dtype::f32, 1u, QK_K),
      .dst =
          make_dst(packed_bl4_f32_out.data(), dtype::f32, 1u, Q4_K_X8_ROWS),
  };
  const emel::kernel::event::op_mul_mat packed_bl8_f32_ev{
      .src0 = make_packed_q4_k_x8_bl8_src(packed_bl8_storage.data(), QK_K,
                                          Q4_K_X8_ROWS),
      .src1 = make_src(input.data(), dtype::f32, 1u, QK_K),
      .dst =
          make_dst(packed_bl8_f32_out.data(), dtype::f32, 1u, Q4_K_X8_ROWS),
  };
  float argmax_score = 0.0f;
  int32_t argmax_index = -1;
  float bl4_argmax_score = 0.0f;
  int32_t bl4_argmax_index = -1;
  const emel::kernel::event::op_mul_mat_argmax packed_bl4_f32_argmax_ev{
      .src0 = make_packed_q4_k_x8_bl4_src(packed_bl4_storage.data(), QK_K,
                                          Q4_K_X8_ROWS),
      .src1 = make_src(input.data(), dtype::f32, 1u, QK_K),
      .dst = make_dst(&bl4_argmax_score, dtype::f32, 1u, 1u),
      .index_out = &bl4_argmax_index,
  };
  const emel::kernel::event::op_mul_mat_argmax packed_bl8_f32_argmax_ev{
      .src0 = make_packed_q4_k_x8_bl8_src(packed_bl8_storage.data(), QK_K,
                                          Q4_K_X8_ROWS),
      .src1 = make_src(input.data(), dtype::f32, 1u, QK_K),
  ...
  aarch64_sm packed_bl4_f32_machine{};
  aarch64_sm packed_bl4_f32_argmax_machine{};
  aarch64_sm packed_bl8_machine{};
  aarch64_sm packed_bl8_f32_machine{};
  aarch64_sm packed_bl8_f32_argmax_machine{};
  CHECK(native_machine.process_event(native_ev));
  CHECK(packed_bl4_machine.process_event(packed_bl4_ev));
  CHECK(packed_bl4_f32_machine.process_event(packed_bl4_f32_ev));
  CHECK(packed_bl4_f32_argmax_machine.process_event(
      packed_bl4_f32_argmax_ev));
  CHECK(packed_bl8_machine.process_event(packed_bl8_ev));
  CHECK(packed_bl8_f32_machine.process_event(packed_bl8_f32_ev));
  CHECK(packed_bl8_f32_argmax_machine.process_event(
      packed_bl8_f32_argmax_ev));

  int32_t expected_argmax_index = 0;
  float expected_argmax_score = native_out[0];
  for (size_t row = 1; row < native_out.size(); ++row) {
    if (native_out[row] > expected_argmax_score) {
      expected_argmax_score = native_out[row];
      expected_argmax_index = static_cast<int32_t>(row);
    }
  }

  for (size_t row = 0; row < native_out.size(); ++row) {
    CHECK(packed_bl4_out[row] ==
          doctest::Approx(native_out[row]).epsilon(1.0e-5f));
    CHECK(packed_bl4_f32_out[row] ==
          doctest::Approx(native_out[row]).epsilon(1.0e-5f));
    CHECK(packed_bl8_out[row] ==
          doctest::Approx(native_out[row]).epsilon(1.0e-5f));
    CHECK(packed_bl8_f32_out[row] ==
          doctest::Approx(native_out[row]).epsilon(1.0e-5f));
    CHECK(std::bit_cast<uint32_t>(packed_bl8_out[row]) ==
          std::bit_cast<uint32_t>(native_out[row]));
    CHECK(std::bit_cast<uint32_t>(packed_bl8_f32_out[row]) ==
          std::bit_cast<uint32_t>(native_out[row]));
  }
  CHECK(bl4_argmax_index == expected_argmax_index);
  CHECK(bl4_argmax_score ==
        doctest::Approx(expected_argmax_score).epsilon(1.0e-5f));
  CHECK(argmax_index == expected_argmax_index);
  CHECK(argmax_score == doctest::Approx(expected_argmax_score).epsilon(1.0e-5f));
  ...
  CHECK(packed_bl4_machine.optimized_q4_vector_packed_q8_rhs_dispatch_count() ==
        1u);
  CHECK(packed_bl4_machine.shared_q4_dispatch_count() == 0u);
  CHECK(packed_bl4_f32_machine.optimized_q4_dispatch_count() == 1u);
  CHECK(packed_bl4_f32_machine.optimized_q4_vector_dispatch_count() == 1u);
  CHECK(packed_bl4_f32_machine.optimized_q4_vector_packed_dispatch_count() ==
        1u);
  CHECK(packed_bl4_f32_machine.shared_q4_dispatch_count() == 0u);
  CHECK(packed_bl4_f32_argmax_machine.optimized_q4_dispatch_count() == 1u);
  CHECK(packed_bl4_f32_argmax_machine.optimized_q4_vector_dispatch_count() ==
        1u);
  CHECK(packed_bl4_f32_argmax_machine
            .optimized_q4_vector_packed_dispatch_count() == 1u);
  CHECK(packed_bl4_f32_argmax_machine.shared_q4_dispatch_count() == 0u);
  ...
  CHECK(packed_bl8_machine.optimized_q4_vector_packed_q8_rhs_dispatch_count() ==
        1u);
  CHECK(packed_bl8_machine.shared_q4_dispatch_count() == 0u);
  CHECK(packed_bl8_f32_machine.optimized_q4_dispatch_count() == 1u);
  CHECK(packed_bl8_f32_machine.optimized_q4_vector_dispatch_count() == 1u);
  CHECK(packed_bl8_f32_machine.optimized_q4_vector_packed_dispatch_count() ==
        1u);
  CHECK(packed_bl8_f32_machine
            .optimized_q4_vector_packed_q8_rhs_dispatch_count() == 0u);
  CHECK(packed_bl8_f32_machine.shared_q4_dispatch_count() == 0u);
  CHECK(packed_bl8_f32_argmax_machine.optimized_q4_dispatch_count() == 1u);
  CHECK(packed_bl8_f32_argmax_machine.optimized_q4_vector_dispatch_count() ==
        1u);
  CHECK(packed_bl8_f32_argmax_machine
            .optimized_q4_vector_packed_dispatch_count() == 1u);
  CHECK(packed_bl8_f32_argmax_machine.shared_q4_dispatch_count() == 0u);
#endif
}

TEST_CASE("kernel_aarch64_q4_packed_bl8_preserves_native_block_order_bits") {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  CHECK(true);
#else
  using emel::kernel::detail::quant::block_q4_k;
  using emel::kernel::detail::quant::block_q4_kx8;
  using emel::kernel::detail::quant::Q4_K_X8_ROWS;
  using emel::kernel::detail::quant::QK_K;

  constexpr uint64_t block_count = 4u;
  constexpr uint64_t cols = block_count * QK_K;
  std::array<block_q4_k, Q4_K_X8_ROWS * block_count> native_rows = {};
  for (size_t row = 0; row < Q4_K_X8_ROWS; ++row) {
    for (size_t block = 0; block < block_count; ++block) {
      auto &value = native_rows[row * block_count + block];
      value.d = static_cast<uint16_t>(0x3000u + row * 31u + block * 7u);
      value.dmin = static_cast<uint16_t>(0x2800u + row * 19u + block * 11u);
      for (size_t index = 0; index < value.scales.size(); ++index) {
        value.scales[index] = static_cast<uint8_t>(
            (row * 37u + block * 53u + index * 17u + 9u) & 0xffu);
      }
      for (size_t index = 0; index < value.qs.size(); ++index) {
        value.qs[index] = static_cast<uint8_t>(
            (row * 29u + block * 43u + index * 13u + 5u) & 0xffu);
      }
    }
  }

  const std::array<float, cols> input = [] {
    std::array<float, cols> values = {};
    for (size_t index = 0; index < values.size(); ++index) {
      const int32_t centered = static_cast<int32_t>(index % 61u) - 30;
      values[index] = static_cast<float>(centered) * 0.03125f;
    }
    return values;
  }();

  std::array<uint8_t, block_count * sizeof(block_q4_kx8)> packed = {};
  REQUIRE(emel::kernel::detail::quant::pack_q4_k_rows_x8_bl8(
      native_rows.data(), Q4_K_X8_ROWS, cols, packed.data()));

  std::array<float, Q4_K_X8_ROWS> native_out = {};
  std::array<float, Q4_K_X8_ROWS> packed_out = {};
  const emel::kernel::event::op_mul_mat native_ev{
      .src0 = make_quantized_src(native_rows.data(), dtype::q4_k, cols,
                                 Q4_K_X8_ROWS),
      .src1 = make_src(input.data(), dtype::f32, 1u, cols),
      .dst = make_dst(native_out.data(), dtype::f32, 1u, Q4_K_X8_ROWS),
  };
  const emel::kernel::event::op_mul_mat packed_ev{
      .src0 = make_packed_q4_k_x8_bl8_src(packed.data(), cols, Q4_K_X8_ROWS),
      .src1 = make_src(input.data(), dtype::f32, 1u, cols),
      .dst = make_dst(packed_out.data(), dtype::f32, 1u, Q4_K_X8_ROWS),
  };

  aarch64_sm native_machine{};
  aarch64_sm packed_machine{};
  REQUIRE(native_machine.process_event(native_ev));
  REQUIRE(packed_machine.process_event(packed_ev));
  for (size_t row = 0; row < native_out.size(); ++row) {
    CHECK(std::bit_cast<uint32_t>(packed_out[row]) ==
          std::bit_cast<uint32_t>(native_out[row]));
  }
#endif
}

TEST_CASE("kernel_aarch64_raw_quantized_prepared_rhs_routes_are_explicit_and_"
          "numeric_match") {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  CHECK(true);
  ...
  constexpr uint64_t qk_rows = 6u;
  std::array<block_q4_k, qk_rows> q4_rows = {};
  std::array<block_q6_k, qk_rows> q6_rows = {};
  for (size_t row = 0; row < qk_rows; ++row) {
    q4_rows[row].d = 0x3800u;
    q4_rows[row].dmin = 0x3400u;
    for (size_t idx = 0; idx < q4_rows[row].scales.size(); ++idx) {
      q4_rows[row].scales[idx] =
          static_cast<uint8_t>(((row + 7u) * 19u + idx * 13u) & 0xffu);
    }
    for (size_t idx = 0; idx < q4_rows[row].qs.size(); ++idx) {
      q4_rows[row].qs[idx] =
          static_cast<uint8_t>(((row + 3u) * 17u + idx * 5u) & 0xffu);
    }

    q6_rows[row].d = 0x3c00u;
    for (size_t idx = 0; idx < q6_rows[row].scales.size(); ++idx) {
      q6_rows[row].scales[idx] =
          static_cast<int8_t>(static_cast<int32_t>((row + idx) % 15u) - 7);
    }
    for (size_t idx = 0; idx < q6_rows[row].ql.size(); ++idx) {
      q6_rows[row].ql[idx] =
          static_cast<uint8_t>(((row + 1u) * 11u + idx * 7u) & 0xffu);
    }
    for (size_t idx = 0; idx < q6_rows[row].qh.size(); ++idx) {
      q6_rows[row].qh[idx] =
          static_cast<uint8_t>(((row + 5u) * 23u + idx * 3u) & 0xffu);
    }
  }

  constexpr uint64_t q8_rows_count = 5u;
  std::array<block_q8_0, q8_rows_count> q8_rows = {};
  for (size_t row = 0; row < q8_rows.size(); ++row) {
    q8_rows[row].d = emel::kernel::detail::quant::fp32_to_fp16(
        0.03125f * static_cast<float>(row + 1u));
    for (size_t idx = 0; idx < q8_rows[row].qs.size(); ++idx) {
      q8_rows[row].qs[idx] = static_cast<int8_t>(
          static_cast<int32_t>(((row + 2u) * 13u + idx * 5u) % 31u) - 15);
    }
  }

  const std::array<float, QK_K> qk_input = [] {
    std::array<float, QK_K> values = {};
    for (size_t idx = 0; idx < values.size(); ++idx) {
      const int32_t centered = static_cast<int32_t>(idx % 29u) - 14;
      values[idx] = static_cast<float>(centered) * 0.0625f;
  ...
  std::array<block_q8_k, QK_K / QK_K> q8_k_input = {};
  emel::kernel::detail::quant::quantize_row_q8_k_strided(
      qk_input.data(), 1u, q8_k_input.data(), static_cast<int64_t>(QK_K));
  std::array<block_q8_0, QK8_0 / QK8_0> q8_0_input = {};
  emel::kernel::detail::quant::quantize_row_q8_0_strided(
      q8_input.data(), 1u, q8_0_input.data(), static_cast<int64_t>(QK8_0));

  std::array<float, qk_rows> q4_reference = {};
  std::array<float, qk_rows> q4_prepared = {};
  const emel::kernel::event::op_mul_mat q4_reference_ev{
      .src0 = make_quantized_src(q4_rows.data(), dtype::q4_k, QK_K, qk_rows),
      .src1 = make_src(qk_input.data(), dtype::f32, 1u, QK_K),
      .dst = make_dst(q4_reference.data(), dtype::f32, 1u, qk_rows),
  };
  const emel::kernel::event::op_mul_mat q4_prepared_ev{
      .src0 = make_quantized_src(q4_rows.data(), dtype::q4_k, QK_K, qk_rows),
      .src1 = make_q8_k_vector_src(q8_k_input.data(), QK_K),
      .dst = make_dst(q4_prepared.data(), dtype::f32, 1u, qk_rows),
  };

  std::array<float, qk_rows> q6_reference = {};
  std::array<float, qk_rows> q6_prepared = {};
  const emel::kernel::event::op_mul_mat q6_reference_ev{
      .src0 = make_quantized_src(q6_rows.data(), dtype::q6_k, QK_K, qk_rows),
      .src1 = make_src(qk_input.data(), dtype::f32, 1u, QK_K),
      .dst = make_dst(q6_reference.data(), dtype::f32, 1u, qk_rows),
  };
  const emel::kernel::event::op_mul_mat q6_prepared_ev{
      .src0 = make_quantized_src(q6_rows.data(), dtype::q6_k, QK_K, qk_rows),
      .src1 = make_q8_k_vector_src(q8_k_input.data(), QK_K),
      .dst = make_dst(q6_prepared.data(), dtype::f32, 1u, qk_rows),
  };

  std::array<float, q8_rows_count> q8_reference = {};
  std::array<float, q8_rows_count> q8_prepared = {};
  const emel::kernel::event::op_mul_mat q8_reference_ev{
      .src0 =
          make_quantized_src(q8_rows.data(), dtype::q8_0, QK8_0, q8_rows_count),
      .src1 = make_src(q8_input.data(), dtype::f32, 1u, QK8_0),
      .dst = make_dst(q8_reference.data(), dtype::f32, 1u, q8_rows_count),
  };
  const emel::kernel::event::op_mul_mat q8_prepared_ev{
      .src0 =
          make_quantized_src(q8_rows.data(), dtype::q8_0, QK8_0, q8_rows_count),
      .src1 = make_q8_0_vector_src(q8_0_input.data(), QK8_0),
      .dst = make_dst(q8_prepared.data(), dtype::f32, 1u, q8_rows_count),
  };

  aarch64_sm q4_reference_machine{};
  aarch64_sm q4_prepared_machine{};
  aarch64_sm q6_reference_machine{};
  aarch64_sm q6_prepared_machine{};
  aarch64_sm q8_reference_machine{};
  aarch64_sm q8_prepared_machine{};
  CHECK(q4_reference_machine.process_event(q4_reference_ev));
  CHECK(q4_prepared_machine.process_event(q4_prepared_ev));
  CHECK(q6_reference_machine.process_event(q6_reference_ev));
  CHECK(q6_prepared_machine.process_event(q6_prepared_ev));
  CHECK(q8_reference_machine.process_event(q8_reference_ev));
  CHECK(q8_prepared_machine.process_event(q8_prepared_ev));

  for (size_t row = 0; row < qk_rows; ++row) {
    CHECK(q4_prepared[row] ==
          doctest::Approx(q4_reference[row]).epsilon(1.0e-5f));
    CHECK(q6_prepared[row] ==
          doctest::Approx(q6_reference[row]).epsilon(1.0e-6f));
  }
  for (size_t row = 0; row < q8_rows_count; ++row) {
    CHECK(q8_prepared[row] ==
          doctest::Approx(q8_reference[row]).epsilon(1.0e-6f));
  }

  CHECK(q4_prepared_machine.optimized_q4_dispatch_count() == 1u);
  CHECK(q4_prepared_machine.optimized_q4_vector_dispatch_count() == 1u);
  CHECK(q4_prepared_machine.optimized_q4_vector_packed_dispatch_count() == 0u);
  CHECK(q4_prepared_machine.shared_q4_dispatch_count() == 0u);
  CHECK(q6_prepared_machine.optimized_q6_dispatch_count() == 1u);
  CHECK(q6_prepared_machine.optimized_q6_vector_dispatch_count() == 1u);
  CHECK(q6_prepared_machine.optimized_q6_vector_packed_dispatch_count() == 0u);
  CHECK(q6_prepared_machine.shared_q6_dispatch_count() == 0u);
  CHECK(q8_prepared_machine.optimized_q8_0_dispatch_count() == 1u);
  CHECK(q8_prepared_machine.optimized_q8_0_vector_dispatch_count() == 1u);
  CHECK(q8_prepared_machine.optimized_q8_0_packed_dispatch_count() == 0u);
  CHECK(q8_prepared_machine.shared_q8_0_dispatch_count() == 0u);
#endif
}
...
TEST_CASE(
    "kernel_aarch64_q8_0_packed_route_matches_multi_block_native_reference") {
  using emel::kernel::detail::quant::block_q8_0;
  using emel::kernel::detail::quant::QK8_0;
  ...
  std::vector<block_q8_0> native_rows(
      static_cast<size_t>(row_count * block_count));
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    for (size_t block = 0; block < static_cast<size_t>(block_count); ++block) {
      ...
      cell.d = emel::kernel::detail::quant::fp32_to_fp16(
          0.0078125f *
          static_cast<float>(((row + 3u) * (block + 5u)) % 23u + 1u));
      for (size_t idx = 0; idx < cell.qs.size(); ++idx) {
        const int32_t centered =
            static_cast<int32_t>(
                ((row + 7u) * 17u + (block + 11u) * 13u + idx * 5u) % 255u) -
            127;
  ...
  const emel::kernel::event::op_mul_mat ev{
      .src0 =
          make_packed_q8_0_x4_bl8_src(packed_bl8.data(), col_count, row_count),
  ...
  const emel::kernel::event::op_mul_mat ev{
      .src0 =
          make_packed_q8_0_x4_bl4_src(packed_bl4.data(), col_count, row_count),
      .src1 = make_q8_0_vector_src(q8_input.data(), col_count),
      .dst = make_dst(packed_out.data(), dtype::f32, 1u, row_count),
  };
  CHECK(machine.process_event(ev));
...
TEST_CASE("kernel_aarch64_q8_0_packed_bl8_matrix_x4_route_is_explicit_and_"
          "numeric_match") {
  using emel::kernel::detail::quant::block_q8_0;
  ...
  for (size_t row = 0; row < static_cast<size_t>(rhs_rows); ++row) {
    emel::kernel::detail::quant::quantize_row_q8_0_strided(
        rhs_dense.data() + row * static_cast<size_t>(col_count), 1u,
  ...
  for (size_t rhs_row = 0; rhs_row < static_cast<size_t>(rhs_rows); ++rhs_row) {
    for (size_t lhs_row = 0; lhs_row < static_cast<size_t>(row_count);
         ++lhs_row) {
  ...
  std::vector<float> packed_out(static_cast<size_t>(rhs_rows * row_count),
                                0.0f);
  aarch64_sm machine{};
  ...
  const emel::kernel::event::op_mul_mat ev{
      .src0 =
          make_packed_q8_0_x4_bl8_src(packed_bl8.data(), col_count, row_count),
      .src1 = make_packed_q8_0_x4_bl8_rhs_src(rhs_packed_bl8.data(), rhs_rows,
                                              col_count),
      .dst = make_batch_major_dst(packed_out.data(), dtype::f32, rhs_rows,
                                  row_count),
  };
  CHECK(machine.process_event(ev));
...
TEST_CASE("kernel_aarch64_q4_k_packed_bl8_matrix_x4_route_is_explicit_and_"
          "numeric_match") {
  using emel::kernel::detail::quant::block_q4_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q8_0_X4_ROWS;
  ...
  std::vector<block_q4_k> native_rows(
      static_cast<size_t>(row_count * block_count));
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    ...
    for (size_t idx = 0; idx < q4.scales.size(); ++idx) {
      q4.scales[idx] =
          static_cast<uint8_t>((idx * (11u + row)) ^ (0x5au + row));
    }
  ...
  std::vector<uint8_t> packed_bl8(
      ...
      emel::kernel::detail::quant::packed_q4_k_x8_group_count(row_count) *
      block_count);
  REQUIRE(emel::kernel::detail::quant::pack_q4_k_rows_x8_bl8(
      native_rows.data(), row_count, col_count, packed_bl8.data()));
  ...
  const emel::kernel::event::op_mul_mat ev{
      .src0 =
          make_packed_q4_k_x8_bl8_src(packed_bl8.data(), col_count, row_count),
...
TEST_CASE("kernel_aarch64_q4_k_packed_bl8_multi_block_matrix_x4_route_matches_"
          "scalar") {
  using emel::kernel::detail::quant::block_q4_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q4_K_X8_ROWS;
  ...
  constexpr uint64_t row_count = Q4_K_X8_ROWS * 2u;
  constexpr uint64_t rhs_rows = Q8_0_X4_ROWS;
  constexpr uint64_t col_count = QK_K * 10u;
  constexpr uint64_t block_count = col_count / QK_K;
  ...
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    for (size_t block = 0; block < static_cast<size_t>(block_count); ++block) {
      auto &q4 = native_rows[row * static_cast<size_t>(block_count) + block];
      q4.d = emel::kernel::detail::quant::fp32_to_fp16(
          0.0009765625f *
          static_cast<float>(((row + 5u) * (block + 3u)) % 53u + 1u));
      q4.dmin = emel::kernel::detail::quant::fp32_to_fp16(
          0.00048828125f *
          static_cast<float>(((row + 11u) * (block + 7u)) % 47u + 1u));
      for (size_t idx = 0; idx < q4.scales.size(); ++idx) {
        q4.scales[idx] = static_cast<uint8_t>(
            ((idx * (13u + row)) + block * 17u + 0x5au) & 0xffu);
      }
      for (size_t idx = 0; idx < q4.qs.size(); ++idx) {
        q4.qs[idx] = static_cast<uint8_t>(
            ((idx * (9u + row)) + block * 31u + 0x96u) & 0xffu);
      }
    }
  }
  ...
  for (size_t row = 0; row < static_cast<size_t>(rhs_rows); ++row) {
    for (size_t col = 0; col < static_cast<size_t>(col_count); ++col) {
      const int32_t centered =
          static_cast<int32_t>(
              ((row + 5u) * 19u + (col + 3u) * 7u + (col / QK_K) * 23u) %
              127u) -
          63;
      rhs_dense[row * static_cast<size_t>(col_count) + col] =
          static_cast<float>(centered) * 0.0078125f;
  ...
  for (size_t rhs_row = 0; rhs_row < static_cast<size_t>(rhs_rows); ++rhs_row) {
    for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
      reference[rhs_row * static_cast<size_t>(row_count) + row] =
          emel::kernel::detail::dot_q4_k_q8_k_row_scalar(
              native_rows.data() + row * static_cast<size_t>(block_count),
              rhs_q8.data() + rhs_row * static_cast<size_t>(block_count),
              block_count);
  ...
  for (size_t idx = 0; idx < packed_out.size(); ++idx) {
    max_abs_delta =
        std::max(max_abs_delta, std::fabs(packed_out[idx] - reference[idx]));
    CHECK(packed_out[idx] == doctest::Approx(reference[idx]).epsilon(1.0e-4f));
  }
  CHECK(max_abs_delta <= 1.0e-3f);
  CHECK(machine.optimized_q4_vector_packed_q8_rhs_dispatch_count() == 1u);
#else
  const emel::kernel::event::op_mul_mat ev{
      .src0 =
...
TEST_CASE("kernel_aarch64_q4_k_packed_bl8_multi_block_matrix_x8_route_matches_"
          "scalar") {
  using emel::kernel::detail::quant::block_q4_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q4_K_X8_ROWS;
...
TEST_CASE(
    "kernel_aarch64_q4_k_packed_bl8_routes_do_not_compile_bad_i8mm_tile") {
  const std::string actions_source = read_text_file(
      repo_root() / "src" / "emel" / "kernel" / "aarch64" / "actions.hpp");
  CHECK(actions_source.find("dot_q4_k_x8_q8_k_group_bl8_i8mm_x4") ==
        std::string::npos);
}

TEST_CASE("kernel_aarch64_q6_k_prepared_q8_rhs_i8mm_matrix_x4_route_is_"
          "explicit_and_numeric_match") {
  using emel::kernel::detail::quant::block_q6_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q8_0_X4_ROWS;
  using emel::kernel::detail::quant::QK_K;
  ...
  std::vector<block_q6_k> native_rows(
      static_cast<size_t>(row_count * block_count));
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    ...
    for (size_t idx = 0; idx < q6.scales.size(); ++idx) {
      const int32_t scale_value =
          static_cast<int32_t>((idx + row * 5u) % 15u) - 7;
  ...
  std::vector<float> prepared_out(static_cast<size_t>(rhs_rows * row_count),
                                  0.0f);
  aarch64_sm machine{};
    ...
    defined(__ARM_FEATURE_MATMUL_INT8)
  const emel::kernel::event::op_mul_mat ev{
      .src0 = make_prepared_q6_k_x8_q8_src(prepared_storage.data(), col_count,
                                           row_count),
      .src1 = make_q8_k_x4_rhs_src(rhs_q8.data(), rhs_rows, col_count),
      .dst = make_batch_major_dst(prepared_out.data(), dtype::f32, rhs_rows,
                                  row_count),
  };
  CHECK(machine.process_event(ev));
  for (size_t idx = 0; idx < prepared_out.size(); ++idx) {
    CHECK(prepared_out[idx] ==
          doctest::Approx(reference[idx]).epsilon(1.0e-5f));
  }
  CHECK(machine.optimized_q6_dispatch_count() == 1u);
  CHECK(machine.optimized_q6_vector_dispatch_count() == 1u);
  ...
  CHECK(machine.optimized_q6_vector_prepared_q8_rhs_i8mm_dispatch_count() ==
        1u);
#else
  const emel::kernel::event::op_mul_mat ev{
      .src0 = make_prepared_q6_k_x8_q8_src(prepared_storage.data(), col_count,
...
TEST_CASE("kernel_aarch64_q6_k_prepared_q8_rhs_i8mm_matrix_x8_route_is_"
          "explicit_and_numeric_match") {
  using emel::kernel::detail::quant::block_q6_k;
  using emel::kernel::detail::quant::block_q8_k;
  ...
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    ...
    for (size_t idx = 0; idx < q6.scales.size(); ++idx) {
      const int32_t scale_value =
          static_cast<int32_t>((idx + row * 7u) % 13u) - 6;
      q6.scales[idx] = static_cast<int8_t>(scale_value);
    }
    for (size_t idx = 0; idx < q6.ql.size(); ++idx) {
...
TEST_CASE("kernel_aarch64_q6_k_prepared_q8_rhs_i8mm_multi_block_vector_route_"
          "matches_scalar") {
  using emel::kernel::detail::quant::block_q6_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q6_K_X8_ROWS;
  ...
  constexpr uint64_t row_count = Q6_K_X8_ROWS * 2u;
  constexpr uint64_t col_count = QK_K * 10u;
  constexpr uint64_t block_count = col_count / QK_K;
  ...
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    for (size_t block = 0; block < static_cast<size_t>(block_count); ++block) {
      auto &q6 = native_rows[row * static_cast<size_t>(block_count) + block];
      q6.d = emel::kernel::detail::quant::fp32_to_fp16(
          0.0009765625f *
          static_cast<float>(((row + 13u) * (block + 7u)) % 61u + 1u));
      for (size_t idx = 0; idx < q6.scales.size(); ++idx) {
        const int32_t scale_value =
            static_cast<int32_t>(
                ((idx + 5u) * (row + 3u) + (block + 11u) * 17u) % 33u) -
            16;
        q6.scales[idx] = static_cast<int8_t>(scale_value);
      }
      for (size_t idx = 0; idx < q6.ql.size(); ++idx) {
        q6.ql[idx] = static_cast<uint8_t>(
            ((idx * (19u + row)) + (block * 37u) + 0x5du) & 0xffu);
      }
      for (size_t idx = 0; idx < q6.qh.size(); ++idx) {
        q6.qh[idx] = static_cast<uint8_t>(
            ((idx * (23u + block)) + (row * 29u) + 0xa3u) & 0xffu);
      }
    }
  }

  std::vector<float> rhs_dense(static_cast<size_t>(col_count), 0.0f);
  for (size_t col = 0; col < static_cast<size_t>(col_count); ++col) {
    const int32_t centered =
        static_cast<int32_t>(((col + 17u) * 41u + (col / QK_K) * 19u) % 251u) -
        125;
    rhs_dense[col] = static_cast<float>(centered) * 0.00390625f;
  }

  std::vector<block_q8_k> rhs_q8(static_cast<size_t>(block_count));
  emel::kernel::detail::quant::quantize_row_q8_k_strided(
      rhs_dense.data(), 1u, rhs_q8.data(), static_cast<int64_t>(col_count));

  std::vector<float> reference(static_cast<size_t>(row_count), 0.0f);
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    reference[row] = emel::kernel::detail::dot_q6_k_q8_k_row_scalar(
        native_rows.data() + row * static_cast<size_t>(block_count),
        rhs_q8.data(), block_count);
  }
  ...
  std::vector<float> prepared_out(static_cast<size_t>(row_count), 0.0f);
  aarch64_sm machine{};
  ...
  const emel::kernel::event::op_mul_mat ev{
      .src0 = make_prepared_q6_k_x8_q8_src(prepared_storage.data(), col_count,
                                           row_count),
      .src1 = make_q8_k_vector_src(rhs_q8.data(), col_count),
      .dst = make_dst(prepared_out.data(), dtype::f32, 1u, row_count),
  };
  CHECK(machine.process_event(ev));
  float max_abs_delta = 0.0f;
  for (size_t idx = 0; idx < prepared_out.size(); ++idx) {
    max_abs_delta =
        std::max(max_abs_delta, std::fabs(prepared_out[idx] - reference[idx]));
    CHECK(prepared_out[idx] ==
          doctest::Approx(reference[idx]).epsilon(1.0e-5f));
  }
  CHECK(max_abs_delta <= 1.0e-4f);
  CHECK(machine.optimized_q6_vector_prepared_q8_rhs_i8mm_dispatch_count() ==
        1u);
#else
  const emel::kernel::event::op_mul_mat ev{
...
TEST_CASE("kernel_aarch64_q6_k_prepared_q8_rhs_i8mm_multi_block_matrix_x4_"
          "matches_scalar") {
  using emel::kernel::detail::quant::block_q6_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q6_K_X8_ROWS;
  ...
  for (size_t row = 0; row < static_cast<size_t>(rhs_rows); ++row) {
    for (size_t col = 0; col < static_cast<size_t>(col_count); ++col) {
      const int32_t centered =
          static_cast<int32_t>(
              ((row + 19u) * 43u + (col + 17u) * 41u + (col / QK_K) * 19u) %
              251u) -
          125;
      rhs_dense[row * static_cast<size_t>(col_count) + col] =
          static_cast<float>(centered) * 0.00390625f;
...
TEST_CASE("kernel_aarch64_q4_k_packed_bl8_matrix_x8_row_sliced_lane_views_are_"
          "supported") {
  using emel::kernel::detail::quant::block_q4_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q4_K_X8_ROWS;
  ...
  constexpr uint64_t row_count = 16u;
  constexpr uint64_t slice_rows = Q4_K_X8_ROWS;
  constexpr uint64_t rhs_rows = Q4_K_X8_ROWS;
  constexpr uint64_t col_count = QK_K;
  constexpr uint64_t block_count = col_count / QK_K;
  ...
  for (size_t row = 0; row < static_cast<size_t>(row_count); ++row) {
    ...
    for (size_t idx = 0; idx < q4.scales.size(); ++idx) {
      q4.scales[idx] =
          static_cast<uint8_t>((idx * (13u + row)) ^ (0x66u - row));
    }
    for (size_t idx = 0; idx < q4.qs.size(); ++idx) {
  ...
  const auto lane0 =
      make_row_sliced_lane_event(full_ev, Q4_K_X8_ROWS, 0u, slice_rows);
  const auto lane1 = make_row_sliced_lane_event(
      full_ev, Q4_K_X8_ROWS, slice_rows, row_count - slice_rows);

#if defined(__aarch64__) && defined(__ARM_NEON) &&                             \
    defined(__ARM_FEATURE_DOTPROD)
  CHECK(machine.process_event(lane0));
  CHECK(machine.process_event(lane1));
  for (size_t idx = 0; idx < packed_out.size(); ++idx) {
    CHECK(packed_out[idx] == doctest::Approx(reference[idx]).epsilon(1.0e-4f));
  }
  CHECK(machine.optimized_q4_vector_packed_q8_rhs_dispatch_count() == 2u);
#else
  CHECK_FALSE(machine.process_event(lane0));
  CHECK_FALSE(machine.process_event(lane1));
#endif
}

TEST_CASE("kernel_aarch64_q6_k_prepared_q8_rhs_i8mm_matrix_x8_row_sliced_lane_"
          "views_are_supported") {
  using emel::kernel::detail::quant::block_q6_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q6_K_X8_ROWS;
  ...
  constexpr uint64_t row_count = 16u;
  constexpr uint64_t slice_rows = Q6_K_X8_ROWS;
  constexpr uint64_t rhs_rows = Q6_K_X8_ROWS;
  constexpr uint64_t col_count = QK_K;
  constexpr uint64_t block_count = col_count / QK_K;
  ...
  const auto lane0 =
      make_row_sliced_lane_event(full_ev, Q6_K_X8_ROWS, 0u, slice_rows);
  const auto lane1 = make_row_sliced_lane_event(
      full_ev, Q6_K_X8_ROWS, slice_rows, row_count - slice_rows);
...
TEST_CASE("kernel_aarch64_q8_0_packed_bl8_matrix_x4_row_sliced_lane_views_are_"
          "supported") {
  using emel::kernel::detail::quant::block_q8_0;
  using emel::kernel::detail::quant::Q8_0_X4_ROWS;
  using emel::kernel::detail::quant::QK8_0;
  ...
  constexpr uint64_t row_count = 8u;
  constexpr uint64_t slice_rows = Q8_0_X4_ROWS;
  constexpr uint64_t rhs_rows = Q8_0_X4_ROWS;
  constexpr uint64_t col_count = QK8_0 * 8u;
  constexpr uint64_t block_count = col_count / QK8_0;
  ...
  const auto lane0 =
      make_row_sliced_lane_event(full_ev, Q8_0_X4_ROWS, 0u, slice_rows);
  const auto lane1 = make_row_sliced_lane_event(
      full_ev, Q8_0_X4_ROWS, slice_rows, row_count - slice_rows);
  ...
  CHECK(machine.optimized_q8_0_packed_bl8_matrix_x4_dispatch_count() == 2u);
#else
  CHECK_FALSE(machine.process_event(lane0));
  CHECK_FALSE(machine.process_event(lane1));
#endif
...
TEST_CASE("kernel_aarch64_q4_k_packed_bl8_matrix_x4_row_sliced_lane_views_are_"
          "supported") {
  using emel::kernel::detail::quant::block_q4_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q4_K_X8_ROWS;
...
TEST_CASE("kernel_aarch64_q6_k_prepared_q8_rhs_i8mm_matrix_x4_row_sliced_lane_"
          "views_are_supported") {
  using emel::kernel::detail::quant::block_q6_k;
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::Q6_K_X8_ROWS;
...
TEST_CASE("kernel_aarch64_q8_0_vector_route_is_explicit_and_numeric_match") {
  ...
  for (size_t row = 0; row < optimized_out.size(); ++row) {
    CHECK(optimized_out[row] ==
          doctest::Approx(scalar_out[row]).epsilon(1.0e-6f));
  }
...
TEST_CASE("kernel_aarch64_q6_packed_vector_argmax_route_is_explicit_and_"
          "numeric_match") {
#if !(defined(__aarch64__) && defined(__ARM_NEON) &&                           \
      defined(__ARM_FEATURE_DOTPROD))
  CHECK(true);
  ...
  const emel::kernel::event::op_mul_mat_argmax argmax_prepared_ev{
      .src0 = make_argmax_prepared_q6_k_x8_q8_src(
          argmax_prepared_storage.data(), QK_K, k_rows),
      .src1 = make_q8_k_vector_src(q8_input.data(), QK_K),
  ...
  CHECK(argmax_prepared_score ==
        doctest::Approx(expected_score).epsilon(1.0e-6f));
  ...
  CHECK(packed_machine.optimized_q6_vector_packed_dispatch_count() == 1u);
  CHECK(packed_machine
            .optimized_q6_vector_packed_q8_rhs_argmax_dispatch_count() == 1u);
  CHECK(packed_machine.shared_q6_dispatch_count() == 0u);
#if defined(__ARM_FEATURE_MATMUL_INT8)
  ...
  CHECK(argmax_prepared_machine.optimized_q6_vector_argmax_dispatch_count() ==
        1u);
  CHECK(argmax_prepared_machine
  ...
  CHECK(packed_machine.optimized_q6_vector_packed_dispatch_count() == 0u);
  CHECK(packed_machine
            .optimized_q6_vector_packed_q8_rhs_argmax_dispatch_count() == 0u);
  CHECK(packed_machine.shared_q6_dispatch_count() == 0u);
  CHECK(argmax_prepared_machine.optimized_q6_dispatch_count() == 0u);
  CHECK(argmax_prepared_machine.optimized_q6_vector_dispatch_count() == 0u);
  CHECK(argmax_prepared_machine.optimized_q6_vector_argmax_dispatch_count() ==
...
TEST_CASE("kernel_aarch64_detail_branch_paths") {
...
#else
  CHECK_FALSE(
      emel::kernel::aarch64::detail::can_use_neon(quant_mul_mat_ev, true));
#endif
  ...
  using emel::kernel::detail::quant::block_q8_k;
  using emel::kernel::detail::quant::QK8_0;
  std::array<block_q4_k, 1> q4_k_rows = {};
  std::array<block_q6_k, 1> q6_k_rows = {};
  std::array<block_q8_k, 1> q8_k_rows = {};
  std::array<block_q8_0, 1> q8_0_rows = {};
  std::array<float, 1> vector_out = {};
  const emel::kernel::event::op_mul_mat q4_q8_ev{
      .src0 = make_quantized_src(q4_k_rows.data(), dtype::q4_k, QK_K, 1u),
      .src1 = make_q8_k_vector_src(q8_k_rows.data(), QK_K),
      .dst = make_dst(vector_out.data(), dtype::f32, 1u, 1u),
  };
  CHECK(emel::kernel::aarch64::detail::
            can_run_neon_mul_mat_q4_vector_q8_rhs_request(q4_q8_ev));
  {
    auto bad = q4_q8_ev;
    bad.src1.ne[0] = 2u;
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q4_vector_q8_rhs_request(bad));
  }
  {
    auto bad = q4_q8_ev;
    bad.dst.ne[1] = 2u;
  ...
  {
    auto bad = q4_q8_ev;
    bad.src0.type = dtype::q6_k;
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q4_vector_q8_rhs_request(bad));
  }
  {
    auto bad = q4_q8_ev;
    bad.src1.nb[1] = bad.src1.nb[1] + 1u;
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q4_vector_q8_rhs_request(bad));
  }
  const auto reject_q4_q8 = [&](const auto mutate) {
    auto bad = q4_q8_ev;
    mutate(bad);
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q4_vector_q8_rhs_request(bad));
  };
  reject_q4_q8([](auto &bad) { bad.src0.ne[0] = 0u; });
  reject_q4_q8([](auto &bad) { bad.src0.ne[1] = 0u; });
  reject_q4_q8([](auto &bad) { bad.dst.ne[0] = 2u; });
  reject_q4_q8([](auto &bad) { bad.src0.ne[2] = 2u; });
  reject_q4_q8([](auto &bad) { bad.src0.ne[3] = 2u; });
  reject_q4_q8([](auto &bad) { bad.src1.ne[2] = 2u; });
  reject_q4_q8([](auto &bad) { bad.src1.ne[3] = 2u; });
  reject_q4_q8([](auto &bad) { bad.dst.ne[2] = 2u; });
  reject_q4_q8([](auto &bad) { bad.dst.ne[3] = 2u; });
  reject_q4_q8([](auto &bad) { bad.src1.type = dtype::f32; });
  reject_q4_q8([](auto &bad) { bad.dst.type = dtype::q4_k; });
  reject_q4_q8([](auto &bad) { bad.src0.nb[0] = 2u; });
  reject_q4_q8([](auto &bad) { bad.src0.nb[1] = bad.src0.nb[1] + 1u; });
  reject_q4_q8([](auto &bad) { bad.src0.nb[2] = bad.src0.nb[2] + 1u; });
  reject_q4_q8([](auto &bad) { bad.src0.nb[3] = bad.src0.nb[3] + 1u; });
  reject_q4_q8([](auto &bad) { bad.src1.nb[0] = 2u; });
  reject_q4_q8([](auto &bad) { bad.src1.nb[2] = bad.src1.nb[2] + 1u; });
  reject_q4_q8([](auto &bad) { bad.src1.nb[3] = bad.src1.nb[3] + 1u; });

  const emel::kernel::event::op_mul_mat q6_q8_ev{
      .src0 = make_quantized_src(q6_k_rows.data(), dtype::q6_k, QK_K, 1u),
      .src1 = make_q8_k_vector_src(q8_k_rows.data(), QK_K),
      .dst = make_dst(vector_out.data(), dtype::f32, 1u, 1u),
  };
  CHECK(emel::kernel::aarch64::detail::
            can_run_neon_mul_mat_q6_vector_q8_rhs_request(q6_q8_ev));
  {
    auto bad = q6_q8_ev;
    bad.src1.ne[1] = QK_K + 1u;
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q6_vector_q8_rhs_request(bad));
  }
  const auto reject_q6_q8 = [&](const auto mutate) {
    auto bad = q6_q8_ev;
    mutate(bad);
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q6_vector_q8_rhs_request(bad));
  };
  reject_q6_q8([](auto &bad) { bad.src0.ne[0] = 0u; });
  reject_q6_q8([](auto &bad) { bad.src0.ne[1] = 0u; });
  reject_q6_q8([](auto &bad) { bad.src1.ne[0] = 2u; });
  reject_q6_q8([](auto &bad) { bad.dst.ne[0] = 2u; });
  reject_q6_q8([](auto &bad) { bad.dst.ne[1] = 2u; });
  reject_q6_q8([](auto &bad) { bad.src0.ne[2] = 2u; });
  reject_q6_q8([](auto &bad) { bad.src0.ne[3] = 2u; });
  reject_q6_q8([](auto &bad) { bad.src1.ne[2] = 2u; });
  reject_q6_q8([](auto &bad) { bad.src1.ne[3] = 2u; });
  reject_q6_q8([](auto &bad) { bad.dst.ne[2] = 2u; });
  reject_q6_q8([](auto &bad) { bad.dst.ne[3] = 2u; });
  reject_q6_q8([](auto &bad) { bad.src0.type = dtype::q4_k; });
  reject_q6_q8([](auto &bad) { bad.src1.type = dtype::f32; });
  reject_q6_q8([](auto &bad) { bad.dst.type = dtype::q6_k; });
  reject_q6_q8([](auto &bad) { bad.src0.nb[0] = 2u; });
  reject_q6_q8([](auto &bad) { bad.src0.nb[1] = bad.src0.nb[1] + 1u; });
  reject_q6_q8([](auto &bad) { bad.src0.nb[3] = bad.src0.nb[3] + 1u; });
  reject_q6_q8([](auto &bad) { bad.src1.nb[0] = 2u; });
  reject_q6_q8([](auto &bad) { bad.src1.nb[2] = bad.src1.nb[2] + 1u; });
  reject_q6_q8([](auto &bad) { bad.src1.nb[3] = bad.src1.nb[3] + 1u; });
  {
    auto bad = q6_q8_ev;
    bad.src0.nb[2] = bad.src0.nb[2] + 1u;
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q6_vector_q8_rhs_request(bad));
  }

  const emel::kernel::event::op_mul_mat q8_0_ev{
      .src0 = make_quantized_src(q8_0_rows.data(), dtype::q8_0, QK8_0, 1u),
      .src1 = make_q8_0_vector_src(q8_0_rows.data(), QK8_0),
      .dst = make_dst(vector_out.data(), dtype::f32, 1u, 1u),
  };
  CHECK(emel::kernel::aarch64::detail::
            can_run_neon_mul_mat_q8_0_vector_q8_rhs_request(q8_0_ev));
  {
    auto bad = q8_0_ev;
    bad.src1.type = dtype::f32;
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q8_0_vector_q8_rhs_request(bad));
  }
  {
    auto bad = q8_0_ev;
    bad.dst.nb[0] = sizeof(float) * 2u;
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q8_0_vector_q8_rhs_request(bad));
  }
  const auto reject_q8_0 = [&](const auto mutate) {
    auto bad = q8_0_ev;
    mutate(bad);
    CHECK_FALSE(emel::kernel::aarch64::detail::
                    can_run_neon_mul_mat_q8_0_vector_q8_rhs_request(bad));
  };
  reject_q8_0([](auto &bad) { bad.src0.ne[0] = 0u; });
  reject_q8_0([](auto &bad) { bad.src0.ne[1] = 0u; });
  reject_q8_0([](auto &bad) { bad.src1.ne[0] = 2u; });
  reject_q8_0([](auto &bad) { bad.src1.ne[1] = QK8_0 + 1u; });
  reject_q8_0([](auto &bad) { bad.dst.ne[0] = 2u; });
  reject_q8_0([](auto &bad) { bad.dst.ne[1] = 2u; });
  reject_q8_0([](auto &bad) { bad.src0.ne[2] = 2u; });
  reject_q8_0([](auto &bad) { bad.src1.ne[2] = 2u; });
  reject_q8_0([](auto &bad) { bad.dst.ne[2] = 2u; });
  reject_q8_0([](auto &bad) { bad.dst.ne[3] = 2u; });
  reject_q8_0([](auto &bad) { bad.src0.type = dtype::q4_k; });
  reject_q8_0([](auto &bad) { bad.dst.type = dtype::q8_0; });
  reject_q8_0([](auto &bad) { bad.src0.nb[0] = 2u; });
  reject_q8_0([](auto &bad) { bad.src0.nb[1] = bad.src0.nb[1] + 1u; });
  reject_q8_0([](auto &bad) { bad.src0.nb[2] = bad.src0.nb[2] + 1u; });
  reject_q8_0([](auto &bad) { bad.src0.nb[3] = bad.src0.nb[3] + 1u; });
  reject_q8_0([](auto &bad) { bad.src1.nb[0] = 2u; });
  reject_q8_0([](auto &bad) { bad.src1.nb[1] = bad.src1.nb[1] + 1u; });
  reject_q8_0([](auto &bad) { bad.src1.nb[2] = bad.src1.nb[2] + 1u; });
  reject_q8_0([](auto &bad) { bad.src1.nb[3] = bad.src1.nb[3] + 1u; });

  std::array<float, 4> f32_matrix = {1.0f, 2.0f, 3.0f, 4.0f};
  const emel::kernel::event::op_mul_mat f32_ev{
      .src0 = make_src(f32_matrix.data(), dtype::f32, 4u, 1u),
      .src1 = make_src(lhs, dtype::f32, 1u, 4u),
      .dst = make_dst(vector_out.data(), dtype::f32, 1u, 1u),
  };
  CHECK(emel::kernel::aarch64::detail::can_run_neon_mul_mat_f32_vector_request(
      f32_ev));
  {
    auto bad = f32_ev;
    bad.src1.ne[0] = 2u;
    CHECK_FALSE(
        emel::kernel::aarch64::detail::can_run_neon_mul_mat_f32_vector_request(
            bad));
  }
  {
    auto bad = f32_ev;
    bad.src0.nb[1] = bad.src0.nb[1] + sizeof(float);
    CHECK_FALSE(
        emel::kernel::aarch64::detail::can_run_neon_mul_mat_f32_vector_request(
            bad));
  }
  const auto reject_f32 = [&](const auto mutate) {
    auto bad = f32_ev;
    mutate(bad);
    CHECK_FALSE(
        emel::kernel::aarch64::detail::can_run_neon_mul_mat_f32_vector_request(
  ...
  reject_f32([](auto &bad) { bad.src0.ne[0] = 0u; });
  reject_f32([](auto &bad) { bad.src0.ne[1] = 0u; });
  reject_f32([](auto &bad) { bad.src1.ne[1] = 5u; });
  reject_f32([](auto &bad) { bad.dst.ne[0] = 2u; });
  reject_f32([](auto &bad) { bad.dst.ne[1] = 2u; });
  reject_f32([](auto &bad) { bad.src0.ne[2] = 2u; });
  reject_f32([](auto &bad) { bad.src0.ne[3] = 2u; });
  reject_f32([](auto &bad) { bad.src1.ne[2] = 2u; });
  reject_f32([](auto &bad) { bad.src1.ne[3] = 2u; });
  reject_f32([](auto &bad) { bad.dst.ne[2] = 2u; });
  reject_f32([](auto &bad) { bad.dst.ne[3] = 2u; });
  reject_f32([](auto &bad) { bad.src0.type = dtype::q4_k; });
  reject_f32([](auto &bad) { bad.src1.type = dtype::q4_k; });
  reject_f32([](auto &bad) { bad.dst.type = dtype::q4_k; });
  reject_f32([](auto &bad) { bad.src0.nb[0] = sizeof(float) * 2u; });
  reject_f32(
      [](auto &bad) { bad.src0.nb[2] = bad.src0.nb[2] + sizeof(float); });
  reject_f32(
      [](auto &bad) { bad.src0.nb[3] = bad.src0.nb[3] + sizeof(float); });
...
TEST_CASE("kernel_aarch64_q4_k_vector_q8_rhs_tail_rows_match_one_row") {
  ...
  constexpr uint64_t ROW_COUNT = 5u;
  constexpr uint64_t BLOCK_COUNT = 7u;
  constexpr uint64_t COL_COUNT = QK_K * BLOCK_COUNT;

  std::array<block_q4_k, ROW_COUNT * BLOCK_COUNT> native_rows = {};
  for (size_t row = 0; row < ROW_COUNT; ++row) {
    for (size_t block = 0; block < BLOCK_COUNT; ++block) {
      auto &q4 = native_rows[row * BLOCK_COUNT + block];
      q4.d = emel::kernel::detail::quant::fp32_to_fp16(
          0.0009765625f * static_cast<float>((row + block) % 31u + 1u));
      q4.dmin = emel::kernel::detail::quant::fp32_to_fp16(
          0.000244140625f * static_cast<float>((row + block * 3u) % 29u + 1u));
      for (size_t idx = 0; idx < q4.scales.size(); ++idx) {
        q4.scales[idx] =
            static_cast<uint8_t>((row * 11u + block * 17u + idx * 7u) & 0xffu);
      }
      for (size_t idx = 0; idx < q4.qs.size(); ++idx) {
        q4.qs[idx] =
            static_cast<uint8_t>((row * 13u + block * 19u + idx * 5u) & 0xffu);
      }
    }
  }

  std::array<float, COL_COUNT> rhs_dense = {};
  for (size_t idx = 0; idx < rhs_dense.size(); ++idx) {
    rhs_dense[idx] =
        static_cast<float>(static_cast<int32_t>((idx * 17u) % 127u) - 63) *
        0.0078125f;
  }
  std::array<block_q8_k, BLOCK_COUNT> rhs_q8 = {};
  emel::kernel::detail::quant::quantize_row_q8_k_strided(
      rhs_dense.data(), 1u, rhs_q8.data(), static_cast<int64_t>(COL_COUNT));

  std::array<float, ROW_COUNT> reference = {};
  for (size_t row = 0; row < ROW_COUNT; ++row) {
    reference[row] = emel::kernel::aarch64::detail::dot_q4_k_q8_k_row_neon(
        native_rows.data() + row * BLOCK_COUNT, rhs_q8.data(), BLOCK_COUNT);
  }

  std::array<float, ROW_COUNT> out = {};
  const emel::kernel::event::op_mul_mat ev{
      .src0 = make_quantized_src(native_rows.data(), dtype::q4_k, COL_COUNT,
                                 ROW_COUNT),
      .src1 = make_q8_k_vector_src(rhs_q8.data(), COL_COUNT),
      .dst = make_dst(out.data(), dtype::f32, 1u, ROW_COUNT),
  };
  ...
  CHECK(allocations.allocations() == 0u);
  for (size_t idx = 0; idx < out.size(); ++idx) {
    CHECK(std::bit_cast<uint32_t>(out[idx]) ==
          std::bit_cast<uint32_t>(reference[idx]));
  }
#else
  aarch64_sm machine{};
  CHECK_FALSE(machine.process_event(ev));
...
TEST_CASE("kernel_aarch64_flash_attn_ext_reuses_persistent_workspace") {
  ...
  const emel::kernel::aarch64::event::dispatch_op_flash_attn_ext dispatch0{
      request, dispatch_ctx0};
  const float *scratch = ctx.flash_attn_workspace.score_buffer.data();

  emel::kernel::aarch64::action::exec_simd_op_flash_attn_ext_f16kv_one_chunk(
      dispatch0, ctx);
  CHECK(dispatch_ctx0.outcome ==
        emel::kernel::aarch64::events::phase_outcome::done);
  ...
  const emel::kernel::aarch64::event::dispatch_op_flash_attn_ext dispatch1{
      request, dispatch_ctx1};

  emel::kernel::aarch64::action::exec_simd_op_flash_attn_ext_f16kv_one_chunk(
      dispatch1, ctx);
  CHECK(dispatch_ctx1.outcome ==
        emel::kernel::aarch64::events::phase_outcome::done);
...
TEST_CASE("kernel_aarch64_flash_attn_ext_uses_optimized_backend_path") {
  ...
  emel::kernel::aarch64::action::context ctx{};
  emel::kernel::aarch64::event::dispatch_ctx dispatch_ctx{};
  const emel::kernel::aarch64::event::dispatch_op_flash_attn_ext dispatch{
      request, dispatch_ctx};
  ...
  const std::vector<float> expected = flash_attn_reference_f16_scores(
      ...
      std::span<const uint16_t>(fixture.v, 8u), 4u, 2u, 1.0f);
...
TEST_CASE(
    "kernel_aarch64_flash_attn_ext_matches_shared_workspace_on_long_kv_spans") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  ...
  request.dst =
      make_dst(dst_shared.data(), dtype::f32, head_dim, 1u, head_count);
  REQUIRE(emel::kernel::detail::run_flash_attn_ext_with_workspace(
      request, shared_workspace));
...
TEST_CASE("kernel_aarch64_flash_attn_ext_matches_online_softmax_f16_reference_"
          "on_long_multihead_kv") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  constexpr uint64_t head_dim = 64u;
  ...
  shared_request.dst =
      make_dst(shared_dst.data(), dtype::f32, head_dim, 1u, head_count);
...
TEST_CASE(
    "kernel_aarch64_flash_attn_ext_matches_masked_total_token_reference") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  constexpr uint64_t head_dim = 64u;
  ...
  request.src1 =
      make_src(k_fp16.data(), dtype::f16, head_dim, kv_tokens, 1u, 1u);
  request.src2 =
      make_src(v_fp16.data(), dtype::f16, head_dim, kv_tokens, 1u, 1u);
  ...
  auto shared_request = request;
  shared_request.dst =
      make_dst(shared_dst.data(), dtype::f32, head_dim, 1u, 1u, 1u);
  ...
  const std::vector<float> expected = flash_attn_reference_masked_total_tokens(
      ...
      std::span<const uint16_t>(v_fp16.data(), v_fp16.size()), head_dim,
      kv_tokens, total_tokens, scale);
  for (uint64_t dim = 0; dim < head_dim; ++dim) {
    CHECK(
        within_flash_online_f16_tolerance(neon_dst[static_cast<size_t>(dim)],
                                          expected[static_cast<size_t>(dim)]));
    CHECK(
        within_flash_online_f16_tolerance(shared_dst[static_cast<size_t>(dim)],
                                          expected[static_cast<size_t>(dim)]));
  }
#endif
}

TEST_CASE("kernel_aarch64_flash_attn_ext_matches_masked_total_token_reference_"
          "on_long_multihead_kv") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  ...
  for (uint64_t head = 0; head < head_count; ++head) {
    ...
    const auto expected_head = flash_attn_reference_masked_total_tokens(
        std::span<const float>(
            q.data() + static_cast<std::ptrdiff_t>(head * head_dim), head_dim),
...
TEST_CASE("kernel_aarch64_flash_attn_ext_does_not_materialize_masked_tail_into_"
  ...
  constexpr uint32_t total_tokens = static_cast<uint32_t>(
      emel::kernel::detail::flash_attn_workspace_token_capacity + 1024u);
  ...
  const std::vector<float> expected = flash_attn_reference_f16_scores(
      std::span<const float>(fixture.q, 4u),
      std::span<const uint16_t>(fixture.k, 8u),
      std::span<const uint16_t>(fixture.v, 8u), 4u, 2u, scale);
  ...
  for (uint64_t dim = 0; dim < 4u; ++dim) {
    CHECK(
        within_flash_online_f16_tolerance(fixture.dst[static_cast<size_t>(dim)],
                                          expected[static_cast<size_t>(dim)]));
    CHECK(
        within_flash_online_f16_tolerance(shared_dst[static_cast<size_t>(dim)],
                                          expected[static_cast<size_t>(dim)]));
...
TEST_CASE(
    "kernel_aarch64_flash_attn_ext_matches_online_softmax_f16_reference") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  ...
  auto shared_request = request;
  shared_request.dst =
      make_dst(shared_dst.data(), dtype::f32, head_dim, 1, 1, 1);
