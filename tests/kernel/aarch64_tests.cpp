...
TEST_CASE("kernel_aarch64_flash_attn_ext_uses_optimized_backend_path") {
  ...
  const std::vector<float> expected = flash_attn_reference_rounded_weight_float_values(
...
TEST_CASE("kernel_aarch64_flash_attn_ext_matches_rounded_weight_float_value_reference_on_long_multihead_kv") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  constexpr uint64_t head_dim = 64u;
...
TEST_CASE("kernel_aarch64_flash_attn_ext_matches_rounded_weight_float_value_reference") {
#if !(defined(__aarch64__) || defined(__ARM_NEON))
  return;
#else
  ...
  const std::vector<float> expected =
      flash_attn_reference_rounded_weight_float_values(q, k, v, head_dim, kv_tokens, scale);
  for (uint64_t dim = 0; dim < head_dim; ++dim) {
