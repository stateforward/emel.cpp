#include <array>
#include <boost/sml.hpp>
#include <cstdint>
#include <memory>
#include <doctest/doctest.h>

#include "emel/emel.h"
#include "emel/memory/kv/actions.hpp"
#include "emel/memory/kv/guards.hpp"
#include "emel/memory/kv/sm.hpp"

TEST_CASE("kv_cache_starts_initialized") {
  emel::memory::kv::sm machine{};
}

TEST_CASE("kv_cache_prepare_plans_slots_and_apply_reports_progressive_n_kv") {
  emel::memory::kv::sm machine{};
  std::array<int32_t, 3> ubatch_sizes = {{2, 2, 1}};
  std::array<int32_t, 8> slot_offsets = {{0, 0, 0, 0, 0, 0, 0, 0}};
  int32_t ubatch_count = 0;
  int32_t kv_tokens = 0;

  CHECK(machine.process_event(emel::memory::kv::event::prepare{
    .ubatch_sizes = ubatch_sizes.data(),
    .ubatch_count = static_cast<int32_t>(ubatch_sizes.size()),
    .requested_capacity = 16,
    .slot_offsets_out = slot_offsets.data(),
    .slot_offsets_capacity = static_cast<int32_t>(slot_offsets.size()),
    .ubatch_count_out = &ubatch_count,
  }));

  CHECK(ubatch_count == 3);
  CHECK(slot_offsets[0] == 0);
  CHECK(slot_offsets[1] == 2);
  CHECK(slot_offsets[2] == 4);

  CHECK(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 0,
    .kv_tokens_out = &kv_tokens,
  }));
  CHECK(kv_tokens == 16);

  CHECK(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 1,
    .kv_tokens_out = &kv_tokens,
  }));
  CHECK(kv_tokens == 16);

  CHECK(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 2,
    .kv_tokens_out = &kv_tokens,
  }));
  CHECK(kv_tokens == 16);
}

TEST_CASE("kv_cache_prepare_fails_when_no_contiguous_slot_fits") {
  emel::memory::kv::sm machine{};
  std::array<int32_t, 2> ubatch_sizes = {{3, 3}};

  CHECK_FALSE(machine.process_event(emel::memory::kv::event::prepare{
    .ubatch_sizes = ubatch_sizes.data(),
    .ubatch_count = static_cast<int32_t>(ubatch_sizes.size()),
    .requested_capacity = 4,
  }));
}

TEST_CASE("kv_cache_apply_requires_sequential_ubatch_order") {
  emel::memory::kv::sm machine{};
  std::array<int32_t, 3> ubatch_sizes = {{2, 2, 1}};
  int32_t kv_tokens = 0;

  CHECK(machine.process_event(emel::memory::kv::event::prepare{
    .ubatch_sizes = ubatch_sizes.data(),
    .ubatch_count = static_cast<int32_t>(ubatch_sizes.size()),
    .requested_capacity = 16,
  }));

  CHECK_FALSE(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 1,
    .kv_tokens_out = &kv_tokens,
  }));
  CHECK(kv_tokens == 0);
}

TEST_CASE("kv_cache_rollback_clears_planned_ranges_from_index") {
  emel::memory::kv::sm machine{};
  std::array<int32_t, 3> ubatch_sizes = {{2, 2, 1}};
  int32_t kv_tokens = 0;

  CHECK(machine.process_event(emel::memory::kv::event::prepare{
    .ubatch_sizes = ubatch_sizes.data(),
    .ubatch_count = static_cast<int32_t>(ubatch_sizes.size()),
    .requested_capacity = 16,
  }));

  CHECK(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 0,
    .kv_tokens_out = &kv_tokens,
  }));
  CHECK(kv_tokens == 16);

  CHECK(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 1,
    .kv_tokens_out = &kv_tokens,
  }));
  CHECK(kv_tokens == 16);

  CHECK(machine.process_event(emel::memory::kv::event::rollback{
    .from_ubatch_index = 0,
  }));

  CHECK(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 0,
    .kv_tokens_out = &kv_tokens,
  }));
  CHECK(kv_tokens == 16);
}

TEST_CASE("kv_cache_reports_validation_errors") {
  emel::memory::kv::sm machine{};
  std::array<int32_t, 2> ubatch_sizes = {{1, 1}};

  CHECK_FALSE(machine.process_event(emel::memory::kv::event::prepare{
    .ubatch_sizes = ubatch_sizes.data(),
    .ubatch_count = 0,
    .requested_capacity = 16,
  }));

  CHECK(machine.process_event(emel::memory::kv::event::prepare{
    .ubatch_sizes = ubatch_sizes.data(),
    .ubatch_count = static_cast<int32_t>(ubatch_sizes.size()),
    .requested_capacity = 16,
  }));

  CHECK_FALSE(machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = -1,
  }));

  CHECK_FALSE(machine.process_event(emel::memory::kv::event::rollback{
    .from_ubatch_index = 3,
  }));
}

TEST_CASE("kv_cache_action_helpers_cover_validate_branches") {
  using emel::memory::kv::action::MAX_KV_CELLS;
  using emel::memory::kv::action::context;

  auto ctx_storage = std::make_unique<context>();
  context & ctx = *ctx_storage;
  int32_t error_out = EMEL_OK;

  std::array<int32_t, 1> sizes = {{1}};
  emel::memory::kv::event::prepare prepare{
    .ubatch_sizes = sizes.data(),
    .ubatch_count = 0,
    .requested_capacity = 4,
  };
  emel::memory::kv::event::validate_prepare validate_prepare{
    .request = &prepare,
    .error_out = &error_out,
  };
  emel::memory::kv::action::begin_prepare(prepare, ctx);
  CHECK_FALSE(emel::memory::kv::guard::valid_prepare_request(validate_prepare, ctx));
  emel::memory::kv::action::reject_invalid_prepare(validate_prepare, ctx);
  CHECK(error_out == EMEL_ERR_INVALID_ARGUMENT);

  prepare.ubatch_count = 1;
  prepare.requested_capacity = MAX_KV_CELLS + 1;
  error_out = EMEL_OK;
  emel::memory::kv::action::begin_prepare(prepare, ctx);
  CHECK_FALSE(emel::memory::kv::guard::valid_prepare_request(validate_prepare, ctx));
  emel::memory::kv::action::reject_invalid_prepare(validate_prepare, ctx);
  CHECK(error_out == EMEL_ERR_INVALID_ARGUMENT);

  prepare.requested_capacity = 8;
  error_out = EMEL_OK;
  emel::memory::kv::action::begin_prepare(prepare, ctx);
  CHECK(emel::memory::kv::guard::valid_prepare_request(validate_prepare, ctx));

  emel::memory::kv::event::apply_ubatch apply{};
  emel::memory::kv::event::validate_apply validate_apply{
    .request = &apply,
    .error_out = &error_out,
  };
  ctx.planned_ubatch_count = 0;
  ctx.applied_ubatches = 0;
  CHECK_FALSE(emel::memory::kv::guard::valid_apply_request(validate_apply, ctx));
  emel::memory::kv::action::reject_invalid_apply(validate_apply, ctx);
  CHECK(error_out == EMEL_ERR_INVALID_ARGUMENT);

  ctx.planned_ubatch_count = 2;
  apply.ubatch_index = 2;
  error_out = EMEL_OK;
  CHECK_FALSE(emel::memory::kv::guard::valid_apply_request(validate_apply, ctx));
  emel::memory::kv::action::reject_invalid_apply(validate_apply, ctx);
  CHECK(error_out == EMEL_ERR_INVALID_ARGUMENT);

  apply.ubatch_index = 1;
  ctx.applied_ubatches = 0;
  error_out = EMEL_OK;
  CHECK_FALSE(emel::memory::kv::guard::valid_apply_request(validate_apply, ctx));
  emel::memory::kv::action::reject_invalid_apply(validate_apply, ctx);
  CHECK(error_out == EMEL_ERR_INVALID_ARGUMENT);

  apply.ubatch_index = 0;
  ctx.applied_ubatches = 0;
  error_out = EMEL_OK;
  CHECK(emel::memory::kv::guard::valid_apply_request(validate_apply, ctx));

  emel::memory::kv::event::rollback rollback{};
  emel::memory::kv::event::validate_rollback validate_rollback{
    .request = &rollback,
    .error_out = &error_out,
  };
  rollback.from_ubatch_index = 3;
  ctx.applied_ubatches = 2;
  ctx.planned_ubatch_count = 2;
  error_out = EMEL_OK;
  CHECK_FALSE(emel::memory::kv::guard::valid_rollback_request(validate_rollback, ctx));
  emel::memory::kv::action::reject_invalid_rollback(validate_rollback, ctx);
  CHECK(error_out == EMEL_ERR_INVALID_ARGUMENT);
}

TEST_CASE("kv_cache_action_helpers_cover_slot_planning_apply_rollback_and_publish_edges") {
  using emel::memory::kv::action::context;

  auto ctx_storage = std::make_unique<context>();
  context & ctx = *ctx_storage;
  int32_t error_out = EMEL_OK;

  ctx.kv_size = 4;
  ctx.n_stream = 1;
  ctx.streams[0].head = 4;
  emel::memory::kv::action::begin_prepare(
      emel::memory::kv::event::prepare{
          .ubatch_sizes = nullptr,
          .ubatch_count = 1,
          .requested_capacity = 4,
      },
      ctx);
  CHECK(ctx.streams[0].head == 0);

  ctx.ubatch_count = 1;
  ctx.ubatch_sizes[0] = 2;
  ctx.ubatch_stream_ids[0] = 0;
  ctx.ubatch_seq_ids[0] = 0;
  ctx.streams[0].pos.fill(emel::memory::kv::action::POS_NONE);
  emel::memory::kv::action::set_cell_pos(ctx.streams[0], 1, 1);
  emel::memory::kv::action::run_prepare_slots(
      emel::memory::kv::event::prepare_slots{
          .error_out = &error_out,
      },
      ctx);
  CHECK(error_out == EMEL_OK);

  ctx.kv_size = 8;
  ctx.streams[0].head = 7;
  ctx.streams[0].pos.fill(emel::memory::kv::action::POS_NONE);
  emel::memory::kv::action::set_cell_pos(ctx.streams[0], 1, 1);
  ctx.ubatch_sizes[0] = 2;
  emel::memory::kv::action::run_prepare_slots(
      emel::memory::kv::event::prepare_slots{
          .error_out = &error_out,
      },
      ctx);
  CHECK(error_out == EMEL_OK);
  CHECK(ctx.slot_offsets[0] == 0);

  emel::memory::kv::event::apply_ubatch apply{};
  emel::memory::kv::event::apply_step apply_step{.request = &apply, .error_out = &error_out};
  ctx.planned_ubatch_count = 1;
  ctx.applied_ubatches = 0;
  ctx.ubatch_sizes[0] = 1;
  ctx.slot_offsets[0] = 0;
  ctx.slot_index_count = 1;
  ctx.slot_indices[0] = 7;
  ctx.slot_stream_ids[0] = 0;
  ctx.ubatch_stream_ids[0] = 0;
  ctx.ubatch_seq_ids[0] = 0;
  ctx.streams[0].pos[7] = emel::memory::kv::action::POS_NONE;
  ctx.kv_tokens = 0;
  emel::memory::kv::action::run_apply_step(
      apply_step, ctx);
  CHECK(error_out == EMEL_OK);
  CHECK(ctx.streams[0].head == 0);
  CHECK(ctx.kv_tokens >= 1);

  emel::memory::kv::event::rollback rollback{};
  emel::memory::kv::event::rollback_step rollback_step{
    .request = &rollback,
    .error_out = &error_out,
  };
  rollback.from_ubatch_index = 0;
  ctx.applied_ubatches = 1;
  ctx.streams[0].head = 8;
  ctx.slot_index_count = 1;
  ctx.slot_indices[0] = 7;
  ctx.slot_stream_ids[0] = 0;
  emel::memory::kv::action::run_rollback_step(
      rollback_step, ctx);
  CHECK(error_out == EMEL_OK);
  CHECK(ctx.applied_ubatches == 0);
  CHECK(ctx.streams[0].head == 7);

  ctx.planned_ubatch_count = 2;
  ctx.slot_offsets[0] = 1;
  ctx.slot_offsets[1] = 3;
  emel::memory::kv::action::run_publish(
      emel::memory::kv::event::publish{
          .error_out = &error_out,
      },
      ctx);
  CHECK(error_out == EMEL_OK);

  emel::memory::kv::action::on_kv_done(emel::memory::kv::events::kv_done{}, ctx);
  CHECK(ctx.applied_ubatches == 0);
  emel::memory::kv::action::on_kv_error(
      emel::memory::kv::events::kv_error{
          .err = EMEL_ERR_BACKEND,
      },
      ctx);
  CHECK(ctx.planned_ubatch_count == 2);
}
