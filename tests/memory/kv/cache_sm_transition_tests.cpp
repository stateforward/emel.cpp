...
TEST_CASE("kv_cache_sm_prepare_populates_outputs_and_capacity_error") {
  ...
  machine.process_event(emel::memory::kv::event::prepare{
...
TEST_CASE("kv_cache_sm_prepare_apply_rollback_optional_outputs") {
  ...
  int32_t kv_tokens = 0;
  err = EMEL_OK;
  machine.process_event(emel::memory::kv::event::apply_ubatch{
    .ubatch_index = 0,
  ...
  machine.process_event(emel::memory::kv::event::rollback{
    .from_ubatch_index = 1,
  });
}
