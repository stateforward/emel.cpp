...
TEST_CASE("memory_coordinator_begin_actions_store_requests") {
  ...
  emel::memory::coordinator::recurrent::action::begin_prepare_update(
      ...
      ctx);
  CHECK(err == EMEL_OK);
  CHECK(ctx.active_request == emel::memory::coordinator::recurrent::action::request_kind::update);
  CHECK(ctx.update_request.status_out == nullptr);
  CHECK(ctx.update_request.error_out == nullptr);

  emel::memory::coordinator::recurrent::action::begin_prepare_batch(
      ...
      ctx);
  CHECK(err == EMEL_OK);
  CHECK(ctx.active_request == emel::memory::coordinator::recurrent::action::request_kind::batch);
  CHECK(ctx.batch_request.status_out == nullptr);
  CHECK(ctx.batch_request.error_out == nullptr);

  emel::memory::coordinator::recurrent::action::begin_prepare_full(
      ...
      ctx);
  CHECK(err == EMEL_OK);
  CHECK(ctx.active_request == emel::memory::coordinator::recurrent::action::request_kind::full);
...
TEST_CASE("memory_coordinator_guards_cover_batch_validation") {
  emel::memory::coordinator::recurrent::action::context ctx{};
  ctx.active_request = emel::memory::coordinator::recurrent::action::request_kind::batch;
  ctx.batch_request = emel::memory::coordinator::event::prepare_batch{
      .n_ubatch = 0,
      .n_ubatches_total = 1,
  };
  CHECK_FALSE(emel::memory::coordinator::recurrent::guard::valid_batch_context{}(ctx));
  CHECK(emel::memory::coordinator::recurrent::guard::invalid_batch_context{}(ctx));

  ctx.batch_request.n_ubatch = 1;
  ctx.batch_request.n_ubatches_total = 2;
  CHECK(emel::memory::coordinator::recurrent::guard::valid_batch_context{}(ctx));
}

TEST_CASE("memory_coordinator_prepare_update_phase_tracks_status") {
  emel::memory::coordinator::recurrent::action::context ctx{};
  ctx.active_request = emel::memory::coordinator::recurrent::action::request_kind::update;
...
TEST_CASE("memory_coordinator_apply_update_phase_clears_pending") {
  ...
  emel::memory::coordinator::recurrent::action::run_apply_update_phase(ctx);
...
TEST_CASE("memory_coordinator_error_actions_set_last_error") {
  ...
  emel::memory::coordinator::recurrent::action::set_invalid_argument(ctx);
  CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  CHECK(ctx.last_error == EMEL_ERR_INVALID_ARGUMENT);

  emel::memory::coordinator::recurrent::action::set_backend_error(ctx);
