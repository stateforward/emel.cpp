...
TEST_CASE("memory_coordinator_guard_helpers_cover_phase_and_apply") {
  ...
  using emel::memory::coordinator::action::request_kind;

  context ctx{};
  CHECK(emel::memory::coordinator::guard::apply_update_invalid_context{}(ctx));

  ctx.active_request = request_kind::update;
  ctx.prepared_status = emel::memory::coordinator::event::memory_status::success;
  CHECK_FALSE(emel::memory::coordinator::guard::apply_update_invalid_context{}(ctx));

  ctx.has_pending_update = true;
  CHECK(emel::memory::coordinator::guard::apply_update_ready{}(ctx));
  CHECK_FALSE(emel::memory::coordinator::guard::apply_update_backend_failed{}(ctx));
  ...
  CHECK(emel::memory::coordinator::guard::apply_update_backend_failed{}(ctx));
}
