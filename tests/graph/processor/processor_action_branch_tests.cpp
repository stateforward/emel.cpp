...
TEST_CASE("compute_executor_actions_return_early_when_error_out_is_null") {
  ...
  emel::graph::processor::action::run_validate(
    emel::graph::processor::event::validate{.request = &request}, ctx);
  emel::graph::processor::action::run_prepare_graph(
    emel::graph::processor::event::prepare_graph{.request = &request}, ctx);
  emel::graph::processor::action::run_alloc_graph(
    emel::graph::processor::event::alloc_graph{.request = &request}, ctx);
  emel::graph::processor::action::run_bind_inputs(
    emel::graph::processor::event::bind_inputs{.request = &request}, ctx);
  emel::graph::processor::action::run_backend(
    emel::graph::processor::event::run_backend{.request = &request}, ctx);
  emel::graph::processor::action::run_extract_outputs(
    emel::graph::processor::event::extract_outputs{.request = &request}, ctx);
}
...
TEST_CASE("compute_executor_guard_reports_graph_reuse_state") {
  ...
  CHECK(emel::graph::processor::guard::graph_reused(ctx));
  CHECK_FALSE(emel::graph::processor::guard::graph_needs_allocation(ctx));

  ctx.graph_reused = false;
  CHECK_FALSE(emel::graph::processor::guard::graph_reused(ctx));
  CHECK(emel::graph::processor::guard::graph_needs_allocation(ctx));
}
...
TEST_CASE("compute_executor_run_validate_handles_missing_and_failed_callbacks") {
  ...
  emel::graph::processor::action::run_validate(
    emel::graph::processor::event::validate{.request = &request, .error_out = &err},
    ctx);
  CHECK(err == EMEL_OK);
...
TEST_CASE("compute_executor_run_phase_actions_report_callback_failures") {
  ...
  emel::graph::processor::action::run_prepare_graph(
    emel::graph::processor::event::prepare_graph{
      .request = &request,
      .reused_out = &reused,
      .error_out = &err,
    },
  ...
  err = EMEL_OK;
  emel::graph::processor::action::run_alloc_graph(
    emel::graph::processor::event::alloc_graph{.request = &request, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_BACKEND);

  err = EMEL_OK;
  emel::graph::processor::action::run_bind_inputs(
    emel::graph::processor::event::bind_inputs{.request = &request, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_BACKEND);

  err = EMEL_OK;
  emel::graph::processor::action::run_backend(
    emel::graph::processor::event::run_backend{.request = &request, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_BACKEND);

  err = EMEL_OK;
  emel::graph::processor::action::run_extract_outputs(
    emel::graph::processor::event::extract_outputs{.request = &request, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_BACKEND);
}
...
TEST_CASE("compute_executor_actions_reject_null_requests") {
  ...
  emel::graph::processor::action::run_validate(
    emel::graph::processor::event::validate{.request = nullptr, .error_out = &err}, ctx);
  ...
  err = EMEL_OK;
  emel::graph::processor::action::run_alloc_graph(
    emel::graph::processor::event::alloc_graph{.request = nullptr, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_INVALID_ARGUMENT);

  err = EMEL_OK;
  emel::graph::processor::action::run_bind_inputs(
    emel::graph::processor::event::bind_inputs{.request = nullptr, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_INVALID_ARGUMENT);

  err = EMEL_OK;
  emel::graph::processor::action::run_backend(
    emel::graph::processor::event::run_backend{.request = nullptr, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_INVALID_ARGUMENT);

  err = EMEL_OK;
  emel::graph::processor::action::run_extract_outputs(
    emel::graph::processor::event::extract_outputs{.request = nullptr, .error_out = &err},
    ctx);
  CHECK(err == EMEL_ERR_INVALID_ARGUMENT);
}
...
TEST_CASE("compute_executor_actions_handle_missing_callbacks_in_phase_helpers") {
  ...
  emel::graph::processor::action::run_validate(
    emel::graph::processor::event::execute{}, ctx);
...
TEST_CASE("compute_executor_run_extract_outputs_updates_outputs") {
  ...
  emel::graph::processor::action::run_extract_outputs(
    emel::graph::processor::event::extract_outputs{
      .request = &request,
      .error_out = &err,
    },
    ctx);
...
TEST_CASE("compute_executor_on_unexpected_sets_error") {
  ...
  emel::graph::processor::action::on_unexpected(
    emel::graph::processor::event::execute{.error_out = &err},
    ctx);
