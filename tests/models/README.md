...
## moshi tiny synthetic fixtures (v1.28 Moshi/PersonaPlex milestone 1)
- Files: `moshi-tiny-lm.gguf`, `moshi-tiny-lm-raw.gguf`, `mimi-tiny.gguf`,
  `mimi-tiny-f16.gguf`, `mimi-tiny-q8.gguf`, `moshi-tiny-voice.gguf`,
  `moshi-tiny-config.json`, `moshi-tokenizer-tiny.model`
- Source: fully synthetic, generated by `tools/bench/moshi_make_tiny_fixture.py`
  (deterministic fixed-seed LCG payloads; regeneration is bit-identical).
- Generator command:
  `python3 tools/bench/moshi_make_tiny_fixture.py --output-dir tests/models`
- Enriched fixtures are produced through the maintained converter
  `tools/bench/moshi_gguf_convert.py` from raw moshi.cpp-style intermediates, so the
  fixtures exercise the exact converter codepath used for real models
  (including CRC-mangled long-name restoration inside `mimi-tiny.gguf`'s raw
  intermediate).
- `moshi-tiny-lm-raw.gguf` is intentionally metadata-less (zero GGUF KV entries,
  moshi.cpp cache style) and is used to prove emel rejects unconverted moshi GGUFs.
<!-- SHA256 checksums generated by tools/bench/moshi_make_tiny_fixture.py for the moshi tiny synthetic fixture files listed above -->
- SHA256:
  - `moshi-tiny-config.json`: `7e6fbcacaa7ce6279fb8731cb01456f371781dc6b17c6e2308212d08b7f8ec0e`
  ...
  - `moshi-tiny-lm.gguf`: `28cc6cd14d7fe8a2954b887e89d1b0d36dcb7fec0c295e21df51e1e15fa2cd8c`
  - `mimi-tiny.gguf`: `518693f7b8d261fb41b2a018518dc1d64dbb505d7f869e996605a83d66d4af0f`
  ...
  - `mimi-tiny-f16.gguf`: `3880b627a5b352a2ce1cf0a3a26ceca190a1ffff4773dda8213afc071dca75c1`
    (f16 operand-class variant: conv/projection weights stored f16 like the
    real Mimi artifact, driving the codec machines' guard_conv_f16 rows and
    the f16 im2col/mul_mat and raw-f16 RVQ projection paths)
  - `mimi-tiny-q8.gguf`: `f5919a3eb852772c67a6a913ccc2ee960d162f79949d8a96dcf5d847c3c44412`
    (q8_0 operand-class variant: dim/codebook_dim widened to one q8 block of
    32 and the transformer + RVQ projections quantized by the converter's
    `--quantize q8_0` path, driving guard_proj_q8 / guard_class_q8 rows and
    the q8 matvec paths)
...
- Modeled contract (tiny scale, not real weights): PersonaPlex-shaped `lm` component
  ...
  depformer dim `16`/`2` layers), Mimi-shaped `mimi` component (dim `16`,
  active `n_q=2`, codebook_dim `8`, `2`-layer codec transformers), and a PersonaPlex `voice`
  prompt component (`voice.embeddings` + `voice.cache`).
- Loader/runtime boundary note: these fixtures prove converter output, moshi
  architecture registration, hparam binding, and execution-contract validation only;
  they carry random weights and are not runnable models.
