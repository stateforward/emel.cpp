...
inline void fixture_fatal(const char * message, const std::string & detail = {}) {
  if (detail.empty()) {
...
inline std::filesystem::path te_fixture_default_path() {
  return repo_root() / "tests" / "models" / "TE-75M-q8_0.gguf";
}

inline bool is_approved_te_fixture_path(const std::filesystem::path & path) {
  const auto filename = path.filename().string();
  return filename == "TE-75M-q8_0.gguf" || filename == "TE-75M-q5_0.gguf";
}

inline std::filesystem::path te_fixture_path() {
  const char * env = std::getenv("EMEL_TE_FIXTURE");
  if (env == nullptr || env[0] == '\0') {
    return te_fixture_default_path();
  }
  ...
  if (!is_approved_te_fixture_path(path)) {
    fixture_fatal("unapproved maintained TE fixture", path.string());
  }
  return path;
}

inline std::string te_fixture_case_slug() {
  const std::string stem = te_fixture_path().stem().string();
  std::string slug = {};
...
inline std::vector<uint8_t> read_binary_file(const std::filesystem::path & path) {
  ...
  if (!stream.good()) {
    fixture_fatal("failed to open binary fixture", path.string());
  }

  stream.seekg(0, std::ios::end);
  const std::streamsize size = stream.tellg();
  if (size <= 0) {
    fixture_fatal("binary fixture is empty", path.string());
  }
  ...
  if (!stream.good()) {
    fixture_fatal("failed to read binary fixture", path.string());
  }
  return bytes;
}
...
inline std::string read_text_file(const std::filesystem::path & path) {
  ...
  if (!stream.good()) {
    fixture_fatal("failed to open text fixture", path.string());
  }
...
inline void materialize_tensor_names(loaded_te_fixture & fixture) {
  ...
  for (uint32_t index = 0u; index < fixture.model->n_tensors; ++index) {
    ...
    if (source_offset + length > fixture.file_bytes.size()) {
      fixture_fatal("tensor name source range is out of bounds");
    }
    if (fixture.model->name_bytes_used + length > fixture.model->name_storage.size()) {
      fixture_fatal("tensor name storage is exhausted");
    }
...
inline loaded_te_fixture load_te_fixture(const std::filesystem::path & model_path) {
  loaded_te_fixture fixture = {};
  fixture.file_bytes = read_binary_file(model_path);
  ...
  if (!loader.process_event(probe)) {
    fixture_fatal("failed to probe TE GGUF fixture");
  }

  if (requirements.tensor_count == 0u || requirements.tensor_count > fixture.model->tensors.size()) {
    fixture_fatal("invalid tensor-count requirements in TE GGUF fixture");
  }
  ...
  if (!loader.process_event(bind)) {
    fixture_fatal("failed to bind TE GGUF storage");
  }
  ...
  if (!loader.process_event(parse)) {
    fixture_fatal("failed to parse TE GGUF fixture");
  }
  ...
  if (!emel::model::detail::load_hparams_from_gguf(binding, *fixture.model)) {
    fixture_fatal("failed to load TE model hyperparameters");
  }
  if (!load_te_vocab_from_file(te_vocab_path(), fixture.model->vocab_data)) {
    fixture_fatal("failed to load TE vocab fixture", te_vocab_path().string());
  }
...
inline loaded_te_fixture load_te_fixture() {
  return load_te_fixture(te_fixture_path());
}
...
inline float max_abs_difference(const std::span<const float> lhs,
  ...
  if (lhs.size() != rhs.size()) {
    fixture_fatal("cannot diff fixture spans with mismatched sizes");
  }
