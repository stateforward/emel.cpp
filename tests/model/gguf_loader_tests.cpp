...
bool write_kv_array_header(std::FILE * file, const char * key,
                           const emel::parser::gguf::value_type elem_type,
...
bool write_kv_string_array(std::FILE * file, const char * key,
  ...
  if (!write_kv_array_header(file, key,
                             emel::parser::gguf::value_type::k_string,
                             values.size())) {
    return false;
  }
...
bool write_kv_f32_array(std::FILE * file, const char * key,
  ...
  if (!write_kv_array_header(file, key,
                             emel::parser::gguf::value_type::k_f32,
                             values.size())) {
    return false;
  }
...
bool write_kv_i32_array(std::FILE * file, const char * key,
  ...
  if (!write_kv_array_header(file, key,
                             emel::parser::gguf::value_type::k_i32,
                             values.size())) {
    return false;
  }
...
bool write_kv_u8_array(std::FILE * file, const char * key,
  ...
  if (!write_kv_array_header(file, key,
                             emel::parser::gguf::value_type::k_u8,
                             values.size())) {
    return false;
  }
...
bool write_kv_bool_array(std::FILE * file, const char * key,
  ...
  if (!write_kv_array_header(file, key,
                             emel::parser::gguf::value_type::k_bool,
                             values.size())) {
    return false;
  }
  return std::fwrite(values.data(), 1, values.size(), file) == values.size();
...
bool write_bad_kv(std::FILE * file,
                  const char * key,
                  const emel::parser::gguf::value_type type,
                  const bool is_array,
                  const emel::parser::gguf::value_type elem_type) {
  if (!write_string(file, key)) {
    return false;
  ...
  if (is_array) {
    const int32_t arr_type = static_cast<int32_t>(emel::parser::gguf::value_type::k_array);
    ...
    if (elem_type == emel::parser::gguf::value_type::k_string) {
      return write_string(file, "x");
    }
    if (elem_type == emel::parser::gguf::value_type::k_u32) {
      return write_u32(file, 1);
    }
    if (elem_type == emel::parser::gguf::value_type::k_f32) {
      return write_f32(file, 1.0f);
    }
    if (elem_type == emel::parser::gguf::value_type::k_bool) {
      const uint8_t raw = 1;
      return std::fwrite(&raw, 1, sizeof(raw), file) == sizeof(raw);
    }
    return true;
  ...
  if (type == emel::parser::gguf::value_type::k_string) {
    return write_string(file, "x");
  }
  if (type == emel::parser::gguf::value_type::k_u32) {
    return write_u32(file, 1);
  }
  if (type == emel::parser::gguf::value_type::k_i32) {
    return write_i32(file, 1);
  }
  if (type == emel::parser::gguf::value_type::k_f32) {
    return write_f32(file, 1.0f);
  }
  if (type == emel::parser::gguf::value_type::k_bool) {
    const uint8_t raw = 1;
    return std::fwrite(&raw, 1, sizeof(raw), file) == sizeof(raw);
  }
  return true;
...
bool pad_to_alignment(std::FILE * file, const uint64_t alignment) {
  ...
  const uint64_t aligned =
    emel::parser::gguf::align_up_u64(static_cast<uint64_t>(pos), alignment);
...
bool write_minimal_gguf(const char * path, const float weight_value) {
  ...
  if (!pad_to_alignment(file, emel::parser::gguf::k_default_alignment)) {
    std::fclose(file);
    return false;
  }
  std::array<uint8_t, emel::parser::gguf::k_default_alignment> payload = {};
...
bool write_suffix_params_gguf(const char * path) {
  ...
  const int64_t n_kv =
    ...
    5;
  if (!write_header(file, emel::parser::gguf::k_gguf_version, 1, n_kv)) {
    std::fclose(file);
    return false;
  }
  if (!write_kv_string(file, "general.architecture", "test") ||
...
TEST_CASE("gguf helpers cover key parsing and arithmetic") {
  ...
  CHECK(emel::parser::gguf::store_metadata_string(meta, "hello", 5, view));
...
TEST_CASE("gguf reader handles strings and keys") {
  ...
  CHECK(std::fwrite("hello", 1, 5, file) == 5);
  std::rewind(file);
  emel::parser::gguf::reader r2{file};
  char small[4] = {};
  len = 123;
  CHECK(emel::parser::gguf::read_key(r2, small, sizeof(small), len));
  CHECK(len == 0);
  CHECK(small[0] == '\0');
  std::fclose(file);
}
...
TEST_CASE("gguf value parsing handles numeric conversions") {
  ...
  emel::parser::gguf::reader r{file};
  uint32_t u32 = 0;
  CHECK(!emel::parser::gguf::parse_u32_value(r, emel::parser::gguf::value_type::k_i32, u32));
  std::fclose(file);
  ...
  emel::parser::gguf::reader r2{file};
  int32_t i32 = 0;
  CHECK(!emel::parser::gguf::parse_i32_value(r2, emel::parser::gguf::value_type::k_u32, i32));
  std::fclose(file);
  ...
  std::rewind(file);
  emel::parser::gguf::reader r3{file};
  bool value = false;
  CHECK(emel::parser::gguf::parse_bool_value(r3, emel::parser::gguf::value_type::k_i8, value));
  CHECK(value);
  std::fclose(file);
}
...
TEST_CASE("gguf parser rejects bad kv types") {
  ...
  auto run_bad_case = [&](const char * key,
    ...
    REQUIRE(write_header(file, emel::parser::gguf::k_gguf_version, 0, n_kv));
    if (needs_arch) {
    ...
    request.format_ctx = &ctx;
    int32_t err = EMEL_OK;
    const bool ok = emel::parser::gguf::map_parser(request, &err);
    if (!ok) {
      CHECK(err != EMEL_OK);
    }
    std::remove(path);
  ...
  for (const char * key : numeric_keys) {
    run_bad_case(key, emel::parser::gguf::value_type::k_string, false,
                 emel::parser::gguf::value_type::k_string);
  }
  for (const char * key : string_keys) {
    run_bad_case(key, emel::parser::gguf::value_type::k_u32, false,
                 emel::parser::gguf::value_type::k_u32);
  }
  for (const char * key : array_string_keys) {
    run_bad_case(key, emel::parser::gguf::value_type::k_array, true,
                 emel::parser::gguf::value_type::k_u32);
  }
...
TEST_CASE("gguf metadata string and arrays parse") {
  ...
  CHECK(emel::parser::gguf::read_metadata_string(
    r, emel::parser::gguf::value_type::k_string, meta, out));
  CHECK(out.length == 4);
  ...
  const int32_t elem_type =
    static_cast<int32_t>(emel::parser::gguf::value_type::k_string);
  ...
  uint32_t count = 0;
  CHECK(emel::parser::gguf::read_metadata_string_array(
    r2, emel::parser::gguf::value_type::k_array, meta, list, count));
  CHECK(count == 2);
  ...
  const int32_t u32_type =
    static_cast<int32_t>(emel::parser::gguf::value_type::k_u32);
  ...
  CHECK(emel::parser::gguf::read_u32_array(
    r3, emel::parser::gguf::value_type::k_array, u32_out, u32_count));
  CHECK(u32_count == 3);
  CHECK(u32_out[2] == 3);
  std::fclose(file);
  ...
  const int32_t i32_type =
    static_cast<int32_t>(emel::parser::gguf::value_type::k_i32);
  CHECK(write_i32(file, i32_type));
  CHECK(write_u64(file, 2));
  CHECK(write_i32(file, -1));
  CHECK(write_i32(file, 7));
  std::rewind(file);
  emel::parser::gguf::reader r4{file};
  std::array<int32_t, 4> i32_out = {};
  int32_t i32_count = 0;
  CHECK(emel::parser::gguf::read_i32_array(
    r4, emel::parser::gguf::value_type::k_array, i32_out, i32_count));
  CHECK(i32_count == 2);
  CHECK(i32_out[0] == -1);
  std::fclose(file);
  ...
  const int32_t f32_type =
    static_cast<int32_t>(emel::parser::gguf::value_type::k_f32);
  CHECK(write_i32(file, f32_type));
  CHECK(write_u64(file, 2));
  CHECK(write_f32(file, 0.25f));
  CHECK(write_f32(file, 0.5f));
  std::rewind(file);
  emel::parser::gguf::reader r5{file};
  std::array<float, emel::model::data::k_max_metadata_arrays> f32_out = {};
  uint32_t f32_count = 0;
  CHECK(emel::parser::gguf::read_f32_array(
    r5, emel::parser::gguf::value_type::k_array, f32_out, f32_count));
  CHECK(f32_count == 2);
  ...
  const int32_t bool_type =
    static_cast<int32_t>(emel::parser::gguf::value_type::k_bool);
  CHECK(write_i32(file, bool_type));
  CHECK(write_u64(file, 3));
  const uint8_t raw[3] = {1, 0, 1};
  CHECK(std::fwrite(raw, 1, sizeof(raw), file) == sizeof(raw));
  std::rewind(file);
  emel::parser::gguf::reader r6{file};
  std::array<uint8_t, emel::model::data::k_max_metadata_arrays> b_out = {};
  uint32_t b_count = 0;
  CHECK(emel::parser::gguf::read_bool_array(
    r6, emel::parser::gguf::value_type::k_array, b_out, b_count));
  CHECK(b_count == 3);
  CHECK(b_out[1] == 0);
  std::fclose(file);
}
...
TEST_CASE("gguf helper routines validate tensor metadata") {
  ...
  record.type = static_cast<int32_t>(emel::parser::gguf::tensor_type::k_f32);
  record.data = buffer.data();

  int32_t err = EMEL_ERR_INVALID_ARGUMENT;
  CHECK(emel::parser::gguf::validate_tensor_data(model, &err));
  CHECK(err == EMEL_OK);
...
TEST_CASE("gguf tensor size and reset helpers") {
  ...
  CHECK(emel::parser::gguf::compute_tensor_size(
    dims, emel::parser::gguf::tensor_type::k_f32, size));
  CHECK(size == 4);

  const int32_t block =
    emel::parser::gguf::blck_size_for(emel::parser::gguf::tensor_type::k_q4_0);
  ...
  emel::parser::gguf::reset_model_data(model);
  ...
  CHECK(ctx.alignment == emel::parser::gguf::k_default_alignment);
...
TEST_CASE("gguf header and split metadata validation") {
  ...
  int64_t n_tensors = 0;
  int64_t n_kv = 0;
  CHECK(emel::parser::gguf::parse_header(r, ctx, n_tensors, n_kv));
  CHECK(n_tensors == 1);
  CHECK(n_kv == 0);
  std::fclose(file);
  ...
  std::rewind(file);
  emel::parser::gguf::reader r2{file};
  CHECK(!emel::parser::gguf::parse_header(r2, ctx, n_tensors, n_kv));
  std::fclose(file);
  ...
  int32_t err = EMEL_OK;
  CHECK(emel::parser::gguf::validate_split_metadata(ctx, 1, err));
  CHECK(err == EMEL_OK);
...
TEST_CASE("gguf array value helpers consume streams") {
  ...
  uint32_t count = 0;
  CHECK(emel::parser::gguf::read_metadata_string_array_values(
    r, emel::parser::gguf::value_type::k_string, 2, meta, out, count));
  CHECK(count == 2);
  std::fclose(file);
  ...
  std::array<uint32_t, emel::model::data::k_max_metadata_arrays> u32_out = {};
  uint32_t u32_count = 0;
  CHECK(emel::parser::gguf::read_u32_array_values(
    r2, emel::parser::gguf::value_type::k_u32, 2, u32_out, u32_count));
  CHECK(u32_count == 2);
  std::fclose(file);
  ...
  std::array<int32_t, 4> i32_out = {};
  int32_t i32_count = 0;
  CHECK(emel::parser::gguf::read_i32_array_values(
    r3, emel::parser::gguf::value_type::k_i32, 2, i32_out, i32_count));
  CHECK(i32_count == 2);
  std::fclose(file);
  ...
  std::array<float, emel::model::data::k_max_metadata_arrays> f32_out = {};
  uint32_t f32_count = 0;
  CHECK(emel::parser::gguf::read_f32_array_values(
    r4, emel::parser::gguf::value_type::k_f32, 2, f32_out, f32_count));
  ...
  CHECK(emel::parser::gguf::read_bool_array_values(
    r5, emel::parser::gguf::value_type::k_u8, 3, bool_out, bool_count));
  CHECK(bool_count == 3);
  std::fclose(file);
}
...
TEST_CASE("gguf validate_row_data covers tensor types") {
  ...
  for (int32_t t = static_cast<int32_t>(emel::parser::gguf::tensor_type::k_f32);
       t < static_cast<int32_t>(emel::parser::gguf::tensor_type::k_count); ++t) {
    const auto type = static_cast<emel::parser::gguf::tensor_type>(t);
    const uint32_t type_size = emel::parser::gguf::type_size_for(type);
    if (type_size == 0) {
      CHECK(!emel::parser::gguf::validate_row_data(type, nullptr, 1));
      continue;
    }
    REQUIRE(type_size <= storage.size());
    CHECK(emel::parser::gguf::validate_row_data(type, storage.data(), type_size));
  }
  CHECK(!emel::parser::gguf::validate_row_data(
    static_cast<emel::parser::gguf::tensor_type>(
      static_cast<int32_t>(emel::parser::gguf::tensor_type::k_count)),
    storage.data(), 4));
}
...
TEST_CASE("gguf weight loader handles streamed and mapped loads") {
  ...
  CHECK(emel::parser::gguf::load_streamed(ev, &done, &total, &err));
  CHECK(err == EMEL_OK);
  CHECK(done == total);
  CHECK(progress_seen);
  ...
  total = 0;
#if defined(_WIN32)
  CHECK(!emel::parser::gguf::map_mmap(mmap_ev, &done, &total, &err));
...
TEST_CASE("gguf weight loader validates inputs") {
  emel::model::weight_loader::event::load_weights ev = {};
  int32_t err = EMEL_OK;
  CHECK(!emel::parser::gguf::init_mappings(ev, &err));
  CHECK(err == EMEL_ERR_INVALID_ARGUMENT);

  err = EMEL_OK;
  CHECK(!emel::parser::gguf::validate_weights(ev, &err));
  CHECK(err == EMEL_ERR_INVALID_ARGUMENT);

  err = EMEL_OK;
  CHECK(!emel::parser::gguf::clean_up_weights(ev, &err));
  CHECK(err == EMEL_ERR_INVALID_ARGUMENT);
}

TEST_CASE("gguf split helpers reject invalid input") {
  size_t prefix_len = 0;
  char out[8] = {};
  char out_good[64] = {};
  const char * good = "x-00001-of-00002.gguf";

  CHECK(!emel::parser::gguf::format_split_path({}, 0, 0, nullptr, 0));
  CHECK(!emel::parser::gguf::format_split_path("bad.bin", 0, 1, out, sizeof(out)));
  CHECK(!emel::parser::gguf::format_split_path("bad", 0, 1, out, sizeof(out)));
  CHECK(!emel::parser::gguf::format_split_path("foo.gguf", 0, 1, out, 4));
  CHECK(emel::parser::gguf::format_split_path(good, 1, 2, out_good, sizeof(out_good)));

  CHECK(emel::parser::gguf::has_split_suffix(good, prefix_len));
  CHECK(prefix_len == 1);
  CHECK(!emel::parser::gguf::has_split_suffix("foo.gguf", prefix_len));
  CHECK(!emel::parser::gguf::has_split_suffix("x_00001-of-00002.gguf", prefix_len));
  CHECK(!emel::parser::gguf::has_split_suffix("x-a0001-of-00002.gguf", prefix_len));
  CHECK(!emel::parser::gguf::has_split_suffix("x-00001_xx_00002.gguf", prefix_len));
  CHECK(!emel::parser::gguf::has_split_suffix("x-00001-of-0000x.gguf", prefix_len));
  CHECK(!emel::parser::gguf::has_split_suffix("x-00001-of-00002.gguX", prefix_len));
}

TEST_CASE("gguf reader rejects null files") {
  emel::parser::gguf::reader reader{};
  uint64_t len = 0;
  char buffer[4] = {};

  CHECK(!reader.read_raw(buffer, sizeof(buffer)));
  CHECK(!reader.read(len));
  CHECK(!reader.skip(4));
  CHECK(!reader.read_string(buffer, sizeof(buffer), len));
  CHECK(!emel::parser::gguf::read_and_discard_string(reader));
  CHECK(!emel::parser::gguf::skip_value(reader, emel::parser::gguf::value_type::k_u8, 1));
  CHECK(!emel::parser::gguf::skip_value(reader, emel::parser::gguf::value_type::k_u32, 1));
  CHECK(!emel::parser::gguf::skip_value(reader, emel::parser::gguf::value_type::k_string, 1));
}
