...
TEST_CASE("batch_planner_guard_inputs_are_valid") {
  ...
  CHECK_FALSE(emel::batch::planner::guard::guard_inputs_valid(make_runtime(request, request_ctx),
                                                             planner_ctx));
  ...
  CHECK(emel::batch::planner::guard::guard_inputs_valid(make_runtime(request, request_ctx),
                                                       planner_ctx));
}
...
TEST_CASE("batch_planner_guard_mode_selection") {
  ...
  CHECK(emel::batch::planner::guard::guard_mode_is_simple(make_runtime(request, request_ctx),
                                                     planner_ctx));
  CHECK_FALSE(emel::batch::planner::guard::guard_mode_is_equal(make_runtime(request, request_ctx),
                                                          planner_ctx));
  CHECK_FALSE(emel::batch::planner::guard::guard_mode_is_sequential(make_runtime(request, request_ctx),
                                                        planner_ctx));

  request.mode = emel::batch::planner::event::plan_mode::equal;
  CHECK(emel::batch::planner::guard::guard_mode_is_equal(make_runtime(request, request_ctx),
                                                    planner_ctx));

  request.mode = emel::batch::planner::event::plan_mode::seq;
  CHECK(emel::batch::planner::guard::guard_mode_is_sequential(make_runtime(request, request_ctx),
                                                  planner_ctx));
}
...
TEST_CASE("batch_planner_guard_planning_failed") {
  ...
  CHECK(emel::batch::planner::guard::guard_planning_failed(make_runtime(request, request_ctx),
                                                      planner_ctx));
  ...
  CHECK_FALSE(emel::batch::planner::guard::guard_planning_failed(make_runtime(request, request_ctx),
                                                            planner_ctx));
...
TEST_CASE("batch_planner_guard_planning_failed_error_classification") {
  ...
  CHECK(emel::batch::planner::guard::guard_planning_failed_with_error(
      make_runtime(request, request_ctx),
                                                                 planner_ctx));
  CHECK_FALSE(emel::batch::planner::guard::guard_planning_failed_without_error(
      make_runtime(request, request_ctx), planner_ctx));

  request_ctx.err = emel::error::cast(emel::batch::planner::error::none);
  CHECK_FALSE(emel::batch::planner::guard::guard_planning_failed_with_error(
      make_runtime(request, request_ctx), planner_ctx));
  CHECK(emel::batch::planner::guard::guard_planning_failed_without_error(
      make_runtime(request, request_ctx), planner_ctx));
}
