...
static_assert(std::is_reference_v<
              decltype(std::declval<
                           const emel::text::generator::event::initialize &>()
...
static_assert(
    std::is_same_v<
        std::remove_cvref_t<decltype(std::declval<const emel::text::generator::
                                                      event::initialize &>()
                                         .sampler_fns)>,
...
emel::text::generator::diagnostics
capture_generator_diagnostics(emel::text::generator::sm &generator) {
  emel::text::generator::diagnostics diagnostics = {};
  CHECK(generator.process_event(
      emel::text::generator::event::capture_diagnostics{diagnostics}));
  return diagnostics;
}
...
bool format_checked_messages(
    void *formatter_ctx, const emel::text::formatter::format_request &request,
    int32_t *error_out) {
  if (error_out != nullptr) {
    *error_out =
        emel::text::formatter::error_code(emel::text::formatter::error::none);
  }
...
void build_prepared_model(prepared_model &prepared) {
  ...
  prepared.data.vocab_data.tokenizer_model_id =
      emel::model::data::tokenizer_model::BPE;
  prepared.data.vocab_data.tokenizer_pre_id =
  ...
  prepared.data.params.n_vocab =
      static_cast<int32_t>(prepared.data.vocab_data.n_tokens);
  ...
  const auto add_name = [&](emel::model::data::tensor_record &tensor,
    ...
    std::memcpy(prepared.data.name_storage.data() +
                    prepared.data.name_bytes_used,
                name.data(), name.size());
...
void build_quantized_contract_prepared_model(prepared_model &prepared) {
  ...
  const auto add_vector = [&](const std::string_view name, const int32_t cols) {
    ...
    tensor.dims[0] = cols;
    tensor.data = prepared.tensor_storage.back().data();
    tensor.data_size = static_cast<uint64_t>(
        prepared.tensor_storage.back().size() * sizeof(float));
  };
  const auto add_matrix = [&](const std::string_view name, const int32_t rows,
                              const int32_t cols) {
    auto &tensor = prepared.data.tensors[tensor_index++];
    add_name(tensor, name);
    prepared.tensor_storage.emplace_back(
        static_cast<size_t>(rows) * static_cast<size_t>(cols), 0.0f);
    tensor.type = static_cast<int32_t>(emel::kernel::event::dtype::f32);
    tensor.n_dims = 2;
    tensor.dims[0] = cols;
    tensor.dims[1] = rows;
...
void build_gemma4_multiwidth_prepared_model(prepared_model &prepared) {
  ...
  prepared.data.params.attention_sliding_window_pattern_flags = {1u, 1u, 1u, 1u,
                                                                 0u};
  ...
  const auto add_block = [&](const int32_t block, const int32_t q_rows,
    ...
    add_matrix(prefix + "attn_q.weight", q_rows, 4,
               std::vector<float>(q_rows * 4, 0.0f));
    add_matrix(prefix + "attn_k.weight", kv_rows, 4,
               std::vector<float>(kv_rows * 4, 0.0f));
    add_matrix(prefix + "attn_v.weight", kv_rows, 4,
               std::vector<float>(kv_rows * 4, 0.0f));
    add_vector(prefix + "attn_q_norm.weight",
               std::vector<float>(static_cast<size_t>(q_rows / 2), 1.0f));
    add_vector(prefix + "attn_k_norm.weight",
               std::vector<float>(static_cast<size_t>(kv_rows), 1.0f));
    add_matrix(prefix + "attn_output.weight", 4, q_rows,
               std::vector<float>(q_rows * 4, 0.0f));
...
emel::error::type
build_generation_contract(prepared_model &prepared,
...
find_stage_audit(const emel::model::generation::quantized_path_audit &audit,
...
void apply_flash_kv_width_mismatch(prepared_model &prepared) {
  const auto tensor_name = [&](const emel::model::data::tensor_record &tensor) {
    return std::string_view(prepared.data.name_storage.data() +
                                tensor.name_offset,
                            tensor.name_length);
  };
...
struct generator_fixture {
  ...
  static constexpr std::array<emel::text::formatter::chat_message, 1>
      k_phase_4_messages = {
  ...
  explicit generator_fixture(
    ...
    REQUIRE(build_generation_contract(prepared, generation_contract) ==
            emel::error::type{0});
    const auto matmul_policy = emel::kernel::matmul::make_execution_policy(
        parallel_matmul_lanes, emel::kernel::detect_host_kind(), matmul_lanes);
  ...
  make_initialize(callback_tracker &tracker,
  ...
  make_generate(callback_tracker &tracker, char *output, size_t output_capacity,
...
TEST_CASE("generator_starts_uninitialized") {
  ...
  CHECK(fixture->generator->is(
      stateforward::sml::state<emel::text::generator::uninitialized>));
}
...
TEST_CASE("generator_dependencies_accept_custom_kv_actor") {
  ...
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::canonical, nullptr,
      emel::text::formatter::format_raw,
  ...
  emel::error::type generate_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto generate_request =
      fixture->make_generate(generate_tracker, output.data(), output.size(),
                             output_length, &generate_error);
...
TEST_CASE("generator_rejects_generate_before_initialize") {
  ...
  emel::error::type error =
      emel::error::cast(emel::text::generator::error::none);
  const auto request = fixture->make_generate(
      tracker, output.data(), output.size(), output_length, &error);
...
TEST_CASE("generator_initialize_succeeds_and_enters_ready") {
  ...
  CHECK(fixture->generator->is(
      stateforward::sml::state<emel::text::generator::ready>));
  CHECK(tracker.initialize_done_called);
  CHECK_FALSE(tracker.initialize_error_called);
  CHECK(error == emel::error::cast(emel::text::generator::error::none));
}

TEST_CASE("generator diagnostics are available before initialization") {
  auto fixture = std::make_unique<generator_fixture>();
...
TEST_CASE(
    "generator_reinitialize_reprepares_backend_when_block_geometry_changes") {
  auto fixture = std::make_unique<generator_fixture>();
  callback_tracker first_tracker{};
  emel::error::type first_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto first_request =
      fixture->make_initialize(first_tracker, &first_error);
  REQUIRE(fixture->generator->process_event(first_request));
  REQUIRE(fixture->generator->is(
      stateforward::sml::state<emel::text::generator::ready>));
...
TEST_CASE("generator_initialize_accepts_explicit_preselected_argmax_mode_"
          "without_sampler_chain") {
  auto fixture = std::make_unique<generator_fixture>();
  callback_tracker tracker{};
  emel::error::type error =
      emel::error::cast(emel::text::generator::error::backend);
...
TEST_CASE(
    "generator_initialize_rejects_block_pool_exceeding_physical_capacity") {
  auto fixture = std::make_unique<generator_fixture>();
  callback_tracker tracker{};
  emel::error::type error =
      emel::error::cast(emel::text::generator::error::none);
...
TEST_CASE("generator_initialize_rejects_default_generation_contract") {
  ...
  emel::model::generation::contract generation_contract{};
  emel::text::generator::sm generator{emel::text::generator::dependencies{
      .generation_contract = generation_contract,
      .conditioner = conditioner,
      .matmul_policy = matmul_policy,
      .runtime_policy = {},
...
TEST_CASE("generator_initialize_reports_original_request_without_generation_"
          "callbacks") {
  auto fixture = std::make_unique<generator_fixture>();
  callback_tracker tracker{};
  emel::error::type error =
...
TEST_CASE("generator_initialize_can_rebind_ready_session_without_re_reserving_"
          "graph") {
  auto fixture = std::make_unique<generator_fixture>();
  callback_tracker first_tracker{};
  emel::error::type first_error =
  ...
  const auto second_request =
      fixture->make_initialize(second_tracker, &second_error);
...
TEST_CASE("generator_generate_runs_native_generator_contract") {
  ...
  const auto initialize_request =
      fixture->make_initialize(initialize_tracker, &initialize_error);
...
TEST_CASE("generator unsupported lane policy stays serial through prefill and "
          "decode") {
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::canonical, nullptr,
      emel::text::formatter::format_raw, emel::memory::hybrid::kv_binding{},
      3u);
  callback_tracker initialize_tracker{};
  emel::error::type initialize_error =
      emel::error::cast(emel::text::generator::error::backend);
  ...
  REQUIRE(initialize_error ==
          emel::error::cast(emel::text::generator::error::none));

  constexpr std::array<emel::text::formatter::chat_message, 1> messages = {
      emel::text::formatter::chat_message{
          .role = "user",
  ...
  generate_request.on_done = emel::callback<void(
      const emel::text::generator::events::generation_done &)>(
      &generate_tracker, on_generate_done);
  generate_request.on_error = emel::callback<void(
      const emel::text::generator::events::generation_error &)>(
...
TEST_CASE("generator_qwen3_generator_initializes_and_generates_one_token") {
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::qwen3_canonical);
  callback_tracker initialize_tracker{};
  emel::error::type initialize_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto initialize_request =
...
TEST_CASE("generator_gemma4_generator_initializes_and_generates_one_token") {
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::gemma4_canonical);
  callback_tracker initialize_tracker{};
  emel::error::type initialize_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto initialize_request =
...
TEST_CASE("generator_gemma4_generator_initializes_with_mixed_sliding_and_full_"
          "attention_widths") {
  auto fixture = std::make_unique<generator_fixture>(
...
TEST_CASE("generator_generate_f32_fixture_does_not_claim_quantized_optimized_"
          "dispatch") {
  auto fixture = std::make_unique<generator_fixture>();
  callback_tracker initialize_tracker{};
  emel::error::type initialize_error =
...
TEST_CASE(
    "generator_quantized_path_audit_classifies_canonical_stage_families") {
  ...
  const auto &feed_forward_norm = find_stage_audit(
      audit,
      emel::model::generation::quantized_stage_family::feed_forward_norm);
...
TEST_CASE("generator_qwen3_quantized_path_audit_publishes_explicit_norm_stage_"
          "families") {
  auto prepared = std::make_unique<prepared_model>();
...
TEST_CASE(
    "generator_qwen3_quantized_path_audit_requires_attention_q_norm_tensor") {
  auto prepared = std::make_unique<prepared_model>();
...
TEST_CASE(
    "generator_qwen3_quantized_path_audit_requires_attention_k_norm_tensor") {
  auto prepared = std::make_unique<prepared_model>();
...
TEST_CASE("generator_quantized_path_audit_marks_unsupported_quantized_stage_no_"
          "claim") {
  auto prepared = std::make_unique<prepared_model>();
  build_prepared_model(*prepared);
...
TEST_CASE("generator_quantized_path_audit_marks_nonvector_f32_stage_disallowed_"
          "fallback") {
  auto prepared = std::make_unique<prepared_model>();
  build_prepared_model(*prepared);
...
TEST_CASE(
    "generator_lfm2_quantized_path_audit_accepts_hybrid_native_quantized_mix") {
  auto prepared = std::make_unique<prepared_model>();
  ...
  const auto &feed_forward_down = find_stage_audit(
      audit,
      emel::model::generation::quantized_stage_family::feed_forward_down);
...
TEST_CASE("generator_initialize_quantized_contract_fixture_reports_zero_"
          "disallowed_fallback_stages") {
  auto fixture = std::make_unique<generator_fixture>(
...
TEST_CASE("generator_generate_quantized_contract_fixture_preserves_zero_"
          "disallowed_fallback") {
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::quantized_contract);
  callback_tracker initialize_tracker{};
  ...
  if (host_is_aarch64()) {
    ...
    CHECK(diagnostics.optimized_q6_vector_prepared_q8_rhs_dispatch_calls > 0u);
    CHECK(diagnostics.optimized_q6_vector_prepared_q8_rhs_i8mm_dispatch_calls >
          0u);
#else
    CHECK(diagnostics.optimized_q6_vector_prepared_q8_rhs_i8mm_dispatch_calls ==
          0u);
...
TEST_CASE("generator_generate_quantized_contract_fixture_supports_explicit_"
          "preselected_argmax_mode") {
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::quantized_contract);
  callback_tracker initialize_tracker{};
  emel::error::type initialize_error =
  ...
  const auto initialize_request = fixture->make_initialize(
      initialize_tracker, &initialize_error,
      emel::text::generator::selection_mode::preselected_argmax);
...
#if defined(__aarch64__) && defined(__ARM_NEON) &&                             \
    defined(__ARM_FEATURE_MATMUL_INT8)
    CHECK(diagnostics.optimized_q6_vector_packed_q8_rhs_argmax_dispatch_calls ==
          0u);
    CHECK(diagnostics
              .optimized_q6_vector_prepared_q8_rhs_argmax_i8mm_dispatch_calls ==
          0u);
    CHECK(
        diagnostics.optimized_q6_vector_q8_argmax_prepared_i8mm_dispatch_calls >
        0u);
...
TEST_CASE("generator_generate_quantized_contract_fixture_allows_repeated_"
          "explicit_preselected_argmax_generates") {
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::quantized_contract);
  callback_tracker initialize_tracker{};
  emel::error::type initialize_error =
  ...
  const auto first_request = fixture->make_generate(
      first_tracker, first_output.data(), first_output.size(),
  ...
  const auto second_request = fixture->make_generate(
      second_tracker, second_output.data(), second_output.size(),
...
TEST_CASE("generator_generate_quantized_contract_fixture_allows_repeated_multi_"
          "token_explicit_preselected_argmax_generates") {
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::quantized_contract);
  callback_tracker initialize_tracker{};
  emel::error::type initialize_error =
  ...
  const auto make_long_generate =
      [](callback_tracker &tracker, std::array<char, 256> &output,
         size_t &output_length, emel::error::type *error_out) {
        emel::text::generator::event::generate request{
            std::span<const emel::text::formatter::chat_message>{
  ...
  emel::error::type first_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto first_request = make_long_generate(
      first_tracker, first_output, first_output_length, &first_error);
  ...
  emel::error::type second_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto second_request = make_long_generate(
      second_tracker, second_output, second_output_length, &second_error);
...
TEST_CASE("generator structured message request pins the phase 4 contract") {
  ...
  CHECK(generate_request.messages.size() ==
        generator_fixture::k_phase_4_messages.size());
  CHECK(generate_request.messages[0].role ==
        generator_fixture::k_phase_4_messages[0].role);
  ...
  CHECK(generate_tracker.tokens_generated ==
        generator_fixture::k_phase_4_max_tokens);
  CHECK(generate_tracker.output_length == 5);
...
TEST_CASE(
    "generator structured message request reaches the conditioner formatter") {
  static constexpr std::array<emel::text::formatter::chat_message, 2>
      k_messages = {
          emel::text::formatter::chat_message{
              .role = "system",
              .content = "policy",
  ...
  auto fixture = std::make_unique<generator_fixture>(
      generator_fixture::model_variant::canonical, &formatter_ctx,
...
TEST_CASE("generator_generate_multiple_tokens_and_resets_sequence_on_reuse") {
  ...
  emel::error::type first_error =
      emel::error::cast(emel::text::generator::error::backend);
  auto first_request = fixture->make_generate(
      first_tracker, first_output.data(), first_output.size(),
      first_output_length, &first_error);
  ...
  CHECK(std::string_view(first_output.data(), first_output_length) ==
        "worldworld");
  ...
  CHECK(std::string_view(second_output.data(), second_output_length) ==
        "world");
}
...
TEST_CASE("generator_resets_renderer_stop_state_when_reusing_a_sequence") {
  ...
  emel::error::type second_error =
      emel::error::cast(emel::text::generator::error::backend);
  auto second_request = fixture->make_generate(
      second_tracker, second_output.data(), second_output.size(),
      second_output_length, &second_error);
...
TEST_CASE("generator_reinitialize_clears_lifecycle_publish_state_before_next_"
          "generate") {
  auto fixture = std::make_unique<generator_fixture>();
  callback_tracker first_initialize_tracker{};
  emel::error::type first_initialize_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto first_initialize = fixture->make_initialize(
      first_initialize_tracker, &first_initialize_error);
  REQUIRE(fixture->generator->process_event(first_initialize));

  callback_tracker first_generate_tracker{};
  std::array<char, 32> first_output = {};
  size_t first_output_length = 0;
  emel::error::type first_generate_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto first_generate = fixture->make_generate(
      first_generate_tracker, first_output.data(), first_output.size(),
      first_output_length, &first_generate_error);
  REQUIRE(fixture->generator->process_event(first_generate));
  REQUIRE(first_generate_error ==
          emel::error::cast(emel::text::generator::error::none));

  callback_tracker second_initialize_tracker{};
  emel::error::type second_initialize_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto second_initialize = fixture->make_initialize(
      second_initialize_tracker, &second_initialize_error);
  REQUIRE(fixture->generator->process_event(second_initialize));
  REQUIRE(second_initialize_error ==
          emel::error::cast(emel::text::generator::error::none));

  callback_tracker second_generate_tracker{};
  std::array<char, 32> second_output = {};
  size_t second_output_length = 0;
  emel::error::type second_generate_error =
      emel::error::cast(emel::text::generator::error::backend);
  const auto second_generate = fixture->make_generate(
      second_generate_tracker, second_output.data(), second_output.size(),
      second_output_length, &second_generate_error);

  CHECK(fixture->generator->process_event(second_generate));
  CHECK(second_generate_error ==
        emel::error::cast(emel::text::generator::error::none));
...
TEST_CASE("generator_docs_table_uses_typed_completion_event_names") {
  ...
  for_each_type(transitions{}, [&]<class transition_t>() {
    ...
    const std::string event_name =
        emel::docs::detail::table_event_name<event>();
...
TEST_CASE(
    "generator_sm_models_explicit_prefill_boundary_and_decode_compute_states") {
  using machine_t = stateforward::sml::sm<emel::text::generator::model>;
  using states = typename machine_t::states;

  CHECK(emel::detail::type_list_contains<emel::text::generator::prefill_running,
                                         states>::value);
  CHECK(emel::detail::type_list_contains<
        emel::text::generator::prefill_result_decision, states>::value);
  ...
  CHECK_FALSE(emel::detail::type_list_contains<
              ...
              states>::value);
  CHECK(emel::detail::type_list_contains<
        emel::text::generator::decode_compute_flash, states>::value);
  CHECK(emel::detail::type_list_contains<
        emel::text::generator::decode_compute_nonflash, states>::value);
}
...
TEST_CASE("generator_sm_models_explicit_initializer_boundary") {
  ...
  CHECK(emel::detail::type_list_contains<emel::text::generator::initializing,
                                         states>::value);
  CHECK(emel::detail::type_list_contains<
        emel::text::generator::initializer_result_decision, states>::value);
  CHECK_FALSE(emel::detail::type_list_contains<
...
TEST_CASE("generator_initialize_public_wrapper_has_no_runtime_branch") {
  std::ifstream stream(repo_root() / "src" / "emel" / "text" / "generator" /
                       "sm.hpp");
  REQUIRE(stream.good());
  ...
  const auto end =
      source.find("bool process_event(const event::generate &", start);
...
TEST_CASE("generator_route_guards_do_not_delegate_behavior_selection_to_detail_"
          "helpers") {
  ...
  const std::string parent_guards = read_source(
      repo_root() / "src" / "emel" / "text" / "generator" / "guards.hpp");
...
TEST_CASE("generator_scalar_kernel_route_choice_stays_in_state_machines") {
  ...
  const std::string detail_source = read_source(
      repo_root() / "src" / "emel" / "text" / "generator" / "detail.hpp");
  ...
  CHECK(detail_source.find("prefill_chunk8_q8_k_backend_ready(") ==
        std::string::npos);
  CHECK(detail_source.find("prefill_chunk4_backend_ready<") ==
        std::string::npos);
  CHECK(detail_source.find("chunk4_matmul_backend_ready(") ==
        std::string::npos);
  CHECK(detail_source.find("run_kernel_mode(") == std::string::npos);
  CHECK(detail_source.find("run_kernel_mode_preselected_argmax(") ==
        std::string::npos);
  CHECK(detail_source.find("plan->kind == step_kind::prefill ?") ==
        std::string::npos);
  CHECK(detail_source.find("phase_lifecycle(") == std::string::npos);

  const auto guarded_validate_sig =
      detail_source.find("validate_guarded_compute(");
  REQUIRE(guarded_validate_sig != std::string::npos);
  const auto guarded_validate_start =
      detail_source.find('{', guarded_validate_sig);
  REQUIRE(guarded_validate_start != std::string::npos);
  const auto guarded_validate_end = detail_source.find(
      "validate_guarded_preselected_argmax(", guarded_validate_start);
  ...
  const auto guarded_bind_end = detail_source.find(
      "template <emel::text::generator::attention_mode", guarded_bind_start);
  REQUIRE(guarded_bind_end != std::string::npos);
  const auto guarded_bind_callback = detail_source.substr(
      guarded_bind_start + 1, guarded_bind_end - guarded_bind_start - 1);
  const std::array<std::string_view, 7> forbidden_bind_callback_patterns{
      "request_plan(", "check_backend(",  "== nullptr",   "expected_outputs",
      "*err_out =",    "k_error_invalid", "return false",
  };
  ...
  const auto guarded_extract_sig =
      detail_source.find("extract_guarded_outputs(");
  REQUIRE(guarded_extract_sig != std::string::npos);
  const auto guarded_extract_start =
      detail_source.find('{', guarded_extract_sig);
  REQUIRE(guarded_extract_start != std::string::npos);
  const auto guarded_extract_end = detail_source.find(
      "extract_guarded_preselected_argmax(", guarded_extract_start);
  ...
  const auto audited_wrappers =
      detail_source.substr(audited_start, audited_end - audited_start);
  ...
  auto matmul_start =
      detail_source.find("inline bool matmul_vector(native_backend");
  REQUIRE(matmul_start != std::string::npos);
  matmul_start = detail_source.find("inline bool matmul_vector(native_backend",
                                    matmul_start + 1u);
  REQUIRE(matmul_start != std::string::npos);
  const auto matmul_end =
      detail_source.find("inline bool matmul_vector_argmax", matmul_start);
  REQUIRE(matmul_end != std::string::npos);
  const auto matmul_body =
      detail_source.substr(matmul_start, matmul_end - matmul_start);
  CHECK(matmul_body.find("packed_q8_0_input_path_supported") ==
        std::string::npos);
  ...
  const auto native_matmul_body = detail_source.substr(
      native_matmul_start, native_matmul_end - native_matmul_start);
  CHECK(native_matmul_body.find("packed_q8_0_input_path_supported") ==
        std::string::npos);
  CHECK(native_matmul_body.find("q8_input_path_supported") ==
        std::string::npos);
  CHECK(native_matmul_body.find("matmul_vector_packed_q8_0(") ==
        std::string::npos);
  CHECK(native_matmul_body.find("matmul_vector_q8_k(") == std::string::npos);

  const std::string parent_actions = read_source(
      repo_root() / "src" / "emel" / "text" / "generator" / "actions.hpp");
  CHECK(parent_actions.find("detail::validate,") == std::string::npos);
  CHECK(parent_actions.find("detail::validate_preselected_argmax") ==
        std::string::npos);
  ...
  CHECK(parent_actions.find("detail::extract_preselected_argmax") ==
        std::string::npos);
  CHECK(parent_actions.find("detail::validate_guarded_compute") !=
        std::string::npos);
  CHECK(parent_actions.find("detail::validate_guarded_preselected_argmax") !=
        std::string::npos);
  CHECK(parent_actions.find("detail::bind_guarded_inputs") !=
        std::string::npos);
  CHECK(parent_actions.find("detail::extract_guarded_outputs") !=
        std::string::npos);
  ...
  CHECK(parent_actions.find("detail::run_kernel_nonflash>") ==
        std::string::npos);
  CHECK(parent_actions.find("detail::run_kernel_flash_preselected_argmax>") ==
        std::string::npos);
  CHECK(
      parent_actions.find("detail::run_kernel_nonflash_preselected_argmax>") ==
      std::string::npos);

  const std::string parent_sm = read_source(repo_root() / "src" / "emel" /
                                            "text" / "generator" / "sm.hpp");
  ...
  CHECK(parent_sm.find("guard::guard_decode_compute_backend_unavailable{}") !=
        std::string::npos);
  CHECK(parent_sm.find(
            "guard::guard_decode_materialized_scalar_kernel_ready{}") !=
        std::string::npos);
  CHECK(
      parent_sm.find("guard::guard_decode_preselected_argmax_kernel_ready{}") !=
      std::string::npos);
  ...
  CHECK(prefill_actions.find("detail::run_kernel_nonflash>") ==
        std::string::npos);
  CHECK(prefill_actions.find("detail::run_kernel_flash_preselected_argmax>") ==
        std::string::npos);
  CHECK(
      prefill_actions.find("detail::run_kernel_nonflash_preselected_argmax>") ==
      std::string::npos);

  const std::string prefill_sm =
      read_source(repo_root() / "src" / "emel" / "text" / "generator" /
                  "prefill" / "sm.hpp");
  CHECK(prefill_sm.find("guard::guard_compute_invalid_request{}") !=
        std::string::npos);
  CHECK(prefill_sm.find("guard::guard_compute_backend_unavailable{}") !=
        std::string::npos);
  CHECK(prefill_sm.find(
            "guard::guard_materialized_logits_with_scalar_kernel_ready{}") !=
        std::string::npos);
  CHECK(prefill_sm.find(
            "guard::guard_preselected_argmax_with_scalar_kernel_ready{}") !=
        std::string::npos);
}
...
TEST_CASE("generator_layer_route_actor_keeps_residual_choice_explicit") {
  ...
  CHECK(layer_sm.find("guard::guard_normalized_failed{}") != std::string::npos);
  ...
  CHECK(layer_sm.find("sml::completion<event::scalar_run>") !=
        std::string::npos);
  CHECK(layer_sm.find("sml::completion<event::chunk4_run>") !=
        std::string::npos);
  CHECK(layer_sm.find("sml::completion<event::chunk8_run>") !=
        std::string::npos);
  ...
  CHECK(layer_actions.find("detail::block_uses_attention") ==
        std::string::npos);
...
TEST_CASE(
    "generator_generic_headers_do_not_own_model_family_detail_contracts") {
  const auto read_source = [](const std::filesystem::path &path) {
    std::ifstream stream(path);
    REQUIRE(stream.good());
    return std::string((std::istreambuf_iterator<char>(stream)),
  ...
  const std::array<std::string_view, 10> forbidden_patterns{
      "emel/model/generation/detail", "emel/model/llama/detail",
      "emel/model/qwen3/detail",      "emel/model/gemma4/detail",
      "emel/model/lfm2/detail",       "model::generation::detail",
      "model::llama::detail",         "model::qwen3::detail",
      "model::gemma4::detail",        "model::lfm2::detail",
  };
...
TEST_CASE("docs_detail_shortens_lambda_type_names_for_mermaid") {
  ...
  CHECK(shorten_type_name("lambda at /tmp/path/my_action.cpp:42:7>") ==
        "lambda_my_action_42_7");
  CHECK(shorten_type_name("lambda at my_action.cpp:42>") ==
        "lambda_my_action_42");
...
TEST_CASE("docs_detail_table_event_name_supports_non_completion_event") {
  const auto event_name = emel::docs::detail::table_event_name<
