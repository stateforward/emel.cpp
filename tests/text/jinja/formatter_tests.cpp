...
auto parse_template(const std::string & text, emel::text::jinja::program & program) {
  ...
  emel::text::jinja::lexer_result lex_res = lex.tokenize(text);
...
render_result render_template(const std::string & templ,
  ...
  result.done = machine.is(boost::sml::state<emel::text::jinja::formatter::done>);
  return result;
}
...
emel::text::jinja::value make_none() {
  emel::text::jinja::value out;
  out.type = emel::text::jinja::value_type::none;
  return out;
}

emel::text::jinja::value make_float(double v) {
  emel::text::jinja::value out;
...
emel::text::jinja::value make_object(emel::text::jinja::object_entry * entries, size_t count) {
  emel::text::jinja::value out;
...
TEST_CASE("jinja_renderer_starts_initialized") {
  emel::text::jinja::formatter::action::context ctx{};
  emel::text::jinja::formatter::sm machine{ctx};
  CHECK(machine.is(boost::sml::state<emel::text::jinja::formatter::initialized>));
}
...
TEST_CASE("jinja_renderer_renders_simple_template") {
  ...
  std::array<emel::text::jinja::object_entry, 1> entries = {};
...
TEST_CASE("jinja_renderer_supports_set_and_member_access") {
  ...
  std::array<emel::text::jinja::object_entry, 1> obj_entries = {};
  obj_entries[0].key = make_string("key");
  obj_entries[0].val = make_string("OK");
  emel::text::jinja::value obj_val = make_object(obj_entries.data(), obj_entries.size());
...
TEST_CASE("jinja_renderer_invalid_request_errors") {
  ...
  CHECK(machine.is(boost::sml::state<emel::text::jinja::formatter::errored>));
...
TEST_CASE("jinja_renderer_if_and_select") {
  std::array<emel::text::jinja::object_entry, 2> entries = {};
  entries[0].key = make_string("name");
...
TEST_CASE("jinja_renderer_loops_and_members") {
  ...
  std::array<emel::text::jinja::value, 3> arr = {make_int(7), make_int(8), make_int(9)};
  emel::text::jinja::value arr_val = make_array(arr.data(), arr.size());

  std::array<emel::text::jinja::object_entry, 4> entries = {};
  entries[0].key = make_string("items");
  entries[0].val = items_val;
...
TEST_CASE("jinja_renderer_dispatch_callbacks") {
  ...
  struct tracker {
    ...
    bool on_error(const emel::text::jinja::events::rendering_error &) {
      error = true;
      return true;
    }
  };
...
TEST_CASE("jinja_renderer_unexpected_event_sets_error") {
  ...
  CHECK(machine.is(boost::sml::state<emel::text::jinja::formatter::unexpected>));
...
TEST_CASE("jinja_renderer_member_and_set_variants") {
  ...
  std::array<emel::text::jinja::value, 3> arr_vals = {make_int(1), make_int(2), make_int(3)};
  emel::text::jinja::value arr_val = make_array(arr_vals.data(), arr_vals.size());
...
TEST_CASE("jinja_renderer_tests_and_memberships") {
  std::array<emel::text::jinja::value, 2> arr_items = {make_int(1), make_int(2)};
  emel::text::jinja::value arr_val = make_array(arr_items.data(), arr_items.size());
  ...
  std::array<emel::text::jinja::object_entry, 8> entries = {};
  entries[0].key = make_string("name");
  entries[0].val = make_string("bob");
...
TEST_CASE("jinja_renderer_errors_on_slice_step_zero") {
  std::array<emel::text::jinja::value, 2> arr_vals = {make_int(1), make_int(2)};
  emel::text::jinja::value arr_val = make_array(arr_vals.data(), arr_vals.size());
...
TEST_CASE("jinja_renderer_detail_unary_paths") {
...

  {
    ...
    formatter::detail::render_io io{};
    char buffer[16] = {};
    formatter::detail::init_writer(io, buffer, sizeof(buffer));
    ...
    auto value = formatter::detail::eval_expr(ctx, &expr, nullptr, io);
...
TEST_CASE("jinja_renderer_detail_limit_paths") {
...

  {
    ...
    CHECK_FALSE(formatter::detail::ensure_steps(ctx, 0));
    CHECK(ctx.phase_error == EMEL_ERR_BACKEND);
  }

  {
    formatter::action::context ctx{};
    formatter::detail::render_io io{};
    io.depth = formatter::action::k_max_capture_depth;
    CHECK_FALSE(formatter::detail::begin_capture(ctx, io));
    CHECK(ctx.phase_error == EMEL_ERR_BACKEND);

    ctx.phase_error = EMEL_OK;
    io.depth = 0;
    emel::text::jinja::value captured{};
    CHECK_FALSE(formatter::detail::end_capture(ctx, io, captured));
    CHECK(ctx.phase_error == EMEL_ERR_BACKEND);
  }

  {
    formatter::action::context ctx{};
    ctx.string_buffer_used = formatter::action::k_max_string_bytes;
    auto stored = formatter::detail::store_string(ctx, "x");
    CHECK(stored.empty());
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }
  ...
  {
    formatter::action::context ctx{};
    ctx.array_items_used = formatter::action::k_max_array_items - 1;
    std::array<value, 2> items = {value{}, value{}};
    auto array_val = formatter::detail::make_array(ctx, items.data(), items.size());
  ...
  {
    formatter::action::context ctx{};
    ctx.object_entries_used = formatter::action::k_max_object_entries - 1;
    std::array<object_entry, 2> entries = {};
    entries[0].key = value{};
    entries[0].val = value{};
    entries[1].key = value{};
    entries[1].val = value{};
    auto obj_val = formatter::detail::make_object(ctx, entries.data(), entries.size(), entries.size(), false);
  ...
  {
    formatter::action::context ctx{};
    ctx.scope_count = formatter::action::k_max_scopes;
    CHECK_FALSE(formatter::detail::push_scope(ctx));
    CHECK(ctx.phase_error == EMEL_ERR_BACKEND);

    ctx = formatter::action::context{};
    ctx.object_entries_used = formatter::action::k_max_object_entries - formatter::action::k_scope_capacity + 1;
    CHECK_FALSE(formatter::detail::push_scope(ctx));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }

  {
    formatter::detail::call_args args{};
    args.pos_count = formatter::detail::k_max_call_args;
    CHECK_FALSE(formatter::detail::add_pos(args, value{}));
    args.kw_count = formatter::detail::k_max_call_args;
    CHECK_FALSE(formatter::detail::add_kw(args, "k", value{}));
  }

  {
    ...
    args.pos_count = formatter::detail::k_max_call_args;
    value out{};
    CHECK_FALSE(formatter::detail::filter_length(ctx, value{}, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }
}
...
TEST_CASE("jinja_renderer_detail_call_statement_errors") {
...

  {
    ...
    CHECK_FALSE(formatter::detail::render_call_statement(ctx, &stmt, nullptr, io));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }
...
TEST_CASE("jinja_renderer_detail_helpers_part1") {
  using namespace emel::text::jinja;

  {
    ...
    CHECK(formatter::detail::store_string(ctx, "").empty());
    ...
    formatter::detail::make_object(ctx, nullptr, 0, 0, false);
    formatter::detail::pop_scope(ctx);
  }

  {
    ...
    CHECK_FALSE(formatter::detail::value_is_truthy(v_none));
    CHECK(formatter::detail::value_is_truthy(v_bool));
    CHECK(formatter::detail::value_is_truthy(v_int));
    CHECK(formatter::detail::value_is_truthy(v_float));
    CHECK(formatter::detail::value_is_truthy(v_string));
    CHECK(formatter::detail::value_is_truthy(v_array));
    CHECK(formatter::detail::value_is_truthy(v_object));
    CHECK(formatter::detail::value_is_truthy(v_func));

    value v_float_one{};
    v_float_one.type = value_type::floating;
    v_float_one.float_v = 1.0;
    v_float_one.int_v = 1;
    CHECK(formatter::detail::value_equal(v_int, v_float_one));

    value v_array_short = v_array;
    v_array_short.array_v.count = 1;
    CHECK_FALSE(formatter::detail::value_equal(v_array, v_array_short));
    ...
    CHECK_FALSE(formatter::detail::value_equal(v_object, v_object_other));

    value v_func_other{};
    v_func_other.type = value_type::function;
    v_func_other.func_v.data = &v_object;
    CHECK_FALSE(formatter::detail::value_equal(v_func, v_func_other));
  }

  {
    ...
    CHECK(formatter::detail::value_to_string(ctx, v_undef).empty());

    value v_arr{};
    v_arr.type = value_type::array;
    CHECK(formatter::detail::value_to_string(ctx, v_arr).empty());

    formatter::detail::call_args args{};
    CHECK(formatter::detail::find_kw(args, "missing") == nullptr);
    CHECK(formatter::detail::get_pos(args, 1) == nullptr);
  }
}

TEST_CASE("jinja_renderer_detail_helpers_part2") {
  using namespace emel::text::jinja;

  {
    ...
    formatter::detail::call_args args{};
    value out{};
    CHECK_FALSE(formatter::detail::builtin_default(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);

    ctx = formatter::action::context{};
    CHECK_FALSE(formatter::detail::builtin_length(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);

    ctx = formatter::action::context{};
    CHECK_FALSE(formatter::detail::builtin_range(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }
...

  {
    ...
    args.pos_count = 0;
    CHECK_FALSE(formatter::detail::builtin_upper(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);

    ctx = formatter::action::context{};
    CHECK_FALSE(formatter::detail::builtin_lower(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);

    ctx = formatter::action::context{};
    CHECK_FALSE(formatter::detail::builtin_trim(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);

    ctx = formatter::action::context{};
    CHECK_FALSE(formatter::detail::builtin_replace(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);

    ctx = formatter::action::context{};
    CHECK_FALSE(formatter::detail::builtin_join(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }

  {
    ...
    CHECK(formatter::detail::write_value(ctx, io, arr));
    CHECK(formatter::detail::write_value(ctx, io, obj));
  }
}

TEST_CASE("jinja_renderer_detail_helpers_part3") {
  using namespace emel::text::jinja;

  {
    ...
    CHECK(formatter::detail::find_kw(args, "k") != nullptr);

    args.pos_count = 1;
    args.pos[0].type = value_type::integer;
    args.pos[0].int_v = 3;
    CHECK(formatter::detail::get_pos(args, 0) != nullptr);

    CHECK(formatter::detail::builtin_namespace(ctx, args, out));
...

  {
    ...
    CHECK(formatter::detail::builtin_upper(ctx, args, out));

    ctx = formatter::action::context{};
    CHECK(formatter::detail::builtin_lower(ctx, args, out));

    ctx = formatter::action::context{};
    CHECK(formatter::detail::builtin_trim(ctx, args, out));
    ...
    CHECK(formatter::detail::builtin_replace(ctx, args, out));

    ctx = formatter::action::context{};
    args.pos_count = 1;
    args.pos[0] = out;
    CHECK(formatter::detail::builtin_join(ctx, args, out));
  }

  {
    ...
    args.pos[0].type = value_type::integer;
    args.pos[0].int_v = 3;
    CHECK(formatter::detail::builtin_range(ctx, args, out));
...

  {
    ...
    args.pos[0].int_v = 0;
    args.pos[1].type = value_type::integer;
    args.pos[1].int_v = static_cast<int64_t>(formatter::action::k_max_array_items + 1);
    CHECK_FALSE(formatter::detail::builtin_range(ctx, args, out));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }
}

TEST_CASE("jinja_renderer_detail_helpers_part4") {
  using namespace emel::text::jinja;

  {
    ...
    args.pos_count = 1;
    args.pos[0] = input;
    CHECK(formatter::detail::filter_default(ctx, input, args, out));

    ctx = formatter::action::context{};
    args.pos_count = 0;
    CHECK_FALSE(formatter::detail::filter_default(ctx, input, args, out));

    ctx = formatter::action::context{};
    args.pos_count = 0;
    CHECK(formatter::detail::filter_upper(ctx, input, args, out));

    ctx = formatter::action::context{};
    CHECK(formatter::detail::filter_lower(ctx, input, args, out));

    ctx = formatter::action::context{};
    CHECK(formatter::detail::filter_trim(ctx, input, args, out));

    ctx = formatter::action::context{};
    CHECK(formatter::detail::filter_length(ctx, input, args, out));

    ctx = formatter::action::context{};
    args.pos_count = 2;
    args.pos[0] = input;
    args.pos[1] = input;
    CHECK(formatter::detail::filter_replace(ctx, input, args, out));

    ctx = formatter::action::context{};
    args.pos_count = 0;
    CHECK(formatter::detail::filter_join(ctx, input, args, out));
  }

  {
    ...
    std::string too_long(formatter::action::k_max_string_bytes + 1, 'a');
    args.pos_count = 1;
    args.pos[0].type = value_type::string;
...
TEST_CASE("jinja_renderer_detail_helpers_part5") {
  using namespace emel::text::jinja;

  {
    ...
    std::string huge_sep(formatter::action::k_max_string_bytes + 1, 'b');
    value delim{};
    ...
    ctx = formatter::action::context{};
    std::string huge_part(formatter::action::k_max_string_bytes + 1, 'c');
...
TEST_CASE("jinja_renderer_detail_helpers_part6") {
  using namespace emel::text::jinja;

  {
    ...
    std::string huge_input(formatter::action::k_max_string_bytes + 1, 'a');
    args.pos_count = 3;
    args.pos[0].type = value_type::string;
...

  {
    ...
    CHECK_FALSE(formatter::detail::value_is_truthy(invalid));
  }

  {
    ...
    CHECK_FALSE(formatter::detail::set_object_value(ctx, obj, "k", v_val));
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }

  {
    ...
    CHECK_FALSE(formatter::detail::write_value(ctx, io, arr));
  }

  {
    ...
    args.kw[0].val.type = value_type::integer;
    args.kw[0].val.int_v = 1;
    CHECK(formatter::detail::find_kw(args, "b") == nullptr);
  }

  {
    ...
    CHECK(formatter::detail::builtin_length(ctx, args, out));
...
TEST_CASE("jinja_renderer_detail_value_equal_cases") {
  ...
  CHECK(formatter::detail::value_equal(undef1, undef2));
  ...
  b2.bool_v = false;
  CHECK_FALSE(formatter::detail::value_equal(b1, b2));
  ...
  f2.float_v = 1.5;
  CHECK(formatter::detail::value_equal(f1, f2));
  ...
  s1.string_v.view = "a";
  CHECK_FALSE(formatter::detail::value_equal(i1, s1));
  ...
  CHECK_FALSE(formatter::detail::value_equal(arr_a, arr_b));
  ...
  CHECK_FALSE(formatter::detail::value_equal(obj_a, obj_b));
  ...
  CHECK_FALSE(formatter::detail::value_equal(obj_c, obj_d));

  value obj_e = obj_a;
  value obj_f = obj_a;
  CHECK(formatter::detail::value_equal(obj_e, obj_f));
  ...
  CHECK(formatter::detail::value_equal(func_a, func_b));

  value invalid_a{};
  value invalid_b{};
  invalid_a.type = static_cast<value_type>(99);
  invalid_b.type = static_cast<value_type>(99);
  CHECK_FALSE(formatter::detail::value_equal(invalid_a, invalid_b));
}
...
TEST_CASE("jinja_renderer_actions_branch_coverage") {
  ...
  using emel::text::jinja::formatter::action::finalize_error;
  using emel::text::jinja::formatter::action::seed_program;
  using emel::text::jinja::formatter::action::write_pending_value;
...

  {
    ...
    ctx.statements = &stmts;
    ctx.callable_count = formatter::action::k_max_callables;
    eval_next_stmt(ctx);
    CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  }
  ...
  {
    auto done_cb = emel::callback<bool(const emel::text::jinja::events::rendering_done &)>::from<
        +[](const emel::text::jinja::events::rendering_done &) -> bool { return true; }>();
    auto error_cb = emel::callback<bool(const emel::text::jinja::events::rendering_error &)>::from<
        +[](const emel::text::jinja::events::rendering_error &) -> bool { return true; }>();
