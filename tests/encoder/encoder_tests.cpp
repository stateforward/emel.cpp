...
TEST_CASE("encoder_ugm_normalization_flags") {
  ...
  const std::string normalized =
    emel::encoder::detail::normalize_ugm(*builder.vocab, ctx, "  hello   world ");
...
TEST_CASE("encoder_assign_bpe_regex_variants") {
  ...
  builder.set_pre("gpt2");
  emel::encoder::detail::assign_bpe_regex(ctx, *builder.vocab);
...
TEST_CASE("encoder_detail_helpers") {
  ...
  const auto wpm_tokens = emel::encoder::detail::wpm_preprocess("Hello!");
  ...
  const auto normalized = emel::encoder::detail::normalize_ugm(*builder.vocab, ctx, "Hello");
  CHECK(!normalized.empty());
}
...
TEST_CASE("encoder_encode_impl_variants") {
  auto run_variant = [](
    ...
    const auto & setup_vocab) {
      ...
      auto result = emel::encoder::detail::encode_impl(ev, ctx);
      (void)result;
      CHECK(err == EMEL_OK);
    };
...
TEST_CASE("encoder_detail_encode_direct_calls") {
...

  {
    ...
    auto result = emel::encoder::detail::encode_bpe(ev_plain, ctx, *builder.vocab);
    (void)result;
    emel::encoder::event::encode ev_punct = ev;
    ev_punct.text = "hello, world!";
    auto result_punct = emel::encoder::detail::encode_bpe(ev_punct, ctx, *builder.vocab);
    (void)result_punct;
    emel::encoder::event::encode ev_empty = ev;
    ev_empty.text = "";
    auto result_empty = emel::encoder::detail::encode_bpe(ev_empty, ctx, *builder.vocab);
    (void)result_empty;
  }

  {
    ...
    auto result = emel::encoder::detail::encode_wpm(ev_wpm, ctx, *builder.vocab);
    (void)result;
    emel::encoder::event::encode ev_unknown = ev;
    ev_unknown.text = "xyzxyz";
    auto result_unknown = emel::encoder::detail::encode_wpm(ev_unknown, ctx, *builder.vocab);
    (void)result_unknown;
  }

  {
    ...
    auto result = emel::encoder::detail::encode_spm(ev_spm, ctx, *builder.vocab);
    (void)result;
  }

  {
    ...
    ev_ugm.text = "hello";
    auto result = emel::encoder::detail::encode_ugm(ev_ugm, ctx, *builder.vocab);
    (void)result;
  }

  {
    ...
    ctx.vocab = builder.vocab;
    CHECK(emel::encoder::detail::ensure_tables(ctx));
    auto result = emel::encoder::detail::encode_rwkv(ev, ctx, *builder.vocab);
    (void)result;
  }

  {
    ...
    ctx.vocab = builder.vocab;
    CHECK(emel::encoder::detail::ensure_tables(ctx));
    auto result = emel::encoder::detail::encode_plamo2(ev, ctx, *builder.vocab);
    (void)result;
  }

  {
    ...
    ctx.vocab = builder.vocab;
    CHECK(emel::encoder::detail::ensure_tables(ctx));
    auto result = emel::encoder::detail::encode_fallback(ev, ctx, *builder.vocab);
    (void)result;
  }
}
...
TEST_CASE("encoder_detail_branch_coverage") {
  ...
  uint32_t next = 0;
  CHECK(!emel::encoder::detail::xcda_next(ctx, 0, 0, next));
  uint32_t value = 0;
  CHECK(!emel::encoder::detail::xcda_value(ctx, 0, value));
  CHECK(emel::encoder::detail::apply_precompiled_charsmap(ctx, "abc") == "abc");
  ...
  builder.vocab->remove_extra_whitespaces = true;
  ctx.vocab = builder.vocab;
  const std::string normalized = emel::encoder::detail::normalize_ugm(*builder.vocab, ctx, "  a");
  CHECK(!normalized.empty());
...
TEST_CASE("encoder_encode_branch_cases") {
...

  {
    ...
    auto result = emel::encoder::detail::encode_bpe(ev, ctx, *builder.vocab);
    (void)result;
  }
...

  {
    ...
    ev_rwkv.text = "x";
    auto result = emel::encoder::detail::encode_rwkv(ev_rwkv, ctx, *builder.vocab);
    (void)result;
  }
