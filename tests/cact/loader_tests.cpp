...
void append_scalar(std::vector<uint8_t> & bytes, const value_type value) {
  for (size_t i = 0; i < sizeof(value_type); ++i) {
    bytes.push_back(static_cast<uint8_t>((static_cast<uint64_t>(value) >> (i * 8u)) & 0xffu));
  }
}
...
std::vector<uint8_t> make_valid_cact_file() {
  ...
  const uint64_t blob_offset =
      ((directory_offset + emel::cact::loader::constants::record_bytes + k_alignment - 1u) /
...
std::vector<uint8_t> make_bad_offset_cact_file() {
  ...
  const uint64_t bogus = (static_cast<uint64_t>(bytes.size()) + k_alignment) & ~static_cast<uint64_t>(k_alignment - 1u);
...
struct callback_state {
  ...
  emel::error::type probe_error = emel::error::cast(emel::cact::loader::error::none);
  emel::error::type bind_error = emel::error::cast(emel::cact::loader::error::none);
  emel::error::type parse_error = emel::error::cast(emel::cact::loader::error::none);
};
...
std::vector<fixture_record> read_directory_fixture(const std::filesystem::path & path) {
  ...
  while (std::getline(input, line)) {
    ...
    const int parsed = std::sscanf(
        line.c_str(), "%u,%u,%u,%u,%u,%u,%u,%llu,%llu,%u,%u",
        &record.index, &record.dtype, &record.ndim,
        &record.shape[0], &record.shape[1], &record.shape[2], &record.shape[3],
        reinterpret_cast<unsigned long long *>(&record.offset),
        reinterpret_cast<unsigned long long *>(&record.nbytes),
        &record.group, &record.bits);
    CHECK(parsed == 11);
...
TEST_CASE("cact loader probe rejects invalid request inputs") {
  ...
  CHECK(state.probe_error == emel::error::cast(emel::cact::loader::error::invalid_request));
  CHECK(machine.is(stateforward::sml::state<emel::cact::loader::state_errored>));
}
...
TEST_CASE("cact loader probe classifies malformed images") {
  SUBCASE("bad tag maps to model_invalid") {
    ...
    CHECK_FALSE(machine.process_event(probe));
    CHECK(state.probe_error == emel::error::cast(emel::cact::loader::error::model_invalid));
    CHECK(machine.is(stateforward::sml::state<emel::cact::loader::state_errored>));
  }
  ...
  SUBCASE("truncated directory maps to parse_failed") {
    ...
    callback_state state = {};
    callback_scope scope{state};
    const std::vector<uint8_t> file_bytes = make_truncated_directory_cact_file();
    emel::cact::loader::geometry geometry = {};
    const emel::cact::loader::event::probe probe{
      std::span<const uint8_t>{file_bytes},
      geometry,
    ...
    CHECK_FALSE(machine.process_event(probe));
    CHECK(state.probe_error == emel::error::cast(emel::cact::loader::error::parse_failed));
    CHECK(machine.is(stateforward::sml::state<emel::cact::loader::state_errored>));
  }
...
TEST_CASE("cact loader bind rejects undersized or unbound storage") {
  ...
  SUBCASE("empty storage span maps to invalid_request") {
    ...
    CHECK(state.bind_error == emel::error::cast(emel::cact::loader::error::invalid_request));
    CHECK(machine.is(stateforward::sml::state<emel::cact::loader::state_errored>));
  }
}
...
TEST_CASE("cact loader parse before bind is rejected") {
  ...
  CHECK(state.parse_error == emel::error::cast(emel::cact::loader::error::invalid_request));
  CHECK(machine.is(stateforward::sml::state<emel::cact::loader::state_errored>));
}

TEST_CASE("cact loader parses the pinned route-w4-qat fixture with directory parity") {
  const std::filesystem::path model_path = repo_relative("tests/models/route-w4-qat.cact");
