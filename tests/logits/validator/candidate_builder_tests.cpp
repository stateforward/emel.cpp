...
TEST_CASE("candidate builder action error paths") {
  emel::logits::validator::action::context ctx{};
  emel::logits::validator::event::build request{};
  int32_t count = 0;
  int32_t err = EMEL_OK;
  ...
  emel::logits::validator::action::begin_build(request, ctx);
  emel::logits::validator::action::run_validate(ctx);
  ...
  ctx.request = &request;
  emel::logits::validator::action::run_build_candidates(ctx);
  CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);

  request.candidate_scores_out = nullptr;
  ctx.candidate_count = 1;
  ctx.request = &request;
  emel::logits::validator::action::run_normalize_scores(ctx);
  CHECK(ctx.phase_error == EMEL_ERR_INVALID_ARGUMENT);
  ...
  emel::logits::validator::action::publish_error(ctx);
