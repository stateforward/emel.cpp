...
#include "emel/logits/sampler/token_selector/sm.hpp"

TEST_CASE("token selector argmax picks highest score") {
  emel::logits::sampler::token_selector::sm machine{};
  ...
  emel::logits::sampler::token_selector::event::select_token request{};
  request.candidate_ids = ids;
  request.candidate_scores = scores;
  request.candidate_count = 3;
  request.policy = emel::logits::sampler::token_selector::event::selection_policy::argmax;
...
TEST_CASE("token selector categorical uses random draw") {
  ...
  request.policy = emel::logits::sampler::token_selector::event::selection_policy::categorical;
...
TEST_CASE("token selector action error paths") {
  emel::logits::sampler::token_selector::action::context ctx{};
  emel::logits::sampler::token_selector::event::select_token request{};
  int32_t selected = -1;
  int32_t err = EMEL_OK;
  ...
  request.selected_token_out = &selected;
  request.error_out = &err;
  emel::logits::sampler::token_selector::action::begin_select_token(request, ctx);
  emel::logits::sampler::token_selector::action::run_validate(ctx);
  ...
  ctx.request = &request;
  emel::logits::sampler::token_selector::action::run_select(ctx);
  CHECK(ctx.phase_error == EMEL_ERR_BACKEND);

  ctx.request = &request;
  ctx.phase_error = EMEL_ERR_BACKEND;
  ctx.last_error = EMEL_OK;
  emel::logits::sampler::token_selector::action::publish_error(ctx);
