...
TEST_CASE("tokenizer_bpe_encode_utf8_branches") {
  char out[4] = {};
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x41u, nullptr, 0) == 0);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x41u, out, 0) == 0);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x41u, out, 1) == 1);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x7FFu, out, 1) == 0);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x7FFu, out, 2) == 2);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0xFFFFu, out, 2) == 0);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0xFFFFu, out, 3) == 3);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x10FFFFu, out, 3) == 0);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x10FFFFu, out, 4) == 4);
  CHECK(emel::text::tokenizer::bpe::detail::encode_utf8(0x110000u, out, 4) == 0);
}

TEST_CASE("tokenizer_bpe_decode_utf8_to_cpts_branches") {
  emel::text::tokenizer::bpe::detail::split_scratch scratch = {};
  CHECK(emel::text::tokenizer::bpe::detail::decode_utf8_to_cpts("A", scratch));
  CHECK(scratch.cpt_count == 1);

  const std::string two = "\xC3\xA9";
  CHECK(emel::text::tokenizer::bpe::detail::decode_utf8_to_cpts(two, scratch));
  CHECK(scratch.cpt_count == 1);

  const std::string three = "\xE2\x82\xAC";
  CHECK(emel::text::tokenizer::bpe::detail::decode_utf8_to_cpts(three, scratch));
  CHECK(scratch.cpt_count == 1);

  const std::string four = "\xF0\x9F\x98\x80";
  CHECK(emel::text::tokenizer::bpe::detail::decode_utf8_to_cpts(four, scratch));
  CHECK(scratch.cpt_count == 1);
}
...
TEST_CASE("tokenizer_bpe_push_offset_branches") {
  size_t out[2] = {};
  size_t out_count = 0;
  CHECK(emel::text::tokenizer::bpe::detail::push_offset(0, out, 2, out_count));
  CHECK(out_count == 0);
  CHECK(emel::text::tokenizer::bpe::detail::push_offset(1, out, 1, out_count));
  CHECK(out_count == 1);
  CHECK_FALSE(emel::text::tokenizer::bpe::detail::push_offset(1, out, 1, out_count));
}
...
TEST_CASE("tokenizer_bpe_split_gpt2_error_paths") {
  ...
  size_t out_count = 0;
  CHECK_FALSE(emel::text::tokenizer::bpe::detail::split_gpt2(
...
TEST_CASE("tokenizer_bpe_split_llama3_error_paths") {
  ...
  size_t offsets_out[4] = {};
  size_t out_count = 0;
  CHECK_FALSE(emel::text::tokenizer::bpe::detail::split_llama3(
      cpts.data(), cpts.size(), offsets_in_bad, 1, offsets_out, 4, out_count));
...
TEST_CASE("tokenizer_bpe_encode_bpe_segment_errors") {
  ...
  CHECK_FALSE(emel::text::tokenizer::bpe::detail::encode_bpe_segment(
...
TEST_CASE("tokenizer_bpe_split_fallback_overflow") {
  emel::text::tokenizer::bpe::detail::regex_list regex = {};
  regex.exprs[0] = "\\p{L}+";
  regex.count = 1;
  emel::text::tokenizer::bpe::detail::split_scratch scratch = {};
  scratch.encoded_size = scratch.encoded.size();
