...
## Archived Milestone: v1.19 Benchmark Tool Pluggable Runner Refactor
...
Reopened 2026-05-01 after a source-backed milestone audit rerun found the serialized process seam
was only test-covered, the benchmark actor-boundary source check did not cover maintained runner
sources, and Nyquist validation artifacts were missing for the v1.19 phase set.
...
- [x] **Phase 164: Benchmark Serialized Runner Process Seam Closure** - Wire the
  `bench_runner_request/v1` and `bench_runner_result/v1` payloads into a live runner process
  entrypoint instead of leaving the serialized seam test-only.
- [x] **Phase 165: Benchmark Actor Boundary Enforcement Closure** - Remove maintained benchmark
  runner reach-through into actor action/detail helpers or broaden source checks to fail on any
  remaining prohibited reach-through.
- [x] **Phase 166: v1.19 Nyquist Validation Backfill** - Produce validation artifacts and evidence
  for the reopened v1.19 phase set after the source-backed closure phases land.
...
### Phase 164: Benchmark Serialized Runner Process Seam Closure

**Goal:** Close `RUNNER-02` by making the `bench_runner_request/v1` and
`bench_runner_result/v1` serialized contract part of a live benchmark runner process entrypoint,
not only a unit-tested helper surface.

**Depends on:** Phase 163

**Requirements:** `RUNNER-02`

**Gap Closure:** Closes the audit finding `Serialized process runner flow`.

**Success Criteria**:
1. `tools/bench` exposes a production runner process entrypoint that reads
   `bench_runner_request/v1`, invokes the selected registered runner path, and writes
   `bench_runner_result/v1`.
2. The existing in-process `bench_runner` behavior, CLI arguments, exit codes, JSONL modes, and
   manifest operations remain stable unless explicitly documented.
3. Malformed payloads, unknown modes, unknown suites, unavailable runner processes, and nonzero
   runner exits fail closed with deterministic error output.
4. Tests prove the serialized entrypoint is exercised outside the existing round-trip helper tests.
5. The changed-file scoped quality gate passes for the touched benchmark runner files.

**Status:** Complete. Phase 164 added production serialized request/result process flags to
`bench_runner`, routed them through the registered runner dispatch path, and added live binary
tests for success plus fail-closed malformed, unknown-mode, unknown-suite, and conflicting JSONL
requests.

### Phase 165: Benchmark Actor Boundary Enforcement Closure

**Goal:** Close `LANE-02` by ensuring benchmark harnesses do not reach directly into actor
`actions.hpp`, `guards.hpp`, or `detail.hpp` helpers, and by broadening source checks so prohibited
reach-through cannot remain outside shared runner files.

**Depends on:** Phase 164

**Requirements:** `LANE-02`

**Gap Closure:** Closes the audit finding `Lane-isolation source enforcement flow`.

**Success Criteria**:
1. Maintained `tools/bench` runner sources drive state machines through public events,
   state-machine wrappers, public contracts, or kernel-owned execution surfaces instead of actor
   actions/guards/detail helpers.
2. Any remaining direct detail include is either removed or justified as kernel-owned/public
   diagnostic surface allowed by the project rules.
3. Source checks cover all relevant `tools/bench` `.cpp` and `.hpp` files for actor
   action/guard/detail include and namespace reach-through patterns, not only shared
   orchestrator files.
4. Existing generation and diarization JSONL behavior checks still pass.
5. The changed-file scoped quality gate passes for the touched benchmark runner files.

**Status:** Complete. Phase 165 removed prohibited maintained-runner actor reach-through from
Jinja, batch planner, and Sortformer benchmark sources, added a recursive source-backed scan over
maintained `tools/bench` `.cpp`/`.hpp` files, fixed scoped benchmark suite filtering for the
affected comparison builds, and passed full `bench_runner_tests` plus the changed-file scoped
quality gate.

### Phase 166: v1.19 Nyquist Validation Backfill

**Goal:** Backfill validation artifacts for the reopened v1.19 phase set so the milestone audit can
classify validation evidence as compliant instead of missing.

**Depends on:** Phase 165

**Requirements:** None directly; closes milestone artifact gaps.

**Gap Closure:** Closes the audit finding that phases 157 through 163 had no `*-VALIDATION.md`
artifacts and ensures reopened phases 164 through 166 have validation evidence before closeout.

**Success Criteria**:
1. Each v1.19 phase has a `*-VALIDATION.md` artifact or equivalent workflow-recognized validation
   record with executable verification commands.
2. Validation records include rule-compliance evidence and no unresolved manual blockers.
3. Validation evidence references live source/test/tool checks rather than artifact agreement
   alone for maintained-path claims.
4. `bench_runner_tests`, dependency-manifest freshness, relevant quality gates, and domain-boundary
   checks are recorded for the final reopened milestone state.
5. A rerun of `$gsd-audit-milestone` no longer reports missing Nyquist validation artifacts.

**Status:** Complete. Phase 166 added validation artifacts for phases 157 through 163, verified
the reopened phase set has Nyquist evidence, reran source-backed closeout checks, and updated the
v1.19 milestone audit report to passed.
