# Requirements Archive: v1.9 Liquid LFM2.5-1.2B Thinking ARM Slice
...
- [x] **FIX-02**: The repo documents one official `LFM2.5-1.2B-Thinking-Q4_K_M.gguf` fixture with
  checksum, source, stable maintained path under `tests/models/`, and download URL.
- [x] **META-01**: The maintained Liquid slice records executable model truth from official
...
- [x] **COND-03**: The maintained Liquid slice uses one explicit canonical request-conditioning
...
- [x] **RUN-03**: EMEL-owned `src/emel` runtime code truthfully accepts the canonical Liquid
  fixture as `lfm2` instead of rejecting it or aliasing it to existing `llama`/`qwen3` paths.
- [x] **RUN-04**: EMEL can initialize and generate on the canonical Liquid fixture through the
  maintained generator path in `src/emel`.
- [x] **RUN-05**: The maintained Liquid runtime explicitly handles the canonical slice's required
  `lfm2` metadata, tensor names, and hybrid block contract instead of pretending broad
  Llama-family compatibility.
- [x] **RUN-06**: The maintained Liquid runtime publishes a truthful quantized-path contract for
  the chosen official fixture and does not claim sibling Liquid quant support that is not proven.

### Verification

- [x] **PAR-02**: `tools/paritychecker --generation` proves EMEL against `llama.cpp` on the
  canonical Liquid slice using the same fixture and conditioning contract.
- [x] **VER-02**: Regression tests cover the canonical Liquid slice and protect the prior
  maintained Llama and Qwen anchors from accidental breakage while support is widened.

### Benchmarking

- [x] **BENCH-08**: `tools/bench` compare output, stored benchmark evidence, and generated docs
