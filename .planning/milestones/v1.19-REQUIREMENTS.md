# Requirements Archive: v1.19 Benchmark Tool Pluggable Runner Refactor

**Archived:** 2026-05-01
**Status:** SHIPPED after reopened source-backed gap closure on 2026-05-01

No root active requirements file exists after closeout. This archive is the historical v1.19
requirement source after the reopened closure phases passed audit.
...
runners"
**Status:** SHIPPED after Phases 164-166 closed the 2026-05-01 source-backed audit gaps
...
- [x] **RUNNER-02**: The orchestrator-to-runner contract supports a process-level runner seam with
  ...
  in-process runtime state. Phase 164 closed the audit finding by wiring serialized request/result
  payloads into a live `bench_runner` process entrypoint.
...
- [x] **LANE-02**: Tests or source checks fail if shared benchmark orchestration or runner code
  ...
  `actions.hpp`/`guards.hpp`/`detail.hpp` helper calls. Phase 165 closes the audit finding that
  the existing source check covered shared runner files but not maintained runner sources by
  removing prohibited maintained-runner actor reach-through and adding a recursive source-backed
  runner scan.
...
*Last updated: 2026-05-01 after v1.19 reopened closeout passed audit*
