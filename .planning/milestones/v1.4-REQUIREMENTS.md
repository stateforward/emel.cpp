# Requirements Archive: v1.4 Full Vectorized Quantized Kernels
...
- [x] **PORT-04**: The canonical Llama-68M ARM generation slice can execute the maintained
  `q2_K x q8_K` dot-product hot path through an EMEL-owned vectorized AArch64 kernel instead of
  the current scalar row helper.
- [x] **PORT-05**: The canonical Llama-68M ARM generation slice can execute the maintained
  `q3_K x q8_K` dot-product hot path through an EMEL-owned vectorized AArch64 kernel instead of
  the current scalar row helper.
- [x] **PORT-06**: The canonical Llama-68M ARM generation slice can execute the maintained
  `q6_K x q8_K` dot-product hot path through an EMEL-owned vectorized AArch64 kernel instead of
  the current scalar row helper.
- [x] **PORT-07**: The vectorized quantized kernels preserve zero-allocation hot-path behavior,
...
- [x] **ARCH-02**: The vectorized quantized kernel rollout remains a data-plane replacement
...
- [x] **PAR-04**: `tools/paritychecker --generation` keeps the maintained `1/10/100/1000` token
  checks and proves the canonical ARM workload exercised the vectorized quantized path.
- [x] **VER-03**: Kernel and regression tests cover `q2_K`, `q3_K`, and `q6_K` vectorized
...
- [x] **BENCH-08**: `tools/bench` runs the maintained canonical ARM compare workload through the
  vectorized quantized path and publishes enough attribution to distinguish it from the current
  scalar row-helper path.
- [x] **BENCH-09**: Maintained compare output republishes `1`, `10`, `100`, and `1000` token
...
| PORT-06 | Phase 19 | Complete |
| PORT-07 | Phase 19 | Complete |
| ARCH-02 | Phase 20 | Complete |
| PAR-04 | Phase 20 | Complete |
| VER-03 | Phase 20 | Complete |
| BENCH-08 | Phase 21 | Complete |
| BENCH-09 | Phase 21 | Complete |
...
*Last updated: 2026-03-25 after parity closure verification refresh*
